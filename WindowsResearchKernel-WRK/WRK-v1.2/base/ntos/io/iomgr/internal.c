...
/*
Routine Description:
    This routine is invoked to abort an I/O request.
    It is invoked during the rundown of a thread.
Arguments:
    Apc - Pointer to the kernel APC structure.
    This structure is contained within the I/O Request Packet (IRP) itself.
*/
{
    PAGED_CODE();

    IopCompleteRequest(Apc, &Apc->NormalRoutine, &Apc->NormalContext, &Apc->SystemArgument1, &Apc->SystemArgument2);// Invoke the normal special kernel APC routine.
}
...
/*
Routine Description:
    This routine is invoked to acquire the lock for a file object whenever there is contention and obtaining the fast lock for the file failed.
Arguments:
    ...
    Interrupted - A variable to receive a BOOLEAN that indicates whether or not the attempt to acquire the lock was interrupted by an alert or an APC.
Return Value:
...
*/
{
    ...
    *Interrupted = FALSE;// Assume that the function will not be interrupted by an alert or an APC while attempting to acquire the lock.
    ...
    for (;;) {
        ...
        // Wait for the event that indicates that the thread that currently owns the file object has released it.
        ...
        // Before returning, however, check the state of the ownership of the file object itself.
...
/*
Routine Description:
    This routine is invoked internally by those system services that attempt to allocate an IRP and fail.
    This routine cleans up the file object and any event object that has been references and releases any locks that were taken out.
Arguments:
...
*/
{
    ...
    // Release the synchronization semaphore if it is currently held and dereference the file object.
...
/*
Routine Description:
    This routine is invoked to allocate an IRP when there are no appropriate packets remaining on the look-aside list,
    and no memory was available from the general non-paged pool, and yet, the code path requiring the
    packet has no way of backing out and simply returning an error.
    There-fore, it must allocate an IRP.  Hence, this routine is called to allocate that packet.
Arguments:
...
*/
{
    ...
    // Attempt to allocate the IRP normally and failing that, wait a second and try again.
...
/*
Routine Description:
    This function is invoked when we need to do a hard error pop-up, but the Irp's originating thread is at APC level, ie. IoPageRead.
    We in a special purpose thread that will go away when the user responds to the pop-up.
Arguments:
...
/*
Routine Description:
    This routine is invoked when a synchronous I/O operation that is blocked in the I/O system needs to be canceled because the thread making the request has either been alerted because it is going away or because of a CTRL/C.
    This routine carefully attempts to work its way out of the current operation so that local events or other local data will not be accessed once the service being interrupted returns.
Arguments:
...
*/
{
    ...
    KeRaiseIrql(APC_LEVEL, &irql);// Begin by blocking special kernel APCs so that the request cannot complete.

    // Check the state of the event to determine whether or not the packet has already been completed.
    if (KeReadStateEvent(Event) == 0) {
        canceled = IoCancelIrp(Irp);// The packet has not been completed, so attempt to cancel it.
        KeLowerIrql(irql);
        if (canceled) {
            // The packet had a cancel routine, so it was canceled.
            // Loop, waiting for the packet to complete.
            ...
            // The packet did not have a cancel routine, so simply wait for the event to be set to the Signaled state.
            // This will save CPU time by not looping, since it is not known when the packet will actually complete.
            // Note, however, that the cancel flag is set in the packet, 
            // so should a driver examine the flag at some point in the future, it will immediately stop processing the request.
            (VOID)KeWaitForSingleObject(Event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
        }
    } else {
        // The packet has already been completed, so simply lower the IRQL back to its original value and exit.
        KeLowerIrql(irql);
    }
}
...
/*
Routine Description:
    This routine checks the validity of the specified get quota buffer to guarantee that its format is proper, no fields hang over, that it is not recursive, etc.
Arguments:
...
Return Value:
    The function value is STATUS_SUCCESS if the get quota buffer contains a valid, properly formed list, otherwise STATUS_QUOTA_LIST_INCONSISTENT.
*/
{
#define GET_OFFSET_LENGTH( CurrentSid, SidBase ) ( (ULONG) ((PCHAR) CurrentSid - (PCHAR) SidBase) )
    ...
    for (;;) {
        ...
        if (sids->NextEntryOffset) {
            ...
            // Ensure that the offset indicates that it is.
            // If it isn't, return an invalid parameter status.
            if (entrySize > (LONG)sids->NextEntryOffset || sids->NextEntryOffset & (sizeof(ULONG) - 1)) {
                *ErrorOffset = GET_OFFSET_LENGTH(sids, QuotaBuffer);
                return STATUS_QUOTA_LIST_INCONSISTENT;
            } else {
                // There is another entry in the buffer, so account for the size of the current entry in the length and get a pointer to the next entry.
    ...
    return STATUS_SUCCESS;// The format of the get quota buffer was correct, so simply return a success status code.
}
...
/*
Routine Description:
    This routine is invoked when the reference count on a device object transitions to a zero and the driver is mark for unload or device has been marked for delete.
    This means that it may be possible to actually unload the driver or delete the device object.
    If all of the devices have a reference count of zero, then the driver is actually unloaded.
    Note that in order to ensure that this routine is not invoked twice, at the same time, on two different processors,
    the I/O database spin lock is still held at this point.
Arguments:
    ...
    OnCleanStack - Indicates whether the current thread is in the middle a driver operation.
...
*/
{
    ...
    if (DeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_REMOVE_PENDING) {
        // Run some tests to determine if it is an appropriate time to notify PnP that all file objects in the attachment chain have gone away.
        ...
        // baseDeviceObject is a PDO, this is a PnP stack.
        // See if an IRP_MN_REMOVE_DEVICE is pending.
        ...
        // It is time to give PnP the notification it was waiting for.
        // We have to release the spinlock before doing so.
        KeReleaseQueuedSpinLock(LockQueueIoDatabaseLock, Irql);
    ...
    if (DeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_DELETE_PENDING) {
        ...
        // If another device is attached to this device, inform the former's driver that the device is being deleted.
        if (DeviceObject->AttachedDevice) {
            ...
            // Increment the device reference count so the detach routine does not recurse back to here.
            DeviceObject->ReferenceCount++;
            KeReleaseQueuedSpinLock(LockQueueIoDatabaseLock, Irql);
            if (fastIoDispatch && fastIoDispatch->SizeOfFastIoDispatch > FIELD_OFFSET(FAST_IO_DISPATCH, FastIoDetachDevice) && fastIoDispatch->FastIoDetachDevice) {
        ...
        IopInsertRemoveDevice(DeviceObject->DriverObject, DeviceObject, FALSE);// Remove this device object from the driver object's list.
        ObDereferenceObject(DeviceObject);// Finally, dereference the object so it is deleted.
        ...
        Irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);// Reacquire the spin lock make sure the unload routine does has not been called.
    ...
    // Scan the list of device objects for this driver, looking for a non-zero reference count.
    // If any reference count is non-zero, then the driver may not be unloaded.
    ...
    // If this is a base filesystem driver and we still have device objects skip the unload.
    ...
    // If the reference counts for all of the devices is zero, then this driver can now be unloaded.
    if (unload) {
...
/*
Routine Description:
    This routine executes as a special kernel APC routine in the context of the Modified Page Writer (MPW) system thread when an out-page operation has completed.
...
*/
{
    ...
    // If this I/O operation did not complete successfully through the dispatch routine of the driver, 
    // then drop everything on the floor now and return to the original call point in the MPW.
    if (!irp->PendingReturned && NT_ERROR(irp->IoStatus.Status)) {
        IoFreeIrp(irp);
        return;
    }

    *irp->UserIosb = irp->IoStatus;// Copy the I/O status from the IRP into the caller's I/O status block.
    ...
    apcRoutine(apcContext, ioStatus, 0);// Finally, invoke the MPW's APC routine.
}
...
/*
Routine Description:
    This routine executes as a special kernel APC routine in the context of the thread which originally requested the I/O operation which is now being completed.
    This routine performs the following tasks:
        o   A check is made to determine whether the specified request ended with an error status.
            If so, and the error code qualifies as one which should be reported to an error port, then an error port is looked for in the thread/process.
            If one exists, then this routine will attempt to set up an LPC to it.
            Otherwise, it will attempt to set up an LPC to the system error port.
        o   Copy buffers.
        o   Free MDLs.
        o   Copy I/O status.
...
#define SynchronousIo( Irp, FileObject ) (  \
    ...
    // Begin by getting the address of the I/O Request Packet.
    ...
    // Check to see whether there is any data in a system buffer which needs to be copied to the caller's buffer.
    // If so, copy the data and then free the system buffer if necessary.
    if (irp->Flags & IRP_BUFFERED_IO) {
        ...
        if (irp->Flags & IRP_INPUT_OPERATION && irp->IoStatus.Status != STATUS_VERIFY_REQUIRED && !NT_ERROR(irp->IoStatus.Status)) {
            ...
            {
                // An exception occurred while attempting to copy the system buffer contents to the caller's buffer.
                // Set a new I/O completion status.
                // If the status is a special one set by Mm then we need to return here and the operation will be retried in IoRetryIrpCompletions.
        ...
        if (irp->Flags & IRP_DEALLOCATE_BUFFER) {// Free the buffer if needed.
    ...
    // If there is an MDL (or MDLs) associated with this I/O request, Free it (them) here.
    // This is accomplished by walking the MDL list hanging off of the IRP and deallocating each MDL encountered.
    if (irp->MdlAddress) {
    ...
    if (!NT_ERROR(irp->IoStatus.Status) ||
        ...
        // If there is an I/O completion port object associated w/this request, save it here so that the file object can be dereferenced.
        ...
        // This is done using an exception handler in case the caller's virtual address space for the I/O status block was deleted or its protection was changed to readonly.
        // Note that if the I/O status block cannot be written, the error is simply ignored since there is no way to tell the caller that something went wrong.
        // This is, of course, by definition, since the I/O status block is where the caller will attempt to look for errors in the first place!
        status = STATUS_SUCCESS;
        try {
            // Since HasOverlappedIoCompleted and GetOverlappedResult only look at the Status field of the UserIosb to determine if the IRP has completed, 
            // the Information field must be written before the Status field.
#if defined(_WIN64)
        ...
        {
            // An exception was incurred attempting to write the caller's I/O status block.
            // Simply continue executing as if nothing ever happened since nothing can be done about it anyway.
            // If the status is a multiple fault status, this is a special status sent by the Memory manager.
            // Mark the IRP and return from this routine.
            // Mm will call us back later and we will retry this operation (IoRetryIrpCompletions)
            if (status == STATUS_MULTIPLE_FAULT_VIOLATION) {
                irp->Tail.Overlay.OriginalFileObject = fileObject;  /* Wiped out by APC  overlay */
                irp->Flags |= IRP_RETRY_IO_COMPLETION;
                return;
        ...
        // Determine whether the caller supplied an event that needs to be set to the Signaled state.
        // If so, then set it; otherwise, set the event in the file object to the Signaled state.

        // It is possible for the event to have been specified as a PKEVENT if this was an I/O operation hand-built for an FSP or an FSD, 
        // or some other types of operations such as synchronous I/O APIs.
        // In any of these cases, the event was not referenced since it is not an object manager event, so it should not be dereferenced.

        // Also, it is possible for there not to be a file object for this IRP.
        // This occurs when an FSP is doing I/O operations to a device driver on behalf of a process doing I/O to a file.
        // The file object cannot be dereferenced if this is the case.
        // If this operation was a create operation then the object should not be dereferenced either.
        // This is because the reference count must be one or it will go away for the caller (not much point in making an object that just got created go away).
        if (irp->UserEvent) {
        ...
        IopDequeueThreadIrp(irp);// Dequeue the packet from the thread's pending I/O request list.

        // If the caller requested an APC, queue it to the thread.
        // If not, then simply free the packet now.
#ifdef  _WIN64
        // For 64 bit systems clear the LSB field of the ApcRoutine that indicates whether the IOSB is a 32 bit IOSB or a 64 bit IOSB.
        irp->Overlay.AsynchronousParameters.UserApcRoutine = (PIO_APC_ROUTINE)((LONG_PTR)(irp->Overlay.AsynchronousParameters.UserApcRoutine) & ~1);
#endif
        ...
        if (irp->Overlay.AsynchronousParameters.UserApcRoutine) {
            ...
            // If there is a completion context associated w/this I/O operation, send the message to the port.
            // Tag completion packet as an Irp.
            irp->Tail.CompletionKey = key;
            irp->Tail.Overlay.PacketType = IopCompletionPacketIrp;
            KeInsertQueue((PKQUEUE)port, &irp->Tail.Overlay.ListEntry);
        } else {
            IoFreeIrp(irp);// Free the IRP now since it is no longer needed.
        }

        if (fileObject && !createOperation) {
            ObDereferenceObjectDeferDelete(fileObject);// Dereference the file object now.
        }
    } else {
        if (irp->PendingReturned && fileObject) {
            // This is an I/O operation that completed as an error for which a pending status was returned and the I/O operation is synchronous.
            // For this case, the I/O system is waiting on behalf of the caller.
            // If the reason that the I/O was synchronous is that the file object was opened for synchronous I/O, 
            // then the event associated with the file object is set to the signaled state.
            // If the I/O operation was synchronous because this is a synchronous API, then the event is set to the signaled state.

            // Note also that the status must be returned for both types of synchronous I/O.
            // If this is a synchronous API, then the I/O system supplies its own status block so it can simply be written;  
            // otherwise, the I/O system will obtain the final status from the file object itself.
            if (irp->Flags & IRP_SYNCHRONOUS_API) {
        ...
        // Note that everything is basically dropped on the floor without doing anything.
        // That is:
...
/*
Routine Description:
    This routine is invoked to connect to the user-mode link tracking service's LPC port.
    It makes a connection which establishes a handle to the port, and then creates a referenced object pointer to the port.
Arguments:
...
#define MESSAGE_SIZE    ( (2 * sizeof( FILE_VOLUMEID_WITH_TYPE )) + \
    ...
    ltp = (PLINK_TRACKING_PACKET)Parameter;// Begin by getting a pointer to the link tracking packet.
    ...
    if (!IopLinkTrackingServiceObject) {
        ...
        if (KeReadStateEvent(IopLinkTrackingServiceEvent)) {
            ...
            status = STATUS_OBJECT_NAME_NOT_FOUND;// The service has not been started so the port does not exist.
        }
    }
...
/*
Routine Description:
    This routine is invoked when the I/O requests for a thread are being cancelled,
    but there is a packet at the end of the thread's queue that has not been completed for such a long period of time that it has timed out.
    It is this routine's responsibility to try to disassociate that IRP with this thread.
*/
{
    KIRQL irql;
    KIRQL spIrql;
    ...
    KeRaiseIrql(APC_LEVEL, &irql);// Begin by ensuring that the packet has not already been removed from the thread's queue.

    thread = PsGetCurrentThread();

    // If there are no packets on the IRP list, then simply return now.
    // All of the packets have been fully completed, so the caller will also simply return to its caller.
    ...
    if (irp->CurrentLocation == irp->StackCount + 2) {
        // The request has just gone through enough of completion that queueing it to the thread is inevitable.
        // Simply release the lock and return.
        KeReleaseQueuedSpinLock(LockQueueIoCompletionLock, spIrql);
        KeLowerIrql(irql);
        return;
    }

    // The packet has been located, and it is not going through completion at this point.
    // Switch threads, so that it will not complete through this thread, remove the request from this thread's queue, and release the spinlock.
    // Final processing of the IRP will occur when I/O completion notices that there is no thread associated with the request.
    // It will essentially drop the I/O on the floor.

    // Also, while the request is still held, attempt to determine on which device object the operation is being performed.
    IopDeadIrp = irp;
    irp->Tail.Overlay.Thread = (PETHREAD)NULL;
    entry = RemoveHeadList(&thread->IrpList);

    // Initialize the thread entry. Otherwise the assertion in IoFreeIrp called via IopDeadIrp will fail.
    ...
    // If a device object could be identified then try to write to the event log about this device object.
    if (deviceObject) {
...
/*
Routine Description:
    This routine attempts to drop everything about the specified IRP on the floor.
Arguments:
    Irp - Supplies the I/O Request Packet to be completed to the bit bucket.
    FileObject - Supplies the file object for which the I/O Request Packet was bound.
*/
{
    PMDL mdl;
    PMDL nextMdl;
    ...
    IoFreeIrp(Irp);// Finally, free the IRP itself.
}
...
Routine Description:
    ...
    not the exception was due to an error that caused an in-page error status code exception to be raised.
    If so, then this routine changes the code in the exception record to the actual error code that was originally raised.
Arguments:
...
*/
{
    // Simply check for an in-page error status code and, if the conditions are right, replace it with the actual status code.
...
/*
Routine Description:
    This routine performs generalized cleanup for the I/O system services when an exception occurs during caller parameter processing.
    This routine performs the following steps:
        o   If a system buffer was allocated it is freed.
        o   If an MDL was allocated it is freed.
        o   The IRP is freed.
        o   If the file object is opened for synchronous I/O, the semaphore is released.
...
*/
{
    ...
    IoFreeIrp(Irp);// Free the I/O Request Packet.

    // Finally, release the synchronization semaphore if it is currently held, dereference the event if one was specified, 
    // free the kernel event if one was allocated, and dereference the file object.
    if (FileObject->Flags & FO_SYNCHRONOUS_IO) {
        IopReleaseFileObjectLock(FileObject);
    }
...
/*
Routine Description:
    This routine frees the specified I/O Request Packet and all of its Memory Descriptor Lists.
Arguments:
...
/*
Routine Description:
    Given a handle to a driver service list key in the registry,
    return the name that represents the Object Manager name space string that should be used to locate/create the driver object.
Arguments:
...
*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (!NT_SUCCESS(status) || !keyValueInformation->DataLength) {
            ...
            // Otherwise, this node is ill-formed.
            if (NT_SUCCESS(status)) {
                ExFreePool(keyValueInformation);
            }
        ...
        // Now determine whether the type of this entry is a driver or a file system.
        // Begin by assuming that it is a device driver.
        ...
        // Get the name of the key that is being used to describe this driver.
        // This will return just the last component of the name string, which can be used to formulate the name of the driver.
        ...
        // Allocate a buffer from pool that is large enough to contain the entire name string of the driver object.
    ...
    // Finally, simply return to the caller with the name filled in.
    // Note that the caller must free the buffer pointed to by the Buffer field of the Unicode string descriptor.
    return STATUS_SUCCESS;
}
...
/*
Routine Description:
    This routine is invoked to asynchronously obtain the name or other information of a file object when the file was opened for synchronous I/O,
    and the previous mode of the caller was kernel mode, and the query was done through the Object Manager.
    In this case, the situation is likely that the Lazy Writer has incurred a write error,
    and it is attempting to obtain the name of the file so that it can output a popup.
    In doing so, a deadlock can occur because another thread has locked the file object synchronous I/O lock.
    Hence, this routine obtains the name of the file w/o acquiring that lock.
Arguments:
...
*/
{
    ...
    ObReferenceObject(FileObject);// Reference the file object here so that no special checks need be made in I/O completion to determine whether or not to dereference the file object.

    // Initialize an event that will be used to synchronize the completion of the query operation.
    // Note that this is the only way to synchronize this since the file object itself cannot be used since it was opened for synchronous I/O and may be busy.
    KeInitializeEvent(&event, SynchronizationEvent, FALSE);

    deviceObject = IoGetRelatedDeviceObject(FileObject);// Get the address of the target device object.

    irp = IoAllocateIrp(deviceObject->StackSize, FALSE);// Allocate and initialize the I/O Request Packet (IRP) for this operation.
    if (!irp) {// An IRP could not be allocated.  Cleanup and return an appropriate error status code.
        ObDereferenceObject(FileObject);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    // Note that the setting of the special query name flag in the packet guarantees that the standard completion for a synchronous file object will not occur because this flag communicates to the I/O completion that it should not do so.
    irp->UserEvent = &event;
    ...
    status = IoCallDriver(deviceObject, irp);// Now simply invoke the driver at its dispatch entry with the IRP.
    if (status == STATUS_PENDING) {// Now get the final status of the operation once the request completes and return the length of the buffer written.
        (VOID)KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
...
*/
{
    ...
    // Check to see whether or not the device is mounted.
    // Note that the caller has probably already looked to see whether or not the device has a VPB
    // outside of owning the lock, 
    // so simply get the lock and check it again to start with, 
    // rather than checking to see whether or not the device still has a VPB without holding the lock.
...
Routine Description:
    ...
    This is done by querying the full key information of the key with a zero-length buffer to determine the size of the data,
    and then allocating a buffer and actually querying the data into the buffer.
    It is the responsibility of the caller to free the buffer.
Arguments:
    KeyHandle - Supplies the key handle whose full key information is to be queried
...
Routine Description:
    ...
    This is done by querying the value of the key with a zero-length buffer to determine the size of the value,
    and then allocating a buffer and actually querying the value into the buffer.
    It is the responsibility of the caller to free the buffer.
Arguments:
...
*/
{
    ...
    // Set an initial size to try when loading a key.
    // Note that KeyValueFullInformation already comes with a single WCHAR of data.
    guessSize = (ULONG)(sizeof(KEY_VALUE_FULL_INFORMATION) + wcslen(ValueName) * sizeof(WCHAR));

    // Now round up to a natural alignment.
    // This needs to be done because our data member will naturally aligned as well.
    guessSize = (ULONG)ALIGN_POINTER_OFFSET(guessSize);

    // Set the data cache length to a ULONG's worth of data, because most data we read via this function is type REG_DWORD.
    ...
    // Figure out how big the data value is so that a buffer of the appropriate size can be allocated.
    status = ZwQueryValueKey(KeyHandle, &unicodeString, KeyValueFullInformation, (PVOID)infoBuffer, guessSize, &keyValueLength);
    if (NT_SUCCESS(status)) {
    ...
    status = ZwQueryValueKey(KeyHandle, &unicodeString, KeyValueFullInformation, infoBuffer, keyValueLength, &keyValueLength);
    if (!NT_SUCCESS(status)) {
        ExFreePool(infoBuffer);
        return status;
    }
...
Routine Description:
    ...
    This is done by calling the IopGetRegistryValue function with the three valid key names.
    It is the responsibility of the caller to free the three buffers.
Arguments:
    KeyHandle - Supplies the key handle whose value is to be queried
    ValueList - Pointer to a buffer in which the three pointers to the value entries will be stored.
Return Value:
    The function value is the final status of the query operation.
Note:
    The values are stored in the order represented by the I/O query device data format.
*/
{
    NTSTATUS status;
    ...
    *(ValueList + 2) = NULL;
    ...
    // Each time, check if an internal error occurred;
    // If the object name was not found, it only means not data was present, and this does not constitute an error.
...
/*
Routine Description:
    This routine is invoked to obtain or set the object ID for a file.
    If one does not exist for the file, then one is created, provided that the underlying file system supports object IDs in the first place (query).
Arguments:
    FileObject - Supplies a pointer to the referenced file object whose ID is to be returned or set.
    Buffer - A variable to receive the object ID of the file (query) or that contains the object ID that is to be set on the file.
...
*/
{
    ...
    // Initialize the event structure to synchronize completion of the I/O request.
    KeInitializeEvent(&event, NotificationEvent, FALSE);

    // Build an I/O Request Packet to be sent to the file system driver to get the object ID.
    ...
    // Fill in the remainder of the IRP to retrieve the object ID for the file.
    irp->Flags |= IRP_SYNCHRONOUS_API;
    ...
    ObReferenceObject(FileObject);// Take out another reference to the file object to guarantee that it does not get deleted.
    status = IoCallDriver(deviceObject, irp);// Call the driver to get the request.
    if (status == STATUS_PENDING) {// Synchronize completion of the request.
...
/*
Routine Description:
    This routine is invoked by the I/O System link tracking code to obtain the volume ID for a file that has been moved or
    is being moved between volumes and potentially between systems.
Arguments:
...
*/
{
    ...
    KeInitializeEvent(&event, NotificationEvent, FALSE);// Initialize the event structure to synchronize completion of the I/O request.

    // Build an I/O Request Packet to be sent to the file system driver to get the volume ID.
    deviceObject = IoGetRelatedDeviceObject(FileObject);
    ...
    // Fill in the remainder of the IRP to retrieve the volume ID for the file.
    irp->Flags |= IRP_SYNCHRONOUS_API;
    ...
    // If the file system returned the volume ID, copy it to the caller's buffer and set the file system tracking type.
    if (NT_SUCCESS(status)) {
...
PIOP_HARD_ERROR_PACKET IopRemoveHardErrorPacket(VOID)
{
    ...
    // The work queue structures are now exclusively owned, so remove the first packet from the head of the list.
...
BOOLEAN IopCheckHardErrorEmpty(VOID)
{
    BOOLEAN MoreEntries;
    KIRQL   oldIrql;
...
/*
Routine Description:
    This function waits for work on the IopHardErrorQueue, and all calls IopRaiseInformationalHardError to actually perform the pop-ups.
Arguments:
...
*/
{
    ...
    // When one is placed onto the queue, wake up, process it, and continue the loop.
    MoreEntries = TRUE;
    do {
        (VOID)KeWaitForSingleObject(&IopHardError.WorkQueueSemaphore, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
        ...
        if (hardErrorPacket->String.Buffer) {
            ExFreePool(hardErrorPacket->String.Buffer);// Now free the packet and the buffer, if one was specified.
        }
...
/*
Routine Description:
    This function is the default dispatch routine for all driver entries not implemented by drivers that have been loaded into the system.
    Its responsibility is simply to set the status in the packet to indicate that the operation requested is invalid for this device type, and then complete the packet.
Arguments:
    DeviceObject - Specifies the device object for which this request is bound.  Ignored by this routine.
    Irp - Specifies the address of the I/O Request Packet (IRP) for this request.
Return Value:
...
*/
{
    ...
    // Simply store the appropriate status, complete the request, and return the same status stored in the packet.
...
LOGICAL IopIsSameMachine(IN PFILE_OBJECT SourceFile, IN HANDLE TargetFile)
/*
Routine Description:
    This routine is invoked to determine whether two file objects that represent files on remote machines actually reside on the same physical system.
Arguments:
    SourceFile - Supplies the file object for the first file.
    TargetFile - Supplies the file object for the second file.
Return Value:
    The final function value is TRUE if the files reside on the same machine, otherwise FALSE is returned.
*/
{
    PDEVICE_OBJECT deviceObject;
    ...
    // Simply invoke the device I/O control function to determine whether or not the two files are on the same server.
    // If the fast I/O path does not exist, or the function fails for any reason, then the two files are assumed to not be on the same machine.
    // Note that this simply means that there will be a performance penalty on open of the target, but
    // the above will only fail if the two files really aren't on the same machine in the first place, 
    // or if there's a filter that doesn't under-stand what is being done here.
...
NTSTATUS IopBuildFullDriverPath(IN PUNICODE_STRING KeyName, IN HANDLE KeyHandle, OUT PUNICODE_STRING FullPath)
/*
Routine Description:
    This routine builds up the full path for the driver.
    If ImagePath is specified, use it or else prepend the standard drivers path.
Arguments:
    KeyHandle - Supplies a handle to the driver service node in the registry that describes the driver to be loaded.
    CheckForSafeBoot - If TRUE, the driver will be loaded only if it belongs to the list of safe mode OK binaries.
...
BOOLEAN IopIsNotNativeDriverImage(IN PUNICODE_STRING ImageFileName)
/*
Routine Description:
...
*/
{
    ...
    Status = ZwCreateSection(&Section, SECTION_MAP_EXECUTE, &ObjectAttributes, NULL, PAGE_EXECUTE, SEC_COMMIT, ImageFileHandle);
    if (!NT_SUCCESS(Status)) {
...
BOOLEAN IopCheckIfNotNativeDriver(IN NTSTATUS InitialDriverLoadStatus, IN PUNICODE_STRING ImageFileName)
/*
Routine Description:
    ...
    if (InitialDriverLoadStatus == STATUS_OBJECT_NAME_NOT_FOUND) {
        while (System32DriversPath != NULL) {
            if (_wcsnicmp(System32DriversPath, SYSTEM32_DRIVERS_DIR, SYSTEM32_DRIVERS_DIR_LEN) == 0) {
                wcsncpy(System32DriversPath + 1, L"SysWow64", ((sizeof(L"SysWow64") - sizeof(UNICODE_NULL)) / sizeof(WCHAR)));
                break;
            }
...
Return Value:
    ...
    If STATUS_FAILED_DRIVER_ENTRY is returned, the driver's return value is stored in DriverEntryStatus.
Notes:
...
*/
{
    ...
    *DriverEntryStatus = STATUS_SUCCESS;
    ...
    // Note that this is used to determine whether or not the driver has already been loaded by the OS loader, 
    // not necessarily in actually loading the driver image, since the node can override that name.
    ...
    HeadlessKernelAddLogEntry(HEADLESS_LOG_LOADING_FILENAME, &baseName);// Log the file name
    ...
    if (CheckForSafeBoot && InitSafeBootMode) {
        ...
        if (!GroupIsGood && !IopSafebootDriverLoad(&baseName)) {// don't load the driver            
    ...
    // No need to do KeEnterCriticalRegion as this is called from system process only.
    ...
    while (nextEntry != &PsLoadedModuleList) {
        ...
        // If this is not the kernel image (ntoskrnl) and not the HAL (hal), then this is a driver, so initialize it.
        if (RtlEqualUnicodeString(&baseName, &driverEntry->FullDllName, TRUE)) {
    ...
    // This driver has not already been loaded by the OS loader.
    // Form the full path name for this driver.
    ...
    // Load the driver image into memory.
    // If this fails partway through the operation, then it will automatically be unloaded.
    ...
    if (!NT_SUCCESS(status)) {
...
#endif
                IopLogBlockedDriverEvent(&baseName, STATUS_INCOMPATIBLE_DRIVER_BLOCKED, status);// Log an event to the eventlog
            }
#endif
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
                // Adjust the exit code so that we can distinguish drivers that aren't present from drivers that are present but have had their driver objects made temporary.
        ...
        // if the unload has already run then driver is almost gone.
        // If the driver has been resurrected then the I/O system still has its original reference.
        ObDereferenceObject(driverObject);
        ExReleaseResourceLite(&IopDriverLoadResource);
        IopBootLog(&baseName, FALSE);
        goto IopLoadExit;
        ...
        // Check should this driver be loaded.
        // If yes, the enum subkey of the service will be prepared.
    ...
    if (serviceName.Buffer) {
        ...
        if (driverObject->DriverExtension->ServiceKeyName.Buffer) {
            ...
            RtlCopyMemory(driverObject->DriverExtension->ServiceKeyName.Buffer, serviceName.Buffer, serviceName.MaximumLength);
        }
    }
    ...
    *DriverEntryStatus = status;
    ...
    // If DriverInit doesn't work, then simply unload the image and mark the driver object as temporary.
    // This will cause everything to be deleted.
    ExFreePool(registryPath);

    // If we load the driver because we think it is a legacy driver and it does not create any device object in its DriverEntry.
    // We will unload this driver.
    ...
    if (!NT_SUCCESS(status)) {
        ObMakeTemporaryObject(driverObject);
        ObDereferenceObject(driverObject);
    } else {// Free the memory occupied by the driver's initialization routines.
        IopBootLog(&baseName, TRUE);
...
IopLoadExit:
    ...
    // If this routine is about to return a failure, then let the Configuration Manager know about it.
    // But, if STATUS_PLUGPLAY_NO_DEVICE, the device was disabled by hardware profile.
    // In this case we don't need to report it.
    if (!NT_SUCCESS(status) && (status != STATUS_PLUGPLAY_NO_DEVICE)) {
        ...
        if (status != STATUS_IMAGE_ALREADY_LOADED) {
            // If driver was loaded, do not call IopDriverLoadingFailed to change the driver loading status.
            // Because, obviously, the driver is running.
...
/*
Routine Description:
    The routine decrements the reference count on a device object.
    If the reference count goes to zero and the device object is a candidate for deletion then IopCompleteUnloadOrDelete is called.
    A device object is subject for deletion if the AlwaysUnload flag is true, or the device object is pending deletion or the driver is pending unload.
Arguments:
...
*/
{
    ...
    // If this is the last last reason that this mini-file system recognizer needs to stay around, then unload it.
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);
...
/*
Routine Description:
    This routine is invoked when a mini-file system recognizer driver recognizes a volume as being a particular file system,
    but the driver for that file system has not yet been loaded.
    This function allows the mini-driver to load the real file system, and remove itself from the system,
    so that the real file system can mount the device in question.
Arguments:
...
*/
{
    ...
    // Begin by building an I/O Request Packet to have the mini-file system driver load the real file system.
    KeInitializeEvent(&event, NotificationEvent, FALSE);
    irp = IoBuildDeviceIoControlRequest(IRP_MJ_DEVICE_CONTROL, attachedDevice, (PVOID)NULL, 0, (PVOID)NULL, 0, FALSE, &event, &ioStatus);
    if (irp) {
        // Change the actual major and minor function codes to be a file system control with a minor function code of load FS driver.
        irpSp = IoGetNextIrpStackLocation(irp);
        irpSp->MajorFunction = IRP_MJ_FILE_SYSTEM_CONTROL;
        irpSp->MinorFunction = IRP_MN_LOAD_FILE_SYSTEM;
        status = IoCallDriver(attachedDevice, irp);// Now issue the request.
...
/*
Routine Description:
    This routine is executed as an EX worker thread routine when a driver is to be loaded or unloaded dynamically.
    It is used because some drivers need to create system threads in the context of the system process,
    which cannot be done in the context of the caller of the system service that was invoked to load or unload the specified driver.
Arguments:
...
*/
{
    ...
    loadPacket = (PLOAD_PACKET)Parameter;// Begin by getting a pointer to the load packet.

    // If the driver object field of the packet is non-NULL, then this is a request to complete the unload of a driver.
    // Simply invoke the driver's unload routine.
    // Note that the final status of the unload is ignored, so it is not set here.
    if (loadPacket->DriverObject) {
        ...
        // Begin by opening the registry node for this driver.
        //  Note that if this is successful, then the load driver routine is responsible for closing the handle.
        ...
        if (NT_SUCCESS(status)) {
            ...
            // This is the same routine that is used by the I/O system initialization code to load drivers.
            ...
            if (status == STATUS_FAILED_DRIVER_ENTRY) {
                ...
                status = STATUS_OBJECT_NAME_NOT_FOUND;// Keep legacy behavior (don't change status code)
            }
...
Routine Description:
    ...
    The Volume Parameter Block (VPB) for the specified device is a "clean" VPB.
    That is, it indicates that the volume has never been mounted.
    It is up to the file system that eventually mounts the volume to determine whether the volume is, or has been, mounted elsewhere.
Arguments:
    DeviceObject - Pointer to device object on which the volume is to be mounted.
    AllowRawMount - This parameter tells us if we should continue our filesystem search to include the Raw file system.
                    This flag will only be passed in as TRUE as a result of a DASD open.
    DeviceLockAlreadyHeld - If TRUE, then the caller has already acquired the device lock and we should not attempt to acquire it.
                            This is currently passed in as TRUE when called from IoVerifyVolume.
Return Value:
...
*/
{
    ...
    // Obtain the lock for the device to be mounted.
    ...
    // Now acquire the resource database lock for the I/O system to perform this operation.
    // This resource protects access to the file system queue.
    ...
    if ((DeviceObject->Vpb->Flags & (VPB_MOUNTED | VPB_REMOVE_PENDING)) == 0) {
        // This volume has never been mounted.
        // Initialize the event and set the status to unsuccessful to set up for the loop.
        // Also if the device has the verify bit set, clear it.
        KeInitializeEvent(&event, NotificationEvent, FALSE);
        status = STATUS_UNSUCCESSFUL;
        DeviceObject->Flags &= ~DO_VERIFY_VOLUME;

        // Get the actual device that this volume is to be mounted on.
        // This device is the final device in the list of devices which are attached to the specified real device.
        attachedDevice = DeviceObject;
        while (attachedDevice->AttachedDevice) {
            attachedDevice = attachedDevice->AttachedDevice;
        }

        ObReferenceObject(attachedDevice);// Reference the device object so it cannot go away.

        // Determine which type of file system should be invoked based on the device type of the device being mounted.
        ...
        // Now loop through each of the file systems which have been loaded in the system to see whether anyone understands the media in the device.
        for (entry = queueHeader->Flink; entry != queueHeader && !NT_SUCCESS(status); entry = entry->Flink) {
            ...
            // If raw mount is the only one requested and this is not the last entry on the list then skip.
            ...
            // Another file system has been found and the volume has still not been mounted.
            // Attempt to mount the volume using this file system.

            // Begin by resetting the event being used for synchronization with the I/O operation.
            KeClearEvent(&event);


            // Allocate and initialize an IRP for this mount operation.
            ...
            // Note that this must be done while still holding the database resource.
            ...
            if (status == STATUS_PENDING) {
                (VOID)KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
            } else {// Ensure that the proper status value gets picked up.
            ...
            // If the device object is for FSREC it could not have gotten de-registered here.
            // It should get de-registered only at the time of loading the driver which should happen later.
            ...
            if (NT_SUCCESS(ioStatus.Status)) {
                ...
                *Vpb = IopMountInitializeVpb(DeviceObject, attachedDevice, rawMountOnly);
                ...
                // Make a special check here to determine whether or not a popup was enabled, and if so,
                ...
                // If there were any registrations or unregistrations during the period we unlocked the database resource bail out and start all over again.
                if (numRegOps != IopFsRegistrationOps) {
                    // Reset the list back to the beginning and start over again.
                    dummy.Flink = queueHeader->Flink;
                    entry = &dummy;
                    status = STATUS_UNRECOGNIZED_VOLUME;
                }

                // Also check to see whether or not this is a volume that has been recognized, but the file system for it needs to be loaded.
                // If so, drop the locks held at this point, tell the mini-file system recognizer to load the driver, and then reacquire the locks.
                if (status == STATUS_FS_DRIVER_REQUIRED) {
                    ...
                    // Now reacquire the locks, in the correct order, and check to see if the volume has been mounted before we could get back.
                    ...
                    if (DeviceObject->Vpb->Flags & VPB_MOUNTED) {
                        ...
                        //  Hence deref the attachedDevice as the other thread that got the reference is the one that's going to be used by the filesystem.
                        ObDereferenceObject(attachedDevice);
                        status = STATUS_SUCCESS;
                        break;
                    }
        ...
        if (!NT_SUCCESS(status)) {
            // The device was not mounted by us so drop the reference.
        ...
        status = STATUS_DEVICE_DOES_NOT_EXIST;// Pnp is attempting to remove this volume.  Don't allow the mount.
    } else {
        // The volume for this device has already been mounted.
    ...
    // Release the I/O database resource lock and the synchronization event for the device.
    if (!DeviceLockAlreadyHeld) {
        KeSetEvent(&DeviceObject->DeviceLock, 0, FALSE);
    }
    KeLeaveCriticalRegionThread(&CurrentThread->Tcb);

    // Finally, if the mount operation failed, and the target device is the boot partition, then bugcheck the system.
    // It is not possible for the system to run properly if the system's boot partition cannot be mounted.
...
/*
Routine Description:
    This routine is used to force filesystems to, as completely as possible, throw out volumes which remain referenced for a given device.
Arguments:
    DeviceObject - Pointer to device object for which volumes are to be invalidated.
Return Value:
    The function value is a successful status code if all filesystems accepted the operation.  Otherwise, an error code is returned.
*/
{
    ...
    // Note that there could be multiple device objects on the stack that has a VPB and could be potentially mounted on by FS.
    // So we call the FS with every device object that has a VPB.
    // This is really a simple brute force approach but this is not a high performance path and is backwards compatible.
    for (attachedDevice = DeviceObject; attachedDevice; attachedDevice = attachedDevice->AttachedDevice) {
        ...
        KeWaitForSingleObject(&(attachedDevice->DeviceLock), Executive, KernelMode, FALSE, NULL);// Synchronize against mounts.

        // Get a handle to this device for use in the fsctl.
        // The way we have to do this is kind of loopy: note we wind up with two references to clean up.

        // The only use of this fileobject/handle is to communicate the device to invalidate volumes on.
        // It isn't used for anything else, and must not be.
        storageHandle = NULL;
        ...
        if (NT_SUCCESS(status)) {
            ...
            // Determine which type of file system should be invoked based on the device type of the device being invalidated.
            if (DeviceObject->DeviceType == FILE_DEVICE_DISK || DeviceObject->DeviceType == FILE_DEVICE_VIRTUAL_DISK) {
                queueHeader = &IopDiskFileSystemQueueHead;
            } else if (DeviceObject->DeviceType == FILE_DEVICE_CD_ROM) {
                queueHeader = &IopCdRomFileSystemQueueHead;
            ...
            // Initialize the event and set the status to set up for the loop.
            KeInitializeEvent(&event, NotificationEvent, FALSE);
            finalStatus = STATUS_SUCCESS;

            // Now loop through each of the file systems which have been loaded in the system and ask them to invalidate volumes they have had mounted on it.
            for (entry = queueHeader->Flink; entry != queueHeader; entry = entry->Flink) {
                // If this is the final entry (Raw file system), then break out of the loop at this point, 
                // as volumes cannot be invalidated for the caller's purposes in Raw.
                ...
                // Another file system has been found.  Attempt to invalidate volumes using this file system.

                // Begin by resetting the event being used for synchronization with the I/O operation.
                KeClearEvent(&event);

                // Build an IRP for this operation.
                irp = IoBuildDeviceIoControlRequest(FSCTL_INVALIDATE_VOLUMES, fsDeviceObject, &storageHandle, sizeof(HANDLE), NULL, 0, FALSE, &event, &ioStatus);
                if (irp == NULL) {
                ...
                irpSp->MajorFunction = IRP_MJ_FILE_SYSTEM_CONTROL;
                status = IoCallDriver(fsDeviceObject, irp);
                if (status == STATUS_PENDING) {// Wait for the I/O operation to complete.
                    (VOID)KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
                    status = ioStatus.Status;
                } else {// Ensure that the proper status value gets picked up.
                    ioStatus.Status = status;
                ...
                // Commute status' indicating the operation is not implemented to success.
                // If a filesystem does not implement, it must not hold volumes that are not mounted.
            ...
            if (storageFileObject) {
                ...
                if (storageHandle) {
                    ZwClose(storageHandle); // Note that this is a close for which the FS has not gotten the corresponding open.
                }
            }
        ...
        KeSetEvent(&(attachedDevice->DeviceLock), IO_NO_INCREMENT, FALSE);// Unlock the device lock to let mounts go
    }
...
/*
Routine Description:
    This routine is invoked by the rename,
    set link and set copy-on-write code in the I/O system's NtSetInformationFile system service when the caller has specified a fully qualified file name as the target of a rename, set link,
    or set copy-on-write operation.
    This routine attempts to open the parent of the specified file and checks the following:
        o   If the file itself exists, then the caller must have specified that the target is to be replaced, otherwise an error is returned.
        o   Ensures that the target file specification refers to the same volume upon which the source file exists.
Arguments:
    TargetHandle - Supplies the address of a variable to return the handle to the opened target file if no errors have occurred.
    Irp - Supplies a pointer to the IRP that represents the current rename request.
    RenameBuffer - Supplies a pointer to the system intermediate buffer that contains the caller's rename parameters.
    FileObject - Supplies a pointer to the file object representing the file being renamed.
Return Value:
    The function value is the final status of the operation.
Note:
    This function assumes that the layout of a rename, set link and set copy-on-write information structure are exactly the same.
*/
{
    NTSTATUS status;
    IO_STATUS_BLOCK ioStatus;
    ...
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, ReplaceIfExists) == FIELD_OFFSET(FILE_LINK_INFORMATION, ReplaceIfExists));
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, RootDirectory) == FIELD_OFFSET(FILE_LINK_INFORMATION, RootDirectory));
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, FileNameLength) == FIELD_OFFSET(FILE_LINK_INFORMATION, FileNameLength));
    ...
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, ReplaceIfExists) == FIELD_OFFSET(FILE_MOVE_CLUSTER_INFORMATION, ClusterCount));
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, RootDirectory) == FIELD_OFFSET(FILE_MOVE_CLUSTER_INFORMATION, RootDirectory));
    ASSERT(FIELD_OFFSET(FILE_RENAME_INFORMATION, FileNameLength) == FIELD_OFFSET(FILE_MOVE_CLUSTER_INFORMATION, FileNameLength));
    ...
    // A fully qualified file name was specified.  Begin by attempting to open the parent directory of the specified target file.
    ...
    if (FileObject->Flags & FO_FILE_OBJECT_HAS_EXTENSION) {
        ...
        status = IoCreateFileSpecifyDeviceObjectHint(&handle,
                                                     ...
                                                     CreateFileTypeNone,
                                                     (PVOID)NULL,
                                                     IO_NO_PARAMETER_CHECKING | IO_OPEN_TARGET_DIRECTORY | IO_FORCE_ACCESS_CHECK | IOP_CREATE_USE_TOP_DEVICE_OBJECT_HINT,
    ...
    if (NT_SUCCESS(status)) {
        // The open operation for the target file's parent directory was successful.
        ...
        if (irpSp->Parameters.SetFile.FileInformationClass == FileLinkInformation &&
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (IoGetRelatedDeviceObject(targetFileObject) != IoGetRelatedDeviceObject(FileObject)) {
                    // The two files refer to different devices.  Clean everything up and return an appropriate error.
                    ObCloseHandle(handle, KernelMode);
                    status = STATUS_NOT_SAME_DEVICE;
                } else {
                    // Otherwise, everything worked, so allow the rename operation to continue.
                    irpSp->Parameters.SetFile.FileObject = targetFileObject;
                    *TargetHandle = handle;
                    status = STATUS_SUCCESS;
                }
            } else {
                // There was an error referencing the handle to what should have been the target directory.
                // This generally means that there was a resource problem or the handle was invalid, etc.
...
*/
{
    ...
    InitializeObjectAttributes(&objectAttributes, KeyName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, BaseHandle, (PSECURITY_DESCRIPTOR)NULL);// Initialize the object for the key.
    if (Create) {// Create the key or open it, as appropriate based on the caller's wishes.
        return ZwCreateKey(Handle, DesiredAccess, &objectAttributes, 0, (PUNICODE_STRING)NULL, REG_OPTION_VOLATILE, &disposition);
    } else {
...
NTSTATUS IopQueryXxxInformation(
    ...
    /*
    Routine Description:
        This routine returns the requested information about a specified file or volume.
        The information returned is determined by the class that is specified, and it is placed into the caller's output buffer.
    Arguments:
    ...
    */
{
    ...
    if (FileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        if (!IopAcquireFastLock(FileObject)) {
            status = IopAcquireFileObjectLock(FileObject, Mode, (BOOLEAN)((FileObject->Flags & FO_ALERTABLE_IO) != 0), &interrupted);
            if (interrupted) {
                ObDereferenceObject(FileObject);
                return status;
            }
    ...
    // This will be used to pass the original function codes and parameters.
    irpSp = IoGetNextIrpStackLocation(irp);
    irpSp->MajorFunction = FileInformation ? RP_MJ_QUERY_INFORMATION : IRP_MJ_QUERY_VOLUME_INFORMATION;
    irpSp->FileObject = FileObject;
...
/*
Routine Description:
    This routine raises a hard error popup in the context of the current thread.
    The APC was used to get into the context of this thread so that the popup would be sent to the appropriate port.
Arguments:
    NormalContext - Supplies a pointer to the I/O Request Packet (IRP) that was initially used to request the operation that has failed.
    SystemArgument1 - Supplies a pointer to the media's volume parameter block. See IoRaiseHardError documentation for more information.
    SystemArgument2 - Supplies a pointer to the real device object.  See IoRaiseHardError documentation for more information.
*/
{
    ...
    if (!NT_SUCCESS(status) || !NT_SUCCESS(status = ObQueryNameString(realDeviceObject, objectName, length, &response))) {
        // Allocation of the pool to put up this popup did not work or something else failed, so there isn't really much that can be done here.
        // Simply return an error back to the user.
        if (objectName) {
    ...
    // The volume label has a max size of 32 characters (Unicode).
    // Convert it to a Unicode string for output in the popup message.
    ...
    // Different pop-ups have different printf formats.
    // Depending on the specific error value, adjust the parameters.
    ...
    // Free any pool or other resources that were allocated to output the popup.
    ExFreePool(objectName);

    // If there was a problem, or the user didn't want to retry, just complete the request.
    // Otherwise simply call the driver entry point and retry the IRP as if it had never been tried before.
    if (!NT_SUCCESS(status) || response != ResponseTryAgain) {
        // Before completing the request, make one last check.
        // If this was a mount request, and the reason for the failure was t/o, no media, 
        // or unrecognized media, then set the Information field of the status block to indicate whether or not an abort was performed.
        ...
        driverObject->MajorFunction[irpSp->MajorFunction](fsDeviceObject, irp);// Retry the request from the top.
    }
}
...
/*
Routine Description:
    This routine performs the actual pop-up.
    It will called from either the hard-error thread, or a APC routine in a user thread after exiting the file system.
Arguments:
...
/*
Routine Description:
    This routine is invoked to mark all of the device objects owned by the specified driver as having been fully initialized and therefore ready for access by other drivers/clients.
Arguments:
    DriverObject - Supplies a pointer to the driver object for the driver whose devices are to be marked as being "ready".
*/
{
    ...
    // Loop through all of the driver's device objects, clearing the DO_DEVICE_INITIALIZING flag.
...
/*
Routine Description:
    This routine is invoked to clear unload pending flag on all of the device objects owned by the specified driver, if the unload routine has not run.
    This allows the driver to come back to life after a pending unload.
Arguments:
    DriverObject - Supplies a pointer to the driver object for the driver whose devices are to be cleared.
Return Value:
    Status - Returns success if the driver's unload routine has not run; otherwise STATUS_IMAGE_ALREADY_LOADED is returned.
*/
{
    PDEVICE_OBJECT deviceObject = DriverObject->DeviceObject;
    KIRQL irql;
    ...
    // Loop through all of the driver's device objects, clearing the DOE_UNLOAD_PENDING flag.
    while (deviceObject) {
        deviceObject->DeviceObjectExtension->ExtensionFlags &= ~DOE_UNLOAD_PENDING;
        deviceObject = deviceObject->NextDevice;
    }
...
VOID IopMarshalIds(
    ...
    /*
    Routine Description:
        This routine marshals the TargetVolumeId and TargetObjectId into the supplied TrackingBuffer in a standard remotable format.
        It also clears the DestinationFile handle to NULL, and sets the ObjectInformationLength to the size of the marshalled data.
    Arguments:
    ...
    */
{
    ...
    RtlZeroMemory(&TrackingBuffer->TrackingInformation.ObjectInformation[ObjectInformationLength], sizeof(TargetVolumeId->Type));
    RtlCopyMemory(&TrackingBuffer->TrackingInformation.ObjectInformation[ObjectInformationLength], &TargetVolumeId->Type, sizeof(TargetVolumeId->Type));
    ObjectInformationLength += sizeof(TargetVolumeId->Type);
    RtlCopyMemory(&TrackingBuffer->TrackingInformation.ObjectInformation[ObjectInformationLength], &TargetVolumeId->VolumeId[0], sizeof(TargetVolumeId->VolumeId));
    ObjectInformationLength += sizeof(TargetVolumeId->VolumeId);
    RtlCopyMemory(&TrackingBuffer->TrackingInformation.ObjectInformation[ObjectInformationLength], &TargetObjectId->ObjectId[0], sizeof(TargetObjectId->ObjectId));
    ObjectInformationLength += sizeof(TargetObjectId->ObjectId);
    RtlCopyMemory(&TrackingBuffer->TrackingInformation.ObjectInformation[ObjectInformationLength], &TrackingInfo->ObjectInformation[0], TrackingInfo->ObjectInformationLength);
...
/*
Routine Description:
    This routine unmarshals the TargetVolumeId and TargetObjectId from the supplied TrackingInformation from a standard remotable format.
Arguments:
...
{
    ULONG ObjectInformationLength = 0;
    RtlCopyMemory(&TargetVolumeId->Type, &TrackingInformation->ObjectInformation[ObjectInformationLength], sizeof(TargetVolumeId->Type));
    ObjectInformationLength += sizeof(TargetVolumeId->Type);
    RtlCopyMemory(&TargetVolumeId->VolumeId[0], &TrackingInformation->ObjectInformation[ObjectInformationLength], sizeof(TargetVolumeId->VolumeId));
    ObjectInformationLength += sizeof(TargetVolumeId->VolumeId);
    RtlCopyMemory(TargetObjectId, &TrackingInformation->ObjectInformation[ObjectInformationLength], sizeof(*TargetObjectId));
...
/*
Routine Description:
    This routine is invoked to send a message to the user-mode link tracking service to inform it that a file has been moved so that it can track it by its object ID.
Arguments:
...
*/
{
    ...
    // Begin by determining whether or not the LPC port to the link tracking service has been opened.
    // If not, then attempt to open it now.

retry:
    if (!IopLinkTrackingServiceObject) {
        // The port has not yet been opened.
        // Check to see whether or not the service has been started.
        // If not, then get out now as there will be no port if the service is not running.
        if (!KeReadStateEvent(IopLinkTrackingServiceEvent)) {
        ...
        for (;; ) {
            ...
            // There is no referenced object pointer to the link tracking port so open it.
            if (!IopLinkTrackingServiceObject) {
                ExInitializeWorkItem(&IopLinkTrackingPacket.WorkQueueItem, IopConnectLinkTrackingPort, &IopLinkTrackingPacket);
                (VOID)KeResetEvent(&IopLinkTrackingPacket.Event);
                ExQueueWorkItem(&IopLinkTrackingPacket.WorkQueueItem, DelayedWorkQueue);
                status = KeWaitForSingleObject(&IopLinkTrackingPacket.Event, Executive, PreviousMode, FALSE, (PLARGE_INTEGER)NULL);
                if ((status == STATUS_USER_APC) || (status == STATUS_ALERTED)) {
                    NOTHING;
            ...
            } else {// The connection is established.
                KeSetEvent(&IopLinkTrackingPortObject, 0, FALSE);
                break;
            }
        }
    ...
    IopUnMarshalIds(TargetObjectInformation, &requestMessage->TargetVolumeId, &requestMessage->TargetObjectId, &requestMessage->TargetMachineId);
...
/*
Routine Description:
    This routine is invoked by the NtSetEa[Quota]InformationFile system services to either modify the EAs on a file or the quota entries on a volume.
    All of the specified entries in the buffer are made to the file or volume.
Arguments:
    FileHandle - Supplies a handle to the file/volume for which the entries are to be applied.
    ...
    SetEa - A BOOLEAN that indicates whether to change the EAs on a file or the quota entries on the volume.
Return Value:
    The status returned is the final completion status of the operation.
*/
{
    ...
    if (requestorMode != KernelMode) {
        // The caller's access mode is user, so probe each of the arguments and capture them as necessary.
        ...
        try {
            ...
            ProbeForRead(Buffer, Length, sizeof(ULONG));// The Buffer parameter must be readable by the caller.
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
    ...
    // Make a special check here to determine whether this is a synchronous I/O operation.
    // If it is, then wait here until the file is owned by the current thread.
    // If this is not a (serialized) synchronous I/O operation, then allocate and initialize the local event.
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        BOOLEAN interrupted;
        if (!IopAcquireFastLock(fileObject)) {
    ...
    KeClearEvent(&fileObject->Event);// Set the file object to the Not-Signaled state.
    deviceObject = IoGetRelatedDeviceObject(fileObject);// Get the address of the target device object.
    ...
    // if the flag is set, then a system buffer is allocated and driver's data is copied to it.
    // If the DO_DIRECT_IO flag is set in the device object, then a Memory Descriptor List (MDL) is allocated and the caller's buffer is locked down using it.
    // Finally, if the driver specifies neither of the flags, 
    // then simply pass the address and length of the buffer and allow the driver to perform all of the checking and buffering if any is required.
    if (deviceObject->Flags & DO_BUFFERED_IO) {
        PVOID systemBuffer;
        ULONG errorOffset;
        if (Length) {
            // The driver wishes the caller's buffer to be copied into an intermediary buffer.
            // Allocate the system buffer and specify that it should be deallocated on completion.
            // Also check to ensure that the caller's EA list or quota list is valid.
            // All of this is performed within an exception handler that will perform cleanup if the operation fails.
            try {
                // Allocate the intermediary system buffer and charge the caller quota for its allocation.
                // Copy the caller's buffer into the system buffer and check to ensure that it is valid.
            ...
            {
                // An exception was incurred while allocating the buffer, copying the caller's data into it, or walking the buffer.
                // Determine what happened, cleanup, and return an appropriate error status code.
                IopExceptionCleanup(fileObject, irp, (PKEVENT)NULL, event);
                return GetExceptionCode();
            }

            irp->Flags |= IRP_BUFFERED_IO | IRP_DEALLOCATE_BUFFER;// Set the flags so that the completion code knows to deallocate the buffer.
        } else {
        ...
        if (Length) {
            ...
            try {
                ...
                // Probe and lock the pages associated with the caller's buffer for read access and fill in the MDL with the PFNs of those pages.
            ...
            {
                // An exception was incurred while either probing the caller's buffer or allocating the MDL.
                // Determine what actually happened, clean everything up, and return an appropriate error status code.
                IopExceptionCleanup(fileObject, irp, (PKEVENT)NULL, event);
                return GetExceptionCode();
            }
        }
    } else {// Pass the address of the user's buffer so the driver has access to it.  It is now the driver's responsibility to do everything.
        irp->UserBuffer = Buffer;
    }
    ...
    status = IopSynchronousServiceTail(deviceObject, irp, fileObject, FALSE, requestorMode, synchronousIo, OtherTransfer);

    // If the file for this operation was not opened for synchronous I/O, 
    // then synchronization of completion of the I/O operation has not yet occurred since the allocated event must be used for synchronous APIs on files opened for asynchronous I/O.
    // Synchronize the completion of the I/O operation now.
    if (!synchronousIo) {
...
/*
Routine Description:
    This routine is invoked to remote an NtSetInformationFile API call via an FSCTL to the Redirector.
    The call will cause the remote system to perform the service call to track the link for a file which was just moved.
Arguments:
    FileObject - Supplies the file object for the file that was moved.
    DestinationFileObject - Optionally supplies the file object for the new destination location for the file.
    FileInformation - Optionally supplies the volume and file object IDs of the target file.
Return Value:
    The final function value is the final completion status of the operation.
*/
{
    ...
    // Build an I/O Request Packet to be sent to the file system driver to get the volume ID.
    deviceObject = IoGetRelatedDeviceObject(FileObject);
    irp = IoBuildDeviceIoControlRequest(FSCTL_LMR_SET_LINK_TRACKING_INFORMATION, deviceObject, NULL, 0, NULL, 0, FALSE, &event, &ioStatus);
    if (!irp) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (DestinationFileObject) {
        ...
        if (FileInformation) {
            // Copy the ObjectInformation from the FileInformation buffer into the TargetLinkTrackingInformationBuffer.
            // Set 'length' to include this buffer.
            remoteBuffer.TrackingInformation.TargetLinkTrackingInformationLength = length = FileInformation->ObjectInformationLength;
            RtlCopyMemory(&remoteBuffer.TrackingInformation.TargetLinkTrackingInformationBuffer, FileInformation->ObjectInformation, length);
        } else {
            // We don't have any extra FileInformation.
            remoteBuffer.TrackingInformation.TargetLinkTrackingInformationLength = 0;
        }

        // Increment the length to include the size of the non-optional fields in REMOTE_LINK_TRACKING_INFORMATION.
        length += sizeof(PFILE_OBJECT) + sizeof(ULONG);
    } else {
        // There's no DestinationFileObject, so all the necessary information is in the FileInformation structure.
    ...
    // Take out another reference to the file object to guarantee that it does not get deleted.
    ObReferenceObject(FileObject);
    status = IoCallDriver(deviceObject, irp);// Call the driver to get the request.
    if (status == STATUS_PENDING) {// Synchronize completion of the request.
        status = KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
...
/*
Routine Description:
    This function is invoked in an ExWorker thread when we need to do a  hard error pop-up, but the Irp's originating thread is at APC level, ie. IoPageRead.
    It starts a thread to hold the pop-up.
Arguments:
    StartContext - Startup context, contains a IOP_APC_HARD_ERROR_PACKET.
*/
{
    ...
    //  Create the hard error pop-up thread.
    //  If for whatever reason we can't do this then just complete the Irp with the error.
    status = PsCreateSystemThread(&thread, 0, (POBJECT_ATTRIBUTES)NULL, (HANDLE)0, (PCLIENT_ID)NULL, IopApcHardError, StartContext);
    if (!NT_SUCCESS(status)) {
    ...
    ZwClose(thread);//  Close thread handle
}
...
/*
Routine Description:
    This routine is invoked when a synchronous API is invoked for a file that has been opened for asynchronous I/O.
    This function synchronizes the completion of the I/O operation on the file.
Arguments:
    ReturnedStatus - Supplies the status that was returned from the call to IoCallDriver.
    Event - Address of the allocated kernel event to be used for synchronization of the I/O operation.
    Irp - Address of the I/O Request Packet submitted to the driver.
    RequestorMode - Processor mode of the caller when the operation was requested.
    LocalIoStatus - Address of the I/O status block used to capture the final status by the service itself.
    IoStatusBlock - Address of the I/O status block supplied by the caller of the system service.
Return Value:
    The function value is the final status of the operation.
*/
{
    ...
    if (status == STATUS_PENDING) {
        ...
        if (status == STATUS_USER_APC) {
            ...
            // In either case, 
            // try to bail out of this I/O request carefully so that the IRP completes before this routine exists or the event will not be around to set to the Signaled state.
    ...
    {
        // An exception occurred attempting to write the caller's I/O status block.
        // Simply change the final status of the operation to the exception code.
        status = GetExceptionCode();
    }
...
Routine Description:
    ...
    It queues the IRP to the thread's queue, updates the transfer count, calls the driver, and finally synchronizes completion of the I/O.
Arguments:
...
*/
{
    ...
    // If deferred I/O completion is possible, check for pending returned from the driver.
    // If the driver did not return pending, then the packet has not actually been completed yet, so complete it here.
    ...
    if (SynchronousIo) {
        if (status == STATUS_PENDING) {
            ...
            if (status == STATUS_ALERTED || status == STATUS_USER_APC) {
                // The wait request has ended either because the thread was alerted or an APC was queued to this thread, because of thread rundown or CTRL/C processing.
                // In either case, 
                // try to bail out of this I/O request carefully so that the IRP completes before this routine exists so that synchronization with the file object will remain intact.
                IopCancelAlertedRequest(&FileObject->Event, Irp);
            }
...
/*
Routine Description:
    This routine scans the I/O system timer database and invokes each driver that has enabled a timer in the list, once every second.
Arguments:
...
*/
{
    ...
    // Check to see whether or not there are any timers in the queue that have been enabled.
    // If so, then walk the list and invoke all of the drivers' routines.
    // Note that if the counter changes, which it can because the spin lock is not owned, then a timer routine may be missed.
    // However, this is acceptable, since the driver inserting the entry could be context switched away from, etc.
    // Therefore, this is not a critical resource for the most part.
    if (IopTimerCount) {
        ...
        // For each entry found that is enabled, invoke the driver's routine with its specified context parameter.
        // The local count is used to abort the queue traversal when there are more entries in the queue, but they are not enabled.
        for (timerEntry = IopTimerQueueHead.Flink; (timerEntry != &IopTimerQueueHead) && i; timerEntry = timerEntry->Flink) {
...
/*
Routine Description:
    This routine is invoked to track a link.
    It tracks the source file's Object ID to the target file so that links to the source will follow to the new location of the target.
Arguments:
    FileObject - Supplies a pointer to the referenced source file object.
    IoStatusBlock - Pointer to the caller's I/O status block.
    FileInformation - A buffer containing the parameters for the move that was performed.
    Length - Specifies the length of the FileInformation buffer.
    Event - An event to be set to the Signaled state once the operation has been performed, provided it was successful.
    RequestorMode - Requestor mode of the caller.
N.B. - Note that the presence of an event indicates that the source file was opened for asynchronous I/O, otherwise it was opened for synchronous I/O.
Return Value:
    The status returned is the final completion status of the operation.
*/
{
    ...
    if (RequestorMode != KernelMode) {
        ...
        {
            // An exception was incurred while allocating the intermediary system buffer or while copying the caller's data into the buffer.
            // Cleanup and return an appropriate error status code.
            if (trackingInfo) {
                ExFreePool(trackingInfo);
            }
    ...
    try {
        // Determine whether this is a local or a remote link tracking operation.
        if (IsFileLocal(FileObject)) {
            // The source file, i.e., the one being moved, is a file local to this system.
            // Determine the form of the target file and track it accordingly.
            if (trackingInfo->DestinationFile) {
                if (IsFileLocal(dstFileObject)) {
                    BOOLEAN IdSetOnTarget = FALSE;
                    // The target file is specified as a handle and it is local.
                    // Simply perform the query and set locally.
                    // Note that if the source file does not have an object ID, then no tracking will be performed, but it will appear as if the operation worked.
                    ...
                    // If the extended info field is zero then this file has no interesting tracking information.
                    if (RtlCompareMemoryUlong(SourceObjectId.BirthObjectId, sizeof(SourceObjectId.BirthObjectId), 0) == sizeof(SourceObjectId.BirthObjectId)) {
                        status = STATUS_SUCCESS;
                        leave;
                    }
                    ...
                    // Delete the ID from the source now, since the target may be on the same volume.
                    // If there's a subsequent error, we'll try to restore it.
                    status = IopGetSetObjectId(FileObject, NULL, 0, FSCTL_DELETE_OBJECT_ID);
                    if (!NT_SUCCESS(status)) {
                        leave;
                    }

                    // Set the ID on the target.  If it's a cross-volume move, set the bit that indicates same.
                    CrossVolumeObjectId = TargetObjectId = SourceObjectId;
                    if (!RtlEqualMemory(&TargetVolumeId.VolumeId[0], &SourceVolumeId.VolumeId[0], sizeof(SourceVolumeId.VolumeId))) {
                        CrossVolumeObjectId.BirthVolumeId[0] |= 1;
                    }

                    status = IopGetSetObjectId(dstFileObject, &CrossVolumeObjectId, sizeof(CrossVolumeObjectId), FSCTL_SET_OBJECT_ID);
                    if (status == STATUS_DUPLICATE_NAME || status == STATUS_OBJECT_NAME_COLLISION) {
                        // This object ID is already in use on the target volume, or the dest file already has an object ID.
                    ...
                    if (NT_SUCCESS(status)) {
                        ...
                        if (!RtlEqualMemory(&TargetVolumeId.VolumeId[0], &SourceVolumeId.VolumeId[0], sizeof(SourceVolumeId.VolumeId))) {
                            ...
                            status = IopSendMessageToTrackService(&SourceVolumeId, &NormalizedObjectId, &trackingBuffer.TrackingInformation);
                        }
                    }

                    // If there was an error after the ObjectID was deleted from the source.  Try to restore it before returning.
                    if (!NT_SUCCESS(status)) {
                        NTSTATUS statusT = STATUS_SUCCESS;
                        if (IdSetOnTarget) {
                            if (RtlEqualMemory(&TargetObjectId.ObjectId, &SourceObjectId.ObjectId, sizeof(TargetObjectId.ObjectId))) {
                    ...
                    // The source file is local, but the destination file object is remote.
                    // For this case query the target file's object ID and notify the link tracking system that the file has been moved across systems.

                    // Begin by ensuring that the source file has an object ID already.
                    // If not, then just make it appear as if the operation worked.
                    status = IopGetSetObjectId(FileObject, &SourceObjectId, sizeof(SourceObjectId), FSCTL_GET_OBJECT_ID);
                    if (!NT_SUCCESS(status)) {
                        status = STATUS_SUCCESS;
                        leave;
                    ...
                    status = IopGetSetObjectId(dstFileObject, &TargetVolumeId, sizeof(FILE_VOLUMEID_WITH_TYPE), FSCTL_LMR_GET_LINK_TRACKING_INFORMATION);
                    if (!NT_SUCCESS(status)) {
                        leave;
                    }
                    ...
                    // Set the Birth ID on the target, turning on the bit that indicates that this file has been involved in a cross-volume move.
                ...
                // A destination file handle was not specified.
                // Simply query the source file's object ID and call the link tracking code.
                // Note that the function input buffer contains the volume ID and file object ID of the target.
                // Note also that it is assumed that the source file has an object ID.
                status = IopGetVolumeId(FileObject, &SourceVolumeId, sizeof(SourceVolumeId));
                if (!NT_SUCCESS(status)) {
                    leave;
                }
                ...
                // Inform the user-mode link tracking service that the file has been moved.
                NormalizedObjectId = SourceObjectId;
                NormalizedObjectId.BirthVolumeId[0] &= 0xfe;
            ...
            // For this case, remote the operation to the system on which the source file is located.
            // Begin by ensuring that the source file actually has an object ID.
            // If not, then get out now since there is nothing to be done.
            status = IopGetSetObjectId(FileObject, &SourceObjectId, sizeof(SourceObjectId), FSCTL_GET_OBJECT_ID);
            if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
                status = STATUS_SUCCESS;
                leave;
            ...
            if (trackingInfo->DestinationFile) {
                // A handle was specified for the destination file.
                // Determine whether it is local or remote.
                // If remote and both handles refer to the same machine, then ship the entire API to that machine and have it perform the operation.

                // Otherwise, query the target file's object ID, and then redo the operation.
                // This will cause the API to be remoted to the machine where the source file resides.
                if (IsFileLocal(dstFileObject)) {
                    // The source is remote and the destination is local, 
                    // so query the object ID of the target and recursively track the link from the source file's remote node.
                    status = IopGetVolumeId(dstFileObject, &TargetVolumeId, sizeof(TargetVolumeId));
                    if (!NT_SUCCESS(status)) {
                        leave;
                    }
                    ...
                    // Set the birth ID on the target, also turning on the bit that indicates that this file has moved across volumes.
                    CrossVolumeObjectId = SourceObjectId;
                    CrossVolumeObjectId.BirthVolumeId[0] |= 1;
                    status = IopGetSetObjectId(dstFileObject,
                                               &CrossVolumeObjectId.ExtendedInfo[0],
                    ...
                    // The source and the target are remote from each other and from this machine.
                    // Query the object ID of the target and recursively track the link from the source file's remote node.
                    ...
                    // Set the birth ID on the target, turning on the bit that indicates that this file has moved across volumes.
                    CrossVolumeObjectId = SourceObjectId;
                    CrossVolumeObjectId.BirthVolumeId[0] |= 1;
                    status = IopGetSetObjectId(dstFileObject,
                                               &CrossVolumeObjectId.ExtendedInfo[0],
                    ...
                    // Both the source and the target are remote and they're both on the same remote machine.
                    // For this case, remote the entire API using the file object pointers.
                ...
                // The source file is remote and the object ID of the target is contained w/in the tracking buffer.
                // Simply remote the API to the remote machine using the source file object pointer and the object ID of the target in the buffer.
                status = IopSetRemoteLink(FileObject, NULL, FileInformation);
            }   // if (trackingInfo->DestinationFile) ... else
        }   // if (IsFileLocal( FileObject )) ... else
    } finally{// Ensure that everything has been cleaned up.
...
/*
Routine Description:
    This routine is invoked in the final processing of an IRP.
    Everything has been completed except that the caller's APC routine must be invoked.
    The system will do this as soon as this routine exits.
    The only processing remaining to be completed by the I/O system is to free the I/O Request Packet itself.
Arguments:
    Apc - Supplies a pointer to kernel APC structure.
    NormalRoutine - Supplies a pointer to a pointer to the normal function that was specified when the APC was initialized.
    NormalContext - Supplies a pointer to a pointer to an arbitrary data structure that was specified when the APC was initialized.
...
*/
{
    ...
    IoFreeIrp(CONTAINING_RECORD(Apc, IRP, Tail.Apc));// Free the packet.
}
...
/*
Routine Description:
    This routine is invoked during thread termination as the rundown routine for it simply calls IopUserCompletion.
Arguments:
    Apc - Supplies a pointer to kernel APC structure.
*/
{
...
/*
Routine Description:
    This service builds descriptors or MDLs for the supplied buffer(s) and passes the untyped data to the driver associated with the file handle.
    handle.
...
*/
{
    ...
    method = IoControlCode & 3;// Get the method that the buffers are being passed by.
    ...
    if (requestorMode != KernelMode) {
        ...
        try {
            ...
            // The output buffer can be used in any one of the following three ways, if it is specified:
            //     0) It can be a normal, buffered output buffer.
            //     1) It can be a DMA input buffer.
            //     2) It can be a DMA output buffer.

            // Which way the buffer is to be used it based on the low-order two bits of the IoControlCode.
        ...
        {
            // An exception was incurred while attempting to probe or write one of the caller's parameters.
    ...
    // Now check the access type for this control code to ensure that the caller has the appropriate access to this file object to perform the operation.
    if (requestorMode != KernelMode) {
    ...
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        if (requestorMode != KernelMode) {
            try {
                ...
                // Note: IopMarkApcRoutineIfAsyncronousIo32 must be called after probing the IoStatusBlock structure for write.
    ...
    if (DeviceIoControl) {
        fastIoDispatch = deviceObject->DriverObject->FastIoDispatch;// Also get the address of the Fast I/O dispatch structure.

        // Turbo device control support.
        // If the device has a fast I/O entry point for DeviceIoControlFile, call the entry point and give it a chance to try to complete the request.
        // Note if FastIoDeviceControl returns FALSE or we get an I/O error, we simply fall through and go the "long way" and create an Irp.
        if (fastIoDispatch && fastIoDispatch->FastIoDeviceControl) {
            // Before we actually call the fast I/O routine in the driver, we must probe OutputBuffer if the method is METHOD_IN_DIRECT or METHOD_OUT_DIRECT.
            if (requestorMode != KernelMode && ARGUMENT_PRESENT(OutputBuffer)) {
                ...
                {
                    // An exception was incurred while attempting to probe the output buffer.
                    // Clean up and return an appropriate error status code.
                    if (synchronousIo) {
                        IopReleaseFileObjectLock(fileObject);
                    }
            ...
            if (fastIoDispatch->FastIoDeviceControl(fileObject,
                ...
                // The driver successfully performed the I/O in it's fast device control routine.
                // Carefully return the I/O status.
                try {
#if defined(_WIN64)
                    // If this is a32-bit thread, and the IO request is asynchronous, then the IOSB is 32-bit.
                    // Wow64 always sends the 32-bit IOSB when the I/O is asynchronous.
                ...
                // If there is an I/O completion port object associated w/this request, save it here.
                // We cannot look at the fileobject after signaling the event as that might result in the attachment of a completion port.
                ...
                // Note that the file object event need not be set to the Signaled state, as it is already set.
                // Release the file object lock, if necessary.
                if (synchronousIo) {
                    IopReleaseFileObjectLock(fileObject);
                }

                // If this file object has a completion port associated with it and this request has a non-NULL APC context then a completion message needs to be queued.
                if (port && ARGUMENT_PRESENT(ApcContext)) {
                    if (!NT_SUCCESS(IoSetIoCompletion(port, key, ApcContext, localIoStatus.Status, localIoStatus.Information, TRUE))) {
    ...
    irp = IopAllocateIrp(deviceObject->StackSize, !synchronousIo);// Allocate and initialize the I/O Request Packet (IRP) for this operation.
    if (!irp) {
        // An IRP could not be allocated.  Cleanup and return an appropriate error status code.
    ...
    // Get a pointer to the stack location for the first driver.
    // This will be used to pass the original function codes and parameters.
    // Note that setting the major function here also sets:
    //      MinorFunction = 0;
    //      Flags = 0;
    //      Control = 0;
    irpSp = IoGetNextIrpStackLocation(irp);
    ...
    *majorFunction = DeviceIoControl ? IRP_MJ_DEVICE_CONTROL : IRP_MJ_FILE_SYSTEM_CONTROL;
    ...
    // Copy the caller's parameters to the service-specific portion of the IRP for those parameters that are the same for all three methods.
    ...
    // Based on the method that the buffer are being passed, either allocate buffers or build MDLs.
    // Note that in some cases no probing has taken place so the exception handler must catch access violations.
    ...
    case METHOD_BUFFERED:
        ...
        try {
            if (InputBufferLength || OutputBufferLength) {
                irp->AssociatedIrp.SystemBuffer = ExAllocatePoolWithQuota(poolType, (InputBufferLength > OutputBufferLength) ? InputBufferLength : OutputBufferLength);
                if (ARGUMENT_PRESENT(InputBuffer)) {
        ...
        {
            // An exception was incurred while either allocating the the system buffer or moving the caller's data.
    ...
    case METHOD_OUT_DIRECT:
        ...
        // Note that the buffer length parameters have been jammed to zero for users if the buffer parameter was not passed.
        // (Kernel callers should be calling the service correctly in the first place.)

        // Note also that it doesn't make a whole lot of sense to specify either method #1 or #2 if the IOCTL does not require the caller to specify an output buffer.
        irp->Flags = 0;
        irpSp->Parameters.DeviceIoControl.Type3InputBuffer = (PVOID)NULL;
        try {
        ...
        {
            // An exception was incurred while either allocating the system buffer, copying the caller's data, allocating the MDL, or probing and locking the caller's buffer.
            // Determine what actually happened, cleanup accordingly, and return an appropriate error status code.
            IopExceptionCleanup(fileObject, irp, eventObject, (PKEVENT)NULL);
            return GetExceptionCode();
        }
    ...
    // Pass the read/write access granted bits down to the driver.
    // This allows drivers to check access for ioctls that were mistakenly defined as FILE_ANY_ACCESS and cannot be redefined for compatibility reasons.
    ...
    // Defer I/O completion for FSCTL requests, but not for IOCTL requests, since file systems set pending properly but device driver do not.
    if (!DeviceIoControl) {
    ...
    // Queue the packet, call the driver, and synchronize appropriately with I/O completion.
    return IopSynchronousServiceTail(deviceObject, irp, fileObject, (BOOLEAN)!DeviceIoControl, requestorMode, synchronousIo, OtherTransfer);
}
...
Arguments:
    KeyHandle - Supplies a handle to the opened registry key, HKLM\System\CurrentControlSet\Control\SystemResources\BusValues.
...
/*
Routine Description:
    Checks to see if a driver or service is included in the current safeboot registry section.
Arguments:
    DriverId - Specifies which driver is to be validated.
        The string should contain a driver executable name like foo.sys or a GUID for a pnp driver class.
Return Value:
...
/*
Routine Description:
    Create and write out a log entry.
    Before NtInitializeRegistry is called, log entries are spooled into the registry.
    When NtInitalizeRegistry is called by the session manager, the log file is created if necessary and truncated.
    Log entries in the registry are then copied into the log file and the registry entries are deleted.
Arguments:
...
/*
Routine Description:
    Copy the text in the registry entries into the log file and delete the registry entries.
    Set the flag that indicates direct logging to the log file.
*/
{
    UNICODE_STRING KeyName;
    WCHAR NameBuffer[BOOTLOG_STRSIZE];
    ...
    if (NT_SUCCESS(Status)) {
        ...
        ZwClose(hBootKey);
        BootLogRecord->FileLogging = TRUE;// Write directly to the file from now on.
    }
...
/*
Routine Description:
    This function inserts an entry at the head of a list using the I/O database lock for synchronization.
Arguments:
    Listhead - Supplies a pointer to the list head.
    ListEntry - Supplies a pointer to the list entry.
Return Value:
    If the list was previously empty, then NULL is returned as the function value.
    Otherwise, a pointer to the previous first entry in the list is returned as the function value.
*/
{
    PLIST_ENTRY entry;
    KIRQL irql;
...
/*
Routine Description:
    This function inserts an entry at the tail of a list using the I/O database lock for synchronization.
Arguments:
    Listhead - Supplies a pointer to the list head.
    ListEntry - Supplies a pointer to the list entry.
Return Value:
    If the list was previously empty, then NULL is returned as the function value.
    Otherwise, a pointer to the previous last entry in the list is returned as the function value.
*/
{
    PLIST_ENTRY entry;
    KIRQL irql;
...
/*
Routine Description:
    This function removes the first entry form a list using the I/O database lock for synchronization.
Arguments:
    Listhead - Supplies a pointer to the list head.
Return Value:
    If the list is empty, then NULL is returned as the function value.
    Otherwise, a pointer to the first entry in the list is returned as the function value.
*/
{
    PLIST_ENTRY entry;
    KIRQL irql;
...
ULONG FASTCALL IopInterlockedDecrementUlong(IN KSPIN_LOCK_QUEUE_NUMBER Number, IN OUT PLONG Addend)
/*
Routine Description:
    This function decrements the specified value using a queued spin lock for synchronization.
Arguments:
...
ULONG FASTCALL IopInterlockedIncrementUlong(IN KSPIN_LOCK_QUEUE_NUMBER Number, IN OUT PLONG Addend)
/*
Routine Description:
    This function increments the specified value using a queued spin lock for synchronization.
Arguments:
    Number - Supplies the number of the queued spin lock.
...
BOOLEAN IopCallDriverReinitializationRoutines(VOID)
/*
Routine Description:
...
Arguments:
    DriverName - supplies a pointer to the name of the driver whose driver object is to be referenced.
Returns:
...
*/
{
    ...
    if (DriverName->Length == 0) {// Make sure the driver name is valid.
        return NULL;
    }

    InitializeObjectAttributes(&objectAttributes, DriverName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    status = ObOpenObjectByName(&objectAttributes, IoDriverObjectType, KernelMode, NULL, FILE_READ_ATTRIBUTES, NULL, &driverHandle);
    if (NT_SUCCESS(status)) {
        status = ObReferenceObjectByHandle(driverHandle, 0, IoDriverObjectType, KernelMode, &driverObject, NULL);// Now reference the driver object.
        NtClose(driverHandle);
        if (NT_SUCCESS(status)) {
            return driverObject;
        }
...
*/
{
    ...
    // We explicitly propagate VPB_RAW_MOUNT as the previous statement that has been there for a long time in NT could be clearing other flags which should be cleared.
    if (RawMountOnly) {
...
/*
Routine Description:
    This routine verifies disk signature that is present in the loader block and that retrieved by the storage drivers.
Arguments:
...
*/
{
    ...
    // Save off the signature in local variable if its a MBR disk
...
/*
Routine Description:
    This routine returns true if a driver specified by name is present in the IO path for the fileobject.
Arguments:
...
*/
{
    ...
    if (FileObject->Vpb != NULL && FileObject->Vpb->DeviceObject != NULL) {
        if (IopVerifyDriverObjectOnStack(FileObject->Vpb->DeviceObject, driverObject)) {// Check the disk filesystem stack.
...
/*
Routine Description:
    This routine returns true if a driver specified by driverObject is present in the IO path for the deviceobject.
Arguments:
...
*/
{
    ...
    // If profile stack count is now enabled, we wait for ProfileDuration * 60 secs to stop it.
    // Once stopped, IopProcessIrpStackProfiler will tally the counts and find the top two stack counts.
...
/*
Routine Description:
    This routine profiles and resets the per-processor counters.
    It sets the variables IopLargeIrpStackLocations and IopSmallIrpStackLocations with the values learned from counters.
*/
{
    ...
    // Update the global variables. This should cause IoAllocateIrp to start using the new IRPs right away.
...
Routine Description:
    ...
    Its called from IoVerifyVolume.
    It takes a reference so that the VPB and the fsDeviceObject don't vanish.
Arguments:
