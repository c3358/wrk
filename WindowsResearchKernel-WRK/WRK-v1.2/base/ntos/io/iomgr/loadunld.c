...
/*
Routine Description:
    This service dynamically loads a device or file system driver into the currently running system.
...
*/
{
    ...
    if (requestorMode != KernelMode) {// The caller's access mode is not kernel so check to ensure that the caller has the privilege to load a driver and probe and capture the name of the driver service entry.
        if (!SeSinglePrivilegeCheck(SeLoadDriverPrivilege, requestorMode)) {
            return STATUS_PRIVILEGE_NOT_HELD;
        }

        // The caller has the appropriate privilege to load and unload drivers, 
        // so capture the driver service name string so that it can be used to locate the driver from the registry node.
        try {
        ...
        {
            // An exception was incurred while attempting to capture the input name string or while attempting to allocate the name string buffer.
            // Simply clean everything up and return an appropriate error status code.
            if (nameBuffer) {
    ...
    // Because drivers may wish to create a system thread and execute in its context, 
    // the remainder of this service must be executed in the context of the primary system process.
    // This is accomplished by queueing a request to one of the EX worker threads and having it invoke the I/O system routine to complete this work.
    // Fill in a request packet and queue it to the worker thread then, so that it can actually do the load.
    ...
    if (PsGetCurrentProcessByThread(CurrentThread) == PsInitialSystemProcess) {
        IopLoadUnloadDriver(&loadPacket);// If we are already in the system process, just use this thread.
    } else {
    ...
    // The load operation is now complete.
    // If a name buffer was allocated, deallocate it now, and return the final status of the load operation.
    if (nameBuffer) {
        ExFreePool(nameBuffer);
    }
...
NTSTATUS IopCheckUnloadDriver(IN PDRIVER_OBJECT driverObject, OUT PBOOLEAN unloadDriver)
{
    ...
    if ((driverObject->DeviceObject == NULL && (driverObject->Flags & DRVO_UNLOAD_INVOKED)) ||
        (!(driverObject->Flags & DRVO_BASE_FILESYSTEM_DRIVER) &&
         driverObject->DeviceObject &&
         driverObject->DeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_UNLOAD_PENDING)) {
        // The driver has already been marked for unload or is being unloaded.
        // Simply return a successful completion status since the driver is on its way out and therefore has been "marked for unload".
        KeReleaseQueuedSpinLock(LockQueueIoDatabaseLock, irql);
        ObDereferenceObject(driverObject);
        return STATUS_SUCCESS;
    }

    // The driver exists, and it implements unload, and it has not, so far, been marked for an unload operation.
    // Simply mark all of the devices that the driver owns as being marked for unload.
    // While this is going on, count the references for each of the devices.
    // If all of the devices have a zero reference count, then tell the driver that it should unload itself.
    ...
    *unloadDriver = TRUE;
    ...
    // If this is a base filesystem driver then delay the unload until all its device objects are deleted.
...
*/
{
    ...
    if ((requestorMode != KernelMode) && (InvokedByPnpMgr == FALSE)) {//应用层的走这里。
        ...
        // Now that the caller's parameters have been captured and everything appears to have checked out, actually attempt to unload the driver.
        // This is done with a previous mode of kernel so that drivers will not fail to unload because 
        // the caller didn't happen to have access to some resource that the driver needs in order to complete its unload operation.
        status = ZwUnloadDriver(&driverServiceName);
        ExFreePool(nameBuffer);
        return status;
    }

    // The caller's mode is now kernel mode.
    // Attempt to actually unload the driver specified by the indicated registry node.
    ...
    InitializeObjectAttributes(&objectAttributes, &driverName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    status = ObOpenObjectByName(&objectAttributes, IoDriverObjectType, KernelMode, NULL, FILE_READ_DATA, (PVOID)NULL, &driverHandle);
    ExFreePool(driverName.Buffer);//Perform some common cleanup by getting rid of buffers that have been allocated up to this point so that error conditions do not have as much work to do on each exit path.
    if (!NT_SUCCESS(status)) {// If the driver object could not be located in the first place, then return now before attempting to do anything else.
        return status;
    }
    ...
    // Also, if the driver has no section associated with it, then it was loaded be the OS loader and therefore cannot be unloaded.
    // If either is true, return an appropriate error status code.
    ...
    if (unloadDriver) {
        if (PsGetCurrentProcess() == PsInitialSystemProcess) {
            driverObject->DriverUnload(driverObject);// The current thread is already executing in the context of the system process, so simply invoke the driver's unload routine.
        } else {
            // The current thread is not executing in the context of the system process, which is required in order to invoke the driver's unload routine.
            // Queue a worker item to one of the worker threads to get into the appropriate process context and then invoke the routine.
            LOAD_PACKET loadPacket;
