...
// Define macros to allocate and free an open packet so platforms that can afford the extra stack space are not penalized by those that cannot.
#if defined(_AMD64_)
#define IopAllocateOpenPacket() _alloca(sizeof(OPEN_PACKET))
#define IopFreeOpenPacket(p)
#else
#define IopAllocateOpenPacket()    ExAllocatePoolWithTag( NonPagedPool, sizeof(OPEN_PACKET), 'pOoI')
...
*/
{
    *Irql = KeAcquireQueuedSpinLock(LockQueueIoCancelLock);// Simply acquire the cancel spin lock and return.
}
...
*/
{
    *Irql = KeAcquireQueuedSpinLock(LockQueueIoVpbLock);// Simply acquire the IopLoadFileSystemDriverVPB spin lock and return.
}
...
#else
    return((*((PDMA_ADAPTER)AdapterObject)->DmaOperations->AllocateAdapterChannel)((PDMA_ADAPTER)AdapterObject, 
                                                                                   DeviceObject, 
...
Routine Description:
    ...
    If the queue is already "busy", then the controller has already been allocated,
    so the device object is simply placed onto the queue and waits until the controller becomes free.
...
*/
{
    ...
    if (!KeInsertDeviceQueue(&ControllerObject->DeviceWaitQueue, &DeviceObject->Queue.Wcb.WaitQueueEntry)) {
        ...
        if (action == DeallocateObject) {// If the driver would like to have the controller deallocated, then deallocate it now.
...
NTSTATUS IoAllocateDriverObjectExtension(IN PDRIVER_OBJECT DriverObject,
    ...
    Return Value:
        ...
        Failure cases are STATUS_INSUFFICIENT_RESOURCES and STATUS_OBJECT_NAME_COLLISION.
    */
{
    KIRQL irql;
    ...
    *DriverObjectExtension = newExtension + 1;// Return a pointer to the client's data area.
...
PVOID IopAllocateErrorLogEntry(IN PDEVICE_OBJECT deviceObject, IN PDRIVER_OBJECT driverObject, IN UCHAR EntrySize)
{
    ...
    elEntry = ExAllocatePoolWithTag(NonPagedPool, size, 'rEoI');// Allocate the packet.
    if (elEntry == NULL) {
        InterlockedExchangeAdd(&IopErrorLogAllocation, -(LONG)size);// Drop the allocation and return.
        return(NULL);
    }
...
/*
Routine Description:
    This routine frees an entry allocated using IoAllocateErrorLogEntry.
    Its used to free an entry if its not used to actually write an errorlog entry.
Arguments:
...
*/
{
    ...
    entry = ((PERROR_LOG_ENTRY)ElEntry) - 1;// Get the address of the error log entry header,
...
*/
{
    ...
    largeIrpStackLocations = (CCHAR)IopLargeIrpStackLocations;// Capture this value once as it can change and use it.

    if ((StackSize <= (CCHAR)largeIrpStackLocations) && ((ChargeQuota == FALSE) || (prcb->LookasideIrpFloat > 0))) {
        ...
        if (IopIrpAutoSizingEnabled() && irp) {
            ...
            // We need to get the size of the IRP from the information field as the size field is overlayed with single list entry.
    ...
    if (!irp) {
        ...
        // or the packet is too large to be allocated from one of the lists, so it must be allocated from nonpaged pool.
        // If quota is to be charged, charge it against the current process.
        // Otherwise, allocate the pool normally.
        if (ChargeQuota) {
            irp = ExAllocatePoolWithQuotaTag(NonPagedPool | POOL_QUOTA_FAIL_INSTEAD_OF_RAISE, allocateSize, ' prI');
        } else {
...
Routine Description:
    ...
    fast interface for the Modified Page Writer (MPW) to write pages to the disk quickly and with very little overhead.
...
*/
{
    ...
    deviceObject = IoGetRelatedDeviceObject(FileObject);// Begin by getting a pointer to the device object that the file resides on.
    irp = IoAllocateIrp(deviceObject->StackSize, FALSE);// Allocate an I/O Request Packet (IRP) for this out-page operation.
    if (!irp) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    //  If specified, let the caller know what Irp is responsible for this transfer.
    //  While this is mainly for debugging purposes, it is absolutely essential to debug certain types of problems,
    //  and is very cheap, thus is included in the FREE build as well.
...
Routine Description:
    ...
    That is, it associates the source device to a target device which enables the I/O system to ensure that the target device
    a) exists, and
    b) cannot be unloaded until the source device has detached.
...
*/
{
    ...
    InitializeObjectAttributes(&objectAttributes, TargetDevice, OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    ...
    if (NT_SUCCESS(status)) {// The open operation was successful.
        ...
        if (NT_SUCCESS(status)) {// Get a pointer to the device object for this file, and close the handle.
    ...
    // Set the attached device pointer so that the driver being attached to cannot unload until the detach occurs,
    // and so that attempts to open the device object go through the attached driver.
...
Return Value:
    ...
    If unsuccessful, this function returns NULL.
    (This could happen if the device currently at the top of the attachment chain is being unloaded, deleted or initialized.)
*/
{
...
Arguments:
    SourceDevice - Specifies the device object that is to be attached to the target device.
    TargetDevice - Specifies the device object to which the attachment is to occur.
    AttachedToDeviceObject - Specifies a pointer  where the attached to device object is stored.
        Its updated while holding the database lock so that when a filter gets an IRP its attached to device object field is updated correctly.
*/
{
...
*/
{
    ...
    // Tell the Special IRP code the stack has changed.
    // Code that will reexamine the stack takes the database lock, so we can place the call here.
    // This also allows us to assert correct behavior *before* the stack is built up.
    IOV_ATTACH_DEVICE_TO_DEVICE_STACK(SourceDevice, TargetDevice);
    deviceObject = IoGetAttachedDevice(TargetDevice);

    ASSERT(sourceExtension->AttachedTo == NULL);// Make sure that the SourceDevice object isn't already attached to something else, this is now illegal.
    ...
    if (deviceObject->Flags & DO_DEVICE_INITIALIZING || 
        deviceObject->DeviceObjectExtension->ExtensionFlags & (DOE_UNLOAD_PENDING | DOE_DELETE_PENDING | DOE_REMOVE_PENDING | DOE_REMOVE_PROCESSED)) {
        ...
        // First update the device previously at the top of the attachment chain.
    ...
    // This is required so that a filesystem filter can attach to a device and before it gets an IRP it can update its lower device object pointer.
...
Routine Description:
    ...
    The request must be one of the following request codes:
        ...
        IRP_MJ_SHUTDOWN
        IRP_MJ_POWER
    This routine provides a simple,
    fast interface to the device driver w/o having to put the knowledge of how to build an IRP into all of the FSDs (and device drivers) in the system.
Arguments:
    ...
    IoStatusBlock - Pointer to the I/O status block for completion status information.
        This parameter is optional since most asynchronous FSD requests will be synchronized by using completion routines, 
        and so the I/O status block will not be written.
Return Value:
...
*/
{
    ...
    // This is where the function codes and the parameters are set.
    irpSp = IoGetNextIrpStackLocation(irp);
    ...
    if (MajorFunction != IRP_MJ_FLUSH_BUFFERS &&
        ...
        if (DeviceObject->Flags & DO_BUFFERED_IO) {
            ...
            // Allocate an MDL large enough to map the buffer and lock the pages into memory.
            ...
            // The operation is neither buffered nor direct.
            // Simply pass the address of the buffer in the packet to the driver.
            irp->UserBuffer = Buffer;
        }

        // Set the parameters according to whether this is a read or a write operation.
        // Notice that these parameters must be set even if the driver has not specified buffered or direct I/O.
        if (MajorFunction == IRP_MJ_WRITE) {
...
Arguments:
    ...
    InternalDeviceIoControl - A BOOLEAN parameter that specifies whether
        the packet that gets generated should have a major function code of IRP_MJ_INTERNAL_DEVICE_CONTROL (the parameter is TRUE),
        or IRP_MJ_DEVICE_CONTROL (the parameter is FALSE).
...
*/
{
    ...
    // Get a pointer to the stack location of the first driver which will be invoked.  This is where the function codes and the parameters are set.
    irpSp = IoGetNextIrpStackLocation(irp);
    ...
    case METHOD_OUT_DIRECT:
        // For these two cases, allocate a buffer that is large enough to contain the input buffer,
        // if any, and copy the information to the allocated buffer.
        ...
        if (ARGUMENT_PRESENT(OutputBuffer)) {
            irp->MdlAddress = IoAllocateMdl(OutputBuffer, OutputBufferLength, FALSE, FALSE, NULL);
            if (irp->MdlAddress == NULL) {
                if (ARGUMENT_PRESENT(InputBuffer)) {
...
*/
{
    ...
    // Set the MdlFlags in the target MDL.  Clear all flags but carry across the allocation information, page read and the system mapped info.
    TargetMdl->MdlFlags &= (MDL_ALLOCATED_FIXED_SIZE | MDL_ALLOCATED_MUST_SUCCEED);
    TargetMdl->MdlFlags |= SourceMdl->MdlFlags & (MDL_SOURCE_IS_NONPAGED_POOL | MDL_MAPPED_TO_SYSTEM_VA | MDL_IO_PAGE_READ | MDL_IO_SPACE);
    ...
    // Determine the base address of the first PFN in the source MDL that needs to be copied to the target.
    // Then, copy as many PFNs as are needed.
...
Arguments:
    MajorFunction - Function to be performed;  see previous list.
    DeviceObject - Pointer to device object on which the I/O will be performed.
    Buffer - Pointer to buffer to get data from or write data into. 
             This parameter is required for read/write, but not for flush or shutdown functions.
    Length - Length of buffer in bytes.  This parameter is required for read/write, but not for flush or shutdown functions.
    StartingOffset - Pointer to the offset on the disk to read/write from/to.
        This parameter is required for read/write, but not for flush or shutdown functions.
    Event - Pointer to a kernel event structure for synchronization. 
            The event will be set to the Signaled state when the I/O has completed.
...
Routine Description:
    ...
    It acquires the cancel spin lock, sets the cancel flag in the IRP,
    and then invokes the cancel routine specified by the appropriate field in the IRP, if a routine was specified.
    It is expected that the cancel routine will release the cancel spinlock.
...
*/
{
    ...
    IoAcquireCancelSpinLock(&irql);// Acquire the cancel spin lock.
    Irp->Cancel = TRUE;// Set the cancel flag in the IRP.
...
/*
Routine Description:
    This routine checks the validity of the specified EA buffer to guarantee that its format is proper,
    no fields hang over, that it is not recursive, etc.
Arguments:
...
#define GET_OFFSET_LENGTH( CurrentEa, EaBase ) (    (ULONG) ((PCHAR) CurrentEa - (PCHAR) EaBase) )
{
    ...
    // Walk the buffer and ensure that its format is valid.
    // That is, ensure that it does not walk off the end of the buffer, is not recursive, etc.
    eas = EaBuffer;
    tempLength = EaLength;
    for (;;) {
        // Get the size of the current entry in the buffer.
        // The minimum size of the entry is the fixed size part of the structure plus the length of the name,
        // a single termination character byte which must be present (a 0), plus the length of the value.
        // If this is not the last entry, then there will also be pad bytes to get to the next longword boundary.
        ...
        if (eas->NextEntryOffset) {
            // There is another entry in the buffer and it must be longword aligned.
            // Ensure that the offset indicates that it is.  If it isn't, return invalid parameter.
            if (ALIGN_LONG(entrySize) != eas->NextEntryOffset || (LONG)eas->NextEntryOffset < 0) {
                *ErrorOffset = GET_OFFSET_LENGTH(eas, EaBuffer);
                return STATUS_EA_LIST_INCONSISTENT;
            } else {
                // There is another entry in the buffer,
                // so account for the size of the current entry in the length and get a pointer to the next entry.
            ...
            // There are no other entries in the buffer.
            // Simply account for the overall buffer length according to the size of the current entry and exit the loop.
            tempLength -= entrySize;
            break;
        }
    }

    // All of the entries in the buffer have been processed.  Check to see whether the overall buffer length went negative.
...
Arguments:
    ...
    IoControlCode - I/O function control code for a device or file system I/O code.  Used only for those two function types.
    Arg1 - Optional argument that depends on the major function.
         Its FileInformationClass if the major function code indicates a query or set file information function is being performed.
         It points to Security Info if major function code is IRP_MJ_*_SECURITY.
    Arg2 - Optional second argument that depends on the major function. Currently its FsInformationClass.
        This parameter MUST be supplied if the major function code indicates that a query or set file system information function is being performed.
Return Value:
    The final status of the access check is the function value.
    If the accessor has the access to the file, then STATUS_SUCCESS is returned;
    otherwise, STATUS_ACCESS_DENIED is returned.
Note:
    The GrantedAccess mask may not contain any generic mappings.
    That is, the IoCheckDesiredAccess function must have been previously invoked to return a full mask.
*/
{
    ...
    // Determine the major function being performed.
    // If the function code is invalid, then return an error.
    ...
    case IRP_MJ_INTERNAL_DEVICE_CONTROL:
    {
        ...
        if (accessMode != FILE_ANY_ACCESS) {
            // This I/O control requires that the caller have read, write, or read/write access to the object.
            // If this is not the case, then cleanup and return an appropriate error status code.
...
Routine Description:
    ...
    It is used primarily by network servers running in kernel mode since no such parameter validity checking is done in the normal path.
Arguments:
    FileInformationClass - Specifies the information class to check checked.
    Length - Specifies the length of the buffer supplied.
    SetOperation - Specifies that the operation was a set file information as opposed to a query operation.
Return Value:
    The function value is STATUS_SUCCESS if the parameters were valid, otherwise an appropriate error is returned.
*/
{
...
Arguments:
    ...
    Length - Specifies the length of the buffer supplied.
    SetOperation - Specifies that the operation was a set volume information as opposed to a query operation.
Return Value:
    The function value is STATUS_SUCCESS if the parameters were valid, otherwise an appropriate error is returned.
*/
{
...
/*
Routine Description:
    This routine checks the validity of the specified quota buffer to guarantee that its format is proper, no fields hang over, 
    that it is not recursive, etc.
Arguments:
    QuotaBuffer - Pointer to the buffer containing the quota entries to be checked.
...
#define GET_OFFSET_LENGTH( CurrentEntry, QuotaBase ) ((ULONG) ((PCHAR) CurrentEntry - (PCHAR) QuotaBase) )
{
    ...
    for (;;) {
        ...
        // Get the size of the current entry in the buffer.
        // The minimum size of the entry is the fixed size part of the structure plus the actual length of the SID.
        // If this is not the last entry, then there will also be pad bytes to get to the next longword boundary.
        // Likewise, ensure that the SID itself is valid.
        ...
        if (quotas->NextEntryOffset) {
            // There is another entry in the buffer and it must be longword aligned.  Ensure that the offset indicates that it is.
            // If it is not, return error status code.
            ...
            // There are no more entries in the buffer.
            // Simply account for the overall buffer length according to the size of the current entry and exit the loop.
            tempLength -= entrySize;
            break;
        }
    ...
    // Check to see whether the overall buffer length went negative.
    // If so, return an error.
    if (tempLength < 0) {
        goto error_exit;
    }
...
Routine Description:
    ...
    The check is made according to:
...
Arguments:
    ...
    ShareAccess - Pointer to the share access structure that describes how the file is currently being accessed.
    Update - Specifies whether or not the share access information for the file is to be updated.
Return Value:
    The final status of the access check is the function value.
    If the accessor has access to the file, STATUS_SUCCESS is returned.
    Otherwise, STATUS_SHARING_VIOLATION is returned.
Note:
    Note that the ShareAccess parameter must be locked against other accesses from other threads while this routine is executing.
    Otherwise the counts will be out-of-synch.
*/
{
    ULONG ocount;
    ...
    // Note that reading and writing attributes are not included in the access check.
    ...
    // There is no more work to do unless the user specified one of the sharing modes above.
    if (FileObject->ReadAccess || FileObject->WriteAccess || FileObject->DeleteAccess) {
        ...
        // Now check to see whether or not the desired accesses are compatible with the way that the file is currently open.
        ...
        if ((FileObject->ReadAccess && (ShareAccess->SharedRead < ocount)) ||
            ...
            // The check failed.  Simply return to the caller indicating that the current open cannot access the file.
            return STATUS_SHARING_VIOLATION;

            // The check was successful.
            // Update the counter information in the shared access structure for this open request if the caller specified that it should be updated.
        } else if (Update) {
...
{
    // This routine will either jump immediately to IopfCompleteRequest, or rather IovCompleteRequest.
...
Routine Description:
    ...
    The functions performed by this routine are as follows.
        1.  A check is made to determine whether the packet's stack locations have been exhausted.
            If not, then the stack location pointer is set to the next location and if there is a routine to be invoked, then it will be invoked.
            This continues until there are either no more routines which are interested or the packet runs out of stack.
            If a routine is invoked to complete the packet for a specific driver which needs to perform work a lot of work or the work needs to be performed in the context of another process,
            then the routine will return an alternate success code of STATUS_MORE_PROCESSING_REQUIRED.
            This indicates that this completion routine should simply return to its caller because the operation will be "completed" by this routine again sometime in the future.
        2.  A check is made to determine whether this IRP is an associated IRP.
            If it is, then the count on the master IRP is decremented.
            If the count for the master becomes zero, then the master IRP will be completed according to the steps below taken for a normal IRP being completed.
            If the count is still non-zero, then this IRP (the one being completed) will simply be deallocated.
        3.  If this is paging I/O or a close operation, then simply write the I/O status block and set the event to the signaled state, and dereference the event.
            If this is paging I/O, deallocate the IRP as well.
        4.  Unlock the pages, if any, specified by the MDL by calling MmUnlockPages.
        5.  A check is made to determine whether or not completion of the request can be deferred until later.
            If it can be, then this routine simply exits and leaves it up to the originator of the request to fully complete the IRP.
            By not initializing and queueing the special kernel APC to the calling thread (which is the current thread by definition), a lot of interrupt and queueing processing can be avoided.
        6.  The final rundown routine is invoked to queue the request packet to the target (requesting) thread as a special kernel mode APC.
Arguments:
    ...
    (IrpSp)->FileObject = (PFILE_OBJECT) NULL; }
{
    ...
    ASSERT(Irp->Type == IO_TYPE_IRP);// Ensure that the packet being completed really is still an IRP.
    ASSERT(!Irp->CancelRoutine);// Ensure that no one believes that this request is still in a cancelable state.

    // Ensure that the packet is not being completed with a thoroughly confusing status code.
    // Actually completing a packet with a pending status probably means that someone forgot to set the real status in the packet.
    ASSERT(Irp->IoStatus.Status != STATUS_PENDING);

    // Ensure that the packet is not being completed with a minus one.
    // This is apparently a common problem in some drivers, and has no meaning as a status code.
    ...
    // Now check to see whether this is the last driver that needs to be invoked for this packet.
    // If not, then bump the stack and check to see whether the driver wishes to see the completion.
    // As each stack location is examined, invoke any routine which needs to be invoked.
    ...
    for (stackPointer = IoGetCurrentIrpStackLocation(Irp), Irp->CurrentLocation++, Irp->Tail.Overlay.CurrentStackLocation++;
        ...
        // A stack location was located.
        // Check to see whether or not it has a completion routine and if so, whether or not it should be invoked.
        ...
        // If a completion routine changed the status then mark the upper level stack pointer as the one that flagged the error.
        ...
        if ((NT_SUCCESS(Irp->IoStatus.Status) && stackPointer->Control & SL_INVOKE_ON_SUCCESS) ||
            ...
            // This driver has specified a completion routine.
            // Invoke the routine passing it a pointer to its device object and the IRP that is being completed.
            ...
            if (status == STATUS_MORE_PROCESSING_REQUIRED) {
                // Note:  Notice that if the driver has returned the above status value, it may have already DEALLOCATED the packet!
                //        Therefore, do NOT touch any part of the IRP in the following code.
                return;
            }
        } else {
    ...
    if (Irp->Flags & IRP_ASSOCIATED_IRP) {
        ...
        // by either doing direct deallocations if they were allocated from a zone or by queueing the packet to a thread to perform the deallocation.

        // Also, check the count of the master IRP to determine whether or not the count has gone to zero.
        // If not, then simply get out of here.
    ...
    // If so set the stage to transmogrify the reparse point data in IopCompleteRequest.
    ...
    // Check the auxiliary buffer pointer in the packet and if a buffer was allocated, deallocate it now.
    // Note that this buffer must be freed here since the pointer is overlayed with the APC that will be used to get to the requesting thread's context.
    if (Irp->Tail.Overlay.AuxiliaryBuffer) {
        ExFreePool(Irp->Tail.Overlay.AuxiliaryBuffer);
        Irp->Tail.Overlay.AuxiliaryBuffer = NULL;
    }

    // Check to see if this is paging I/O or a close operation.
    // If either, then special processing must be performed.
    // The reasons that special processing must be performed is different based on the type of operation being performed.
    // The biggest reasons for special processing on paging operations are that using a special kernel APC for an in-page operation cannot work since the special kernel APC can incur another pagefault.
    // Likewise, all paging I/O uses MDLs that belong to the memory manager, not the I/O system.

    // Close operations are special because the close may have been invoked because of a special kernel APC
    // (some IRP was completed which caused the reference count on the object to become zero while in the I/O system's special kernel APC routine).
    // Therefore, a special kernel APC cannot be used since it cannot execute until the close APC finishes.
    ...
    // It should also be noted that the logic for completing a Mount request operation is exactly the same as a Page Read.
    // No assumptions should be made here about this being a Page Read operation w/o carefully checking to ensure that they are also true for a Mount.
    // That is:
    //     IRP_PAGING_IO  and  IRP_MOUNT_COMPLETION

    // are the same flag in the IRP.

    // Also note that the last time the IRP is touched for a close operation must be just before the event is set to the signaled state.
    // Once this occurs, the IRP can be deallocated by the thread waiting for the event.

    // IRP_CLOSE_OPERATION and IRP_SET_USER_EVENT are the same flags.
    // They both indicate that only the user event field should be set and no APC should be queued.
    // Unfortunately IRP_CLOSE_OPERATION is used by some drivers to do exactly this so it cannot be renamed.
    ...
    // Make a final check here to determine whether or not this is a synchronous I/O operation that is being completed in the context of the original requestor.
    // If so, then an optimal path through I/O completion can be taken.
    ...
    // Finally, initialize the IRP as an APC structure and queue the special kernel APC to the target thread.
    thread = Irp->Tail.Overlay.Thread;
    ...
    if (!Irp->Cancel) {
        ...
        // Ensure that access to the thread is synchronized, 
        // otherwise it may go away while attempting to get through the remainder of completion for this request.
        // This happens when the thread times out waiting for the request to be completed once it has been cancelled.

        // Note that it is safe to capture the thread pointer above, w/o having the lock because the cancel flag was not set at that point,
        // and the code that disassociates IRPs must set the flag before looking to
        // see whether or not the packet has been completed, and this packet will appear to be completed because it no longer belongs to a driver.
        ...
        if (thread) {
            ...
            // This request has been aborted from completing in the caller's thread.
            // This can only occur if the packet was cancelled, and the driver did not complete the request, so it was timed out.
            // Attempt to drop things on the floor, since the originating thread has probably exited at this point.
            KeReleaseQueuedSpinLock(LockQueueIoCompletionLock, irql);
            ASSERT(Irp->Cancel);
            IopDropIrp(Irp, fileObject);// Drop the IRP on the floor.
        }
    }
}
...
Arguments:
    InterruptObject - Address of a variable to receive a pointer to the first interrupt object allocated and initialized.
    ServiceRoutine - Address of the interrupt service routine (ISR) that should be executed when the interrupt occurs.
    ServiceContext - Supplies a pointer to the context information required by the ISR.
    SpinLock - Supplies a pointer to a spin lock to be used when synchronizing with the ISR.
    ...
    ShareVector - Supplies a boolean value that specifies whether the vector can be shared with other interrupt objects or not.
        ...
        Latched.
    ProcessorEnableMask - Specifies a bit-vector for each processor on which the interrupt is to be connected.
        A value of one in the bit position corresponding to the processor number indicates that the interrupt should be allowed on that processor.
        At least one bit must be set.
    FloatingSave - A BOOLEAN that specifies whether or not the machine's floating point state should be saved before invoking the ISR.
Return Value:
    The function value is the final function status.
    The three status values that this routine can itself return are:
...
*/
{
    ...
    *InterruptObject = (PKINTERRUPT)NULL;
    ...
    // Determine how much memory is to be allocated based on how many processors this system may have and how many bits are set in the processor enable mask.
    ...
    // If any interrupt objects are to be allocated and initialized, allocate the memory now.
    if (count) {
        interruptStructure = ExAllocatePoolWithTag(NonPagedPool, ((count - 1) * sizeof(KINTERRUPT)) + sizeof(IO_INTERRUPT_STRUCTURE), 'nioI');
    ...
    // If the caller specified a spin lock to be used for the interrupt object, use it.
    // Otherwise, provide one by using the one in the structure that was just allocated.
    ...
    // Initialize and connect the interrupt objects to the appropriate processors.

    // Return the address of the first interrupt object in case an interrupt is pending for the device when it is initially connected and the driver must synchronize its execution with the ISR.
    *InterruptObject = &interruptStructure->InterruptObject;

    // Begin by getting a pointer to the start of the memory to be used for interrupt objects other than the builtin object.
    interruptObject = (PKINTERRUPT)(interruptStructure + 1);
    builtinUsed = FALSE;
    processorMask = ProcessorEnableMask & KeActiveProcessors;

    // Now zero the interrupt structure itself so that if something goes wrong it can be backed out.
    RtlZeroMemory(interruptStructure, sizeof(IO_INTERRUPT_STRUCTURE));

    // For each entry in the processor enable mask that is set, connect and initialize an interrupt object.
    // The first bit that is set uses the builtin interrupt object, and all others use the pointers that follow it.
    for (count = 0; processorMask; count++, processorMask >>= 1) {
        if (processorMask & 1) {
            ...
            if (!KeConnectInterrupt(builtinUsed ? interruptObject : &interruptStructure->InterruptObject)) {
                // An error occurred while attempting to connect the interrupt.
                // This means that the driver either specified the wrong type of interrupt mode, 
                // or attempted to connect to some processor that didn't exist, or whatever.
                // In any case, the problem turns out to be an invalid parameter was specified.
                // Simply back everything out and return an error status.

                // Note that if the builtin entry was used,
                // then the entire structure needs to be walked as there are entries that were successfully connected.
                // Otherwise, the first attempt to connect failed, so simply free everything in-line.
                if (builtinUsed) {
            ...
            // If the builtin entry has been used, then the interrupt object just connected was one of the pointers, 
            // so fill it in with the address of the memory actually used.
            if (builtinUsed) {
                interruptStructure->InterruptArray[count] = interruptObject++;
            } else {
                // Otherwise, the builtin entry was used, so indicate that it is no longer valid to use and start using the pointers instead.
                builtinUsed = TRUE;
            }
        }
    }

    // Finally, reset the address of the interrupt object if the function failed and return the final status of the operation.
...
/*
Routine Description:
    This routine creates a controller object that can be used to synchronize access to a physical device controller from two or more devices.
Arguments:
...
*/
{
    ...
    // Initialize the object attributes structure in preparation for creating the controller object.
    InitializeObjectAttributes(&objectAttributes, (PUNICODE_STRING)NULL, OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    ...
    if (NT_SUCCESS(status)) {
        status = ObInsertObject(controllerObject, NULL, FILE_READ_DATA | FILE_WRITE_DATA, 1, (PVOID *)&controllerObject, &handle);// Insert the controller object into the table.
        if (!NT_SUCCESS(status)) {// If the insert operation fails, set return value to NULL.
            controllerObject = (PCONTROLLER_OBJECT)NULL;
        } else {// The insert completed successfully.  Close the handle so that if the driver is unloaded, the controller object can go away.
            (VOID)ObCloseHandle(handle, KernelMode);

            RtlZeroMemory(controllerObject, sizeof(CONTROLLER_OBJECT) + Size);// Zero the memory for the controller object.
            ...
            KeInitializeDeviceQueue(&controllerObject->DeviceWaitQueue);// Finally, initialize the controller's device queue.
        }
    } else {
        controllerObject = (PCONTROLLER_OBJECT)NULL;
    }
...
Arguments:
    ...
    DeviceExtensionSize - Size, in bytes, of extension to device object; i.e., the size of the driver-specific data for this device object.
    DeviceName - Optional name that should be associated with this device.
        If the DeviceCharacteristics has the FILE_AUTOGENERATED_DEVICE_NAME flag set, this parameter is ignored.
    ...
    Exclusive - Indicates that the device object should be created with using the exclusive object attribute.
        NOTE: This flag should not be used for WDM drivers.
        Since only the PDO is named, it is the only device object in a devnode attachment stack that is openable.
        However, since this device object is created by the underlying bus driver (which has no knowledge about what type of device this is),
        there is no way to know whether this flag should be set.
        Therefore, this parameter should always be FALSE for WDM drivers.
        Drivers attached to the PDO (e.g., the function driver) must enforce any exclusivity rules.
...
*/
{
    ...
    // Enclose the creation of the device object in a do/while, 
    // in the rare event where we have to retry because some other driver is using our naming scheme for auto-generated device object names.
    do {
        if (DeviceCharacteristics & FILE_AUTOGENERATED_DEVICE_NAME) {
            // The caller has requested that we automatically generate a device object name.
            // Retrieve the next available number to use for this purpose, and create a name of the form "\Device\<n>", 
            // where <n> is the (8 hexadecimal digit) character representation of the unique number we retrieve.
            ...
            if (retryWithNewName) {
                // We've already done this once (hence, the unicode device name string is all set up, as is all the security information).
                // Thus, we can skip down to where we re-attempt the creation of the device object using our new name.
                ...
                // if the caller had specified it (note, we explicitly ignore anything the caller passes us for device name if the FILE_AUTOGENERATED_DEVICE_NAME characteristic is specified.
                RtlInitUnicodeString(&autoGeneratedDeviceName, deviceNameBuffer);
                DeviceName = &autoGeneratedDeviceName;
            }
        }

        // Remember whether or not this device was created with a name so that it can be deallocated later.
        deviceHasName = (BOOLEAN)(ARGUMENT_PRESENT(DeviceName) ? TRUE : FALSE);

        // Determine whether or not this device needs to have a security descriptor placed on it that allows read/write access, 
        // or whether the system default should be used.
        // Disks, virtual disks, and file systems simply use the system default descriptor.
        // All others allow read/write access.

        // NOTE: This routine assumes that it is in the system's security context.
        //       In particular, it assumes that the Default DACL is the system's Default DACL.
        //       If this assumption changes in future releases, then use of the Default DACL below should be reviewed for appropriateness.

        // If the device is a pnp device then wait until it registers a device class before doing the default setup.
        securityDescriptor = IopCreateDefaultDeviceSecurityDescriptor(DeviceType,
    ...
    attemptDeviceObjectCreation:
        // Initialize the object attributes structure in preparation for creating device object.
        // Note that the device may be created as an exclusive device so that only one open can be done to it at a time.
        // This saves single user devices from having drivers that implement special code to make sure that only one connection is ever valid at any given time.
        InitializeObjectAttributes(&objectAttributes, DeviceName, OBJ_KERNEL_HANDLE, (HANDLE)NULL, securityDescriptor);
        ...
        status = ObCreateObject(KernelMode,
                                ...
                                KernelMode,
                                (PVOID)NULL,
                                (ULONG) sizeof(DEVICE_OBJECT) + sizeof(DEVOBJ_EXTENSION) + RoundedSize,
                                0,
                                0,
                                (PVOID *)&deviceObject);
        if ((status == STATUS_OBJECT_NAME_COLLISION) && (DeviceCharacteristics & FILE_AUTOGENERATED_DEVICE_NAME)) {
            // Some other driver is using our naming scheme, and we've picked a device name already in use.
    ...
    if (!NT_SUCCESS(status)) {
        // Creating the device object was not successful.
        // Clean everything up and indicate that the object was not created.
        deviceObject = (PDEVICE_OBJECT)NULL;
    } else {
        // The device was successfully created.
        // Initialize the object so that it can be inserted into the object table.
        // Begin by zeroing the memory for the device object.
        ...
        // Note: the size of a Device Object Extension is initialized specifically to ZERO so no driver will depend on it.
        ...
        // If this device is either a tape or a disk, allocate a Volume Parameter Block (VPB) which indicates that the volume has never been mounted,
        // and set the device object's VPB pointer to it.
        ...
        case FILE_DEVICE_TAPE_FILE_SYSTEM:
            // This device represents a file system of some sort.
            // Simply initialize the queue list head in the device object.
            InitializeListHead(&deviceObject->Queue.ListEntry);
            break;
        default:
            // This is a real device of some sort.
            // Allocate a spin lock and initialize the device queue object in the device object.
            KeInitializeDeviceQueue(&deviceObject->DeviceQueue);
            break;
        }

        // Insert the device object into the table.
        status = ObInsertObject(deviceObject, NULL, FILE_READ_DATA | FILE_WRITE_DATA, 1, (PVOID *)&deviceObject, &handle);
        if (NT_SUCCESS(status)) {
            // Reference the driver object.
            // When the device object goes away the reference will be removed.
            // This prevents the driver object and driver image from going away while the device object is in the pending delete state.
            ObReferenceObject(DriverObject);
            ASSERT((DriverObject->Flags & DRVO_UNLOAD_INVOKED) == 0);

            // The insert completed successfully.
            // Link the device object and driver objects together.
            // Close the handle so that if the driver is unloaded, the device object can go away.
            ...
            // Fortunately it dropped the reference count on the device - since that was the last one all the cleanup should be done for us.
...
Routine Description:
    ...
    This procedure is also used internally by kernel mode components, such as the network server, to perform the same type of operation,
    but allows kernel mode code to force checking arguments and access to the file,
    rather than bypassing these checks because the code is running in kernel mode.
Arguments:
    ...
    ObjectAttributes - Supplies the attributes to be used for the file object (name, SECURITY_DESCRIPTOR, etc.)
    ...
    AllocationSize - Initial size that should be allocated to the file.
        This parameter only has an affect if the file is created.
        Further, if not specified, then it is taken to mean zero.
    ...
    ExtraCreateParameters - Optionally specifies a pointer to extra create parameters.
        The format of the parameters depends on the value of CreateFileType.
    Options - Specifies the options that are to be used during generation of the create IRP.
Return Value:
    The function value is the final status of the create/open operation.
Warning:
    If a pointer to ExtraCreateParameters is passed the data must be readable from kernel mode.
*/
{
#define IOP_CREATE_FILE_TIMEOUT 1000

    NTSTATUS status = STATUS_SUCCESS;  // compiling with W4 incorrectly reports an error at the break instruction inside the loop
    ...
    do {
        ...
        //  If another thread is in the process of atomic file rename, we need to retry opening the file in case of failures.
        //  If a high priority thread enters this code while a another thread is in the middle of doing a double-rename operation,
        //  we need to back off a bit this thread and sleep until others threads involved in hotpatching finish the operation.

        //  Note: SleepInterval starts from 0, and it will be incremented after the test.
        //  So the thread will not sleep first time calling IoCreateFile
        if ((CapturedSequence & 1) && (SleepInterval++)) {
            LARGE_INTEGER interval;

            //  We are during an atomic rename operation.
            //  Check if this happens in the context of this thread.
            //  Do not attempt to retry if yes.
            //  This handles cases where AV drivers open files synchronous as effect of a file rename.
            if (ExSyncRenameOwner == KeGetCurrentThread()) {
                //  Note that this code path is not taken for the first pass through this do / while loop, 
                //  so the correct status will be initialized if this condition is true
                break;
            }

            interval.QuadPart = -(LONGLONG)SleepInterval * 10 * 1000;
            if (SleepInterval > IOP_CREATE_FILE_TIMEOUT) {
                //  We still do not make any progress.
                //  Limit the timeout next sleep to something reasonable and assert a possible deadlock in checked builds.
        ...
        status = IopCreateFile(FileHandle,
                               ...
                               0,
                               NULL);
    } while ((!NT_SUCCESS(status)) && ((CapturedSequence & 1) || (CapturedSequence != ExHotpSyncRenameSequence)));

    return status;
}


NTSTATUS IoCreateFileSpecifyDeviceObjectHint(OUT PHANDLE FileHandle,
                                             IN ACCESS_MASK DesiredAccess,
                                             IN POBJECT_ATTRIBUTES ObjectAttributes,
                                             OUT PIO_STATUS_BLOCK IoStatusBlock,
                                             IN PLARGE_INTEGER AllocationSize OPTIONAL,
...
NTSTATUS IopCreateFile(OUT PHANDLE FileHandle,
                       IN ACCESS_MASK DesiredAccess,
                       IN POBJECT_ATTRIBUTES ObjectAttributes,
                       OUT PIO_STATUS_BLOCK IoStatusBlock,
                       IN PLARGE_INTEGER AllocationSize OPTIONAL,
...
Routine Description:
    This is the common routine for both NtCreateFile and NtOpenFile to allow a user to create or open a file.
    This procedure is also used internally by kernel mode components, such as the network server,
    to perform the same type of operation, but allows kernel mode code to force checking
    arguments and access to the file, rather than bypassing these checks because the code is running in kernel mode.
Arguments:
    FileHandle - A pointer to a variable to receive the handle to the open file.
    DesiredAccess - Supplies the types of access that the caller would like to the file.
    ObjectAttributes - Supplies the attributes to be used for the file object (name, SECURITY_DESCRIPTOR, etc.)
    IoStatusBlock - Specifies the address of the caller's I/O status block.
    AllocationSize - Initial size that should be allocated to the file.
        This parameter only has an affect if the file is created.  Further, if not specified, then it is taken to mean zero.
    FileAttributes - Specifies the attributes that should be set on the file, if it is created.
    ShareAccess - Supplies the types of share access that the caller would like to the file.
    Disposition - Supplies the method for handling the create/open.
    CreateOptions - Caller options for how to perform the create/open.
...
*/
{
    ...
    if (requestorMode != KernelMode || Options & IO_CHECK_CREATE_PARAMETERS) {
        ...
        if (
            ...
            (FileAttributes & ~FILE_ATTRIBUTE_VALID_FLAGS) ||// Check that no invalid file attributes flags were specified.
            (ShareAccess & ~FILE_SHARE_VALID_FLAGS) ||// Check that no invalid share access flags were specified.
            (Disposition > FILE_MAXIMUM_DISPOSITION) ||// Ensure that the disposition value is in range.
            (CreateOptions & ~FILE_VALID_OPTION_FLAGS) ||// Check that no invalid create options were specified.

            // If the caller specified synchronous I/O, then ensure that (s)he also asked for synchronize desired access to the file.
            (CreateOptions & (FILE_SYNCHRONOUS_IO_ALERT | FILE_SYNCHRONOUS_IO_NONALERT) && (!(DesiredAccess & SYNCHRONIZE))) ||

            // Also, if the caller specified that the file is to be deleted on close,
            // then ensure that delete is specified as one of the desired accesses requested.
            ...
            //     o  No other flags other than the synchronous I/O flags, write-through, or open by file ID are set.
            //     o  The disposition value is one of create, open, or open-if.
            ...
            ((CreateOptions & FILE_DIRECTORY_FILE)
             ...
             ((CreateOptions & FILE_COMPLETE_IF_OPLOCKED) && (CreateOptions & FILE_RESERVE_OPFILTER)) ||//  FILE_COMPLETE_IF_OPLOCK and FILE_RESERVE_OPFILTER are incompatible options.
            (IopFailZeroAccessCreate && !DesiredAccess) ||// Fail the create if desired access is zero.

            // Finally, if the no intermediate buffering option was specified, ensure that the caller did not also request append access to the file.
        ...
        if (CreateFileType == CreateFileTypeNone) {
            ...
            if (!ARGUMENT_PRESENT(ExtraCreateParameters)) {
                ...
                // Check the parameters for creating a named pipe to ensure that no invalid parameters were passed.
                if ((NamedPipeCreateParameters->NamedPipeType > FILE_PIPE_MESSAGE_TYPE) ||// Check the NamedPipeType field to ensure that it is within range.
                    (NamedPipeCreateParameters->ReadMode > FILE_PIPE_MESSAGE_MODE) ||// Check the ReadMode field to ensure that it is within range.
                    (NamedPipeCreateParameters->CompletionMode > FILE_PIPE_COMPLETE_OPERATION) ||// Check the CompletionMode field to ensure that it is within range.

                    // Check the ShareAccess parameter to ensure that it does not specify shared delete access.
                    // The Named Pipe File System itself will need to ensure that at least one of SHARE_READ or
                    // SHARE_WRITE is specified if the first instance of the pipe is being created.
                    (ShareAccess & FILE_SHARE_DELETE) ||

                    // Check the Disposition parameter to ensure that is does not specify anything other than create, open, or open if.
                    (Disposition < FILE_OPEN || Disposition > FILE_OPEN_IF) ||

                    // Finally, check the CreateOptions parameter to ensure that it does not contain any invalid options for named pipes.
            ...
            if (!ARGUMENT_PRESENT(ExtraCreateParameters)) {
                ...
                // Check the parameters for creating a mailslot to ensure that no invalid parameters were passed.
                if ((ShareAccess & FILE_SHARE_DELETE) ||// Check the ShareAccess parameter to ensure that it does not specify shared delete access.
                    !(ShareAccess & ~FILE_SHARE_WRITE) ||// Check the ShareAccess parameter to ensure that shared write access is specified.
                    (Disposition != FILE_CREATE) ||// Check the Disposition parameter to ensure that it specifies that the file is to be created.

                    // Check the CreateOptions parameter to ensure that it does not contain any options that are invalid for mailslots.
    ...
    if (requestorMode != KernelMode) {
        // The caller's access mode is not kernel so probe each of the arguments and capture them as necessary.
        // If any failures occur, the condition handler will be invoked to handle them.
        // It will simply cleanup and return an access violation status code back to the system service dispatcher.
        ...
        try {
            ...
            ProbeForWriteIoStatus(IoStatusBlock);// The IoStatusBlock parameter must be writeable by the caller.

            // The AllocationSize parameter must be readable by the caller if it is present.  If so, probe and capture it.
        ...
        {
            // An exception was incurred while attempting to access the caller's parameters.
            // Simply return the reason for the exception as the service status.
            IopFreeOpenPacket(openPacket);
            return GetExceptionCode();
        }

        // Finally, if an EaBuffer was specified, ensure that it is readable from the caller's mode and capture it.
        if (ARGUMENT_PRESENT(EaBuffer) && EaLength) {
            ...
            try {
                ...
                // Walk the buffer and ensure that its format is valid.  Note that has been probed.
            ...
            {
                // An exception was incurred while attempting to access the caller's parameters.
                // Check to see whether or not an EA buffer was allocated and deallocate if so.
        ...
        // The caller's mode is kernel.
        // Copy the input parameters to their expected locations for later use.
        // Also, put move attach device flag where it belongs.
    ...
    // Now fill in an Open Packet (OP) to be used in calling the device object parse routine.
    // This packet will allow information to be passed between this routine and the parse routine so that a common context may be kept.
    // For most services this would be done with an I/O Request Packet (IRP),
    // but this cannot be done here because the number of stack entries which need to be allocated in the IRP is not yet known.
    ...
    openPacket->FinalStatus = STATUS_SUCCESS;// Assume that the operation is going to be successful.

    // Zero the file object field in the OP so the parse routine knows that this is the first time through.
    // For reparse operations it will continue to use the same file object that it allocated the first time.
    openPacket->FileObject = (PFILE_OBJECT)NULL;

    IopUpdateOtherOperationCount();// Update the open count for this process.

    // Attempt to open the file object by name.
    // This will yield the handle that the user is to use as his handle to the file in all subsequent calls, if it works.

    // This call performs a whole lot of the work for actually getting every- thing set up for the I/O system.
    // The object manager will take the name of the file and will translate it until it reaches a device object (or it fails).
    // If the former, then it will invoke the parse routine set up by the I/O system for device objects.
    // This routine will actually end up creating the file object, allocating an IRP, filling it in,
    // and then invoking the driver's dispatch routine with the packet.
    status = ObOpenObjectByName(ObjectAttributes, (POBJECT_TYPE)NULL, requestorMode, NULL, DesiredAccess, openPacket, &handle);

    // If an EA buffer was allocated, 
    // deallocate it now before attempting to determine whether or not the operation was successful so that it can be done in one place rather than in two places.
    if (openPacket->EaBuffer) {
        ExFreePool(openPacket->EaBuffer);
    }

    // Check the status of the open.  If it was not successful, cleanup and get out.
    // Notice that it is also possible, because this code does not explicitly request that a particular type of object (because the Object
    // Manager does not check when a parse routine is present and because the
    // name first refers to a device object and then a file object), a check must be made here to ensure that what was returned was really a file object.
    // The check is to see whether the device object parse routine believes that it successfully returned a pointer to a file object.
    // If it does, then OK;  otherwise, something went wrong somewhere.
    SuccessfulIoParse = (BOOLEAN)(openPacket->ParseCheck == OPEN_PACKET_PATTERN);
    if (!NT_SUCCESS(status) || !SuccessfulIoParse) {
        if (NT_SUCCESS(status)) {
            // The operation was successful as far as the object system is concerned,
            // but the I/O system device parse routine was never successfully completed so this operation has actually completed with an error because of an object mismatch.
            // Therefore, this is the wrong type of object so dereference whatever was actually referenced by closing the handle that was created for it.
            // We have to do a ZwClose as this handle can be a kernel handle if IoCreateFile was called by a driver.
            ZwClose(handle);
            status = STATUS_OBJECT_TYPE_MISMATCH;
        }

        // If the final status according to the device parse routine indicates that the operation was not successful, then use that
        // routine's final status because it is more descriptive than the status which was returned by the object manager.
        if (!NT_SUCCESS(openPacket->FinalStatus)) {
            ...
            //     1)  The parse routine was invoked at least once and a reparse was performed but the parse routine did not actually complete.
            //     2)  The parse routine was successful so everything worked but the object manager incurred an error after the parse routine completed.
            // For case #1, there is an outstanding file object that still exists.  This must be cleaned up.
            // For case #2, nothing must be done as the object manager has already dereferenced the file object.
            // Note that this code is not invoked if the parse routine completed with a successful status return (SuccessfulIoParse is TRUE).
        ...
        // When an NTFS file junction or an NTFS directory junction is traversed OBJ_MAX_REPARSE_ATTEMPTS namy times,
        // the object manager gives up and returns the code STATUS_OBJECT_NAME_NOT_FOUND.
        // This can happen in the following cases:
        //      1) One encounters a legal chain of directory junctions that happen to be longer than the value of the above constant.
        //      2) One encounters a self-referential file or directory junction that is, in effect, a tight name cycle.
        //      3) One encounters a name cycle composed of several NTFS junctions.
        // To improve on this return code see if  openPacket->Information  is the trace of an NTFS name junction.
        ...
        // At this point, the open/create operation has been successfully completed.
        // There is a handle to the file object, which has been created, and the file object has been signaled.

        // The remaining work to be done is to complete the operation.
        // This is performed as follows:
        //    1.  The file object has been signaled, so no work needs to be done for it.
        ...
        try {
            *FileHandle = handle;// Return the file handle.
    ...
    // If the parse routine successfully created a file object then dereference it here.
...
/*
Routine Description:
    This routine creates a named notification event for use in notifying different system components or drivers that an event occurred.
Arguments:
...
*/
{
    ...
    InitializeObjectAttributes(&objectAttributes, EventName, OBJ_OPENIF | OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    status = ZwCreateEvent(&eventHandle, EVENT_ALL_ACCESS, &objectAttributes, NotificationEvent, TRUE);// Now create or open the event.
    if (!NT_SUCCESS(status)) {
        return (PKEVENT)NULL;
    }

    // Reference the object by its handle to get a pointer that can be returned to the caller.
...
Routine Description:
    ...
    The input file object represents the file object that already exists for a file,
    and the newly created stream file object is used to access other parts of the file other than the data.
    Some uses of stream file objects are the EAs or the SECURITY_DESCRIPTORs on the file.
...
Arguments:
    ...
    DeviceObject - Pointer to the device object on which the stream file is to be opened.  
                   This pointer is not optional if the FileObject pointer is not specified.
...
*/
{
    ...
    // Begin by getting the device object from either the file object or the device object parameter.
    if (ARGUMENT_PRESENT(FileObject)) {
        DeviceObject = FileObject->DeviceObject;
    }

    // Increment the reference count for the target device object.
    // Note that no check is made to determine whether or
    // not the device driver is attempting to unload since this is an implicit open of a pseudo- file that is being made, not a real file open request.
    // In essence, no new file is really being opened.
    IopInterlockedIncrementUlong(LockQueueIoDatabaseLock, &DeviceObject->ReferenceCount);

    // Initialize the object attributes that will be used to create the file object.
    InitializeObjectAttributes(&objectAttributes, (PUNICODE_STRING)NULL, OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    ...
    // Insert the device object into the table.
    // Note that this is done w/a pointer bias so that the object cannot go away if some random user application closes the handle before this code is finished w/it.
    ...
    // The insert completed successfully.
    // Update the bookkeeping so that the fact that there is a handle is reflected.
    newFileObject->Flags |= FO_HANDLE_CREATED;
    ASSERT(newFileObject->Type == IO_TYPE_FILE);

    // Synchronize here with the file system to make sure that volumes don't go away while en route to the FS.
    if (DeviceObject->Vpb) {
        IopInterlockedIncrementUlong(LockQueueIoVpbLock, (PLONG)&DeviceObject->Vpb->ReferenceCount);
    }

    if (FileHandle == NULL) {// Finally, close the handle to the file. and clear the forward cluster
        ObCloseHandle(handle, KernelMode);
    } else {
        *FileHandle = handle;
        ObDereferenceObject(newFileObject);// Get rid of the reference in ObInsertObject.
    }
...
*/
{
    ...
    // if the driver has been marked for an unload or deleted operation, and the reference count goes to zero, 
    // then the driver may need to be unloaded or deleted at this point.
    // file that is being made, not a real file open request.
    // In essence, no new file is really being opened.
    IopInterlockedIncrementUlong(LockQueueIoDatabaseLock, &DeviceObject->ReferenceCount);
...
/*
Routine Description:
    This routine creates a named synchronization event for use in serialization of access to hardware between two otherwise non-related drivers.
    The event is created if it does not already exist, otherwise it is simply opened.
Arguments:
    EventName - Supplies the full name of the event.
    EventHandle - Supplies a location to return a handle to the event.
Return Value:
...
*/
{
    ...
    InitializeObjectAttributes(&objectAttributes, EventName, OBJ_OPENIF | OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);// Begin by initializing the object attributes.
    status = ZwCreateEvent(&eventHandle, EVENT_ALL_ACCESS, &objectAttributes, SynchronizationEvent, TRUE);// Now create or open the event.
    if (!NT_SUCCESS(status)) {
        return (PKEVENT)NULL;
    }
    ...
    *EventHandle = eventHandle;// Return the handle and the pointer to the event.
    return eventObject;
}
...
/*
Routine Description:
    This routine is invoked to assign an unprotected symbolic link name to a device.
    That is, a symbolic link that can be dynamically reassigned without any special authorization.
    The security descriptor is inherited from the container (like DosDevices) which has the right symbolic link.
Arguments:
    SymbolicLinkName - Supplies the symbolic link name as a Unicode string.
    DeviceName - Supplies the name to which the symbolic link name refers.
Return Value:
...
/*
Routine Description:
    This routine deletes the specified controller object from the system so that it may no longer be referenced from a driver.
    It is invoked when either the driver is being unloaded from the system, 
    or the driver's initialization routine failed to properly initialize the device or a fatal driver initialization error was encountered.
Arguments:
    ControllerObject - Pointer to the controller object that is to be deleted.
*/
{
    PAGED_CODE();

    // The controller was created as a temporary object, and all of the handles for the object have already been closed.
    // At this point, simply dereferencing the object will cause it to be deleted.
...
/*
Routine Description:
    This routine deletes the specified device object from the system so that it may no longer be referenced.
    It is invoked when either the device driver is being unloaded from the system, 
    or the driver's initialization routine failed to properly initialize the device or a fatal driver
    initialization error was encountered, or when the device is being removed from the system.
Arguments:
...
*/
{
    ...
    // Check to see whether or not the device has registered a shutdown handler if necessary, and if so, unregister it.
    if (DeviceObject->Flags & DO_SHUTDOWN_REGISTERED) {
        IoUnregisterShutdownNotification(DeviceObject);
    }

    // Release the pool that was allocated to contain the timer dispatch routine and its associated context if there was one.
    ...
    // If this device has a name, then mark the object as temporary so that when it is dereferenced it will be deleted.
    if (DeviceObject->Flags & DO_DEVICE_HAS_NAME) {
        ObMakeTemporaryObject(DeviceObject);
    }

    PoRunDownDeviceObject(DeviceObject);// PoRunDownDeviceObject will clean up any power management structures attached to the device object.
...
if (NT_SUCCESS(Status)) {
    // Since SessionId is a ULONG , the prefix (\\Sessions\\<SessionId>\\DosDevices) cannot be more that 128 characters in length
    ...
    while (TRUE) {
        ...
        // This generates session specific symbolic link path \Sessions\<id>\DosDevices\<LinkName>
        RtlInitUnicodeString(&UnicodeString, L"\\Sessions\\");
        ...
        // Begin by initializing the object attributes for the symbolic link.
        InitializeObjectAttributes(&Attributes, &SymbolicLinkName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
        ...
        if (NT_SUCCESS(Status)) {
            // The symbolic link was successfully opened.
            // Attempt to make it a temporary object, and then close the handle.
...
*/
{
    ...
    // Detach the device object attached to the target device. 
    // This also includes decrementing the reference count for the device. 
    // Note that if the driver has been marked for an unload operation, and the
    // reference count goes to zero, then the driver may need to be unloaded at this point.
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);

    // Tell the Special IRP code the stack has changed. Code that will reexamine the stack takes the database lock, so we can place the call here.
    // This also allows us to assert correct behavior *before* the stack is torn down.
...
/*
Routine Description:
    This routine disconnects all of the interrupt objects that were initialized and connected by the IoConnectInterrupt routine.
    Note that no interrupt objects directly connected using the kernel services may be input to this routine.
Arguments:
...
*/
{
    ...
    // Now loop through each of the interrupt objects pointed to by the structure and disconnect each.
    ...
    ExFreePool(interruptStructure);// Finally, deallocate the memory associated with the entire structure.
}
...
/*
Routine Description:
    This routine enqueues the specified I/O Request Packet (IRP) to the thread's IRP pending queue.
    The thread that the IRP is queued to is specified by the IRP's Thread field.
Arguments:
...
*/
{
    ...
    IopQueueThreadIrp(Irp);// Simply enqueue the IRP to the thread's IRP queue.
}
...
Routine Description:
    ...
    This involves a specialized interface between this function and the I/O system's device parse method.
    This allows the parse method to have the file system pseudo-open the file, obtain the appropriate attributes for the file,
    and return them to the caller w/as little overhead as possible from either the Object Manager or the I/O system itself.
Arguments:
...
*/
{
    ...
    // Build a parse open packet that tells the parse method to open the file and query its network attributes using the fast path,
    // if it exists for this file.
    ...
    // Open the object by its name.
    // Because of the special QueryOnly flag set in the open packet, 
    // the parse routine will open the file using the fast path open and perform the query, effectively closing it as well.
    status = ObOpenObjectByName(ObjectAttributes, (POBJECT_TYPE)NULL, KernelMode, NULL, DesiredAccess, &openPacket, &handle);

    // The operation is successful if the parse check field of the open packet indicates that the parse routine was actually invoked,
    // and the final status field of the packet is set to success.
    // The QueryOnly field is set to whether or not the fast path was invoked.
    if (openPacket.ParseCheck != OPEN_PACKET_PATTERN) {
        if (NT_SUCCESS(status)) {// The parse routine was not invoked properly so the operation did not work at all.
            ZwClose(handle);
            status = STATUS_OBJECT_TYPE_MISMATCH;
        }

        IoStatus->Status = status;
    } else {// The fast path routine was successfully invoked so return the final status of the operation.
...
Routine Description:
    ...
    No checks are made to ensure that the controller is really allocated to a device object.
    However, if it is not, then kernel will bugcheck.

    If another device is waiting in the queue to allocate the controller object it will be pulled from the queue and its execution routine will be invoked.
Arguments:
...
*/
{
    ...
    if (packet != NULL) {
        ...
        action = deviceObject->Queue.Wcb.DeviceRoutine(deviceObject, deviceObject->CurrentIrp, 0, deviceObject->Queue.Wcb.DeviceContext);
        if (action == DeallocateObject) {// If the execution routine wants the controller to be deallocate now, deallocate it.
            IoFreeController(ControllerObject);
        }
    }
}
...
*/
{
    ...
    // Ensure that all of the owners of the IRP have at least been notified that the request is going away.
    ...
    if (!(Irp->AllocationFlags & IRP_ALLOCATED_FIXED_SIZE) || (Irp->AllocationFlags & IRP_ALLOCATED_MUST_SUCCEED)) {
        ...
        if (IopIrpAutoSizingEnabled() && (Irp->Size != IoSizeOfIrp(IopLargeIrpStackLocations)) && (Irp->Size != IoSizeOfIrp(1))) {
            ExFreePool(Irp);
            return;
        }
        ...
        if (ExQueryDepthSList(&lookasideList->ListHead) >= lookasideList->Depth) {
            ...
            if (ExQueryDepthSList(&lookasideList->ListHead) >= lookasideList->Depth) {
                ...
                InterlockedPushEntrySList(&lookasideList->ListHead, (PSLIST_ENTRY)Irp);
            }
        } else {
            if (Irp->AllocationFlags & IRP_QUOTA_CHARGED) {
                Irp->AllocationFlags ^= IRP_QUOTA_CHARGED;
...
Routine Description:
    ...
    It only frees the specified MDL; any chained MDLs must be freed explicitly through another call to this routine.
Arguments:
    Mdl - Pointer to the Memory Descriptor List to be freed.
*/
{
    // Tell memory management that this MDL will be re-used.
    // This will cause MM to unmap any pages that have been mapped for this MDL if it is a partial MDL.
...
Routine Description:
    ...
    N.B. Caller must own the IopDatabaseLock. External callers of this function must ensure nobody is attaching or detaching from the stack.
    If they cannot, they *must* use IoGetAttachedDeviceReference.
Arguments:
    DeviceObject - Supplies a pointer to the device for which the attached device is to be returned.
Return Value:
    The function value is the highest level device attached to the specified device.
    If no devices are attached, then the pointer to the device object itself is returned.
*/
{
    // Loop through all of the device object's attached to the specified device.
    // When the last device object is found that is not attached to, return it.
...
/*
Routine Description:
    This routine synchronizes with the Io database and returns a reference to the highest level device object associated with the specified device.
Arguments:
    DeviceObject - Supplies a pointer to the device for which the attached device is to be returned.
Return Value:
    The function value is a reference to the highest level device attached to the specified device.
    If no devices are attached, then the pointer to the device object itself is returned.
*/
{
    KIRQL               irql;
...
/*
Routine Description:
    This routine returns the base (lowest-level) file system volume device object associated with a file.
    I.e., it locates the file system w/o walking the attached device object list.
Arguments:
    FileObject - Supplies a pointer to the file object for which the base file system device object is to be returned.
Return Value:
    The function value is the lowest level volume device object associated w/the file.
*/
{
    PDEVICE_OBJECT deviceObject;
    ...
    } else {// Otherwise, just return the real device object.
        deviceObject = FileObject->DeviceObject;
    }

    ASSERT(deviceObject != NULL);

    return deviceObject;// Simply return the resultant file object.
}
...
/*
Routine Description:
    This routine returns a pointer to the system's device configuration information structure so that drivers and
    the system can determine how many different types of devices exist in the system.
Return Value:
...
*/
{
    ...
    return (&ConfigurationInformation);// Simply return a pointer to the built-in configuration information structure.
}
...
/*
Routine Description:
    This routine returns a pointer to the current process.
    It is actually a jacket routine for the PS version of the same function since device drivers using the ntddk header file cannot see into a thread object.
Return Value:
    The function value is a pointer to the current process.
Note:
    Note that this function cannot be paged because it is invoked at raised IRQL in debug builds,
    which is the only time that the PAGED_CODE macro actually does anything.
    Therefore, it is impossible to find code that invokes this function at raised IRQL in a normal system w/o simply running into the "proper conditions".
    This is too risky to actually page this routine, so it is left nonpaged.
*/
{
    return PsGetCurrentProcess();// Simply return a pointer to the current process.
}
...
Routine Description:
    ...
    It also returns a pointer to the referenced file object that has been opened to the device that ensures that the device cannot go away.
...
Return Value:
    The function value is a referenced pointer to the specified device object, if the device exists.
...
*/
{
    ...
    if (NT_SUCCESS(status)) {// The open operation was successful.
        ...
        if (NT_SUCCESS(status)) {
            ...
            *DeviceObject = IoGetRelatedDeviceObject(fileObject);// Get a pointer to the device object for this file.
        }
...
Routine Description:
    ...
    The pointer is set in the thread object by a device driver when the disk or CD-ROM media appears to have changed since the last access to the device.
Arguments:
...
Routine Description:
    ...
    This extension was allocated using IoAllocateDriverObjectExtension.
    If an extension with the create Id does not exist for the specified driver object then NULL is returned.
Arguments:
    DriverObject - Pointer to driver object owning the extension.
    ClientIdentificationAddress - Supplies the unique identifier which was used to create the extension.
Return Value:
    The function value is a pointer to the client driver object extension, or NULL.
*/
{
    KIRQL irql;
    PIO_CLIENT_EXTENSION extension;
...
*/
{
    PAGED_CODE()
        ...
        return (PGENERIC_MAPPING)&IopFileMapping;// Simply return a pointer to the generic mapping for a file object.
}
...
/*
Routine Description:
    This routine returns the base initial address of the current thread's stack.
Return Value:
...
Routine Description:
    ...
    This can be a thread stack or a DPC stack depending on the execution context.
Arguments:
...
Return Value:
    None.
    For any kind of error both LowLimit and HighLimit will be zeroed.
    For example this will happen if the driver illegally switched to a different stack.
Note:
...
*/
{
    ...
    // The local variable is marked volatile so that the compiler will not get any ideas to optimize its use.
    // We really need this to be stack based so that its address is within current stack limits.
    StackAddress = (ULONG_PTR)(&StackAddress);

    // Get the stack associated with current thread.
    // This call is not aware of DPCs. It will just read stack limits from current thread control block.
    ...
    if ((StackAddress < *LowLimit) || (StackAddress > *HighLimit)) {
        ...
        // This would signal error for stacks switched without kernel's knowledge.
        // However there are drivers switching stacks like this and they get confused by the zeroed limits.
        // Therefore we do not do it.
        if (KeGetCurrentIrql() >= DISPATCH_LEVEL) {
            PKPRCB Prcb = KeGetCurrentPrcb();
            ULONG_PTR DpcStack = (ULONG_PTR)(Prcb->DpcStack);

            if ((Prcb->DpcRoutineActive) && (StackAddress <= DpcStack) && (StackAddress >= DpcStack - KERNEL_STACK_SIZE)) {
                *HighLimit = DpcStack;
...
NTSTATUS IoComputeDesiredAccessFileObject(IN PFILE_OBJECT FileObject, OUT PNTSTATUS DesiredAccess)
/*
Routine Description
...
/*
Routine Description:
    This routine returns a pointer to the actual device object than an I/O Request Packet (IRP) should be given to based on the specified file object.

    N.B. - Callers of this function must ensure no device object is attaching or detaching from this stack for the duration of this call.
    This is because the database lock is *not* held!
Arguments:
    FileObject - Pointer to the file object representing the open file.
Return Value:
    The return value is a pointer to the device object for the driver to whom the request is to be given.
*/
{
    PDEVICE_OBJECT deviceObject;

    // If the file object was taken out against the mounted file system, it will have a Vpb. Traverse it to get to the DeviceObject.
    // Note that in this case we should never follow FileObject->DeviceObject, as that mapping may be invalid after a forced dismount.
    if (FileObject->Vpb != NULL && FileObject->Vpb->DeviceObject != NULL) {
        ASSERT(!(FileObject->Flags & FO_DIRECT_DEVICE_OPEN));
        deviceObject = FileObject->Vpb->DeviceObject;

        // If a driver opened a disk device using direct device open and later on it uses IoGetRelatedTargetDeviceObject to find the
        // device object it wants to send an IRP then it should not get the filesystem device object.
        // This is so that if the device object is in the process of being mounted then vpb is not stable.
    } else if (!(FileObject->Flags & FO_DIRECT_DEVICE_OPEN) &&
               FileObject->DeviceObject->Vpb != NULL &&
               FileObject->DeviceObject->Vpb->DeviceObject != NULL) {
        deviceObject = FileObject->DeviceObject->Vpb->DeviceObject;

        // This is a direct open against the device stack (and there is no mounted file system to strain the IRPs through).
    } else {
        deviceObject = FileObject->DeviceObject;
    }
    ...
    // If so, return the highest level device; otherwise, return a pointer to the device object itself.
...
/*
Routine Description:
    This routine returns a 32-bit value that is unique to the process that originally requested the specified I/O operation.
Arguments:
...
*/
{
    ...
    if (process != NULL) {
        return HandleToUlong(process->UniqueProcessId);// UniqueProcessId is a kernel-mode handle, safe to truncate to ULONG.
    } else {
        return 0;// Return a PID of zero if there is no process associated with the IRP.
    }
}
...
/*
Routine Description:
    This routine returns a pointer to the process that originally requested the specified I/O operation.
Arguments:
    Irp - Pointer to the I/O Request Packet.
Return Value:
...
*/
{
    ...
    if (thread) {
        // The thread was not attached when the IRP was issued. So get the original process.
        // Note that this API could be called from another process or thread.
        if (Irp->ApcEnvironment == OriginalApcEnvironment) {
            return (THREAD_TO_PROCESS(thread));

            // The thread was attached when the IRP was issued.
            // In this case give the process to which the thread is currently attached.
            // Note that this only works if the thread that issued the IO request while it was attached does not attach again. This is not allowed.
...
/*
Routine Description:
    This routine returns the contents of the TopLevelIrp field of the current thread.
Return Value:
...
*/
{
    return (PIRP)(PsGetCurrentThread()->TopLevelIrp);// Simply return the TopLevelIrp field of the thread.
}
...
Routine Description:
    ...
    It does what IoInitializeIrp does but it saves the allocation flags so that we know how to free the Irp and take care of quote requirements.
    Drivers should call IoReuseIrp instead of calling IoInitializeIrp to reinitialize an IRP.
Arguments:
...
*/
{
    ...
    ASSERT(Irp->CancelRoutine == NULL);// Did anyone forget to pull their cancel routine?

    // We probably don't want thread enqueue'd IRPs to be used ping-pong style as they cannot be dequeue unless they complete entirely.
    // Not really an issue for worker threads, but definitely for operations on application threads.
    ASSERT(IsListEmpty(&Irp->ThreadListEntry));
...
*/
{
    ...
    if (!timer) {
        ...
        // Initialize the timer entry so that it is suitable for being placed into the I/O system's timer queue.
    ...
    // Set the address of the driver's timer routine and the context parameter passed to it and insert it onto the timer queue.
    // Note that the timer enable flag is not set, so this routine will not actually be invoked yet.
...
/*
Routine Description:
    This routine determines whether an I/O operation is to be considered synchronous or an asynchronous, from the implementors point-of-view.
    Synchronous I/O is defined by how the file was opened, or the API being used to perform the operation,
    or by the type of paging I/O being performed, if the operation is paging I/O.

    It is possible for asynchronous paging I/O to occur to a file that was opened for synchronous I/O.
    This occurs when the Modified Page Writer is doing I/O to a file that is mapped, when too many modified pages exist in the system.
Arguments:
    Irp - Pointer to the I/O Request Packet (IRP) representing the operation to be performed.
Return Value:
    A value of TRUE is returned if the operation is synchronous, otherwise FALSE is returned.
*/
{
    // Determine whether this is a synchronous I/O operation.  Synchronous I/O is defined as an operation that is:
    ...
    if ((IoGetCurrentIrpStackLocation(Irp)->FileObject->Flags & FO_SYNCHRONOUS_IO || Irp->Flags & IRP_SYNCHRONOUS_API ||
         Irp->Flags & IRP_SYNCHRONOUS_PAGING_IO) && !(Irp->Flags & IRP_PAGING_IO && !(Irp->Flags & IRP_SYNCHRONOUS_PAGING_IO))) {
        return TRUE;
    } else {
        return FALSE;
    }
...
Return Value:
    A value of TRUE is returned if the indicated thread is a system thread, else FALSE.
Note:
    This function cannot be made a macro, since fields in the thread object move from release to release, so this must remain a full function.
*/
{
...
/*
Routine Description:
    This routine returns a BOOLEAN indicating whether or not the specified buffer is a valid name grafting buffer.
    All internal validity checks are encapsulated in this routine.
    Among the checks performed is whether the name lengths stored within the buffer in the private data section are compatible with the total size of the buffer that has been passed in.
Arguments:
    Irp - Pointer to the I/O Request Packet (IRP) representing the operation to be performed.
    Buffer - Pointer to a reparse data buffer that is supposed to contain a self-consistent set of names to perform name grafting.
Return Value:
    A value of TRUE is returned if the buffer is correct for name grafting, else FALSE.
Note:
    This function needs to be kept synchronized with the definition of REPARSE_DATA_BUFFER.
*/
{
    ...
    if (ReparseBuffer->ReparseTag != IO_REPARSE_TAG_MOUNT_POINT) {
        return FALSE;// The reparse tag is not an NT name grafting tag.
    }
    ...
    if (ReparseBuffer->ReparseDataLength < 
        ...
        // The buffer is shorter than the minimum needed to express a pair of valid names.
        return FALSE;
    }

    thisStackPointer = IoGetCurrentIrpStackLocation(Irp);// Get the address of the current stack location.

    // Determine whether the data lengths returned are consistent with the buffer in which they are retrieved.

    // This check is meaningful only when the buffer has been allocated.
    // When this routine is used when a name grafting is being set there is no allocated output buffer.
    if ((thisStackPointer->Parameters.FileSystemControl.OutputBufferLength > 0) &&
        ...
        // The length of the appropriate buffer header, 
        // plus the lengths of the substitute and print names are longer than the length of the buffer passed in.
        // Thus, this data is not self-consistent.

        // Note that it is only the I/O subsystem that needs to check for this internal consistency in the buffer as it will do a blind data copy using these lengths when transmogrifying the names.
        // The file system returning the buffer only needs to ascertain that the total size of the data retrieved does not exceed the size of the output buffer.
        return FALSE;
    }

    // Now we determine whether the names were placed according to the reparse point specification.

    // Determine whether the SubstituteNameOffset is zero.
    if (ReparseBuffer->MountPointReparseBuffer.SubstituteNameOffset != 0) {
        return FALSE;// Incorrect offset for the substitute name.
    }
    ...
    if (ReparseBuffer->MountPointReparseBuffer.PrintNameOffset != 
        ...
        return FALSE;// Incorrect offset for the print name.
    }
    ...
    {
        // This conditional is a transcription of part of the code of RtlDetermineDosPathNameType_U present in ntos\dll\curdir.c
        ...
        if ((ReparseBuffer->MountPointReparseBuffer.SubstituteNameLength > 6) &&
            ...
            (ReparseBuffer->MountPointReparseBuffer.PathBuffer[2] == L'?'))) {
            return FALSE;// The name is not one we want to deal with.
        }

        // When  RtlDosPathNameToNtPathName_U  is used, the UNC names are returned with a prefix of the form  \??\UNC\
        ...
        if ((ReparseBuffer->MountPointReparseBuffer.SubstituteNameLength > 12) &&
            ...
            if (NT_SUCCESS(status)) {// Now query the link and see if there is a redirection
                linkValueBuffer = ExAllocatePoolWithTag(NonPagedPool, 2 * 260, '  oI');
                if (!linkValueBuffer) {
                    ZwClose(linkHandle);
                    return FALSE;// Insufficient resources. Return FALSE.
                }
                ...
                ZwClose(linkHandle);
                if (NT_SUCCESS(status)) {
                    // The link is a re-directed drive when it has the prefix \Device\LanmanRedirector\
                    ...
                    {
                    ExFreePool(linkValueBuffer);// Free the buffer.
                    return FALSE;
                    }
                }
    ...
    // This closes the door of having an arbitrary device name that, with the help of the server,
    // can be used to bypass access checks to the underlying device.
    {
        UNICODE_STRING volumeName;

        if (
            // The shortest valid name is one of the kind \??\C: whose length is 12 when in Unicode.
            // All names used by volume mount points are longer.
        ...
        // We have verified above that MountPointReparseBuffer.SubstituteNameOffset is zero.
        ...
        // When we do not have a name that begins with a drive letter and it is not a valid volume mount point name then we return false.
...
Arguments:
    ...
    ReparseBuffer - Pointer to a reparse data buffer that is supposed to contain a self-consistent set of names to perform name grafting.
Return Value:
...
*/
{
    ...
    // We do the appropriate paste of the new name in the FileName buffer and deallocate the buffer that brought the data from the file system.
    ...
    // Determine whether the lengths returned are consistent with the maximum buffer.
    // This is the best self-defense check we can do at this time as the stack pointer is already invalid.
    if (ReparseBuffer->ReparseDataLength >= 
        (FIELD_OFFSET(REPARSE_DATA_BUFFER, MountPointReparseBuffer.PathBuffer[0]) - REPARSE_DATA_BUFFER_HEADER_SIZE)) {
    ...
    // The value in  ReparseBuffer->Reserved  is the length of the file name that has still to be parsed.
    ...
    if (NT_SUCCESS(Irp->IoStatus.Status)) {
        pathBuffer = (PWSTR)((PCHAR)ReparseBuffer->MountPointReparseBuffer.PathBuffer + ReparseBuffer->MountPointReparseBuffer.SubstituteNameOffset);
        pathLength = ReparseBuffer->MountPointReparseBuffer.SubstituteNameLength;
    }

    // Notice that if the data returned in AuxiliaryBuffer is not long enough then pathLength has value 0 and pathBuffer has value NULL.
    // The value in  ReparseBuffer->Reserved  is the length of the file name that has still to be parsed.
    // We only care to do this if we have no error conditions.
    if (ReparseBuffer->Reserved < 0) {
        Irp->IoStatus.Status = STATUS_IO_REPARSE_DATA_INVALID;// This is an invalid offset.
    }

    if (NT_SUCCESS(Irp->IoStatus.Status)) {
        // Check for overflow. (pathLength <= MAXIMUM_REPARSE_DATA_BUFFER_SIZE) so pathLength + sizeof(UNICODE_NULL) cannot overflow.
    ...
    if (NT_SUCCESS(Irp->IoStatus.Status)) {
        ...
        // Copy into the front of the out name buffer the value of the reparse point.
        if (pathLength) {
...
/*
Routine Description:
    This routine allocates an I/O Request Packet from the system nonpaged pool and makes it an associated IRP to the specified IRP.
    The packet will be allocated to contain StackSize stack locations.
    The IRP will also be initialized.

    Note that it is up to the caller to have set the number of associated IRPs in the master packet before calling this routine for the first time.
    The count should be set in the master packet in:  AssociatedIrp.IrpCount.
Arguments:
    Irp - Pointer to master IRP to be associated with.
    StackSize - Specifies the maximum number of stack locations required.
Return Value:
    The function value is the address of the associated IRP or NULL, if the IRP could be allocated.
*/
{
    USHORT allocateSize;
    UCHAR fixedSize;
    ...
    if (StackSize <= largeIrpStackLocations) {
        ...
        if (IopIrpAutoSizingEnabled() && associatedIrp) {
            // See if this IRP is a stale entry. If so just free it.
            // This can happen if we decided to change the lookaside list size.
            // We need to get the size of the IRP from the information field as the size field is overlaid with the single list entry.
            if (associatedIrp->IoStatus.Information < packetSize) {
                lookasideList->TotalFrees += 1;
                ExFreePool(associatedIrp);
                associatedIrp = NULL;
            } else {// Update allocateSize to the correct value.
                allocateSize = (USHORT)associatedIrp->IoStatus.Information;
            }
        }
    }

    // If an IRP was not allocated from the lookaside list, then allocate the packet from nonpaged pool.
    if (!associatedIrp) {
        // There are no free packets on the lookaside list, or the packet is too large to be allocated from one of the lists,
        // so it must be allocated from general non-paged pool.
    ...
    associatedIrp->Tail.Overlay.Thread = Irp->Tail.Overlay.Thread;// Set the thread ID to be that of the master.
    associatedIrp->AssociatedIrp.MasterIrp = Irp;// Now make the association between this packet and the master.
...
Routine Description:
    ...
    All of the special handling for this request is recognized by setting the IRP_PAGING_IO flag in the IRP flags word.
    In-page operations are detected by using the IRP flag IRP_INPUT_OPERATION.
Arguments:
    FileObject - A pointer to a referenced file object describing which file the read should be performed from.
    MemoryDescriptorList - An MDL which describes the physical pages that the pages should be read into from the disk.
        All of the pages have been locked in memory.
        The MDL also describes the length of the read operation.

        If the low bit of the MDL is set, then the I/O is issued asynchronously, otherwise it is done synchronously.
    StartingOffset - Pointer to the offset in the file from which the read should take place.
    Event - A pointer to a kernel event structure to be used for synchronization purposes.
        The event will be set to the Signaled state once the in-page operation completes.
    IoStatusBlock - A pointer to the I/O status block in which the final status and information should be stored.
Return Value:
    The function value is the final status of the queue request to the I/O system subcomponents.
Notes:
    This routine is invoked at APC_LEVEL; this level is honored throughout the execution of the entire I/O request, including completion.
*/
{
    PIRP irp;
    PIO_STACK_LOCATION irpSp;
    PDEVICE_OBJECT deviceObject = IoGetRelatedDeviceObject(FileObject);// Begin by getting a pointer to the device object that the file resides on.

    irp = IoAllocateIrp(deviceObject->StackSize, FALSE);// Allocate an I/O Request Packet (IRP) for this in-page operation.
    if (!irp) {
    ...
    IoStatusBlock->Information = (ULONG_PTR)irp;// For debugging purposes.

    // Increment performance counter.  The Cache Manager I/Os always are "recursive".
    if (MmIsRecursiveIoFault()) {
        *CcMissCounter += (MemoryDescriptorList->ByteCount + PAGE_SIZE - 1) >> PAGE_SHIFT;
    }

    return IoCallDriver(deviceObject, irp);// Queue the packet to the appropriate driver based on whether or not there is a VPB associated with the device.
}
...
Routine Description:
    ...
    The information returned is determined by the FileInformationClass that is specified, and it is placed into the caller's FileInformation buffer.
Arguments:
    FileObject - Supplies a pointer to the file object about which the requested information is returned.
    FileInformationClass - Specifies the type of information which should be returned about the file.
    Length - Supplies the length, in bytes, of the FileInformation buffer.
    FileInformation - Supplies a buffer to receive the requested information returned about the file. 
                      This buffer must not be pageable and must reside in system space.
    ReturnedLength - Supplies a variable that is to receive the length of the information written to the FileInformation buffer.
Return Value:
...
*/
{
    ...
    return IopQueryXxxInformation(FileObject, FileInformationClass, Length, KernelMode, FileInformation, ReturnedLength, TRUE);
}
...
Routine Description:
    ...
    The information returned is determined by the FsInformationClass that is specified, 
    and it is placed into the caller's FsInformation buffer.
Arguments:
    FileObject - Supplies a pointer to the file object about which the requested information is returned.
    FsInformationClass - Specifies the type of information which should be returned about the volume.
    Length - Supplies the length of the FsInformation buffer in bytes.
    FsInformation - Supplies a buffer to receive the requested information returned about the file. 
                    This buffer must not be pageable and must reside in system space.
    ReturnedLength - Supplies a variable that is to receive the length of the information written to the FsInformation buffer.
Return Value:
    The status returned is the final completion status of the operation.
*/
{
    ...
    // Simply invoke the common routine to perform the query operation.
    return IopQueryXxxInformation(FileObject, FsInformationClass, Length, KernelMode, FsInformation, ReturnedLength, FALSE);
}
...
/*
Routine Description:
    This routine queues the specified I/O Request Packet (IRP) to the current thread's IRP pending queue.
    This queue locates all of the outstanding I/O requests for the thread.
Arguments:
    Irp - Pointer to the I/O Request Packet (IRP) to be queued.
*/
{
    IopQueueThreadIrp(Irp);// Simply queue the packet using the internal queueing routine.
}
...
/*
Routine Description:
    This routine pops up a hard error in the context of the thread that originally requested the I/O operation specified by the input IRP.
    This is done by queueing a kernel APC to the original thread, passing it a pointer to the device objects and the IRP.
    Once the pop up is performed, the routine either completes the I/O request then, or it calls the driver back with the same IRP.

    If the original request was an IoPageRead, then it was at APC level and we have to create a thread to "hold" this pop-up.
    Note that we have to queue to an ExWorker thread to create the thread since this can only be done from the system process.
Arguments:
    Irp - A pointer to the I/O Request Packet (IRP) for the request that failed.
    Vpb - This is the volume parameter block of the offending media.  If the media not yet mounted, this parameter should be absent.
    RealDeviceObject - A pointer to the device object that represents the device that the file system believes it has mounted.
        This is generally the "real" device object in the VPB, but may, in fact, be a device object attached to the physical device.
*/
{
    PIO_STACK_LOCATION IrpSp;
    ULONG              irpFlags;
    ...
    // If pop-ups are disabled for the requesting thread, just complete the request.
    Thread = Irp->Tail.Overlay.Thread;
    if ((Thread->CrossThreadFlags&PS_CROSS_THREAD_FLAGS_HARD_ERRORS_DISABLED) != 0) {
        // An error was incurred, so zero out the information field before completing the request if this was an input operation.
    ...
    //  If this Irp resulted from a call to IoPageRead(), the caller must have been at APC level, so don't try enqueing an APC.
    //  Also if this is a cleanup Irp, force this pop-up to go to the new thread so that it cannot be disabled.
    IrpSp = IoGetCurrentIrpStackLocation(Irp);
    irpFlags = Irp->Flags & (~IRP_VERIFIER_MASK);
    if ((irpFlags == (IRP_PAGING_IO | IRP_NOCACHE | IRP_SYNCHRONOUS_PAGING_IO | IRP_INPUT_OPERATION)) ||
        ...
        // Need to check here whether hard errors are disabled for the thread as we will be in system context later and it will be too late
        if ((IrpSp->MajorFunction == IRP_MJ_FILE_SYSTEM_CONTROL) && !IS_SYSTEM_THREAD(Thread)) {
            NTSTATUS status;
            ULONG hardErrorMode;
            SIZE_T bytesCopied;
            BOOLEAN fail = TRUE;
            ...
            } else {// The issuing thread is in another process so we need to read the TEB from that processes memory
        ...
        PKAPC apc = ExAllocatePoolWithTag(NonPagedPool, sizeof(KAPC), 'CPAK');// Begin by allocating and initializing an APC that can be sent to the target thread.
        if (apc == NULL) {// If we could not get the pool, we have no choice but to just complete the Irp, thereby passing the error onto the caller.
            IoCompleteRequest(Irp, IO_DISK_INCREMENT);
            return;
        }
        KeInitializeApc(apc, &Thread->Tcb, Irp->ApcEnvironment, IopDeallocateApc, IopAbortRequest, IopRaiseHardError, KernelMode, Irp);
...
/*
Routine Description:
    This routine pops up a hard error in the hard error popup thread.
    The routine returns immediately, enqueuing the actual pop-up to a worker thread.
    The hard error that is raised is informational in the sense that only the OK button is displayed.
Arguments:
    ErrorStatus - The error condition.
    String - Depending on the error, a string may have to be enqueued.
    Thread - If present, enqueue an APC to this thread rather than using the hard error thread.
Return Value:
    BOOLEAN - TRUE if we decided to dispatch a pop-up.  FALSE if we decided not to because:
        - pop-ups are disabled in the requested thread, or
        - a pool allocation failed, or
        - an equivalent pop-up is currently pending a user response (i.e. waiting for the user to press <OK>) or in the queue, or
...
#define ArePacketsEquivalent(P1,P2) (                              \
    (P1->ErrorStatus == P2->ErrorStatus) && ((!P1->String.Buffer && !P2->String.Buffer) ||                 \
   ((P1->String.Length == P2->String.Length) && (RtlEqualMemory(P1->String.Buffer, P2->String.Buffer, P1->String.Length)))))
{
    KIRQL oldIrql;
    PVOID stringBuffer;
    PLIST_ENTRY links;
    ...
    //  If there is an Thread, enqueue an APC for ourself, otherwise send it off to the to the hard error thread.
    if (ARGUMENT_PRESENT(Thread)) {
        PKAPC apc = ExAllocatePoolWithTag(NonPagedPool, sizeof(KAPC), 'CPAK');// Begin by allocating and initializing an APC that can be sent to the target thread.
        if (apc == NULL) {// If we could not get the pool, we have no choice but to just free the packet and return.
        ...
        //  Bump the count on the semaphore so that the hard error thread will know that an entry has been placed in the queue.
        (VOID)KeReleaseSemaphore(&IopHardError.WorkQueueSemaphore, 0, 1L, FALSE);

        //  If we are not currently running in an ExWorkerThread, queue a work item.
        ...
        //  The combination of releasing both the event and the semaphore will enable the thread to wake up and obtain the entry.
        ExReleaseSpinLock(&IopHardError.WorkQueueSpinLock, oldIrql);
    }
...
Arguments:
    ...
    DriverReinitializationRoutine - The address of the reinitialization routine that is to be invoked.
    Context - Pointer to the context that is passed to the driver's reinitialization routine.
*/
{
    ...
    // Allocate a reinitialization entry to be inserted onto the list.
    // Note that if the entry cannot be allocated, then the request is simply dropped on the floor.
...
*/
{
    ...
    // Insert the device object into the appropriate file system queue based on the driver type in the device object.
    // Notice that if the device type is unrecognized, the file system is simply not registered.
    ...
    //  Low priority filesystems are inserted one-from-back on the queue (ahead of raw, behind everything else), as opposed to on the front.
    if (listHead != NULL) {
    ...
    DeviceObject->Flags &= ~DO_DEVICE_INITIALIZING;// Ensure that this file system's device is operable.
    ...
    ExReleaseResourceLite(&IopDatabaseResource);// Release the I/O database resource.
    IopInterlockedIncrementUlong(LockQueueIoDatabaseLock, &DeviceObject->ReferenceCount);// Increment the number of reasons that this driver cannot be unloaded.
}
...
*/
{
    ...
    // Begin by attempting to allocate storage for the shutdown packet.
    // If one cannot be allocated, simply return an appropriate error.
...
/*
Routine Description:
    This routine allows a driver to register that it would like to have its shutdown routine invoked at very late in system shutdown.
    This gives the driver an opportunity to get control just before the system is fully shutdown.
Arguments:
...
*/
{
    ...
    // Note that this is done because some drivers have dependencies on LIFO notification ordering.
    ...
    // Do the bookkeeping to indicate that this driver has successfully registered a shutdown notification routine.
...
/*
Routine Description:
    This routine allows a driver to register that it would like to have its shutdown routine invoked when the system is being shutdown.
    This gives the driver an opportunity to get control just before the system is fully shutdown.
Arguments:
    DeviceObject - Pointer to the driver's device object.
Return Value:
...
*/
{
    KeReleaseQueuedSpinLock(LockQueueIoCancelLock, Irql);// Simply release the cancel spin lock.
}
...
/*
Routine Description:
    This routine is invoked to release the Volume Parameter Block (VPB) spin lock.
    This spin lock must be acquired before accessing the mount flag, reference count, and device object fields of a VPB.
Arguments:
    Irql - Supplies the IRQL value returned from acquiring the spin lock.
*/
{
    KeReleaseQueuedSpinLock(LockQueueIoVpbLock, Irql);// Simply release the VPB spin lock.
}
...
/*
Routine Description:
    This routine is invoked to remove the access and share access information in a file system Share Access structure for a given open instance.
Arguments:
...
*/
{
    ...
    // If this accessor wanted some type of access other than READ_ or WRITE_ATTRIBUTES, then account for the fact that he has closed the file.
    // Otherwise, he hasn't been accounted for in the first place so don't do anything.
    ...
    if (FileObject->ReadAccess || FileObject->WriteAccess || FileObject->DeleteAccess) {
        ...
        // For each access type, decrement the appropriate count in the Share Access structure.
        ...
        // For each shared access type, decrement the appropriate count in the Share Access structure.
...
/*
Routine Description:
    This routine sets the device to verify field in the thread object.
    This function is invoked by file systems to NULL this field, or to set it to predefined values.
Arguments:
...
*/
{
    Thread->DeviceToVerify = DeviceObject;// Simply set the device to be verified in the specified thread.
}
...
/*
Routine Description:
    This routine is invoked when a driver realizes that the media has possibly changed on a device,
    and it must be verified before continuing, or a hard error has occured.
    The device is stored in the thread local storage of the Irp's originating thread.
Arguments:
...
*/
{
    ...
    // Its a defensive check to catch drivers that pass all kinds of IRPs to this routine.
    if (!Irp->Tail.Overlay.Thread) {
        return;
    }

    // Store the address of the device object that needs verification in the appropriate field of the thread pointed to by the specified I/O Request Packet.
...
Routine Description:
    ...
    The information that is set is determined by the FileInformationClass parameter, 
    and the information itself is passed in the FileInformation buffer.
Arguments:
    FileObject - Supplies a pointer to the file object for the file that is to be changed.
    FileInformationClass - Specifies the type of information that should be set on the file.
    Length - Supplies the length of the FileInformation buffer in bytes.
    FileInformation - A buffer containing the file information to set.  This buffer must not be pageable and must reside in system space.
Return Value:
    The status returned is the final completion status of the operation.
*/
{
    ...
    // Reference the file object here so that no special checks need be made in I/O completion to determine whether or not to dereference the file object.
    ObReferenceObject(FileObject);

    // Make a special check here to determine whether this is a synchronous I/O operation.
    // If it is, then wait here until the file is owned by the current thread.
    // If this is not a (serialized) synchronous I/O operation, then initialize the local event.
    if (FileObject->Flags & FO_SYNCHRONOUS_IO) {
        BOOLEAN interrupted;
        if (!IopAcquireFastLock(FileObject)) {
            status = IopAcquireFileObjectLock(FileObject, KernelMode, (BOOLEAN)((FileObject->Flags & FO_ALERTABLE_IO) != 0), &interrupted);
            if (interrupted) {
                ObDereferenceObject(FileObject);
                return status;
            }
    ...
    deviceObject = IoGetRelatedDeviceObject(FileObject);// Get the address of the target device object.

    // Allocate and initialize the I/O Request Packet (IRP) for this operation.
    // The allocation is performed with an exception handler in case there is not enough memory to satisfy the request.
    ...
    // Get a pointer to the stack location for the first driver.  This will be used to pass the original function codes and parameters.
    irpSp = IoGetNextIrpStackLocation(irp);
    irpSp->MajorFunction = IRP_MJ_SET_INFORMATION;
    irpSp->FileObject = FileObject;

    // Set the system buffer address to the address of the caller's buffer and set the flags so that the buffer is not deallocated.
    ...
    // However, it is possible that the information that the caller wants to set is device independent (I/O system dependent).
    // If this is the case, then the request can be satisfied here without having to have all of the drivers implement the same code.
    // Note that having the IRP is still necessary since the I/O completion code requires it.
    if (FileInformationClass == FileModeInformation) {
        ...
        // Note that the following code assumes that a rename information and a set link information structure look exactly the same.
        ...
        if (renameBuffer->FileName[0] == (UCHAR)OBJ_NAME_PATH_SEPARATOR || renameBuffer->RootDirectory != NULL) {
            // A fully qualified file name was specified as the target of the rename operation.
            // Attempt to open the target file and ensure that the replacement policy for the file is consistent with the caller's request,
            // and ensure that the file is on the same volume.
            status = IopOpenLinkOrRenameTarget(&targetHandle, irp, renameBuffer, FileObject);
            if (!NT_SUCCESS(status)) {
                IoSetNextIrpStackLocation(irp);
                IoCompleteRequest(irp, 2);
            } else {
                // The fully qualified file name specifies a file on the same volume and if it exists, then the caller specified that it should be replaced.
                status = IoCallDriver(deviceObject, irp);
            }
        } else {
            // This is a simple rename operation, so call the driver and let it perform the rename operation within the same directory as the source file.
            status = IoCallDriver(deviceObject, irp);
        }
    } else {
    ...
    // If this operation was a synchronous I/O operation, check the return status to determine whether or not to wait on the file object.
    ...
    if (synchronousIo) {
        ...
        // This is a normal synchronous I/O operation, as opposed to a serialized synchronous I/O operation.
        // For this case, wait for the local event and copy the final status information back to the caller.
        if (status == STATUS_PENDING) {
    ...
    // If a target handle was created because of a rename operation, close the handle now.
...
*/
{
    ...
    if (FileObject->Flags & FO_FILE_OBJECT_HAS_EXTENSION) {
        PIOP_FILE_OBJECT_EXTENSION  fileObjectExtension = (PIOP_FILE_OBJECT_EXTENSION)(FileObject + 1);
        if (fileObjectExtension->FileObjectExtensionFlags & FO_EXTENSION_IGNORE_SHARE_ACCESS_CHECK) {
            //  This fileobject is marked to ignore share access checks so we also don't want to affect the file/directory's ShareAccess structure counts.
            update = FALSE;
        }
    }

    // Check to see whether the current file opener would like to read, write, or delete the file.
    // If so, account for it in the share access structure; otherwise, skip it.
    if (FileObject->ReadAccess || FileObject->WriteAccess || FileObject->DeleteAccess) {
        // Only update the share modes if the user wants to read, write or delete the file.
        FileObject->SharedRead = (BOOLEAN)((DesiredShareAccess & FILE_SHARE_READ) != 0);
        FileObject->SharedWrite = (BOOLEAN)((DesiredShareAccess & FILE_SHARE_WRITE) != 0);
        FileObject->SharedDelete = (BOOLEAN)((DesiredShareAccess & FILE_SHARE_DELETE) != 0);
        ...
        if (update) {            
            ...
            // Set the number of shared readers, writers, and deleters in the Share Access structure.
            ShareAccess->SharedRead = FileObject->SharedRead;
            ShareAccess->SharedWrite = FileObject->SharedWrite;
            ShareAccess->SharedDelete = FileObject->SharedDelete;
        }
    } else {
        // No read, write, or delete access has been requested.
        // Simply zero the appropriate fields in the structure so that the next accessor sees a consistent state.
        if (update) {
...
/*
Routine Description:
    This routine either enables or disables hard errors for the current thread and returns the old state of the flag.
Arguments:
    EnableHardErrors - Supplies a BOOLEAN value indicating whether or not hard errors are to be enabled for the current thread.
Return Value:
    The final function value is the previous state of whether or not hard errors were enabled.
*/
{
    PETHREAD thread;
    BOOLEAN oldFlag;

    // Get a pointer to the current thread, capture the current state of hard errors, and set the new state.
    thread = PsGetCurrentThread();
...
/*
Routine Description:
    This routine sets the top level IRP field in the current thread's thread object.
    This function is invoked by file systems to either set this field to the address of an I/O Request Packet (IRP) or to null it.
Arguments:
...
*/
{
    // Simply set the top level IRP field in the current thread's thread object.
...
/*
Routine Description:
    This routine shuts down the I/O portion of the system in preparation for a power-off or reboot.
Arguments:
...
*/
{
    ...
    // Initialize the event used to synchronize the complete of all of the shutdown routines.
    KeInitializeEvent(&event, NotificationEvent, FALSE);
    if (Phase == 0) {
        ZwClose(IopLinkTrackingServiceEventHandle);
        IoShutdownPnpDevices();

        // Walk the list of the drivers in the system that have registered themselves as wanting to know when the system is about to be shutdown and invoke each.
        while ((entry = IopInterlockedRemoveHeadList(&IopNotifyShutdownQueueHead)) != NULL) {
            ...
            // Another driver has been found that has indicated that it requires shutdown notification.  Invoke the driver's shutdown entry point.
            deviceObject = IoGetAttachedDeviceReference(shutdown->DeviceObject);
            irp = IoBuildSynchronousFsdRequest(IRP_MJ_SHUTDOWN, deviceObject, (PVOID)NULL, 0, (PLARGE_INTEGER)NULL, &event, &ioStatus);

            // If we get an IRP issue the shutdown. Otherwise just skip the driver.
            // Its unfortunate but even drivers if they fail in memory allocations on shutdown have to skip it.
            if (irp) {
        ...
        // If this is a remote boot client then allow the cache to close the database and mark it clean.
        IopShutdownCsc();
#endif // defined(REMOTE_BOOT)

        // Gain access to the file system header queues by acquiring the database resource for shared access.
        ExAcquireResourceExclusiveLite(&IopDatabaseResource, TRUE);
        ...
        // Walk the list of the drivers in the system that have registered themselves as wanting to know at the last chance when the system is about to be shutdown and invoke each.
        ...
        // N.B. The system has stopped.
        // The IopDatabaseResource lock is not released so that no other mount operations can take place.
...
VOID IopShutdownBaseFileSystems(IN PLIST_ENTRY  ListHead)
{
    ...
    // Loop through each of the disk file systems, invoking each to shutdown each of their mounted volumes.
    KeInitializeEvent(&event, NotificationEvent, FALSE);
    entry = RemoveHeadList(ListHead);
    while (entry != ListHead) {
        baseDeviceObject = CONTAINING_RECORD(entry, DEVICE_OBJECT, Queue.ListEntry);

        // We have removed the entry. If the filesystem in this thread calls IoUnregisterFileSystem then we won't remove the entry from the list as the Flink == NULL.
        ...
        // Also prevent the base device object from going away as we need to decrement the reasons for unload count later.
        ...
        // Another file system has been found.  Invoke this file system at its shutdown entry point.
        irp = IoBuildSynchronousFsdRequest(IRP_MJ_SHUTDOWN, deviceObject, (PVOID)NULL, 0, (PLARGE_INTEGER)NULL, &event, &ioStatus);

        // Its possible that the drivers are unloaded before this call returns but IoCallDriver takes a reference to the device object before calling the driver.
        // So the image will not get unloaded.
        if (irp) {
            if (IoCallDriver(deviceObject, irp) == STATUS_PENDING) {
                (VOID)KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
            }
...
/*
Routine Description:
    This routine is invoked to dequeue the next packet (IRP) from the specified device work queue and invoke the device driver's start I/O routine for it.
    If the Cancelable parameter is TRUE, then the update of current IRP is synchronized using the cancel spinlock.
Arguments:
...
*/
{
    ...
    // Begin by checking to see whether or not this driver's requests are to be considered cancelable.  If so, then acquire the cancel spinlock.
    if (Cancelable) {
        IoAcquireCancelSpinLock(&cancelIrql);
    }

    DeviceObject->CurrentIrp = (PIRP)NULL;// Clear the current IRP field before starting another request.

    // Remove the next packet from the head of the queue.  If a packet was found, then process it.
    packet = KeRemoveDeviceQueue(&DeviceObject->DeviceQueue);
    if (packet) {
        irp = CONTAINING_RECORD(packet, IRP, Tail.Overlay.DeviceQueueEntry);

        // A packet was located so make it the current packet for this device.
        DeviceObject->CurrentIrp = irp;
        if (Cancelable) {
            // If the driver does not want the IRP in the cancelable state then set the routine to NULL
        ...
        DeviceObject->DriverObject->DriverStartIo(DeviceObject, irp);// Invoke the driver's start I/O routine for this packet.
    } else {
        // No packet was found, so simply release the cancel spinlock if it was acquired.
        if (Cancelable) {
            IoReleaseCancelSpinLock(cancelIrql);
        }
    }
...
VOID IopStartNextPacketByKey(IN PDEVICE_OBJECT DeviceObject, IN LOGICAL Cancelable, IN ULONG Key)
/*
Routine Description:
    This routine is invoked to dequeue the next packet (IRP) from the specified device work queue by key and invoke the device driver's start I/O routine for it.
    If the Cancelable parameter is TRUE, then the update of current IRP is synchronized using the cancel spinlock.
Arguments:
    DeviceObject - Pointer to device object itself.
    Cancelable - Indicates that IRPs in the device queue may be cancelable.
...
*/
{
    ...
    // Begin by determining whether or not requests for this device are to be considered cancelable.  If so, then acquire the cancel spinlock.
    if (Cancelable) {
        IoAcquireCancelSpinLock(&cancelIrql);
    }
    ...
    // Attempt to remove the indicated packet according to the key from the device queue.  If one is found, then process it.
    packet = KeRemoveByKeyDeviceQueue(&DeviceObject->DeviceQueue, Key);
    if (packet) {
        irp = CONTAINING_RECORD(packet, IRP, Tail.Overlay.DeviceQueueEntry);

        // A packet was successfully located.  Make it the current packet and invoke the driver's start I/O routine for it.
        DeviceObject->CurrentIrp = irp;
        if (Cancelable) {
            // If the driver does not want the IRP in the cancelable state then set the routine to NULL
            if (DeviceObject->DeviceObjectExtension->StartIoFlags & DOE_STARTIO_NO_CANCEL) {
        ...
        // No packet was found, so release the cancel spinlock if it was acquired.
        if (Cancelable) {
            IoReleaseCancelSpinLock(cancelIrql);
        }
    }
...
/*
Routine Description:
    This routine attempts to start the specified packet request (IRP) on the specified device.
    If the device is already busy, then the packet is simply queued to the device queue.
    If a non-NULL CancelFunction is supplied, it will be put in the IRP.
    If the IRP has been canceled, the CancelFunction will be called after the IRP has been inserted into the queue or made the current packet.
Arguments:
    DeviceObject - Pointer to device object itself.
    Irp - I/O Request Packet which should be started on the device.
    Key - Key to be used in inserting packet into device queue;  optional (if not specified, then packet is inserted at the tail).
...
*/
{
    ...
    KeRaiseIrql(DISPATCH_LEVEL, &oldIrql);// Raise the IRQL of the processor to dispatch level for synchronization.

    // If the driver has indicated that packets are cancelable, then acquire
    // the cancel spinlock and set the address of the cancel function to indicate that the packet is not only cancelable, 
    // but indicates what routine to invoke should it be cancelled.
    if (CancelFunction) {
        IoAcquireCancelSpinLock(&cancelIrql);
        Irp->CancelRoutine = CancelFunction;
    }

    // If a key parameter was specified, then insert the request into the work queue according to the key; 
    // otherwise, simply insert it at the tail.
    if (Key) {
    ...
    // If the packet was not inserted into the queue, then this request is now the current packet for this device.
    // Indicate so by storing its address in the current IRP field, and begin processing the request.
    if (!i) {
        DeviceObject->CurrentIrp = Irp;
        if (CancelFunction) {
            // If the driver does not want the IRP in the cancelable state then set the routine to NULL
        ...
        // Make one last check to determine whether or not the packet has already been marked cancelled.
        // If it has, then invoke the driver's cancel routine now.
        // Note that because the cancel
        // spinlock is currently being held, 
        // an attempt to cancel the request from another processor at this point will simply wait until this routine is finished,
        // and then get it cancelled.
        if (CancelFunction) {
            if (Irp->Cancel) {
    ...
    // Restore the IRQL back to its value upon entry to this function before returning to the caller.
    KeLowerIrql(oldIrql);
}
...
/*
Routine Description:
    This routine ensures that if the IoStartPacket* routines are called from inside StartIo then it defers calling the startio until after the StartIo call returns.
    It does this by keeping a count.
    It also keeps track of whether its cancelable or has a key by storing the value in the device object extension.
    They are updated without a lock because its incorrect to have two parallel calls to IoStartNextPacket or IoStartNextPacketByKey.
Arguments:
    DeviceObject - Pointer to device object itself.
    Key - Specifics the Key used to remove the entry from the queue.
...
*/
{
    ...
    do {
        ...
        if (InterlockedIncrement(&(DeviceObject->DeviceObjectExtension->StartIoCount)) > 1) {
            ...
            DeviceObject->DeviceObjectExtension->StartIoFlags &= ~(DOE_STARTIO_REQUESTED | DOE_STARTIO_REQUESTED_BYKEY | DOE_STARTIO_CANCELABLE);
        ...
        if (InterlockedDecrement(&(DeviceObject->DeviceObjectExtension->StartIoCount)) == 0) {
            Flags = DeviceObject->DeviceObjectExtension->StartIoFlags & (DOE_STARTIO_REQUESTED | DOE_STARTIO_REQUESTED_BYKEY | DOE_STARTIO_CANCELABLE);
...
/*
Routine Description:
    This routine sets the StartIo attributes so that drivers can change the behavior of when StartIo can be called.
Arguments:
    DeviceObject - Pointer to device object itself.
    NonCancelable - If TRUE that IRP passed to StartIo is not in the cancelable state.
    DeferredStartIo - If TRUE startIo is not called recursively and is deferred until the previous StartIo call returns to the IO manager.
*/
{
    if (DeferredStartIo) {
...
*/
{
    ...
    timer = DeviceObject->Timer;// Get the address of the timer.
    ...
    if (!(DeviceObject->DeviceObjectExtension->ExtensionFlags & 
        ...
        // Likewise, check to see whether or not the timer is already enabled. 
        // If so, then simply exit.  Otherwise, enable the timer by placing it into the I/O system timer queue.
        ExAcquireFastLock(&IopTimerLock, &irql);
        if (!timer->TimerFlag) {
...
/*
Routine Description:
    This routines stops the timer associated with the specified device object from invoking being invoked.
Arguments:
...
*/
{
    ...
    timer = DeviceObject->Timer;// Obtain the I/O system timer queue lock, and disable the specified timer.
...
Arguments:
    ...
    StartingOffset - Pointer to the offset in the file from which the write should take place.
    Event - A pointer to a kernel event structure to be used for synchronization purposes.
            The event will be set to the Signaled state once the pages have been written.
    IoStatusBlock - A pointer to the I/O status block in which the final status and information should be stored.
Return Value:
    The function value is the final status of the queue request to the I/O system subcomponents.
*/
...
*/
{
    return THREAD_TO_PROCESS(Thread);// Simply return the thread's process.
}
...
/*
Routine Description:
    This routine removes the device object for the file system from the active list of file systems in the system.
Arguments:
    DeviceObject - Pointer to device object for the file system.
*/
{
    ...
    (VOID)ExAcquireResourceExclusiveLite(&IopDatabaseResource, TRUE);// Acquire the I/O database resource for a write operation.

    // Remove the device object from whatever queue it happens to be in at the moment.
    // There is no need to check here to determine if the device queue is in a queue since it is assumed that the caller registered it as a valid file system.
    ...
    IopFsRegistrationOps++;
    ExReleaseResourceLite(&IopDatabaseResource);// Release the I/O database resource.
    IopInterlockedDecrementUlong(LockQueueIoDatabaseLock, &DeviceObject->ReferenceCount);// Decrement the number of reasons that this driver cannot be unloaded.
}
...
/*
Routine Description:
    This routine unregisters the specified driver's notification routine from begin invoked whenever a file system registers or unregisters itself as an active file system in the system.
Arguments:
    DriverObject - Pointer to the driver object for the driver.
...
*/
{
    ...
    // Walk the list of registered notification routines and unregister the specified routine.
...
/*
Routine Description:
    This routine removes a registered driver from the shutdown notification queue.
    Henceforth, the driver will not be notified when the system is being shutdown.
Arguments:
    DeviceObject - Pointer to the driver's device object.
*/
{
    ...
    // Acquire the spinlock that protects the shutdown notification queue, and walk the queue looking for the caller's entry.
    // Once found, remove it from the queue.  It is an error to not find it, but it is ignored here.
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);
...
/*
Routine Description:
    This routine updates the share access context for a file according to the desired access and share access by the current open requestor.
    The IoCheckShareAccess routine must already have been invoked and succeeded in order to invoke this routine.
    Note that when the former routine was invoked the Update parameter must have been FALSE.
Arguments:
    FileObject - Pointer to the file object of the current open request.
    ShareAccess - Pointer to the share access structure that describes how the file is currently being accessed.
*/
{
    ...
    // Check to see whether or not the desired accesses need read, write, or delete access to the file.
    if ((FileObject->ReadAccess || FileObject->WriteAccess || FileObject->DeleteAccess) && update) {
        // The open request requires read, write, or delete access so update the share access context for the file.
        ShareAccess->OpenCount++;
        ShareAccess->Readers += FileObject->ReadAccess;
        ShareAccess->Writers += FileObject->WriteAccess;
        ShareAccess->Deleters += FileObject->DeleteAccess;
...
/*
Routine Description:
    This routine is invoked to check a mounted volume on the specified device when it appears as if the media may have changed since it was last accessed.
    If the volume is not the same volume, and a new mount is not to be attempted, return the error.

    If the verify fails, this routine is used to perform a new mount operation on the device.
    In this case, a "clean" VPB is allocated and a new mount operation is attempted.
    If no mount operation succeeds, then again the error handling described above occurs.
Arguments:
    DeviceObject - Pointer to device object on which the volume is to be mounted.
    AllowRawMount - Indicates that this verify is on behalf of a DASD open request, thus we want to allow a raw mount if the verify fails.
Return Value:
    The function value is a successful status code if a volume was successfully mounted on the device.  Otherwise, an error code is returned.
*/
{
    NTSTATUS status;
    KEVENT event;
    ...
    //  Acquire the DeviceObject lock.  Nothing in this routine can raise so no try {} finally {} is required.
    status = KeWaitForSingleObject(&DeviceObject->DeviceLock, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
    ASSERT(status == STATUS_SUCCESS);

    // If this volume is not mounted by anyone, skip the verify operation, but do the mount.
    if (!IopReferenceVerifyVpb(DeviceObject, &vpb, &fsBaseDeviceObject)) {
        verifySkipped = TRUE;
        status = STATUS_SUCCESS;
    } else {
        // This volume needs to be verified.  Initialize the event to be used while waiting for the operation to complete.
        KeInitializeEvent(&event, NotificationEvent, FALSE);
        status = STATUS_UNSUCCESSFUL;

        // Allocate and initialize an IRP for this verify operation.
        // Notice that the flags for this operation appear the same as a page read operation.
        // This is because the completion code for both of the operations is exactly the same logic.
        ...
        IopDereferenceVpbAndFree(vpb);// Deref the VPB obtained above.
    }

    // If the verify operation was skipped or unsuccessful perform a mount operation.
    if ((status == STATUS_WRONG_VOLUME) || verifySkipped) {
        // A mount operation is to be attempted.  Allocate a new VPB for this device and attempt to mount it.
        if (NT_SUCCESS(IopCreateVpb(DeviceObject))) {
            ...
            if (!NT_SUCCESS(IopMountVolume(DeviceObject, AllowRawMount, TRUE, FALSE, &mountVpb))) {
                ...
                if (mountVpb) {
                    ...
                    IopInterlockedDecrementUlong(LockQueueIoVpbLock, (PLONG)&mountVpb->ReferenceCount);
                }
            }
        } else {
            DeviceObject->Flags &= ~DO_VERIFY_VOLUME;
    ...
    KeSetEvent(&DeviceObject->DeviceLock, 0, FALSE);//  Release the device lock.
    return status;// Return the status from the verify operation as the final status of this function.
}
...
*/
{
    ...
    // Get the address of the error log entry header, acquire the spin lock, insert the entry onto the queue,
    // if there are no pending requests then queue a worker thread request and release the spin lock.
    entry = ((PERROR_LOG_ENTRY)ElEntry) - 1;
    if (IopErrorLogDisabledThisBoot) {// Do nothing, drop the reference.
        if (entry->DeviceObject != NULL) {
...
*/
{
    ...
    RtlInitAnsiString(&arcBootDeviceString, LoaderBlock->ArcBootDeviceName);// Get ARC boot device name from loader block.
    RtlInitAnsiString(&arcSystemDeviceString, LoaderBlock->ArcHalDeviceName);// Get ARC system device name from loader block.

    // For each disk, get its drive layout and check to see if the signature is among the list of signatures in the loader block.
    // If yes, check to see if the disk contains the boot or system partitions.
    // If yes, fill up the requested structure.
    for (diskNumber = 0; diskNumber < totalDriverDisksFound; diskNumber++) {
        ...
        for (listEntry = arcInformation->DiskSignatures.Flink; 
            ...
            if ((singleBiosDiskFound && (totalDriverDisksFound == 1) && (driveLayout->PartitionStyle == PARTITION_STYLE_MBR)) ||
                    ...
                    if (RtlEqualString(&arcNameString, &arcBootDeviceString, TRUE)) {
                        ...
                        if (driveLayout->PartitionStyle == PARTITION_STYLE_GPT) {
                            if (bootDiskInformationEx) {
                                ...
                                bootDiskInformationEx->BootDeviceGuid = driveLayout->Gpt.DiskId;// Structure copy.
                            }
                        } else {
                            if (bootDiskInformationEx) {
                    ...
                    if (RtlEqualString(&arcNameString, &arcSystemDeviceString, TRUE)) {
                        ...
                        if (driveLayout->PartitionStyle == PARTITION_STYLE_GPT) {
                            if (bootDiskInformationEx) {
                                ...
                                bootDiskInformationEx->SystemDeviceGuid = driveLayout->Gpt.DiskId;// Structure copy.
                            }
                        } else {
                            if (bootDiskInformationEx) {
    ...
    *AllocatedAcl = NULL;
    ...
    case FILE_DEVICE_NETWORK:
    {
        if ((DeviceHasName) && ((DeviceCharacteristics & FILE_FLOPPY_DISKETTE) != 0)) {
            ...
            // This is done by making a copy of the default public ACL and changing the accesses granted to the administrators alias.
...
*/
{
    ...
    if (status == STATUS_PENDING) {// If no error was incurred, wait for the I/O operation to complete.
        (VOID)KeWaitForSingleObject(&event, UserRequest, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
    }

    // The following code tears down the IRP by hand since it may not be possible for it to be completed (either because this code was
    // invoked as APC_LEVEL in the first place - or because the reference count on the object cannot be incremented due to this routine being invoked by the delete file procedure below).
    ...
    IoFreeIrp(irp);// Reuse the IRP for the next operation.
...
Routine Description:
    ...
    It's called on the special occasion when a thread faults a page and then when
    it's waiting for the inpage to complete, an IopCompleteRequest APC fires and we fault the same page again (say if the user buffer falls on the same page).
    Note the fault during the APC may be referencing the same or a different user virtual address but this is irrelevant - the problem lies
    in the fact that both virtual address references are to the same physical page and thus result in a collided fault in the Mm.
    ...
    Later on when Mm has decided the fault has progressed far enough to avoid deadlock,
    it calls back into this routine which calls IopCompleteRequest again.
...
#if defined(_WIN64)
BOOLEAN IoIs32bitProcess(IN PIRP Irp OPTIONAL)
/*
Routine Description:
   This API returns TRUE if the process that originated the IRP is running a 32 bit x86 application.
   If there is no IRP then a NULL can be passed to the API and that implies that the current process context is used to test if its running a 32 bit x86 application.
...
*/
{
    if (Irp) {
        ...
        if ((ExGetPreviousMode() == UserMode) && (PsGetProcessWow64Process(PsGetCurrentProcess()))) {
            return TRUE;
        }
    }
...
NTSTATUS IoQueryFileDosDeviceName(IN PFILE_OBJECT FileObject, OUT POBJECT_NAME_INFORMATION *ObjectNameInformation)
/*
Routine Description:
    Thin shell around IopQueryNameInternal that returns a dos device name for a file e.g c:\foo
Arguments:
    FileObject - Points to the file that needs to be closed.
    ObjectNameInformation - structure to return name in note this will be a flat unicode string where the buffer is adjacent to the string
...
*/
{
    ...
    //  Allocate an initial buffer to query the filename, query, then retry if needed with the correct length.
    ...
    while (TRUE) {
        ...
        Status = IopQueryNameInternal(FileObject, TRUE, TRUE, ObjectNameInfo, ObjectNameInfoLength, &ObjectNameInfoLength, KernelMode);
...
NTSTATUS IopUnloadSafeCompletion(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
...
NTSTATUS IoSetCompletionRoutineEx(IN PDEVICE_OBJECT DeviceObject,
                                  IN PIRP Irp,
...
//     CompletionRoutine - Address of the completion routine that is to be invoked once the next level driver completes the packet.
//     Context - Specifies a context parameter to be passed to the completion routine.
//     InvokeOnSuccess - Specifies that the completion routine is invoked when the operation is successfully completed.
//     InvokeOnError - Specifies that the completion routine is invoked when the operation completes with an error status.
//     InvokeOnCancel - Specifies that the completion routine is invoked when the operation is being canceled.
{
...
/*
Routine Description:
    This routine creates a driver object for a kernel component that was not loaded as a driver.
    If the creation of the driver object succeeds, Initialization function is invoked with the same parameters as passed to DriverEntry.
Parameters:
    DriverName - Supplies the name of the driver for which a driver object is to be created.
...
*/
{
    ...
    status = ObCreateObject(KernelMode, IoDriverObjectType, &objectAttributes, KernelMode, NULL, objectSize, 0, 0, &driverObject);
    if (!NT_SUCCESS(status)) {
        return status;// Driver object creation failed
    }
    ...
    status = ObInsertObject(driverObject, NULL, FILE_READ_DATA, OBJ_KERNEL_HANDLE, NULL, &driverHandle);
    if (!NT_SUCCESS(status)) {
    ...
    status = ObReferenceObjectByHandle(driverHandle, 0, IoDriverObjectType, KernelMode, (PVOID *)&driverObject, (POBJECT_HANDLE_INFORMATION)NULL);
    if (!NT_SUCCESS(status)) {
    ...
    // Store the name of the device driver in the driver object so that it can be easily found by the error log thread.
    ...
    errorFreeDriverObject:
        // If we were unsuccessful, we need to get rid of the driverObject that we created.
...
/*
Routine Description:
    This routine deletes a driver object created explicitly through IoCreateDriver.
Parameters:
...
Parameters:
    DeviceObject - Supplies a pointer to the deviceObject whose next device object needs to be obtained.
ReturnValue:
...
*/
{
    ...
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);// Any examination of attachment chain linkage must be done with IopDatabaseLock taken.
    baseDeviceObject = IopGetDeviceAttachmentBase(DeviceObject);// Find the base of the attachment chain.
...
NTSTATUS IoGetDiskDeviceObject(IN PDEVICE_OBJECT FileSystemDeviceObject, OUT PDEVICE_OBJECT *DiskDeviceObject)
/*
Routine Description:
    This routine returns the disk device object associated with a filesystem volume device object.
    The disk device object need not be an actual disk but in general associated with storage.
Arguments:
    FileSystemDeviceObject - Supplies a pointer to the device for which the bottom of attachment chain is to be found.
    DiskDeviceObject     - Supplies storage for the return value.
Return Value:
    The function returns the disk device object associated with a filesystem device object.
    Returns a referenced device object.
    If the VPB reference count is zero we cannot rely on the device object pointer.
*/
{
    KIRQL   irql;
    PVPB    vpb;

    if (FileSystemDeviceObject->Vpb) {// Filesystem deviceobject's VPB field should be NULL
        return STATUS_INVALID_PARAMETER;
    }
...
BOOLEAN IoIsFileOriginRemote(IN PFILE_OBJECT FileObject)
/*
Routine Description:
    This routine returns the origin of the original create request for the specified file.
    That is, was it created locally on this machine or remotely on another machine via a network provider.
Arguments:
...
*/
{
    ...
    if (FileObject->Flags & FO_REMOTE_ORIGIN) {//  Check the origin flag and return the appropriate result.
        Remote = TRUE;
    } else {
        Remote = FALSE;
    }
...
NTSTATUS IoSetFileOrigin(IN PFILE_OBJECT FileObject, IN BOOLEAN Remote)
/*
Routine Description:
    This routine sets the origin of the original create request for the specified file.
    That is, was it created locally on this machine or remotely on another machine via a network provider.
    By default file objects are considered to have a local origin.
    Network providers should call this function in their server for any file objects that were created to satisfy a create request from their client.
Arguments:
...
PVOID IoGetFileObjectFilterContext(IN PFILE_OBJECT FileObject)
/*
Routine Description:
...
NTSTATUS IoChangeFileObjectFilterContext(IN PFILE_OBJECT FileObject, IN PVOID FilterContext, IN BOOLEAN Set)
/*
Routine Description:
...
BOOLEAN IoIsDeviceEjectable(IN  PDEVICE_OBJECT DeviceObject)
{
...
NTSTATUS IoValidateDeviceIoControlAccess(IN PIRP Irp, IN ULONG RequiredAccess)
/*
Routine Description:
    ...
    defined as FILE_ANY_ACCESS and cannot be changed for compatibility reasons but really has to be validated for read/write access.
Arguments:
...
/*
Routine Description:
    This routine returns the paging priority for an IRP. 
    Its called by the storage stack to determine how to queue the IRP to the disk.
Arguments:
...
PDEVICE_OBJECT IoFindDeviceThatFailedIrp(IN  PIRP Irp)
/*
Routine Description:
    This routine returns the top most device object in the stack that failed the IRP.
    It returns NULL if the IRP's status is successful.
    Note that its the responsibility of the caller to ensure that the device object is valid.
Arguments:
    IRP  - Pointer to IRP.
Return Value:
...
*/
{
    ...
    irpSp = ((PIO_STACK_LOCATION)((UCHAR *)(Irp)+sizeof(IRP) + ((stackSize - 1) * sizeof(IO_STACK_LOCATION))));
...
*/
{
    ...
    ExAcquireResourceSharedLite(&IopDatabaseResource, TRUE);//  Lock the list shared

    numListEntries = DriverObjectListSize / sizeof(PDRIVER_OBJECT);//  Calculate how many entries can be returned
    ...
    *ActualNumberDriverObjects = numDriverObjects;//  Return total number of entries on the list
