...
Abstract:
    This module contains the code to implement the NtQueryInformationFile and NtSetInformationFile system services for the NT I/O system.
*/

#include "iomgr.h"

// Create local definitions for long flag names to make code slightly more readable.
...
/*
Routine Description:
    This encapsulates extracting and translating the mode bits from the passed file object, to be returned from a query information call.
Arguments:
...
NTSTATUS NtQueryInformationFile(
    ...
    Arguments:
        FileHandle - Supplies a handle to the file about which the requested information should be returned.
    ...
    */
{
    ...
    if (requestorMode != KernelMode) {
        ...
        // Ensure that the supplied buffer is large enough to contain the information associated with the specified set operation that is to be performed.
...
#elif defined(_WIN64)
            if (PsGetCurrentProcessByThread(CurrentThread)->Wow64Process) {// If we are a wow64 process, follow the X86 rules
                ProbeForWrite(FileInformation, Length, sizeof(ULONG));
            } else {
        ...
        {// An exception was incurred while probing the caller's parameters.  Simply return an appropriate error status code.
            return GetExceptionCode();
        }

#if DBG
    } else {
        // The caller's mode is kernel.  Ensure that at least the information class and lengths are appropriate.
        if ((ULONG)FileInformationClass >= FileMaximumInformation || !IopQueryOperationLength[FileInformationClass]) {
            return STATUS_INVALID_INFO_CLASS;
        }
    ...
    // Also get the address of the Fast Io dispatch structure.
    ...
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        // Make a special check here to determine whether or not the caller is attempting to query the file position pointer.
        // If so, then return it immediately and get out.
        if (FileInformationClass == FilePositionInformation) {
            // The caller has requested the current file position context information.
            // This is a relatively frequent call, so it is optimized here to cut through the normal IRP path.

            // Begin by establishing a condition handler and attempting to return both the file position information as well as the I/O status block.
            // If writing the output buffer fails, then return an appropriate error status code.
            // If writing the I/O status block fails, then ignore the error.
            // This is what would normally happen were everything to go through normal special kernel APC processing.
            ...
            {
                // One of writing the caller's buffer or writing the I/O status block failed.  Set the final status appropriately.
                if (writingBuffer) {
                    status = GetExceptionCode();
                }
            }

            // Note that the state of the event in the file object has not yet been reset, so it need not be set either.
            // Therefore, simply cleanup and return.
            IopReleaseFileObjectLock(fileObject);
            ObDereferenceObject(fileObject);
            return status;

            // Also do a special check if the caller it doing a query for basic or standard information and if so then try the fast query calls if they exist.
            ...
            // Do the query and setting of the IoStatusBlock inside an exception handler.
            // Note that if an exception occurs, other than writing the status back, then the IRP route will be taken.
            // If an error occurs attempting to write the status back to the caller's buffer then it will be ignored, just as it would be on the long path.
            try {
                if (FileInformationClass == FileBasicInformation) {
                    queryResult = fastIoDispatch->FastIoQueryBasicInfo(fileObject, TRUE, FileInformation, &localIoStatus, deviceObject);
                } else {
                    queryResult = fastIoDispatch->FastIoQueryStandardInfo(fileObject, TRUE, FileInformation, &localIoStatus, deviceObject);
                }
            ...
            {
                // If the result of the preceding block is an exception that occurred after the Fast I/O path itself, 
                // then the query actually succeeded so everything is done already, but the user's I/O status buffer is not writable.
                // This case is ignored to be consistent w/the long path.
                if (!writingStatus) {
                    status = GetExceptionCode();
                }
            }
    ...
    // Allocate a buffer which should be used to put the information into by the driver.
    // This will be copied back to the caller's buffer when the service completes.
    // This is done by setting the flag which says that this is an input operation.
    irp->UserBuffer = FileInformation;
    irp->AssociatedIrp.SystemBuffer = (PVOID)NULL;
    irp->MdlAddress = (PMDL)NULL;
    try {
        // Allocate the system buffer using an exception handler so that errors can be caught and handled.
        irp->AssociatedIrp.SystemBuffer = ExAllocatePoolWithQuota(NonPagedPool, Length);
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
        // An exception was incurred by attempting to allocate the intermediary system buffer.
        // Cleanup everything and return an appropriate error status code.
    ...
    // However, it is possible that the information that the caller wants is device independent.
    ...
    if (FileInformationClass == FileAccessInformation) {
        ...
        accessBuffer->AccessFlags = handleInformation.GrantedAccess;// Return the access information for this file.
        ...
        modeBuffer->Mode = IopGetModeInformation(fileObject);// Return the mode information for this file.
        ...
        alignmentInformation->AlignmentRequirement = deviceObject->AlignmentRequirement;// Return the alignment information for this file.
        ...
        // This request is handled specially because the service will fill in the Access and Mode and Alignment information in the buffer and then pass the buffer to the driver to fill in the remainder.        
        allInformation->AccessInformation.AccessFlags = handleInformation.GrantedAccess;// Begin by returning the Access information for the file.
        allInformation->ModeInformation.Mode = IopGetModeInformation(fileObject);// Return the mode information for this file.
        allInformation->AlignmentInformation.AlignmentRequirement = deviceObject->AlignmentRequirement;// Return the alignment information for this file.

        // Finally, set the information field of the IoStatus block in the IRP to account for the amount information already filled in and invoke the driver to fill in the remainder.
        irp->IoStatus.Information = sizeof(FILE_ACCESS_INFORMATION) + sizeof(FILE_MODE_INFORMATION) + sizeof(FILE_ALIGNMENT_INFORMATION);
    }

    if (skipDriver) {
        // The requested operation has already been performed.  Simply set the final status in the packet and the return state.
        status = STATUS_SUCCESS;
        irp->IoStatus.Status = STATUS_SUCCESS;
    } else {
        // This is not a request that can be [completely] performed here, so invoke the driver at its appropriate dispatch entry with the IRP.
    ...
    if (status == STATUS_PENDING) {
        if (synchronousIo) {
            ...
            if (status == STATUS_ALERTED || status == STATUS_USER_APC) {
                // The wait request has ended either because the thread was alerted or an APC was queued to this thread, because of thread rundown or CTRL/C processing.
                // In either case, try to bail out of this I/O request carefully so that the IRP completes before this routine exists or the event will not be around to set to the Signaled state.
        ...
        KIRQL irql;
        if (!synchronousIo) {
            // This is not a synchronous I/O operation, it is a synchronous I/O API to a file opened for asynchronous I/O.
            // Since this code path need never wait on the allocated and supplied event, 
    ...
    */
{
    ...
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        // Make a special check here to determine whether or not the caller is attempting to set the file position pointer information.
        // If so, then set it immediately and get out.
        if (FileInformationClass == FilePositionInformation) {
            // The caller has requested setting the current file position context information.
            // This is a relatively frequent call, so it is optimized here to cut through the normal IRP path.

            // Begin by checking to see whether the file was opened with no intermediate buffering.
            // If so, then the file pointer must be set in a manner consistent with the alignment requirement of read and write operations to a non-buffered file.
            PFILE_POSITION_INFORMATION fileInformation = FileInformation;
            ...
            currentByteOffset.HighPart < 0) {
                ...
                try {
                    IoStatusBlock->Status = STATUS_SUCCESS;// Write the I/O status block.
                    IoStatusBlock->Information = 0;
                } except(EXCEPTION_EXECUTE_HANDLER)
                {
                    NOTHING;// Writes to I/O status blocks are ignored since the operation succeeded.
                }
            }

            // Update the transfer count statistic for the current process for operations other than read and write.
            IopUpdateOtherTransferCount(Length);

            // Note that the file object's event has not yet been reset, 
            // so it is not necessary to set it to the Signaled state, since that is it's state at this point by definition.
            // Therefore, simply cleanup and return.
            IopReleaseFileObjectLock(fileObject);
            ObDereferenceObject(fileObject);
            return status;
    ...
    // Allocate a buffer and copy the information that is to be set on the file into it.
    // Also, set the flags so that the completion code will properly handle getting rid of the buffer and will not attempt to copy data.
    irp->AssociatedIrp.SystemBuffer = (PVOID)NULL;
    irp->MdlAddress = (PMDL)NULL;
    ...
    // Everything is now set to invoke the device driver with this request.
    // However, it is possible that the information that the caller wants to set is device independent.
    // If this is the case, then the request can be satisfied here without having to have all of the drivers implement the same code.
    // Note that having the IRP is still necessary since the I/O completion code requires it.
    if (FileInformationClass == FileModeInformation) {
        PFILE_MODE_INFORMATION modeBuffer = irp->AssociatedIrp.SystemBuffer;
        // Set the various flags in the mode field for the file object, if they are reasonable.
        // There are 4 different invalid combinations that the caller may not specify:
        ...
        //     3)  The file is opened for synchronous I/O but the caller did not set either of the synchronous I/O flags (alert or non-alert).
        ...
        // The information being set is a variable-length structure with embedded size information.
        // Walk the structure to ensure that it is valid so the driver does not walk off the end and incur an access violation in kernel mode.
        ...
        if (shortnameBuffer->FileNameLength <= 0) {
            ...
            status = STATUS_INVALID_PARAMETER;
            irp->IoStatus.Status = status;
        } else if (shortnameBuffer->FileName[0] == (WCHAR)OBJ_NAME_PATH_SEPARATOR) { // The short name must not begin with a separator character.
            status = STATUS_INVALID_PARAMETER;
            irp->IoStatus.Status = status;
        } else {// Pass the request to the driver below.
            status = IoCallDriver(deviceObject, irp);
        }
        ...
        // Check to see whether the disposition delete field has been set to TRUE and, if so, copy the handle being used to do this to the IRP stack location parameter.
        if (disposition->DeleteFile) {
            irpSp->Parameters.SetFile.DeleteHandle = FileHandle;
        }

        status = IoCallDriver(deviceObject, irp);// Simply invoke the driver to perform the (un)delete operation.
    ...
    if (status == STATUS_PENDING) {
        if (synchronousIo) {
            ...
            {
                // An exception occurred attempting to write the caller's I/O status block.  Simply change the final status of the operation to the exception code.
                status = GetExceptionCode();
            }
