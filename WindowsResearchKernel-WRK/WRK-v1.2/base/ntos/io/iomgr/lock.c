...
Abstract:
    This module contains the code to implement the NtLockFile and the NtUnlockFile system services for the NT I/O system.
*/
...
/*
Routine Description:
    This service locks a specified range of bytes on the file specified by the FileHandle parameter.
    The lock may either be an exclusive lock or a shared lock.
    Furthermore, the caller has the option of specifying whether or not the service should return immediately if the lock cannot be acquired without waiting.
Arguments:
    FileHandle - Supplies a handle to an open file.
    Event - Supplies an optional event to be set to the Signaled state when the operation is complete.
    ApcRoutine - Supplies an optional APC routine to be executed when the operation is complete.
    ...
    FailImmediately - Specifies that if the lock cannot immediately be acquired that the service should return to the caller.
    ExclusiveLock - Specifies, if TRUE, that the lock should be an exclusive lock;  otherwise the lock is a shared lock.
Return Value:
...
*/
{
    ...
    if (requestorMode != KernelMode) {
        ...
        // Check to ensure that the caller has either READ or WRITE access to the file.  If not, cleanup and return an error.
        ...
        try {
            ...
            // The ByteOffset parameter must be readable by the caller.  Probe and capture it.
            ProbeForReadSmallStructure(ByteOffset, sizeof(LARGE_INTEGER), sizeof(ULONG));
            fileOffset = *ByteOffset;

            // Likewise, the Length parameter must also be readable by the caller.  Probe and capture it as well.
        ...
        {
            // An exception was incurred attempting to probe the caller's parameters.
            // Dereference the file object and return an appropriate error status code.
            ObDereferenceObject(fileObject);
            return GetExceptionCode();
        }
    } else {// The caller's mode was kernel.  Get the ByteOffset and Length parameter 's to the expected locations.
        fileOffset = *ByteOffset;
        length = *Length;
    }
    ...
    // Get the address of the target device object and the fast Io dispatch structure.
    deviceObject = IoGetRelatedDeviceObject(fileObject);
    fastIoDispatch = deviceObject->DriverObject->FastIoDispatch;

    // Turbo lock support.  If the fast Io Dispatch specifies a fast lock routine then we'll first try and calling it with the specified lock parameters.
    if (fastIoDispatch && fastIoDispatch->FastIoLock) {
        IO_STATUS_BLOCK localIoStatus;

        if (fastIoDispatch->FastIoLock(
            fileObject,
            &fileOffset,
            &length,
            ...
            if (fileObject->CompletionContext && ARGUMENT_PRESENT(ApcContext)) {
                if (!NT_SUCCESS(IoSetIoCompletion(
                    fileObject->CompletionContext->Port,
    ...
    // Set the file object to the Not-Signaled state and mark it as having had a lock operation performed on it.
    ...
    try {
        ...
        // Attempt to allocate an intermediary buffer to hold the length of this lock operation.
        // If it fails, either because there is no more quota, or because there are no more resources, then the exception handler will be invoked to cleanup and exit.
    ...
    {
        // An exception was incurred.  Simply clean everything up and return an appropriate error status code.
...
*/
{
    ...
    if (requestorMode != KernelMode) {
        ...
        // The caller's mode was kernel.  Get the ByteOffset and Length parameter 's to the expected locations.
        fileOffset = *ByteOffset;
        length = *Length;
    }
    ...
    // Also get the fast I/O dispatch address.
    ...
    if (fastIoDispatch && fastIoDispatch->FastIoUnlockSingle) {
        ...
        if (fastIoDispatch->FastIoUnlockSingle(fileObject, &fileOffset, &length, PsGetCurrentProcessByThread(CurrentThread), Key, &localIoStatus, deviceObject)) {
            // Carefully return the I/O status.
            try {
                *IoStatusBlock = localIoStatus;
            } except(EXCEPTION_EXECUTE_HANDLER)
