...
Abstract:
    This module implements procedures to read a hive into memory, applying logs, etc.
    NOTE:   Alternate image loading is not supported here, that is done by the boot loader.
Revision History:
...
NTSTATUS HvpRecoverWholeHive(PHHIVE  Hive, ULONG   FileOffset);
...
NTSTATUS HvMapHive(PHHIVE  Hive)
/*
Routine Description:
    Hive must be fully initialized, in particular, file handles must be set up.
    This routine is not intended for loading hives from images already in memory.

    This routine will apply whatever fixes are available for errors in the hive image.
    In particular, if a log exists, and is applicable, this routine will automatically apply it.

    The difference from HvLoadHive is that this routine is NOT loading the hive into memory.
    It instead maps view of the hive in memory and does the bin enlisting and hive checking stuff.

    If errors are detected, the memory hive-loading is performed, log is applied and then bins are discarded.
    ...
    If STATUS_REGISTRY_RECOVERED is returned, then
        If (Log) was used, DirtyVector and DirtyCount are set, caller is expected to flush the changes (using a NEW log file)
...
Return Value:
    STATUS:
        ...
        STATUS_REGISTRY_CORRUPT         - bad signatures in the log, bad stuff in both in alternate, inconsistent log
        STATUS_REGISTRY_IO_FAILED       - data read failed
        STATUS_RECOVERED                - successfully recovered the hive, a semi-flush of logged data is necessary.
...
*/
{
    ...
    // at this point, we have a sane baseblock.  we know for sure that the pimary registry file is valid, so we don't need any data recovery
    ...
    // we need to make sure all the cell's data is faulted in inside a try/except block, as the IO to fault the data in can throw exceptions STATUS_INSUFFICIENT_RESOURCES, in particular
    try {
...
Exit2:
    HvpFreeAllocatedBins(Hive);// Clean up the bins already allocated
    HvpCleanMap(Hive);// Clean up the directory table
...
*/
{
    ...
    // at this point, we have a sane baseblock.  we may or may not still need to apply data recovery
...
/*
Routine Description:
    Read the hive from the file and allocate storage for the hive image in chunks of HBINs.
    Build the hive map "on the fly".
...
*/
{
    ...
    while (FileOffset < (Length + HBLOCK_SIZE)) {
        ...
        while (IOBufferOffset < IOBufferSize) {
            MarkBinDirty = FALSE;
            if (Bin == NULL) {
                // this is the beginning of a new bin perform bin validation and allocate the bin
                ...
                BinSize = Bin->Size;
                if ((BinSize > Length) || (BinSize < HBLOCK_SIZE) || (Bin->Signature != HBIN_SIGNATURE) || (Bin->FileOffset != (BinFileOffset - HBLOCK_SIZE))) {
                    // Bin is bogus
                    ...
                    if (CmDoSelfHeal()) {
                        ...
                        if (((Bin->FileOffset + BinSize) > Length) || (BinSize < HBLOCK_SIZE) || (BinSize % HBLOCK_SIZE)) {
                ...
                if (BinDataInBuffer >= BinDataNeeded) {
                    ...
                    ASSERT(IOBufferOffset == IOBufferSize);// if we are here, the buffer must have been exhausted
                }
            }
        }
    }

    ASSERT(Bin == NULL);// if we got here, we shouldn't have a bin under construction
    ExFreePool(IOBuffer);// Free the buffer used for I/O operations
...
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
    BaseBlock - supplies pointer to variable to receive pointer to HBASE_BLOCK, if we can successfully read one.
    TimeStamp - pointer to variable to receive time stamp (serial number) of hive, be it from the baseblock or from the Link field of the first bin.
Return Value:
    RESULT code
*/
{
    ...
    *BaseBlock = NULL;
    ...
    if ((rc == FALSE) || (HvpHeaderCheckSum(buffer) != buffer->CheckSum)) {
        ...
        // base block is bogus, but bin is OK, so tell caller to look for a log file and apply recovery
...
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
    BaseBlock - supplies pointer to variable to receive pointer to HBASE_BLOCK, if we can successfully read one.
    TimeStamp - pointer to variable holding TimeStamp, which must match the one in the log file.
Return Value:
    RESULT
*/
{
    ...
    *BaseBlock = NULL;
    ...
    if ((rc == FALSE) ||
        ...
        if (CmDoSelfHeal()) {
            ...
            Status = ZwQueryVolumeInformationFile(((PCMHIVE)Hive)->FileHandles[HFILE_TYPE_PRIMARY], &IoStatusBlock, &FsSizeInformation, sizeof(FILE_FS_SIZE_INFORMATION), FileFsSizeInformation);
            if (!NT_SUCCESS(Status)) {
                Cluster = 1;
...
*/
{
    ...
    while (BinOffset < Length) {
        ...
        while ((Me = HvpGetCellMap(Hive, BinOffset)) != NULL) {
            // attention here ! Bins crossing the CM_VIEW_SIZE boundary should be allocated from paged pool !!!!!
            if ((Me->BinAddress & HMAP_INVIEW) == 0) {
                break;// we have reached the end of the view
            }
            ...
            BinSize = Bin->Size;
            if ((BinSize > Length) || (BinSize < HBLOCK_SIZE) || (Bin->Signature != HBIN_SIGNATURE) || (Bin->FileOffset != BinOffset)) {
                // Bin is bogus
                Status2 = STATUS_REGISTRY_CORRUPT;
                HvCheckHiveDebug.Hive = Hive;
                HvCheckHiveDebug.Status = 0xA001;
                ...
                if (CmDoSelfHeal()) {
                    ...
                    Status2 = STATUS_REGISTRY_RECOVERED;
                    CmMarkSelfHeal(Hive);
                    MarkBinDirty = TRUE;// remember to mark the bin dirty after we enlist it
                } else {
                    Bin->Signature = 0; //TRICK!!!!
                    BinSize = Bin->Size = HBLOCK_SIZE;
                    Bin->FileOffset = BinOffset;
            ...
            // We fix it here, by loading the entire bin into paged pool
            if (HvpCheckViewBoundary(BinOffset, BinOffset + BinSize - 1) == FALSE) {
                ...
                Bin = (PHBIN)(Hive->Allocate)(BinSize, TRUE, CM_FIND_LEAK_TAG35);// first, allocate the bin
                if (Bin == NULL) {
                    Status = STATUS_INSUFFICIENT_RESOURCES;
                    goto ErrorExit;
                }
                ...
                Status = HvpEnlistBinInMap(Hive, Length, Bin, BinOffset, NULL);// enlist the bin as in paged pool
            } else {
                Status = HvpEnlistBinInMap(Hive, Length, Bin, BinOffset, CmView);// Now that we have the entire bin mapped in memory, Enlist It!
            }
            ...
            BinOffset += BinSize;// advance to the new bin
...
*/
{
    ...
    RequestedReadBufferSize = VectorSize + sizeof(DirtyVectorSignature);// we need to align the reads at sector size too
    ...
    while (Current < VectorSize) {
        ...
        // allocate a buffer to read the whole run from the file;
        // This is a temporary block that'll be freed immediately, so don't charge quota for it.
        ...
        // is damaged (truncated) such that the read fails. Thus we will cache the file offset we attempted to read.
        ...
        if (rc == FALSE) {
            ...
            if (CmDoSelfHeal()) {
                ...
                // actual hive sectors but only the location of the said sectors in the log file.
                // In particular, regardless of where the sectors belong in the hive they are all located contiguously in the log file.
                // In other words, "Start" marks the beginning of the sectors we were attempting to restore.
        ...
        while (Start < End) {
            ...
            if (Me->BinAddress & HMAP_INVIEW) {
                // pin the view (if not already pinned), as changes have to be flushed to the disk.
                ...
                if (IsListEmpty(&(Me->CmView->PinViewList)) == TRUE) {
                    ...
                    ASSERT_VIEW_PINNED(Me->CmView);// view is already pinned; do nothing
                }
                CmView = Me->CmView;
            } else {
                CmView = NULL;
            ...
            if ((SectorOffsetInBin == 0) && (((PHBIN)Source)->Size > Bin->Size)) {
                ...
                while (Bin->FileOffset < (Address + SourceBin->Size)) {
                    HvpDelistBinFreeCells(Hive, Bin, Stable);// Delist this bin free cells
                    ...
                    if ((Bin->FileOffset + Bin->Size) < Hive->BaseBlock->Length) {
                        ...
                        if (Me->BinAddress & HMAP_INPAGEDPOOL) {
                            (Hive->Free)(Bin, Bin->Size);// Free the old bin
                        }

                        Me = HvpGetCellMap(Hive, LocalFileOffset + LocalBinSize);
                        VALIDATE_CELL_MAP(__LINE__, Me, Hive, LocalFileOffset + LocalBinSize);

                        ASSERT(Me->BinAddress & HMAP_NEWALLOC);// the new address must be the beginning of a new allocation
                        Bin = (PHBIN)HBIN_BASE(Me->BinAddress);
                    } else {
                        ...
                        ASSERT((Bin = NULL) == NULL);// debug purposes only
                ...
                ASSERT(SectorOffsetInBin < Bin->Size);// Offset should fall within bin memory layout
                ...
                if (SectorImage == (PUCHAR)Bin) {
                    ...
                    if ((LogBin->Size < HBLOCK_SIZE) || (LogBin->Signature != HBIN_SIGNATURE) || (Bin->FileOffset != LogBin->FileOffset)) {
                        ...
                        if (CmDoSelfHeal()) {
                            ...
                            RtlClearBits(&BitMap, Bin->FileOffset / HSECTOR_SIZE, (Hive->BaseBlock->Length - Bin->FileOffset) / HSECTOR_SIZE);
                            goto Done;
                        }
                    }
                }
                ...
                NumberOfSectors = 0;
                while (((SectorImage + (NumberOfSectors * HSECTOR_SIZE)) < (PUCHAR)((PUCHAR)Bin + Bin->Size)) && ((Start + NumberOfSectors) < End)) {
                    // we are still inside the same bin;
                    // deal with all sectors inside the same bin at once
                    NumberOfSectors++;
                }

                RtlCopyMemory(SectorImage, Source, NumberOfSectors * HSECTOR_SIZE);// finally, copy the memory
            ...
            if (!NT_SUCCESS(Status)) {
                ...
                if (CmDoSelfHeal() && (Status == STATUS_REGISTRY_RECOVERED)) {
                    CmMarkSelfHeal(Hive);// .LOG is bad too, but enlisting fixed the bin
                } else {
                    goto ErrorExit;
                }
                goto ErrorExit;
        ...
        ExFreePool(MemoryBlock);// get rid of the temporary pool
    }

Done:
    // put correct dirty vector in Hive so that recovered data can be correctly flushed
...
/*
Routine Description:
    We have the whole hive inside the log. Redo the mapping and copy from the log to the actual storage.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
...
*/
{
    ...
    while (BinOffset < Length) {
        ...
        if (MappedHive && !(Me->BinAddress & HMAP_INVIEW)) {
            ...
            if (CmView) {
                ...
                } else {// view is already pinned; do nothing
        ...
        if ((!MappedHive) || (HvpCheckViewBoundary(BinOffset, BinOffset + LogBinSize - 1) == FALSE)) {
            ...
            EnlistCmView = NULL;// this will enlist the bin as in paged pool
        } else {
            ASSERT(Me->BinAddress & HMAP_INVIEW);
        ...
        BinOffset += LogBinSize;// advance to the new bin
    }
    ...
    if (CmView) {
        ...
        if (IsListEmpty(&(CmView->PinViewList)) == TRUE) {
            ...
            ASSERT_VIEW_PINNED(CmView);// view is already pinned; do nothing
        }
    }
...
NTSTATUS  HvCloneHive(PHHIVE  SourceHive, PHHIVE  DestHive, PULONG  NewLength)
/*
Routine Description:
...
*/
{
    ...
    // We'll keep track of the freebins and update the Spare field in each bin accordingly.
    ...
    while (FileOffset < Length) {
        ...
        if (Me->BinAddress & HMAP_DISCARDABLE) {
            ...
            ShiftOffset += BinSize;// all we need to do here is to keep track of shifting offset
...
NTSTATUS HvShrinkHive(PHHIVE  Hive, ULONG   NewLength)
/*
Routine Description:
