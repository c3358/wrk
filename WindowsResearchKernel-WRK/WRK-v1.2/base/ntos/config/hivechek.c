...
/*
Routine Description:
    Check the consistency of a hive.
    Apply CheckBin to bins, make sure all pointers in the cell map point to correct places.
Arguments:
...
Return Value:
    0 if Hive is OK.  Error return indicator if not.  Error value comes from one of the check procedures.
...
*/
{
    ...
    // try/except block, as the IO to fault the data in can throw exceptions STATUS_INSUFFICIENT_RESOURCES, in particular
    try {
...
/*
Routine Description:
    Step through all of the cells in the bin.
    Make sure that they are consistent with each other, and with the bin header.
Arguments:
...
#pragma prefast(suppress:12004, "no overflow.")
            ...
            // Fail 2
            // This test will always fail prior to the failure of the below test
            if (((ULONG)p->Size > Bin->Size) || ((PHCELL)(p->Size + (PUCHAR)p) > (PHCELL)((PUCHAR)Bin + Bin->Size)) || (p->Size == 0)) {
