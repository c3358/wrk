...
ULONG CmpKeyInfoProbeAlingment(IN KEY_INFORMATION_CLASS KeyInformationClass);
...
ULONG CmpExceptionFilter(IN PEXCEPTION_POINTERS ExceptionPointers);
...
Routine Description:
    ...
    For the create attempt to succeed, the new node must be a direct child of the node referred to by KeyHandle.
    If the node exists, it is opened.
    Its value is not affected in any way.

    Share access is computed from desired access.

    NOTE:
        If CreateOptions has REG_OPTION_BACKUP_RESTORE set, then DesiredAccess will be ignored.
        If the caller has the privilege SeBackupPrivilege asserted, a handle with KEY_READ | ACCESS_SYSTEM_SECURITY will be returned.
        If SeRestorePrivilege, then same but KEY_WRITE rather than KEY_READ.
        If both, then both access sets.
        If neither privilege is asserted, then the call will fail.
...
Arguments:
    ...
    ObjectAttributes - Specifies the attributes of the key being opened.
        Note that a key name must be specified.
        If a Root Directory is specified, the name is relative to the root.
        The name of the object must be within the name space allocated to the Registry, that is, all names beginning "\Registry".
        RootHandle, if present, must be a handle to "\", or "\Registry", or a key under "\Registry".

        RootHandle must have been opened for KEY_CREATE_SUB_KEY access if a new node is to be created.

        NOTE:   Object manager will capture and probe this argument.

    TitleIndex - Specifies the index of the localized alias for the name of the key.  The title index specifies the index of the localized alias for the name.
        Ignored if the key already exists.

    Class - Specifies the object class of the key.  (To the registry this is just a string.)  Ignored if NULL.
    CreateOptions - Optional control values:
        REG_OPTION_VOLATILE - Object is not to be stored across boots.
    Disposition - This optional parameter is a pointer to a variable that will receive a value indicating whether a new Registry key was created or an existing one opened:
...
*/
{
    ...
    if (HvShutdownComplete == TRUE) {
        if (PoCleanShutdownEnabled() & PO_CLEAN_SHUTDOWN_REGISTRY) {// It is forbidden to wite to the registry after it has been shutdown
            CM_BUGCHECK(REGISTRY_ERROR, INVALID_WRITE_OPERATION, 1, ObjectAttributes, 0);// if in clean shutdown mode all processes should have been killed and all drivers unloaded at this point
        }
#if DBG
        {
    ...
    try {
        ...
        if ((CreateOptions & (REG_LEGAL_OPTION | REG_OPTION_PREDEF_HANDLE)) != CreateOptions) {
            EndWmiCmTrace(STATUS_INVALID_PARAMETER, 0, &CapturedObjectName, EVENT_TRACE_TYPE_REGCREATE);// End registry call tracing
        ...
        if (status == STATUS_PREDEFINED_HANDLE) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                // Make sure we do the dereference and close before accessing user space as the reference might fault.
            ...
            *KeyHandle = Handle;
            CmpAddKeyTracker(Handle, mode);// need to do this only on clean shutdown
        }
    ...
    EndWmiCmTrace(status, 0, &CapturedObjectName, EVENT_TRACE_TYPE_REGCREATE);// End registry call tracing
    return  status;
}
...
//          Remove this one when object manager guys will export this via ob.h
...
/*
Routine Description:
    A registry key may be marked for delete, causing it to be removed from the system.  It will remain in the name space until the last handle to it is closed.
Arguments:
    KeyHandle - Specifies the handle of the Key to delete, must have been opened for DELETE access.
*/
{
    PCM_KEY_BODY                KeyBody;
    NTSTATUS                    status;
    ...
    if (NT_SUCCESS(status)) {
        ...
        if ((ExpControlKey[0] && KeyBody->KeyControlBlock == ExpControlKey[0]->KeyControlBlock) || (ExpControlKey[1] && KeyBody->KeyControlBlock == ExpControlKey[1]->KeyControlBlock)) {
            status = STATUS_SUCCESS;
        } else {
            ...
            if (NT_SUCCESS(status)) {
                if (HandleInfo.HandleAttributes & OBJ_AUDIT_OBJECT_CLOSE) {// Audit the deletion
                    SeDeleteObjectAuditAlarm(KeyBody, KeyHandle);
                }
            }
        }

        CmPostCallbackNotification(RegNtPostDeleteKey, KeyBody, status);// just a notification; disregard the return status
        ObDereferenceObject((PVOID)KeyBody);
    }

    EndWmiCmTrace(status, 0, NULL, EVENT_TRACE_TYPE_REGDELETE);// End registry call tracing
    return status;
}
...
/*
Routine Description:
    One of the value entries of a registry key may be removed with this call.  To remove the entire key, call NtDeleteKey.

    The value entry with ValueName matching ValueName is removed from the key.
    If no such entry exists, an error is returned.
Arguments:
    KeyHandle - Specifies the handle of the key containing the value entry of interest.  Must have been opened for KEY_SET_VALUE access.
...
*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        try {
            ...
            if ((LocalValueName.Length & (sizeof(WCHAR) - 1)) != 0) {// Length needs to be even multiple of the size of UNICODE char
                status = STATUS_INVALID_PARAMETER;// adjust normalize length so wmi can log value name correctly.
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    END_LOCK_CHECKPOINT;
                    CmPostCallbackNotification(RegNtPostDeleteValueKey, KeyBody, status);// just a notification; disregard the return status
                }
            }
    ...
    EndWmiCmTrace(status, 0, &LocalValueName, EVENT_TRACE_TYPE_REGDELETEVALUE);// End registry call tracing
    return status;
}
...
Routine Description:
    ...
    NtEnumerateKey returns the name of the Index'th sub key of the open key specified by KeyHandle.
    The value STATUS_NO_MORE_ENTRIES will be returned if value of Index is larger than the number of sub keys.

    Note that Index is simply a way to select among child keys.  Two calls to NtEnumerateKey with the same Index are NOT guaranteed to return the same results.
    If KeyInformation is not long enough to hold all requested data, STATUS_BUFFER_OVERFLOW will be returned, and ResultLength will be set to the number of bytes actually required.
Arguments:
    KeyHandle - Handle of the key whose sub keys are to be enumerated.  Must be open for KEY_ENUMERATE_SUB_KEY access.
    Index - Specifies the (0-based) number of the sub key to be returned.
    KeyInformationClass - Specifies the type of information returned in Buffer.  One of the following types:
        KeyBasicInformation - return last write time, title index, and name. (see KEY_BASIC_INFORMATION structure)
        KeyNodeInformation - return last write time, title index, name, class. (see KEY_NODE_INFORMATION structure)
...
*/
{
    ...
    if ((KeyInformationClass != KeyBasicInformation) && (KeyInformationClass != KeyNodeInformation) && (KeyInformationClass != KeyFullInformation)) {
        HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook the kcb for WMI
        EndWmiCmTrace(STATUS_INVALID_PARAMETER, Index, NULL, EVENT_TRACE_TYPE_REGENUMERATEKEY);// End registry call tracing
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                // CmEnumerateKey is protected to user mode buffer exceptions all other exceptions are cm internals and should result in a bugcheck
                BEGIN_LOCK_CHECKPOINT;
                status = CmEnumerateKey(KeyBody->KeyControlBlock, Index, KeyInformationClass, KeyInformation, Length, ResultLength);
                END_LOCK_CHECKPOINT;
                CmPostCallbackNotification(RegNtPostEnumerateKey, KeyBody, status);// just a notification; disregard the return status
            }
        }
    ...
    EndWmiCmTrace(status, Index, NULL, EVENT_TRACE_TYPE_REGENUMERATEKEY);// End registry call tracing
    return status;
}
...
/*
Routine Description:
    The value entries of an open key may be enumerated with NtEnumerateValueKey.

    NtEnumerateValueKey returns the name of the Index'th value entry of the open key specified by KeyHandle.
    The value STATUS_NO_MORE_ENTRIES will be returned if value of Index is larger than the number of sub keys.

    Note that Index is simply a way to select among value entries.  Two calls to NtEnumerateValueKey with the same Index are NOT guaranteed to return the same results.
...
Arguments:
    KeyHandle - Handle of the key whose value entries are to be enumerated. Must have been opened with KEY_QUERY_VALUE access.
    Index - Specifies the (0-based) number of the sub key to be returned.
    KeyValueInformationClass - Specifies the type of information returned in Buffer. One of the following types:
...
*/
{
    ...
    if ((KeyValueInformationClass != KeyValueBasicInformation) && (KeyValueInformationClass != KeyValueFullInformation) && (KeyValueInformationClass != KeyValuePartialInformation)) {
        HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook the kcb for WMI
        EndWmiCmTrace(STATUS_INVALID_PARAMETER, Index, NULL, EVENT_TRACE_TYPE_REGENUMERATEVALUEKEY);// End registry call tracing
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                // CmEnumerateValueKey is protected to user mode buffer exceptions all other exceptions are cm internals and should result in a bugcheck
                BEGIN_LOCK_CHECKPOINT;
                status = CmEnumerateValueKey(KeyBody->KeyControlBlock, Index, KeyValueInformationClass, KeyValueInformation, Length, ResultLength);
                END_LOCK_CHECKPOINT;
                CmPostCallbackNotification(RegNtPostEnumerateValueKey, KeyBody, status);// just a notification; disregard the return status
            }
        }
    ...
    EndWmiCmTrace(status, Index, NULL, EVENT_TRACE_TYPE_REGENUMERATEVALUEKEY);// End registry call tracing
    return status;
}
...
/*
Routine Description:
    Changes made by NtCreateKey or NtSetKey may be flushed to disk with NtFlushKey.
    NtFlushKey will not return to its caller until any changed data associated with KeyHandle has been written to permanent store.
    WARNING: NtFlushKey will flush the entire registry tree, and thus will burn cycles and I/O.
Arguments:
...
*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (KeyBody->KeyControlBlock->Delete) {
            ...
            status = CmFlushKey(KeyBody->KeyControlBlock, FALSE);// call the worker to do the flush
        }
    ...
    EndWmiCmTrace(status, 0, NULL, EVENT_TRACE_TYPE_REGFLUSH);// End registry call tracing
    return status;
}
...
Routine Description:
    ...
    1) It is called from SM after autocheck (chkdsk) has run and the paging files have been opened.
    It's function is to bind in memory hives to their files, and to open any other files yet to be used.
    2) It is called from SC after the current boot has been accepted and the control set used for the boot process should be saved as the LKG control set.

    After this routine accomplishes the work of situation #1 and #2, further requests for such work will not be carried out.
Arguments:
    BootCondition -
         REG_INIT_BOOT_SM -     The routine has been called from SM in situation #1.
         REG_INIT_BOOT_SETUP -  The routine has been called to perform situation #1 work but has been called from setup and needs to do some special work.
        REG_INIT_BOOT_ACCEPTED_BASE + Num
                        (where 0 < Num < 1000) - The routine has been called in situation #2. "Num" is the number of the control set to which the boot control set should be saved.
Return Value:
    NTSTATUS - Result code from call, among the following:
        STATUS_SUCCESS - it worked
        STATUS_ACCESS_DENIED - the routine has already done the work requested and will not do it again.
*/
{
    BOOLEAN     SetupBoot;
    NTSTATUS    Status;
    ...
    if (KeGetPreviousMode() == UserMode) {// Force previous mode to be KernelMode
        ...
        if ((BootCondition >= REG_INIT_BOOT_ACCEPTED_BASE) && (BootCondition <= REG_INIT_BOOT_ACCEPTED_MAX)) {
            ...
            // Calculate the control set we want to save the boot control set to
            BootCondition -= REG_INIT_BOOT_ACCEPTED_BASE;
            if (BootCondition) {
                // OK, this is a good boot for the purposes of LKG, and we have a valid control set number passed into us.
                // We save off our control set and then notify everyone who wants to do post WinLogon work.
                Status = CmpSaveBootControlSet(BootCondition);// Note that none of this happens during Safe Mode boot!
                HalEndOfBoot();// Mark the boot good for the Hal. This lets the Hal do things like optimize reboot performance.
                CcPfBeginBootPhase(PfBootAcceptedRegistryInitPhase);// Notify prefetcher of boot progress.
                CmpRaiseSelfHealWarningForSystemHives();// inform the user in the event one of the system core hives have been self healed
                CmpHoldLazyFlush = FALSE;// enable lazy flusher
                CmpLazyFlush();
                return Status;
            } else {
        ...
        SetupBoot = (BootCondition == REG_INIT_BOOT_SETUP ? TRUE : FALSE);// called from setup?
        ...
        WmiBootPhase1();// Notify WMI.

        CcPfBeginBootPhase(PfSMRegistryInitPhase);// Notify prefetcher of boot progress.
...
/*
Routine Description:
    Notification of key creation, deletion, and modification may be obtained by calling NtNotifyChangeKey.

    NtNotifyChangeKey monitors changes to a key - if the key or subtree specified by KeyHandle are modified, the service notifies its caller.
    It also returns the name(s) of the key(s) that changed.
    All names are specified relative to the key that the handle represents (therefore a NULL name represents that key).
    The service completes once the key or subtree has been modified based on the supplied CompletionFilter.
    The service is a "single shot" and therefore needs to be reinvoked to watch the key for further changes.

    The operation of this service begins by opening a key for KEY_NOTIFY access.
    Once the handle is returned, the NtNotifyChangeKey service may be invoked to begin watching the values and subkeys of the specified key for changes.
    The first time the service is invoked, the BufferSize parameter supplies not only the size of the user's Buffer, but also the size of the buffer that will be used by the
    Registry to store names of keys that have changed.
    Likewise, the CompletionFilter and WatchTree parameters on the first call indicate how notification should operate for all calls using the supplied KeyHandle.
    These two parameters are ignored on subsequent calls to the API with the same instance of KeyHandle.

    Once a modification is made that should be reported, the Registry will complete the service.
    The names of the files that have changed since the last time the service was called will be placed into the caller's output Buffer.
    The Information field of IoStatusBlock will contain the number of bytes placed in Buffer, or zero if too many keys have
    changed since the last time the service was called, in which case the application must Query and Enumerate the key and sub keys to discover changes.
    The Status field of IoStatusBlock will contain the actual status of the call.

    If Asynchronous is TRUE, then Event, if specified, will be set to the Signaled state.
    If no Event parameter was specified, then KeyHandle will be set to the Signaled state.
    If an ApcRoutine was specified, it is invoked with the ApcContext and the address of the IoStatusBlock as its arguments.
    If Asynchronous is FALSE, Event, ApcRoutine, and ApcContext are ignored.

    This service requires KEY_NOTIFY access to the key that was actually modified

    The notify "session" is terminated by closing KeyHandle.
Arguments:
    KeyHandle-- Supplies a handle to an open key.  This handle is effectively the notify handle, because only one set of notify parameters may be set against it.
    ...
    ApcRoutine - An optional procedure to be invoked once the operation completes.  For more information about this parameter see the NtReadFile system service description.
        If PreviousMode == Kernel, this parameter is an optional pointer to a WORK_QUEUE_ITEM to be queued when the notify is signaled.
    ApcContext - A pointer to pass as an argument to the ApcRoutine, if one was specified, when the operation completes.
                    This argument is required if an ApcRoutine was specified.
        If PreviousMode == Kernel, this parameter is an optional WORK_QUEUE_TYPE describing the queue to be used.
        This argument is required if an ApcRoutine was specified.
    IoStatusBlock - A variable to receive the final completion status.
        For more information about this parameter see the NtCreateFile system service description.
    CompletionFilter -- Specifies a set of flags that indicate the types of operations on the key or its value that cause the call to complete.
                        The following are valid flags for this parameter:
        REG_NOTIFY_CHANGE_NAME -- Specifies that the call should be completed if a subkey is added or deleted.
        REG_NOTIFY_CHANGE_ATTRIBUTES -- Specifies that the call should be completed if the attributes (e.g.: ACL) of the key or any subkey are changed.
        REG_NOTIFY_CHANGE_LAST_SET -- Specifies that the call should be completed if the lastWriteTime of the key or any of its subkeys is changed.
                                        (Ie. if the value of the key or any subkey is changed).
        REG_NOTIFY_CHANGE_SECURITY -- Specifies that the call should be completed if the security information (e.g. ACL) on the key or any subkey is changed.
    WatchTree -- A BOOLEAN value that, if TRUE, specifies that all changes in the subtree of this key should also be reported.
        If FALSE, only changes to this key, its value, and its immediate subkeys (but not their values nor their subkeys) are reported.
    Buffer -- A variable to receive the name(s) of the key(s) that changed.  See REG_NOTIFY_INFORMATION.
    BufferSize -- Specifies the length of Buffer.
    Asynchronous  -- If FALSE, call will not return until complete (synchronous) if TRUE, call may return STATUS_PENDING.
...
*/
{
    ...
    return NtNotifyChangeMultipleKeys(KeyHandle, 0, NULL, Event, ApcRoutine, ApcContext, IoStatusBlock, CompletionFilter, WatchTree, Buffer, BufferSize, Asynchronous);
}
...
/*
Routine Description:
    Notificaion of creation, deletion and modification on multiple keys may be obtained with NtNotifyChangeMultipleKeys.

    NtNotifyMultipleKeys monitors changes to any of the MasterKeyHandle or one of SlaveObjects and/or their subtrees, whichever occurs first.
    When an event on these keys is triggered, the notification is considered fulfilled, and has to be "armed" again, in order to watch for further changes.

    The mechanism is similar to the one described in NtNotifyChangeKey.

    The MasterKeyHandle key, give the caller control over the lifetime of the notification.
    The notification will live as long as the caller keeps the MasterKeyHandle open, or an event is triggered.

    The caller doesn't have to open the SlaveKeys.
    He will provide the routine with an array of OBJECT_ATTRIBUTES, describing the slave objects.
    The routine will open the objects, and ensure keep a reference on them until the back-end side will close them.

    The notify "session" is terminated by closing MasterKeyHandle.

Obs:
    For the time being, the routine supports only one slave object.
    When more than one slave object is provided, the routine will signal an error of STATUS_INVALID_PARAMETER.
    However, the interface is designed for future enhancements (taking an array of slave objects), that may be provided with future versions(w2001).

    When no slave object is supplied (i.e. Count == 0) we have the identical behavior as for NtNotifyChangeKey.
Arguments:
    MasterKeyHandle - Supplies a handle to an open key.  This handle is the "master handle". It has control overthe lifetime of the notification.
    Count - Number of slave objects. For the time being, this should be 1
    SlaveObjects - Array of slave objects. Only the attributes of the objects are provided, so the caller doesn't have to take care of them.

    Event,ApcRoutine,ApcContext,IoStatusBlock,CompletionFilter,WatchTree, Buffer,BufferSize,Asynchronous - same as for NtNotifyChangeKey
*/
{
    ...
    if (HvShutdownComplete == TRUE) {
        return STATUS_TOO_LATE;// too late to do registry operations.
    }

    if (Count > 1) {
        return STATUS_INVALID_PARAMETER;// This version supports only one slave object
    }

    if (Count == 1) {
        SlavePresent = TRUE;// We have one slave, so we are in the NtNotifyChangeMultipleKeys case
    }
    ...
    if (PreviousMode != KernelMode) {
#if defined(_WIN64)
        UseIosb32 = (PsGetCurrentProcess()->Wow64Process != NULL ? TRUE : FALSE);// Process is 32bit if Wow64 is not NULL.
#endif
        ...
        try {
            ...
            CmpSetIoStatus(IoStatusBlock, STATUS_PENDING, 0, UseIosb32);// Initialize IOSB
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
        ...
        if (Asynchronous) {
            ...
            if (Count > 0) {
                return STATUS_INVALID_PARAMETER;// we don't allow multiple asynchronous kernel notifications
            }
        }
    }
    ...
    // Allocate master and slave post blocks, and init it.  Do NOT put it on the chain, CmNotifyChangeKey will do that while holding a mutex.

    // WARNING: PostBlocks MUST BE ALLOCATED from Pool, since back side of Notify will free it!
    ...
    if ((PostType == PostAsyncUser) || (PostType == PostAsyncKernel)) {
        ...
        if (PostType == PostAsyncUser) {
            ...
            // Initialize APC.  May or may not be a user apc, will always be a kernel apc.
    ...
    // Exclusively lock the registry; We want nobody to mess with it while we are doing the post/notify list manipulation;
    ...
    if (MasterKeyBody->KeyControlBlock->Delete || (SlavePresent && SlaveKeyBody->KeyControlBlock->Delete)) {
    ...
    status = CmpNotifyChangeKey(MasterKeyBody, MasterPostBlock, CompletionFilter, WatchTree, Buffer, BufferSize, MasterPostBlock);
    if (!NT_SUCCESS(status)) {
    ...
    if (SlavePresent) {
        if (status == STATUS_SUCCESS) {
            // The notify has already been triggered for the master, there is no point to set one for the slave too Clean up the mess we made for the slave object and signal as there is no slave present
            CmpFreePostBlock(SlavePostBlock);
            SlavePresent = FALSE;
        } else {// Call worker for slave
            status = CmpNotifyChangeKey(SlaveKeyBody, SlavePostBlock, CompletionFilter, WatchTree, Buffer, BufferSize, MasterPostBlock);
            if (!NT_SUCCESS(status)) {
                // if we are here, the slave key has been deleted in between or there was no memory available to allocate a notify block for the slave key.
                // We do the cleanup here since we already hold the registry lock
                // exclusively and we don't want to give a anybody else a chance to trigger the notification on master post (otherwise we could end up freeing it twice).
                // The master post block and the user event are cleaned later, covering both single and multiple notifications cases

                CmpRemoveEntryList(&(MasterPostBlock->NotifyList));// Use Cmp variant to protect for multiple deletion of the same object
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (PostType == PostSynchronous) {
            WaitStatus = KeWaitForSingleObject(MasterPostBlock->u->Sync.SystemEvent, Executive, PreviousMode, TRUE, NULL);
            if ((WaitStatus == STATUS_ALERTED) || (WaitStatus == STATUS_USER_APC)) {
                // The wait was aborted, clean up and return.
                // 1. Remove the PostBlocks from the notify list.  This is normally done by the back end of notify, but we have to do it here since the back end is not involved.
                ...
                if (SlavePresent) {
                    if (SlavePostBlock->NotifyList.Flink != NULL) {
                        CmpRemoveEntryList(&(SlavePostBlock->NotifyList));// Use Cmp variant to protect for multiple deletion of the same object
                    }
                    CmpRemoveEntryList(&(SlavePostBlock->ThreadList));// Use Cmp variant to protect for multiple deletion of the same object
                }
                ...
                // The wait was satisfied, which means the back end has already removed the postblock from the notify list.
                // We just have to delist and free the post block.
                CmpLockRegistry();// Acquire the registry lock exclusive to enter the post block rule prerequisites
        ...
        CmpFreePostBlock(MasterPostBlock);
        if (UserEvent != NULL) {// it didn't work, clean up for error path
            ObDereferenceObject(UserEvent);
        }
    }
...
Arguments:
    ...
    ObjectAttributes - Specifies the attributes of the key being opened.
        ...
        The name of the object must be within the name space allocated to the Registry, that is, all names beginning "\Registry".
        RootHandle, if present, must be a handle to "\", or "\Registry", or a key under "\Registry".
        If the specified key does not exist, or access requested is not allowed, the operation will fail.
        NOTE:   Object manager will capture and probe this argument.
*/
{
    ...
    if (HvShutdownComplete == TRUE) {
        return STATUS_TOO_LATE;// it is now too late to do registry operations
    }
    ...
    try {
        ...
        HookKcbFromHandleForWmiCmTrace(ObjectAttributes->RootDirectory);// hook it for WMI
    } except(CmpExceptionFilter(GetExceptionInformation()))
    {
    ...
    EndWmiCmTrace(status, 0, &CapturedObjectName, EVENT_TRACE_TYPE_REGOPEN);// End registry call tracing
    return  status;
}
...
/*
Routine Description:
    Data about the class of a key, and the numbers and sizes of its children and value entries may be queried with NtQueryKey.

    If KeyValueInformation is not long enough to hold all requested data, STATUS_BUFFER_OVERFLOW will be returned, and ResultLength will be set to the number of bytes actually required.

    NOTE: The returned lengths are guaranteed to be at least as long as the described values, but may be longer in some circumstances.
Arguments:
    KeyHandle - Handle of the key to query data for.  Must have been opened for KEY_QUERY_KEY access.
    KeyInformationClass - Specifies the type of information returned in Buffer.  One of the following types:
        KeyBasicInformation - return last write time, title index, and name. (See KEY_BASIC_INFORMATION)
        KeyNodeInformation - return last write time, title index, name, class. (See KEY_NODE_INFORMATION)
        KeyFullInformation - return all data except for name and security. (See KEY_FULL_INFORMATION)
    KeyInformation -Supplies pointer to buffer to receive the data.
    Length - Length of KeyInformation in bytes.
    ResultLength - Number of bytes actually written into KeyInformation.
*/
{
    ...
    if ((KeyInformationClass != KeyBasicInformation) &&
        ...
        HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook it for WMI
        EndWmiCmTrace(STATUS_INVALID_PARAMETER, KeyInformationClass, NULL, EVENT_TRACE_TYPE_REGQUERY);// End registry call tracing
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (NT_SUCCESS(status)) {
        ...
        try {
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (NT_SUCCESS(status)) {
                    status = CmQueryKey(KeyBody->KeyControlBlock, KeyInformationClass, KeyInformation, Length, ResultLength);// CmQueryKey is writting to user-mode buffer
                    CmPostCallbackNotification(RegNtPostQueryKey, KeyBody, status);// just a notification; disregard the return status
                }
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
    ...
    EndWmiCmTrace(status, KeyInformationClass, NULL, EVENT_TRACE_TYPE_REGQUERY);// End registry call tracing
    return status;
}
...
Return Value:
    NTSTATUS
    NOTE: The IopQueryRegsitryValues() routine in the IO system assumes STATUS_OBJECT_NAME_NOT_FOUND is returned if the value being queried for does not exist.
*/
{
    NTSTATUS    status;
    PCM_KEY_BODY   KeyBody;
    ...
    if ((KeyValueInformationClass != KeyValueBasicInformation) &&
        ...
        HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook it for WMI
        EndWmiCmTrace(STATUS_INVALID_PARAMETER, KeyValueInformationClass, NULL, EVENT_TRACE_TYPE_REGQUERYVALUE);// End registry call tracing
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (NT_SUCCESS(status)) {
        ...
        try {
            ...
            if ((LocalValueName.Length & (sizeof(WCHAR) - 1)) != 0) {
                status = STATUS_INVALID_PARAMETER;// adjust normalize length so wmi can log value name correctly.
            } else {// do NOT allow trailing NULLs at the end of the ValueName.
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                CmPostCallbackNotification(RegNtPostQueryValueKey, KeyBody, status);// just a notification; disregard the return status
            }
        }
    ...
    EndWmiCmTrace(status, KeyValueInformationClass, &LocalValueName, EVENT_TRACE_TYPE_REGQUERYVALUE);// End registry call tracing
    return status;
}
...
/*
Routine Description:
    A file in the format created by NtSaveKey may be loaded into the system's active registry with NtRestoreKey.
    An entire subtree is created in the active registry as a result.
    All of the data for the new sub-tree, including such things as security descriptors, will be read from the source file.
    The data will not be interpreted in any way.

    This call (unlike NtLoadKey, see below) copies the data.
    The system will NOT be using the source file after the call returns.

    If the flag REG_WHOLE_HIVE_VOLATILE is specified, a new hive can be created.
    It will be a memory only copy.
    The restore must be done to the root of a hive (e.g. \registry\user\<name>)

    If the flag is NOT set, then the target of the restore must be an existing hive.
    The restore can be done to an arbitrary location within an existing hive.
    ...
    Notifies are flushed.
    The hive file will be resized, the log will not.
    If there is any volatile space in the hive being refreshed, STATUS_UNSUCCESSFUL will be returned.
    (It's much too obscure a failure to warrant a new error code.)

    If the flag REG_FORCE_RESTORE is set, the restore operation is done even if the KeyHandle has open subkeys by other applications
Arguments:
    KeyHandle - refers to the Key in the registry which is to be the root of the new tree read from the disk.
                This key will be replaced.
    FileHandle - refers to file to restore from, must have read access.
    Flags   - If REG_WHOLE_HIVE_VOLATILE is set, then the copy will exist only in memory, and disappear when the machine is rebooted.
              No hive file will be created on disk.
...
*/
{
    ...
    if (!SeSinglePrivilegeCheck(SeRestorePrivilege, mode)) {// Check to see if the caller has the privilege to make this call.
...
/*
Routine Description:
    A subtree of the active registry may be written to a file in a format suitable for use with NtRestoreKey.
    All of the data in the subtree, including such things as security descriptors will be written out.
    Caller must have SeBackupPrivilege privilege.
    This function will always save the hive in HSYS_MINOR format. For saving in other format (latest - 1.5) NtSaveKeyEx  is provided.
Arguments:
    KeyHandle - refers to the Key in the registry which is the root of the tree to be written to disk.  The specified node will be included in the data written out.
    FileHandle - a file handle with write access to the target file of interest.
*/
{
    NTSTATUS    status;
    PCM_KEY_BODY   KeyBody;
    ...
    if (!SeSinglePrivilegeCheck(SeBackupPrivilege, mode)) {// Check to see if the caller has the privilege to make this call.
        return(STATUS_PRIVILEGE_NOT_HELD);
    }
...
Routine Description:
    ...
    Caller must have SeBackupPrivilege privilege.
Arguments:
    KeyHandle - refers to the Key in the registry which is the root of the tree to be written to disk.
    The specified node will be included in the data written out.
    FileHandle - a file handle with write access to the target file of interest.
...
*/
{
    ...
    if ((Format != REG_STANDARD_FORMAT) && (Format != REG_LATEST_FORMAT) && (Format != REG_NO_COMPRESSION)) {// param validation
        return STATUS_INVALID_PARAMETER;
    }
...
/*
Routine Description:
    Two subtrees of the registry can be merged. The resulting subtree may be written to a file in a format suitable for use with NtRestoreKey.
    All of the data in the subtree, including such things as security descriptors will be written out.

    Caller must have SeBackupPrivilege privilege.
Arguments:
    HighPrecedenceKeyHandle - refers to the key in the registry which is the root of the HighPrecedence tree.
                I.e., when a key is present in both trees headed by the two keys, the key underneath HighPrecedence tree will always prevail.
                The specified node will be included in the data written out.
    LowPrecedenceKeyHandle - refers to the key in the registry which is the root of the "second choice" tree.
                Keys from this trees get saved when there is no equivalent key in the tree headed by HighPrecedenceKey
    FileHandle - a file handle with write access to the target file of interest.
*/
{
    NTSTATUS    status;
...
Routine Description:
    ...
    If a value entry with a Value ID (i.e. name) matching the one specified by ValueName exists, it is deleted and replaced with the one specified.
    If no such value entry exists, a new one is created.  NULL is a legal Value ID.
    While Value IDs must be unique within any given key, the same Value ID may appear in many different keys.
Arguments:
    KeyHandle - Handle of the key whose for which a value entry is to be set.  Must be opened for KEY_SET_VALUE access.
    ValueName - The unique (relative to the containing key) name of the value entry.  May be NULL.
    TitleIndex - Supplies the title index for ValueName.  The title index specifies the index of the localized alias for the ValueName.
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (mode == UserMode) {
            try {
                ...
                // Capture the name buffer. Note that a zero-length name is valid, that is the "Default" value.
        ...
        if (CmIsKcbReadOnly(KeyBody->KeyControlBlock)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                END_LOCK_CHECKPOINT;
                CmPostCallbackNotification(RegNtPostSetValueKey, KeyBody, status);// just a notification; disregard the return status
            }
        }

    Exit:
        EndWmiCmTrace(status, 0, &LocalValueName, EVENT_TRACE_TYPE_REGSETVALUE);// End registry call tracing
...
/*
Routine Description:
    A hive (file in the format created by NtSaveKey) may be linked into the active registry with this call.
    UNLIKE NtRestoreKey, the file specified to NtLoadKey will become the actual backing store of part of the registry (that is, it will NOT be copied.)

    The file may have an associated .log file.

    If the hive file is marked as needing a .log file, and one is not present, the call will fail.

    The name specified by SourceFile must be such that ".log" can be appended to it to generate the name of the log file.
    Thus, on FAT file systems, the hive file may not have an extension.

    Caller must have SeRestorePrivilege privilege.

    This call is used by logon to make the user's profile available in the registry.
    It is not intended for use doing backup, restore, etc.  Use NtRestoreKey for that.

Arguments:
    TargetKey - specifies the path to a key to link the hive to. path must be of the form "\registry\user\<username>"
    SourceFile - specifies a file.  while file could be remote, that is strongly discouraged.
*/
{
...
Arguments:
    TargetKey - specifies the path to a key to link the hive to. path must be of the form "\registry\user\<username>"
    SourceFile - specifies a file.  while file could be remote, that is strongly discouraged.
    Flags - specifies any flags that should be used for the load operation. The only valid flag is REG_NO_LAZY_FLUSH.
*/
{
...
Routine Description:
    ...
    This API allows for establiching 'classes of trust' within the UNTRUSTED name space.
Arguments:
    TargetKey - specifies the path to a key to link the hive to. path must be of the form "\registry\user\<username>"
    SourceFile - specifies a file.  while file could be remote, that is strongly discouraged.
    Flags - specifies any flags that should be used for the load operation. The only valid flag is REG_NO_LAZY_FLUSH.
    TrustClassKey - new hives that is loaded will be put in the same trust class with the hive represented by TrustClassKey
*/
{
    ...
    // The way we do this is a cronk, but at least it's the same cronk we use for all the registry I/O.

    // The file needs to be opened in the worker thread's context, since the resulting handle must be valid when we poke him to go read/write from.
    // So we just capture the object attributes for the hive file here, then poke the worker thread to go do the rest of the work.
    PreviousMode = KeGetPreviousMode();
    ...
    try {
        ...
        Key = *TargetKey;// Capture the object attributes.
...
Routine Description:
    ...
    If there are handles open to the hive being dropped, this call will fail.  Terminate relevant processes so that handles are closed.
...
Arguments:
    ...
    Flags - controls force unload. If 0, the same as NtUnloadKey.
            if REG_FORCE_UNLOAD the hive is unloaded even if there are open subkeys inside of it.
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (Flags == REG_FORCE_UNLOAD) {
            ...
            // we only need it shared for the regular (not force unload) path plus this guys and his parent
        ...
        if (KeyBody->KeyControlBlock->Delete) {
            ...
            if (!IsHiveFrozen((PCMHIVE)(KeyBody->KeyControlBlock->KeyHive))) {
                ...
                CmpReportNotify(KeyBody->KeyControlBlock, KeyBody->KeyControlBlock->KeyHive, KeyBody->KeyControlBlock->KeyCell, REG_NOTIFY_CHANGE_LAST_SET);
                ...
                TryExclusive:
                    ...
                    if ((Status == STATUS_CANNOT_DELETE) && TryAgain) {
                        // ok; it didn't work; this might be because we couldn't safely lock all hash entries we'll have to take the perf hit and try with exclusive lock
                        ...
                        TryAgain = FALSE;
                        UnloadControlFlags |= CM_UNLOAD_REG_LOCKED_EX;
                        if ((!(KeyBody->KeyControlBlock->Delete)) && (!IsHiveFrozen((PCMHIVE)(KeyBody->KeyControlBlock->KeyHive))) && (!CmIsKcbReadOnly(KeyBody->KeyControlBlock))) {
                            goto TryExclusive;
                        }
                ...
                Status = STATUS_TOO_LATE;// don't let them hurt themselves by calling it twice
            }
        }
...
Routine Description:
    ...
    Cannot be applied to core system hives (HARDWARE, SYSTEM, etc.)
    Can be applied to user hives loaded via NtRestoreKey or NtLoadKey.
    If there are handles open to the hive being dropped, the hive will be frozen and all calls to CmDeleteKey will be watched as when the last handle inside this hive is closed, the hive will be unloaded.
    Caller must have SeRestorePrivilege privilege.
Arguments:
    TargetKey - specifies the path to a key to link the hive to. path must be of the form "\registry\user\<username>"
Return Value:
    STATUS_SUCCESS - hive successfully unloaded - no late-unloading needed
    STATUS_PENDING - hive has been frozen and the event (if any) will be signaled when the hive unloads
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (KeyBody->KeyControlBlock->Delete) {
            Status = STATUS_KEY_DELETED;
        } else {// Report the notify here, because the KCB won't be around later.
            CmpReportNotify(KeyBody->KeyControlBlock, KeyBody->KeyControlBlock->KeyHive, KeyBody->KeyControlBlock->KeyCell, REG_NOTIFY_CHANGE_LAST_SET);
        ...
        // if hive was successfully unloaded (or something wrong happened, we need to deref user event otherwise the back-end routine will deref it after signaling
...
)
{
    ...
    case KeyWriteTimeInformation:
        if (KeySetInformationLength != sizeof(KEY_WRITE_TIME_INFORMATION)) {
            HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook it for WMI
            EndWmiCmTrace(STATUS_INFO_LENGTH_MISMATCH, 0, NULL, EVENT_TRACE_TYPE_REGSETINFORMATION);// End registry call tracing
    ...
    default:
        HookKcbFromHandleForWmiCmTrace(KeyHandle);// hook it for WMI
        EndWmiCmTrace(STATUS_INVALID_INFO_CLASS, 0, NULL, EVENT_TRACE_TYPE_REGSETINFORMATION);// End registry call tracing
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (CmIsKcbReadOnly(KeyBody->KeyControlBlock)) {
            ...
            case KeyUserFlagsInformation:
                ...
                break;
            default:
                ASSERT(FALSE);// we shouldn't go through here
            }
        }

        CmPostCallbackNotification(RegNtPostSetInformationKey, KeyBody, status);// just a notification; disregard the return status
        ObDereferenceObject((PVOID)KeyBody);
    }

    END_LOCK_CHECKPOINT;

    EndWmiCmTrace(status, 0, NULL, EVENT_TRACE_TYPE_REGSETINFORMATION);// End registry call tracing
    return status;
}
...
/*
Routine Description:
    A hive file may be "replaced" under a running system, such that the new file will be the one actually used at next boot, with this call.

    This routine will:
        Open newfile, and verify that it is a valid Hive file.

        Rename the Hive file backing TargetHandle to OldFile.
        All handles will remain open, and the system will continue to use the file until rebooted.

        Rename newfile to match the name of the hive file backing TargetHandle.
...
Arguments:
    NewFile - specifies the new file to use.
              must not be just a handle, since NtReplaceKey will insist on opening the file for exclusive access (which it will hold until the system is rebooted.)
    TargetHandle - handle to a registry hive root
    OldFile - name of file to apply to current hive, which will become old hive
*/
{
    KPROCESSOR_MODE PreviousMode;
    ...
    if (NT_SUCCESS(Status)) {
        if (CmIsKcbReadOnly(KeyBody->KeyControlBlock)) {
            ...
            BEGIN_LOCK_CHECKPOINT;
            Status = CmReplaceKey(KeyBody->KeyControlBlock->KeyHive, KeyBody->KeyControlBlock->KeyCell, &NewHiveName, &OldFileName);
            END_LOCK_CHECKPOINT;
        }
...
/*
Routine Description:
    Multiple values of any key may be queried atomically with this api.
Arguments:
    ...
    RequiredBufferLength - if present, Returns the length in bytes of the ValueBuffer array required to return all the values of this key.
*/
{
    KPROCESSOR_MODE PreviousMode;
    NTSTATUS Status;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        try {
            if (PreviousMode == UserMode) {
                ...
                // Put an arbitrary 64K limit on the number of entries to prevent bogus apps from passing an EntryCount large enough to overflow the EntryCount * sizeof(KEY_VALUE_ENTRY) calculation.
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    //BEGIN_LOCK_CHECKPOINT;
                    Status = CmQueryMultipleValueKey(KeyBody->KeyControlBlock, ValueEntries, EntryCount, ValueBuffer, &LocalBufferLength, RequiredBufferLength);
                    //END_LOCK_CHECKPOINT;
                    // anybody messed with BufferLength in between?
                    *BufferLength = LocalBufferLength;
                    CmPostCallbackNotification(RegNtPostQueryMultipleValueKey, KeyBody, Status);// just a notification; disregard the return status
                }
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
    ...
    EndWmiCmTrace(Status, EntryCount, NULL, EVENT_TRACE_TYPE_REGQUERYMULTIPLEVALUE);// End registry call tracing
    return(Status);
}
...
/*
Routine Description:
    This is a helper routine that converts OBJECT_ATTRIBUTES into a full object pathname.
    This is needed because we cannot pass handles to the worker thread, since it runs in a different process.

    This routine will also probe and capture the attributes based on PreviousMode.

    Storage for the string buffer is allocated from paged pool, and should be freed by the caller.
Arguments:
...
*/
{
    ...
    try {
        ...
        if (CapturedAttributes.RootDirectory != NULL) {
            if ((FileName.Buffer != NULL) && (FileName.Length >= sizeof(WCHAR)) && (*(FileName.Buffer) == OBJ_NAME_PATH_SEPARATOR)) {
                return(STATUS_OBJECT_PATH_SYNTAX_BAD);
            }

            // Find the name of the root directory and append the name of the relative object to it.
            Status = ZwQueryObject(CapturedAttributes.RootDirectory, ObjectNameInformation, ObjectNameInfo, sizeof(ObjectNameInfo), &ObjectNameLength);
            ...
            // Overflow test: If Length overflows the USHRT_MAX value cleanup and return STATUS_OBJECT_PATH_INVALID
            if (Length > 0xFFFF) {
...
*/
{
    ...
    CmpClearListEntry(&(PostBlock->CancelPostList));// Protect for multiple deletion of the same object

    if (PostBlock->PostKeyBody) {// Cleanup for objects referenced by NtNotifyMultipleKeys
        ASSERT(PostBlock->PostKeyBody->KeyBody);// If we have a PostKeyBody, the attached key body must not be NULL
        ASSERT(IsListEmpty(&(PostBlock->PostKeyBody->KeyBodyList)));// KeyBodyList must be used only in CmpPostBlock implementation for the delayed dereferencing mechanism.
        ObDereferenceObjectDeferDelete(PostBlock->PostKeyBody->KeyBody);// dereference the actual keybody
        ExFreePool(PostBlock->PostKeyBody);// Free the PostKeyBody structure
    }

    if (IsMasterPostBlock(PostBlock)) {
        switch (PostBlockType(PostBlock)) // this members are allocated only for master post blocks
        {
    ...
    ExFreePool(PostBlock);// and the storage for the Post object
}
...
/*
Routine Description:
    Allocates a post block from pool.  The non-pageable stuff comes from NonPagedPool, the pageable stuff from paged pool.  Quota will be charged.
Arguments:
    ...
    PostFlags      - specifies the flags to be set on the allocated post block
                valid flags:
                    - REG_NOTIFY_MASTER_POST - the post block to be allocated is a master post block.
    KeyBody     - The Key object to whom this post block is attached.
                  On master blocks this is NULL.
                  When the post object is freed, the KeyBody object is dereferenced (if not NULL - i.e. for slave blocks).
                  This allow us to perform back-end cleanup for "fake-slave" keys opened by NtNotifyMultipleKeys
...
*/
{
    ...
    ASSERT(!PostFlags || (!MasterBlock && !KeyBody));// protection against outrageous calls
    ...
    if (IsMasterPostBlock(PostBlock)) {
        ...
        PostBlock->u = ALLOCATE_WITH_QUOTA(NonPagedPool, sizeof(CM_POST_BLOCK_UNION), CM_FIND_LEAK_TAG44);// master post block ==> allocate the storage
        if (PostBlock->u == NULL) {
        ...
        PostBlock->u = MasterBlock->u;// Slave post block ==> copy storage allocated for the master post block
...
/*
Routine Description:
    Dumps all the subkeys of the target key that are kept open by some other process; Returns the number of open subkeys
Arguments:
    TargetKey - specifies the path to a key to link the hive to.
                path must be of the form "\registry\user\<username>"
*/
{
    ...
    try {
        if (PreviousMode == UserMode) {
            ...
            CapturedAttributes = ProbeAndReadStructure(TargetKey, OBJECT_ATTRIBUTES);// probe and capture the ObjectAttributes as we shall use it for opening the kernel handle
            ProbeAndReadUnicodeStringEx(&CapturedObjectName, CapturedAttributes.ObjectName);
            ProbeForRead(CapturedObjectName.Buffer, CapturedObjectName.Length, sizeof(WCHAR));
            CapturedAttributes.ObjectName = &CapturedObjectName;
        } else {
    ...
    if (NT_SUCCESS(Status)) {
        ...
        RtlInitUnicodeString(&HiveName, (PCWSTR)Hive->BaseBlock->FileName);// Dump the hive name and hive address
#if DBG
        DbgPrintEx(DPFLTR_CONFIG_ID, DPFLTR_ERROR_LEVEL, "\n Subkeys open inside the hive (%p) (%.*S) :\n\n", Hive, HiveName.Length / sizeof(WCHAR), HiveName.Buffer);
#endif

        CmpOpenSubKeys = CmpSearchForOpenSubKeys(KeyBody->KeyControlBlock, SearchAndCount, FALSE, NULL);// dump open subkeys (if any)
        CmpUnlockTwoHashEntries(ConvKey1, ConvKey2);
        CmpUnlockRegistry();
        END_LOCK_CHECKPOINT;
        ObDereferenceObject((PVOID)KeyBody);
        try {
            *HandleCount = CmpOpenSubKeys;// protect user mode memory
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            Status = GetExceptionCode();
        }
...
*/
{
    ...
    try {
        ...
        if (PreviousMode == UserMode) {// probe and capture the ObjectAttributes as we shall use it for opening the kernel handle
            CapturedAttributes = ProbeAndReadStructure(TargetKey, OBJECT_ATTRIBUTES);
            ProbeAndReadUnicodeStringEx(&CapturedObjectName, CapturedAttributes.ObjectName);
            ProbeForRead(CapturedObjectName.Buffer, CapturedObjectName.Length, sizeof(WCHAR));
            CapturedAttributes.ObjectName = &CapturedObjectName;
            ProbeForWriteUlong(RequiredSize);
            ProbeForWrite(QueryContext.Buffer, BufferLength, sizeof(ULONG));
        } else {
            CapturedAttributes = *TargetKey;
        }
    ...
    if (NT_SUCCESS(Status)) {
        ...
        ObDereferenceObject((PVOID)KeyBody);
        try {
            *RequiredSize = QueryContext.RequiredSize;// protect user mode memory
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            Status = GetExceptionCode();
        }
...
*/
{
    ...
    try {
        ...
        // validate new name
        if ((LocalKeyName.Length > REG_MAX_KEY_NAME_LENGTH) || (LocalKeyName.Length == 0) || ((LocalKeyName.Length & (sizeof(WCHAR) - 1)) != 0)) {
            return STATUS_INVALID_PARAMETER;
        }
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            CmPostCallbackNotification(RegNtPostRenameKey, KeyBody, status);// just a notification; disregard the return status
        }
...
/*
Routine Description:
    Compacts the keys in the given array together, so they will end up in the same bin (or adjacent)
Arguments:
...
*/
{
    ...
    for (i = 0; i < Count; i++) {
        ...
        if (i > 0) {
            if (KeyHive != (KeyBodyArray[i])->KeyControlBlock->KeyHive) {
                status = STATUS_INVALID_PARAMETER;// not same hive
                goto Exit;
            }
        } else {
    ...
    for (i = 0; i < Count; i++) {
        ...
        if (!NT_SUCCESS(status2) && NT_SUCCESS(status)) {
            status = status2;// record the status and go on with the remaining
        }
    }
...
/*
Routine Description:
    Compresses the specified key (must be the root of a hive), by simulating an "in-place" SaveKey.
Arguments:
