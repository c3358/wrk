...
VOID KeCheckIfStackExpandCalloutActive(VOID)
/*
Routine Description:
    This function check whether a kernel stack expand callout is active for the current thread
*/
{
    PKTHREAD Thread = KeGetCurrentThread();
    PKERNEL_STACK_CONTROL Control = (PKERNEL_STACK_CONTROL)Thread->InitialStack;

    if (Control->Previous.StackBase != 0) {
        KeBugCheckEx(KERNEL_EXPAND_STACK_ACTIVE, (ULONG64)Thread, 0, 0, 0);// and bugchecks if the result is positive.
    }
}
...
*/
{
    ...
    KeCheckIfStackExpandCalloutActive();// Check if a kernel stack expand callout is active.

    // Get the user mode stack pointer and attempt to copy input buffer to the user stack.
    TrapFrame = KeGetCurrentThread()->TrapFrame;
    OldStack = TrapFrame->Rsp;
    try {// Compute new user mode stack address, probe for writability, and copy the input buffer to the user stack.
        ULONG Length = ((InputLength + STACK_ROUND) & ~STACK_ROUND) + UCALLOUT_FRAME_LENGTH;
    ...
    Status = KiCallUserMode(OutputBuffer, OutputLength);// Call user mode.

    // When returning from user mode, any drawing done to the GDI TEB batch must be flushed.

    // N.B. It is possible to fault while referencing the user TEB. If a fault occurs, then always flush the batch count.
    BatchCount = 1;
    try {
