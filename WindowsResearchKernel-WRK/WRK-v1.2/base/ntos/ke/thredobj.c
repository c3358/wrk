...
Abstract:
    ...
    Functions are provided to initialize, ready, alert, test alert, boost priority, enable APC queuing, disable APC queuing, confine,
    set affinity, set priority, suspend, resume, alert resume, terminate, read thread state, freeze, unfreeze,
    query data alignment handling mode, force resume, and enter and leave critical regions for thread objects.
*/
...
NTSTATUS KeInitThread(__out PKTHREAD Thread,
    ...
    */
{
    ...
    // N.B. This is the only time the wait block is initialized since this information is constant.
    Timer = &Thread->Timer;
    ...
    // Allocate a kernel stack if necessary and set the initial kernel stack, stack base, and stack limit.
    if (KernelStack == NULL) {
...
VOID KeUninitThread(__inout PKTHREAD Thread)
/*
Routine Description:
...
VOID KeStartThread(__inout PKTHREAD Thread)
/*
Routine Description:
    This function initializes remaining thread fields and inserts the thread in the thread's process list.
    From this point on the thread must run.
Arguments:
    Thread - Supplies a pointer to a dispatcher object of type thread.
*/
{
    ...
    KeAcquireInStackQueuedSpinLockRaiseToSynch(&Process->ProcessLock, &LockHandle);// Raise IRQL to SYNCH_LEVEL and acquire the process lock.
    ...
    // N.B. It is guaranteed that the process affinity intersects the process ideal node affinity.
#if defined(NT_UP)
    ...
    // If possible bias the ideal processor to a different SMT set than the last thread.
#if defined(NT_SMT)
    ...
    // If the thread is the first entry in the process thread list and the process is not the idle process, 
    // then insert the process in the kernel process list.
#if defined(_AMD64_)
    if ((IsListEmpty(&Process->ThreadListHead) == TRUE) && (Process != &KiInitialProcess.Pcb)) {
        KeAcquireInStackQueuedSpinLockAtDpcLevel(&KiProcessListLock, &ListHandle);
        InsertTailList(&KiProcessListHead, &Process->ProcessListEntry);
        KeReleaseInStackQueuedSpinLockFromDpcLevel(&ListHandle);
    }
#endif

    KiLockDispatcherDatabaseAtSynchLevel();// Lock the dispatcher database.

    // Insert the thread in the process thread list and increment the kernel stack count.
    // N.B. The process is not swappable until its first thread has been initialized.
    InsertTailList(&Process->ThreadListHead, &Thread->ThreadListEntry);
    ASSERT(Process->StackCount != MAXULONG_PTR);
    Process->StackCount += 1;

    // Unlock the dispatcher database, release the process lock, and lower IRQL to its previous value.
    KiUnlockDispatcherDatabaseFromSynchLevel();
    KeReleaseInStackQueuedSpinLock(&LockHandle);
}


NTSTATUS KeInitializeThread(__out PKTHREAD Thread,
                            __in_opt PVOID KernelStack,
                            __in PKSYSTEM_ROUTINE SystemRoutine,
                            __in_opt PKSTART_ROUTINE StartRoutine,
                            __in_opt PVOID StartContext,
...
BOOLEAN KeAlertThread(__inout PKTHREAD Thread, __in KPROCESSOR_MODE AlertMode)
/*
Routine Description:
...
*/
{
    ...
    Alerted = Thread->Alerted[AlertMode];// Capture the current state of the alerted variable for the specified processor mode.
    if (Alerted == FALSE) {// If the alerted state for the specified processor mode is Not-Alerted, then attempt to alert the thread.
    ...
    // Unlock the dispatcher database from SYNCH_LEVEL, release the thread APC queue lock, exit the scheduler,
...
ULONG KeAlertResumeThread(__inout PKTHREAD Thread)
/*
Routine Description:
...
*/
{
    ...
    if (Thread->Alerted[KernelMode] == FALSE) {// If the kernel mode alerted state is FALSE, then attempt to alert the thread for kernel mode.
        ...
        if ((Thread->State == Waiting) && (Thread->Alertable == TRUE)) {
            KiUnwaitThread(Thread, STATUS_ALERTED, ALERT_INCREMENT);
        } else {// Else set the kernel mode alerted variable.
            Thread->Alerted[KernelMode] = TRUE;
        }
    }

    OldCount = Thread->SuspendCount;// Capture the current suspend count.
    if (OldCount != 0) {// If the thread is currently suspended, then decrement its suspend count.
    ...
    // Unlock the dispatcher database from SYNCH_LEVEL, release the thread APC queue lock, 
    // exit the scheduler, and return the previous suspend count.
    KiUnlockDispatcherDatabaseFromSynchLevel();
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
    KiExitDispatcher(LockHandle.OldIrql);
    return OldCount;
...
VOID KeBoostPriorityThread(__inout PKTHREAD Thread, __in KPRIORITY Increment)
/*
Routine Description:
...
*/
{
    ...
    if (Thread->Priority < LOW_REALTIME_PRIORITY) {// If the thread does not run at a realtime priority level, then boost the thread priority.
...
*/
{
    ...
    KeAcquireInStackQueuedSpinLockRaiseToSynch(&Thread->ApcQueueLock, &LockHandle);// Raise IRQL to SYNCH_LEVEL and acquire the thread APC queue lock.
    OldCount = Thread->SuspendCount + Thread->FreezeCount;// Capture the current suspend count.

    if (OldCount != 0) {// If the thread is currently suspended, then force resumption of thread execution.
    ...
    // Unlock he thread APC queue lock, exit the scheduler, and return the previous suspend count.
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
    KiExitDispatcher(LockHandle.OldIrql);
    return OldCount;
}
...
*/
{
    ...
    // Set the address of the current thread object and the current process object.
    CurrentThread = KeGetCurrentThread();
    Process = CurrentThread->ApcState.Process;

    KeAcquireInStackQueuedSpinLockRaiseToSynch(&Process->ProcessLock, &ProcessHandle);// Raise IRQL to SYNCH_LEVEL and acquire the process lock.

    // If the freeze count of the current thread is not zero, then there is another thread that is trying to freeze this thread.
    // Unlock the the process lock and lower IRQL to its previous value, 
    // allow the suspend APC to occur, then raise IRQL to SYNCH_LEVEL and lock the process lock.
    ...
    do {
        Thread = CONTAINING_RECORD(NextEntry, KTHREAD, ThreadListEntry);// Get the address of the next thread.
        KeAcquireInStackQueuedSpinLockAtDpcLevel(&Thread->ApcQueueLock, &ThreadHandle);// Acquire the thread APC queue lock.

        // If the thread is not the current thread and APCs are queueable, then attempt to suspend the thread.
        if ((Thread != CurrentThread) && (Thread->ApcQueueable == TRUE)) {
            ...
            // N.B. The APC MUST be queued using the internal interface so the system argument fields of the APC do not get written.
        ...
        KeReleaseInStackQueuedSpinLockFromDpcLevel(&ThreadHandle);// Release the thread APC queue lock.
...
LOGICAL KeQueryAutoAlignmentThread(__in PKTHREAD Thread)
/*
Routine Description:
...
LONG KeQueryBasePriorityThread(__in PKTHREAD Thread)
/*
Routine Description:
...
*/
{
    ...
    // Release the thread lock, lower IRQL to its previous value, and return the previous thread base priority increment.
...
KPRIORITY KeQueryPriorityThread(__in PKTHREAD Thread)
/*
Routine Description:
...
ULONG KeQueryRuntimeThread(__in PKTHREAD Thread, __out PULONG UserTime)
/*
Routine Description:
...
BOOLEAN KeReadStateThread(__in PKTHREAD Thread)
/*
Routine Description:
...
*/
{
    ...
    // Raise IRQL to dispatcher level, lock dispatcher database, ready the specified thread for execution,
...
ULONG KeResumeThread(__inout PKTHREAD Thread)
/*
Routine Description:
...
*/
{
    ...
    KeAcquireInStackQueuedSpinLockRaiseToSynch(&Thread->ApcQueueLock, &LockHandle);// Raise IRQL to SYNCH_LEVEL and lock the thread APC queue.
    OldCount = Thread->SuspendCount;// Capture the current suspend count.
    if (OldCount != 0) {// If the thread is currently suspended, then decrement its suspend count.
        Thread->SuspendCount -= 1;
    ...
    // Release the thread APC queue, exit the scheduler, and return the previous suspend count.
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
    KiExitDispatcher(LockHandle.OldIrql);
    return OldCount;
}
...
*/
{
    ...
    CurrentThread->Affinity = CurrentThread->UserAffinity;// Set the current affinity to the user affinity and the ideal processor to the user ideal processor.
...
/*
Routine Description:
    This function is called by the executive to rundown thread structures which must be guarded by the dispatcher database lock
    and which must be processed before actually terminating the thread.
...
*/
{
    ...
    // If the mutant is a kernel mutex, then bugcheck.
    ...
    KiUnlockDispatcherDatabase(OldIrql);// Release dispatcher database lock and lower IRQL to its previous value.
}


KAFFINITY KeSetAffinityThread(__inout PKTHREAD Thread, __in KAFFINITY Affinity)
/*
Routine Description:
...
*/
{
    ...
    KiLockDispatcherDatabase(&OldIrql);// Raise IRQL to dispatcher level and lock dispatcher database.
    OldAffinity = KiSetAffinityThread(Thread, Affinity);// Set the thread affinity to the specified value.
...
VOID KeSetSystemAffinityThread(__in KAFFINITY Affinity)
/*
Routine Description:
...
*/
{
    ...
    // Set the current affinity to the specified affinity and set system affinity active.
    CurrentThread->Affinity = Affinity;
    CurrentThread->SystemAffinityActive = TRUE;

    // If the ideal processor is not a member of the new affinity set, then recompute the ideal processor.
    // N.B. System affinity is only set temporarily, and therefore,
    // the ideal processor is set to a convenient value if it is not already a member of the new affinity set.
#if !defined(NT_UP)
...
LONG KeSetBasePriorityThread(__inout PKTHREAD Thread, __in LONG Increment)
/*
Routine Description:
...
*/
{
    ...
    // Acquire the thread lock, capture the base priority of the specified thread, and determine whether saturation if being forced.
    KiAcquireThreadLock(Thread);
    ...
    // Set the base priority of the specified thread. If the thread's process is in the realtime class, then limit the change to the realtime class.
    ...
    // Release the thread lock, unlock the dispatcher database, lower IRQL to its previous value, and return the previous thread base priority.
    KiReleaseThreadLock(Thread);
    KiUnlockDispatcherDatabase(OldIrql);
    return OldIncrement;
}
...
LOGICAL KeSetAutoAlignmentThread(__inout PKTHREAD Thread, __in LOGICAL Enable)
/*
Routine Description:
    This function sets the data alignment handling mode for the specified thread.
Arguments:
    Thread - Supplies a pointer to a dispatcher object of type thread.
    Enable - Supplies a boolean value that determines the handling of data alignment exceptions for the specified thread.
Return Value:
    The previous value of auto alignment for the specified thread is returned as the function value.
*/
{
    ASSERT_THREAD(Thread);

    // Capture the previous data alignment handling mode and set the specified data alignment mode.
    if (Enable != FALSE) {
        return InterlockedBitTestAndSet(&Thread->ThreadFlags, KTHREAD_AUTO_ALIGNMENT_BIT);
    } else {
        return InterlockedBitTestAndReset(&Thread->ThreadFlags, KTHREAD_AUTO_ALIGNMENT_BIT);
    }
}


LOGICAL KeSetDisableBoostThread(__inout PKTHREAD Thread, __in LOGICAL Disable)
/*
Routine Description:
    This function sets the disable priority boost state for the specified thread.
Arguments:
    Thread  - Supplies a pointer to a dispatcher object of type thread.
    Disable - Supplies a logical value that determines whether priority boosts are disabled or enabled.
Return Value:
    The previous value of the disable boost for the specfied thread is returned as the function value.
*/
{
    ASSERT_THREAD(Thread);

    // Capture the current state of the disable boost variable and set its state to the specified value.
    if (Disable != FALSE) {
        return InterlockedBitTestAndSet(&Thread->ThreadFlags, KTHREAD_DISABLE_BOOST_BIT);
    } else {
        return InterlockedBitTestAndReset(&Thread->ThreadFlags, KTHREAD_DISABLE_BOOST_BIT);
    }
}


UCHAR KeSetIdealProcessorThread(__inout PKTHREAD Thread, __in UCHAR Processor)
/*
Routine Description:
    This function sets the ideal processor for the specified thread execution.
    N.B. If the specified processor is less than the number of processors in the system and is a member of the specified thread's current affinity set,
         then the ideal processor is set.
...
*/
{
    ...
    // Raise IRQL, lock the dispatcher database, and capture the previous ideal processor value.
    KiLockDispatcherDatabase(&OldIrql);
    OldProcessor = Thread->UserIdealProcessor;

    // If the specified processor is less than the number of processors in the system and is a member of the specified thread's current affinity set,
    // then the ideal processor is set.
    // Otherwise, no operation is performed.
    if ((Processor < KeNumberProcessors) && ((Thread->Affinity & AFFINITY_MASK(Processor)) != 0)) {
    ...
    // Unlock dispatcher database, lower IRQL to its previous value, and return the previous ideal processor.
    KiUnlockDispatcherDatabase(OldIrql);
    return OldProcessor;
}


BOOLEAN KeSetKernelStackSwapEnable(__in BOOLEAN Enable)
/*
Routine Description:
...
KPRIORITY KeSetPriorityThread(__inout PKTHREAD Thread, __in KPRIORITY Priority)
/*
Routine Description:
    This function sets the priority of the specified thread to a new value.
    If the new thread priority is lower than the old thread priority, then rescheduling may take place if the thread is currently running on,
    or about to run on, a processor.
Arguments:
    Thread  - Supplies a pointer to a dispatcher object of type thread.
...
*/
{
    ...
    // Raise IRQL to dispatcher level and lock dispatcher database.
    // Acquire the thread lock, capture the current thread priority, set the thread priority to the the new value, and replenish the thread quantum.
    ...
    if (Priority != Thread->Priority) {
        Thread->Quantum = Thread->QuantumReset;
        if ((Thread->BasePriority != 0) && (Priority == 0)) {
            Priority = 1;
        }
    ...
    // Release the thread lock, unlock the dispatcher database, lower IRQL to its previous value, and return the previous thread priority.
    KiReleaseThreadLock(Thread);
    KiUnlockDispatcherDatabase(OldIrql);
    return OldPriority;
}
...
VOID KeSetPriorityZeroPageThread(__in KPRIORITY Priority)
/*
Routine Description:
    This function sets the priority and base priority of the current thread to a new value.
    If the new thread priority is lower than the old thread priority, then rescheduling may take place if the thread is currently running on,
    or about to run on, a processor.
    N.B. This function is for use only by memory managment to change the priority of the zero page thread.
...
*/
{
    ...
    // Release the thread lock, unlock the dispatcher database, and lower IRQL to its previous value.
    KiReleaseThreadLock(Thread);
    KiUnlockDispatcherDatabase(OldIrql);
}
...
*/
{
    ...
    // If the suspend count is at its maximum value, then unlock the dispatcher database,
    ...
    // Don't suspend the thread if APC queuing is disabled. In this case the thread is being deleted.
    ...
    // Release the thread APC queue lock, exit the scheduler, and  return the old count.
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
    KiExitDispatcher(LockHandle.OldIrql);
    return OldCount;
}
...
VOID KeTerminateThread(__in KPRIORITY Increment)
/*
Routine Description:
    This function terminates the execution of the current thread, sets the signal state of the thread to Signaled,
    and attempts to satisfy as many Waits as possible.
...
*/
{
    ...
    // If the thread is the last entry in the process thread list, then remove the process from the kernel process list.
#if defined(_AMD64_)
    ...
    // Sum the kernel and user time of the thread into the process kernel and user times.
    ...
    // then remove the thread from the queue object thread list and attempt to activate another thread that is blocked on the queue object.
    Queue = Thread->Queue;
    if (Queue != NULL) {
    ...
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);// Release the process lock, but don't lower the IRQL.

    // Set thread scheduling state to terminated, decrement the process' stack count,
    // and initiate an outswap of the process if the stack count is zero.
    ...
    // Unlock the dispatcher database and get off the processor for the last time.
    KiUnlockDispatcherDatabaseFromSynchLevel();
    KiSwapThread(Thread, KeGetCurrentPrcb());
}


BOOLEAN KeTestAlertThread(__in KPROCESSOR_MODE AlertMode)
/*
Routine Description:
...
*/
{
    ...
    // Release the thread APC queue lock, lower IRQL to its previous value, and return the previous alerted state for the specified mode.
    KeReleaseInStackQueuedSpinLock(&LockHandle);
    return Alerted;
}
...
*/
{
    ...
    // Thaw the execution of all threads in the current process that have been frozen.
    ListHead = &Process->ThreadListHead;
    NextEntry = ListHead->Flink;
    do {
        Thread = CONTAINING_RECORD(NextEntry, KTHREAD, ThreadListEntry);// Get the address of the next thread.
    ...
    // Release the process lock, exit the scheduler, and leave critical region.
    KeReleaseInStackQueuedSpinLockFromDpcLevel(&ProcessHandle);
    KiExitDispatcher(ProcessHandle.OldIrql);
