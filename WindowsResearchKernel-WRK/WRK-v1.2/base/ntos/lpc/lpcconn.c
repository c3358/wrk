...
Routine Description:
    ...
    The PortName parameter specifies the name of the server port to connect to.
    It must correspond to an object name specified on a call to NtCreatePort.
    The service sends a connection request to the server thread that is listening for them with the NtListenPort service.
    The client thread then blocks until a server thread receives the connection request and responds with a call to the NtCompleteConnectPort service.
    The server thread receives the ID of the client thread, along with any information passed via the ConnectionInformation parameter.
    The server thread then decides to either accept or reject the connection request.

    The server communicates the acceptance or rejection with the NtCompleteConnectPort service.
    The server can pass back data to the client about the acceptance or rejection via the ConnectionInformation data block.

    If the server accepts the connection request, then the client receives a communication port object in the location pointed to by the PortHandle parameter.
    This object handle has no name associated with it and is private to the client process (i.e.  it cannot be inherited by a child process).
    ...
    If the server rejects the connection request, then no communication port object handle is returned, and the return status indicates an error occurred.
    The server may optionally return information in the ConnectionInformation data block giving the reason the connection requests was rejected.
...
*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            if (ARGUMENT_PRESENT(RequiredServerSid)) {
                Status = SeCaptureSid(RequiredServerSid, PreviousMode, NULL, 0, PagedPool, TRUE, &CapturedRequiredServerSid);
    ...
    //  If this is NtSecureConnectPort, validated the required SID against the SID of the server process.  Fail if not equal.
    if (ARGUMENT_PRESENT(RequiredServerSid)) {
        ...
        //  We are all done with the required server sid if specified so now release one if we had to capture it
        if (CapturedRequiredServerSid != RequiredServerSid) {
            SeReleaseSid(CapturedRequiredServerSid, PreviousMode, TRUE);
        }
    ...
    //  Note, that from here on, none of the error paths dereference the connection port pointer, just the newly created client port pointer.
    //  The port delete routine will get called when the client port is deleted and it will dereference the connection port pointer stored in the client port object.
    ...
    //  Set the security tracking mode, and initialize the client security context if it is static tracking.
    ...
    //  If client has allocated a port memory section, then map a view of that section into the client's address space.
    //  Also reference the section object so we can pass a pointer to the section object in connection request message.
    ...
    if (ARGUMENT_PRESENT(ClientView)) {
        ...
        //  We'll add an extra-reference to the current process, when we have a section mapped in that process.
    ...
    //  Adjust the size of the connection info length that the client supplied to be the no longer than one the connection port will accept
    ...
    //  If we have a client view then copy over the client view information otherwise we'll zero out all of the view information
    if (ARGUMENT_PRESENT(ClientView)) {
    ...
    Msg->Request.u1.s1.DataLength = (CSHORT)(sizeof(*ConnectMsg) + ConnectionInfoLength);//  The data length is everything after the port message within the lpcp message.  In other words the connection message and the user supplied information
    Msg->Request.u1.s1.TotalLength = (CSHORT)(sizeof(*Msg) + Msg->Request.u1.s1.DataLength);//  The total length add on the LPCP message
    Msg->Request.u2.s2.Type = LPC_CONNECTION_REQUEST;//  This will be a connection request message
    ...
    //  Also acquire the semaphore that guards the connection request message queue.
    //  Stamp the connection request message with a serial number, 
    //  insert the message at the tail of the connection request message queue and remember the address of the message in the LpcReplyMessage field for the current thread.
    Status = STATUS_SUCCESS;
    LpcpAcquireLpcpLockByThread(CurrentThread);

    //  See if the port name has been deleted from under us.  If so, then don't queue the message and don't wait for a reply
    if (ConnectionPort->Flags & PORT_NAME_DELETED) {
        Status = STATUS_OBJECT_NAME_NOT_FOUND;
    } else {
        LpcpTrace(("Send Connect Msg %lx to Port %wZ (%lx)\n", Msg, PortName, ConnectionPort));

        //  Stamp the request message with a serial number, insert the message at the tail of the request message queue
        ...
        //  Reference the port we are passing in the connect msg so if we die it will still be valid for the server in NtAcceptConnectPort.  
        //  The reference will be released when the message is freed.
        ObReferenceObject(ClientPort);
        ConnectMsg->ClientPort = ClientPort;
        KeEnterCriticalRegionThread(&CurrentThread->Tcb);
    }

    //  Add an extra-reference to the connection port to prevent going away if the server closes the handle.
    //  The reference we already have is not enough because closing the connection port will delete the connection message from
    //  the queue, which will delete the client port, which will dereference the connection port.
    //  Therefore right after releasing the lock the connection port might be invalid in the absence of this extra reference.
    ObReferenceObject(ConnectionPort);
    LpcpReleaseLpcpLock();

    //  At this point the client's communication port is all set up and the connection request message is in the server's queue.  
    //  So now we have to single the server and wait for a reply
    if (NT_SUCCESS(Status)) {
        ...
        //  Increment the connection request message queue semaphore by one for the newly inserted connection request message. 
        //  Release the spin locks, while remaining at the dispatcher IRQL.  
        //  Then wait for the reply to this connection request by waiting on the LpcReplySemaphore for the current thread.
    ...
    //  A connection request is accepted if the ConnectedPort of the client's communication port has been filled in.
    if (Status == STATUS_SUCCESS) {
        ...
        if (Msg != NULL) {
            ...
            //  Insert client communication port object in specified object table.  Set port handle value if successful.
            ...
            if (ClientPort->ConnectedPort != NULL) {
                ...
                //  Before we do the object insert we need to get the max message length because right after the call the object could be dereferenced and gone away
                ...
                //  Synchronize with the deletion path for the port object If the server accepted the connection and immediately closed the server handle, 
                //  the ConnectionPort field will be NULL.
                //  If the server closed the connection port as well, the captured value for the connection port will be invalid.
                LpcpAcquireLpcpLockByThread(CurrentThread);
            ...
            LpcpFreeToPortZone(Msg, 0);//  Free the reply message back to the port zone
        } else {//  We did not get a reply message so the connection must have been refused
            if (SectionToMap != NULL) {
                ObDereferenceObject(SectionToMap);
            }
    ...
    } else {//  Our wait was not successful
        //  Remove the connection request message from the received queue and free the message back to the connection port's zone.
        SectionToMap = LpcpFreeConMsg(&Msg, &ConnectMsg, CurrentThread);
        ...
        ObDereferenceObject(ClientPort);//  If the connection was rejected or the wait failed, then dereference the client port object, which will cause it to be deleted.
    }

    ObDereferenceObject(ConnectionPort);//  Remove the extra reference we added to the connection port
...
*/
{
    ...
    //  Acquire the LPC mutex, remove the connection request message from the received queue and free the message back to the connection port's zone.
    LpcpAcquireLpcpLock();
    ...
    } else {//  Otherwise there is no LPC message to be handle so we'll return null's to our caller
        *Msg = NULL;
        SectionToMap = NULL;
    }
