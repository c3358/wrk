...
Abstract:
    This module contains the routines to manipulate physical memory from user space.

    There are restrictions on how user controlled physical memory can be used.
    Realize that all this memory is nonpaged and hence applications should allocate this with care as it represents a very real system resource.
    ...
    2.  The same physical page cannot be mapped at 2 different virtual addresses.
    3.  Callers must have LOCK_VM privilege to create these VADs.
    4.  Device drivers cannot call MmSecureVirtualMemory on it - this means that applications should not expect to use this memory for win32k.sys calls.
    5.  NtProtectVirtualMemory only allows read-write protection on this memory.  No other protection (no access, guard pages, readonly, etc) are allowed.
    6.  NtFreeVirtualMemory allows only MEM_RELEASE and NOT MEM_DECOMMIT on these VADs.  Even MEM_RELEASE is only allowed on entire VAD ranges - that is, splitting of these VADs is not allowed.
...
NTSTATUS MiCaptureUlongPtrArray(OUT PVOID Destination, IN PVOID Source, IN ULONG_PTR ArraySize);
...
// This local stack size definition is deliberately large as ISVs have told us they expect to typically do up to this amount.
...
Arguments:
    VirtualAddress - Supplies a user virtual address within a UserPhysicalPages Vad.
    NumberOfPages - Supplies the number of pages to map.
    UserPfnArray - Supplies a pointer to the page frame numbers to map in.
                   If this is zero, then the virtual addresses are set to NO_ACCESS.
Return Value:
...
*/
{
    ...
    // Note we deliberately pass MiHighestUserPte here to the macro because the user may have passed a kernel virtual address.
    // We don't check the virtual address till element lookup below but don't want the PTE construction macro to assert.
    MI_MAKE_VALID_USER_PTE(NewPteContents, PageFrameIndex, MM_READWRITE, MiHighestUserPte);
    MI_SET_PTE_DIRTY(NewPteContents);
    PteFlushList.Count = 0;

    // A memory barrier is needed to read the EPROCESS AweInfo field in order to ensure the writes to the AweInfo structure fields are visible in correct order.
    // This avoids the need to acquire any stronger synchronization (ie: spinlock/pushlock, etc) in the interest of best performance.
    KeMemoryBarrier();
    ...
    // Pushlock protection protects insertion/removal of Vads into each process' AweVadList.
    // It also protects creation/deletion and adds/removes of the VadPhysicalPagesBitMap.
    // Finally, it protects the PFN modifications for pages in the bitmap.
    ...
    if ((PhysicalView != NULL) && (PhysicalView->VadType == VadAwe) && (VirtualAddress >= MI_VPN_TO_VA(PhysicalView->StartingVpn)) && (EndAddress <= MI_VPN_TO_VA_ENDING(PhysicalView->EndingVpn))) {
        NOTHING;
    } else {
    ...
    // Ensure the PFN element corresponding to each specified page is owned by the specified VAD.

    // Since this ownership can only be changed while holding this process' working set lock, the PFN can be scanned here without holding the PFN lock.

    // Note the PFN lock is not needed because any race with MmProbeAndLockPages can only result in the I/O going to the old page or the new page.
    // If the user breaks the rules, the PFN database (and any pages being windowed here) are still protected because of the reference counts on the pages with inprogress I/O.
    // This is possible because NO pages are actually freed here - they are just windowed.
    if (ARGUMENT_PRESENT(UserPfnArray)) {
        // By keeping the PFN bitmap in the VAD (instead of in the PFN database itself), a few benefits are realized:
        ...
        // 4. Less nonpaged memory is used (for the bitmap vs adding a field to the PFN) on systems with no unused pack space in the PFN database, presuming not many of these VADs get allocated.
#if defined(_AMD64_)
        // Perform a prefetchw of the PFN database cachelines that will be updated later.

        // Note that at this point the page frame numbers haven't been validated and may in fact be completely bogus.
        // Prefetch semantics allow this.
        do {
        ...
        // N.B.  This implies that PFN_NUMBER is always ULONG_PTR in width as PFN_NUMBER is not exposed to application code today.
        ...
        do {
            ...
            // Also guard against the malicious user issuing more than one remap request for all or portions of the same region simultaneously.
            Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
            ASSERT(MI_PFN_IS_AWE(Pfn1));
            ASSERT(Pfn1->u2.ShareCount == 1);
            OldPte = InterlockedCompareExchangePointer(&Pfn1->PteAddress, PointerPte, NULL);
            if (OldPte != NULL) {
        ...
        // Even though we have already validated the new PFNs are not mapped anywhere, an interlocked sequence must still be used on the
        // target PTE to prevent a concurrent thread trying to map a different PFN at this address from corrupting things.
        ...
        do {
            ...
            OldPteContents.u.Long = InterlockedExchangePte(PointerPte, NewPteContents.u.Long);

            // The PTE is now pointing at the new frame.
            // Note that another thread can immediately access the page contents via this PTE even though they're not supposed to until this API returns.
            // Thus, the page frames are handled carefully so that malicious apps cannot corrupt frames they don't really still or yet own.
            if (OldPteContents.u.Hard.Valid == 1) {
                // The old frame was mapped so the TB entry must be flushed.
                // Note the app could maliciously dirty data in the old frame until the TB flush completes, so don't allow frame reuse till then (although allowing remapping within this process is ok).
        ...
        while (PointerPte < LastPte) {
            ...
            // Note that another thread can still be accessing the page contents via the stale PTE until the TB entry is flushed even though they're not supposed to.
            // Thus, the page frames are handled carefully so that malicious apps cannot corrupt frames they don't still own.
            if (OldPteContents.u.Hard.Valid == 1) {
                // The old frame was mapped so the TB entry must be flushed.
                // Note the app could maliciously dirty data in the old frame until the TB flush completes, so don't allow frame reuse till then (although allowing remapping within this process is ok).
                Pfn1 = MI_PFN_ELEMENT(OldPteContents.u.Hard.PageFrameNumber);
    ...
    // Note this can be done without holding the AWE pushlock because the PTEs have already been filled so any concurrent (bogus) map/unmap call will see the right entries.
    // AND any free of the physical pages will also see the right
    // entries (although the free must do a TB flush while holding the AWE pushlock exclusive to ensure no thread gets to continue using a stale mapping to the page being freed prior to the flush below).
...
Routine Description:
    ...
    Note no WSLEs are maintained for this range as it is all nonpaged.
Arguments:
    VirtualAddresses - Supplies a pointer to an array of user virtual addresses within UserPhysicalPages Vads.  Each array entry is presumed to map a single page.
    NumberOfPages - Supplies the number of pages to map.
    UserPfnArray - Supplies a pointer to the page frame numbers to map in.
                   If this is zero, then the virtual addresses are set to NO_ACCESS.
                   If the array entry is zero then just the corresponding virtual address is set to NO_ACCESS.
Return Value:
    Various NTSTATUS codes.
*/
{
    ...
    if (NumberOfPages > SMALL_COPY_STACK_SIZE) {
        ...
        PoolVirtualAreaBase = ExAllocatePoolWithTag(NonPagedPool, NumberOfPoolBytes, 'wRmM');
    ...
    Status = MiCaptureUlongPtrArray(PoolVirtualArea, VirtualAddresses, NumberOfPages);
    if (!NT_SUCCESS(Status)) {
        goto ErrorReturn;
    }
    ...
    // Note we deliberately pass MiHighestUserPte here to the macro because the user may have passed a kernel virtual address.
    // We don't check the virtual address till element lookup below but don't want the PTE construction macro to assert.
    MI_MAKE_VALID_USER_PTE(NewPteContents0, PageFrameIndex, MM_READWRITE, MiHighestUserPte);
    ...
    // Note that the PFN lock is not needed to traverse this list (even though MmProbeAndLockPages uses it), because the pushlock has been acquired.
    Processor = KeGetCurrentProcessorNumber();
    LocalPhysicalView = AweInfo->PhysicalViewHint[Processor];
    if ((LocalPhysicalView != NULL) && (LocalPhysicalView->VadType != VadAwe)) {
    ...
    do {
        ...
        if (LocalPhysicalView != NULL) {
            ...
            if ((VirtualAddress >= MI_VPN_TO_VA(LocalPhysicalView->StartingVpn)) && (VirtualAddress <= MI_VPN_TO_VA_ENDING(LocalPhysicalView->EndingVpn))) {
        ...
        if (PhysicalView != NULL) {
            ...
            if ((VirtualAddress >= MI_VPN_TO_VA(PhysicalView->StartingVpn)) && (VirtualAddress <= MI_VPN_TO_VA_ENDING(PhysicalView->EndingVpn))) {
                // The virtual address is within the hint so it's good.
                PoolVirtualArea += 1;
        ...
        if ((SearchResult == TableFoundNode) && (PhysicalView->VadType == VadAwe) && (VirtualAddress >= MI_VPN_TO_VA(PhysicalView->StartingVpn)) && (VirtualAddress <= MI_VPN_TO_VA_ENDING(PhysicalView->EndingVpn))) {
            ...
            // No virtual address is reserved at the specified base address, return an error.
            ExReleaseCacheAwarePushLockShared(PushLock);
            KeLeaveGuardedRegionThread(&CurrentThread->Tcb);
            Status = STATUS_INVALID_PARAMETER_1;
            goto ErrorReturn;
    ...
    PoolVirtualArea = PoolVirtualAreaBase;
    if (ARGUMENT_PRESENT(UserPfnArray)) {
        // By keeping the PFN bitmap in the process (instead of in the PFN database itself), a few benefits are realized:
        // 1. No need to acquire the PFN lock here.
        // 2. Faster handling of PFN databases with holes.
        // 3. Transparent support for dynamic PFN database growth.
        // 4. Less nonpaged memory is used (for the bitmap vs adding a field to the PFN) on systems with no unused pack space in the PFN database.
#if defined(_AMD64_)
        // Perform a prefetchw of the PFN database cachelines that will be updated later.

        // Note that at this point the page frame numbers haven't been
        // validated and may in fact be completely bogus.  Prefetch semantics allow this.
        do {
            PageFrameIndex = *FrameList;
            if (PageFrameIndex != 0) {
                Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
        ...
        // This pass actually inserts them all into the page table pages and the TBs now that we know the frames are good.
        // Check the PTEs and PFNs carefully as a malicious user may issue more than one remap request for all or portions of the same region simultaneously.
        FrameList = (PPFN_NUMBER)PoolArea;
        PoolVirtualArea = PoolVirtualAreaBase;
        do {
            PageFrameIndex = *FrameList;
        ...
        do {
            ...
            // The PTE is now zeroed.
            // Note that another thread can still maliciously dirty data in the old frame until the TB flush completes,
            // so don't allow frame reuse till then (although allowing remapping within this process is ok) to prevent the app from corrupting frames it doesn't really still own.
            if (OldPteContents.u.Hard.Valid == 1) {
                // The old frame was mapped so the TB entry must be flushed.
                Pfn1 = MI_PFN_ELEMENT(OldPteContents.u.Hard.PageFrameNumber);
                ASSERT(Pfn1->PteAddress != NULL);
    ...
    // Flush the TB entries for any relevant pages.
    // Note this can be done without holding the AWE pushlock because the PTEs have already been filled so any concurrent (bogus) map/unmap call will see the right entries.
    // AND any free of the physical pages will also see the right entries (although the free must do a TB flush while holding the AWE pushlock exclusive to ensure no thread gets to continue using a
...
/*
Routine Description:
    This function allocates an AWE structure for the current process.
    Note this structure is never destroyed while the process is alive in order to allow various checks to occur lock free.
Return Value:
    A non-NULL AweInfo pointer on success, NULL on failure.
Environment:
    Kernel mode, APC_LEVEL or below, address space mutex (or no locks at all) held.
*/
{
    PAWEINFO AweInfo;
    ...
    if (AweInfo != NULL) {
        ...
        RtlZeroMemory(&AweInfo->PhysicalViewHint, MAXIMUM_PROCESSORS * sizeof(PMI_PHYSICAL_VIEW));
        RtlZeroMemory(&AweInfo->AweVadRoot, sizeof(MM_AVL_TABLE));
...
*/
{
    ...
    // A memory barrier is needed to ensure the writes initializing the AweInfo fields are visible prior to setting the EPROCESS AweInfo pointer.
    // This is because the reads from these fields are done lock free for improved performance.
    // There is no need to explicitly add one here as the InterlockedCompare already has one.
    if (InterlockedCompareExchangePointer(&Process->AweInfo, AweInfo, NULL) != NULL) {
        // Another thread has already inserted the AWE info structure so free our caller's.
        ExFreeCacheAwarePushLock(AweInfo->PushLock);
        ExFreePool(AweInfo);
...
/*
Routine Description:
    This function allocates nonpaged physical pages for the specified subject process.

    No WSLEs are maintained for this range.

    The caller must check the NumberOfPages returned to determine how many pages were actually allocated (this number may be less than the requested amount).

    On success, the user array is filled with the allocated physical page frame numbers (only up to the returned NumberOfPages is filled in).

    No PTEs are filled here - this gives the application the flexibility to order the address space with no metadata structure imposed by the Mm.
    Applications do this via NtMapUserPhysicalPages - ie:

        - Each physical page allocated is set in the process's bitmap.
          This provides remap, free and unmap a way to validate and rundown these frames.

          Unmaps may result in a walk of the entire bitmap, but that's ok as unmaps should be less frequent.
          The win is it saves us from using up system virtual address space to manage these frames.

        - Note that the same physical frame may NOT be mapped at two different virtual addresses in the process.
          This makes frees and unmaps substantially faster as no checks for aliasing need be performed.
Arguments:
    ProcessHandle - Supplies an open handle to a process object.
    NumberOfPages - Supplies a pointer to a variable that supplies the desired size in pages of the allocation.  This is filled with the actual number of pages allocated.
    UserPfnArray - Supplies a pointer to user memory to store the allocated frame numbers into.
Return Value:
    Various NTSTATUS codes.
*/
{
    ...
    // Get the working set mutex to synchronize.
    // This also blocks APCs so an APC which takes a page fault does not corrupt various structures.
    ...
    if (BitMap == NULL) {
...
#if defined (_WIN64)
        // Force a 32-bit maximum on any page allocation because the bitmap package is currently 32-bit.
        ...
        // Charge quota for the nonpaged pool for the bitmap.
        // This is done here rather than by using ExAllocatePoolWithQuota so the process object is not referenced by the quota charge.
    ...
    // Don't use the low 16mb of memory so that at least some low pages are left for 32/24-bit device drivers.
    // Just under 4gb is the maximum allocation per MDL so the ByteCount field does not overflow.
    ...
    do {
        ...
        MemoryDescriptorList = MiAllocatePagesForMdl(LowAddress, HighAddress, SkipBytes, MdlRequestInPages << PAGE_SHIFT, MiCached, MI_ALLOCATION_IS_AWE);
        if (MemoryDescriptorList == NULL) {
            // No (more) pages available.  If this becomes a common situation, all the working sets could be flushed here.
        ...
        // pages freed by the exiting thread, but this thread is still responsible for freeing the pool containing the MDLs themselves.
        if (Process->Flags & PS_PROCESS_FLAGS_VM_DELETED) {
            UNLOCK_ADDRESS_SPACE(Process);
        ...
        // Recheck the process and job limits as they may have changed when the address space mutex was released above.
        if (AweInfo->VadPhysicalPagesLimit != 0) {
            if ((AweInfo->VadPhysicalPages >= AweInfo->VadPhysicalPagesLimit) || (AllocatedPages > AweInfo->VadPhysicalPagesLimit - AweInfo->VadPhysicalPages)) {
                UNLOCK_ADDRESS_SPACE(Process);
                MmFreePagesFromMdl(MemoryDescriptorList);
                ExFreePool(MemoryDescriptorList);
                if (TotalAllocatedPages == 0) {
        ...
        if (Process->JobStatus & PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES) {
            if (PsChangeJobMemoryUsage(PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES, AllocatedPages) == FALSE) {
                UNLOCK_ADDRESS_SPACE(Process);
                MmFreePagesFromMdl(MemoryDescriptorList);
                ExFreePool(MemoryDescriptorList);
                if (TotalAllocatedPages == 0) {
        ...
        // Note the PFN lock is not needed to modify the PFN fields below as these are brand new pages owned only by this thread.
        ...
        for (i = 0; i < AllocatedPages; i += 1) {
            ...
            // Once this bit is set (and the mutex released below), a rogue thread that is passing random frame numbers to NtFreeUserPhysicalPages can free this frame.
            // This means NO references can be made to it by this routine after this point without first re-checking the bitmap.
        ...
        // Try the same memory range again - there might be more pages left in it that can be claimed as a truncated MDL had to be used for the last request.
    } while (TRUE);
    ...
    // Establish an exception handler and carefully write out the number of pages and the frame numbers.
    try {

        ASSERT(TotalAllocatedPages <= CapturedNumberOfPages);

        // Deliberately only write out the number of pages if the operation succeeded.
        // This is because this was the behavior on Windows 2000.
        ...
        while (MemoryDescriptorList != NULL) {
            MdlPage = (PPFN_NUMBER)(MemoryDescriptorList + 1);
            AllocatedPages = MemoryDescriptorList->ByteCount >> PAGE_SHIFT;
            RtlCopyMemory((PVOID)UserPfnArray, MdlPage, AllocatedPages * sizeof(PFN_NUMBER));
            UserPfnArray += AllocatedPages;
            MemoryDescriptorList = MemoryDescriptorList->Next;
        }
    } except(ExSystemExceptionFilter())
    {
        // If anything went wrong communicating the pages back to the user then the user has really hurt himself because these addresses passed the probe tests at the beginning of the service.
        // Rather than carrying around extensive recovery code, just return back success as this scenario is the same as if the user scribbled over the output parameters after the service returned anyway.
        // You can't stop someone who's determined to lose their values !

        // Fall through...
    }

    // Free the space consumed by the MDLs now that the page frame numbers have been saved in the bitmap and copied to the user.
    MemoryDescriptorList = MemoryDescriptorHead;
    while (MemoryDescriptorList != NULL) {
...
/*
Routine Description:
    This function frees the nonpaged physical pages for the specified subject process.  Any PTEs referencing these pages are also invalidated.

    Note there is no need to walk the entire VAD tree to clear the PTEs that
    match each page as each physical page can only be mapped at a single virtual address (alias addresses within the VAD are not allowed).
Arguments:
    ProcessHandle - Supplies an open handle to a process object.
    NumberOfPages - Supplies the size in pages of the allocation to delete.
                    Returns the actual number of pages deleted.
    UserPfnArray - Supplies a pointer to memory to retrieve the page frame numbers from.
Return Value:
    Various NTSTATUS codes.
*/
{
    ...
    // Establish an exception handler, probe the specified addresses for read access and capture the page frame numbers.
    CurrentThread = PsGetCurrentThread();
    PreviousMode = KeGetPreviousModeByThread(&CurrentThread->Tcb);
    if (PreviousMode != KernelMode) {
        try {
            ...
            // Initialize the NumberOfPages freed to zero so the user can be reasonably informed about errors that occur midway through the transaction.
            *NumberOfPages = 0;
        } except(ExSystemExceptionFilter())
        {
            // If an exception occurs during the probe or capture of the initial values, then handle the exception and return the exception code as the status value.
...
repeat:
    ...
    // Establish an exception handler, probe the specified addresses for read access and capture the page frame numbers.
    if (PreviousMode != KernelMode) {
        Status = MiCaptureUlongPtrArray((PVOID)MdlPage, UserPfnArray, MdlPages);
        if (!NT_SUCCESS(Status)) {
            goto ErrorReturn;
        }
    } else {
    ...
    // Flush the entire TB for this process while holding its AWE push lock exclusive so that if this free is occurring prior to any pending
    // flushes at the end of an in-progress map/unmap, the app is not left with a stale TB entry that would allow him to corrupt pages that no longer belong to him.
    ExAcquireCacheAwarePushLockExclusive(AweInfo->PushLock);
    ...
    while (MdlPage < LastMdlPage) {
        ...
        // If the frame is currently mapped in a Vad then the PTE must be cleared and the TB entry flushed.
        PointerPte = Pfn1->PteAddress;
        if (PointerPte != NULL) {
            // Note the exclusive hold of the AWE pushlock prevents any other concurrent threads from mapping or unmapping right now.
            // This also eliminates the need to update the PFN PteAddress with an interlocked sequence as well.
            Pfn1->PteAddress = NULL;
    ...
    // Set the ByteCount to the actual number of validated pages - the caller may have lied and we have to sync up here to account for any bogus frames.
    ...
    if (PagesInMdl != 0) {
        ...
        MmFreePagesFromMdl(MemoryDescriptorList);
        if (Process->JobStatus & PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES) {
            PsChangeJobMemoryUsage(PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES, -(SSIZE_T)PagesInMdl);
        }
    } else {
        UNLOCK_ADDRESS_SPACE(Process);
    }
...
ErrorReturn:
    ...
    // Establish an exception handler and carefully write out the number of pages actually processed.
    try {
        *NumberOfPages = PagesProcessed;
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
        // Return success at this point even if the results cannot be written.
        NOTHING;
    }
...
/*
Routine Description:
    This function removes the user-physical-pages mapped region from the current process's address space.  This mapped region is private memory.
    The physical pages of this Vad are unmapped here, but not freed.
    Pagetable pages are freed and their use/commitment counts/quotas are managed by our caller.
Arguments:
...
*/
{
    ...
    // The caller must have removed this Vad from the physical view list, otherwise another thread could immediately remap pages back into this same Vad.
    ...
    while (PointerPte <= EndingPte) {
        ...
        // The frame is currently mapped in this Vad so the PTE must be cleared and the TB entry flushed.
        ...
        // Note the PFN lock is not needed here because we have acquired the pushlock exclusive so no one can be mapping or unmapping right now.
        // In fact, the PFN PteAddress doesn't even have to be updated with an interlocked sequence because the pushlock is held exclusive.
        Pfn1->PteAddress = NULL;
        if (PteFlushList.Count != MM_MAXIMUM_FLUSH_COUNT) {
...
/*
Routine Description:
    This routine frees the VadPhysicalBitMap, any remaining physical pages (as they may not have been currently mapped into any Vads) and returns the bitmap quota.
Arguments:
...
Finish:
    ...
    if (TotalFreedPages != 0) {
        ...
        if (Process->JobStatus & PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES) {
            PsChangeJobMemoryUsage(PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES, -(SSIZE_T)TotalFreedPages);
        }
    }
}
...
/*
Routine Description:
    This function inserts a new AWE or large page view into the specified process' AWE chain.
Arguments:
...
/*
Routine Description:
    This function removes an AWE or large page Vad from the specified process' AWE chain.
Arguments:
...
*/
{
    ...
    if ((AweView->VadType == VadAwe) || (AweView->VadType == VadLargePages)) {
        RtlZeroMemory(&AweInfo->PhysicalViewHint, MAXIMUM_PROCESSORS * sizeof(PMI_PHYSICAL_VIEW));
    }
    ExReleaseCacheAwarePushLockExclusive(AweInfo->PushLock);
...
VOID MiReturnLargePages(IN PMI_LARGEPAGE_MEMORY_RUN LargePageListHead)
{
    ...
    while (LargePageListHead != NULL) {
        ...
        do {
            ...
            // The most likely case is that the pages will be reused with a fully cached attribute.
            // Convert the pages now (and flush the TB) to avoid having to flush the TB as each page gets reallocated.
...
/*
Routine Description:
    This routine allocates contiguous physical memory and ensures it is zero filled on return.  The caller must map it into the relevant virtual address space.
Arguments:
    NumberOfPages - Supplies the number of pages to allocate.
    ProtectionMask - Supplies the protection mask the caller will map the pages with.
Return Value:
...
*/
{
    ...
    do {
        ...
        if (LargePageInfo == NULL) {
            LargePageInfo = ExAllocatePoolWithTag(NonPagedPool, sizeof(MI_LARGEPAGE_MEMORY_RUN), 'lLmM');
            if (LargePageInfo == NULL) {
        ...
        PageFrameIndexLarge = MiFindLargePageMemory(ColoredPageInfoBase, ChunkSize, ProtectionMask, &ZeroSize);
        if (PageFrameIndexLarge != 0) {
            // Save the start and length of each run for subsequent zeroing and PDE filling.
        ...
        case 1:
            ...
            break;
        case 0:
            // Halve the request size.  If needed, then round down to the next page directory multiple.  Then retry.
            ChunkSize >>= 1;
...
/*
Routine Description:
    This routine allocates contiguous physical memory and then initializes page directory and page table pages to map it with large pages.
Arguments:
    StartingAddress - Supplies the starting address of the range.
    EndingAddress - Supplies the ending address of the range.
    ProtectionMask - Supplies the protection mask the caller will map the pages with.
    CallerHasPages - Supplies TRUE if the caller already has pages and will fill in the page directory entries on return.
Environment:
...
*/
{
    ...
    if (CallerHasPages == FALSE) {
        if (AweInfo->VadPhysicalPagesLimit != 0) {
            ...
            if (Process->JobStatus & PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES) {
                if (PsChangeJobMemoryUsage(PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES, NumberOfPages) == FALSE) {
                    return STATUS_COMMITMENT_LIMIT;
                }
                ChargedJob = TRUE;
            }
...
#if (_MI_PAGING_LEVELS >= 3)
    // Charge resident available pages for all of the page directory pages as they will not be paged until the VAD is freed.

    // Note that commitment is not charged here because the VAD insertion charges commit for the entire paging hierarchy (including the nonexistent page tables).
    ...
    if ((SPFN_NUMBER)PagesNeeded > MI_NONPAGEABLE_MEMORY_AVAILABLE() - 20) {
        ...
        if (CallerHasPages == FALSE) {
            ...
            AweInfo->VadPhysicalPages -= NumberOfPages;
            if (ChargedJob == TRUE) {
                PsChangeJobMemoryUsage(PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES, -(SSIZE_T)NumberOfPages);
            }
        }
    ...
    while (PointerPpe <= LastPpe) {
        // Pointing to the next page directory page, make it exist and make it valid.

        // Note this ripples sharecounts through the paging hierarchy so there is no need to up sharecounts to prevent trimming of the
        // page directory parent page as making the page directory valid below does this automatically.
        ...
        // Increment the count of non-zero page directory entries for this page directory - even though this entry is still zero, this is a special case.
    ...
    if (CallerHasPages == FALSE) {
        ...
        while (LargePageListHead != NULL) {
            ...
            // Initialize each page directory page.
            // Acquire the working set pushlock exclusive to prevent races with concurrent MmProbeAndLockPages calls.
...
/*
Routine Description:
    This routine deletes page directory and page table pages for a user-controlled large page range.
Arguments:
    StartingAddress - Supplies the starting address of the range.
    EndingAddress - Supplies the ending address of the range.
    CallerHadPages - Supplies TRUE if the caller already had pages and filled in the page directory entries himself.
Environment:
    Kernel mode, APCs disabled, WorkingSetMutex and AddressCreation mutexes held.
*/
{
    PMMPTE PointerPde;
    PMMPTE LastPde;
    ...
    // Return resident available pages for all of the page directory pages as they can now be paged again.

    // Note that commitment is not returned here because the VAD release returns commit for the entire paging hierarchy (including the nonexistent page tables).
    PagesNeeded = LastPpe - PointerPpe + 1;
    ...
    while (PointerPde <= LastPde) {
        ...
        // Flush the mapping so the pages can be immediately reused without any possibility of conflicting TB entries.
        MI_FLUSH_PROCESS_TB(FALSE);
        if (CallerHadPages == FALSE) {
            MiFreeLargePageMemory(PageFrameIndex, MM_VA_MAPPED_BY_PDE >> PAGE_SHIFT);
        }
    ...
    do {
        ...
        // If all the entries have been eliminated from the previous page directory page, delete the page directory page itself.
        if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
            ...
            MiDeletePte(PointerPpe - 1, TempVa, FALSE, CurrentProcess, NULL, NULL, OldIrql);
            ...
            if ((MiIsPteOnPdeBoundary(PointerPpe)) || (PointerPpe > LastPpe)) {
                if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
                    ...
                    MiDeletePte(PointerPxe, TempVa, FALSE, CurrentProcess, NULL, NULL, OldIrql);
                }
            }
#endif
        }
...
PFN_NUMBER MmSetPhysicalPagesLimit(IN PFN_NUMBER NewPhysicalPagesLimit)
/*
Routine Description:
    ...
    Note the process may already be over the new limit at the time this routine is called.
    If so, no new AWE or large page allocations will succeed until existing allocations are freed such that the process satisfies the new limit.
Arguments:
    NewPhysicalPagesLimit - Supplies the new limit to be enforced or zero if the caller is simply querying for an existing limit.
Return Value:
...
/*
Routine Description:
    This routine captures a user-supplied array of ULONG_PTR elements into a system-supplied buffer.
    In the case of Wow64, the user buffer may be supplied as an array of ULONG elements, in which case an unsigned conversion to ULONG_PTR is performed for each element.
Arguments:
    Destination - Supplies the system buffer to contain the captured array.
    Source - Supplies the user buffer from which to perform the capture. On some platforms, this is an array of ULONG if called from within a Wow64 process.
...
*/
{
    ...
    try {
        ...
        if (PsGetCurrentProcess()->Wow64Process != NULL) {
            ...
            // The compiler is hesitant to perform much optimization within a try block so help it out as much as possible.
            index = 0;
...
#endif
        {
            ProbeForRead(Source, ArraySize * sizeof(ULONG_PTR), sizeof(ULONG_PTR));
            RtlCopyMemory(Destination, Source, ArraySize * sizeof(ULONG_PTR));
        }
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
...
/*
Routine Description:
    This function applies the specified protection on the specified AWE user address range.
Arguments:
    StartingAddress - Supplies the starting user virtual address within a UserPhysicalPages Vad.
    EndingAddress - Supplies the ending user virtual address within a UserPhysicalPages Vad.
...
*/
{
    ...
    Process = PsGetCurrentProcessByThread(CurrentThread);
    PageFrameIndex = 0;
    MI_MAKE_VALID_USER_PTE(NewPteContents, PageFrameIndex, ProtectionMask, PointerPte);
    ...
    // No memory barrier is needed to read the EPROCESS AweInfo field here because the caller has already ensured that this region is a valid AWE region and the address space pushlock is held.
    AweInfo = (PAWEINFO)Process->AweInfo;
    ...
    while (PointerPte <= LastPte) {
        PteContents = *PointerPte;
        if (PteContents.u.Hard.Valid) {
            // The frame is currently mapped in this VAD so the TB entry must be flushed.
            PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte);
            NewPteContents.u.Hard.PageFrameNumber = PageFrameIndex;
