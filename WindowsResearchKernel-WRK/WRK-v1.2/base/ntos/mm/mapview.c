...
NTSTATUS MiSetPageModified(IN PMMVAD Vad, IN PVOID Address);
...
VOID MiInsertPhysicalViewAndRefControlArea(IN PEPROCESS Process, IN PCONTROL_AREA ControlArea, IN PMI_PHYSICAL_VIEW PhysicalView);
...
Arguments:
    ...
    BaseAddress - Supplies a pointer to a variable that will receive the base address of the view.
                  ...
                  If the initial value of this argument is null, then the operating system will determine
                  where to allocate the view using the information specified by the ZeroBits argument value and the section allocation attributes (i.e. based and tiled).
    ZeroBits - Supplies the number of high order address bits that must be zero in the base address of the section view.
               The value of this argument must be less than or
               equal to the maximum number of zero bits and is only used when memory management determines where to allocate the view (i.e. when BaseAddress is null).
               ...
               Bits 63:32 are also required to be zero.
               This retains compatibility with 32-bit systems.

               If ZeroBits is greater than 32, then it is considered as a mask and the number of leading zeroes are counted out in the mask.
               This then becomes the zero bits argument.

    CommitSize - Supplies the size of the initially committed region of the view in bytes.
                 This value is rounded up to the next host page size boundary.

    SectionOffset - Supplies the offset from the beginning of the section to the view in bytes.
                    This value is rounded down to the next host page size boundary.

    ViewSize - Supplies a pointer to a variable that will receive the actual size in bytes of the view.
               If the value of this argument is zero,
               then a view of the section will be mapped starting at the specified section offset and continuing to the end of the section.
               Otherwise the initial value of this argument specifies the size of the view in bytes and is rounded up to the next host page size boundary.

    InheritDisposition - Supplies a value that specifies how the view is to be shared by a child process created with a create process operation.
    ...
    Win32Protect - Supplies the protection desired for the region of initially committed pages.

        Win32Protect Values

         PAGE_NOACCESS - No access to the committed region of pages is allowed.
                         An attempt to read, write, or execute the committed region results in an access violation.
         PAGE_EXECUTE - Execute access to the committed region of pages is allowed.
                        An attempt to read or write the committed region results in an access violation.
         PAGE_READONLY - Read only and execute access to the committed region of pages is allowed.
                         An attempt to write the committed region results in an access violation.
         PAGE_READWRITE - Read, write, and execute access to the region of committed pages is allowed.
...
*/
{
...
#ifdef i386
    // Only allow DOS_LIM support for i386.  The MEM_DOS_LIM flag allows map views of data sections to be done on 4k boundaries rather than 64k boundaries.
    ...
    if (CapturedBase > MM_HIGHEST_VAD_ADDRESS) {
        return STATUS_INVALID_PARAMETER_3;// Invalid base address.
    }
    ...
    // Reference the section object, if a view is mapped to the section object, 
    // the object is not dereferenced as the virtual address descriptor contains a pointer to the section object.
    Status = ObReferenceObjectByHandle(SectionHandle, DesiredSectionAccess, MmSectionObjectType, PreviousMode, (PVOID *)&Section, NULL);
    if (!NT_SUCCESS(Status)) {
        goto ErrorReturn1;
    }

    // Check to see if this the section backs physical memory, if so DON'T align the offset on a 64K boundary, just a 4k boundary.
    if (Section->Segment->ControlArea->u.Flags.PhysicalMemory) {
        ...
        // Make sure alignments are correct for specified address and offset into the file.
    ...
    Status = MmMapViewOfSection(
        ...
        in32Protect);
    if (!NT_SUCCESS(Status)) {
        if ((Status == STATUS_CONFLICTING_ADDRESSES) && (Section->Segment->ControlArea->u.Flags.Image) && (Process == CurrentProcess)) {
            DbgkMapViewOfSection(Section, CapturedBase, CapturedOffset.LowPart, CapturedViewSize);
        }
        goto ErrorReturn;
    }

    // Any time the current process maps an image file, a potential debug event occurs.
    //  DbgkMapViewOfSection handles these events.
...
Routine Description:
    ...
    NOTE:
    CapturedViewSize, SectionOffset, and CapturedBase must be captured in non-paged system space (i.e., kernel stack).
Arguments:
    ...
    BaseAddress - Supplies a pointer to a variable that will receive the base address of the view.
                  ...
                  where to allocate the view using the information specified by the ZeroBits argument value and the section allocation attributes (i.e. based and tiled).
    ZeroBits - Supplies the number of high order address bits that must be zero in the base address of the section view.
               The value of this argument must be less than 21 and is only used when the operating system determines where to allocate the view (i.e. when BaseAddress is NULL).
    CommitSize - Supplies the size of the initially committed region of the view in bytes.
                 This value is rounded up to the next host page size boundary.
    SectionOffset - Supplies the offset from the beginning of the section to the view in bytes.
                    This value is rounded down to the next host page size boundary.
...
*/
{
    ...
    if (Section->u.Flags.Image == 0) {
        ...
        if (!MiIsProtectionCompatible(Section->InitialPageProtection, Win32Protect)) {
            return STATUS_SECTION_PROTECTION;
        }
    }

    // Check to make sure the view size plus the offset is less than the size of the section.
    if ((ULONGLONG)(SectionOffset->QuadPart + *CapturedViewSize) < (ULONGLONG)SectionOffset->QuadPart) {
        return STATUS_INVALID_VIEW_SIZE;
    }

    if (((ULONGLONG)(SectionOffset->QuadPart + *CapturedViewSize) > (ULONGLONG)Section->SizeOfSection.QuadPart) && ((AllocationType & MEM_RESERVE) == 0)) {
        return STATUS_INVALID_VIEW_SIZE;
    }

    if (*CapturedViewSize == 0) {
        // Set the view size to be size of the section less the offset.
        TempViewSize.QuadPart = Section->SizeOfSection.QuadPart - SectionOffset->QuadPart;
        *CapturedViewSize = (SIZE_T)TempViewSize.QuadPart;
        if (
            ...
            return STATUS_INVALID_VIEW_SIZE;// Invalid region size;
        }
    }
    ...
    // Get the address creation mutex to block multiple threads creating or deleting address space at the same time.
    ...
    if (ControlArea->u.Flags.PhysicalMemory) {
        status = MiMapViewOfPhysicalSection(ControlArea, Process, CapturedBase, SectionOffset, CapturedViewSize, ProtectionMask, ZeroBits, AllocationType);
...
/*
Routine Description:
    This is a nonpaged helper routine to insert a physical view and reference the control area accordingly.
Arguments:
...
*/
{
    ...
    // Increment the count of the number of views for the section object.  This requires the PFN lock to be held.
...
LOGICAL MiCheckCacheAttributes(IN PFN_NUMBER PageFrameIndex, IN PFN_NUMBER NumberOfPages, IN MI_PFN_CACHE_ATTRIBUTE CacheAttribute)
{
    KIRQL OldIrql;
    PMMPFN Pfn1;
    ...
    if (AttemptedMapOfUnownedFrame == TRUE) {
        ...
        for (i = 0; i < BACKTRACE_LENGTH; i += 1) {
            ...
            if ((DataTableEntry != NULL) && ((PVOID)DataTableEntry != (PVOID)PsLoadedModuleList.Flink)) {
                BadDriverName = &DataTableEntry->FullDllName;// Found the bad caller.
            }
        }
...
*/
{
    ...
    if ((MI_IS_GUARD(ProtectionMask)) || (ProtectionMask == MM_NOACCESS)) {
        // These cannot be represented with valid PTEs so reject these requests.
        // Note copy on write doesn't make any sense for these either, 
        // but those can be safely rejected if/when the application really does try to write to the mapping.
        return STATUS_INVALID_PAGE_PROTECTION;
    }
...
#endif
        (ZeroBits == 0) && ((*CapturedViewSize & (MM_MINIMUM_VA_FOR_LARGE_PAGE - 1)) == 0)) {
    ...
    if (*CapturedBase == NULL) {
        ...
        if (TryLargePages == TRUE) {
            ...
            // Check whether the registry indicates that all applications should be given virtual address ranges from the highest address downwards in order to test 3GB-aware apps on 32-bit machines and 64-bit apps on NT64.
            if ((AllocationType & MEM_TOP_DOWN) || (Process->VmTopDown == 1)) {
                HighestUserAddress = MM_HIGHEST_VAD_ADDRESS;
                Status = MiFindEmptyAddressRangeDown(&Process->VadRoot, PhysicalViewSize, HighestUserAddress, MM_MINIMUM_VA_FOR_LARGE_PAGE, &StartingAddress);
            } else {
                Status = MiFindEmptyAddressRange(PhysicalViewSize, MM_MINIMUM_VA_FOR_LARGE_PAGE, (ULONG)ZeroBits, &StartingAddress);
            }

            if (NT_SUCCESS(Status)) {
                EndingAddress = (PVOID)((ULONG_PTR)StartingAddress + PhysicalViewSize - 1L);
                goto GotRange;
            }
        }
        ...
        if ((AllocationType & MEM_TOP_DOWN) || (Process->VmTopDown == 1)) {
            ...
            Status = MiFindEmptyAddressRangeDown(&Process->VadRoot, PhysicalViewSize, HighestUserAddress, Alignment, &StartingAddress);
        } else {
...
GotRange:
    ...
    // If a noncachable mapping is requested, none of the physical pages in the range can reside in a large cached page.
    ...
    // Build the PTE.
    MI_MAKE_VALID_USER_PTE(TempPte, StartingPageFrameIndex, ProtectionMask, PointerPte);
    ...
    // See if the mapping is to I/O space.
    // If so, the cache attribute may need to be updated to work around hardware platform problems.
    IoMapping = 1;
    if (MI_IS_PFN(StartingPageFrameIndex)) {
        IoMapping = 0;
    }

    // For non I/O space mappings, we already have the correct attributes in the PTE.
    // But we still need to compute the CacheAttribute variable so we'll know if the cache needs to be swept, etc.
    CacheType = MmCached;
    ...
    // The working set mutex synchronizes the allocation of the EPROCESS PhysicalVadRoot.
    ...
    Failure1:
        ...
        // The pool allocation succeeded but the quota charge failed, deallocate the pool and return an error.
        ExFreePool(Vad);
        return Status;
    }
    ...
    if (TryLargePages == TRUE) {
        ...
        // Temporarily make the VAD protection no access.
        // This allows us to safely release the working set mutex while trying to find contiguous memory to fill the large page range.
        // If another thread tries to access the large page VA range before we find (and insert) a contiguous chunk, the thread will get an AV.
        Vad->u.VadFlags.Protection = MM_NOACCESS;
        UNLOCK_WS_UNSAFE(Thread, Process);

        // Lock down the page directory page(s) as these entries are not going to be paged.
        Status = MiAllocateLargePages(StartingAddress, EndingAddress, ProtectionMask, TRUE);
        if (!NT_SUCCESS(Status)) {
            PMMVAD NextVad;
            PMMVAD PreviousVad;
        ...
        while (PointerPte <= LastPte) {
            ...
            // Increment the count of non-zero page table entries for this page table and the number of private pages for the process.
    ...
    // Increment the count of the number of views for the section object.
    // This requires the PFN lock to be held.
    // At the same time, insert the physical view descriptor now that the page table page hierarchy is in place.
    // Note probes can find this descriptor immediately.
...
/*
Routine Description:
    This is a nonpaged helper routine to mark the specified control area as having its debug symbols loaded.
Arguments:
    ControlArea - Supplies a pointer to the control area.
*/
{
...
*/
{
    ...
    // Check to see if a purge operation is in progress and if so, wait for the purge to complete.
    // In addition, up the count of mapped views for this control area.
    ...
    TryLargePages = FALSE;
    if ((AllocationType & MEM_LARGE_PAGES) && (SectionOffset->QuadPart == 0) &&
#ifdef _X86_
    (KeFeatureBits & KF_LARGE_PAGE) &&
#endif
        (SeSinglePrivilegeCheck(SeLockMemoryPrivilege, KeGetPreviousMode()))) {
        // Since large page mappings essentially separate this instance of the image from any others, 
        // the image section must not have any global subsections in it.
...
Top:
    ...
    if (*CapturedBase != NULL) {
        ...
        // If a large page mapping is requested, see if the virtual address space can accommodate it.

        // Note any large page image mappings are built with the *ENTIRE* image being read-write-execute (backed by the pagefile), 
        // so there is no text protection, etc.

        // A second point worth noting is that even if the address space can accommodate mapping this image large (ie: no adjacent VADs are within the large page boundaries this image would require),
        // the fact that this image's VAD is expanded here to large page boundaries means some subsequent DLL load in this image may fail if it is rebased to an address that falls within this large page allocation and cannot be relocated.
        if (TryLargePages == TRUE) {
            LargeStartingAddress = MI_ALIGN_TO_SIZE(StartingAddress, MM_MINIMUM_VA_FOR_LARGE_PAGE);
            LargeEndingAddress = (PVOID)((ULONG_PTR)EndingAddress | (MM_MINIMUM_VA_FOR_LARGE_PAGE - 1));
            ...
            // If the Vad is NULL then virtual address space exists to map this image with large pages.

            // See if ample pages exist as the image pages will be immediately copied into large pages and thus never paged.
            if ((Vad == NULL) && (TryLargePages == TRUE)) {
                ...
                if (Process->AweInfo != NULL) {
                    Status = MiAllocateLargePages(LargeStartingAddress, LargeEndingAddress, MM_EXECUTE_READWRITE, FALSE);
                    if (NT_SUCCESS(Status)) {
        ...
        // A conflicting VAD was not found and the specified address range is within the user address space.
        // If the image will not reside at its base address, then set a special return status.
        if (((ULONG_PTR)StartingAddress - (ULONG_PTR)MI_64K_ALIGN(SectionOffset->LowPart)) != (ULONG_PTR)BasedAddress) {
            ReturnedStatus = STATUS_IMAGE_NOT_AT_BASE;
        }
    } else {
        ...
        // If the captured view size is greater than the largest size that can fit in the user address space,
        // then it is not possible to map the image.
        ...
        // A second point worth noting is that even if the address space can accommodate mapping this image large 
        // (ie: no adjacent VADs are within the large page boundaries this image would require),
        // the fact that this image's VAD is expanded here to large page boundaries
        // means some subsequent DLL load in this image may fail if it is rebased to an address that falls within this large page allocation and cannot be relocated.
        if (TryLargePages == TRUE) {
            LargeStartingAddress = MI_ALIGN_TO_SIZE(StartingAddress, MM_MINIMUM_VA_FOR_LARGE_PAGE);
            LargeEndingAddress = (PVOID)((ULONG_PTR)EndingAddress | (MM_MINIMUM_VA_FOR_LARGE_PAGE - 1));
            LargeCapturedViewSize = (ULONG_PTR)LargeEndingAddress - (ULONG_PTR)LargeStartingAddress + 1;
        ...
        if (Vad != NULL) {
            // The image could not be mapped at its natural base address try to find another place to map it.

            // If the system has been biased to an alternate base address to allow 3gb of user address space,
            // then make sure the high order address bit is zero.
            ...
            if (Process->VmTopDown == 1) {
                ...
                Status = MiFindEmptyAddressRangeDown(&Process->VadRoot, NeededViewSize, HighestUserAddress, X64K, &StartingAddress);
            } else {
...
AllocateVad:
    ...
    if (Vad == NULL) {
        if (TryLargePages == TRUE) {
            MiReleasePhysicalCharges(BYTES_TO_PAGES((PCHAR)LargeEndingAddress + 1 - (PCHAR)LargeStartingAddress), Process);
    ...
    // NOTE: the full commitment is charged even if a partial map of an image is being done.
    // This saves from having to run through the entire image (via prototype PTEs) and calculate the charge on a per page basis for the partial map.
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        // The quota charge in InsertVad failed, deallocate the pool and return an error.
        ExFreePool(Vad);
        return Status;
    }
    ...
    if (TryLargePages == TRUE) {
        ...
        // Update the image map using the prototype PTEs, initializing the PTEs so on return,
        // no faults will be taken on this view.

        // Note we keep our reference to the image so it stays "busy".
        // Even though this view is not affected by any changes to the image, 
        // this provides the same semantics to callers as small views.
        ProtoPte = Vad->FirstPrototypePte;
        VirtualAddress = StartingAddress;

        // Map the view with small pages to make it easy to copy the data into the large page view.
        // Note that the ordinary user can see the data in the small view (but cannot protect or delete the small view since we hold the address space mutex throughout), this is ok.
        ...
        while (VirtualAddress < EndingAddress) {
            ...
            // Otherwise, it is in the pagefile, transition or in prototype PTE format - if the permissions indicate that it is accessible, copy it.
            if ((ProtoPte->u.Hard.Valid == 1) || (ProtoPte->u.Soft.Protection != MM_NOACCESS)) {
                // These accesses could get in-page errors from the network (or disk).
                try {
                    RtlCopyMemory(VirtualAddress, SmallVirtualAddress, PAGE_SIZE);
                } except(EXCEPTION_EXECUTE_HANDLER)
                {
        ...
        VirtualAddress = StartingAddress;
        while (VirtualAddress < EndingAddress) {
            // If the prototype PTE is valid, transition or in prototype PTE format, bring the page into memory and set the modified bit.
            if ((ProtoPte->u.Hard.Valid == 1) || 
                (((ProtoPte->u.Soft.Prototype == 1) || (ProtoPte->u.Soft.Transition == 1)) && (ProtoPte->u.Soft.Protection != MM_NOACCESS))) {
                Status = MiSetPageModified(Vad, VirtualAddress);
                if (!NT_SUCCESS(Status)) {
                    // An in page error must have occurred touching the image, Ignore the error and continue to the next page - unless it's being run over a network.
                    // If it's being run over a net and the control area is marked as floppy,
                    // then the image must be marked NET_RUN_FROM_SWAP, so any inpage error must be treated as terminal now - so the app doesn't later spontaneously abort when referencing this page.
                    // This provides app writers with a way to mark their app in a way which is robust regardless of network conditions.
    ...
    *CapturedBase = OutputStartingAddress;
    ...
    if (PsImageNotifyEnabled) {
        ...
        if ((StartingAddress < MmHighestUserAddress) && (Process->UniqueProcessId) && (Process != PsInitialSystemProcess)) {
            ...
            PsCallImageNotifyRoutines(&ControlArea->FilePointer->FileName, Process->UniqueProcessId, &ImageInfo);
        }
    }
...
Arguments:
    ...
    LastPteOffset - Supplies the last PTE offset to end the views at.
                    Supplies zero if views are desired from the supplied subsection to the end of the file.
Environment:
    Kernel Mode, address creation mutex optionally held if called in process context.  APC_LEVEL or below.
*/
{
    KIRQL OldIrql;
    NTSTATUS Status;
...
/*
Routine Description:
    This non-pageable routine reference counts the specified subsection if its prototype PTEs are already setup, otherwise it returns FALSE.
Arguments:
...
*/
{
    ...
    // The mapping base exists so the number of mapped views can be safely incremented.
    // This prevents a trim from starting after we release the lock.
    ...
    // Set the access bit so an already ongoing trim won't blindly delete the prototype PTEs on completion of a mapped write.
    // This can happen if the current thread dirties some pages and then deletes the view before the trim write finishes - this bit informs the trimming thread that a rescan is needed so that writes are not lost.
    MappedSubsection->u2.SubsectionFlags2.SubsectionAccessed = 1;
    return TRUE;
}


NTSTATUS MiAddViewsForSection(IN PMSUBSECTION StartMappedSubsection, IN UINT64 LastPteOffset OPTIONAL, IN KIRQL OldIrql, OUT PULONG Waited)
/*
Routine Description:
    ...
    N.B. Callers may pass in view lengths that exceed the length of the section and this must succeed.
         Thus MiAddViews must check for this and know to stop the references at the end.
         More importantly, MiRemoveViews must also contain the same logic.
Arguments:
    StartMappedSubsection - Supplies the mapped subsection to start at.
    LastPteOffset - Supplies the number of PTEs (beginning at the supplied subsection) to provide views for.
                    Supplies zero if views are desired from the supplied subsection to the end of the file.
    Waited - Supplies a pointer to a ULONG to increment if the PFN lock is released and reacquired.
Return Value:
    ...
    do {
        ...
        if (MappedSubsection->SubsectionBase != NULL) {
            ...
            *Waited = 1;
            ...
            ASSERT(Size != 0);
            ProtoPtes = (PMMPTE)ExAllocatePoolWithTag(PagedPool | POOL_MM_ALLOCATION, Size, MMSECT);
            if (ProtoPtes == NULL) {
            ...
            // The section will control access to these and the segment must provide a method to allow other users to map the file for various protections.
            ...
            // Now that the mapping base is guaranteed to be nonzero (shortly), the number of mapped views can be safely incremented.
            // This prevents a trim from starting after we release the lock.
            MappedSubsection->NumberOfMappedViews += 1;
            ...
            // Check to make sure another thread didn't do this already while the lock was released.
            if (MappedSubsection->SubsectionBase == NULL) {
                ...
                // This unlock and release of pool could be postponed until the end of this routine when the lock is released anyway but this should be a rare case anyway so don't bother.
                UNLOCK_PFN(OldIrql);
...
Failed:
    ...
    // A prototype PTE pool allocation failed.  Carefully undo any allocations and references done so far.
    while (StartMappedSubsection != MappedSubsection) {
        ...
        if (StartMappedSubsection->NumberOfMappedViews == 0) {
            ...
            // Since it's not likely there are any resident protos at this point, enqueue each subsection at the front.
...
/*
Routine Description:
    This non-pageable routine removes the views from the specified section if the reference count reaches zero.
    N.B. Callers may pass in view lengths that exceed the length of the section and this must succeed.
         Thus MiAddViews checks for this and knows to stop the references at the end.
         More importantly, MiRemoveViews must also contain the same logic.
Arguments:
    StartMappedSubsection - Supplies the mapped subsection to start at.
    LastPteOffset - Supplies the number of PTEs (beginning at the supplied subsection) to remove.
                    Supplies zero to remove views from the supplied subsection to the end of the file.
Environment:
    Kernel Mode, PFN lock held.
*/
{
    ...
    do {
        ...
        ASSERT(((LONG_PTR)MappedSubsection->NumberOfMappedViews >= 1) || (MappedSubsection->u.SubsectionFlags.SubsectionStatic == 1));
        MappedSubsection->NumberOfMappedViews -= 1;
...
VOID MiConvertStaticSubsections(IN PCONTROL_AREA ControlArea)
{
    ...
    do {
        ...
        if (MappedSubsection->DereferenceList.Flink != NULL) {
            ...
            // This subsection was not put on the dereference subsection list because it was created as part of an extension or nowrite.
            // Since this is the last segment dereference, convert the subsection and its prototype PTEs to dynamic now (iff nowrite is clear).
            ...
            // This subsection has already had its prototype PTEs reclaimed (or never allocated), hence it is not on any reclaim lists.
            NOTHING;
        } else {
            // This subsection is being processed by the dereference segment thread right now ! 
            // The dereference thread sets the mapped view count to 1 when it starts processing the subsection.
...
VOID MiMapLargePageSection(IN PEPROCESS Process, IN PVOID StartingAddress, IN PVOID EndingAddress, IN PMMVAD Vad, IN MM_PROTECTION_MASK ProtectionMask)
/*
Routine Description:
    This routine maps the specified section into the specified process's address space using large page mappings.
Arguments:
...
*/
{
    ...
    ProtoPte = Vad->FirstPrototypePte;
    MI_MAKE_VALID_PTE(TempPde, 0, ProtectionMask, MiGetPteAddress(StartingAddress));
...
/*
Routine Description:
    This routine maps the specified mapped file or pagefile-backed section into the specified process's address space.
Arguments:
    see MmMapViewOfSection above...
    ControlArea - Supplies the control area for the section.
    Process - Supplies the process pointer which is receiving the section.
...
*/
{
    ...
    if ((ControlArea->FilePointer == NULL) && (CommitSize != 0) && (Segment->NumberOfCommittedPages < TotalNumberOfPtes)) {
        PointerPte = TheFirstPrototypePte;
        LastPte = PointerPte + BYTES_TO_PAGES(CommitSize);

        // Charge quota for the entire requested range.  If the charge succeeds, excess is returned when the PTEs are actually filled in.
    ...
    // If the requested size and start address are large page multiples and this is a large page segment, then map it with large pages.
    // Otherwise, fall back to small pages.
    TryLargePages = FALSE;
    if ((Segment->SegmentFlags.LargePages == 1) && ((*CapturedViewSize % MM_MINIMUM_VA_FOR_LARGE_PAGE) == 0) && ((AllocationType & MEM_RESERVE) == 0) &&
#ifdef _X86_
    (KeFeatureBits & KF_LARGE_PAGE) &&
#endif
        (!MI_IS_GUARD(ProtectionMask)) && (ProtectionMask != MM_NOACCESS) && (MI_IS_PTE_PROTECTION_COPY_WRITE(ProtectionMask) == 0)) {
        TryLargePages = TRUE;
    }

    if ((*CapturedBase == NULL) && (CapturedStartingVa == NULL)) {
        // The section is not based, find an empty range starting on a 64k boundary.
        if ((AllocationType & MEM_TOP_DOWN) || (Process->VmTopDown == 1)) {
            if (ZeroBits != 0) {
                HighestUserAddress = (PVOID)((ULONG_PTR)MM_USER_ADDRESS_RANGE_LIMIT >> ZeroBits);
                if (HighestUserAddress > MM_HIGHEST_VAD_ADDRESS) {
            ...
            if (TryLargePages == TRUE) {
                Status = MiFindEmptyAddressRangeDown(&Process->VadRoot, *CapturedViewSize, HighestUserAddress, MM_MINIMUM_VA_FOR_LARGE_PAGE, &StartingAddress);
                if (NT_SUCCESS(Status)) {
                    goto FoundRange;
                }
                TryLargePages = FALSE;
            ...
            Status = MiFindEmptyAddressRangeDown(&Process->VadRoot, *CapturedViewSize, HighestUserAddress, Alignment, &StartingAddress);
        } else {
            if (TryLargePages == TRUE) {
                Status = MiFindEmptyAddressRange(*CapturedViewSize, MM_MINIMUM_VA_FOR_LARGE_PAGE, (ULONG)ZeroBits, &StartingAddress);
                if (NT_SUCCESS(Status)) {
                    goto FoundRange;
                }
                TryLargePages = FALSE;
    ...
    // Note that MEM_DOS_LIM significance is lost here, but those files are not mapped MEM_RESERVE.
    ...
    if (AllocationType & MEM_RESERVE) {
        ...
        if (ExtendInfo) {
            ...
            if (ExtendInfo == NULL) {
                ...
                // The pool allocation succeeded, but the quota charge in InsertVad failed, deallocate the pool and return an error.
                ExFreePool(Vad);
                return STATUS_INSUFFICIENT_RESOURCES;
            }
            ExtendInfo->ReferenceCount = 1;
    ...
    // If the page protection is write-copy or execute-write-copy charge for each page in the view as it may become private.
...
#if defined (_WIN64)
    // See if ample pages for the page table pages as the target pages will be mapped physically with large pages and thus never be paged.
    if (TryLargePages == TRUE) {
        TryLargePages = MiCreatePageDirectoriesForPhysicalRange(Process, StartingAddress, EndingAddress);
    ...
    if (!NT_SUCCESS(Status)) {
        if (ControlArea->FilePointer != NULL) {
            ...
            LOCK_WS_UNSAFE(Thread, Process);
            MiDeletePageDirectoriesForPhysicalRange(StartingAddress, EndingAddress);
            UNLOCK_WS_UNSAFE(Thread, Process);
        }
#endif
    ...
    if (TryLargePages == TRUE) {
        MiMapLargePageSection(Process, StartingAddress, EndingAddress, Vad, ProtectionMask);
    }
    ...
    *CapturedBase = StartingAddress;
    ...
    // Update the count of writable user mappings so the transaction semantics can be supported.
    // Note that no lock synchronization is needed here as the transaction manager must already check for any open writable handles
    // to the file - and no writable sections can be created without a writable file handle.
    // So all that needs to be provided is a way for the transaction manager to know that there are lingering views or created sections still open that have write access.
    if (((ProtectionMask == MM_READWRITE) || (ProtectionMask == MM_EXECUTE_READWRITE)) && (ControlArea->FilePointer != NULL)) {
...
/*
Routine Description:
    This routine synchronizes with any on going purge operations on the same segment (identified via the control area).
    If another purge operation is occurring, the function blocks until it is completed.

    When this function returns the MappedView and the NumberOfUserReferences count for the control area will be incremented thereby referencing the control area.
Arguments:
    ControlArea - Supplies the control area for the segment to be purged.
    FailIfSystemViews - Supplies TRUE if the request should fail when there are active kernel mappings.
                        When the image is mapped in system or session space as a driver, copy on write does not work so user processes are not allowed to map the image.
Return Value:
...
*/
{
    ...
    while (ControlArea->u.Flags.BeingPurged != 0) {
        ...
        if (ControlArea->WaitingForDeletion == NULL) {
            ...
            // No interlock is needed for the RefCount increment as no thread can be decrementing it since it is still pointed to by the control area.
        ...
        // Before this event can be set, the control area WaitingForDeletion field must be cleared (and may be reinitialized to something else), 
        // but cannot be reset to our local event.
        // This allows us to dereference the event count lock free.
    ...
    if ((FailIfSystemViews == TRUE) && (ControlArea->u.Flags.ImageMappedInSystemSpace) && (KeGetPreviousMode() != KernelMode)) {
        UNLOCK_PFN(OldIrql);
        Status = STATUS_CONFLICTING_ADDRESSES;
    } else {
...
PVOID MiCacheImageSymbols(IN PVOID ImageBase)
{
    ...
    try {
        DebugDirectory = RtlImageDirectoryEntryToData(ImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_DEBUG, &DebugSize);
...
NTSYSAPI NTSTATUS NTAPI NtAreMappedFilesTheSame(__in PVOID File1MappedAsAnImage, __in PVOID File2MappedAsFile)
/*
Routine Description:
...
*/
{
    ...
    if ((ControlArea1->FilePointer == NULL) || (ControlArea2->FilePointer == NULL)) {
        Status = STATUS_CONFLICTING_ADDRESSES;
        goto ErrorReturn;
    }

    // Be sure the correct image section object is retrieved for the file mapped as either data or an image, particularly in the case of multiple sessions.
    // The actual list need only be searched if the image section is flagged as global only per session, allowing reduction of PFN lock hold times in the common case.
    if ((PVOID)ControlArea1 == ControlArea2->FilePointer->SectionObjectPointer->ImageSectionObject) {
        Status = STATUS_SUCCESS;
...
*/
{
    ...
    // Charge commitment up front even though we may not use it because failing to get it later would make things messy.
    RealCharge = 1;
    ...
    if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
        ...
        // We didn't need to (and shouldn't have) charged commitment for this page as it was already pagefile backed (ie: someone else already charged commitment for it earlier).
    ...
    if (ReturnCommitment == TRUE) {
        ...
        // Commit has been charged for the copied page, add the charge to the VAD now so it is automatically returned when the VAD is deleted later.
...
NTSTATUS MmMapViewInSystemSpace(__in PVOID Section, __deref_inout_bcount(*ViewSize) PVOID *MappedBase, __inout PSIZE_T ViewSize)
/*
Routine Description:
...
Arguments:
    ...
    *MappedBase - Returns the address where the section was mapped.
    ViewSize - Supplies the size of the view to map.  If this is specified as zero, the whole section is mapped. Returns the actual size mapped.
Return Value:
    Status of the map view operation.
Environment:
...
NTSTATUS MmMapViewInSessionSpace(__in PVOID Section, __deref_inout_bcount(*ViewSize) PVOID *MappedBase, __inout PSIZE_T ViewSize)
/*
Routine Description:
...
*/
{
    ...
    // 4GB-64K is the maximum individual view size allowed since we encode this into the bottom 16 bits of the MMVIEW entry.
    if (SizeIn64k >= X64K) {
        MiDereferenceControlArea(ControlArea);
        return STATUS_INVALID_VIEW_SIZE;
    }
    ...
    if (Session == &MmSession) {
        ...
        Status = MiSessionCommitPageTables(Base, (PVOID)((ULONG_PTR)Base + NumberOfBytes));
    }
...
NTSTATUS MmGetSessionMappedViewInformation(OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT PULONG Length, IN PULONG SessionId OPTIONAL)
/*
Routine Description:
    This function returns information about all the per-session mapped views in the system.
Arguments:
    ...
    Length - Receives the number of bytes placed (or would have been placed) in the system information buffer.
...
*/
{
    ...
    *Length = 0;
    ...
    if (SystemInformationLength > 0) {
        ...
        // This indicates the caller just wants to know the size of the buffer to allocate but is not prepared to accept any data content in this instance.
        MappedAddress = NULL;
        LockVariable = NULL;
    }

    for (OpaqueSession = MmGetNextSession(NULL); OpaqueSession != NULL; OpaqueSession = MmGetNextSession(OpaqueSession)) {
        SessionMappedViewInfo = (PSYSTEM_SESSION_MAPPED_VIEW_INFORMATION)((PUCHAR)MappedAddress + TotalSize);
        ...
        if ((*SessionId == 0xFFFFFFFF) || (CurrentSessionId == *SessionId)) {
            ...
            if (NT_SUCCESS(MmAttachSession(OpaqueSession, &ApcState))) {
                ...
                if (TotalSize > SystemInformationLength) {
                    ...
                    LOCK_SYSTEM_VIEW_SPACE(Session);
                    ContiguousFree = RtlFindLongestRunClear(Session->SystemSpaceBitMap, &StartingIndex);
                    do {
                        Index = RtlFindClearBits(Session->SystemSpaceBitMap, 1, Hint);
                    ...
                    SessionMappedViewInfo->NumberOfBytesAvailable = (SIZE_T)TotalFree * X64K;
                    SessionMappedViewInfo->NumberOfBytesAvailableContiguous = (SIZE_T)ContiguousFree * X64K;
                    SessionMappedViewInfo->SessionId = CurrentSessionId;
                    SessionMappedViewInfo->ViewFailures = Session->BitmapFailures;
                    SessionMappedViewInfo->NextEntryOffset = TotalSize;// Point to next session.
                }

                MmDetachSession(OpaqueSession, &ApcState);// Detach from session.
            }
...
/*
Routine Description:
    This routine allocates page tables and fills the system page directory entries for the specified virtual address range.
Arguments:
...
*/
{
    ...
    do {
        ...
        if (PointerPxe->u.Hard.Valid == 0) {
            ...
            LOCK_PFN(OldIrql);
            if (PointerPxe->u.Hard.Valid == 0) {
                if ((MmAvailablePages < MM_HIGH_LIMIT) && (MiEnsureAvailablePageOrWait(NULL, OldIrql))) {
                ...
                PageFrameIndex = MiRemoveZeroPage(MI_GET_PAGE_COLOR_FROM_PTE(PointerPxe));
        ...
        if (FirstSystemPde->u.Hard.Valid == 0) {
            ...
            if (FirstSystemPde->u.Hard.Valid == 0) {
                ...
                MiChargeCommitmentCantExpand(1, TRUE);
                MM_TRACK_COMMIT(MM_DBG_COMMIT_FILL_SYSTEM_DIRECTORY, 1);
                PageFrameIndex = MiRemoveZeroPage(MI_GET_PAGE_COLOR_FROM_PTE(FirstSystemPde));
                TempPte.u.Hard.PageFrameNumber = PageFrameIndex;
                ...
                // The FirstPde and FirstSystemPde may be identical even on 32-bit machines if we are currently running in the system process, 
                // so check for the valid bit first so we don't assert on a checked build.
...
NTSTATUS MmUnmapViewInSystemSpace(__in PVOID MappedBase)
/*
Routine Description:
...
NTSTATUS MmUnmapViewInSessionSpace(__in PVOID MappedBase)
/*
Routine Description:
    This routine unmaps the specified section from the system's address space.
Arguments:
...
*/
{
    ...
    if (Session->SystemSpaceHashEntries + 8 > Session->SystemSpaceHashSize) {
        ...
        // The SystemSpaceViewTable for system (not session) space is only allocated from nonpaged pool so it can be safely torn down during clean shutdowns.
        // Otherwise it could be allocated from paged pool just like the session SystemSpaceViewTable.
        if (Session == &MmSession) {
    ...
    if (Session->SystemSpaceHashEntries == Session->SystemSpaceHashSize) {
        // There are no free hash slots to place a new entry into even though there may still be unused virtual address space.
        UNLOCK_SYSTEM_VIEW_SPACE(Session);
        return NULL;
    }
...
/*
Routine Description:
    This routine looks up the specified view in the system space hash table and unmaps the view from system space and the table.
Arguments:
    Session - Supplies the session data structure for this view.
...
LOGICAL MiInitializeSystemSpaceMap(PVOID InputSession OPTIONAL)
/*
Routine Description:
...
Arguments:
    InputSession - Supplies NULL if this is the initial system session (non-Hydra),
                   a valid session pointer (the pointer must be in global space, not session space) for Hydra session initialization.
Return Value:
...
*/
{
    ...
    // Save a global pointer to the mutex below because multiple sessions will generally give us a session-space (not a global space) pointer to the MMSESSION in subsequent calls.
    // We need the global pointer for the mutex field for the kernel primitives to work properly.
    ...
    // If the kernel image has not been biased to allow for 3gb of user space, then the system space view starts at the defined place.
    // Otherwise, it starts 16mb above the kernel image.
...
HANDLE MmSecureVirtualMemory(__in_bcount(Size) PVOID Address, __in SIZE_T Size, __in ULONG ProbeMode)
/*
Routine Description:
    This routine probes the requested address range and protects the specified address range from having its protection made more restricted and being deleted.
...
Return Value:
    ...
    If the range could not be locked because of protection problems or noncommitted memory, the value (HANDLE)0 is returned.
Environment:
    Kernel Mode.
*/
{
...
Arguments:
    ...
    Size - Supplies the size of the range to secure.
    ProbeMode - Supplies one of PAGE_READONLY or PAGE_READWRITE.
    AddressSpaceMutexHeld - Supplies TRUE if the mutex is already held, FALSE if not.
Return Value:
    Returns a handle to be used to unsecure the range.
    If the range could not be locked because of protection problems or noncommitted memory, the value (HANDLE)0 is returned.
Environment:
...
#define PageSize PAGE_SIZE
    ...
    // Check for a private committed VAD first instead of probing to avoid all the page faults and zeroing.
    // If we find one, then we run the PTEs instead.
    if (Size >= 64 * 1024) {
        ...
        if ((Vad->u.VadFlags.VadType == VadAwe) || (Vad->u.VadFlags.VadType == VadLargePages)) {
            goto Return1;
        }
        ...
        do {
            while (MiDoesPxeExistAndMakeValid(PointerPxe, Process, MM_NOIRQL, &Waited) == FALSE) {
                // Extended page directory parent entry is empty, go to the next one.
                PointerPxe += 1;
            ...
            while (MiDoesPpeExistAndMakeValid(PointerPpe, Process, MM_NOIRQL, &Waited) == FALSE) {
            ...
            while (MiDoesPdeExistAndMakeValid(PointerPde, Process, MM_NOIRQL, &Waited) == FALSE) {
    ...
    LongWay:
        ...
        // sneak its stack in as the argument region and trick us into trying to grow it if the reference faults 
        // (as this would cause a deadlock since this thread already owns the address space mutex).
        // Note this would have the side effect of not allowing this thread
        // to fault on guard pages in other data regions while the accesses below are ongoing - but that could only happen in an APC and those are blocked right now anyway.
...
EditVad:
    // Round the start and end addresses to the appropriate page boundary (factoring in NT64 wow64 process page size as well).
    // This is so they can be properly compared during attempted protection changes.
    EndAddress |= (PageSize - 1);
    StartAddress = MI_ALIGN_TO_SIZE(StartAddress, PageSize);

    // If this is a short or regular VAD, it needs to be reallocated as a large VAD.
    // Note that a short VAD that was previously converted to a long VAD here will still be marked as private memory, 
    // thus to handle this case the NoChange bit must also be tested.
    if (((Vad->u.VadFlags.PrivateMemory) && (Vad->u.VadFlags.NoChange == 0)) || (Vad->u2.VadFlags2.LongVad == 0)) {
        ...
        if ((Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) || (Vad->u.VadFlags.VadType == VadRotatePhysical) || (Vad->u.VadFlags.VadType == VadWriteWatch)) {
    ...
    if (Vad->u2.VadFlags2.MultipleSecured) {
        ...
        // The VAD may be either a regular VAD or a long VAD (it cannot be a short VAD) at this point.
        // If it is a regular VAD, it must be reallocated as a long VAD before any operation can proceed so the secured range can be inserted.
        Vad->u.VadFlags.NoChange = 1;
...
VOID MmUnsecureVirtualMemory(__in HANDLE SecureHandle)
/*
Routine Description:
    This routine unsecures memory previously secured via a call to MmSecureVirtualMemory.
Arguments:
...
*/
{
    ...
    // Make sure the address space was not deleted (ie, the thread that secured the virtual memory was attached).
    // If the process has exited, then our VAD (or MMSECURE) pointer has already been freed so just return.
    if (Process->Flags & PS_PROCESS_FLAGS_VM_DELETED) {
        if (AddressSpaceMutexHeld == FALSE) {
            UNLOCK_ADDRESS_SPACE(Process);
        }
    ...
    if (Vad->u2.VadFlags2.OneSecured) {
        ...
        if (IsListEmpty(&Vad->u3.List)) {
            ...
            if ((Vad->u2.VadFlags2.SecNoChange == 0) && (Vad->u.VadFlags.PrivateMemory == 0)) {
                ...
                // If this VAD is private, removing the state NoChange flag indicates that this is a short VAD which it no longer is.
...
VOID MiDumpConflictingVad(IN PVOID StartingAddress, IN PVOID EndingAddress, IN PMMVAD Vad)
{
    if (NtGlobalFlag & FLG_SHOW_LDR_SNAPS) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MM: [%p ... %p) conflicted with Vad %p\n", StartingAddress, EndingAddress, Vad);
        if ((Vad->u.VadFlags.PrivateMemory == 1) || (Vad->ControlArea == NULL)) {
            return;
        }

        if (Vad->ControlArea->u.Flags.Image)
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "    conflict with %Z image at [%p .. %p)\n",
            ...
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "    conflict with %Z file at [%p .. %p)\n",
                       &Vad->ControlArea->FilePointer->FileName, MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn));
        else
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "    conflict with section at [%p .. %p)\n",
