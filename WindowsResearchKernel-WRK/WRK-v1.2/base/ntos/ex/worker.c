...
// They run at csrss and below csrss to avoid pre-empting the user interface under heavy load.
...
// Flag to pass in to the worker thread, indicating whether it is dynamic or not.
...
// A reference to the balance manager thread, so that shutdown can wait for it to terminate.
PETHREAD ExpWorkerThreadBalanceManagerPtr;

// A pointer to the last worker thread to exit (so the balance manager can wait for it before exiting).
PETHREAD ExpLastWorkerThread;

// These are used to keep track of the set of workers, and whether or not we're allowing them to be paged.
// Note that we can't use this list for shutdown (sadly), as we can't just terminate the threads, we need to flush their queues.
FAST_MUTEX ExpWorkerSwapinMutex;
LIST_ENTRY ExpWorkerListHead;
BOOLEAN    ExpWorkersCanSwap;

// Worker queue item that can be filled in by the kernel debugger to get code to run on the system.
...
LOGICAL ExpCheckQueueShutdown(IN WORK_QUEUE_TYPE QueueType, IN PSHUTDOWN_WORK_ITEM ShutdownItem);
...
LOGICAL __forceinline ExpNewThreadNecessary(IN PEX_WORK_QUEUE Queue)
/*
Routine Description:
    This function checks the supplied worker queue and determines whether it is appropriate to spin up a dynamic worker thread for that queue.
Arguments:
    Queue - Supplies the queue that should be examined.
Return Value:
    TRUE if the given work queue would benefit from the creation of an additional thread, FALSE if not.
*/
{
    if ((Queue->Info.MakeThreadsAsNecessary == 1) && (IsListEmpty(&Queue->WorkerQueue.EntryListHead) == FALSE) &&
        (Queue->WorkerQueue.CurrentCount < Queue->WorkerQueue.MaximumCount) && (Queue->DynamicThreadCount < MAX_ADDITIONAL_DYNAMIC_THREADS)) {

        // We know these things:
        // - This queue is eligible for dynamic creation of threads to try to keep the CPUs busy,
        // - There are work items waiting in the queue,
        // - The number of runable worker threads for this queue is less than the number of processors on this system, and
        // - We haven't reached the maximum dynamic thread count.

        // An additional worker thread at this point will help clear the backlog.
        return TRUE;
    }

    return FALSE;// One of the above conditions is false.
}


NTSTATUS ExpWorkerInitialization(VOID)
{
    ULONG Index;
    ...
    // We did this in Windows XP, because 3COM NICs would take a long time with the network stack tying up the delayed worker threads.
    // When Mm would need a worker thread to load a driver on the critical path of boot, it would also get stuck for a few seconds and hurt boot times.
    // Ideally we'd spawn new delayed threads as necessary as well to prevent such contention from hurting boot and resume.
    ...
    // Always make stack for this thread resident so that worker pool deadlock magic can run even when what we are trying to do is inpage the hyper critical worker thread's stack.
    // Without this fix, we hold the process lock but this thread's stack can't come in, and the deadlock detection cannot create new threads to break the system deadlock.
    ExWorkerQueue[HyperCriticalWorkQueue].Info.WaitMode = KernelMode;
    if (NtAs) {
        ExWorkerQueue[CriticalWorkQueue].Info.WaitMode = KernelMode;
    }

    // We only create dynamic threads for the critical work queue (note this doesn't apply to dynamic threads created to break deadlocks.)

    // The rationale is this: folks who use the delayed work queue are not time critical, and the hypercritical queue is used rarely by folks who are non-blocking.
    ...
    // Create the desired number of executive worker threads for each of the work queues.
    ...
    for (Index = 0; Index < NumberOfThreads; Index += 1) {
        Status = ExpCreateWorkerThread(CriticalWorkQueue, FALSE);// Create a worker thread to service the critical work queue.
    ...
    for (Index = 0; Index < NumberOfThreads; Index += 1) {
        Status = ExpCreateWorkerThread(DelayedWorkQueue, FALSE);// Create a worker thread to service the delayed work queue.
        if (!NT_SUCCESS(Status)) {
            break;
        }
    }
    ...
    Status = ExpCreateWorkerThread(HyperCriticalWorkQueue, FALSE);// Create the hypercritical worker thread.

    // Create the worker thread set manager thread.
    InitializeObjectAttributes(&ObjectAttributes, NULL, 0, NULL, NULL);
    Status = PsCreateSystemThread(&Thread, THREAD_ALL_ACCESS, &ObjectAttributes, 0, NULL, ExpWorkerThreadBalanceManager, NULL);
    if (NT_SUCCESS(Status)) {
        Status = ObReferenceObjectByHandle(Thread, SYNCHRONIZE, NULL, KernelMode, &ExpWorkerThreadBalanceManagerPtr, NULL);
        ZwClose(Thread);
    }
...
VOID ExQueueWorkItem(__inout PWORK_QUEUE_ITEM WorkItem, __in WORK_QUEUE_TYPE QueueType)
/*
Routine Description:
    This function inserts a work item into a work queue that is processed by a worker thread of the corresponding type.
Arguments:
    WorkItem - Supplies a pointer to the work item to add the the queue.
        This structure must be located in NonPagedPool.
        The work item structure contains a doubly linked list entry, the address of a routine to call and a parameter to pass to that routine.
    QueueType - Specifies the type of work queue that the work item should be placed in.
*/
{
    ...
    if ((ULONG64)WorkItem->WorkerRoutine <= MmUserProbeAddress) {
        KeBugCheckEx(WORKER_INVALID, 0x1, (ULONG_PTR)WorkItem, (ULONG_PTR)WorkItem->WorkerRoutine, 0);
    }

    Queue = &ExWorkerQueue[QueueType];
    KeInsertQueue(&Queue->WorkerQueue, &WorkItem->List);// Insert the work item in the appropriate queue object.

    // We check the queue's shutdown state after we insert the work item to avoid the race condition when the queue's marked between checking the queue and inserting the item.
    // It's possible for the queue to be marked for shutdown between the insert and this assert (so the insert would've barely sneaked in), 
    // but it's not worth guarding against this -- barely
    // sneaking in is not a good design strategy, and at this point in the shutdown sequence, the caller simply should not be trying to insert new queue items.
    ASSERT(!Queue->Info.QueueDisabled);

    // Determine whether another thread should be created, and signal the thread set balance manager if so.
...
Routine Description:
    ...
    The worker thread manager thread is created during system initialization and begins execution in this function.

    This thread is responsible for detecting and breaking circular deadlocks in the system worker thread queues.
    It will also create and destroy additional worker threads as needed based on loading.
Arguments:
...
*/
{
    ...
    // Raise the thread priority to just higher than the priority of the critical work queue.
    ...
    while (TRUE) {
        KeSetTimer(&PeriodTimer, DueTime, NULL);// Set the timer to expire at the next periodic interval.

        // Wake up when the timer expires or the set manager event is signaled.
        Status = KeWaitForMultipleObjects(MaximumBalanceObject, WaitObjects, WaitAny, Executive, KernelMode, FALSE, NULL, NULL);
        switch (Status) {
        case TimerExpiration:
            // Periodic timer expiration - go see if any work queues are deadlocked.
            ExpDetectWorkerThreadDeadlock();
            break;
        case ThreadSetManagerEvent:
            // Someone has asked us to check some metrics to determine whether we should create another worker thread.
        ...
        // This checks if special debugging routines need to be run on the behalf of the debugger.
...
/*
Routine Description:
    This routine is called when there is reason to believe that a work queue might benefit from the creation of an additional worker thread.
    This routine checks each queue to determine whether it would benefit from an additional worker thread (see ExpNewThreadNecessary()),
    and creates one if so.
*/
{
    PEX_WORK_QUEUE Queue;
    WORK_QUEUE_TYPE QueueType;
    ...
    Queue = &ExWorkerQueue[0];// Check each worker queue.
    for (QueueType = 0; QueueType < MaximumWorkQueue; Queue += 1, QueueType += 1) {
        if (ExpNewThreadNecessary(Queue)) {
            // Create a new thread for this queue.  
            // We explicitly ignore an error from ExpCreateDynamicThread(): there's nothing we can or should do in the event of a failure.
...
/*
Routine Description:
    This function creates new work item threads if a possible deadlock is detected.
*/
{
    ULONG Index;
    PEX_WORK_QUEUE Queue;
    ...
    for (Index = 0; Index < MaximumWorkQueue; Index += 1) {
        ...
        if ((Queue->QueueDepthLastPass > 0) && (Queue->WorkItemsProcessed == Queue->WorkItemsProcessedLastPass) && (Queue->DynamicThreadCount < MAX_ADDITIONAL_DYNAMIC_THREADS)) {
            ...
            // Things look like they're stuck, create a new thread for this queue.

            // We explicitly ignore an error from ExpCreateDynamicThread():
            // we'll try again in another detection period if the queue looks like it's still stuck.
            ExpCreateWorkerThread(Index, TRUE);
        }

        // Update some bookkeeping.

        // Note that WorkItemsProcessed and the queue depth must be recorded in that order to avoid getting a false deadlock indication.
...
NTSTATUS ExpCreateWorkerThread(IN WORK_QUEUE_TYPE QueueType, IN BOOLEAN Dynamic)
/*
Routine Description:
    This function creates a single new static or dynamic worker thread for the given queue type.
Arguments:
    QueueType - Supplies the type of the queue for which the worker thread should be created.
    Dynamic - If TRUE, the worker thread is created as a dynamic thread that will terminate after a sufficient period of inactivity.
              If FALSE, the worker thread will never terminate.
Return Value:
    The final status of the operation.
Notes:
    This routine is only called from the worker thread set balance thread, therefore it will not be reentered.
*/
{
    OBJECT_ATTRIBUTES ObjectAttributes;
    NTSTATUS Status;
    ...
    Status = PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, &ObjectAttributes, 0L, NULL, ExpWorkerThread, (PVOID)(ULONG_PTR)Context);
    if (!NT_SUCCESS(Status)) {
        return Status;
    }
...
VOID ExpWorkerThread(IN PVOID StartContext)
{
    ...
    if (((ULONG_PTR)StartContext & DYNAMIC_WORKER_THREAD) == 0) {
        // We are being created as a static thread.  As such it will not terminate, so there is no point in timing out waiting for a work item.
        Timeout = NULL;
    } else {
        // This is a dynamic worker thread.  It has a non-infinite timeout so that it can eventually terminate.
    ...
    // If the thread is a critical worker thread, then set the thread priority to the lowest realtime level. 
    // Otherwise, set the base thread priority to time critical.
    ...
    // In diskless NT scenarios ensure that the kernel stack of the worker threads will not be swapped out.
    if (IoRemoteBootClient) {
        KeSetKernelStackSwapEnable(FALSE);
    }
#endif // defined(REMOTE_BOOT)

    // Register as a worker, exiting if the queue's going down and there aren't any workers in the queue to hand us the shutdown
    // work item if we enter the queue (we want to be able to enter a queue even if the queue's shutting down, 
    // in case there's a backlog of work items that the balance manager thread's decided we should be helping to process).
    ...
    } while (OldWorkerInfo.QueueWorkerInfo != InterlockedCompareExchange(&WorkerQueue->Info.QueueWorkerInfo, NewWorkerInfo.QueueWorkerInfo, OldWorkerInfo.QueueWorkerInfo));

    // As of this point, we must only exit if we decrement the worker count without the queue disabled flag being set.  
    // (Unless we exit due to the shutdown work item, which also decrements the worker count).
    Thread->ActiveExWorker = 1;

    // Loop forever waiting for a work queue item, calling the processing routine, and then waiting for another work queue item.
    do {
        // Wait until something is put in the queue or until we time out.

        // By specifying a wait mode of UserMode, the thread's kernel stack is swappable.
        ...
        if ((ULONG_PTR)Entry != STATUS_TIMEOUT) {
            ...
            ASSERT((ULONG_PTR)WorkerRoutine > MmUserProbeAddress);// Catch worker routines referencing a user mode address.
            ((PWORKER_THREAD_ROUTINE)WorkerRoutine) (Parameter);// Execute the specified routine.

#if DBG
            if (IsListEmpty(&Thread->IrpList)) {
                ExCheckIfResourceOwned();// See if a worker just returned while holding a resource
            }
#endif

            // Catch worker routines that forget to leave a critial/guarded region.
            // In the debug case execute a breakpoint.
            // In the free case zero the flag so that  APCs can continue to fire to this thread.
            ...
            if (KeGetCurrentIrql() != PASSIVE_LEVEL) {
                KeBugCheckEx(WORKER_THREAD_RETURNED_AT_BAD_IRQL, (ULONG_PTR)WorkerRoutine, (ULONG_PTR)KeGetCurrentIrql(), (ULONG_PTR)Parameter, (ULONG_PTR)WorkItem);
            }

            if (PS_IS_THREAD_IMPERSONATING(Thread)) {
                KeBugCheckEx(IMPERSONATING_WORKER_THREAD, (ULONG_PTR)WorkerRoutine, (ULONG_PTR)Parameter, (ULONG_PTR)WorkItem, 0);
            }
        ...
        // These things are known:
        // - Static worker threads do not time out, so this is a dynamic worker thread.
        // - This thread has been waiting for a long time with nothing to do.
        if (IsListEmpty(&Thread->IrpList) == FALSE) {
            continue;// There is still I/O pending, can't terminate yet.
        }
        ...
        if (OldWorkerInfo.QueueDisabled) {
            continue;// We're exiting via the queue disable work item
        }
...
/*
Routine Description:
    Sets the kernel stacks of the delayed worker threads to be swappable or pins them into memory.
Arguments:
    AllowSwap - Supplies TRUE if worker kernel stacks should be swappable, FALSE if not.
*/
{
    PETHREAD         Thread;
    ...
    ExAcquireFastMutex(&ExpWorkerSwapinMutex);// Serialize callers.
    ExpWorkersCanSwap = AllowSwap;// Stop new threads from swapping.
    ...
    for (Thread = PsGetNextProcessThread(Process, NULL);
        ...
        // Skip threads that are not worker threads or worker threads that were permanently marked noswap at creation time.
        if (Thread->ExWorkerCanWaitUser == 0) {
            continue;
        }

        if (Thread == CurrentThread) {
            KeSetKernelStackSwapEnable(AllowSwap);// No need to use an APC on the current thread.
        } else {// Queue an APC to the thread, and wait for it to fire:
            KeInitializeApc(&Apc, &Thread->Tcb, InsertApcEnvironment, ExpSetSwappingKernelApc, NULL, NULL, KernelMode, &AllowSwap);
...
LOGICAL ExpCheckQueueShutdown(IN WORK_QUEUE_TYPE QueueType, IN PSHUTDOWN_WORK_ITEM ShutdownItem)
{
    ...
    // Note that using interlocked sequences to increment the worker count and decrement it to CountForQueueEmpty ensures that once it *is* equal to CountForQueueEmpty and the disabled flag is set,
    // we won't be incrementing it any more, so we're safe making this check without locks.
...
VOID ExpShutdownWorker(IN PVOID Parameter)
{
    ...
    if (ShutdownItem->PrevThread != NULL) {
        // Wait for the previous thread to exit -- if it's in the same queue, it probably has already, 
        // but we need to make sure (and if it's not, we *definitely* need to make sure).
    ...
    CurrentThread = PsGetCurrentThread();
    if ((!ExpCheckQueueShutdown(DelayedWorkQueue, ShutdownItem)) && (!ExpCheckQueueShutdown(CriticalWorkQueue, ShutdownItem))) {
...
VOID ExpShutdownWorkerThreads(VOID)
{
    ...
    // After all currently queued work items are complete, this will fire, repeatedly taking out every worker thread in every queue until they're all done.
...
/*
Routine Description:
    This is a worker thread for the kernel debugger that can be used to perform certain tasks on the target machine asynchronously.
    This is necessary when the machine needs to run at Dispatch level to perform certain operations, such as paging in data.
Arguments:
