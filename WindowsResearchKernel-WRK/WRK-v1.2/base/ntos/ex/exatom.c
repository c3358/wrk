...
PVOID ExpDummyGetAtomTable(VOID);
...
Arguments:
    ...
    Atom - Supplies an optional pointer to a variable that receives the atom value.
Return Value:
...
*/
{
    ...
    // If the previous mode is user, then probe and copy the arguments as appropriate.

    // N.B. It is known that the length is less than a page from the above check and, therefore, a small buffer probe can be used.
...
NTSTATUS NtQueryInformationAtom(__in RTL_ATOM Atom, __in ATOM_INFORMATION_CLASS InformationClass, __out_bcount(AtomInformationLength) PVOID AtomInformation,
                                __in ULONG AtomInformationLength, __out_opt PULONG ReturnLength)
    /*
    Routine Description:
    ...
    */
{
    ...
    try {
        ...
        case AtomBasicInformation:// Query basic atom information.
            ...
            break;
        case AtomTableInformation:// Query atom table information.
            ...
            break;
        default:// Invalid information class.
...
/*
Routine Description:
    This function is a dummy function that is used until the win32k subsystem defines the atom table call back address.
*/
{
    return NULL;
}