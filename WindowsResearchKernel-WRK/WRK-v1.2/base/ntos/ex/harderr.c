...
#if defined(_AMD64_)
VOID ExpSystemErrorHandler2(IN NTSTATUS ErrorStatus,
                            IN ULONG NumberOfParameters,
                            IN ULONG UnicodeStringParameterMask,
                            IN PULONG_PTR Parameters,
                            IN BOOLEAN CallShutdown);
...
#endif
{
...
#if !defined(_AMD64_)
    // This handler is called whenever a hard error occurs before the default handler has been installed.

    // This is done regardless of whether or not the process has chosen default hard error processing.

    // Capture the callers context as closely as possible into the debugger's processor state area of the Prcb

    // N.B. There may be some prologue code that shuffles registers such that they get destroyed.
    ...
    for (Counter = 0; Counter < NumberOfParameters; Counter += 1) {
        if (UnicodeStringParameterMask & 1 << Counter) {
            ...
            Status = RtlUnicodeStringToAnsiString(&AnsiString, (PUNICODE_STRING)Parameters[Counter], TRUE);
    ...
    if (PsSystemDllDllBase != NULL) {
        try {
            // If we are on a DBCS code page, we have to use ENGLISH resource instead of default resource because HalDisplayString() can only display ASCII characters on the blue screen.
            Status = RtlFindMessage(PsSystemDllDllBase,
                                    11,
                                    NlsMbCodePageTag ? MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US) : 0,
                                    ErrorStatus,
                                    &MessageEntry);
            if (!NT_SUCCESS(Status)) {
                ErrorCaption = (PSZ)UnknownHardError;
    ...
    // Allocate the OEM string out of nonpaged pool so that bugcheck can read it.
                           ...
                           OUT PULONG Response)
{
    ...
    if (ValidResponseOptions == OptionShutdownSystem) {
        if (!SeSinglePrivilegeCheck(SeShutdownPrivilege, PreviousMode)) {// Check to see if the caller has the privilege to make this call.
    ...
    // If the default handler is not installed, then call the fatal hard error handler if the error status is error

    // Let GDI override this since it does not want to crash the machine when a bad driver was loaded via MmLoadSystemImage.
    ...
    // If the process has an error port, then if it wants default handling, use its port. If it disabled default handling, then return the error to the caller.
    // If the process does not have a port, then use the registered default handler.
    ErrorPort = NULL;
    if (Process->ExceptionPort) {
        if (Process->DefaultHardErrorProcessing & 1) {
            ErrorPort = Process->ExceptionPort;
        } else {
            if (ErrorStatus & HARDERROR_OVERRIDE_ERRORMODE) {// If error processing is disabled, check the error override status.
                ErrorPort = Process->ExceptionPort;
            }
        }
    } else {
...
NTSTATUS NtRaiseHardError(__in NTSTATUS ErrorStatus,
                          ...
                          __out PULONG Response)
{
    ...
    if (PreviousMode != KernelMode) {
                ...
                if (UnicodeStringParameterMask) {// Probe all strings.
                    for (Counter = 0; Counter < NumberOfParameters; Counter += 1) {
                        if (UnicodeStringParameterMask & (1 << Counter)) {// if there is a string in this position, then probe and capture the string
                            ...
                            ProbeForRead(CapturedString.Buffer, CapturedString.MaximumLength, sizeof(UCHAR));// Now probe the string
                        }
                    }
                }
            }
        ...
        // Call ExpRaiseHardError. All parameters are probed and everything should be user-mode.
        // ExRaiseHardError will squirt all strings into user-mode without any probing
        ...
        Status = ExRaiseHardError(ErrorStatus, NumberOfParameters, UnicodeStringParameterMask, Parameters, ValidResponseOptions, &LocalResponse);
        *Response = LocalResponse;
    }
...
NTSTATUS ExRaiseHardError(__in NTSTATUS ErrorStatus,
                          __in ULONG NumberOfParameters,
                          __in ULONG UnicodeStringParameterMask,
                          __in_ecount(NumberOfParameters) PULONG_PTR Parameters,
                          __in ULONG ValidResponseOptions,
                          __out PULONG Response)
{
    ...
    // If we are in the process of shutting down the system, do not allow hard errors.
    ...
    // If the parameters contain strings, we need to capture the strings and the string descriptors and push them into user-mode.
    if (ARGUMENT_PRESENT(Parameters)) {
            ...
            try {
                for (Counter = 0; Counter < NumberOfParameters; Counter += 1) {
                    ...
                    if (UnicodeStringParameterMask & 1 << Counter) {
                        ...
                        UserModeStringDataBase += CapturedStrings[Counter].MaximumLength;// Adjust the string data base.
                    } else {
