...
Abstract:
   This module implements the executive profile object.
   Functions are provided to create, start, stop, and query profile objects.
*/
...
BOOLEAN ExpProfileInitialization(VOID)
/*
Routine Description:
    This function creates the profile object type descriptor at system initialization and stores the address of the object type descriptor in global storage.
Return Value:
    A value of TRUE is returned if the profile object type descriptor is successfully initialized. Otherwise a value of FALSE is returned.
*/
{
    ...
    KeInitializeMutex(&ExpProfileStateMutex, MUTEX_LEVEL_EX_PROFILE);// Initialize mutex for synchronizing start and stop operations.
    RtlInitUnicodeString(&TypeName, L"Profile");// Initialize string descriptor.
    ...
    return (BOOLEAN)(NT_SUCCESS(Status));// If the event object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
}
...
Arguments:
    ProfileHandle - Supplies a pointer to a variable that will receive the profile object handle.
    Process - Optionally, supplies the handle to the process whose address space to profile.
              If the value is NULL (0), then all address spaces are included in the profile.
    RangeBase - Supplies the address of the first byte of the address space for which profiling information is to be collected.
    RangeSize - Supplies the size of the range to profile in the address space.
                RangeBase and RangeSize are interpreted such that RangeBase <= address < RangeBase+RangeSize will generate a profile hit.
    BucketSize - Supplies the LOG base 2 of the size of the profiling bucket.
                 Thus, BucketSize = 2 yields four-byte buckets, BucketSize = 7 yields 128-byte buckets.
                 All profile hits in a given bucket will increment the corresponding counter in Buffer.
                 Buckets cannot be smaller than a ULONG.
                 The acceptable range of this value is 2 to 30 inclusive.
    Buffer - Supplies an array of ULONGs.  Each ULONG is a hit counter, which records the number of hits of the corresponding bucket.
...
*/
{
    ...
    //        Improper use of bucket size.
    //        If bucket size is zero, and RangeBase < 64K, then create a profile object to attach to a non-flat code segment.
    //        In this case, RangeBase is the non-flat CS for this profile object.
#ifdef i386
    ...
    // Establish an exception handler, probe the output handle address, and attempt to create a profile object.
    ...
    try {
        ...
        // If an exception occurs during the probe of the output handle address, then always handle the exception and return the exception code as the status value.
    ...
    if (!ARGUMENT_PRESENT(Process)) {
        ...
        // Profile all processes. Make sure that the specified address range is in system space, unless SeSystemProfilePrivilege.
        if (RangeBase <= MM_HIGHEST_USER_ADDRESS) {
            // Check for privilege before allowing a user to profile all processes and USER addresses.
            if (PreviousMode != KernelMode) {
    ...
    } else {// We failed, remove our reference to the process object.
        if (ProcessAddress != NULL) {
...
/*
Routine Description:
    The NtStartProfile routine starts the collecting data for the specified profile object.
    This involved allocating nonpaged pool to lock the specified buffer in memory,
    creating a kernel profile object and starting collecting on that profile object.
Arguments:
...
*/
{
    ...
    PreviousMode = KeGetPreviousMode();
    Status = ObReferenceObjectByHandle(ProfileHandle, PROFILE_CONTROL, ExProfileObjectType, PreviousMode, &Profile, NULL);
    if (!NT_SUCCESS(Status)) {
        return Status;
    }

    // Acquire the profile state mutex so two threads can't operate on the same profile object simultaneously.
    ...
    ProfileObject = ExAllocatePoolWithTag(NonPagedPool, MmSizeOfMdl(Profile->Buffer, Profile->BufferSize) + sizeof(KPROFILE), 'forP');
    ...
    // Since kernel space is specified below, this call cannot raise an exception.
    ...
    KeInitializeProfile(ProfileObject, Profile->Process, Profile->RangeBase, Profile->RangeSize, Profile->BucketSize, Profile->Segment, Profile->ProfileSource, Profile->Affinity);
...
/*
Routine Description:
    The NtStopProfile routine stops collecting data for the specified profile object.
    This involves stopping the data collection on the profile object, unlocking the locked buffers, and deallocating the pool for the MDL and profile object.
Arguments:
...
/*
Routine Description:
    This routine allows the system-wide interval (and thus the profiling rate) for profiling to be set.
Arguments:
...
/*
Routine Description:
    This routine queries the system-wide interval (and thus the profiling rate) for profiling.
Arguments:
...
/*
Routine Description:
    This function returns current value of performance counter and, optionally, the frequency of the performance counter.

    Performance frequency is the frequency of the performance counter in Hertz, i.e., counts/second.
    Note that this value is implementation dependent.
    If the implementation does not have hardware to support performance timing, the value returned is 0.
Arguments:
    PerformanceCounter - supplies the address of a variable to receive the current Performance Counter value.
    PerformanceFrequency - Optionally, supplies the address of a variable to receive the performance counter frequency.
Return Value:
