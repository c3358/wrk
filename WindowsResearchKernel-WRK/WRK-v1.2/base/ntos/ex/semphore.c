...
Abstract:
   This module implements the executive semaphore object. Functions are provided to create, open, release, and query semaphore objects.
*/

#include "exp.h"

ULONG ExpSemaphoreBoost = SEMAPHORE_INCREMENT;// Temporary so boost is patchable
POBJECT_TYPE ExSemaphoreObjectType;// Address of semaphore object type descriptor.

// Structure that describes the mapping of generic access rights to object specific access rights for semaphore objects.
...
*/
{
    ...
    RtlInitUnicodeString(&TypeName, L"Semaphore");// Initialize string descriptor.
    ...
    // If the semaphore object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        // If the semaphore object was successfully inserted in the current process' handle table, then attempt to write the semaphore handle value.
...
*/
{
    ...
    Status = ObOpenObjectByName(ObjectAttributes, ExSemaphoreObjectType, PreviousMode, NULL, DesiredAccess, NULL, &Handle);

    // If the open was successful, then attempt to write the semaphore object handle value.
    // If the write attempt fails, then do not report an error.
    // When the caller attempts to access the handle value, an access violation will occur.
    if (NT_SUCCESS(Status)) {
        if (PreviousMode != KernelMode) {
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(SemaphoreHandle, SEMAPHORE_QUERY_STATE, ExSemaphoreObjectType, PreviousMode, &Semaphore, NULL);

    // If the reference was successful, then read the current state and maximum count of the semaphore object, dereference semaphore object,
    // fill in the information structure, and return the length of the information structure if specified.
    // If the write of the semaphore information or the return length fails, then do not report an error.
...
*/
{
    ...
    // Reference semaphore object by handle.
    Status = ObReferenceObjectByHandle(SemaphoreHandle, SEMAPHORE_MODIFY_STATE, ExSemaphoreObjectType, PreviousMode, &Semaphore, NULL);
    ...
    // has been exceeded, then dereference the semaphore object and return the exception code as the service status.
    // Otherwise write the previous count value if specified.
