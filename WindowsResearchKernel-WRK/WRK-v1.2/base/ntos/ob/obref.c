...
KSPIN_LOCK ObpStackTraceLock;// Spin lock for object tracing

#define OBTRACE_HASHOBJECT(x) (((((ULONG)(ULONG_PTR)(&(x)->Body)) >> 4) & 0xfffff) % OBTRACE_OBJECTBUCKETS)


POBJECT_REF_INFO ObpGetObjectRefInfo(POBJECT_HEADER ObjectHeader)
/*
Routine Description:
    This routine returns a pointer to the OBJECT_REF_INFO for the specified object, or NULL, if it doesn't exist.
Arguments:
...
ULONG ObpGetTraceIndex(POBJECT_REF_TRACE Trace)
/*
Routine Description:
    This routine returns the index of 'Trace' in the stack trace hash table (ObpStackTable).
    If Trace does not exist in the table, it is added, and the new index is returned.
Arguments:
...
*/
{
    ...
    // Loop through the ObpTracePoolTagsBuffer string, and convert it to an array of pool tags.
    ...
    // If object tracing was turned on via the registry key, then we need to allocate memory for the tables.
    // If the memory allocations fail, we turn off tracing by clearing the pool tag array.
    if (ObpTracePoolTags[0] != 0) {
        ...
        if (ObpStackTable != NULL) {
            ...
            ObpObjectTable = ExAllocatePoolWithTag(NonPagedPool, OBTRACE_OBJECTBUCKETS * sizeof(POBJECT_REF_INFO), 'TSbO');
...
BOOLEAN ObpIsObjectTraced(POBJECT_HEADER ObjectHeader)
/*
Routine Description:
    This routine determines if an object should have its references and dereferences traced.
Arguments:
...
*/
{
    ...
    if (ObjectHeader != NULL) {
        // Loop through the ObpTracePoolTags array, and return true if the object type key matches one of them.
        for (i = 0; i < sizeof(ObpTracePoolTags) / sizeof(ULONG); i++) {
...
Routine Description:
    ...
    It determines if the object should be traced, and if so, adds it to the hash table.
Arguments:
...
*/
{
    ...
    if (ObpIsObjectTraced(ObjectHeader)) {
        ...
        if (ObjectRefInfo != NULL) {
            ...
            RtlCopyMemory(ObjectRefInfo->ImageFileName, PsGetCurrentProcess()->ImageFileName, sizeof(ObjectRefInfo->ImageFileName));
...
Routine Description:
    ...
    It determines if the object is traced, and if so, deletes it from the hash table.
Arguments:
...
/*
Routine Description:
    This routine is called each time an object is referenced or dereferenced.
    It determines if the object is traced, and if so, adds the necessary trace to the object reference info.
Arguments:
...
*/
{
    ...
    if (ObpIsObjectTraced(ObjectHeader)) {// Are we tracing this object?
        ...
        if (CapturedTraces >= 1) {
            ...
            if (ObjectInfo) {
                StackIndex = ObpGetTraceIndex(&Stack);// Get the table index for the trace
                if (StackIndex < OBTRACE_STACKS) {
                    if (ObjectInfo->NextPos < OBTRACE_STACKSPEROBJECT) {// Add new reference info to the object
...
Routine Description:
    ...
    Soon after entering we capture the SubjectContext for the caller.
    This context must remain captured until auditing is complete, and passed to any routine that may have to do access checking or auditing.
Arguments:
    ...
    AccessState - Supplies an optional pointer to the current access status describing already granted access types,
...
Return Value:
    If the object is successfully opened, then a handle for the object is created and a success status is returned.
...
*/
{
    ...
    *Handle = NULL;
    if (!ARGUMENT_PRESENT(ObjectAttributes)) {
        Status = STATUS_INVALID_PARAMETER;//  If the object attributes are not specified, then return an error.
    } else {
        ...
        Status = ObpCaptureObjectCreateInformation(ObjectType, AccessMode, AccessMode, ObjectAttributes, &CapturedObjectName, &TempBuffer->ObjectCreateInfo, TRUE);
        if (NT_SUCCESS(Status)) {//  If the object creation information is successfully captured, then generate the access state.
            if (!ARGUMENT_PRESENT(AccessState)) {
                GenericMapping = NULL;

                //If an object type descriptor is specified, then use associated generic mapping. Otherwise, use no generic mapping.
                if (ARGUMENT_PRESENT(ObjectType)) {
                    GenericMapping = &ObjectType->TypeInfo.GenericMapping;
                }
                AccessState = &TempBuffer->LocalAccessState;
                Status = SeCreateAccessState(&TempBuffer->LocalAccessState, &TempBuffer->AuxData, DesiredAccess, GenericMapping);
                if (!NT_SUCCESS(Status)) {
                    goto FreeCreateInfo;
                }
            }

            //  If there is a security descriptor specified in the object attributes, then capture it in the access state.
            ...
            Status = ObpValidateAccessMask(AccessState);//  Validate the access state.
            if (NT_SUCCESS(Status)) {//  If the access state is valid, then lookup the object by name.
                Status = ObpLookupObjectName(TempBuffer->ObjectCreateInfo.RootDirectory,
                                             ...
                                             &ExistingObject);
                if (NT_SUCCESS(Status)) {//If the object was successfully looked up, then attempt to create or open a handle.
                    ObjectHeader = OBJECT_TO_OBJECT_HEADER(ExistingObject);
                    if (ObjectHeader->Flags & OB_FLAG_NEW_OBJECT) {//If the object is being created, then the operation must be a open-if operation.				                      
                    ...
                    } else {//Otherwise, a handle to an object is being opened.
                        OpenReason = ObOpenHandle;
                    }

                    if (ObjectHeader->Type->TypeInfo.InvalidAttributes & TempBuffer->ObjectCreateInfo.Attributes) {
                        Status = STATUS_INVALID_PARAMETER;//  If any of the object attributes are invalid, then return an error status.
                        ObpReleaseLookupContext(&TempBuffer->LookupContext);
                        ObDereferenceObject(ExistingObject);
                    } else {//The status returned by the object lookup routine must be returned if the creation of a handle is successful.												
                        ...
                        if (!NT_SUCCESS(HandleStatus)) {
                            ObDereferenceObject(ExistingObject);
                            Status = HandleStatus;//  Otherwise, the handle creation status is returned.
                        } else {
                            *Handle = NewHandle;
                        }
                    }
            ...
            if (AccessState == &TempBuffer->LocalAccessState) {//If the access state was generated, then delete the access state.
                SeDeleteAccessState(AccessState);
            }

        FreeCreateInfo://  Free the create information.
...
*/
{
    ...
    //  First increment the pointer count for the object.  This routine also checks the object types
    Status = ObReferenceObjectByPointer(Object, 0, ObjectType, AccessMode);
    if (NT_SUCCESS(Status)) {
        ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);//  Get the object header for the input object body

        //  If the caller did not pass in an access state then we will create a new one based on the desired access and the object types generic mapping
        ...
        } else {//  Otherwise the caller did specify an access state so we use the one passed in.
            AccessState = PassedAccessState;
        }

        //  Make sure the caller is asking for handle attributes that are valid for the given object type
    ...
    if (NT_SUCCESS(Status)) {//  If we successfully opened by object and created a new handle then set the output variable correctly
        *Handle = NewHandle;
    } else {
        *Handle = NULL;
    }

    if (AccessState == &LocalAccessState) {//  Check if we used our own access state and now need to cleanup
        SeDeleteAccessState(AccessState);
    }
...
*/
{
    ...
    *Object = NULL;
    ...
    if ((LONG)(ULONG_PTR)Handle < 0) {
        //  If the handle is equal to the current process handle and the object type is NULL or type process, 
        //  then attempt to translate a handle to the current process.
        ...
        if (Handle == NtCurrentProcess()) {
            ...
            //  If the handle is equal to the current thread handle and the object type is NULL or type thread,
            //	then attempt to translate a handle to the current thread.
            //	Otherwise, the we'll try and translate the handle
            ...
            return STATUS_INVALID_HANDLE;// The previous mode was user for this kernel handle value. Reject it here.
        }
    } else {
    ...
    KeEnterCriticalRegionThread(&Thread->Tcb);// Protect this thread from being suspended while we hold the handle table entry lock
    ...
    if (ObjectTableEntry != NULL) {//  Make sure the object table entry really does exist
        ...
        // Optimize for a successful reference by bringing the object header into the cache exclusive.
        ...
        if ((ObjectHeader->Type == ObjectType) || (ObjectType == NULL)) {
            ...
            if ((SeComputeDeniedAccesses(GrantedAccess, DesiredAccess) == 0) || (AccessMode == KernelMode)) {
                ...
                //  Access to the object is allowed. Return the handle information is requested, increment the object pointer count, 
                //  unlock the handle table and return a success status.
                //  Note that this is the only successful return path out of this routine if the user did not specify the current process or 
                //  current thread in the input handle.
                if (ARGUMENT_PRESENT(HandleInformation)) {
                    HandleInformation->GrantedAccess = GrantedAccess;
                    HandleInformation->HandleAttributes = ObpGetHandleAttributes(ObjectTableEntry);
                }

                //  If this object was audited when it was opened, it may be necessary to generate an audit now.
                //  Check the audit mask that was saved when the handle was created.

                //  It is safe to do this check in a non-atomic fashion, because bits will never be added to this mask once it is created.
...
NTSTATUS ObpReferenceProcessObjectByHandle(IN HANDLE Handle,
...
*/
{
    ...
    *Object = NULL;
    ...
    if ((LONG)(ULONG_PTR)Handle < 0) {
        ...
        if (Handle == NtCurrentProcess()) {
            ...
            *Object = Thread;
            ...
            Handle = DecodeKernelHandle(Handle);//  Make the handle look like a regular handle            
            HandleTable = ObpKernelHandleTable;//  The global kernel handle table
        } else {// The previous mode was user for this kernel handle value. Reject it here.            
    ...
    if (ObjectTableEntry != NULL) {//  Make sure the object table entry really does exist
        ...
        //  Return the handle information, increment the object pointer count, unlock the handle table and return a success status.
        ...
        if (ObjectInfo != NULL) {//  Return handle audit information to the caller
...
Routine Description:
    ...
    This routine is meant to solve a very particular handle reference issue with file object access auditing.
    Do not call this unless you understand exactly what you are doing.
Arguments:
    Handle - Supplies a handle to the IoFileObjectType being referenced.
    AccessMode - Supplies the processor mode of the access
    FileObject - Receives a pointer to the object body if the operation is successful
    HandleInformation - receives information regarding the input handle.
Return Value:
    An appropriate NTSTATUS value
*/
{
    ...
    if ((LONG)(ULONG_PTR)Handle < 0) {// Check is this handle is a kernel handle
        if ((AccessMode == KernelMode) && (Handle != NtCurrentProcess()) && (Handle != NtCurrentThread())) {
            Handle = DecodeKernelHandle(Handle);//  Make the handle look like a regular handle            
            HandleTable = ObpKernelHandleTable;//  The global kernel handle table
        } else {// The previous mode was user for this kernel handle value, or it was a builtin handle. Reject it here.            
            return STATUS_INVALID_HANDLE;
        }
    } else {
        HandleTable = PsGetCurrentProcessByThread(Thread)->ObjectTable;
    ...
    if (ObjectTableEntry != NULL) {//  Make sure the object table entry really does exist
        ...
        if (NT_SUCCESS(IoComputeDesiredAccessFileObject((PFILE_OBJECT)& ObjectHeader->Body, (PNTSTATUS)& DesiredAccess))) {
            ...
            //  Return the handle information, increment the object pointer count, 
            //  compute correct access, audit, unlock the handle table and return a success status.

            //  Note that this is the only successful return path out of this routine.
            HandleInformation->GrantedAccess = GrantedAccess;
            HandleInformation->HandleAttributes = ObpGetHandleAttributes(ObjectTableEntry);

            // Check to ensure that the caller has either WRITE_DATA or APPEND_DATA access to the file.
            // If not, cleanup and return an access denied error status value.
            // Note that if this is a pipe then the APPEND_DATA access check may not be made since this access code is overlaid with CREATE_PIPE_INSTANCE access.
    ...
    //  No handle translation is possible. Set the object address to NULL and return an error status.
    *FileObject = NULL;
    return Status;
}
...
Routine Description:
    ...
    If so, it will clear the bits in the handle and generate the appropriate audit before returning.

    The bits in the handle's audit mask are cleared in an atomic way so that multiple threads coming through
    this code do not generate more than one audit for the same operation.
Arguments:
    Handle - Supplies the handle being accessed.
    AccessMode - The mode (kernel or user) that originated the handle.
    DesiredAccess - Supplies the access mask describing how the handle is being used in this operation.
*/
{
    PHANDLE_TABLE HandleTable;
    PHANDLE_TABLE_ENTRY ObjectTableEntry;
    ...
    // Do not currently support this on kernel mode handles.
...
Arguments:
    Handle - Supplies the handle being accessed.
    ObjectTableEntry - Supplies the object table entry for the handle passed in the first parameter.
    DesiredAccess - Supplies the access mask describing how the handle is being used in this operation.
*/
{
    ACCESS_MASK t1, t2, r;
    ...
    //  Determine if this access is to be audited, and if so, clear the bits in the ObjectTableEntry.
    while (ObjectTableEntryInfo->AuditMask != 0) {
        ...
        } else {// else, somebody changed it, go around for another try            
            return;//  There are no bits in the AuditMask that we want to audit here, just leave.
        }
    }
}


NTSTATUS ObReferenceObjectByName(__in PUNICODE_STRING ObjectName,
...
*/
{
    ...
    //  If the object name descriptor is not specified, or the object name length is zero (tested after capture), then the object name is invalid.
    if (ObjectName == NULL) {
    ...
    if (NT_SUCCESS(Status)) {
        ...
        *Object = NULL;
        if (NT_SUCCESS(Status)) {//  If the lookup was successful, then return the existing object if access is allowed. Otherwise, return NULL.
        ...
        if (AccessState == &LocalAccessState) {//  If the access state was generated, then delete the access state.
            SeDeleteAccessState(AccessState);
        }

    FreeBuffer:
        ObpFreeObjectNameBuffer(&CapturedObjectName);//  Free the object name buffer.
    }
...
*/
{
    ...
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);//  Translate the pointer to the object body to a pointer to the object header

    //  If the specified object type does not match and either the caller is not kernel mode or it is not a symbolic link object then it is an error
...
VOID ObpDeferObjectDeletion(IN POBJECT_HEADER ObjectHeader)
{
    ...
    // Push this object on the list. If we make an empty to non-empty transition then we may have to start a worker thread.
    while (1) {
    ...
    if (OldValue == NULL) {//  If we have to start the worker thread then go ahead and enqueue the work item        
        ExQueueWorkItem(&ObpRemoveObjectWorkItem, CriticalWorkQueue);
    }
}


LONG_PTR FASTCALL ObfReferenceObject(__in PVOID Object)
/*
Routine Description:
    This function increments the reference count for an object.
    N.B. This function should be used to increment the reference count when the accessing mode is kernel or the objct type is known.
Arguments:
...
LONG_PTR FASTCALL ObReferenceObjectEx(IN PVOID Object, IN ULONG Count)
/*
Routine Description:
...
LONG_PTR FASTCALL ObDereferenceObjectEx(IN PVOID Object, IN ULONG Count)
/*
Routine Description:
...
Routine Description:
    This function increments the reference count for an object.
    It returns FALSE if the object is being deleted or TRUE if it's safe to use the object further
Arguments:
    Object - Supplies a pointer to the object whose reference count is incremented.
Return Value:
...
LONG_PTR FASTCALL ObfDereferenceObject(__in PVOID Object)
/*
Routine Description:
    This routine decrments the reference count of the specified object and does whatever cleanup there is if the count goes to zero.
Arguments:
...
*/
{
    ...
    //  Decrement the point count and if the result is now then there is extra work to do
    ...
    if (Result == 0) {
        ...
        if (!KeAreAllApcsDisabled()) {//  If we're at the passive level then go ahead and delete the object now.
#ifdef POOL_TAGGING
...
VOID ObDereferenceObjectDeferDelete(IN PVOID Object)
{
    ...
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);//  Translate a pointer to the object body to a pointer to the object header.
...
*/
{
    ...
    // The list head serves two purposes.
    // First it maintains the list of objects we need to delete and second it signals that this thread is active.
    // While we are processing the latest list we leave the header as the value 1. 
    // This will never be an object address as the bottom bits should be clear for an object.
    while (1) {
        ...
        if (ObpRemoveObjectList == (PVOID)1 && InterlockedCompareExchangePointer(&ObpRemoveObjectList, NULL, (PVOID)1) == (PVOID)1) {
            break;
        }
    }
}
...
Arguments:
    ...
    CalledOnWorkerThread - TRUE if called on worker thread, FALSE if called in the context of the ObDereferenceObject.
*/
{
    NTSTATUS Status;
    POBJECT_HEADER ObjectHeader;
    ...
    //  Retrieve an object header from the object body, and also get the object type, creator and name info if available
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
    ObjectType = ObjectHeader->Type;
    CreatorInfo = OBJECT_HEADER_TO_CREATOR_INFO(ObjectHeader);
    NameInfo = OBJECT_HEADER_TO_NAME_INFO(ObjectHeader);

    //  If there is a creator info record and we are on the list for the object type then remove this object from the list
    if (CreatorInfo != NULL && !IsListEmpty(&CreatorInfo->TypeList)) {
        ...
        ObpLeaveObjectTypeMutex(ObjectType);//  We are done with the object type object so we can now release it
    }

    //  If there is a name info record and the name buffer is not null then free the buffer and zero out the name record
    ...
    //  Security descriptor deletion must precede the call to the object's DeleteProcedure.
    //  Check if we have a security descriptor and if so then call the routine to delete the security descritpor.
    ...
    ObpFreeObject(Object);//  Finally return the object back to pool including releasing any quota charges
}
...
*/
{
    ...
    //  Translate the object body to an object header also get the object type and name info if present
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
    NameInfo = ObpReferenceNameInfo(ObjectHeader);
    ObjectType = ObjectHeader->Type;

    //  Make sure that the object has a zero handle count, has a non empty name buffer, and is not a permanent object
    if ((ObjectHeader->HandleCount == 0) &&
        ...
        //  Check that the object we is still in the directory otherwise then is nothing for us to remove
        if (Object == ObpLookupDirectoryEntry(NameInfo->Directory, &NameInfo->Name, 0, FALSE, &LookupContext)) {
            //  Now reacquire the lock on the object type and check check the handle count again.
            //  If it is still zero then we can do the actual delete name operation
            ...
            if (ObjectHeader->HandleCount == 0 && (ObjectHeader->Flags & OB_FLAG_PERMANENT_OBJECT) == 0) {
                ObpDeleteDirectoryEntry(&LookupContext);//  Delete the directory entry

                if (ObjectType == ObpSymbolicLinkObjectType) {//  If this is a symbolic link object then we also need to delete the symbolic link
        ...
        //  If there is a directory object for the name then decrement its reference count for it and for the object
        ObpDereferenceNameInfo(NameInfo);
        if (DirObject != NULL) {
            //  Dereference the name twice: one because we referenced it to safely access the name info, 
            //  and the second deref is because we want a deletion for the NameInfo
            ObpDereferenceNameInfo(NameInfo);
            ObDereferenceObject(Object);
        }
    } else {
...
LONG_PTR ObDereferenceObject(IN PVOID Object)
/*
Routine Description:
...
BOOLEAN ObIsObjectDeletionInline(IN PVOID Object)
/*
Routine Description:
    This is available only of object DeleteProcedure callbacks.
    It allows the callback to determine whether the stack on which it is invoked is
Arguments:
    Object - Supplies a pointer to the body of the object being deleted
Return Value:
    TRUE if the deletion procedure is being invoked on the same stack as the ObDereferenceObject,
    and FALSE if the procedure is being invoked from a queued work item.
*/
{
    POBJECT_HEADER ObjectHeader;
