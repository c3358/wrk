...
Abstract:
    The Fast I/O path is used to avoid calling the file systems directly to do a cached read.
    This module is only used if the file object indicates that caching is enabled (i.e., the private cache map is not null).
*/
...
/*
Routine Description:
    This routine does a fast cached read bypassing the usual file system entry routine (i.e., without the Irp).
    It is used to do a copy read of a cached file object.
    For a complete description of the arguments see CcCopyRead.
Arguments:
...
*/
{
    ...
    if (Length != 0) {
        //  Check for overflow. Returning false here will re-route this request through the IRP based path, but this isn't performance critical.
        ...
        FsRtlEnterFileSystem();//  Enter the file system

        //  Increment performance counters and get the resource
        if (Wait) {
            HOT_STATISTIC(CcFastReadWait) += 1;
            (VOID)ExAcquireResourceSharedLite(Header->Resource, TRUE);//  Acquired shared on the common fcb header
        } else {
            HOT_STATISTIC(CcFastReadNoWait) += 1;

            //  Acquired shared on the common fcb header, and return if we don't get it
        ...
        //  Now that the File is acquired shared, we can safely test if it is really cached and if we can do fast i/o and if not, then release the fcb and return.
        ...
        //  Check if fast I/O is questionable and if so then go ask the file system the answer
        if (Header->IsFastIoPossible == FastIoIsQuestionable) {
            ...
            //  All file systems that set "Is Questionable" had better support fast I/O
            ASSERT(FastIoDispatch != NULL);
            ASSERT(FastIoDispatch->FastIoCheckIfPossible != NULL);

            //  Call the file system to check for fast I/O.
            //  If the answer is anything other than GoForIt then we cannot take the fast I/O path.
        ...
        //  We can do fast i/o so call the cc routine to do the work and then release the fcb when we've done.
        //  If for whatever reason the copy read fails, then return FALSE to our caller.

        //  Also mark this as the top level "Irp" so that lower file system levels will not attempt a pop-up
        ...
        try {
            if (Wait && ((BeyondLastByte.HighPart | Header->FileSize.HighPart) == 0)) {
                ...
                FileObject->Flags |= FO_FILE_FAST_IO_READ;
                ASSERT(!Status || (IoStatus->Status == STATUS_END_OF_FILE) || ((LONGLONG)(FileOffset->QuadPart + IoStatus->Information) <= Header->FileSize.QuadPart));
            }
...
/*
Routine Description:
    This routine does a fast cached write bypassing the usual file system entry routine (i.e., without the Irp).
    It is used to do a copy write of a cached file object.
    For a complete description of the arguments see CcCopyWrite.
Arguments:
...
*/
{
    ...
    BOOLEAN WriteToEndOfFile = (BOOLEAN)((FileOffset->LowPart == FILE_WRITE_TO_END_OF_FILE) && (FileOffset->HighPart == -1));

    PAGED_CODE();
    UNREFERENCED_PARAMETER(DeviceObject);

    Header = (PFSRTL_COMMON_FCB_HEADER)FileObject->FsContext;//  Get a real pointer to the common fcb header

    //  Do we need to verify the volume?  If so, we must go to the file system.
    //  Also return FALSE if FileObject is write through, the File System must do that.
    if (CcCanIWrite(FileObject, Length, Wait, FALSE) && !FlagOn(FileObject->Flags, FO_WRITE_THROUGH) && CcCopyWriteWontFlush(FileObject, FileOffset, Length)) {
        ...
        if (Length != 0) {
            ...
            //  Split into separate paths for increased performance.
            //  First we have the faster path which only supports Wait == TRUE and 32 bits.
            //  We will make an unsafe test on whether the fast path is ok, then just return FALSE later if we were wrong.
            //  This should virtually never happen.

            //  IMPORTANT NOTE: It is very important that any changes made to this path also be applied to the 64-bit path which is the else of this test!
            if (Wait && (Header->AllocationSize.HighPart == 0)) {
                ...
                //  Make our best guess on whether we need the file exclusive or shared.
                //  Note that we do not check FileOffset->HighPart until below.
                NewFileSize = FileOffset->LowPart + Length;

                if (WriteToEndOfFile || (NewFileSize > Header->ValidDataLength.LowPart)) {
                    ExAcquireResourceExclusiveLite(Header->Resource, TRUE);//  Acquired shared on the common fcb header
                } else {
                    ExAcquireResourceSharedLite(Header->Resource, TRUE);//  Acquired shared on the common fcb header
                    AcquiredShared = TRUE;
                }

                //  We have the fcb shared now check if we can do fast i/o and if the file space is allocated, and if not then release the fcb and return.
                if (WriteToEndOfFile) {
                ...
                //  if it is really cached and if we can do fast i/o and we do not have to extend. If not then release the fcb and return.

                //  Get out if we have too much to zero.
                //  This case is not important for performance, and a file system supporting sparseness may have a way to do this more efficiently.
                ...
                //  If we will be extending ValidDataLength, we will have to get the Fcb exclusive, and make sure that FastIo is still possible.
                //  We should only execute this block of code very rarely, when the unsafe test for ValidDataLength failed above.
                ...
                //  Now see if we will change FileSize.  We have to do it now so that our reads are not nooped.
                ...
                //  We can do fast i/o so call the cc routine to do the work and then release the fcb when we've done.
                //  If for whatever reason the copy write fails, then return FALSE to our caller.
                ...
                if (Status) {
                    //  In the case of ValidDataLength, we really have to check again since we did not do this when we acquired the resource exclusive.
                    if (NewFileSize > Header->ValidDataLength.LowPart) {
                        Header->ValidDataLength.LowPart = NewFileSize;
                    }

                    FileObject->Flags |= FO_FILE_MODIFIED;//  Set this handle as having modified the file
                    ...
                    //  If we did not succeed, then we must restore the original FileSize while holding the PagingIoResource exclusive if it exists.
                ...
                //  Now that the File is acquired shared, we can safely test if it is really cached and if we can do fast i/o and we do not have to extend.
                ...
                if (NewFileSize.QuadPart > Header->FileSize.QuadPart) {
                    ...
                    //  Deal with an extremely rare pathological case here the file size wraps.
                ...
                if (Status) {
                    ...
                    if (NewFileSize.QuadPart > Header->ValidDataLength.QuadPart) {
                        //  Deal with an extremely rare pathological case here the ValidDataLength wraps.
                    ...
                    FileObject->CurrentByteOffset.QuadPart = Offset.QuadPart + Length;//  Also update the current file position pointer
            ...
            return TRUE;//  A zero length transfer was requested.
        }
    } else {
        return FALSE;// The volume must be verified or the file is write through.
    }
}
...
/*
Routine Description:
    This routine does a fast cached mdl read bypassing the usual file system entry routine (i.e., without the Irp).
    It is used to do a copy read of a cached file object.
    For a complete description of the arguments see CcMdlRead.
Arguments:
    FileObject - Pointer to the file object being read.
    FileOffset - Byte offset in file for desired data.
    Length - Length of desired data in bytes.
...
*/
{
    ...
    //  We can do fast i/o so call the cc routine to do the work and then release the fcb when we've done.
    //  If for whatever reason the mdl read fails, then return FALSE to our caller.
...
*/
{
    ...
    if ((FastIoDispatch != NULL) &&
        ...
        //  Get the DeviceObject for the volume.
        //  If that DeviceObject is different, and it specifies the FastIo routine, then we have to return FALSE here and cause an Irp to get generated.
        ...
        if ((VolumeDeviceObject != DeviceObject) &&
            ...
            (FastIoDispatch->MdlRead != NULL)) {
            return FALSE;
        } else {//  Otherwise, call the default routine.
...
BOOLEAN FsRtlMdlReadComplete(__in PFILE_OBJECT FileObject, __in PMDL MdlChain)
/*
Routine Description:
    This routine does a fast cached mdl read bypassing the usual file system entry routine (i.e., without the Irp).
    It is used to do a copy read of a cached file object.
...
BOOLEAN FsRtlMdlReadCompleteDev(__in PFILE_OBJECT FileObject, __in PMDL MdlChain, __in PDEVICE_OBJECT DeviceObject)
/*
Routine Description:
    This routine does a fast cached mdl read bypassing the usual file system entry routine (i.e., without the Irp).
    It is used to do a copy read of a cached file object.
...
*/
{
    ...
    //  Call CcCanIWrite.  Also return FALSE if FileObject is write through, the File System must do that.
    if (!CcCanIWrite(FileObject, Length, TRUE, FALSE) || FlagOn(FileObject->Flags, FO_WRITE_THROUGH)) {
        return FALSE;
    }

    IoStatus->Status = STATUS_SUCCESS; //  Assume our transfer will work

    if (Length == 0) {//  Special case the zero byte length
        return TRUE;
    }

    Header = (PFSRTL_COMMON_FCB_HEADER)FileObject->FsContext;//  Get a real pointer to the common fcb header
    FsRtlEnterFileSystem();//  Enter the file system
    NewFileSize.QuadPart = FileOffset->QuadPart + (LONGLONG)Length;//  Make our best guess on whether we need the file exclusive or shared.

    if (WriteToEndOfFile || (NewFileSize.QuadPart > Header->ValidDataLength.QuadPart)) {
        //  Acquired exclusive on the common fcb header, and return if we don't get it.
        ExAcquireResourceExclusiveLite(Header->Resource, TRUE);
    } else {
        //  Acquired shared on the common fcb header, and return if we don't get it.
    ...
    if (Status) {
        ...
        //  If we did not succeed, then we must restore the original FileSize and release the resource.
        //  In the success path, the cache manager will release the resource.
    } else {
        if (FileSizeChanged) {
            if (Header->PagingIoResource != NULL) {
                (VOID)ExAcquireResourceExclusiveLite(Header->PagingIoResource, TRUE);
...
BOOLEAN FsRtlMdlWriteComplete(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in PMDL MdlChain)
/*
Routine Description:
...
BOOLEAN FsRtlMdlWriteCompleteDev(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in PMDL MdlChain, __in PDEVICE_OBJECT DeviceObject)
/*
Routine Description:
    This routine completes an Mdl write.
Arguments:
...
NTKERNELAPI NTSTATUS FsRtlRegisterFileSystemFilterCallbacks(__in PDRIVER_OBJECT FilterDriverObject, __in PFS_FILTER_CALLBACKS Callbacks)
/*
Routine Description:
    This routine registers the FilterDriverObject to receive the notifications specified in Callbacks at the appropriate times for the devices to which this driver is attached.

    This should only be called by a file system filter during its DriverEntry routine.
Arguments:
    ...
    ResourceToRelease - Returns the resource to release.  Not defined if FALSE is returned.
Return Value:
    STATUS_SUCCESS - The callbacks were successfully registered for this driver.
    STATUS_INSUFFICIENT_RESOURCES - There wasn't enough memory to store these callbacks for the driver.
    STATUS_INVALID_PARAMETER - Returned in any of the parameters are invalid.
*/
{
    ...
    FsFilterCallbacks = ExAllocatePoolWithTag(NonPagedPool, Callbacks->SizeOfFsFilterCallbacks, FSRTL_FILTER_MEMORY_TAG);
...
Routine Description:
    ...
    We pass back the resource Mm has to release when the write completes.

    This routine is obsolete --- should call FsRtlAcquireFileForModWriteEx instead.
Arguments:
    FileObject - Pointer to the file object being written.
    EndingOffset - The offset of the last byte being written + 1.
    ByteCount - Length of data in bytes.
...
*/
{
    ...
    //  Just call the new version of this routine and process the NTSTATUS returned into TRUE for success and FALSE for failure.
...
Routine Description:
    ...
    The operation is presented to any file system filters attached to this volume before and after the file system is asked to acquire this resource.
Arguments:
    FileObject - Pointer to the file object being written.
    EndingOffset - The offset of the last byte being written + 1.
    ByteCount - Length of data in bytes.
...
*/
{
    ...
    //  There are cases when the device that is the base fs device for this file object will register for the FsFilter callbacks instead of the legacy FastIo interfaces (DFS does this).
    //  It then can redirect these operations to another stack that could possibly have file system filter drivers correctly.
    DeviceObject = IoGetRelatedDeviceObject(FileObject);
    ...
    //  If a device provides support for both interfaces, we will only use the FsFilterCallback interface.
    ...
    if (DeviceObject == BaseFsDeviceObject && !BaseFsGetsFsFilterCallbacks) {
        //  There are no filters attached to this device and the base file system does not want these callbacks.
        //  This quick check allows us to bypass the logic to see if any filters are interested.
        CallFilters = NULL;
    }

    if (CallFilters) {
        //  Call routine to initialize the control structure.
        Status = FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_ACQUIRE_FOR_MOD_WRITE, DeviceObject, BaseFsDeviceObject, FileObject, TRUE);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
    ...
    if (Status == STATUS_FSFILTER_OP_COMPLETED_SUCCESSFULLY) {
        //  The filter/file system completed the operation, therefore we just need to call the completion callbacks for this operation.
        //  There is no need to try to call the base file system.
        Status = STATUS_SUCCESS;
    } else if (NT_SUCCESS(Status)) {
        if (CallFilters) {
            FileObject = FsFilterCtrl.Data.FileObject;// Always copy out file object
        ...
        if (!(VALID_FS_FILTER_CALLBACK_HANDLER(FsFilterCallbacks, PreAcquireForModifiedPageWriter) ||
            ...
            if (VALID_FAST_IO_DISPATCH_HANDLER(FastIoDispatch, AcquireForModWrite)) {
                Status = FastIoDispatch->AcquireForModWrite(FileObject, EndingOffset, ResourceToRelease, BaseFsDeviceObject);
            } else {
                Status = STATUS_INVALID_DEVICE_REQUEST;
            }

            BaseFsFailedOperation = TRUE;//  If there is a failure at this point, we know that the failure was caused by the base file system.
        }
    ...
    //  If the base file system didn't have an AcquireForModWrite handler or couldn't return STATUS_SUCCESS or STATUS_CANT_WAIT, we need to perform the default actions here.
    if ((Status != STATUS_SUCCESS) && (Status != STATUS_CANT_WAIT) && BaseFsFailedOperation) {
        //  We follow the following rules to determine which resource to acquire.
        //  We use the flags in the common header.
        //  These flags can't change once we have acquired any resource.
        //  This means we can do an unsafe test and optimistically acquire a resource.
        //  At that point we can test the bits to see if we have what we want.

        //  0 - If there is no main resource, acquire nothing.
        //  1 - Acquire the main resource exclusively if the ACQUIRE_MAIN_RSRC_EX flag is set or we are extending valid data.
        //  2 - Acquire the main resource shared if there is no paging io resource or the ACQUIRE_MAIN_RSRC_SH flag is set.
        ...
        if (FlagOn(Header->Flags, FSRTL_FLAG_ACQUIRE_MAIN_RSRC_EX) ||
            (EndingOffset->QuadPart > Header->ValidDataLength.QuadPart && Header->ValidDataLength.QuadPart != Header->FileSize.QuadPart)) {
        ...
        //  Perform the following in a loop in case we need to back and check the state of the resource acquisition.
        //  In most cases the initial checks will succeed and we can proceed immediately.
        //  We have to worry about the two FsRtl bits changing but if there is no paging io resource before there won't ever be one.
        while (TRUE) {
            ...
            //  set and we don't have the main resource exclusive then release the current resource and acquire the main resource exclusively and move to the top of the loop.

            //  We must get it exclusive in all cases where the ending offset is beyond vdl.
            //  It used to be allowed shared if vdl == fs, but this neglected the possibility that the file could be extended under our shared (pagingio) access.
            if (FlagOn(Header->Flags, FSRTL_FLAG_ACQUIRE_MAIN_RSRC_EX) || EndingOffset->QuadPart > Header->ValidDataLength.QuadPart) {
                //  If we don't have the main resource exclusively then release the current resource and attempt to acquire the main resource exclusively.
                if (!AcquireExclusive) {
            ...
            //  If we should be acquiring the main resource shared then move to acquire the correct resource and proceed to the top of the loop.
            } else if (FlagOn(Header->Flags, FSRTL_FLAG_ACQUIRE_MAIN_RSRC_SH)) {
                //  If we have the main resource exclusively then downgrade to shared and exit the loop.
                if (AcquireExclusive) {
                    ExConvertExclusiveToSharedLite(ResourceAcquired);

                    //  If we have the paging io resource then give up this resource and acquire the main resource exclusively.
                    //  This is going at it with a large hammer but is guaranteed to be resolved in the next pass through the loop.
            ...
            //  At this point we should have the paging Io resource shared if it exists.
            //  If not then acquire it shared and release the other resource and exit the loop.
            ...
            //  We should have the main resource shared.  If we don't then degrade our lock to shared access.
            } else if (AcquireExclusive) {
                ExConvertExclusiveToSharedLite(ResourceAcquired);
                //  We now have the correct resource.  Exit the loop.
            }

            break;//  We have the correct resource.  Exit the loop.
        }
...
FsRtlAcquireFileForModWrite_CallCompletionCallbacks:
    ...
    //  Again, we only want to call try to do completion callbacks if there are any filters attached to this device that have completion callbacks.
    //  In any case, if we called down to the filters we need to free the FsFilterCtrl.
    if (CallFilters) {
...
/*
Routine Description:
    This routine releases a file system resource previously acquired for the modified page writer.
Arguments:
    FileObject - Pointer to the file object being written.
    ResourceToRelease - Supplies the resource to release.  Not defined if FALSE is returned.
*/
{
    ...
    if (CallFilters) {
        FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_RELEASE_FOR_MOD_WRITE, DeviceObject, BaseFsDeviceObject, FileObject, FALSE);
    ...
    //  We expect STATUS_SUCCESS to be returned if the operation was completely processed by the ReleaseForModWrite handler or STATUS_INVALID_DEVICE_REQUEST if the default processing is being requested.
    //  To protect against incorrect driver implementation, 
    //  we will do the default logic on any non-success status code if we think the base file system has failed the release operation.
    ASSERT((Status == STATUS_SUCCESS) || (Status == STATUS_INVALID_DEVICE_REQUEST));

    //  If the base file system doesn't provide a handler for this operation or the handler couldn't release the lock, 
    //  perform the default action, which is releasing the ResourceToRelease.
    if (!NT_SUCCESS(Status) && BaseFsFailedOperation) {
        ExReleaseResourceLite(ResourceToRelease);
        Status = STATUS_SUCCESS;
    }

    //  Again, we only want to try to do completion callbacks if there are any filters attached to this device that have completion callbacks.
    //  In any case, if we called down to the filters we need to free the FsFilterCtrl.
    if (CallFilters) {
        if (FS_FILTER_HAVE_COMPLETIONS(CallFilters)) {
            FsFilterPerformCompletionCallbacks(&FsFilterCtrl, Status);
...
Routine Description:
    ...
    This routine is obsolete --- FsRtlAcquireFileForCcFlushEx should be used instead.
Arguments:
    FileObject - Pointer to the file object being written.
*/
{
    ...
    //  Just call the new version of this routine and ignore the return value.
    //  In the debug version, we will assert if we see a failure here to encourage people to call the FsRtlAcquireFileForCcFlushEx.
...
Routine Description:
    This routine acquires a file system resource prior to a call to CcFlush.
    This operation is presented to all the file system filters in the filter stack for this volume.
    If all filters success the operation, the base file system is requested to acquire the file system resource for CcFlush.
Arguments:
    FileObject - Pointer to the file object being written.
Return Value:
    None.
*/
{
    ...
    if (CallFilters) {
        //  Call routine to initialize the control structure.
        Status = FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_ACQUIRE_FOR_CC_FLUSH, DeviceObject, BaseFsDeviceObject, FileObject, TRUE);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }

        //  There are no operation specific parameters for this operation, so just perform the pre-callbacks.
        FsRtlEnterFileSystem();

        Status = FsFilterPerformCallbacks(&FsFilterCtrl, TRUE, TRUE, &BaseFsFailedOperation);
    } else {
        //  We don't have any filters to call, but we still need to disable APCs.
        FsRtlEnterFileSystem();
    }
    ...
    if (Status == STATUS_FSFILTER_OP_COMPLETED_SUCCESSFULLY) {
        ...
        if (!(VALID_FS_FILTER_CALLBACK_HANDLER(FsFilterCallbacks, PreAcquireForCcFlush) ||
            ...
            //  If there is a failure at this point, we know that the failure was caused by the base file system.
    ...
    //  If the file system doesn't have a dispatch handler or failed this this operation, try to acquire the appropriate resources ourself.
    if (Status == STATUS_INVALID_DEVICE_REQUEST && BaseFsFailedOperation) {
        PFSRTL_COMMON_FCB_HEADER Header = FileObject->FsContext;

        //  If not already owned get the main resource exclusive because we may extend ValidDataLength.
        //  Otherwise acquire it one more time recursively.
        if (Header->Resource != NULL) {
    ...
    //  If this lock was not successfully acquired, then the lock will not need to be released.
    //  Therefore, we need to call FsRtlExitFileSystem now.
    if (!NT_SUCCESS(Status)) {
        FsRtlExitFileSystem();
    }
...
/*
Routine Description:
    This routine releases a file system resource previously acquired for the CcFlush.
Arguments:
    FileObject - Pointer to the file object being written.
*/
{
    ...
    if (CallFilters) {
        FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_RELEASE_FOR_CC_FLUSH, DeviceObject, BaseFsDeviceObject, FileObject, FALSE);

        //  There are no operation-specific parameters to initialize, so perform the pre-operation callbacks.
        Status = FsFilterPerformCallbacks(&FsFilterCtrl, FALSE, TRUE, &BaseFsFailedOperation);
    }
    ...
    if (Status == STATUS_INVALID_DEVICE_REQUEST && BaseFsFailedOperation) {
        ...
        //  The base file system doesn't provide a handler for this operation, so perform the default actions.
...
NTKERNELAPI VOID FsRtlAcquireFileExclusive(__in PFILE_OBJECT FileObject)
/*
Routine Description:
    This routine is used by NtCreateSection to pre-acquire file system resources in order to avoid deadlocks.
    If there is a FastIo entry for AcquireFileForNtCreateSection then that routine will be called.
    Otherwise, we will simply acquire the main file resource exclusive.
    If there is no main resource then we acquire nothing and return FALSE.
    In the cases that we acquire a resource,
    we also set the TopLevelIrp field in the thread local storage to indicate to file systems beneath us that we have acquired file system resources.
Arguments:
    FileObject - Pointer to the file object being written.
*/
{
    ...
    //  Just call the common version of this function, FsRtlAcquireFileExclusiveCommon.
    Status = FsRtlAcquireFileExclusiveCommon(FileObject, SyncTypeOther, 0);

    //  This should always be STATUS_SUCCESS since we are not allowing failures and the file system cannot fail this operation...
    ASSERT(NT_SUCCESS(Status));
}


NTKERNELAPI NTSTATUS FsRtlAcquireToCreateMappedSection(IN PFILE_OBJECT FileObject, IN ULONG SectionPageProtection)
/*
Routine Description:
    This routine is meant to replace FsRtlAcquireFileExclusive for the memory manager.
    Mm calls this routine to synchronize for a mapped section create, but filters are allowed to fail this operation.
    Other components that want to synchronize with section creation should call FsRtlAcquireFileExclusive.

    This routine calls FsRtlAcquireFileExclusiveCommon to do all the work.
...
Arguments:
    FileObject - Pointer to the file object being written.
    SectionPageProtection - The access requested for the section being created.
Return Value:
...
NTKERNELAPI NTSTATUS FsRtlAcquireFileExclusiveCommon(IN PFILE_OBJECT FileObject, IN FS_FILTER_SECTION_SYNC_TYPE SyncType, IN ULONG SectionPageProtection)
/*
Routine Description:
    This routine is used to pre-acquire file system resources in order to avoid deadlocks.
    The file system filters for this volume will be notified about this operation, then,
    if there is a FastIo entry for AcquireFileForNtCreateSection, that routine will be called.
    Otherwise, we will simply acquire the main file resource exclusive.
    If there is no main resource then we acquire nothing and return STATUS_SUCCESS.
    Finally, the file system filters will be notified whether or not this resource has been acquired.
Arguments:
    FileObject - Pointer to the file object being written.
    CreatingMappedSection - TRUE if this lock is being acquired so that a mapped section can be created.
        Filters are allowed to fail this operation.
        FALSE otherwise.
Return Value:
    NONE
*/
{
    ...
    if (CallFilters) {
        //  Initialize operation specific parameters for this operation.
        ...
        //  Call routine to initialize the control structure.
        Status = FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_ACQUIRE_FOR_SECTION_SYNCHRONIZATION, DeviceObject, BaseFsDeviceObject, FileObject, AllowFilterToFailOperation);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }

        FsRtlEnterFileSystem();//  There are no operation specific parameters for this operation, so just perform the pre-callbacks.

        //  Note: If the filter is allowed to fail the operation, so is the base file system, 
        //  so we will just use that variable for both parameters to FsFilterPerformCallbacks.
        Status = FsFilterPerformCallbacks(&FsFilterCtrl, AllowFilterToFailOperation, AllowFilterToFailOperation, &BaseFsFailedOperation);
    } else {
        //  We don't have any filters to call, but we still need to disable APCs.
        FsRtlEnterFileSystem();
    }
    ...
    if (Status == STATUS_FSFILTER_OP_COMPLETED_SUCCESSFULLY) {
        ...
        if (!(VALID_FS_FILTER_CALLBACK_HANDLER(FsFilterCallbacks, PreAcquireForSectionSynchronization) ||
            ...
            if (VALID_FAST_IO_DISPATCH_HANDLER(FastIoDispatch, AcquireFileForNtCreateSection)) {
                ...
                //  The status should already be STATUS_SUCCESS if we come down this path.
                //  Since the FastIo handler doesn't return a value the status should remain STATUS_SUCCESS.
    ...
    if (Status == STATUS_INVALID_DEVICE_REQUEST && BaseFsFailedOperation) {
        ...
        //  The file system doesn't have a dispatch handler for this operation, so try to acquire the appropriate resources ourself.
...
NTKERNELAPI VOID FsRtlReleaseFile(__in PFILE_OBJECT FileObject)
/*
Routine Description:
...
*/
{
    ...
    if (VALID_FS_FILTER_CALLBACK_HANDLER(FsFilterCallbacks, PreReleaseForSectionSynchronization) || VALID_FS_FILTER_CALLBACK_HANDLER(FsFilterCallbacks, PostReleaseForSectionSynchronization)) {
        BaseFsGetsFsFilterCallbacks = TRUE;
    }
    ...
    if (CallFilters) {
        FsFilterCtrlInit(&FsFilterCtrl, FS_FILTER_RELEASE_FOR_SECTION_SYNCHRONIZATION, DeviceObject, BaseFsDeviceObject, FileObject, FALSE);
...
NTSTATUS FsRtlGetFileSize(__in PFILE_OBJECT FileObject, __inout PLARGE_INTEGER FileSize)
/*
Routine Description:
    This routine is used to call the File System to get the FileSize for a file.
    It does this without acquiring the file object lock on synchronous file objects.
    This routine is therefore safe to call if you already own file system resources, while IoQueryFileInformation could (and does) lead to deadlocks.
Arguments:
    FileObject - The file to query
    FileSize - Receives the file size.
Return Value:
    NTSTATUS - The final I/O status of the operation.  If the FileObject refers to a directory, STATUS_FILE_IS_A_DIRECTORY is returned.
*/
{
    IO_STATUS_BLOCK IoStatus;
    PDEVICE_OBJECT DeviceObject;
    ...
    FastIoDispatch = DeviceObject->DriverObject->FastIoDispatch;
    if (FastIoDispatch && FastIoDispatch->FastIoQueryStandardInfo && FastIoDispatch->FastIoQueryStandardInfo(FileObject, TRUE, &FileInformation, &IoStatus, DeviceObject)) {
        //  Cool, it worked.
    } else {
        ...
        KeInitializeEvent(&Event, NotificationEvent, FALSE);//  Initialize the event.
        ...
        //  Disable hard errors over this call. Caller owns resources, is in a critical region and cannot complete hard error APCs.
        ...
        Status = IoCallDriver(DeviceObject, Irp);//  Queue the packet to the appropriate driver based.  This routine should not raise.

        //  If pending is returned (which is a successful status), we must wait for the request to complete.
        if (Status == STATUS_PENDING) {
            KeWaitForSingleObject(&Event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
        }

        //  If we got an error back in Status, then the Iosb was not written, so we will just copy the status there, then test the final status after that.
        if (!NT_SUCCESS(Status)) {
            IoStatus.Status = Status;
        }

        IoSetThreadHardErrorMode(HardErrorState);//  Reset the hard error state.
    }

    //  If the call worked, check to make sure it wasn't a directory and if not, fill in the FileSize parameter.
    if (NT_SUCCESS(IoStatus.Status)) {
        if (FileInformation.Directory) {
            IoStatus.Status = STATUS_FILE_IS_A_DIRECTORY;// Can't get file size for a directory. Return error.
        } else {
...
/*
Routine Description:
    This routine is used to call the File System to update FileSize for a file.

    It does this without acquiring the file object lock on synchronous file objects.
    This routine is therefore safe to call if you already own file system resources, while IoSetInformation could (and does) lead to deadlocks.
Arguments:
...
*/
{
    ...
    Buffer.EndOfFile = *FileSize;//  Copy FileSize to our buffer.
    KeInitializeEvent(&Event, NotificationEvent, FALSE);//  Initialize the event.
    ...
    //  Get a pointer to the first stack location in the packet.  This location will be used to pass the function codes and parameters to the first driver.
    IrpSp = IoGetNextIrpStackLocation(Irp);
    ...
    //  Queue the packet to the appropriate driver based on whether or not there is a VPB associated with the device.  This routine should not raise.
    Status = IoCallDriver(DeviceObject, Irp);
    if (Status == STATUS_PENDING) {//  If pending is returned (which is a successful status), we must wait for the request to complete.
        KeWaitForSingleObject(&Event, Executive, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
    }
