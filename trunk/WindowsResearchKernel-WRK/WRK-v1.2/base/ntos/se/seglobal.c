...
Abstract:
   This module contains the global variables used and exported by the security component.
*/
...
// security descriptor with a SACL to be used for adding a SACL on system processes
...
// Some variables we are going to use to help speed up access checking.
...
// Boolean to hold whether or not the user wants the system to crash when an audit fails.
...
*/
{
    ...
    // Fail initialization if we didn't get enough memory for the universal SIDs.
    if ((SeNullSid == NULL) ||
        (SeWorldSid == NULL) ||
        (SeLocalSid == NULL) ||
    ...
    *(RtlSubAuthoritySid(SeAliasBackupOpsSid, 1)) = DOMAIN_ALIAS_RID_BACKUP_OPS;
    ...
    SeImpersonatePrivilege = RtlConvertLongToLuid(SE_IMPERSONATE_PRIVILEGE);
    SeCreateGlobalPrivilege = RtlConvertLongToLuid(SE_CREATE_GLOBAL_PRIVILEGE);

    // Initialize the SeExports structure for exporting all of the information we've created out of the kernel.
    ...
    // Initialize frequently used privilege sets to speed up access validation.
...
Routine Description:
    ...
    that is used by SepAddSaclToProcess to add SACL to the existing security descriptor on a system process.

    A system process is defined as the one whose token has at least one of the following sids.
    -- SeLocalSystemSid
    -- SeLocalServiceSid
    -- SeNetworkServiceSid
...
#define PROCESS_ACCESSES_TO_AUDIT ( PROCESS_CREATE_THREAD   |\
    ...
    Sd2 = (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag(NonPagedPool, TotalSdLength, 'cAeS');
    if (Sd2 == NULL) {
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto Cleanup;
    }
...
/*
Routine Description:
    This function initializes the system's default dacls & security descriptors.
Arguments:
    None.
Return Value:
    None.
...
{
    NTSTATUS Status;
    ULONG PublicLength, PublicUnrestrictedLength, SystemLength, PublicOpenLength, LocalServiceLength;
    ...
    SystemLength = (ULONG)sizeof(ACL) + (2 * ((ULONG)sizeof(ACCESS_ALLOWED_ACE))) + SeLengthSid(SeLocalSystemSid) + SeLengthSid(SeAliasAdminsSid);
    ...
    Status = RtlAddAccessAllowedAce(SeLocalServicePublicDacl, ACL_REVISION2, GENERIC_ALL, SeLocalSystemSid);
    ASSERT(NT_SUCCESS(Status));
    ...
    // Local Service
    Status = RtlAddAccessAllowedAce(SeLocalServicePublicDacl, ACL_REVISION2, GENERIC_ALL, SeLocalServiceSid);
    ASSERT(NT_SUCCESS(Status));
...
/*
Routine Description:
    This routine is called once during system initialization to pre-allocate and initialize some commonly used privilege sets.
Arguments:
    None
Return Value:
    None.
...
Arguments:
    ...
    SystemSecurity - Provides a boolean indicating whether to put SeSecurityPrivilege into the output privilege set.
    WriteOwner - Provides a boolean indicating whether to put SeTakeOwnershipPrivilege into the output privilege set.
    Privileges - Supplies a pointer that will return the privilege set.  Should be freed with ExFreePool when no longer needed.
Return Value:
    None.
*/
{
...
/*
Routine Description:
    Initializes the mutex and list head used to queue work from the Executive to LSA.
    This mechanism operates on top of the normal ExWorkerThread mechanism by capturing the first thread to perform LSA work and keeping it until all the current work is done.
