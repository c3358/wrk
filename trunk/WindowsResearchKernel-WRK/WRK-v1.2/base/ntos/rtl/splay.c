...
Routine Description:
    ...
    the tree.  The caller's pointer is updated upon return.  If the tree is now empty, the value is NULL.
...
*/
{
    ...
    if ((RtlLeftChild(Links) != NULL) && (RtlRightChild(Links) != NULL)) {
        ...
        Predecessor = RtlSubtreePredecessor(Links);
        if (RtlIsRoot(Links)) {
            //  If we're switching with the root of the tree, fix the caller's root pointer
            *Root = Predecessor;
        }
...
Routine Description:
    ...
    the subtree rooted at the input node.  If there is not a successor, the return value is NULL.
Arguments:
    Links - Supplies a pointer to a splay link in a tree.
Return Value:
...
Routine Description:
    ...
    the subtree rooted at the input node.  If there is not a predecessor, the return value is NULL.
Arguments:
    Links - Supplies a pointer to a splay link in a tree.
Return Value:
