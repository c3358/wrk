...
*/
{
    ...
    *pdwSpecialChar = 0;
    ...
    quick_copy:
        ...
        // The ACP is a multibyte code page.
        // Check each character to see if it is a lead byte before doing the translation.
        while (MaxCharsInUnicodeString && BytesInMultiByteString) {
            MaxCharsInUnicodeString--;
            BytesInMultiByteString--;
            if (NlsLeadByteInfo[*(PUCHAR)MultiByteString]) {
                // Lead byte - Make sure there is a trail byte.
                // If not, pass back a space rather than an error.
                // Some 3.x applications pass incorrect strings and don't expect to get an error.
                ...
                *UnicodeString = (WCHAR)TranslateTable[Entry + *(PUCHAR)MultiByteString++];
                ...
                // Decrement count of bytes in multibyte string to account for the double byte character.
                BytesInMultiByteString--;
            } else {
...
bad_case:
    // this is a low probability case, so we optimized the loop.
    // If have a special char, finish trans and notify caller.
    *pdwSpecialChar = 1;
        ...
        while (MaxCharsInUnicodeString && BytesInMultiByteString) {
            ...
            if (NlsLeadByteInfo[*(PUCHAR)MultiByteString]) {
                ...
                *UnicodeString = (WCHAR)TranslateTable[Entry + *(PUCHAR)MultiByteString++];
                ...
                BytesInMultiByteString--;// Decrement count of bytes in multibyte string to account for the double byte character.
...
Routine Description:
    ...
    NOTE: This routine only supports precomposed Unicode characters.
Arguments:
    UnicodeString - Returns a unicode string that is equivalent to the oem source string.
    MaxBytesInUnicodeString - Supplies the maximum number of bytes to be written to UnicodeString.
        If this causes UnicodeString to be a truncated equivalent of OemString, no error condition results.
    BytesInUnicodeString - Returns the number of bytes in the returned unicode string pointed to by UnicodeString.
    OemString - Supplies the oem source string that is to be converted to unicode.
    BytesInOemString - The number of bytes in the string pointed to by OemString.
Return Value:
    SUCCESS - The conversion was successful
...
*/
{
    ...
    // The OCP is a multibyte code page.
    // Check each character to see if it is a lead byte before doing the translation.
    MaxCharsInUnicodeString = MaxBytesInUnicodeString / sizeof(WCHAR);
        ...
        while (MaxCharsInUnicodeString && BytesInOemString) {
            ...
            if (NlsOemLeadByteInfo[*(PUCHAR)OemString]) {
                ...
                *UnicodeString = TranslateTable[Entry + *(PUCHAR)OemString++];
                ...
                BytesInOemString--;// Decrement count of bytes in oem string to account for the double byte character.
            } else {
                *UnicodeString++ = NlsOemToUnicodeData[*(PUCHAR)OemString++];// Single byte character.
            }
        }
...
Arguments:
    BytesInUnicodeString - Returns the number of bytes a Unicode translation of the ANSI string pointed to by MultiByteString would contain.
    MultiByteString - Supplies the ansi source string whose Unicode length is to be calculated.
    BytesInMultiByteString - The number of bytes in the string pointed to by MultiByteString.
Return Value:
    SUCCESS - The conversion was successful
*/
    ...
    if (NlsMbCodePageTag) {
        ...
        while (BytesInMultiByteString--) {
            if (NlsLeadByteInfo[*(PUCHAR)MultiByteString++]) {
                ...
                // NOTE: make sure we have a trail byte to convert.
                if (BytesInMultiByteString == 0) {
                    // RtlMultibyteToUnicodeN() uses the unicode default character if the last multibyte character is a lead byte.
        ...
        *BytesInUnicodeString = BytesInMultiByteString * sizeof(WCHAR);// The ACP is a single byte code page.
    }
...
Arguments:
    BytesInMultiByteString - Returns the number of bytes an ANSI translation of the Unicode string pointed to by UnicodeString would contain.
    UnicodeString - Supplies the unicode source string whose ANSI length is to be calculated.
    BytesInUnicodeString - The number of bytes in the the string pointed to by UnicodeString.
Return Value:
    SUCCESS - The conversion was successful
    !SUCCESS - The conversion failed.  A unicode character was encountered that has no translation for the current ANSI Code Page (ACP).
*/
{
    ULONG cbMultiByte = 0;
...
*/
{
    ...
    if (!NlsMbCodePageTag) {
        ...
        do {
            // Convert to ANSI and back to Unicode before upper casing to ensure the visual best fits are converted and upper cased properly.
            switch (TmpCount) {
            default:
                UnicodeString += 0x10;
                MultiByteString += 0x10;
...
/*
Routine Description:
    This function looks for the default character in the Oem string, making sure it was not a correct translation from the Unicode source string.
...
Return Value:
    TRUE if the Unicode to Oem translation caused no default characters to be inserted.  FALSE otherwise.
*/
{
    ULONG OemOffset;
    BOOLEAN Result = TRUE;
...
*/
{
    ...
    quick_copy:
        ...
        // The CP is a multibyte code page.
        // Check each character to see if it is a lead byte before doing the translation.
        ...
        while (MaxCharsInUnicodeString && BytesInCustomCPString) {
            ...
            if (NlsCustomLeadByteInfo[*(PUCHAR)CustomCPString]) {
                ...
                *UnicodeString = TranslateTable[Entry + *(PUCHAR)CustomCPString++];
                ...
                BytesInCustomCPString--;// Decrement count of bytes in multibyte string to account for the double byte character.
            } else {// Single byte character.
...
*/
{
    ...
    // Convert Unicode byte count to character count. Byte count of multibyte string is equivalent to character count.
    if (!(CustomCP->DBCSCodePage)) {
...
*/
{
    ...
    if (!(CustomCP->DBCSCodePage)) {
        ...
        do {
            // Convert to Single Byte and back to Unicode before upper casing to ensure the visual best fits are converted and upper cased properly.
            switch (TmpCount) {
            default:
                UnicodeString += 0x10;
                CustomCPString += 0x10;
