...
Abstract:
    ...
    It is called by both the user mode Win32 Atom API functions (Local/GlobalxxxAtom) and by the kernel mode window manager code to access global atoms.
*/
...
PVOID RtlpAllocateAtom(IN ULONG NumberOfBytes, IN ULONG Tag)
{
    ...
    AllocatedBlock = ExAllocatePoolWithTag(PagedPool, NumberOfBytes, Tag);
    if (AllocatedBlock) {
        AllocatedBlock->QuotaBlock = PsChargeSharedPoolQuota(PsGetCurrentProcess(), NumberOfBytes, 0);
...
void RtlpFreeAtom(IN PVOID p)
{
    ...
    PsReturnSharedPoolQuota(AllocatedBlock->QuotaBlock, AllocatedBlock->PagedAmount, 0);
...
NTSTATUS RtlAddAtomToAtomTable(__in PVOID AtomTableHandle, __in PWSTR AtomName, __inout_opt PRTL_ATOM Atom)
{
    NTSTATUS Status;
    PRTL_ATOM_TABLE p = (PRTL_ATOM_TABLE)AtomTableHandle;
        ...
        if (RtlpGetIntegerAtom(AtomName, &Temp)) {
            ...
            if (*AtomName == UNICODE_NULL) {
                ...
                if (a == NULL) {
                    if (pa != NULL) {
                        Status = STATUS_NO_MEMORY;
                        a = RtlpAllocateAtom(FIELD_OFFSET(RTL_ATOM_TABLE_ENTRY, Name) + NameLength + sizeof(UNICODE_NULL), 'AmtA');
                        if (a != NULL) {
                            a->HashLink = NULL;
                            a->ReferenceCount = 1;
                            a->Flags = 0;
...
NTSTATUS RtlLookupAtomInAtomTable(__in PVOID AtomTableHandle, __in PWSTR AtomName, __out_opt PRTL_ATOM Atom)
{
    NTSTATUS Status;
    PRTL_ATOM_TABLE p = (PRTL_ATOM_TABLE)AtomTableHandle;
    PRTL_ATOM_TABLE_ENTRY a;
        ...
        if (Atom < RTL_ATOM_MAXIMUM_INTEGER_ATOM) {
            if (Atom == RTL_ATOM_INVALID_ATOM) {
                ...
                if (ARGUMENT_PRESENT(AtomName)) {
                    CopyLength = _snwprintf(AtomNameBuffer, sizeof(AtomNameBuffer) / sizeof(WCHAR), L"#%u", Atom) * sizeof(WCHAR);
            ...
            if (a != NULL && a->Atom == Atom) {
                ...
                if (ARGUMENT_PRESENT(AtomName)) {
                    // Fill in as much of the atom string as possible, and always zero terminate. This is what win3.1 does.
