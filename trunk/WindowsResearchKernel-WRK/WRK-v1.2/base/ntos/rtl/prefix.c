...
Abstract:
    This module implements the prefix table utility.
    The two structures used in a prefix table are the PREFIX_TABLE and PREFIX_TABLE_ENTRY.
    Each table has one prefix table and multiple prefix table entries corresponding to each prefix stored in the table.
    ...
    while the other entries are in a circular linked list joined with the tree member.
*/
...
*/
{
    ...
    //  If the name length of the current tree is not equal to the prefix name length then the tree does not exist and we need to make a new tree node.
    if (CurrentTree->NameLength != (CSHORT)PrefixNameLength) {
        //  Insert the new prefix entry to the list between previous and current tree
        PreviousTree->NextPrefixTree = PrefixTableEntry;
        PrefixTableEntry->NextPrefixTree = CurrentTree;
        PrefixTableEntry->NodeTypeCode = RTL_NTC_ROOT;//  And set the node type code
        return TRUE;//  And tell our caller everything worked fine
    }

    //  The tree does exist so now search the tree for our position in it.
    //  We only exit the loop if we've inserted a new node, and node is left is left pointing to the tree position
    Node = CurrentTree;
    while (TRUE) {
        //  Compare the prefix in the tree with the prefix we want to insert
        Comparison = CompareNamesCaseSensitive(Node->Prefix, Prefix);

        //  If we do match case sensitive then we cannot add this prefix so we return false.
        //  Note this is the only condition where we return false
        if (Comparison == IsEqual) {
            return FALSE;
        }

        //  If the tree prefix is greater than the new prefix then we go down the left subtree
        if (Comparison == IsGreaterThan) {
            //  We want to go down the left subtree, first check to see if we have a left subtree
            if (RtlLeftChild(&Node->Links) == NULL) {
                //  there isn't a left child so we insert ourselves as the new left child
                ...
                //  there is a left child so simply go down that path, and go back to the top of the loop
            ...
            if (RtlRightChild(&Node->Links) == NULL) {
                ...
                //  there is a right child so simply go down that path, and go back to the top of the loop
    ...
    //  Set the new root's node type code and make it part of the root tree list
    Node->NodeTypeCode = RTL_NTC_ROOT;
    PreviousTree->NextPrefixTree = Node;
    Node->NextPrefixTree = NextTree;

    return TRUE;//  tell our caller everything worked fine
}
...
*/
{
    ...
    case RTL_NTC_ROOT:
        //  The node is internal or root node so we need to delete it from the tree, but first find the root of the tree
        ...
        if (Links == NULL) {
            ...
            //  We've located the previous tree so now just have it point around the deleted node
            PreviousTree->NextPrefixTree = Root->NextPrefixTree;
            return;//  and return the our caller
        }
        ...
        //  We didn't change roots so everything is fine and we can simply return to our caller
        return;
    default:
        //  If we get here then there was an error and the node type code is unknown
        return;
    }
}
...
*/
{
    ...
    //  Now search for a prefix until we find one or until we exhaust the prefix trees
    while (CurrentTree->NameLength > 0) {
        ...
        while (Links != NULL) {
            ...
            if (Comparison == IsGreaterThan) {
                //  The prefix is greater than the full name so we go down the left child
                Links = RtlLeftChild(Links);

                //  And continue searching down this tree
            } else if (Comparison == IsLessThan) {
                //  The prefix is less than the full name so we go down the right child
    ...
    return NULL;//  We sesarched everywhere and didn't find a prefix so tell the caller none was found
}
...
Routine Description:
    ...
    To handle ill-formed names (i.e., names that do not contain a backslash) this routine really returns the number of backslashes plus 1.
Arguments:
...
*/
{
    ...
    //  Save the name length, this should make the compiler be able to optimize not having to reload the length each time
    ...
    if (NlsMbCodePageTag) {
        ...
        for (i = 0, Count = 1; i < NameLength; i += 1) {
            if (Name->Buffer[i] == '\\') {//  check for a back slash
                Count += 1;
            }
        }
    }

    return Count;//  return the number of back slashes we found
}
...
*/
{
    ...
    //  Loop through looking at all of the characters in both strings testing for equality, less than, and greater than
    ...
    //  They match up to the minimum length so now figure out the largest string and see if one is a proper prefix of the other
    if (PrefixLength < NameLength) {
        //  The prefix string is shorter so if it is a proper prefix we return prefix otherwise we return less than (e.g., "\a" < "\ab")
        ...
        //  The Prefix string is longer so we say that the prefix is greater than the name (e.g., "\ab" > "\a")
        return IsGreaterThan;
    } else {
        return IsEqual;//  They lengths are equal so the strings are equal
    }
}
...
*/
{
    ...
    Node = CurrentTree;
    while (TRUE) {
        //  Compare the prefix in the tree with the prefix we want to insert.  Do the compare case blind
        Comparison = CompareUnicodeStrings(Node->Prefix, Prefix, 0);

        //  If they are equal then this node gets added as a case match, provided it doesn't case sensitive match anyone
        if (Comparison == IsEqual) {
            PUNICODE_PREFIX_TABLE_ENTRY Next;

            //  Loop through the case match list checking to see if we match case sensitive with anyone.  Get the first node
            Next = Node;

            //  And loop checking each node until we're back to where we started
            do {
                //  If we do match case sensitive then we cannot add this prefix so we return false.
                //  Note this is the only condition where we return false
                ...
                Next = Next->CaseMatch;//  Get the next node in the case match list

                //  And continue looping until we're back where we started
            } while (Next != Node);

            //  We've searched the case match and didn't find an exact match so we can insert this node in the case match list
            ...
            break;//  And exit out of the while loop
        }
        ...
        if (Comparison == IsGreaterThan) {
            ...
            if (RtlRightChild(&Node->Links) == NULL) {
                ...
                RtlInsertAsRightChild(&Node->Links, &PrefixTableEntry->Links);
                break;//  and exit the while loop
            } else {//  there is a right child so simply go down that path, and go back to the top of the loop
...
*/
{
    ...
    case RTL_NTC_UNICODE_CASE_MATCH:
        //  The prefix entry is a case match record so we only need to remove it from the case match list.
        //  Locate the previous PrefixTableEntry that reference this case match record
        ...
        //  Now that we have the previous record just have it point around the case match that is being deleted
        PreviousCaseMatch->CaseMatch = PrefixTableEntry->CaseMatch;
        return;//  And return to our caller
    case RTL_NTC_UNICODE_INTERNAL:
    case RTL_NTC_UNICODE_ROOT:
        //  The prefix entry is an internal/root node so check to see if it has any case match nodes with it
        if (PrefixTableEntry->CaseMatch != PrefixTableEntry) {
            //  There is at least one case match that goes with this node, so we need to make the next case match the new node and remove this node.
            //  Locate the previous prefix table entry that references this case match record
            PreviousCaseMatch = PrefixTableEntry->CaseMatch;
            while (PreviousCaseMatch->CaseMatch != PrefixTableEntry) {
                PreviousCaseMatch = PreviousCaseMatch->CaseMatch;
            }

            //  Now that we have the previous record just have it point around the node being deleted
            PreviousCaseMatch->CaseMatch = PrefixTableEntry->CaseMatch;
            ...
            //  Now take care of the back pointers to this new internal node in the splay tree, first do the parent's pointer to us.
            if (RtlIsRoot(&PrefixTableEntry->Links)) {
                ...
                //  Fix up the root tree list, by first finding the previous pointer to us
                PreviousTree = PrefixTableEntry->NextPrefixTree;
                while (PreviousTree->NextPrefixTree != PrefixTableEntry) {
                    PreviousTree = PreviousTree->NextPrefixTree;
                }

                //  We've located the previous tree so now have the previous tree point to our new root
                PreviousTree->NextPrefixTree = PreviousCaseMatch;
            } else if (RtlIsLeftChild(&PrefixTableEntry->Links)) {
                RtlParent(&PrefixTableEntry->Links)->LeftChild = &PreviousCaseMatch->Links;//  The node was the left child so make the new node the left child
            } else {
                RtlParent(&PrefixTableEntry->Links)->RightChild = &PreviousCaseMatch->Links;//  The node was the right child so make the new node the right child
            }
        ...
        //  The node is internal or root node and does not have any case match nodes so we need to delete it from the tree, but first find the root of the tree
        Links = &PrefixTableEntry->Links;
        while (!RtlIsRoot(Links)) {
            Links = RtlParent(Links);
        }
        ...
        return;
    default:
        return;//  If we get here then there was an error and the node type code is unknown
    }
}
...
Arguments:
    PrefixTable - Supplies the prefix table to search
    FullString - Supplies the name to search for
    CaseInsensitiveIndex - Indicates the wchar index at which to do a case insensitive search.
        All characters before the index are searched case sensitive and all characters at and after the index are searched insensitive.
Return Value:
    PPREFIX_TABLE_ENTRY - a pointer to the longest prefix found if one exists, and NULL otherwise
*/
{
    ...
    while (CurrentTree->NameLength > 0) {
        ...
        while (Links != NULL) {
            ...
            //  Compare the prefix in the tree with the full name, do the compare case blind
            ...
            if (Comparison == IsGreaterThan) {
                ...
                //  We have either a prefix or a match either way we need to check if we should do case sensitive searches
                if (CaseInsensitiveIndex == 0) {
                    ...
                    return Node;//  Now return the root to our caller
                }

                //  The caller wants an exact match so search the case match until we find a complete match.  Get the first node
                Next = Node;

                //  Loop through the case match list checking to see if we match case sensitive with anyone.
                do {
                    //  If we do match case sensitive then we found one and we return it to our caller
                    Comparison = CompareUnicodeStrings(Next->Prefix, FullName, CaseInsensitiveIndex);
                    if ((Comparison == IsEqual) || (Comparison == IsPrefix)) {
                        return Next;//  We found a good one, so return it to our caller
                    }

                    Next = Next->CaseMatch;//  Get the next case match record

                    //  And continue the loop until we reach the original node again
                } while (Next != Node);
...
*/
{
    ...
    if (Restart || (PrefixTable->LastNextEntry == NULL)) {
        //  we are restarting the sequence so locate the first entry in the first tree
        ...
        if (Node->NodeTypeCode == RTL_NTC_UNICODE_PREFIX_TABLE) {
            return NULL;//  No we aren't so the table must be empty
        }
        ...
        Node = CONTAINING_RECORD(Links, UNICODE_PREFIX_TABLE_ENTRY, Links);//  Set it as our the node we're returning
    } else if (PrefixTable->LastNextEntry->CaseMatch->NodeTypeCode == RTL_NTC_UNICODE_CASE_MATCH) {
        Node = PrefixTable->LastNextEntry->CaseMatch;//  The last node has a case match that we should be returning this time around
    } else {
        //  Move over the last node returned by the case match link, this will enable us to finish off the last case match node if there was one, and go to the next internal/root node.
        //  If this node does not have a case match then we simply circle back to ourselves
        Node = PrefixTable->LastNextEntry->CaseMatch;

        Links = RtlRealSuccessor(&Node->Links);//  Find the successor for the last node we returned

        //  If links is null then we've exhausted this tree and need to the the next tree to use
        if (Links == NULL) {
            ...
            //  Now we've found the root see if there is another tree to enumerate
            Node = Node->NextPrefixTree;
            if (Node->NameLength <= 0) {
                return NULL;//  We've run out of tree so tell our caller there are no more
            }
    ...
    PrefixTable->LastNextEntry = Node;//  Save node as the last next entry
    return Node;//  And return this entry to our caller
}
...
*/
{
    ...
    for (i = 0, Count = 1; i < NameLength - 1; i += 1) {
        if (Name->Buffer[i] == UnicodeBackSlash) {//  check for a back slash
            Count += 1;
        }
    }
...
Arguments:
    ...
    Name - Supplies the full name input string
    CaseInsensitiveIndex - Indicates the wchar index at which to do a case insensitive search.
        All characters before the index are searched case sensitive and all characters at and after the index are searched
Return Value:
    COMPARISON - returns
        IsLessThan    if Prefix < Name lexicalgraphically,
        IsPrefix      if Prefix is a proper prefix of Name
...
*/
{
    ...
    MinLength = (PrefixLength < NameLength ? PrefixLength : NameLength);//  Figure out the minimum of the two lengths

    //  Loop through looking at all of the characters in both strings testing for equality.
    //  First to the CaseSensitive part, then the CaseInsensitive part.
    ...
    if (PrefixLength < NameLength) {
        ...
        return IsGreaterThan;//  The Prefix string is longer so we say that the prefix is greater than the name (e.g., "\ab" > "\a")
    } else {
        return IsEqual;//  They lengths are equal so the strings are equal
    }
}