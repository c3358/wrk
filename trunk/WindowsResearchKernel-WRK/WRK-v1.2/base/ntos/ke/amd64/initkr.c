...
Abstract:
    This module contains the code to initialize the kernel data structures and to initialize the idle thread,
    its process, the processor control block, and the processor control region.
*/

#include "ki.h"
#include <kddll.h>

KIRQL KiProfileIrql = PROFILE_LEVEL;// Define default profile IRQL level.
UCHAR KiCFlushSize = 0;// Define the system cache flush size.
...
ULONG KeLastBranchMSR = 0;// Define last branch control register MSR.
ULONG KiMxCsrMask = 0xFFBF;// Define the MxCsr mask.

// Define the prefetch retry flag. Each processor that requires prefetch retry will set bit 0 of prefetch retry.
// After all processors have been started, bit 7 of prefetch retry is cleared.
...
// KiInitializeIdtEntry (IN PKIDTENTRY64 Entry, IN PVOID Address, IN USHORT Level)
// Arguments:
...
ULONG KiFatalFilter(IN ULONG Code, IN PEXCEPTION_POINTERS Pointers);
VOID KiSetCacheInformation(VOID);
VOID KiSetCacheInformationAmd(VOID);
VOID KiSetCacheInformationIntel(VOID);
VOID KiSetCpuVendor(VOID);
VOID KiSetFeatureBits(IN PKPRCB Prcb);
...
VOID KeCompactServiceTable(IN PVOID Table, IN ULONG Limit, IN BOOLEAN Win32)
/*
Routine Description:
    This function compacts the specified system service table into an array of 32-bit displacements with the number of arguments encoded in the low bits of the relative address.
Arguments:
    Table - Supplies the address of a system service table.
    Limit - Supplies the number of entries in the system service table.
    Win32 - Supplies a boolean variable that signifies whether the system service table is the win32k table or the kernel table.
*/
{
    ULONG64 Base;
    ULONG Index;
    ...
    // Copy and compact the specified system servicde table into an array of relative addresses.
    Base = (ULONG64)Table;
    ...
    if (Win32 == TRUE) {// If the specified system service table in the Win32 table, then copy the status translation vector.
...
/*
Routine Description:
    This function gains control after the system has been booted, but before the system has been completely initialized.
    Its function is to initialize the kernel data structures, initialize the idle thread and process objects,
    complete the initialization of the processor control block (PRCB) and processor control region (PCR),
    call the executive initialization routine, then return to the system startup routine.
    This routine is also called to initialize the processor specific structures when a new processor is brought on line.
    N.B. Kernel initialization is called with interrupts disabled at IRQL HIGH_LEVEL and returns with with interrupts enabled at DISPATCH_LEVEL.
Arguments:
...
*/
{
    ...
    KiSetCpuVendor();// Set CPU vendor.
    KiSetProcessorType();// Set processor type.
    FeatureBits = Prcb->FeatureBits;// get the processor feature bits.

    // Retrieve MxCsr mask, if any.
    RtlZeroMemory(&XmmSaveArea, sizeof(XmmSaveArea));
    KeSaveLegacyFloatingPointState(&XmmSaveArea);

    // If this is the boot processor, then enable global pages, set the page attributes table, set machine check enable, set large page enable,
    // enable debug extensions, and set multithread information. Otherwise, propagate multithread information.

    // N.B. This only happens on the boot processor and at a time when there can be no coherency problem.
    //      On subsequent, processors this happens during the transistion into 64-bit mode which is also at a time that there can be no coherency problems.
    if (Number == 0) {
        ...
        KeFlushCurrentTb();// Flush the entire TB and enable global pages.

        // Set page attributes table and flush cache.
        KiSetPageAttributesTable();
        WritebackInvalidate();

        // Parse boot options to determine desired level of no execute protection.
        ...
        // If no execute protection is supported, then turn on no execute protection for memory management.
        // Otherwise, make sure the feature bits reflect that no execute is not supported no matter what boot options were specified.
        // N.B. No execute protection is always enabled during processor initialization if it is present on the respective processor.
        ...
        WriteCR4(ReadCR4() | CR4_DE | CR4_PSE);// Set debugger extension and large page enable.
        KeFlushCurrentTb();// Flush the entire TB.

        // Set the multithread processor set and the multithread set master for the boot processor, 
        // and set the number of logical processors per physical processor.
        ...
        KeCompactServiceTable(&KiServiceTable[0], KiServiceLimit, FALSE);// Compact the system service table.
    } else {
        if (KiLogicalProcessors == 1) {// If the system is not a multithread system, then initialize the multithread processor set and multithread set master.
            Prcb->MultiThreadProcessorSet = Prcb->SetMember;
            Prcb->MultiThreadSetMaster = Prcb;
            KiPhysicalProcessors += 1;
        } else {// Otherwise, propagate multithread set information.
    ...
    KiSetCacheInformation();// set processor cache size information.
    PoInitializePrcb(Prcb);// Initialize power state information.

    // If the initial processor is being initialized, then initialize the per system data structures. Otherwise, check for a valid system.
    if (Number == 0) {
        ...
        // Initialize the node block array with pointers to temporary node blocks to be used during initialization.
#if !defined(NT_UP)
        ...
        KiInitSystem();// Performance architecture independent initialization.
        ...
        KeInitializeProcess(Process, (KPRIORITY)0, (KAFFINITY)(-1), &DirectoryTableBase[0], TRUE);
        ...
        // If the CPU feature bits are not identical or the number of logical processors per physical processors are not identical, then bugcheck.
        if ((FeatureBits != (KeFeatureBits & ~(KF_GLOBAL_32BIT_NOEXECUTE | KF_GLOBAL_32BIT_EXECUTE))) ||
            (MxCsrMask != KiMxCsrMask) || (KiCFlushSize != Prcb->CFlushSize) || (KiLogicalProcessors != Prcb->LogicalProcessorsPerPhysicalProcessor)) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, (ULONG64)FeatureBits, (ULONG64)KeFeatureBits, 0, 0);
        }
    ...
    // If the initial processor is being initialized, then compute the timer table reciprocal value,
    // reset the PRCB values for the controllable DPC behavior in order to reflect any registry overrides, and initialize the global unwind history table.
    if (Number == 0) {
    ...
    // Raise IRQL to dispatch level, enable interrupts, and set the priority of the idle thread to zero.
    // This will have the effect of immediately causing the phase one initialization thread to get scheduled.
    // The idle thread priority is then set of the lowest realtime priority.
    ...
    LoaderBlock->Prcb = (ULONG64)NULL;// Signal that this processor has completed its initialization.
}
...
/*
Routine Description:
    This function initializes the boot structures for a processor.
    It is only called by the system start up code.
    Certain fields in the boot structures have already been initialized.
    In particular:
...
Arguments:
    LoaderBlock - Supplies a pointer to the loader block that has been initialized for this processor.
*/
{
    ULONG ApicMask;
    ULONG Bit;
    ...
    KiProcessorBlock[Number] = Prcb;// Initialize the processor block address.
    Prcb->DpcStack = (PVOID)LoaderBlock->KernelStack;// Initialize the PRCB address of the DPC stack.

    IdtBase = Pcr->IdtBase;
    if (Number == 0) {// If this is processor zero, then initialize the IDT according to the contents of interrupt initialization table.
        IntInitRec = KiInterruptInitTable;
        for (Index = 0; Index < MAXIMUM_IDTVECTOR; Index += 1) {
            // If the vector is found in the initialization table then set up the IDT entry accordingly and advance to the next entry in the initialization table.
            // Otherwise set the IDT to reference the unexpected interrupt handler.
            if (Index == IntInitRec->Vector) {
                KiInitializeIdtEntry(&IdtBase[Index], IntInitRec->Handler, IntInitRec->Dpl, IntInitRec->IstIndex);
                IntInitRec += 1;
            } else {
                ...
                JumpOffset = (LONG64)KiUnexpectedInterrupt - ((LONG64)UnexpectedInterrupt + RTL_SIZEOF_THROUGH_FIELD(UNEXPECTED_INTERRUPT, JmpOffset));
                UnexpectedInterrupt->JmpOffset = (LONG)JumpOffset;
                KiInitializeIdtEntry(&IdtBase[Index], UnexpectedInterrupt, 0, 0);
            }
        }
    } else {// Otherwise, initialize the secondary processor clock interrupt.
    ...
    // If this is processor zero, then set the number of logical processors per physical processor, the APIC mask, and the initial number of physical processors.
    if (Number == 0) {
    ...
    ExInitPoolLookasidePointers();// Initialize the PRCB temporary pool look aside pointers.
    HalInitializeProcessor(Number, LoaderBlock);// Initialize the HAL for this processor.
    KeActiveProcessors |= AFFINITY_MASK(Number);// Set the appropriate member in the active processors set.

    // Set the number of processors based on the maximum of the current number of processors and the current processor number.
    if ((Number + 1) > KeNumberProcessors) {
        KeNumberProcessors = Number + 1;
    }

    KiSaveInitialProcessorControlState(&Prcb->ProcessorState);// Initialize the processor control state in the PRCB.
}
...
/*
Routine Description:
    This function is executed if an unhandled exception occurs during phase 0 initialization.
    Its function is to bugcheck the system with all the context information still on the stack.
Arguments:
    Code - Supplies the exception code.
    Pointers - Supplies a pointer to the exception information.
Return Value:
    None - There is no return from this routine even though it appears there is.
*/
{
...
*/
{
    ...
    // Query the HAL to determine if the write combining can be used for the frame buffer.
    Status = HalQuerySystemInformation(HalFrameBufferCachingInformation, sizeof(BOOLEAN), &UseFrameBufferCaching, &Size);

    // If the status is successful and frame buffer caching is disabled, then don't enable write combining.
...
/*
Routine Description:
    This function sets the current processor cache information in the PCR and PRCB.
*/
{
    ULONG AdjustedSize;
    UCHAR Associativity;
    ...
    // Scan through the cache descriptors initialized by the processor specific code and compute cache parameters for page coloring and largest cache line size.
    AdjustedSize = 0;
    Cache = Prcb->Cache;
    Pcr->SecondLevelCacheSize = 0;

    for (i = 0; i < Prcb->CacheCount; i++) {
        if ((Cache->Level >= 2) && ((Cache->Type == CacheData) || (Cache->Type == CacheUnified))) {
            Associativity = Cache->Associativity;
            if (Associativity == CACHE_FULLY_ASSOCIATIVE) {
                // Temporarily preserve existing behavior of treating fully associative cache as a 16 way associative cache until MM interprets the cache descriptors directly.
                Associativity = 16;
            }
            ...
            // If the line size is greater then the current largest line size, then set the new largest line size.
...
/*
Routine Description:
    This function extracts the cache hierarchy information of an AMD processor and initializes cache descriptors in the PRCB.
*/
{
    UCHAR Associativity;
    PCACHE_DESCRIPTOR Cache;
    ...
    // Iterate through the cache levels and generate the cache information for each level.
    Prcb = KeGetCurrentPrcb();
    Prcb->CacheCount = 0;
    Cache = &Prcb->Cache[0];
    Index = 0;
    do {
        ...
        case 1:
            ...
            break;
        case 2:// Get L2 unified cache information.
            ...
            case 0x0:// L2 cache is not present
                continue;
            case 0x2:// L2 cache is two way associative.
                Associativity = 2;
                break;
            case 0x4:// L2 cache is four way associative.
                Associativity = 4;
                break;
            case 0x6:// L2 cache is eight way associative.
                Associativity = 8;
                break;
            case 0x8:// L2 cache is sixteen way associative.
                Associativity = 16;
                break;
            case 0xf:// L2 cache is fully associative.
...
/*
Routine Description:
    This function extracts the cache hierarchy information of an Intel processor and initializes cache descriptors in the PRCB.
*/
{
    PCACHE_DESCRIPTOR Cache;
    CPU_INFO CpuInfo;
    ...
    // This cpuid function should be supported on all versions EM64T family processors.
    KiCpuId(0, 0, &CpuInfo);
    ...
    // Enumerate the details of cache hierarchy by executing CPUID instruction repeatedly until no more cache information to be returned.
    do {
        ...
        CacheSize = (CacheInfoEbx.Associativity + 1) * (CacheInfoEbx.Partitions + 1) * (CacheInfoEbx.LineSize + 1) * (CpuInfo.Ecx + 1);

        Cache->Size = (ULONG)CacheSize; ASSERT(CacheSize == Cache->Size);
        Cache++;
        Prcb->CacheCount++;
...
*/
{
    ...
    KiCpuId(0, 0, &CpuInformation);// Get the CPU vendor string.

    // Copy vendor string to PRCB.
    Temp = CpuInformation.Ecx;
    CpuInformation.Ecx = CpuInformation.Edx;
    CpuInformation.Edx = Temp;
    RtlCopyMemory(Prcb->VendorString, &CpuInformation.Ebx, sizeof(Prcb->VendorString) - 1);
...
*/
{
    ...
    if (((InformationStandard.Edx & HF_REQUIRED) != HF_REQUIRED) || ((InformationExtended.Edx & XHF_SYSCALL) == 0)) {
    ...
    // If the processor being initialized is the boot processor or the boot processor is NX capable, 
    // then set the no execute feature bit for this processor as appropriate.
    if ((Prcb->Number == 0) || ((KeFeatureBits & KF_NOEXECUTE) != 0)) {
        if ((InformationExtended.Edx & XHF_NOEXECUTE) != 0) {
            FeatureBits |= KF_NOEXECUTE;
        }
    }

    // Check for fast floating/save restore and, if present, enable for the current processor.
    ...
    if ((InformationStandard.Edx & HF_SMT) != 0) {
        Prcb->LogicalProcessorsPerPhysicalProcessor = (UCHAR)(InformationStandard.Ebx >> 16);
    }
...
*/
{
    ...
    KiCpuId(1, 0, &CpuInformation);// Get cpu feature information.
    ...
    if ((Prcb->CpuVendor == CPU_AMD) && (Family <= 0x0F) && (Model <= 5) && (Stepping <= 8)) {
