...
// Declare routines who's addresses are taken but that are not otherwise referenced in this module.
...
VOID KiGetInterruptDispatchPatchAddresses(PULONG_PTR Address1, PULONG_PTR Address2);
...
// Define prototypes and static initialization for the fast zero page routines.
VOID FASTCALL KiZeroPages(IN PVOID PageBase, IN SIZE_T NumberOfBytes);
VOID FASTCALL KiXMMIZeroPages(IN PVOID PageBase, IN SIZE_T NumberOfBytes);
VOID FASTCALL KiXMMIZeroPagesNoSave(IN PVOID PageBase, IN SIZE_T NumberOfBytes);

KE_ZERO_PAGE_ROUTINE KeZeroPages = KiZeroPages;
KE_ZERO_PAGE_ROUTINE KeZeroPagesFromIdleThread = KiZeroPages;

// Line size of the d-cache closest to the processor.   Used by machine dependent prefetch routines.  Default to 32.
ULONG KePrefetchNTAGranularity = 32;

VOID FASTCALL RtlPrefetchMemoryNonTemporal(PVOID   Memory, SIZE_T  Length);

// The following spinlock is for compatiblity with 486 systems that don't have a cmpxchg8b instruction and therefore need to synchronize using a spinlock.
// NOTE: This spinlock should be initialized on x86 systems.
...
#if defined(_X86PAE_)
FORCEINLINE VOID KiEnableNXSupport(VOID)
/*
Routine Description:
...
*/
{
    ...
    SharedUserData->ProcessorFeatures[PF_NX_ENABLED] = TRUE;// Set NX enabled in user shared page.
}
#endif
...
/*
Routine Description:
    This function gains control after the system has been bootstrapped and before the system has been initialized.
    Its function is to initialize the kernel data structures, initialize the idle thread and process objects,
    initialize the processor control block, call the executive initialization routine, and then return to the system startup routine.
    This routine is also called to initialize the processor specific structures when a new processor is brought on line.
Arguments:
...
*/
{
    ...
    PoInitializePrcb(Prcb);// Initialize processor's PowerState
    ...
    FeatureBits = KiGetFeatureBits();// Get the processor FeatureBits for this processor.

    // Enable no execute protection if the host processor supports the feature and it is enabled via a loader option.

    // N.B. If no execute protection is enabled, then other processors are enabled in the HAL during their startup.
    ...
    // Do one time initialization of the ProcessorControlSpace in the PRCB so local kernel debugger can get things like the GDT.
    KiSaveProcessorControlState(&Prcb->ProcessorState);
    KiGetCacheInformation();// Get processor Cache Size information.
    KiInitSpinLocks(Prcb, Number);// Initialize the per processor lock data.

    // If the initial processor is being initialized, then initialize the per system data structures.
    if (Number == 0) {
        // Set default node.  Used in non-multinode systems and in multinode systems until the node topology is available.
        KeNodeBlock[0] = &KiNode0;

#if defined(KE_MULTINODE)
        for (Index = 1; Index < MAXIMUM_CCNUMA_NODES; Index++) {
            KeNodeBlock[Index] = &KiNodeInit[Index];// Set temporary node.
        }
#endif
        ...
        KeLowerIrql(APC_LEVEL);// Lower IRQL to APC level.
        KeInitializeSpinLock(&KiFreezeExecutionLock);// Initialize kernel internal spinlocks
        KeInitializeSpinLock(&Ki486CompatibilityLock);// Initialize 486 compatibility lock

#if !defined(NT_UP)
        Prcb->MultiThreadSetMaster = Prcb;// Set this processor as the master (ie first found) processor in this SMT set (whether or not it is actually SMT).

        // During Text Mode setup, it is possible the system is running with an MP kernel and a UP HAL.
        // On X86 systems, spinlocks are implemented in both the kernel and the HAL with the verisons that alter IRQL in the HAL.
        // If the HAL is UP, it will not actually acquire/release locks while the MP kernel will which will cause the system to hang (or crash).
        // As this can only occur during text mode setup, 
        // we will detect the situation and disable the kernel only versions of queued spinlocks if the HAL is UP (and the kernel MP).
...
#undef RET
        ...
        if (FxsrPresent != KeI386FxsrPresent) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_FXSR, 0, 0, 0);// FXSR support must be available on all processors or on none
        }

        XMMIPresent = ((FeatureBits & KF_XMMI) ? TRUE : FALSE);
        if (XMMIPresent != KeI386XMMIPresent) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_XMMI, 0, 0, 0);// XMMI support must be available on all processors or on none
        }

        if (NpxFlag != KeI386NpxPresent) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, 0x387, 0, 0, 0);// NPX support must be available on all processors or on none
        }
        ...
        if ((KiBootFeatureBits & KF_CMPXCHG8B) && !(FeatureBits & KF_CMPXCHG8B)) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_CMPXCHG8B, 0, 0, 0);// cmpxchg8b must be available on all processors, if installed at boot
        }

        if ((KeFeatureBits & KF_GLOBAL_PAGE) && !(FeatureBits & KF_GLOBAL_PAGE)) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_GLOBAL_PAGE, 0, 0, 0);// Global page support must be available on all processors, if on boot processor
        }

        if ((KeFeatureBits & KF_PAT) && !(FeatureBits & KF_PAT)) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_PAT, 0, 0, 0);// PAT must be available on all processors, if on boot processor
        }

        if ((KeFeatureBits & KF_MTRR) && !(FeatureBits & KF_MTRR)) {
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_MTRR, 0, 0, 0);// MTRR must be available on all processors, if on boot processor
        }
        ...
        if ((KeFeatureBits & KF_NOEXECUTE) && !(FeatureBits & KF_NOEXECUTE)) {
            ...
            KeBugCheckEx(MULTIPROCESSOR_CONFIGURATION_NOT_SUPPORTED, KF_NOEXECUTE, 0, 0, 0);
        }

        if ((KeFeatureBits & KF_FAST_SYSCALL) != (FeatureBits & KF_FAST_SYSCALL)) {
            KiFastSystemCallDisable = 1;// If this feature is not available on all processors don't use it at all.
        }

        if ((KeFeatureBits & KF_XMMI64) != (FeatureBits & KF_XMMI64)) {
            KeFeatureBits &= ~KF_XMMI64;// If not all processors support Streaming SIMD Extensions 64bit FP don't use it at all.
        }
        ...
        KeFeatureBits &= FeatureBits;// Use subset of all NT feature bits available on each processor
        KeLowerIrql(DISPATCH_LEVEL);// Lower IRQL to DISPATCH level.
#endif
    }
    ...
    SharedUserData->ProcessorFeatures[PF_XMMI64_INSTRUCTIONS_AVAILABLE] = ((KeFeatureBits & KF_FXSR) && (KeFeatureBits & KF_XMMI64)) ? TRUE : FALSE;
    ...
    // If the initial processor is being initialized, 
    // then compute the timer table reciprocal value and reset the PRCB values for the controllable DPC behavior in order to reflect any registry overrides.
    if (Number == 0) {
    ...
    // check to see if there are any ready threads; otherwise add this processors to the IdleSummary
    ...
    LoaderBlock->Prcb = (ULONG)NULL;// This processor has initialized
}
...
Routine Description:
    ...
    It simply stuffs values into the PCR.
    (The PCR is not initialized statically because the number varies with the number of processors.)
...
*/
{
    ...
    // N.B. -1 does not seem to be a valid value for the map base.
    //      If this value is used, byte immediate in's and out's will actually go the hardware when executed in V86 mode.
...
/*
Routine Description:
    Examine the processor specific feature bits to determine the Windows supported features supported by this processor.
Return Value:
...
*/
{
    ...
    CpuVendor = KiGetCpuVendor();// Determine the processor type
    ...
    // CPUID(1) now returns information in EBX.
    // On the grounds that the low functions are supposed to be standard, 
    // we record the information regardless of processor vendor even though it may be 0 or undefined on older implementations.
    ...
    if (CpuVendor == CPU_INTEL) {
        ...
        if (((ProcessorSignature & 0x0FF0) == 0x0610 && (ProcessorSignature & 0x000F) <= 0x9) ||
            ...
            // If the boot processor has PII spec A27 errata (also present in early Pentium Pro chips),
            // then use only one processor to avoid unpredictable eflags corruption.
            NtBits &= ~KF_WORKING_PTE;
        }

        // Don't support prior attempts at implementing syscall/sysexit instructions.
    ...
    if (CpuVendor == CPU_CYRIX) {
        ...
        // held high too long during an FP instruction and causing random Trap07 with no exception bits.
        extern BOOLEAN KiIgnoreUnexpectedTrap07;

        KiIgnoreUnexpectedTrap07 = TRUE;

        // Workaround CMPXCHG bug to Cyrix processors where
        // Family = 6, Model = 0, Stepping <= 1. 
        // Note that Prcb->CpuStep contains both model and stepping.
...
#define CCR1    0xc1
            ...
            WRITE_PORT_UCHAR(CRC_NDX, CCR1); // Get current setting.

            ValueCCR1 = READ_PORT_UCHAR(CRC_DAT);

            ValueCCR1 |= 0x10;// Set the NO_LOCK bit and write it back.
...
#undef CRC_NDX
    ...
    // This was incorrect in NT4 and resulted processor vendors claiming not to support cmpxchg8b even if they did.  
    // Windows XP requires cmpxchg8b, work around this problems for the cases we know about.

    // Because cmpxchg8b is a requirement for Windows XP, winnt32 needs to be modified if new processors are added to the following list.
    ...
    if ((ProcessorFeatures & 0x00000100) == 0) {
        ...
        if ((CpuVendor == CPU_TRANSMETA) && (Prcb->CpuType >= 5) && (Prcb->CpuStep >= 0x402)) {
            ...
            // Embedded x86 processor from Rise.
            // This processor doesn't have a mechanism to turn on this feature bit but does have the functionality.
            // Act as if we saw the feature bit.
            ProcessorFeatures |= 0x100;
        }
    }
    ...
    // Test for SMT and determine the number of logical processors the underlying physical processor supports.
    ...
    // First, check for existence, then check extended function 0x80000001 (Extended Processor Features) if present.

    // Note: Intel guarantees that no processor that doesn't support extended CPUID functions will ever return a value with the most significant bit set.
    // Microsoft asks all CPU vendors to make the same guarantee.
    if (ExtendedCPUIDSupport != FALSE) {
        ...
        if ((Temp & 0xffffff00) == 0x80000000) {
            // Check extended processor features.
            // These, by definition, can vary on a processor by processor basis.
            if (Temp >= 0x80000001) {
                ...
                case CPU_AMD:
                    ...
                    // If the host processor supports no execute protection, then it is a K8 chip and it also supports 40-bits of physical address.
                    // For this case the MTRR register
                    // variables must be initialized to support 40-bits of physical memory.
                    if (ExtendedProcessorFeatures & 0x00100000) {
...
#define CPUID_REG_COUNT 4
    ...
    case CPU_INTEL:
        ...
        if (CpuidData[0] >= 2) {
            // The above returns a series of bytes.   
            // (In EAX, EBX, ECX and EDX).   The least significant byte (of EAX) gives the
            // number of times CPUID(2 ...) should be issued to return
            // the complete set of data.   The bytes are self describing data.
                ...
                for (i = 0; i < CPUID_REG_COUNT; i++) {
                    ...
                    while (CpuidReg) {
                        ...
                        if (((Descriptor > 0x40) && (Descriptor <= 0x47)) ||
                            ...
                            // below are 4 way.  The others will be special cased below.
                            ...
                            // There may be cache descriptors in this range that we don't understand accurately yet.
                        ...
                        } else if (((Descriptor > 0x65) && (Descriptor < 0x69)) || (Descriptor == 0x2C) || (Descriptor == 0xF0)) {
                            // L1 Descriptor with line size of 64 bytes or an explicit prefetch descriptor indicating 64 bytes.
                            KePrefetchNTAGranularity = 64;
                        } else if (Descriptor == 0xF1) {
                            // Explicit prefetch descriptor indicating 128 bytes.
                            KePrefetchNTAGranularity = 128;
                        } else if (((Descriptor >= 0x4A) &&
                            (Descriptor <= 0x4C)) ||
                                    (Descriptor == 0x78) ||
                                   (Descriptor == 0x7D) ||
                                   (Descriptor == 0x7F) ||
                                   (Descriptor == 0x86) ||
                                   (Descriptor == 0x87)) {
                            // These are the 64 byte cache line entries for L2 caches that do not fit the pattern described above.
                            if (Line < 64) {
                                Line = 64;
                            }
    ...
    case CPU_AMD:
        ...
        if ((Pcr->PrcbData.CpuType == 0x6) && (Pcr->PrcbData.CpuStep == 0x300)) {
            // Model 6,3,0 uses a different algorithm to report cache size.
            Size = 64 * 1024;
        }
...
VOID KiLockStepProcessor(PKIPI_CONTEXT SignalDone, IN PVOID Arg1, IN PVOID Arg2, IN PVOID Proceed)
{
    UNREFERENCED_PARAMETER(Arg1);
    UNREFERENCED_PARAMETER(Arg2);

    // Tell initiating processor that this processor is now waiting and wait until the initial processor signals this processor to continue.
    KiIpiSignalPacketDoneAndStall(SignalDone, Proceed);
}


VOID KiLockStepOtherProcessors(PULONG Proceed)
{
    PKPRCB Prcb;
    ...
    if (TargetProcessors != 0) {
        ...
        KiIpiSendSynchronousPacket(Prcb, TargetProcessors, KiLockStepProcessor, NULL, NULL, Proceed);
...
VOID KiUnlockStepOtherProcessors(PULONG Proceed)
{
    (*Proceed) += 1;
}
...
BOOLEAN KiInitMachineDependent(VOID)
{
    ...
    if (KeFeatureBits & KF_LARGE_PAGE) {
        ...
        Ki386ClearIdentityMap(&IdentityMap);// Always call Ki386ClearIdentityMap() to free any memory allocated
    }

    // If PDE/PTE global page is supported, enable it
    if (KeFeatureBits & KF_GLOBAL_PAGE) {
        NumberProcessors = KeNumberProcessors;
        KeIpiGenericCall((PKIPI_BROADCAST_WORKER)Ki386EnableGlobalPage, (ULONG)(&NumberProcessors));
    }
    ...
    if (KeFeatureBits & (KF_PAT | KF_MTRR)) {
        ...
        if (NT_SUCCESS(Status) && (UseFrameBufferCaching == FALSE)) {
            KeFeatureBits &= ~(KF_PAT | KF_MTRR);// Hal says don't use.
        }
    }
    ...
    case 0:
        break;// Use the emulator based on the value in KeI386NpxPresent
    case 1:
        // Only use the emulator if any processor has the known Pentium floating point division problem.
        if (KeI386NpxPresent) {
            ...
            for (CurrentAffinity = 1; ActiveProcessors; CurrentAffinity <<= 1) {
                if (ActiveProcessors & CurrentAffinity) {
                    ...
                    _asm {
                        ; This is going to destroy the state in the coprocesssor, but we know that there's no state currently in it.
    ...
    if (!KeI386NpxPresent) {
        // MMx, fast save/restore, streaming SIMD not available when emulator is used.  (Nor FP errata).
    ...
    if (KeFeatureBits & KF_CR4) {
        NumberProcessors = KeNumberProcessors;
        KeIpiGenericCall((PKIPI_BROADCAST_WORKER)Ki386EnableDE, (ULONG)(&NumberProcessors));
    }

    // If FXSR feature is supported, set OSFXSR (bit 9) in CR4
    if (KeFeatureBits & KF_FXSR) {
        NumberProcessors = KeNumberProcessors;
        KeIpiGenericCall((PKIPI_BROADCAST_WORKER)Ki386EnableFxsr, (ULONG)(&NumberProcessors));
        ...
        if (KeFeatureBits & KF_XMMI) {
            KeIpiGenericCall((PKIPI_BROADCAST_WORKER)Ki386EnableXMMIExceptions, (ULONG)(&NumberProcessors));
...
#define XMMI_ZEROING_DISALLOW_OVERRIDE                  2
            ...
            // 2. Some Intel pentium 4 systems do not operate properly with this feature enabled.
            //  These machines hit a livelock.  Thus by default, if your system is a pentium 4, this feature gets disabled.

            // 3. Not all pentium 4 based systems have this livelock issue.  To enable these
            //  systems that work properly to take advantage of this feature, we allow an explicit enable registry key to be set.
            if (KiXMMIZeroingEnable != XMMI_ZEROING_DISALLOW_OVERRIDE) {
                // Enable non-temporal zeroing on all machines except MP Pentium 4 machines.
                // Pentium 4 machines can explicitly request this functionality via registry key.
                // This was done to address a livelock issue.
                if ((strcmp((PCHAR)Prcb->VendorString, CmpIntelID) != 0) || (Prcb->CpuType != 15) || (KiXMMIZeroingEnable == XMMI_ZEROING_ALLOW_OVERRIDE))
#endif
    ...
    // If the system (ie all processors) supports fast system call/return, initialize the machine specific registers required to support it.
    ...
    for (CurrentAffinity = 1; ActiveProcessors; CurrentAffinity <<= 1) {
        if (ActiveProcessors & CurrentAffinity) {
            ...
            // If this processor supports fast floating save/restore, determine the MXCSR mask value that should be used.
            if (KeFeatureBits & KF_FXSR) {
                ...
                // If the processor supplied a mask value, use that, otherwise set the default value.
                if (NpxFrame->U.FxArea.MXCsrMask != 0) {
                    MXCsrMask = NpxFrame->U.FxArea.MXCsrMask;
                }

                // All processors must use the same (most restrictive) value.
                if (KiMXCsrMask == 0) {
    ...
    // If ISR time limits are being enforced, modify KiDispatchInterrupt and KiChainedDispatch2ndLvl to call into the appropriate equivalent timing routines.
    if (KiTimeLimitIsrMicroseconds != 0) {
        ...
        KiLockStepOtherProcessors(&Proceed);// Freeze the other processors.
        KfRaiseIrql(HIGH_LEVEL);// Patch KiChainedDispatch2ndLvl to branch into KiTimedChainedDispatch2ndLvl.
        ...
        *(PULONG)Code = Target;
        ...
        KiGetInterruptDispatchPatchAddresses(&Source, &SourceEnd);// Get addresses of code to patch in KiInterruptDispatch
...
Routine Description:
    ...
    If this is a 386 system without 387s (all processors must be symmetrical)
    then this function will set the trap 07 vector on all processors to point to the address passed in
    (which should be the entry point of the 80387 emulator in NTDLL, NPXNPHandler).
Arguments:
    ...
    if (!KeI386NpxPresent) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (NT_SUCCESS(Status)) {
                    KiMoveRegTree(SourceHandle, DestHandle);// Move it
                    ZwClose(DestHandle);
                }
...
NTSTATUS KiMoveRegTree(HANDLE  Source, HANDLE  Dest)
{
    ...
    for (; ;) {
        ...
        Status = ZwDeleteValueKey(Source, &ValueName);// Delete value and get first value again
        if (!NT_SUCCESS(Status)) {
            break;
        }
    }
    ...
    for (; ;) {
        ...
        // Loop and get first key.  (old first key was deleted by the call to KiMoveRegTree).
    }
...
#define IDT_SKIP   (7 * sizeof (KIDTENTRY))
    ...
    BasePage = MmAllocateIndependentPages(2 * PAGE_SIZE, 0);// Allocate memory for a new copy of the processor's IDT
    NewBase = BasePage + PAGE_SIZE - IDT_SKIP;// The IDT base is such that the first 7 entries are on the first (read-only) page, and the remaining entries are on the second (read-write) page
    Enable = KeDisableInterrupts();// Disable interrupts on this processor while updating the IDT base
    ...
    _asm {
        lidt IdtDescriptor.Limit// Set the new IDT
    }

    KeGetPcr()->IDT = (PKIDTENTRY)NewBase;// Update the PCR
    KeEnableInterrupts(Enable);// Restore interrupts
    Status = MmSetPageProtection(BasePage, PAGE_SIZE, PAGE_READONLY);// Mark the first page which contains IDT entries 0-6 as read-only
    ASSERT(Status);
}