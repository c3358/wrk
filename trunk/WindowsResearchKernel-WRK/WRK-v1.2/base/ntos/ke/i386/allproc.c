...
BOOLEAN KiStartWaitAcknowledge(VOID);
VOID  KiSetHaltedNmiandDoubleFaultHandler(VOID);
VOID KiDummyNmiHandler(VOID);
VOID KiDummyDoubleFaultHandler(VOID);
VOID KiClearBusyBitInTssDescriptor(IN ULONG DescriptorOffset);
VOID KiHaltThisProcessor(VOID);
...
// Statically preallocate enough KNODE structures to allow MM to allocate pages by node during system initialization.
// As processors are brought online, real KNODE structures are allocated in the appropriate memory for the node.

// This statically allocated set will be deallocated once the system is initialized.
...
*/
{
    ...
    KiCloneDescriptor(&Descriptor, &pProcessorState->SpecialRegisters.Gdtr, Base + GdtOffset);
...
#if defined(KE_MULTINODE)
    // If this is the first processor on this node, use the space allocated for KNODE as the KNODE.
...
/*
Routine Description:
    Called by p0 during phase 1 of bootup.
    This function implements the x86 specific code to contact the hal for each system processor.
Return Value:
...
*/
{
    ...
    // Do not start additional processors if the RELOCATEPHYSICAL loader switch has been specified.
...
/*
Routine Description:
    Makes a copy of the current selector's data, and updates the new GDT's linear address to point to the new copy.
Arguments:
...
/*
Routine Description:
    This routine is called (possibly while the dispatcher lock is held) after processors are added to or removed from the system.
    It runs thru the PRCBs for each processor in the system and adjusts scheduling data.
*/
{
    ...
    for (ProcessorNumber = 0; ProcessorNumber < (ULONG)KeNumberProcessors; ProcessorNumber++) {
        ...
        // Skip processors which are not present or which do not support Simultaneous Multi Threading.
        if ((Prcb == NULL) || (Prcb->LogicalProcessorsPerPhysicalProcessor == 1)) {
            continue;
        }
        ...
        // Examine each remaining processor to see if it is part of the same set.
        for (BuddyNumber = ProcessorNumber + 1; BuddyNumber < (ULONG)KeNumberProcessors; BuddyNumber++) {
            ...
            if ((BuddyPrcb == NULL) || (BuddyPrcb->LogicalProcessorsPerPhysicalProcessor == 1)) {
                continue;
            }

            // Does this processor have the same ID as the one we're looking for?
            if (((BuddyPrcb->InitialApicId & ApicMask) != ApicId) || (BuddyPrcb->ParentNode != Prcb->ParentNode)) {
                continue;
            }
...
#if defined(KE_MULTINODE)
    // Make sure there are no references to the temporary nodes used during initialization.
    for (i = 0; i < KeNumberNodes; i++) {
        if (KeNodeBlock[i] == &KiNodeInit[i]) {
            // No processor started on this node so no new node structure has been allocated.
            // This is possible if the node contains only memory or IO busses.
            // At this time we need to allocate a permanent node structure for the node.
        ...
        KeNodeBlock[i]->NodeNumber = (UCHAR)i;// Set the node number.
    }
...
/*
Routine Description:
    This routine is a stub used on non NUMA systems to provide a consistent method of determining the NUMA configuration rather than checking for the presence of multiple nodes inline.
Arguments:
    ProcessorNumber supplies the system logical processor number.
    Identifier      supplies the address of a variable to receive the unique identifier for this processor.
    NodeNumber      supplies the address of a variable to receive the number of the node this processor resides on.
Return Value:
    Returns success.
*/
{
...
/*
Routine Description:
    This routine is a called before the application processor that is not going to be started is put into halt.
    It is used to hook a dummy Nmi and double fault handler.
*/
{
    PKPCR Pcr;
    PKGDTENTRY GdtPtr;
...
/*
Routine Description:
    Called to clear busy bit in descriptor from the NMI and double Fault Handlers
Arguments:
    None.
*/
{
...
/*
Routine Description:
  After Clearing the busy bit (just being paranoid here) we halt this processor.
*/
{
    while (1) {
        _asm {
