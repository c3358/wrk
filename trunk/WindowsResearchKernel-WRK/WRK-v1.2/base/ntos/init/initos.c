...
#if defined(_X86_)
VOID KiInitializeInterruptTimers(VOID);
#endif

VOID Phase1InitializationDiscard(IN PVOID Context);
VOID CmpInitSystemVersion(IN ULONG stage, IN PVOID param);
VOID HeadlessInit(IN PLOADER_PARAMETER_BLOCK LoaderBlock);

// The INIT section is not pageable during initialization, so these functions can be in INIT rather than in .text.
...
BOOLEAN ExpOkayToLockRoutine(IN PEXLOCK Lock)
{
    return TRUE;
}
...
NTSTATUS ExpAcquireLockRoutine(PEXLOCK Lock)
{
    ExAcquireSpinLock(&Lock->SpinLock, &Lock->Irql);
    return STATUS_SUCCESS;
}

NTSTATUS ExpReleaseLockRoutine(PEXLOCK Lock)
{
...
/*
Routine Description:
    This routine is called from the kernel initialization routine during bootstrap to initialize the executive and all of its subcomponents.
    Each subcomponent is potentially called twice to perform Phase 0, and then Phase 1 initialization.
    During Phase 0 initialization, the only activity that may be performed is the initialization of subcomponent specific data.
    Phase 0 initialization is performed in the context of the kernel start up routine with interrupts disabled.
    During Phase 1 initialization, the system is fully operational and subcomponents may do any initialization that is necessary.
Arguments:
...
*/
{
    ...
    if (Number == 0) {
        ...
        // Determine whether this is textmode setup and whether this is a remote boot client.
            ...
            if (MemoryOption != NULL) {
                ...
                if (MemoryOption != NULL) {
                    ...
                    if (PagesToBurn != 0) {
                        ...
                        BBTPagesToReserve = ExBurnMemory(LoaderBlock, PagesToBurn, LoaderBBTMemory, &BBTMemoryDescriptor);
                    }
                }
            }

            // Burn memory - consume the amount of memory specified in the OS Load Options.  This is used for testing reduced memory configurations.
        ...
        // Initialize the translation tables using the loader loaded tables.
        ...
        // Set the interrupt time forward so the Win32 tick count will wrap within one hour to make rollover errors show up in fewer than 49.7 days.
#if DBG
        ...
        Status = RtlFormatBuffer2(Buffer, sizeof(Buffer), "C:%s", LoaderBlock->NtBootPathName, 0);
        ...
        while (NextEntry != &LoaderBlock->LoadOrderListHead) {
            ...
            if (ImageCount >= 2) {
                ...
                if (DataTableEntry->FullDllName.Buffer[0] == L'\\') {// Correct fullname already available
                    ...
                    if (sizeof(Buffer) < Length + sizeof(ANSI_NULL)) {
                        BufferSizeOk = FALSE;// DllName too long.
                    } else {
                        Count = 0;
                        do {
        ...
        // Snapshot the NLS tables into paged pool and then reset the translation tables.
        ...
        if (NtGlobalFlag & FLG_KERNEL_STACK_TRACE_DB) {
            ...
            if (StackTraceDataBase != NULL) {
                ...
                Status = RtlInitializeStackTraceDataBase(StackTraceDataBase, StackTraceDataBaseLength, StackTraceDataBaseLength);
            } else {
        ...
        // Compute the tick count multiplier that is used for computing the windows millisecond tick count and copy the resultant value to the memory that is shared between user and kernel mode.
...
#endif
    } else {// Initialize the Hardware Architecture Layer (HAL).
        if (HalInitSystem(InitializationPhase, LoaderBlock) == FALSE) {
            KeBugCheck(HAL_INITIALIZATION_FAILED);
        }
    }
...
/*
Routine Description:
    This routine is a paged stub that calls a discardable routine to do the real work.
    This is because Mm reuses the discardable routine's (INIT) memory after phase 1 completes,
    but must keep this routine around so that the stack walker can obtain correct prologues/epilogues.
Arguments:
...
VOID Phase1InitializationDiscard(IN PVOID Context)
{
    ...
    ProcessInformation = ExAllocatePoolWithTag(NonPagedPool, sizeof(*ProcessInformation), 'tinI');
    ...
    // Set the phase number and raise the priority of current thread to a high priority so it will not be preempted during initialization.
    ...
    // Allow the boot video driver to behave differently based on the OsLoadOptions.
    ...
    // There is now enough functionality for the system Boot Video Driver to run.
    InbvDriverInitialize(LoaderBlock, 18);
    if (NOGUIBOOT) {// If the user specified the noguiboot switch we don't want to use the bootvid driver, so release display ownership.
    ...
    // The user may have put a /YEAR=2000 switch on the OSLOADOPTIONS line.
    // This allows us to enforce a particular year on hardware that has a broken clock.
    if (Options) {
    ...
    // N.B. This cannot be done until after the phase one initialization of the HAL Layer.
    if (ExCmosClockIsSane && HalQueryRealTimeClock(&TimeFields)) {
        ...
        if (!ExpRealTimeIsUniversal) {
            // If the system stores time in local time. This is converted to universal time before going any further

            // If we have previously set the time through NT, then ExpLastTimeZoneBias should contain the timezone bias in effect when the clock was set.
            // Otherwise, we will have to resort to our next best guess which would be the programmed bias stored in the registry
    ...
    KeStartAllProcessors();// If this is an MP build of the kernel start any other processors now

    // Since starting processors has thrown off the system time, get it again from the RTC and set the system time again.
    if (ExCmosClockIsSane && HalQueryRealTimeClock(&TimeFields)) {
        if (YearOverrideOption) {
            TimeFields.Year = (SHORT)CurrentYear;
        }
    ...
    // Set the affinity of the system process and all of its threads to all processors in the host configuration.
    ...
    // Signify to the HAL that all processors have been started and any post initialization should be performed.
    ...
    // SE expects directory and executive objects to be available, but must be before device drivers are initialized.
    if (!SeInitSystem()) {
    ...
    // Snapshot the NLS tables into a page file backed section, and then reset the translation tables.
    ...
    // Map it back into the system cache, but now the pages will no longer be valid.
    ...
    RtlInitNlsTables((PVOID)(
        (PUCHAR)InitNlsTableBase + InitAnsiCodePageDataOffset),
        (PVOID)((PUCHAR)InitNlsTableBase + InitOemCodePageDataOffset),
        (PVOID)((PUCHAR)InitNlsTableBase + InitUnicodeCaseTableDataOffset),
        &InitTableInfo);
    RtlResetRtlTranslations(&InitTableInfo);
    ...
    // two parts.  Part 1 makes \REGISTRY\MACHINE\SYSTEM and \REGISTRY\MACHINE\HARDWARE available.
    // These are needed to complete IO init.
    if (!CmInitSystem1(LoaderBlock)) {
        KeBugCheck(CONFIG_INITIALIZATION_FAILED);
    }

    // Initialize the prefetcher after registry is initialized so we can query the prefetching parameters.
    ...
    if (ResetActiveTimeBias) {
        ...
        KeBootTimeBias = 0;
        KeSetSystemTime(&UniversalTime, &OldTime, FALSE, NULL);
    } else {// Check to see if a timezone switch occurred prior to boot...
    ...
    // Initialize the range list package - this must be before PNP initialization as PNP uses range lists.
    RtlInitializeRangeListPackage();
    HalReportResourceUsage();
    KdDebuggerInitialize1(LoaderBlock);

    // Perform phase1 initialization of the Plug and Play manager.  This must be done before the I/O system initializes.
    if (!PpInitSystem()) {
        KeBugCheck(PP1_INITIALIZATION_FAILED);
    }
    InbvUpdateProgressBar(20);

    // LPC needs to be initialized before the I/O system, since some drivers may create system threads that will terminate and cause LPC to be called.
    if (!LpcInitSystem()) {
        KeBugCheck(LPC_INITIALIZATION_FAILED);
    }

    ExInitSystemPhase2();// Now that system time is running, initialize more of the Executive.
    KdpTimeSlipPending = 0;// Allow time slip notification changes.
    ...
    // Note:  If this fails, Vdms may not be able to run, but it isn't necessary to bugcheck the system because of this.
    KeI386VdmInitialize();

#if !defined(NT_UP)
    // Now that the error log interface has been initialized, 
    // write an informational message if it was determined that the processors in the system are at differing revision levels.
    ...
    // Okay to call PsInitSystem now that \SystemRoot is defined so it can locate NTDLL.DLL and SMSS.EXE.
    ...
    // Perform Phase 1 Reference Monitor Initialization.
    // This includes creating the Reference Monitor Command Server Thread, a permanent thread of the System Init process.
    // That thread will create an LPC port called the Reference Monitor Command Port through which commands sent by the Local Security Authority Subsystem will be received.
    // These commands (e.g. Enable Auditing) change the Reference Monitor State.
    if (!SeRmInitPhase1()) {
    ...
    // NOTE: Remote boot allocates an extra DOS_MAX_PATH_LENGTH number of WCHARs in order to hold command line arguments to smss.exe.
    Size = sizeof(*ProcessParameters) + ((DOS_MAX_PATH_LENGTH * 6) * sizeof(WCHAR));
    ProcessParameters = NULL;
    Status = ZwAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&ProcessParameters, 0, &Size, MEM_COMMIT, PAGE_READWRITE);
    if (!NT_SUCCESS(Status)) {
    ...
    Dst = (PWSTR)((PCHAR)ProcessParameters->CurrentDirectory.DosPath.Buffer + ProcessParameters->CurrentDirectory.DosPath.MaximumLength);
    ...
    RtlCopyMemory(ProcessParameters->ImagePathName.Buffer, NtInitialUserProcessBuffer, ProcessParameters->ImagePathName.Length);
    ...
    InbvEnableDisplayString(TRUE);// Turn on debug output so that we can see chkdsk run.

    // Wait five seconds for the session manager to get started or terminate.
    // If the wait times out, then the session manager is assumed to be healthy and the zero page thread is called.
...
NTSTATUS CreateSystemRootLink(IN PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    status = NtCreateSymbolicLinkObject(&linkHandle, SYMBOLIC_LINK_ALL_ACCESS, &objectAttributes, &deviceNameUnicodeString);
    RtlFreeUnicodeString(&linkUnicodeString);
