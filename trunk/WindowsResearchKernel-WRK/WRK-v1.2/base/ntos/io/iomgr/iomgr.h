...
// Define Information fields values for the return value from popups when a volume mount is in progress but failed.
...
typedef struct _OPEN_PACKET {
    ...
    // The following are the open-specific parameters.
    // Notice that the desired access field is passed through to the parse routine via the object management architecture,
    // so it does not need to be repeated here.
    // Also note that the same is true for the file name.
    ...
    // The following is used when performing a fast query during open to get back the file attributes for a file.
    PFILE_BASIC_INFORMATION BasicInformation;

    // The following is used when performing a fast network query during open to get back the network file attributes for a file.
    PFILE_NETWORK_OPEN_INFORMATION NetworkInformation;

    // The type of file to create.
    CREATE_FILE_TYPE CreateFileType;

    // The following pointer provides a way of passing the parameters specific to the file type of the file being created to the parse routine.
    PVOID ExtraCreateParameters;

    // The following is used to indicate that an open of a device has been performed and the access check for the device has already been done,
    // but because of a reparse, the I/O system has been called again for the same device.
    // Since the access check has already been made, 
    // the state cannot handle being called again (access was already granted) and it need not anyway since the check has already been made.
    BOOLEAN Override;

    // The following is used to indicate that a file is being opened for the sole purpose of querying its attributes.
    // This causes a considerable number of shortcuts to be taken in the parse, query, and close paths.
    BOOLEAN QueryOnly;

    // The following is used to indicate that a file is being opened for the sole purpose of deleting it.
    // This causes a considerable number of shortcurs to be taken in the parse and close paths.
    BOOLEAN DeleteOnly;

    // The following is used to indicate that a file being opened for a query only is being opened to query its network attributes rather than just its FAT file attributes.
    BOOLEAN FullAttributes;


    // The following pointer is used when a fast open operation for a fast delete or fast query attributes call is being made rather than a general file open.
    // The dummy file object is actually stored on the the caller's stack rather than allocated pool to speed things up.
    PDUMMY_FILE_OBJECT LocalFileObject;

    // The following is used to indicate we passed through a mount point while parsing the filename.
    // We use this to do an extra check on the device type for the final file
...
// Define the default number of I/O stack locations a large IRP should have if not specified by the registry.
...
// The following declaration cannot go in EX.H since POBJECT_TYPE is not defined until OB.H, which depends on EX.H.
// Hence, it is not exported by the EX component at all.
...
static  FORCEINLINE BOOLEAN IopAcquireFastLock(IN  PFILE_OBJECT FileObject)
{
...
#define IopAcquireCancelSpinLockAtDpcLevel()        KiAcquireQueuedSpinLock ( &KeGetCurrentPrcb()->LockQueue[LockQueueIoCancelLock] )
#define IopReleaseCancelSpinLockFromDpcLevel()      KiReleaseQueuedSpinLock ( &KeGetCurrentPrcb()->LockQueue[LockQueueIoCancelLock] )
#define IopAllocateIrp(StackSize, ChargeQuota)      IoAllocateIrp((StackSize), (ChargeQuota))
#define IsIoVerifierOn()    IopVerifierOn


static __inline  VOID IopProbeAndLockPages(IN OUT PMDL MemoryDescriptorList,
...
NTSTATUS IopSendMessageToTrackService(IN PLINK_TRACKING_INFORMATION SourceVolumeId,
...
FORCEINLINE VOID IopUpdateOtherOperationCount(VOID)
/*
Routine Description:
...
FORCEINLINE VOID IopUpdateReadOperationCount(VOID)
/*
Routine Description:
...
FORCEINLINE VOID IopUpdateWriteOperationCount(VOID)
/*
Routine Description:
...
FORCEINLINE VOID IopUpdateOtherTransferCount(IN ULONG TransferCount)
/*
Routine Description:
...
FORCEINLINE VOID IopUpdateReadTransferCount(IN ULONG TransferCount)
/*
Routine Description:
...
FORCEINLINE VOID IopUpdateWriteTransferCount(IN ULONG TransferCount)
/*
Routine Description:
...
NTSTATUS FORCEINLINE IopfCallDriver(IN PDEVICE_OBJECT DeviceObject, IN OUT PIRP Irp)
/*
Routine Description:
...
*/
{
    ...
    ASSERT(Irp->Type == IO_TYPE_IRP);// Ensure that this is really an I/O Request Packet.

    Irp->CurrentLocation--;// Update the IRP stack to point to the next location.
    ...
    driverObject = DeviceObject->DriverObject;// Invoke the driver at its dispatch routine entry point.

    status = driverObject->MajorFunction[irpSp->MajorFunction](DeviceObject, Irp);// Prevent the driver from unloading.
    return status;
}
