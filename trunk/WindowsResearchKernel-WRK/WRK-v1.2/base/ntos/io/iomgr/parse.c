...
NTSTATUS IopCheckDeviceAndDriver(IN POPEN_PACKET op, IN PDEVICE_OBJECT parseDeviceObject)
{
    NTSTATUS status;
    KIRQL irql;

    // Make sure that the device and its driver are really there and they are going to stay there.
    // The object itself cannot go away just yet because the object management system has performed a reference which bumps the count of the number of reasons why the object must stick around.
    // However, the driver could be attempting to unload itself, so perform this check.
    // If the driver is being unloaded, then set the final status of the operation to "No such device" and return with a NULL file object pointer.

    // Note that it is possible to "open" an exclusive device more than once provided that the caller is performing a relative open.
    // This feature is how users "allocate" a device, and then use it to perform operations.
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);
    if (parseDeviceObject->DeviceObjectExtension->ExtensionFlags &
        (DOE_UNLOAD_PENDING | DOE_DELETE_PENDING | DOE_REMOVE_PENDING | DOE_REMOVE_PROCESSED | DOE_START_PENDING) || parseDeviceObject->Flags & DO_DEVICE_INITIALIZING) {
...
PVPB IopCheckVpbMounted(IN POPEN_PACKET op, IN PDEVICE_OBJECT parseDeviceObject, IN OUT PUNICODE_STRING RemainingName, OUT PNTSTATUS status)
{
    ...
    // Loop here until the VPB_MOUNTED test can be passed while holding the VPB spinlock.
    // After the mount succeeds, it is still necessary to acquire the spinlock to check that the VPB (which may be different from the one before the mount) is still mounted.
    // If it is, then its reference count is incremented before releasing the spinlock.
    ...
    while (!(parseDeviceObject->Vpb->Flags & VPB_MOUNTED)) {
        ...
        mountVpb = NULL;
        *status = IopMountVolume(parseDeviceObject, (BOOLEAN)(!RemainingName->Length && !op->RelatedFileObject), FALSE, alertable, &mountVpb);
        if (!NT_SUCCESS(*status) || *status == STATUS_USER_APC || *status == STATUS_ALERTED) {//If the mount operation was unsuccessful, adjust the reference count for the device and return now.
        ...
        } else {// In this case IopMountVolume did the synchronization already.
            if (mountVpb) {
                return mountVpb;
            }
        }
    ...
    vpb = parseDeviceObject->Vpb;// Synchronize here with the file system to make sure that volumes do not go away while en route to the FS.
...
Arguments:
    TargetDeviceObject - Device object whose stack the hint must be on for it to be valid; holds the hint on return if it is valid.
...
Return Value:
    ...
    STATUS_MOUNT_POINT_NOT_RESOLVED - The target device object and the hint are on different stacks and a mount point was previously traversed.
    STATUS_INVALID_DEVICE_OBJECT_PARAMETER - The target device object and the hint are on different stacks.
    STATUS_SUCCESS otherwise.
*/
{
    PDEVICE_OBJECT deviceObject = *TargetDeviceObject;

    // You cannot use the device object hint if you are trying to open the device directly or if you are dealing with a device that is not a file system.
...
reparse_loop:
    *Object = (PVOID)NULL;// Assume failure by setting the returned object pointer to NULL.
    op = Context;// Get the address of the Open Packet (OP).

    // Ensure that this routine is actually being invoked because someone is attempting to open a device or a file through NtCreateFile.
    // This code must be invoked from there (as opposed to some other random object create or open routine).
    if (op == NULL || op->Type != IO_TYPE_OPEN_PACKET || op->Size != sizeof(OPEN_PACKET)) {
        return STATUS_OBJECT_TYPE_MISMATCH;
    }

    parseDeviceObject = (PDEVICE_OBJECT)ParseObject;// Obtain a pointer to the parse object as a device object, which is the actual type of the object anyway.

    // If we passed through a mountpoint do an extra set of validation checks that we don't go to a remote device.
    // We really have to let the object manager open the new path.
    // If we fail it in the path that obtains the reparse point and validates the name (using IoIsValidNameGraftingBuffer) its not sufficient.
    // This is because the path may be valid at that time and change before OB does the reparse.
    if (op->TraversedMountPoint) {
    ...
    // is really being opened from the related file object and use that for the remainder of this function and for all operations performed on the file object that is about to be created.
    ...
    // Since ObOpenObjectByName is called without being passed any object type information, we need to map the generic bits in the DesiredAccess mask here.
    // We also need to save the object's generic mapping in the access state structure here, because this is the earliest opportunity we have to do so.
    ...
    // If this is not the first time through here for this object, and the object itself is being opened, 
    // then the desired access must also include the previously granted access from the last pass.
    // Likewise, if the privileges have been checked already, then this is another pass through for a file, so add in the previously granted access.
    ...
    // If its a filesystem volume open and we are doing a relative open to it then do the access check.
    // Note that relative opens can be nested and we propagate the fact that the relative open is for a volume using the FO_VOLUME_OPEN flag.
    if (op->RelatedFileObject) {
    ...
    // Now determine what type of security check should be made.
    // This is based on whether the remaining name string is null.
    // If it is null, then the device itself is being opened, so a full security check is performed.
    // Otherwise, only a check to ensure that the caller can traverse the device object is made. 
    // Note that these checks are only made if the caller's mode is user, or if access checking is being forced.
    // Note also that if an access check was already made on the device itself, and this code is being executed again because of a reparse, 
    // then the access check need not be made the second time around.
    if ((AccessMode != KernelMode || op->Options & IO_FORCE_ACCESS_CHECK) && (!op->RelatedFileObject || relativeVolumeOpen) && !op->Override) {
        ...
        // The caller's mode is either user or access checking is being forced.  Perform the appropriate access check on the device object.
        if (!RemainingName->Length) {
            ...
            // The device is not being opened, rather, a file on the device is being opened or created. 
            // Therefore, only perform a check here for traverse access to the device.
            ...
            if (!(AccessState->Flags & TOKEN_HAS_TRAVERSE_PRIVILEGE) ||
                ...
                KeEnterCriticalRegionThread(&CurrentThread->Tcb);
                ExAcquireResourceSharedLite(&IopSecurityResource, TRUE);
                accessGranted = SeFastTraverseCheck(parseDeviceObject->SecurityDescriptor, AccessState, FILE_TRAVERSE, UserMode);
                if (!accessGranted) {
                    PPRIVILEGE_SET privileges = NULL;

                    // The caller was not granted traverse access through the normal fast path lookup.
                    // Perform a full-blown access check to determine whether some other ACE allows traverse access.
                    SeLockSubjectContext(&AccessState->SubjectSecurityContext);
                    subjectContextLocked = TRUE;
                    accessGranted = SeAccessCheck(parseDeviceObject->SecurityDescriptor,
                                                  &AccessState->SubjectSecurityContext,
        ...
        // Unlock the subject's security context so that it can be changed, if it was locked.
    ...
    if (RemainingName->Length == 0 &&
        ...
        ((desiredAccess & ~(SYNCHRONIZE | FILE_READ_ATTRIBUTES | READ_CONTROL | ACCESS_SYSTEM_SECURITY | WRITE_OWNER | WRITE_DAC)) == 0) &&
        realFileObjectRequired) {
        // If the name of the object being opened is just the name of the
        // device itself, and there is no related file object, and the caller is opening the device for only read attributes access,
        // then this device will not be mounted.
        // This allows applications to obtain attributes about the device without actually mounting it.

        // Note that if this *is* a direct device open, then the normal path through the I/O system and drivers may never be used,
        // even if the device appears to be mounted.
        // This is because the user may remove the media from the drive (even though it is mounted), and
        // now attempting to determine what type of drive it is will still fail, this time very hard, 
        // because a whole mount process is now required, thus defeating this feature.
        directDeviceOpen = TRUE;
    } else {
        directDeviceOpen = FALSE;// Otherwise, this is a normal open of a file, directory, device, or volume.
    }

    // There are now five different cases.  These are as follows:
    //    1)  This is a relative open, in which case we want to send the request to then same device that opened the relative file object.
    //    2)  The VPB pointer in the device object is NULL.  This means that this device does not support a file system.  This includes devices such as terminals, etc.
    ...
    //        b)  The VPB is non-blank, but the verify flag on the device is set, indicating that the door to the drive may have been opened and the media may therefore have been changed.
    //        c)  The VPB is non-blank and the verify flag is not set.
    //        Both of the latter are not explicitly checked for, as #c is the normal case, and #b is the responsibility of the file system to check.

    //  If this is a file system that supports volumes, vpbRefCount will be filled in to point to the reference count in the Vpb.
    //  Error exits paths later on key off this value to see if they should decrement the ref count.
    //  Note that a direct device open does not make it to the file system, so no increment is needed, and no decrement will be performed in objsup.c IopDeleteFile().
    vpb = NULL;

    // If the related open was a direct device open then we should go through the full mount path for this open as this may not be a direct device open.
    if (op->RelatedFileObject && (!(op->RelatedFileObject->Flags & FO_DIRECT_DEVICE_OPEN))) {
        ...
        if (parseDeviceObject->Vpb && !directDeviceOpen) {
            ...
            deviceObject = vpb->DeviceObject;// Set the address of the device object associated with the VPB.
        }

        // If the top deviceobject hint is set use the hint if possible.
        if (op->InternalFlags & IOP_CREATE_USE_TOP_DEVICE_OBJECT_HINT) {
            checkDeviceHint = TRUE;
        } else {// Walk the attached device list.
    ...
    if (checkDeviceHint) {
        status = IopCheckTopDeviceHint(&deviceObject, op, directDeviceOpen);
        if (!NT_SUCCESS(status)) {
            if (vpb) {
    ...
    //  If the TraversedMountPoint flag is still set, clear it now.  
    //  We needed to keep it to return the correct status if IopVerifyDeviceObjectOnStack failed above.
    if (op->TraversedMountPoint) {
        op->TraversedMountPoint = FALSE;
    }
    ...
    if ((deviceObject->Characteristics & FILE_DEVICE_SECURE_OPEN) &&
        ...
        // If the device wants to ensure secure opens then lets check the two cases which were skipped earlier.
        // These cases are if its a relative open or if there are trailing names.
        KeEnterCriticalRegionThread(&CurrentThread->Tcb);
        ExAcquireResourceSharedLite(&IopSecurityResource, TRUE);
        SeLockSubjectContext(&AccessState->SubjectSecurityContext);
        subjectContextLocked = TRUE;
    ...
    // Allocate and fill in the I/O Request Packet (IRP) to use in interfacing to the driver.
    // The allocation is done using an exception handler in case the caller does not have enough quota to allocate the packet.
    ...
    // This is where the original function codes and parameters are passed.
    irpSp = IoGetNextIrpStackLocation(irp);
    irpSp->Control = 0;
    if (op->CreateFileType == CreateFileTypeNone) {// This is a normal file open or create function.
    ...
    } else if (op->CreateFileType == CreateFileTypeNamedPipe) {// A named pipe is being created.
        irpSp->MajorFunction = IRP_MJ_CREATE_NAMED_PIPE;
        irpSp->Parameters.CreatePipe.Parameters = op->ExtraCreateParameters;
    } else {// A mailslot is being created.
    ...
    // Fill in local parameters so this routine can determine when the I/O is finished, and the normal I/O completion code will not get any errors.
    ...
    // Allocate and initialize the file object that will be used in dealing with the device for the remainder of this session with the user.
    // How the file object is allocated is based on whether or not a real file object is actually required.
    // It is not required for the query and delete only operations.
    if (realFileObjectRequired) {
        OBJECT_ATTRIBUTES objectAttributes;
        ULONG             fileObjectSize;

        // A real, full-blown file object is actually required.
        InitializeObjectAttributes(&objectAttributes, (PUNICODE_STRING)NULL, Attributes, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
        if (op->InternalFlags & (IOP_CREATE_USE_TOP_DEVICE_OBJECT_HINT | IOP_CREATE_IGNORE_SHARE_ACCESS_CHECK | IOP_CREATE_DEVICE_OBJECT_EXTENSION)) {
        ...
        // Now fill in the file object as best is possible at this point and set a pointer to it in the IRP so everyone else can find it.
        ...
        // This is either a quick delete or query operation.
        // For these cases,
        // it is possible to optimize the Object Manager out of the picture by simply putting together something that "looks" like a file object, 
    ...
    // Allocate a file name string buffer which is large enough to contain the entire remaining name string and initialize the maximum length.
    if (RemainingName->Length) {
    ...
    // Now copy the name string into the file object from the remaining name that is being reparsed.  
    // If the driver decides to reparse, then it must replace this name.
    RtlCopyUnicodeString(&fileObject->FileName, RemainingName);

    // Before invoking the driver's open routine, check to see whether or not this is a fast network attributes query and, 
    // if so, and the driver implements the function, attempt to call it here.
    if (op->QueryOnly) {
        ...
        if (fastIoDispatch &&
            ...
            if (result) {
                ...
                // Finally, indicate that the parse routine was actually invoked and that the information returned herein can be used.
                ...
                return status;
            } else {// The fast I/O operation did not work, so take the longer route.
    ...
    if (status == STATUS_PENDING) {
        ...
        KeRaiseIrql(APC_LEVEL, &irql);// Now finish up the request.

        // Note that normally the system would simply call IopCompleteRequest here to complete the packet.
        // However, because this is a create operation, 
        // several assumptions can be made that make it much faster to perform the couple of operations that completing the request would perform.
        // These include:  copying the I/O status block, dequeueing the IRP and freeing it, and setting the file object's event to the signaled state.
        // The latter is done here by hand, since it is known that it is not possible for any thread to be waiting on the event.
    ...
    if (!NT_SUCCESS(status)) {
        // If we are canceling the open of a real file object, we simply dereference it.
        // The object manager does the rest, i.e. the file system and filters will get their cleanup and close at the correct time.
        ...
        // The operation resulted in a reparse.
        // This means that the file name in the file object is the new name to be looked up.
        // Replace the complete name string with the new name and return STATUS_REPARSE so the object manager knows to start over again.
        // Note, however, that the file name buffer in the file object itself is kept intact so that it can be reused when coming back here again.

        // A reparse status may also have been returned from the file system if the volume that was in a drive needed to have been verified, 
        // but the verification failed, and a new volume was mounted.
        // In this case, everything starts over again using the new volume.
        ...
        if ((ioStatus.Information == IO_REPARSE) || (ioStatus.Information == IO_REPARSE_TAG_MOUNT_POINT)) {
            if (CompleteName->MaximumLength < fileObject->FileName.Length) {// If the complete name buffer isn't large enough, reallocate it.
                PVOID buffer;
        ...
        if (ioStatus.Information == IO_REPARSE_TAG_RESERVED_ONE) {
            ...
            op->RelatedFileObject = (PFILE_OBJECT)NULL;// Really reparsing a symbolic link, so go back to the object manager so it can begin the parse from the top.

            // Note that the mountpoint should be set only for the correct tag.
            // IO_REMOUNT is sent by FAT,CDFS and UDFS to remount a volume.
        ...
        // The operation was successful. The first thing to do is to see if the device that processed the open also opened the file.
        // If not, we need to adjust the vpb reference counts. Then, if this is
        // not a query or a delete, but rather a normal open/create, 
        // return the address of the FileObject to the caller and set the information returned in the original requestor's I/O status block.
        // Also set the value of the parse check field in the open packet to a value which will let the caller know that this routine was successful in creating the file object.
        ...
        if (deviceObject != deviceObjectThatOpenedFile) {
            ...
            // The device that opened the related file is not the one that opened this file.
            // So, readjust the vpb reference counts.
            newVpb = fileObject->Vpb;

            // If the new VPB is not the same as the original VPB then reference the new one before freeing the old one.
            // If a filter was just added to the fileobject stack the VPB will not change.
        ...
        if (realFileObjectRequired) {
            *Object = fileObject;
            ...
            // Add a reference so the file object cannot go away before the create routine gets chance to flag the object for handle create.
            ObReferenceObject(fileObject);

            // If the filename length is zero and its not a relative open or its a relative open to a volume open then set the volume open flag.
            // Also set it only for filesystem device object volume.
            if ((!fileObject->RelatedFileObject || fileObject->RelatedFileObject->Flags & FO_VOLUME_OPEN) && (!fileObject->FileName.Length)) {
            ...
            // This is either a quick query or delete operation.
            // Determine which it is and quickly perform the operation.
            if (op->QueryOnly) {
                ...
                if (!op->FullAttributes) {
                    ...
                    // This is a simple FAT file attribute query.
                    // Attempt to obtain the basic information about the file.
                    try {
                    ...
                    // This is a full attribute query.
                    // Attempt to obtain the full network attributes for the file.
                    // This includes both the basic and standard information about the file.
                    // Try the fast path first, if it exists.
                    if (fastIoDispatch &&
                    ...
                    if (!queryResult) {
                        // Either the fast dispatch routine did not exist, or it simply wasn't callable at this time.
                        // Attempt to obtain all of the information at once via an IRP-based call.
            ...
            } else {// There is nothing to do for a quick delete since the caller set the FILE_DELETE_ON_CLOSE CreateOption so it is already set in the file system.
                NOTHING;
            }
...
Routine Description:
    ...
    Its called out as a separate routine from IopParseDevice to save stack for common create paths.
Arguments:
...
#define COPY_ATTRIBUTES( n, b, s ) {                                    \
    ...
    // The IRP-based call did not work either, so simply try to obtain the information by doing IRP-based queries for the basic and standard information and piecing together the results into the caller's buffer.
    // Note that it might be possible to perform fast I/O operations to get the data, but it might also fail because of the above.
    // So simply query the information the long way.
    status = IoQueryFileInformation(FileObject, FileBasicInformation, sizeof(FILE_BASIC_INFORMATION), &basicInfo, &returnedLength);
    if (NT_SUCCESS(status)) {
        status = IoQueryFileInformation(FileObject, FileStandardInformation, sizeof(FILE_STANDARD_INFORMATION), &stdInfo, &returnedLength);
        if (NT_SUCCESS(status)) {
...
Routine Description:
    This routine interfaces to the NT Object Manager.
    It is invoked when the object system is given the name of an entity to create or open and is also given a handle to a directory file object that the operation is to be performed relative to.
    This routine is specified as the parse routine for all file objects.

    This routine simply invokes the parse routine for the appropriate device that is associated with the file object.
    It is the responsibility of that routine to perform the operation.
Arguments:
...
*/
{
    ...
    op = (POPEN_PACKET)Context;// Get the address of the Open Packet (OP).
    ...
    deviceObject = IoGetRelatedDeviceObject((PFILE_OBJECT)ParseObject);// Get a pointer to the device object for this file.
    op->RelatedFileObject = (PFILE_OBJECT)ParseObject;// Pass the related file object to the device object parse routine.
    return IopParseDevice(deviceObject, ObjectType, AccessState, AccessMode, Attributes, CompleteName, RemainingName, Context, SecurityQos, Object);// Open or create the specified file.
}


NTSTATUS IopQueryNameInternal(IN PVOID Object,
    ...
    */
{
    ...
    // Ensure that the size of the output buffer is at least the minimum size required to include the basic object name information structure.
    ...
    // and that we should free it (the Rtl call did not know we allocated this big buffer, and made a new one).
    try {
        if (UseDosDeviceName && dosLookupSuccess) {
            ULONG BaseCopyLength;
            ULONG NameCopyLength;

            // We will never take an exception in this path. That's why we don't have to free the device name buffer in the exception handler.
            ...
            // Copy in two parts - the base chunk of the UNICODE_STRING and then as much of the name as will fit.
        ...
        // Reset the state for the buffer to obtain the filename portion of the name and calculate the remaining length of the caller's buffer.
        // Note that in the following calculations, there are two assumptions and and dependencies:
        //     1)  The above query of the device name's returned length needed include a NULL character which will be included at the end of the entire name.
        //         This is included in the calculations although it does not appear to be included.
        //     2)  The sizeof the object name information buffer is assumed (and guaranteed because it can never change) to be larger than the filename offset in a file name information buffer.
        //         Therefore it is known that the new length of the "buffer" variable can be set to the remaining length plus at least 4.
        ...
        if (((Mode == UserMode) && (!UseDosDeviceName)) || !(fileObject->Flags & FO_SYNCHRONOUS_IO)) {
            status = IopQueryXxxInformation(fileObject, FileNameInformation, length, Mode, (PVOID)fileNameInfo, &lengthNeeded, TRUE);// Query the name of the file based using an intermediary buffer.
        } else {
            // This is a kernel mode request for a file that was opened for synchronous I/O.
            // A special function that does not obtain the file object lock is required, otherwise the request may deadlock since the lock is probably already owned.
        ...
        // Set the remaining length of the caller's buffer as well as the total length needed to contain the entire name of the file.
        ...
        // Attempt to copy the name of the file into the output buffer.
        // Note that if the file name does not begin w/a '\', then it is not volume relative, 
        // so the name of the file cannot be expressed as the concatenation of the name of the device and the file.
        // Therefore an error is returned.

        // The only example of this situation known at this time is when one opens a directory by file ID, and then opens a file relative to that directory.
        // When attempting to query the path, if the caller did not
        // have traverse access to open the directory, then the only name that can be returned is the path name to the file from the directory,
        // but the volume-relative name cannot be returned.
        // Therefore, the file system returns only the name of the directory and the path to the file, 
        // but this is not volume-relative so the only recourse is to return an error.

        // Note that if the caller were to call NtQueryInformationFile and request FileNameInformation, 
        // then the name above named will be successfully returned from the file system.
    ...
    } finally{// Finally, free the temporary buffer.
        ExFreePool(buffer);
    }
    ...
    */
{
    ...
    return IopQueryNameInternal(Object, HasObjectName, FALSE, ObjectNameInfo, Length, ReturnLength, Mode);
}
    ...
    */
{
    ...
    if (*CreateOptions & FILE_OPEN_FOR_BACKUP_INTENT) {
        ...
        // If the caller has requested MAXIMUM_ALLOWED, 
        // then make it appear as if the request was for everything permitted by Backup and Restore, and then grant everything that can actually be granted.
        if (desiredAccess & MAXIMUM_ALLOWED) {
            desiredAccess |= (readAccess | writeAccess);
        }

        // If the disposition says that we're opening the file, check for both backup and restore privilege, depending on what's in the desired access.

        // If the disposition says that we're creating or trying to overwrite the file, 
        // then all we need to do is to check for restore privilege, and if it's there, grant every possible access.
        if ((Disposition == FILE_OPEN) || (Disposition == FILE_OPEN_IF) || (Disposition == FILE_OVERWRITE_IF)) {
            // If the request was for any of the bits in the read access mask, then assume that this is a backup operation, and check for the Backup privilege.
            // If the caller has it, then grant the intersection of the desired access and read access masks.
            if (readAccess & desiredAccess) {
                ...
                if (accessGranted) {// The caller has Backup privilege, so grant the appropriate accesses.
        ...
        // If the request was for any of the bits in the write access mask, then assume that this is a restore operation, so check for the Restore privilege.
        // If the caller has it, then grant the intersection of the desired access and write access masks.
        if ((writeAccess & desiredAccess) || ForceRestoreCheck) {
            ...
            requiredPrivileges.Privilege[0].Attributes = 0;
            accessGranted = SePrivilegeCheck(&requiredPrivileges, &AccessState->SubjectSecurityContext, PreviousMode);
            if (accessGranted) {// The caller has Restore privilege, so grant the appropriate accesses.
                keepBackupIntent = TRUE;
                (VOID)SeAppendPrivileges(AccessState, &requiredPrivileges);
        ...
        // If either of the access types was granted because the caller had backup or restore privilege, then the backup intent flag is kept.
        // Otherwise, it is cleared so that it is not passed onto the driver so that it is not incorrectly propagated anywhere else, 
        // since this caller does not actually have the privilege enabled.
        if (!keepBackupIntent) {
