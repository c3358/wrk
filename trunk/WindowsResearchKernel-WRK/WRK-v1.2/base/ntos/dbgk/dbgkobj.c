...
// Non-pageable data
FAST_MUTEX DbgkpProcessDebugPortMutex;// This mutex protects the debug port object of processes.
...
VOID DbgkpCloseObject(IN PEPROCESS Process,
                      IN PVOID Object,
    ...
    */
{
    ...
    // Debug port propagation was disabled by setting the delete pending flag above so we only have to do this once.
    // No more refs can appear now.
...
*/
{
    ...
    if (Process->DebugPort != NULL) {
        ...
        if (DebugObject != NULL) {
            Status = ObOpenObjectByPointer(DebugObject, 0, NULL, MAXIMUM_ALLOWED, DbgkDebugObjectType, PreviousMode, pHandle);
            if (!NT_SUCCESS(Status)) {
                ObDereferenceObject(DebugObject);
            }
        }
...
/*
Routine Description:
    Creates a new debug object that maintains the context for a single debug session.
    Multiple processes may be associated with a single debug object.
Arguments:
...
*/
{
    ...
    if (DebugObject) {// Remove any events for this process and wake up the threads.
        InitializeListHead(&TempList);// Remove any events and queue them to a temporary queue
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        while (1) {
            // Acquire the debug port mutex so we know that any new threads will have to wait to behind us.
            GlobalHeld = TRUE;

            ExAcquireFastMutex(&DbgkpProcessDebugPortMutex);

            if (Process->DebugPort != NULL) {// If the port has been set then exit now.
                Status = STATUS_PORT_ALREADY_SET;
                break;
            }

            Process->DebugPort = DebugObject;// Assign the debug port to the process to pick up any new threads
            ObReferenceObject(LastThread);// Reference the last thread so we can deref outside the lock

            // Search forward for new threads
            Thread = PsGetNextProcessThread(Process, LastThread);
            if (Thread != NULL) {
                // Remove the debug port from the process as we are about to drop the lock
                Process->DebugPort = NULL;
                ExReleaseFastMutex(&DbgkpProcessDebugPortMutex);
                GlobalHeld = FALSE;
    ...
    ExAcquireFastMutex(&DebugObject->Mutex);// Lock the debug object so we can check its deleted status
    ...
    */
{
    ...
    for (Thread = StartThread; Thread != NULL; Thread = PsGetNextProcessThread(Process, Thread)) {
        ...
        // We use this as a starting point for new threads after we really attach so we can pick up any new threads.
        if (LastThread != NULL) {
            ObDereferenceObject(LastThread);
        }
        LastThread = Thread;
        ...
        // This stops the thread exiting so we know it can't send it's termination message
        if (ExAcquireRundownProtection(&Thread->RundownProtect)) {
            Flags |= DEBUG_EVENT_RELEASE;

            // Suspend the thread if we can for the debugger
            // We don't suspend terminating threads as we will not be giving details of these to the debugger.
            ...
            // This means the thread is exiting.
            // We will mark this thread later so it doesn't sent a thread termination message.
            ...
            for (LdrNext32 = (PVOID)UlongToPtr(LdrHead32->Flink), i = 0;
                ...
                if (i > 0) {
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        if (pSys != NULL) {
                            RtlCopyMemory(pSys + 1, WOW64_SYSTEM_DIRECTORY_U, sizeof(WOW64_SYSTEM_DIRECTORY_U) - sizeof(UNICODE_NULL));
                        }
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        if (ExAcquireRundownProtection(&Process->RundownProtect)) {// We will be touching process address space. Block process rundown.
            Status = DbgkpPostFakeProcessCreateMessages(Process, DebugObject, &LastThread);// Post the fake process create messages etc.
            Status = DbgkpSetProcessDebugObject(Process, DebugObject, Status, LastThread);// Set the debug port. If this fails it will remove any faked messages.
            ExReleaseRundownProtection(&Process->RundownProtect);
        } else {
...
*/
{
    ...
    case DbgCreateThreadStateChange:
        ...
        Status = ObOpenObjectByPointer(Thread,
                                       0,
                                       NULL,
                                       THREAD_GET_CONTEXT | THREAD_SET_CONTEXT | THREAD_SUSPEND_RESUME | THREAD_QUERY_INFORMATION | THREAD_SET_INFORMATION | THREAD_TERMINATE | READ_CONTROL | SYNCHRONIZE,
                                       PsThreadType,
                                       KernelMode,
    ...
    case DbgCreateProcessStateChange:
        ...
        Status = ObOpenObjectByPointer(Process,
                                       0,
                                       NULL,
                                       PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_DUP_HANDLE | PROCESS_QUERY_INFORMATION | PROCESS_SET_INFORMATION | PROCESS_CREATE_THREAD | PROCESS_TERMINATE | READ_CONTROL | SYNCHRONIZE,
                                       PsProcessType,
                                       KernelMode,
...
NTSTATUS NtWaitForDebugEvent(IN HANDLE DebugObjectHandle,
    ...
    */
{
    ...
    while (1) {
        ...
        if ((DebugObject->Flags & DEBUG_OBJECT_DELETE_PENDING) == 0) {
            ...
            } else {// No unread events there. Clear the event.
        ...
        if (NT_SUCCESS(Status)) {
            if (GotEvent == FALSE) {// If we woke up and found nothing
                if (Tmo.QuadPart < 0) {// If timeout is a delta time then adjust it for the wait so far.
                ...
                // Fixup needed handles. The caller could have guessed the thread id etc by now and made the target thread continue.
                // This isn't a problem as we won't do anything damaging to the system in this case.
                // The caller won't get the correct results but they set out to break us.
    ...
    for (Entry = DebugObject->EventList.Flink; Entry != &DebugObject->EventList; Entry = Entry->Flink) {
        ...
        if (DebugEvent->ClientId.UniqueProcess == Clid.UniqueProcess) {
            if (!GotEvent) {
                ...
                // VC breaks if it sees more than one event at a time for the same process.
                DebugEvent->Flags &= ~DEBUG_EVENT_INACTIVE;
                KeSetEvent(&DebugObject->EventsPresent, 0, FALSE);
                break;
            }
    ...
    */
{
    ...
    case DebugObjectFlags:
        ...
        Status = ObReferenceObjectByHandle(DebugObjectHandle, DEBUG_SET_INFORMATION, DbgkDebugObjectType, PreviousMode, &DebugObject, NULL);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
