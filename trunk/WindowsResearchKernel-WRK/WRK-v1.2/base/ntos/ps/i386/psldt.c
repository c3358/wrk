...
Notes:
    The nonpaged pool consumed by the LDT is returned to the system at process deletion time.
    The process deletion handler calls PspDeleteLdt.
    We do not keep a reference to the process once the ldt is created.

    Note that the LDT must be kept in nonpaged memory because the EXIT_ALL
    macros that return from traps and interrupts pop ds (which may be an LDT selector) and then other registers.
...
LOGICAL PspIsDescriptorValid(IN PLDT_ENTRY Descriptor);
...
NTSTATUS PspLdtInitialize(VOID)
/*
Routine Description:
...
*/
{
    ...
    // structure.  We subtract one LDT entry, because the structure is declared to contain one.
    ...
    if ((ProcessLdtInfo) && (ProcessLdtInfo->Size)) {
        ...
        //  the end of the information the user requested or the end of the information that is actually there
...
*/
{
    ...
    if (Length == 0) {
        ...
        if (Ldt == NULL) {
            // We cannot reduce the allocation, but we can reduce the LDT selector limit (done using Ke386SetLdtProcess)
            Ldt = OldLdt;
...
*/
{
    ...
    // Ensure that the buffer it large enough to contain the specified number of selectors.
    ...
    // If the process doen't have an LDT information structure, allocate one and attach it to the process
    ...
    if (LdtInfo->Length == 0) {
        ...
        if (LdtInfo->Length + LdtInfo->Start > ProcessLdtInfo->AllocatedSize) {
            // Current LDT allocation is not large enough, so create a new larger LDT
            OldSize = ProcessLdtInfo->AllocatedSize;
...
/*
Routine Description:
    This routine allocates space in nonpaged pool for an LDT, and copies the specified selectors into it.
...
NTSTATUS PsSetLdtEntries(IN ULONG Selector0,
                         IN ULONG Entry0Low,
                         IN ULONG Entry0Hi,
                         IN ULONG Selector1,
                         IN ULONG Entry1Low,
...
*/
{
    ...
    // will be large enough.  for this, we just set the descriptors and return
    if (ProcessLdtInformation) {
        ...
        if (ProcessLdtInformation->AllocatedSize >= LdtSize) {
            ...
            // We will then put the LDT back. 
            // Since we have to change the limit anyway, it would take two calls to the kernel ldt management minimum to set the descriptors. 
            // Each of those calls would stall all of the processors in an MP system. 
            // If we didn't remove the ldt first, and we were setting two descriptors,
            // we would have to call the LDT management 3 times (once per descriptor, and once to change the limit of the LDT).
    ...
    // If the process does not yet have an LDT information structure, allocate and attach one.
    OldLdt = NULL;
    ...
    // Now, we either need to create or grow an LDT, so allocate some memory, and copy as necessary
...
NTSTATUS NtSetLdtEntries(__in ULONG Selector0,
...
NTSTATUS PsSetProcessLdtInfo(IN PPROCESS_LDT_INFORMATION LdtInformation, IN ULONG LdtInformationLength)
/*
Routine Description:
    This function alters the ldt for a specified process.
    It can alter portions of the LDT, or the whole LDT.
...
*/
{
    ...
    // Ensure that the buffer is large enough to contain the specified number of selectors.
    if (LdtInformationLength - sizeof(PROCESS_LDT_INFORMATION) + sizeof(LDT_ENTRY) < LdtInfo->Length) {
        ExFreePool(LdtInfo);
        return STATUS_INFO_LENGTH_MISMATCH;
    }
    ...
    // If the process doesn't have an Ldt information structure, allocate one and attach it to the process
    if (ProcessLdtInfo == NULL) {
        ProcessLdtInfo = ExAllocatePoolWithTag(NonPagedPool, sizeof(LDTINFORMATION), 'dLsP');
        if (ProcessLdtInfo == NULL) {
            goto SetInfoCleanup;
