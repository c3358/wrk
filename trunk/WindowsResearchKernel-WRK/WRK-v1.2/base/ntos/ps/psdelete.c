...
NTSTATUS PspFreezeProcessWorker(PEPROCESS Process, PVOID Context);
...
ULONG PsSetBBTNotifyRoutine(__in PBBT_NOTIFY_ROUTINE BBTNotifyRoutine)
{
    PAGED_CODE();
...
/*
Routine Description:
    This routine implements the thread reaper.
    The reaper is responsible for processing terminated threads.
    This includes:
...
*/
{
    ...
    // Remove the current list of threads from the reaper list and place a marker in the reaper list head.
    // This marker will prevent another worker thread from being queued until all threads in the reaper list have been processed.
    do {
        NextEntry = InterlockedExchangePointer(&PsReaperListHead.Next, (PVOID)1);
        ASSERT((NextEntry != NULL) && (NextEntry != (PVOID)1));

        // Delete the respective kernel stack and dereference each thread in the reaper list.
        do {
    ...
    } while (InterlockedCompareExchangePointer(&PsReaperListHead.Next, NULL, (PVOID)1) != (PVOID)1);
}
...
*/
{
    ...
    if (DirectTerminate && Thread == PsGetCurrentThread()) {
        ...
        if ((OldMask & PS_CROSS_THREAD_FLAGS_TERMINATED) == 0) {// If we are the first to set the terminating flag then queue the APC
            ...
            } else {// We queued the APC to the thread. Wake up the thread if it was suspended.
...
*/
{
    ...
    // Do the same for processes being debugged. This is so a process can never lock itself into the system by debugging itself or have a handle open to itself.
...
NTSTATUS PsTerminateProcess(PEPROCESS Process, NTSTATUS Status)
{
...
*/
{
    ...
    // If there are no threads in this process then clear out its handle table.
    // Do the same for processes being debugged.
    // This is so a process can never lock itself into the system by debugging itself or have a handle open to itself.
...
NTSTATUS PspWaitForUsermodeExit(IN PEPROCESS Process)
/*
Routine Description:
...
*/
{
    ...
    if (!ARGUMENT_PRESENT(ThreadHandle)) {
        ...
        // This routine gets called this way first and if it returns the base code does an exit process call.
...
VOID PspExitNormalApc(IN PVOID NormalContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2)
{
    ...
    // We just queued a user APC to this thread. User APC won't fire until we do an alertable wait so we need to set this flag here.
...
*/
{
    ...
    PERFINFO_THREAD_DELETE(Thread);// Notify registered callout routines of thread deletion.
    ...
    if (Process->DebugPort != NULL) {// If we need to send debug messages then do so.
        if (!IS_SYSTEM_THREAD(Thread)) {// Don't report system thread exit to the debugger as we don't report them.
            if (LastThread) {
...
#if DBG
    ExCheckIfResourceOwned();// See if we are exiting while holding a resource
#endif
    ...
    if (LastThread) {
        ...
        if (Process->SectionObject != NULL) {
            ObDereferenceObject(Process->SectionObject);// Release the image section
    ...
    KeEnterCriticalRegionThread(&Thread->Tcb);// Rundown pending APCs. Protect against being frozen after we raise IRQL but before dispatcher lock is taken.
    Thread->Tcb.ApcQueueable = FALSE;// Disable APC queueing for the current thread.

    // At this point we may have been frozen and the APC is pending. First we remove the suspend/freeze bias that may exist and then drop IRQL.
    // The suspend APC if present will fire and drop through.
    // No further suspends are allowed as the thread is marked to prevent APC's
    KeForceResumeThread(&Thread->Tcb);
...
VOID PspExitProcess(IN BOOLEAN LastThreadExit, IN PEPROCESS Process)
{
    ...
    // Dereference (close) the security port.
    // This will stop any authentication or EFS requests from this process to the LSA process.
    // The "well known" value of 1 will prevent the security system from try to re-establish the connection during the process shutdown (e.g. when the rdr deletes a handle)
    if (Process->SecurityPort) {
        ...
        // Even if there have never been any requests to the LSA process, i.e. the pointer is NULL, set it to 1 anyway.
        // Filter drivers can apparently cause a network hop at this point.
        // This will prevent any such from deadlocking.
        Process->SecurityPort = (PVOID)1;
    }
    ...
    if (LastThreadExit) {
        ...
        if (CCPF_IS_PREFETCHER_ACTIVE()) {
            CcPfProcessExitNotification(Process);// Let prefetcher know that this process is exiting.
        }
    } else {
        MmCleanProcessAddressSpace(Process);
    }
...
VOID PspProcessDelete(IN PVOID Object)
{
    ...
    // Zero the GrantedAccess field so the system will not panic when this process is missing from the PsActiveProcess list but is still found in the CID table.
#if defined(_AMD64_)
    ...
    {
        // Free any alignment exception tracking structures that might have been around to support a user-mode debugger.
...
BOOLEAN PsIsThreadTerminating(__in PETHREAD Thread)
/*
Routine Description:
...
#define WAIT_BATCH THREAD_WAIT_OBJECTS
    ...
    // Killing all processes without letting any execute any usermode code prevents any unwanted initiated actions.
    Thread = PsGetCurrentThread();
    ...
    // This loop kills all the processes and then waits for one of a subset of them to die.
    // They must all be killed first (before any can be waited on) so that any process like a debuggee that is waiting for a debugger won't stall us.
    ...
    do {
        ...
        // ASSERT on checked builds so the faulty component causing this can be debugged and fixed.
    ...
    if (PoCleanShutdownEnabled() && ExpDefaultErrorPortProcess) {
        // Explicitly kill csrss -- we don't want to do this in the loop, because we don't want to wait on it, because it has system threads which will exit later.
        // But we can terminate the user threads,
        // now that everything else has died (we can't terminate them earlier, because DestroyWindowStation()/TerminateConsole() depends on them being around).
        PsTerminateProcess(ExpDefaultErrorPortProcess, STATUS_SYSTEM_SHUTDOWN);

        PspWaitForUsermodeExit(ExpDefaultErrorPortProcess);// Now, make sure that csrss's usermode threads have gotten a chance to terminate.
    }
...
BOOLEAN PsWaitForAllProcesses(VOID)
/*
Routine Description:
