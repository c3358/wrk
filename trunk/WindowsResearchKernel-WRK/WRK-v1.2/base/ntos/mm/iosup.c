...
VOID MiInsertPteTracker(IN PMDL MemoryDescriptorList,
                        IN ULONG Flags,
                        IN LOGICAL IoMapping,
                        IN MI_PFN_CACHE_ATTRIBUTE CacheAttribute,
                        IN PVOID MyCaller,
                        IN PVOID MyCallersCaller);
VOID MiRemovePteTracker(IN PMDL MemoryDescriptorList OPTIONAL, IN PVOID VirtualAddress, IN PFN_NUMBER NumberOfPtes);
LOGICAL MiReferenceIoSpace(IN PMDL MemoryDescriptorList, IN PPFN_NUMBER Page);
VOID MiZeroAwePageWorker(IN PVOID Context);
...
VOID MmLockPagableSectionByHandle(__in PVOID ImageSectionHandle);
VOID MmUnlockPagableImageSection(__in PVOID ImageSectionHandle);
...
//  Note: this should be > 2041 to account for the cache manager's aggressive zeroing logic.
...
Arguments:
    MemoryDescriptorList - Supplies a pointer to a Memory Descriptor List (MDL).
                           The supplied MDL must supply a virtual address, byte offset and length field.
...
Environment:
    Kernel mode.  APC_LEVEL and below for pageable addresses, DISPATCH_LEVEL and below for non-pageable addresses.
*/
{
    ULONG Processor;
    LOGICAL PfnHeldToo;
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
    ...
    if (Va <= MM_HIGHEST_USER_ADDRESS) {
        ...
        if (CurrentProcess->AweInfo != NULL) {
            ...
            // Block APCs to prevent recursive pushlock scenarios as this is not supported.
            KeEnterGuardedRegionThread(&Thread->Tcb);
            PushLock = ExAcquireCacheAwarePushLockShared(AweInfo->PushLock);
            // Provide a fast path for transfers that are within a single AWE region.
            ...
            if (PhysicalView->VadType == VadLargePages) {
                // The PTE cannot be referenced (it doesn't exist), 
                // but it serves the useful purpose of identifying when we cross PDEs and therefore must recompute the base PFN.
                PointerPte = MiGetPteAddress(StartVa);
...
DefaultProbeAndLock:
    ...
    if (Va <= MM_HIGHEST_USER_ADDRESS) {
        ...
        NumberOfPagesToLock = ADDRESS_AND_SIZE_TO_SPAN_PAGES(Va, MemoryDescriptorList->ByteCount);
        if (PointerPte == NULL) {
            // On certain architectures, virtual addresses may be physical and hence have no corresponding PTE.
            ...
            do {
                if (MI_IS_PFN(PageFrameIndex)) {
                    // Check to make sure each page is not locked down an unusually high number of times.
        ...
        // Since this operation is to a system address, no need to check for PTE write access below so mark the access as a read so only the
        // operation type (and not where the Va is) needs to be checked in the subsequent loop.
        Operation = IoReadAccess;
        LastPte = MiGetPteAddress((PCHAR)EndVa - 1);

        // A driver may build an MDL for a user address and then get a system VA for it.
        // The driver may then call MmProbeAndLockPages again, this time with the system VA (instead of the original user VA) for the same range.

        // Thus the PFNs will really point at PFNs which generally require the relevant process' working set pushlock, 
        // but in this instance we would only be holding the system working set pushlock.
        // Therefore the user can change PFN sharecounts and active states at any time.
        // The only thing we'd know for sure is that the refcount must be nonzero on entry and
        // will stay that way because it would be illegal for the driver to have called MmUnlockPages prior to this probe call.

        // However, this is not enough state to ensure we charge locking properly (without leaking) so the code that used to acquire the
        // system working set mutex here has been changed to instead always use the PFN lock to synchronize system space VA arguments.
        LockType = LOCK_TYPE_PFN;
        LOCK_PFN2(OldIrql);
    }

    do {
        // Set the current MDL entry to empty so all error paths can call MmUnlockPages.
        *Page = MM_EMPTY_LIST;
...
#endif
            ((PointerPde->u.Hard.Valid == 0) || (((MI_PDE_MAPS_LARGE_PAGE(PointerPde)) == 0) && (PointerPte->u.Hard.Valid == 0)))) {
            // The VA is not resident, release the PFN lock and access the page to make it appear.
        ...
        if (MI_PDE_MAPS_LARGE_PAGE(PointerPde)) {
            if ((Operation != IoReadAccess) && (PointerPde->u.Hard.Write == 0)) {
                if (LockType == LOCK_TYPE_WS) {
                    UNLOCK_WS_SHARED(Thread, CurrentProcess);
                } else {
                    ASSERT(LockType == LOCK_TYPE_PFN);
...
#if defined (_X86PAE_)
            // Pick up the PTE as an interlocked operation to prevent AWE updates from tearing our read on these platforms.
            PteContents.u.Long = InterlockedCompareExchangePte(PointerPte, BogusPte.u.Long, BogusPte.u.Long);
            ...
            // There is a subtle race here where the PTE contents can get zeroed by a thread running on another processor.
            // This can only happen for an AWE address space because these ranges (deliberately for performance reasons) do not acquire the PFN lock during remap operations.
            // In this case, one of 2 scenarios is possible - either the old PTE is read or the new.
            // The new may be a zero PTE if the map request was to invalidate *or* non-zero (and valid) if the map request was inserting a new entry.
            // For the latter, we don't care if we lock the old or new frame here as it's an application bug to provoke this behavior - and
            // regardless of which is used, no corruption can occur because the PFN lock is acquired during an NtFreeUserPhysicalPages.
            // But the former must be checked for explicitly here.
            // As a separate note, the PXE/PPE/PDE accesses above are always safe even for the AWE deletion race because these tables are never lazy-allocated for AWE ranges.
            if (PteContents.u.Hard.Valid == 0) {
            ...
            if (Operation != IoReadAccess) {
                if ((PteContents.u.Long & MM_PTE_WRITE_MASK) == 0) {
                    if (PteContents.u.Long & MM_PTE_COPY_ON_WRITE_MASK) {
                        // The protection has changed from writable to copy on write.
                        // This can happen if a fork is in progress for example.
                        // Restart the operation at the top.
                        Va = MiGetVirtualAddressMappedByPte(PointerPte);
                        if (Va <= MM_HIGHEST_USER_ADDRESS) {
                            if (LockType == LOCK_TYPE_WS) {
                                UNLOCK_WS_SHARED(Thread, CurrentProcess);
        ...
        if (MI_IS_PFN(PageFrameIndex)) {
            ...
            if ((CurrentProcess != NULL) && (CurrentProcess->PhysicalVadRoot != NULL)) {
                ...
                if ((SearchResult == TableFoundNode) && (PhysicalView->VadType == VadDevicePhysicalMemory)) {
                    ...
                    // Continue to hold the PFN lock until the reference count on the desired PFN is incremented.
                }
            }
            ...
            if (MI_ADD_LOCKED_PAGE_CHARGE_FOR_PROBE(Pfn1, FALSE, PointerPte) == FALSE) {
                ...
                if (MI_ADD_LOCKED_PAGE_CHARGE_FOR_PROBE(Pfn1, TRUE, PointerPte) == FALSE) {
                    // The only reason we don't force for privileged code/data is if it would have caused a reference count wrap.
                    // Typically this indicates a driver is forgetting to call MmUnlockPages as the page is locked down an unusually high number of times.
            ...
            if (CurrentProcess != NULL) {
                ...
                if ((SearchResult == TableFoundNode) &&
                    ...
                    ASSERT((ULONG)PhysicalView->Vad->u.VadFlags.VadType == (ULONG)PhysicalView->VadType);
                    // The range lies within a rotate physical VAD or a device physical memory VAD.

                    // We already know the access is ok because the PTE was already checked for read/write permissions above.
                    NOTHING;
                } else {
            ...
            if (((MemoryDescriptorList->MdlFlags & MDL_IO_SPACE) == 0) && (CurrentProcess != NULL)) {
    ...
    if (AlignedVa <= MM_HIGHEST_USER_ADDRESS) {
        // User space buffers that reside in I/O space need to be reference counted because SANs will want to reuse the physical space but cannot
        // do this unless it is guaranteed there are no more pending I/Os going from/to it.
        if (MemoryDescriptorList->MdlFlags & MDL_IO_SPACE) {
            ...
            if (EntryCounter != *(volatile PLONG)&MiActiveIoCounter) {
                ...
                if (MmTrackLockedPages == TRUE) {
                    ASSERT(NumberOfPagesSpanned != 0);
                    RtlGetCallersAddress(&CallingAddress, &CallersCaller);
                    MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesSpanned, 2);
                }
    ...
    if (MmTrackLockedPages == TRUE) {
        ASSERT(NumberOfPagesSpanned != 0);
        RtlGetCallersAddress(&CallingAddress, &CallersCaller);
        MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesSpanned, 3);
    }

    return;

FailureReleaseLocks:
    // A violation was detected.  Release the synchronization primitive we used and unlock any pages locked so far.
...
FailureUnlockAnyPages:
    ...
    if ((MmTrackLockedPages == TRUE) && ((MemoryDescriptorList->MdlFlags & MDL_DESCRIBES_AWE) == 0)) {
        // Compute NumberOfPagesToLock as MmUnlockPages will also when freeing the MDL tracker.
        NumberOfPagesToLock = ADDRESS_AND_SIZE_TO_SPAN_PAGES(StartVa, MemoryDescriptorList->ByteCount);
        RtlGetCallersAddress(&CallingAddress, &CallersCaller);
        MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesToLock, 5);
    }
...
*/
{
    ...
    // It's ok to check unsynchronized for aborted tracking as the worst case is just that one more entry gets added which will be freed later anyway.
    // The main purpose behind aborted tracking is that frees and exits don't mistakenly bugcheck when an entry cannot be found.
    ...
    if (Tracker == NULL) {
        // It's ok to set this without synchronization as the worst case is just that a few more entries gets added which will be freed later anyway.
        // The main purpose behind aborted tracking is that frees and exits don't mistakenly bugcheck when an entry cannot be found.
    ...
    // Update the list for this process.  First make sure it's not already inserted.
...
/*
Routine Description:
    This deletes an MDL from the specified process' chain.
    Used specifically by MmProbeAndLockSelectedPages () because it builds an MDL in its local stack and
    then copies the requested pages into the real MDL.
    This lets us track these pages.
Arguments:
...
LOGICAL MmUpdateMdlTracker(IN PMDL MemoryDescriptorList, IN PVOID CallingAddress, IN PVOID CallersCaller)
/*
Routine Description:
    This updates an MDL in the specified process' chain.
    Used by the I/O system so that proper driver identification can be done even when I/O is actually locking the pages on their behalf.
Arguments:
...
*/
{
    ...
    // Walk the list backwards as it's likely the MDL was just recently inserted.
    ...
    return FALSE;// The caller is trying to update an MDL that is no longer locked.
}


LOGICAL MiUpdateMdlTracker(IN PMDL MemoryDescriptorList, IN ULONG AdvancePages)
/*
Routine Description:
    This updates an MDL in the specified process' chain.
Arguments:
...
*/
{
    ...
    if (NumberOfPagesToLock > COPY_STACK_SIZE) {
        EntrySegment = ExAllocatePoolWithTag(NonPagedPool, NumberOfPagesToLock * sizeof(FILE_SEGMENT_ELEMENT), 'rPmM');
    ...
    // Capture the specified segment elements so we can safely access them while locks are held below.
    // This is because the caller's segment array can be in paged pool.
    RtlCopyMemory(SegmentArray, PagedSegmentArray, NumberOfPagesToLock * sizeof(FILE_SEGMENT_ELEMENT));
...
Top:
    ...
    ASSERT((MemoryDescriptorList->MdlFlags &
        (MDL_PAGES_LOCKED | MDL_MAPPED_TO_SYSTEM_VA | MDL_SOURCE_IS_NONPAGED_POOL | MDL_PARTIAL | MDL_IO_SPACE)) == 0);
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
        ...
        if (Va <= MM_HIGHEST_USER_ADDRESS) {
            ...
            if ((AweOk == TRUE) && (CurrentProcess->AweInfo != NULL)) {
                ...
                if ((PhysicalView != NULL) &&
                    ...
                    if ((SearchResult == TableFoundNode) &&
                        ...
                        if (Page == (PPFN_NUMBER)(MemoryDescriptorList + 1)) {
                            // This is the first page of the transfer.
                            // This process has AWE mappings, but this range is not in it.
                            // That's ok, just use the working set pushlock to process it normally.
                            goto DefaultProbeAndLock;
                        }

                        // Release the existing MDL's pages as they will have to be relocked without using the AWE mechanism.
                        MmUnlockPages(MemoryDescriptorList);
                        goto Top;
                    }
                }
                ...
                if (PhysicalView->VadType == VadAwe) {
                    ...
                    *Page = PageFrameIndex;
                    Page += 1;
                    continue;// Captured this page, march on to the next one.
                }
            ...
            if (AccessMode != KernelMode) {// The access mode must be kernel to reference kernel addresses.
                ASSERT(LockType != LOCK_TYPE_PFN);// Order the checks from hot to cool (lockwise).
                if (LockType == LOCK_TYPE_WS) {
                    UNLOCK_WS_SHARED(Thread, CurrentProcess);
    ...
    DefaultProbeAndLock:
        ...
        if (Va <= MM_HIGHEST_USER_ADDRESS) {
            ...
            if (LockedPagesCharged == FALSE) {
                InterlockedExchangeAddSizeT(&CurrentProcess->NumberOfLockedPages, NumberOfPagesToLock);
            ...
            // PTE write access below so mark the access as a read so only the operation type (and not where the Va is) needs to be checked in the subsequent loop.
            Operation = IoReadAccess;

            // A driver may build an MDL for a user address and then get a system VA for it.
            // The driver may then call MmProbeAndLockPages again, this time with the system VA (instead of the original user VA) for the same range.

            // Thus the PFNs will really point at PFNs which generally require the relevant process' working set pushlock, but in
            // this instance we would only be holding the system working set pushlock.
            // Therefore the user can change PFN sharecounts and active states at any time.
            // The only thing we'd know for sure is that the refcount must be nonzero on entry and will stay
            // that way because it would be illegal for the driver to have called MmUnlockPages prior to this probe call.
        ...
        if (MI_PDE_MAPS_LARGE_PAGE(PointerPde)) {
            ...
            // In this case, one of 2 scenarios is possible - either the old PTE is read or the new.
            // The new may be a zero PTE if the map request was to invalidate *or* non-zero (and valid) if the map request was inserting a new entry.
            // For the latter, we don't care if we lock the old or 
            // new frame here as it's an application bug to provoke this behavior - and regardless of which is used, no corruption can occur because
            ...
            if (Operation != IoReadAccess) {
                if ((PteContents.u.Long & MM_PTE_WRITE_MASK) == 0) {
                    ...
                    // The caller has made the page protection more restrictive, this should never be done once the
                    // request has been issued !  Rather than wading through the PFN database entry to see if it could possibly work out, 
                    // give the caller an access violation.
#if DBG
        ...
        // We have a PFN that is mapped by the caller.
        // If this is a user space mapping, check to ensure the frame is owned by the caller.
        // The only way this isn't TRUE is if the caller has a \Device\PhysicalMemory mapping for frames he doesn't own.
        if (MI_IS_PFN(PageFrameIndex)) {
            Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
            ASSERT((CurrentProcess == NULL) || (LockType == LOCK_TYPE_WS));
            PfnHeldToo = FALSE;
    ...
    if (Va <= MM_HIGHEST_USER_ADDRESS) {
        ...
        // counted because SANs will want to reuse the physical space but cannot do this unless it is guaranteed there are no more pending I/Os going from/to it.
        if (MemoryDescriptorList->MdlFlags & MDL_IO_SPACE) {
            ASSERT((MemoryDescriptorList->MdlFlags & MDL_DESCRIBES_AWE) == 0);
            if (MiReferenceIoSpace(MemoryDescriptorList, Page) == FALSE) {
            ...
            if (EntryCounter != *(volatile PLONG)&MiActiveIoCounter) {
                ...
                if (MmTrackLockedPages == TRUE) {
                    RtlGetCallersAddress(&CallingAddress, &CallersCaller);
                    MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesToLock, 6);
                }
    ...
    if ((MmTrackLockedPages == TRUE) && ((MemoryDescriptorList->MdlFlags & MDL_DESCRIBES_AWE) == 0)) {
        ASSERT(NumberOfPagesToLock != 0);
        RtlGetCallersAddress(&CallingAddress, &CallersCaller);
        MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesToLock, 7);
    }
...
FailureUnlockAnyPages:
    ...
    if ((MmTrackLockedPages == TRUE) && ((MemoryDescriptorList->MdlFlags & MDL_DESCRIBES_AWE) == 0)) {
        RtlGetCallersAddress(&CallingAddress, &CallersCaller);
        MiAddMdlTracker(MemoryDescriptorList, CallingAddress, CallersCaller, NumberOfPagesToLock, 8);
    }
    ...
    ExRaiseStatus(status);// Raise an exception of access violation to the caller.
}
...
*/
{
    ...
    if (Pfn1->u3.e2.ReferenceCount >= 2) {
        ...
        // This is the final dereference - the page was sitting in limbo (not on any list) waiting for this last I/O to complete.
...
LOGICAL MiReferenceIoSpace(IN OUT PMDL MemoryDescriptorList, IN PPFN_NUMBER Page)
/*
Routine Description:
    This routine reference counts physical pages which reside in I/O space when they are probed on behalf of a user-initiated transfer.
    These counts cannot be kept inside PFN database entries because there are no PFN entries for I/O space.

    These counts are kept because SANs will want to reuse the physical space but cannot do this unless it is guaranteed there are no more pending I/Os going from/to it.
    If the process has not exited, but the SAN driver has unmapped the views to its I/O space,
    it still needs this as a way to know that the application does not have a transfer in progress to the previously mapped space.
Arguments:
...
*/
{
    ...
    RtlCopyMemory((PVOID)Tracker, (PVOID)MemoryDescriptorList, MdlSize);
...
NTKERNELAPI LOGICAL MiDereferenceIoSpace(IN OUT PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine decrements the reference counts on physical pages which reside in I/O space that were previously probed on behalf of a user-initiated transfer.
    These counts cannot be kept inside PFN database entries because there are no PFN entries for I/O space.
...
LOGICAL MmIsIoSpaceActive(__in PHYSICAL_ADDRESS StartAddress, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    This routine returns TRUE if any portion of the requested range still has an outstanding pending I/O.
    It is the calling driver's responsibility to unmap all usermode mappings to the specified range (so another transfer cannot be initiated) prior to calling this API.
...
Arguments:
    StartAddress - Supplies the physical address of the start of the I/O range.
                   This MUST NOT be within system DRAM as those pages are not tracked by this structure.
    ...
    while (MemoryDescriptorList != NULL) {
        ...
        do {
            if (*Page == MM_EMPTY_LIST) {
                break;// There are no more locked pages.
            }
...
Routine Description:
    This routine returns TRUE if any portion of the requested range still has an outstanding pending I/O.
    These counts are kept because drivers will want to remap the physical space but cannot do this unless it is guaranteed there are no more pending I/Os going from/to it.
Arguments:
...
VOID MmUnlockPages(__inout PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine unlocks physical pages which are described by a Memory Descriptor List.
Arguments:
    MemoryDescriptorList - Supplies a pointer to a memory descriptor list (MDL).
                           The supplied MDL must have been supplied to MmLockPages to lock the pages down.
                           As the pages are unlocked, the MDL is updated.
Environment:
    Kernel mode, IRQL of DISPATCH_LEVEL or below.
*/
{
    ...
    // Carefully snap a copy of the MDL flags - realize that bits in it may change due to some of the subroutines called below.
    // Only bits that we know can't change are examined in this local copy.
    // This is done to reduce the amount of processing while the PFN lock is held.
    MdlFlags = MemoryDescriptorList->MdlFlags;
    if (MdlFlags & MDL_MAPPED_TO_SYSTEM_VA) {
        MmUnmapLockedPages(MemoryDescriptorList->MappedSystemVa, MemoryDescriptorList);// This MDL has been mapped into system space, unmap now.
    }
    ...
    if (MdlFlags & MDL_DESCRIBES_AWE) {
        ...
        LastPage = Page + NumberOfPages;
        // Note neither AWE nor PFN locks are needed for unlocking these MDLs in all but the very rare cases (see below).
        do {
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
            if (*Page == MM_EMPTY_LIST) {
                // There are no more locked pages - if there were none at all then we're done.
                break;
            }
            ...
            do {
                ...
                ASSERT(Pfn1->u4.AweAllocation == 1);
                ASSERT(Pfn1->u3.e2.ReferenceCount != 0);
                OriginalCount = InterlockedCompareExchange(&Pfn1->AweReferenceCount, EntryCount - 1, EntryCount);
                if (OriginalCount == EntryCount) {
                    ...
                    if (OriginalCount == 1) {
                        ...
                        // This path should be rare since typically I/Os complete before these types of pages are freed by the app.
    ...
    if ((MdlFlags & MDL_IO_SPACE) == 0) {
        ...
        if (MdlProcess != NULL) {
            ...
            InterlockedExchangeAddSizeT(&MdlProcess->NumberOfLockedPages, 0 - NumberOfPages);
            // See if this was a transfer from user space and we are in the initiating process context.
            // If so, set Thread to indicate the fastpath (using the working set pushlock instead of the PFN lock) should be used.
            if (((MdlFlags & MDL_WRITE_OPERATION) == 0) && (KeGetCurrentIrql() == PASSIVE_LEVEL)) {
        ...
        // Calculate PFN addresses and termination without the PFN lock (it's not needed for this) to reduce PFN lock contention.
        ...
        // If this was a transfer from user space and we are in the initiating process context then take advantage of this fact
        // and unlock the pages using the process' working set pushlock instead of the PFN lock.
        if (CurrentThread != NULL) {
            ...
            // If all the pages are still valid in the process' address space then process the pages directly here without the PFN lock.
            // Note if any of the pages are no longer valid in this process' address space then the entire MDL must be processed under the PFN lock.
            do {
                Pfn1 = (PMMPFN)(*Page);
                if (Pfn1->u2.ShareCount >= 1) {
                    if (Pfn1->u3.e1.PrototypePte == 0) {
                        // This page is still mapped and it's private so it must be mapped in this process.
                        NOTHING;
                    } else if (CurrentProcess != MdlProcess) {
                        // This is a shared page so the working set pushlock is not sufficient - the PXE/PPE/PDE/PTE mappings must be checked.

                        // Since we are not in the address space of the MDL's initiating process, it's not worth the cost of attaching to interrogate.
                        // Just go the expensive (PFN) way instead.
                        UNLOCK_WS_SHARED_CROSS_PROCESS(CurrentThread, MdlProcess);
                        CurrentThread = NULL;
                        break;
                    } else {
                        // This is a sharable page so the current process' working set pushlock is not enough to check the PFN contents.
                        // The PXE/PPE/PDE/PTE hierarchy must be checked to ensure the user has not deleted (and replaced) the original virtual address space.
                        PVOID TempVa;
                        PFN_NUMBER PageFrameIndex;
#if (_MI_PAGING_LEVELS>=4)
                        PMMPTE PointerPxe;
                        ...
                        TempVa = (PVOID)((PCHAR)MemoryDescriptorList->StartVa + ((Page - (PPFN_NUMBER)(MemoryDescriptorList + 1)) << PAGE_SHIFT));
        ...
        // If the MDL can be queued so the PFN acquisition/release can be amortized then do so.
        if (NumberOfPages <= MI_MAX_DEREFERENCE_CHUNK) {
#if defined(MI_MULTINODE)
            PKNODE Node = KeGetCurrentNode();
            // The node may change beneath us but that should be fairly infrequent and not worth checking for.
            // Just make sure the same node that gives us a free entry gets the deferred entry back.
...
#if defined (_WIN64)
                // Avoid the majority of the high cost of RtlCopyMemory on 64 bit platforms.
        ...
        if (MdlFlags & MDL_WRITE_OPERATION) {
            LOCK_PFN2(OldIrql);
            do {
                // If this was a write operation set the modified bit in the PFN database.
                Pfn1 = (PMMPFN)(*Page);
        ...
        if (NumberOfPages <= MI_MAX_DEREFERENCE_CHUNK) {
            // The only reason this code path is being reached is because a deferred entry was not available so clear the list now.
...
/*
Routine Description:
    This routine unlocks physical pages which were previously described by a Memory Descriptor List.
Arguments:
    Flags - Supplies a bitfield of the caller's needs :
        MI_DEFER_PFN_HELD - Indicates the caller holds the PFN lock on entry.
        MI_DEFER_DRAIN_LOCAL_ONLY - Indicates the caller only wishes to drain the current processor's queue.
                                    This only has meaning in NUMA systems.
Environment:
...
*/
{
    ...
    do {
        ...
        do {
...
#if DBG
                // Mark the entry as processed so if it mistakenly gets reprocessed, we will assert above.
            ...
            if (InterlockedCompareExchangePointer(PfnDeferredList, NULL, FirstEntry) == FirstEntry) {
...
#if !defined(MI_MULTINODE)
    InterlockedPushListSList(PfnDereferenceSListHead, FirstEntry, VeryLastEntry, ListCount);// If possible, push the processed chain after releasing the PFN lock.
#endif
}


VOID MmBuildMdlForNonPagedPool(__inout PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine fills in the "pages" portion of the MDL using the PFN numbers corresponding to the buffers which reside in non-paged pool.

    Unlike MmProbeAndLockPages, there is no corresponding unlock as no reference counts are incremented as the buffers being in nonpaged pool are always resident.
Arguments:
    MemoryDescriptorList - Supplies a pointer to a Memory Descriptor List (MDL).
                           The supplied MDL must supply a virtual address, byte offset and length field.
                           The physical page portion of the MDL is updated when the pages are locked in memory.
                           The virtual address must be within the non-paged portion of the system space.
Environment:
    Kernel mode, IRQL of DISPATCH_LEVEL or below.
*/
{
    ...
    NumberOfPages = ADDRESS_AND_SIZE_TO_SPAN_PAGES(MemoryDescriptorList->MappedSystemVa, MemoryDescriptorList->ByteCount);
    ASSERT(NumberOfPages != 0);
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
    ...
    ASSERT((MmIsNonPagedSystemAddressValid(MemoryDescriptorList->StartVa)) || (MmAreMdlPagesLocked(MemoryDescriptorList)));
    MemoryDescriptorList->MdlFlags |= MDL_SOURCE_IS_NONPAGED_POOL;

    // Assume either all the frames are in the PFN database (ie: the MDL maps pool) or none of them (the MDL maps dualport RAM) are.
    if (!MI_IS_PFN(PageFrameIndex)) {
        MemoryDescriptorList->MdlFlags |= MDL_IO_SPACE;
    }
}
...
*/
{
    ...
    case 0:
        // Regular MDL mapping.
        StartingVa = (PVOID)((PCHAR)MemoryDescriptorList->StartVa + MemoryDescriptorList->ByteOffset);
        NumberOfPtes = ADDRESS_AND_SIZE_TO_SPAN_PAGES(StartingVa, MemoryDescriptorList->ByteCount);
        Tracker->Mdl = MemoryDescriptorList;
        Tracker->StartVa = MemoryDescriptorList->StartVa;
        Tracker->Offset = MemoryDescriptorList->ByteOffset;
        Tracker->Length = MemoryDescriptorList->ByteCount;
...
/*
Routine Description:
    This function removes a PTE tracking block from the lists as the PTEs are being freed.
Arguments:
    MemoryDescriptorList - Supplies a valid Memory Descriptor List.
...
*/
{
    ...
    while (NextEntry != &MiPteHeader.ListHead) {
        ...
        if (VirtualAddress == PAGE_ALIGN(Tracker->SystemVa)) {
            if (LastFound != NULL) {
                // Duplicate map entry.
                KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x1, (ULONG_PTR)Tracker, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)LastFound);
            }
            ...
            if ((ARGUMENT_PRESENT(MemoryDescriptorList)) &&
                ...
                if (Tracker->SystemVa != MemoryDescriptorList->MappedSystemVa) {// Not unmapping the same address that was mapped.
                ...
                if (Tracker->Page != *(PPFN_NUMBER)(MemoryDescriptorList + 1)) {// The first page in the MDL has changed since it was mapped.
                    KeBugCheckEx(SYSTEM_PTE_MISUSE,
                                 0x4,
                                 (ULONG_PTR)Tracker,
    ...
    if ((LastFound == NULL) && (MiTrackPtesAborted == FALSE)) {
        ...
        KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x6, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)VirtualAddress, (ULONG_PTR)NumberOfPtes);
    }
    ...
    // Insert the tracking block into the dead PTE list for later release.
    // Locks (including the PFN lock) may be held on entry, thus the block cannot be directly freed to pool at this time.
    if (LastFound != NULL) {
        InterlockedPushEntrySList(&MiDeadPteTrackerSListHead, (PSLIST_ENTRY)LastFound);
    }
}
...
/*
Routine Description:
    This function examines the PTE tracking blocks and returns the biggest consumer.
Arguments:
    None.
Return Value:
...
*/
{
    ...
    *NumberOfPtes = 0;
    ...
    // Walk the loaded module list backwards so we do the kernel last as the bit bucket for everyone else.
    ...
    while (NextEntry != &PsLoadedModuleList) {
        ...
        while (NextEntry2 != &MiPteHeader.ListHead) {
            ...
            if (Tracker->Matched == 1) {
                ...
                // See if the caller's caller can be charged instead.
                // This way drivers that call MmMapLockedPages (which in turn calls MmMapLockedPagesSpecifyCache where the charging is done)
                // don't get their allocations incorrectly logged against the kernel.
...
*/
{
    ...
    Tracker = ExAllocatePoolWithTag(NonPagedPool, sizeof(MMIO_TRACKER), 'ySmM');
    if (Tracker == NULL) {
        return MiNotMapped;
    }
    ...
    while (NextEntry != &MmIoHeader) {
        Tracker2 = (PMMIO_TRACKER)CONTAINING_RECORD(NextEntry, MMIO_TRACKER, ListEntry.Flink);
        if ((Tracker->PageFrameIndex < Tracker2->PageFrameIndex + Tracker2->NumberOfPages) &&
...
#if DBG
            if ((MmShowMapOverlaps & 0x1) || ((Tracker->CacheAttribute != Tracker2->CacheAttribute) && (MmShowMapOverlaps & 0x2))) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MM: Iospace mapping overlap %p %p\n", Tracker, Tracker2);
                ...
                DbgPrintEx(DPFLTR_MM_ID,
                           ...
                           (Tracker->PageFrameIndex + Tracker->NumberOfPages) << PAGE_SHIFT, MiCacheStrings[Tracker->CacheAttribute],
                           Tracker->BaseVa);
            ...
            // This would be a very strange driver bug and is already caught by the verifier anyway.
        }
...
VOID MiRemoveIoSpaceMap(IN PVOID BaseVa, IN PFN_NUMBER NumberOfPages)
/*
Routine Description:
...
*/
{
    ...
    NextEntry = MmIoHeader.Flink;
    while (NextEntry != &MmIoHeader) {
        Tracker = (PMMIO_TRACKER)CONTAINING_RECORD(NextEntry, MMIO_TRACKER, ListEntry.Flink);
        if ((PAGE_ALIGN(Tracker->BaseVa) == AlignedVa) && (Tracker->NumberOfPages == NumberOfPages)) {
            RemoveEntryList(NextEntry);
    ...
    // Can't unmap something that was never (or isn't currently) mapped.
    KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x400, (ULONG_PTR)BaseVa, (ULONG_PTR)NumberOfPages, 0);
}
...
*/
{
    ...
    case MiWriteCombined:
        ...
        break;
    case MiNotMapped:
        // This better be for a page allocated with MmAllocatePagesForMdl.
        // Otherwise it might be a page on the freelist which could subsequently be given out with a different attribute !
        ASSERT((Pfn1->u4.PteFrame == MI_MAGIC_AWE_PTEFRAME) || (Pfn1->PteAddress == (PVOID)(ULONG_PTR)(X64K | 0x1)));
        LOCK_PFN2(OldIrql);
...
PVOID MmMapLockedPages(__in PMDL MemoryDescriptorList, __in KPROCESSOR_MODE AccessMode)
/*
Routine Description:
    This function maps physical pages described by a memory descriptor list into the system virtual address space or
    the user portion of the virtual address space.
Arguments:
    ...
    AccessMode - Supplies an indicator of where to map the pages;
                 KernelMode indicates that the pages should be mapped in the system part of the address space,
                 UserMode indicates the pages should be mapped in the user part of the address space.
Return Value:
...
Environment:
    Kernel mode.  DISPATCH_LEVEL or below if access mode is KernelMode, APC_LEVEL or below if access mode is UserMode.
*/
{
...
/*
Routine Description:
    This routine unmaps a single locked page which was previously mapped via an MiMapSinglePage call.
Arguments:
...
Environment:
    ...
    Since the process working set pushlock may be held by callers, this routine is nonpaged.
*/
{
    PMMPTE PointerPte;
...
__bcount(NumberOfBytes) PVOID MmAllocateMappingAddress(__in SIZE_T NumberOfBytes, __in ULONG PoolTag)
/*
Routine Description:
...
*/
{
    ...
    *(PULONG_PTR)PointerPte = (PoolTag & ~0x1);
    ...
    if (MmTrackPtes & 0x1) {
        ...
        Page = (PPFN_NUMBER)(MemoryDescriptorList + 1);
        *Page = 0;
        MiInsertPteTracker(MemoryDescriptorList, 2, FALSE, MiCached, CallingAddress, CallersCaller);
    }
...
VOID MmFreeMappingAddress(__in PVOID BaseAddress, __in ULONG PoolTag)
/*
Routine Description:
    This routine unmaps a virtual address range previously reserved with MmAllocateMappingAddress.
Arguments:
...
*/
{
    ...
    // Note the tag and size are nulled out when the PTEs are released below so any drivers that try to use their mapping after freeing it get caught immediately.
    MiReleaseSystemPtes(PointerBase, (ULONG)NumberOfPages + 2, SystemPteSpace);
}


PVOID MmMapLockedPagesWithReservedMapping(__in PVOID MappingAddress,
                                          __in ULONG PoolTag,
                                          __in PMDL MemoryDescriptorList,
                                          __in MEMORY_CACHING_TYPE CacheType)
    /*
    Routine Description:
        This function maps physical pages described by a memory descriptor list into the system virtual address space.
    Arguments:
        MappingAddress - Supplies a valid mapping address obtained earlier via MmAllocateMappingAddress.
        PoolTag - Supplies the caller's identifying tag.
        MemoryDescriptorList - Supplies a valid Memory Descriptor List which has been updated by MmProbeAndLockPages.
        CacheType - Supplies the type of cache mapping to use for the MDL.
                    MmCached indicates "normal" kernel or user mappings.
    ...
    Environment:
        Kernel mode.  DISPATCH_LEVEL or below.  The caller must synchronize usage of the argument virtual address space.
    */
{
    KIRQL OldIrql;
    CSHORT IoMapping;
    ...
    while (PointerPte < LastPte) {
        if (PointerPte->u.Long != 0) {
            KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x107, (ULONG_PTR)MappingAddress, (ULONG_PTR)PointerPte, (ULONG_PTR)LastPte);
        }
    ...
    // If a noncachable mapping is requested, none of the pages in the requested MDL can reside in a large page.
    // Otherwise we would be creating an incoherent overlapping TB entry as the same physical page would be mapped by 2 different TB entries with different cache attributes.
    ...
    do {
        ...
        if ((IoMapping == 0) || (MI_IS_PFN(*Page))) {
            ...
            case MiNotMapped:
                // This better be for a page allocated with MmAllocatePagesForMdl.
                // Otherwise it might be a page on the freelist which could subsequently be given out with a different attribute !
                ASSERT((Pfn2->u4.PteFrame == MI_MAGIC_AWE_PTEFRAME) || (Pfn2->PteAddress == (PVOID)(ULONG_PTR)(X64K | 0x1)));
...
/*
Routine Description:
    This routine unmaps locked pages which were previously mapped via a MmMapLockedPagesWithReservedMapping call.
Arguments:
...
#if DBG
        ASSERT((*Page == MI_GET_PAGE_FRAME_FROM_PTE(PointerPte)) || (MemoryDescriptorList->MdlFlags & MDL_FREE_EXTRA_PTES));
...
NTKERNELAPI NTSTATUS MmAdvanceMdl(__inout PMDL Mdl, __in ULONG NumberOfBytes)
/*
Routine Description:
    This routine takes the specified MDL and "advances" it forward by the specified number of bytes.
    If this causes the MDL to advance past the initial page,
    the pages that are advanced over are immediately unlocked and the system VA that maps the MDL is also adjusted (along with the user address).

    WARNING !  WARNING !  WARNING !

    This means the caller MUST BE AWARE that the "advanced" pages are immediately reused and therefore MUST NOT BE REFERENCED by the caller once this routine has been called.
    Likewise the virtual address as that is also being adjusted here.

    Even if the caller has statically allocated this MDL on his local stack,
    he cannot use more than the space currently described by the MDL on return from this routine unless he first unmaps the MDL (if it was mapped).
...
*/
{
    ...
    if (Mdl->ByteOffset != 0) {
        ...
        if (NumberOfBytes < Slush) {
            ...
            // StartVa never includes the byte offset (it's always page-aligned) so don't adjust it here.
            // MappedSystemVa does include byte offsets so do adjust that.
    ...
    // The MDL start is now nicely page aligned.
    // Make sure there's still data left in it (we may have finished it off above), then operate on it.
    if (NumberOfBytes != 0) {
        Mdl->ByteCount -= NumberOfBytes;
    ...
    if (PageCount != 0) {
        // Slide the page frame numbers forward decrementing reference counts on the ones that are released.
        // Then adjust the mapped system VA (if there is one) to reflect the current frame.
        // Note that the TB does not need to be flushed due to the careful sliding and when the MDL is finally completely unmapped,
        // the extra information added to the MDL here is used to free the entire original PTE mapping range in one chunk so as not to fragment the PTE space.
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
                ...
                do {
                    ...
                    if (OriginalCount == EntryCount) {
                        // This thread can be racing against other threads also calling MmUnlockPages and also a thread calling NtFreeUserPhysicalPages.
                        // All threads can safely do interlocked decrements on the "AWE reference count".
                        // Whichever thread drives it to zero is responsible for decrementing the actual PFN reference count (which may be greater than 1 due to other non-AWE API calls being used on the same page).
                        // The thread that drives this reference count to zero must put the page on the actual freelist at that time and decrement various resident available and commitment counters also.
                        if (OriginalCount == 1) {
                            // This thread has driven the AWE reference count to zero so it must initiate a decrement of the PFN reference count (while holding the PFN lock), etc.
            ...
            for (i = 0; i < PageCount; i += 1) {
                // Decrement the stale page frames now, this will unlock them resulting in them being immediately reused if necessary.
            ...
            if (Process != NULL) {
                if ((MdlFlags & MDL_PAGES_LOCKED) && ((MdlFlags & MDL_IO_SPACE) == 0)) {
                    ...
                    InterlockedExchangeAddSizeT(&Process->NumberOfLockedPages, 0 - (PFN_NUMBER)PageCount);
                }
        ...
        if (MdlFlags & MDL_MAPPED_TO_SYSTEM_VA) {
            ...
            if (MdlFlags & MDL_FREE_EXTRA_PTES) {
                // This MDL has already been advanced at least once.  Any PTEs from those advancements need to be preserved now.
...
NTKERNELAPI NTSTATUS MmProtectMdlSystemAddress(__in PMDL MemoryDescriptorList, __in ULONG NewProtect)
/*
Routine Description:
    This function protects the system address range specified by the argument Memory Descriptor List.
    Note the caller must make this MDL mapping readwrite before finally freeing (or reusing) it.
Arguments:
...
Environment:
    Kernel mode, IRQL DISPATCH_LEVEL or below.  The caller is responsible for synchronizing access to this routine.
*/
{
    KIRQL OldIrql;
    PVOID BaseAddress;
    ...
    // On some architectures, we may use a large or KSEG-based MDL mapping which must always be READWRITE, so disallow any changes to these.
    ...
    if ((ProtectionMask == MM_INVALID_PROTECTION) || (MI_IS_GUARD(ProtectionMask)) || (MI_IS_NOCACHE(ProtectionMask)) ||
        (MI_IS_WRITECOMBINE(ProtectionMask)) || (ProtectionMask == MM_WRITECOPY) || (ProtectionMask == MM_EXECUTE_WRITECOPY)) {
    ...
    // Initializing Map is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    Map = NULL;
    ...
    while (NumberOfPages != 0) {
        ...
        if (ProtectionMask == MM_NOACCESS) {
            ...
            MI_MAKE_VALID_KERNEL_PTE(PteContents, PageFrameIndex, ProtectionMask, PointerPte);
            if (ProtectionMask & MM_READWRITE) {
                MI_SET_PTE_DIRTY(PteContents);
            }

            // Extract cache type from the original PTE so it can be preserved.
            // Note that since we only allow protection changes (not caching attribute changes), 
            // there is no need to flush or sweep TBs on insertion below.
...
/*
Routine Description:
    This function determines whether the faulting virtual address lies within the non-writable alternate system PTE mappings.
Arguments:
    StoreInstruction - Supplies nonzero if the operation causes a write into memory, zero if not.
...
*/
{
    ...
    // If PTE mappings with various protections are active and the faulting address lies within these mappings, 
        ...
        if ((VirtualAddress >= MapEntry->SystemVa) && (VirtualAddress < MapEntry->SystemEndVa)) {
            ...
            if (StoreInstruction != 0) {
                if ((ProtectionCode & MM_READWRITE) == 0) {
                    KeBugCheckEx(ATTEMPTED_WRITE_TO_READONLY_MEMORY, (ULONG_PTR)VirtualAddress, (ULONG_PTR)PointerPte->u.Long, 0, 16);
                }
            }

            MI_NO_FAULT_FOUND(StoreInstruction, PointerPte, VirtualAddress, FALSE);

            return TRUE;// Fault was handled directly here, no need for the caller to do anything.
        }
        NextEntry = NextEntry->Flink;
    }
...
/*
Routine Description:
    This function creates a physical VAD AVL root table for the specified process.
Arguments:
...
*/
{
    ...
    // The address space mutex synchronizes the allocation of the EPROCESS PhysicalVadRoot.
    // This table root is not deleted until the process exits.
    if (Process->PhysicalVadRoot == NULL) {
        PhysicalVadRoot = (PMM_AVL_TABLE)ExAllocatePoolWithTag(NonPagedPool, sizeof(MM_AVL_TABLE), MI_PHYSICAL_VIEW_ROOT_KEY);
        ...
        // Synchronize with concurrent MmProbeAndLockPages callers by acquiring the working set mutex.
        if (WsHeld == FALSE) {
...
*/
{
    ...
    if (Vad->u.VadFlags.VadType == VadRotatePhysical) {
        ...
        PagesRequired = MiResidentPagesForSpan(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn));
...
Arguments:
    ...
    BaseVa - Supplies the base address of the view.
             If the initial value of this argument is not null,
             then the view will be allocated starting at the specified virtual address rounded down to the next 64kb address boundary.
             If the initial value of this argument is null,
             then the operating system will determine where to allocate the view.
Return Value:
    ...
    This routine will raise an exception if quota limits or VM limits are exceeded.
Environment:
    Kernel mode.  APC_LEVEL or below.
*/
{
    ...
    // Map the pages into the user part of the address as user read/write no-delete.
    ...
    // Make sure the specified starting and ending addresses are within the user part of the virtual address space.
    if (BaseVa != NULL) {
        if (BYTE_OFFSET(BaseVa) != 0) {
            Status = STATUS_INVALID_ADDRESS;// Invalid base address.
            goto ErrorReturn;
        }
        ...
        if ((EndingAddress <= BaseVa) || (EndingAddress > MM_HIGHEST_VAD_ADDRESS)) {
            Status = STATUS_INVALID_ADDRESS;// Invalid region size.
            goto ErrorReturn;
        }
    ...
    // The PhysicalVadRoot table root is not deleted until the process exits.
    if ((Process->PhysicalVadRoot == NULL) && (MiCreatePhysicalVadRoot(Process, FALSE) == NULL)) {
        UNLOCK_ADDRESS_SPACE(Process);
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto ErrorReturn;
    }
    ...
    // The VAD has been inserted, but the physical view descriptor cannot be until the page table page hierarchy is in place.
    // This is to prevent races with probes.
    ...
    do {
        ...
        // A PTE just went from not present, not transition to present.
        // The share count and valid count must be updated in the page table page which contains this PTE.
    ...
    // Insert the physical view descriptor now that the page table page hierarchy is in place.
    // Note probes can find this descriptor immediately once the working set mutex is released.
    ASSERT(Process->PhysicalVadRoot != NULL);
...
*/
{
    ...
    // This was mapped into the user portion of the address space and the corresponding virtual address descriptor must be deleted.
    ...
    // Get the PFN lock so we can safely decrement share and valid counts on page table pages.
    LOCK_PFN(OldIrql);
    do {
        if (*Page == MM_EMPTY_LIST) {
            break;
        ...
        if ((MiIsPteOnPdeBoundary(PointerPte)) || (NumberOfPages == 0)) {
            ...
            if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
                ...
                MiDeletePte(PointerPde, TempVa, FALSE, Process, NULL, NULL, OldIrql);
                ...
                if ((MiIsPteOnPpeBoundary(PointerPte)) || (NumberOfPages == 0)) {
                    ...
                    if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
                        ...
                        MiDeletePte(PointerPpe, TempVa, FALSE, Process, NULL, NULL, OldIrql);
                        ...
                        if ((MiIsPteOnPxeBoundary(PointerPte)) || (NumberOfPages == 0)) {
                            ...
                            if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
                                ...
                                MiDeletePte(PointerPxe, TempVa, FALSE, Process, NULL, NULL, OldIrql);
                            }
                        }
#endif
                    }
...
Environment:
    Kernel mode, INIT time.  Resident available pages are not yet initialized, but everything else is.
*/
{
#if (_MI_PAGING_LEVELS>=3)
    ULONG PageColor;
...
#else
    // Initialize process tracking so that large page system PTE mappings can be rippled during creation/deletion.
...
Routine Description:
    ...
    If the range cannot be mapped using large TB entries then NULL is returned and the caller will map it with small TB entries.
Arguments:
    PageFrameIndex - Supplies the starting page frame index to map.
    NumberOfPages - Supplies the number of pages to map.
    Protection - Supplies the number of pages to map.
    CacheType - Supplies MmNonCached if the physical address is to be mapped as non-cached, MmCached if the address should be cached, and
                MmWriteCombined if the address should be cached and write-combined as a frame buffer which is to be used only by the video port driver.
                All other callers should use MmUSWCCached.
                MmUSWCCached is available only if the PAT feature is present and available.
...
*/
{
    ...
    StartPosition = RtlFindClearBitsAndSet(&MiLargeVaBitMap, NumberOfBits, 0);
    ...
    if (IoMapping) {
        CacheAttribute = MiInsertIoSpaceMap(BaseVa, PageFrameIndex, NumberOfPages, CacheAttribute);
...
#else
    PointerPde = MiReserveAlignedSystemPtes((ULONG)NumberOfPages, SystemPteSpace, MM_MINIMUM_VA_FOR_LARGE_PAGE);
    if (PointerPde == NULL) {
        return NULL;
    }
    ...
    if (IoMapping) {
        CacheAttribute = MiInsertIoSpaceMap(BaseVa, PageFrameIndex, NumberOfPages, CacheAttribute);
        if (CacheAttribute == MiNotMapped) {
            MiReleaseSystemPtes(PointerPde, NumberOfPages, SystemPteSpace);
            return NULL;
        }
    }
    ...
    MI_MAKE_VALID_PTE(TempPde, PageFrameIndex, Protection, PointerPde);
    ...
    while (NextEntry != &MmProcessList) {
        ...
        // 1.  Processes that are just being created where they are still initializing their page directory, etc.
        // 2.  Processes that are outswapped.
        if (Process->Flags & PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED) {
            // The process is further along in process creation or is still outswapped.
            // Either way, an update is already queued so our current changes will be processed later anyway before the process can run so no need to do anything here.
            NOTHING;
        } else if (Process->Pcb.DirectoryTableBase[0] == 0) {
            // This process is being created and there is no way to tell where during creation it is (ie: it may be filling PDEs right now!).
            // So just mark the process as needing a PDE update at the beginning of MmInitializeProcessAddressSpace.
            PS_SET_BITS(&Process->Flags, PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED);
        } else if (Process->Flags & PS_PROCESS_FLAGS_OUTSWAPPED) {
            // This process is outswapped.
            // Even though the page directory may still be in transition,
            // the process must be inswapped before it can run again, so just mark the process as needing a PDE update at that time.
            PS_SET_BITS(&Process->Flags, PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED);
        } else {
            // This process is resident so update the relevant PDEs in its address space right now.
            ...
            // The range cannot cross PAE PDPTE entries, but we do need to locate which entry it does lie in.
            ...
            TargetPdeBase = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
    ...
    MI_FLUSH_ENTIRE_TB(3);// Force all processors to use the latest mappings.
    return BaseVa;
}
...
/*
Routine Description:
    This function unmaps a range of physical addresses which were previously mapped via MiMapWithLargePages.
Arguments:
...
Environment:
    Kernel mode, Should be IRQL of APC_LEVEL or below, but unfortunately callers are coming in at DISPATCH_LEVEL and
...
*/
{
    ...
    while (NextEntry != &MmProcessList) {
        ...
        if (Process->Flags & PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED) {
            ...
            // This process is outswapped.
            // Even though the page directory may still be in transition, the process must be inswapped before it can run again, 
            // so just mark the process as needing a PDE update at that time.
            PS_SET_BITS(&Process->Flags, PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED);
        } else {
            // This process is resident so update the relevant PDEs in its address space right now.
            PointerPde = PointerPdeBase;
    ...
    MI_FLUSH_ENTIRE_TB(4);// Force all processors to use the latest mappings.
    ...
    MiReleaseSystemPtes(PointerPde, (ULONG)(NumberOfBytes >> PAGE_SHIFT), SystemPteSpace);
#endif
}
...
Arguments:
    ...
    CacheType - Supplies MmNonCached if the physical address is to be mapped as non-cached, MmCached if the address should be cached,
                and MmWriteCombined if the address should be cached and
                write-combined as a frame buffer which is to be used only by the video port driver.
                All other callers should use MmUSWCCached.
                MmUSWCCached is available only if the PAT feature is present and available.
...
*/
{
    ...
    // For compatibility for when CacheType used to be passed as a BOOLEAN mask off the upper bits (TRUE == MmCached, FALSE == MmNonCached).
    CacheType &= 0xFF;
    ...
    // See if the first frame is in the PFN database and if so, they all must be.
    // Note since the caller is mapping the frames, they must already be locked so the PFN lock is not needed to protect against a hot-remove.
    PageFrameIndex = (PFN_NUMBER)(PhysicalAddress.QuadPart >> PAGE_SHIFT);
    IoMapping = (CSHORT)(!MI_IS_PFN(PageFrameIndex));
    CacheAttribute = MI_TRANSLATE_CACHETYPE(CacheType, IoMapping);
    if (IoMapping) {
        // If the size and start address are an exact multiple of the minimum large page size, try to use large pages to map the request.
        if (((PhysicalAddress.LowPart % MM_MINIMUM_VA_FOR_LARGE_PAGE) == 0) && ((NumberOfBytes % MM_MINIMUM_VA_FOR_LARGE_PAGE) == 0)) {
            BaseVa = MiMapWithLargePages(PageFrameIndex, NumberOfPages, MM_EXECUTE_READWRITE, CacheType);
            if (BaseVa != NULL) {
                goto Done;
            }
        }
...
#if defined(_X86_)
    // Set the physical range to the proper caching type.
    // If the PAT feature is supported, then we just use the caching type in the PTE.
    // Otherwise modify the MTRRs if applicable.

    // Note if the cache request is for cached or noncached, don't waste an MTRR on this range because the PTEs can be encoded to provide equivalent functionality.
    if ((MiWriteCombiningPtes == FALSE) && (CacheAttribute == MiWriteCombined)) {
        NTSTATUS Status;// If the address is an I/O space address, use MTRRs if possible.

        // If the address is a memory address, 
        // don't risk using MTRRs because other pages in the range are likely mapped with differing attributes in the TB and we must not add a conflicting range.
        if (Pfn1 != NULL) {
            MiReleaseSystemPtes(PointerPte, NumberOfPages, SystemPteSpace);
            return NULL;
        }

        // Since the attribute may have been overridden (due to a collision with a prior exiting mapping), 
        // make sure the CacheType is also consistent before editing the MTRRs.
        ...
        // Override the write combine (weak UC) bits in the PTE and instead use a cached attribute.
        // This is because the processor will use the least cachable (ie: functionally safer) attribute of the PTE & MTRR to use - so specifying fully cached for the PTE ensures that the MTRR value will win out.
        TempPte = ValidKernelPte;
    }
#endif
...
Done:
    if (MmTrackPtes & 0x1) {
        ...
        *(PPFN_NUMBER)(TempMdl + 1) = PageFrameIndex;
        ...
        CacheAttribute = MI_TRANSLATE_CACHETYPE(CacheType, IoMapping);
        MiInsertPteTracker(TempMdl, 1, IoMapping, CacheAttribute, CallingAddress, CallersCaller);
    }
...
VOID MmUnmapIoSpace(__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    This function unmaps a range of physical addresses which were previously mapped via an MmMapIoSpace function call.
Arguments:
    BaseAddress - Supplies the base virtual address where the physical address was previously mapped.
    NumberOfBytes - Supplies the number of bytes which were mapped.
Environment:
...
*/
{
    ...
    if (MI_PDE_MAPS_LARGE_PAGE(PointerPde) == 0) {
        ...
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte);
        if (!MI_IS_PFN(PageFrameIndex)) {
            // The PTEs must be invalidated and the TB flushed *BEFORE* removing the I/O space map.
            // This is because another thread can immediately map the same I/O space with another set of PTEs (and conflicting TB attributes) before we call MiReleaseSystemPtes.
        ...
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPde) + MiGetPteOffset(BaseAddress);
        if (!MI_IS_PFN(PageFrameIndex)) {
            MiRemoveIoSpaceMap(BaseAddress, NumberOfPages);
        }

        // There is a race here because the I/O space mapping entry has been removed but the TB has not yet been flushed (nor have the PDEs been invalidated).
        // Another thread could request a map in between the above removal and the below invalidation.
        // If this map occurs where a driver provides the wrong page attribute it will not be detected.
        // This is not worth closing as it is a driver bug anyway and you really can't always stop them from hurting themselves if they are determined to do so.
        // Note the alternative of invalidating first isn't attractive because then the same PTEs could be
        // immediately reallocated and the new owner might want to add an I/O space entry before this thread got to remove his.
        // So additional lock serialization would need to be added.
        // Not worth it.
...
PVOID MiAllocateContiguousMemory(
...
Routine Description:
    ...
    It relies on the fact that non-paged pool is built at system initialization time from a contiguous range of physical memory.
    It allocates the specified size of non-paged pool and then checks to ensure it is contiguous as pool expansion does not maintain the contiguous nature of non-paged pool.
    This routine is designed to be used by a driver's initialization routine to allocate a contiguous block of physical memory for issuing DMA requests from.
Arguments:
    NumberOfBytes - Supplies the number of bytes to allocate.
    LowestAcceptablePfn - Supplies the lowest page frame number which is valid for the allocation.
    HighestAcceptablePfn - Supplies the highest page frame number which is valid for the allocation.
    BoundaryPfn - Supplies the page frame number multiple the allocation must not cross.  0 indicates it can cross any boundary.
    CacheType - Supplies the type of cache mapping that will be used for the memory.
...
Return Value:
    ...
    NON-NULL - Returns a pointer (virtual address in the nonpaged portion of the system) to the allocated physically contiguous memory.
Environment:
    Kernel mode, IRQL of DISPATCH_LEVEL or below.
*/
{
    ...
    if (MiNoLowMemory != 0) {
        if (HighestAcceptablePfn < MiNoLowMemory) {
            return MiAllocateLowMemory(NumberOfBytes, LowestAcceptablePfn, HighestAcceptablePfn, BoundaryPfn, CallingAddress, CacheType, 'tnoC');
        }
    ...
    // N.B. This setting of SizeInPages to exactly the request size means the non-NULL return value from MiCheckForContiguousMemory is guaranteed to be the BaseAddress.
    // If this size is ever changed, then the non-NULL return value must be checked and split/returned accordingly.
    ...
    if (CacheAttribute == MiCached) {
        ...
        if (BaseAddress != NULL) {
            if (MiCheckForContiguousMemory(BaseAddress, SizeInPages, SizeInPages, LowestPfn, HighestPfn, BoundaryPfn, CacheAttribute)) {
                return BaseAddress;
            }

            // The allocation from pool does not meet the contiguous requirements.
            // Free the allocation and see if any of the free pool pages do.
    ...
    BaseAddress = MiFindContiguousMemory(LowestPfn, HighestPfn, BoundaryPfn, SizeInPages, CacheType, CallingAddress);
    return BaseAddress;
}


__bcount(NumberOfBytes) PVOID MmAllocateContiguousMemorySpecifyCache(
    __in SIZE_T NumberOfBytes,
...
/*
Routine Description:
    This function allocates a range of physically contiguous non-cached, non-paged memory.
    This is accomplished by using MmAllocateContiguousMemory which uses nonpaged pool virtual addresses to map the found memory chunk.

    Then this function establishes another map to the same physical addresses, but this alternate map is initialized as non-cached.
    All references by our caller will be done through this alternate map.

    This routine is designed to be used by a driver's initialization routine to allocate a contiguous block of noncached physical memory for things like the AGP GART.
Arguments:
    NumberOfBytes - Supplies the number of bytes to allocate.
    LowestAcceptableAddress - Supplies the lowest physical address which is valid for the allocation.
                              For example, if the device can only reference physical memory in the 8M to 16MB range, this value would be set to 0x800000 (8Mb).
    HighestAcceptableAddress - Supplies the highest physical address which is valid for the allocation.
                               For example, if the device can only reference physical memory below 16MB, this value would be set to 0xFFFFFF (16Mb - 1).
    BoundaryAddressMultiple - Supplies the physical address multiple this allocation must not cross.
Return Value:
    NULL - a contiguous range could not be found to satisfy the request.
    NON-NULL - Returns a pointer (virtual address in the nonpaged portion of the system) to the allocated physically contiguous memory.
Environment:
...
*/
{
    ...
    if (LowestPfn > HighestPfn) {
        // The caller's range is beyond what physically exists, it cannot succeed.
        // Bail now to avoid an expensive fruitless search.
        return NULL;
    }

    BaseAddress = MiAllocateContiguousMemory(NumberOfBytes, LowestPfn, HighestPfn, BoundaryPfn, CacheType, CallingAddress);
    return BaseAddress;
}


__bcount(NumberOfBytes) PVOID MmAllocateContiguousMemory(__in SIZE_T NumberOfBytes, __in PHYSICAL_ADDRESS HighestAcceptableAddress)
/*
Routine Description:
    This function allocates a range of physically contiguous non-paged pool.
    This routine is designed to be used by a driver's initialization routine to allocate a contiguous block of physical memory for issuing DMA requests from.
Arguments:
    NumberOfBytes - Supplies the number of bytes to allocate.
    HighestAcceptableAddress - Supplies the highest physical address which is valid for the allocation.
                               For example, if the device can only reference physical memory in the lower 16MB this value would be set to 0xFFFFFF (16Mb - 1).
Return Value:
    NULL - a contiguous range could not be found to satisfy the request.
    NON-NULL - Returns a pointer (virtual address in the nonpaged portion of the system) to the allocated physically contiguous memory.
Environment:
...
VOID MmFreeContiguousMemory(__in PVOID BaseAddress)
/*
Routine Description:
    This function deallocates a range of physically contiguous non-paged pool which was allocated with the MmAllocateContiguousMemory function.
Arguments:
    BaseAddress - Supplies the base virtual address where the physical address was previously mapped.
Environment:
...
*/
{
    ...
    if (((BaseAddress >= MmNonPagedPoolStart) && (BaseAddress < (PVOID)((ULONG_PTR)MmNonPagedPoolStart + MmSizeOfNonPagedPoolInBytes))) ||
        ...
        // unlock.  Since these pages may be immediately released, force any pending delayed actions to occur now.
        ...
        // Notify deadlock verifier that a region that can contain locks will become invalid.
        if (MmVerifierData.Level & DRIVER_VERIFIER_DEADLOCK_DETECTION) {
            VerifierDeadlockFreePool(BaseAddress, SizeInPages << PAGE_SHIFT);
        }

        MmUnmapIoSpace(BaseAddress, SizeInPages << PAGE_SHIFT);// Release the mapping.
...
Arguments:
    ...
    NumberOfBytes - Supplies the number of bytes allocated to the request.
                    This must be the same number that was obtained with the MmAllocateContiguousMemorySpecifyCache call.
    CacheType - Supplies the cachetype used when the caller made the MmAllocateContiguousMemorySpecifyCache call.
Environment:
    Kernel mode, IRQL of APC_LEVEL or below.
*/
{
...
PVOID MmAllocateIndependentPages(IN SIZE_T NumberOfBytes, IN ULONG Node)
/*
Routine Description:
    This function allocates a range of virtually contiguous nonpaged pages without using superpages.
    This allows the caller to apply independent page protections to each page.
Arguments:
    NumberOfBytes - Supplies the number of bytes to allocate.
    Node - Supplies the preferred node number for the backing physical pages.
           If pages on the preferred node are not available, any page will be used.  -1 indicates no preferred node.
Return Value:
...
*/
{
    ...
    MI_MAKE_VALID_KERNEL_PTE(TempPte, 0, MM_READWRITE, PointerPte);
    MI_SET_PTE_DIRTY(TempPte);
    LOCK_PFN(OldIrql);
...
BOOLEAN MmSetPageProtection(__in_bcount(NumberOfBytes) PVOID VirtualAddress, __in SIZE_T NumberOfBytes, __in ULONG NewProtect)
/*
Routine Description:
    This function sets the specified virtual address range to the desired protection.
    This assumes that the virtual addresses are backed by PTEs which can be set (ie: not in kseg0 or large pages).
Arguments:
...
*/
{
    ...
    NumberOfPages = BYTES_TO_PAGES(NumberOfBytes);
    ASSERT(NumberOfPages != 0);
    MI_MAKE_VALID_KERNEL_PTE(NewPteContents, 0, ProtectionMask, PointerPte);
    LOCK_PFN(OldIrql);
...
/*
Routine Description:
    This routine is the worker routine executed by all processors to fan out page zeroing for AWE allocations.
Arguments:
...
*/
{
    ...
    // Use the initiating thread's priority instead of the default system thread priority.
    ...
    // Dispatch each worker thread to a processor local to the memory being zeroed.
    ...
    if ((ColoredPageInfo->Affinity != 0) && (ColoredPageInfo->Affinity != DefaultProcess->Pcb.Affinity)) {
        ...
        if (Processor != NO_BITS_FOUND) {
            Processor = (CCHAR)KeSetIdealProcessorThread(Thread, (CCHAR)Processor);
            SetIdeal = TRUE;
        } else {
            SetIdeal = FALSE;
        }
    ...
    do {
        ...
        if (BasePte != NULL) {
            ...
            PointerPte = BasePte;
            MI_MAKE_VALID_KERNEL_PTE(TempCachedPte, 0, MM_READWRITE, PointerPte);
    ...
    // Restore the entry thread priority and ideal processor - this is critical if we were called directly from the initiator instead of in the context of a system thread.
#if defined(MI_MULTINODE)
    if (SetIdeal == TRUE) {
...
Routine Description:
    ...
    This is done even on UP machines because the worker thread code maps large MDLs and is thus better performing than zeroing a single page at a time.
Arguments:
    ColoredPageInfoBase - Supplies the informational structure about which pages to zero.
Environment:
    Kernel mode, IRQL of APC_LEVEL or below.  The caller must lock down the PAGELK section that this routine resides in.
*/
{
#if defined(MI_MULTINODE)
    ULONG i;
    ...
    // Raise our priority to the highest non-realtime priority.
    // This usually allows us to spawn all our worker threads without
    // interruption from them, but also doesn't starve the modified or 
    // mapped page writers because we are going to access pageable code and data below and during the spawning.
    OldBasePriority = Thread->BasePriority;
    ...
    for (Color = 0; Color < MmSecondaryColors; Color += 1) {
        ...
        if (Pfn1 != (PMMPFN)MM_EMPTY_LIST) {
            // Assume no memory penalty for non-local memory on this machine so there's no need to run with restricted affinity.
            ...
            KeInitializeEvent(&ColoredPageInfo->Event, SynchronizationEvent, FALSE);

            // Don't spawn threads to zero the memory if we are a system thread.
            // This is because this routine can be called by drivers in the context of the segment dereference thread, 
            // so referencing pageable memory here can cause a deadlock.
            ...
            if (WaitCount == MAXIMUM_WAIT_OBJECTS) {
                // Done issuing the first round of workitems, lower priority & wait.
                KeSetPriorityThread(Thread, OldPriority);
                Thread->BasePriority = OldBasePriority;
...
PMDL MiAllocatePagesForMdl(
    ...
    Routine Description:
        ...
        This routine is designed to be used by an AGP driver to obtain physical memory in a specified range since hardware may provide substantial performance wins depending on where the backing memory is allocated.

        Because the caller may use these pages for a noncached mapping,
        care is taken to never allocate any pages that reside in a large page
        (in order to prevent TB incoherency of the same page being mapped by multiple translations with different attributes).
    Arguments:
        LowAddress - Supplies the low physical address of the first range that the allocated pages can come from.
        HighAddress - Supplies the high physical address of the first range that the allocated pages can come from.
        SkipBytes - Number of bytes to skip (from the Low Address) to get to the next physical address range that allocated pages can come from.
    ...
    Return Value:
        MDL - An MDL mapping a range of pages in the specified range.
              This may map less memory than the caller requested if the full amount is not currently available.
        NULL - No pages in the specified range OR not enough virtually contiguous nonpaged pool for the MDL is available at this time.
    Environment:
        Kernel mode, IRQL of APC_LEVEL or below.
    */
{
    ...
    // Check without the PFN lock as the actual number of pages to get will be recalculated later while holding the lock.
    ...
    // Systems utilizing memory compression may have more pages on the zero, free and standby lists than we want to give out.
    // Explicitly check MmAvailablePages instead (and recheck whenever the PFN lock is released and reacquired).
    ...
    // Charge resident available pages now for all the pages so the PFN lock can be released between the loops below.
    // Excess charging is returned at the conclusion of the loops.
    ...
    // Grab all zeroed (and then free) pages first directly from the colored lists to avoid multiple walks down these singly linked lists.
    // Then snatch transition pages as needed.
    // In addition to optimizing the speed of the removals this also avoids cannibalizing the page cache unless it's absolutely needed.
    NodePassesLeft = 1;
    ...
    do {
        ...
        // Initialize the loop iteration controls.
        // Clearly pages can be added or removed from the colored lists when we deliberately drop the PFN lock below (just to be a good citizen),
        // but even if we never released the lock, we wouldn't have scanned more than the colorhead count anyway, so this is a much better way to go.
        ColorHeadsDrained = 0;
        PagesExamined = 0;
        ...
        do {
            ...
            if (ColoredPageInfo->PagesLeftToScan == 0) {
                continue;// This colored list has already been completely searched.
            }

            if (ColorHead->Flink == MM_EMPTY_LIST) {// This colored list is empty.
            ...
            while (ColorHead->Flink != MM_EMPTY_LIST) {
                ...
                PagePlacementOk = FALSE;// See if the page is within the caller's page constraints.

                // Since the caller may do anything with these frames
                // including mapping them uncached or write combined, don't give out frames that are being mapped by (cached) superpages.
                if ((CacheAttribute == MiCached) || (Pfn1->u4.MustBeCached == 0)) {
                    LowPage1 = LowPage;
                    HighPage1 = HighPage;
                ...
                // The Flink and Blink must be nonzero here for the page to be on the listhead.
                // Only code that scans the MmPhysicalMemoryBlock has to check for the zero case.
                ASSERT(Pfn1->u1.Flink != 0);
                ASSERT(Pfn1->u2.Blink != 0);
                if (PagePlacementOk == FALSE) {
                    if (ColoredPageInfo->PagesLeftToScan == 0) {
                        break;// No more pages to scan in this colored chain.
                    }

                    // If the colored list has more than one entry then move this page to the end of this colored list.
                    PageNextColored = (PFN_NUMBER)Pfn1->OriginalPte.u.Long;
                    if (PageNextColored == MM_EMPTY_LIST) {// No more pages in this colored chain.
                        ColoredPageInfo->PagesLeftToScan = 0;
                        ColorHeadsDrained += 1;
                        break;
                    }
                ...
                // Add free pages to the list of pages to be zeroed before returning.
                if ((MemoryList == FreePageList) && ((Flags & MM_DONT_ZERO_ALLOCATION) == 0)) {
                ...
                *MdlPage = Page;
                ...
                // March on to the next colored chain so the overall allocation round-robins the page colors.
                PagesExamined = PAGE_SIZE;
                break;
            }

            // If we've held the PFN lock for a while, release it to give DPCs and other processors a chance to run.
            ...
            // Systems utilizing memory compression may have more pages on the zero, free and standby lists than we want to give out.
            // The same is true when machines are low on memory - we don't want this thread to gobble up the pages from every modified write that completes because that would starve waiting threads.
        ...
        // Release the PFN lock to give DPCs and other processors a chance to run.
        // Nothing magic about the instructions between the unlock and the relock.
        UNLOCK_PFN(OldIrql);

        // Increment the start color so the next pass will keep walking forwards through the color choices.
        StartColor = Color + 1;
        StartColor &= MmSecondaryColorMask;
    ...
    // Briefly release the PFN lock to give DPCs and other processors a time slice.
    UNLOCK_PFN(OldIrql);
    ...
    // Walk the transition lists looking for pages satisfying the constraints as walking the physical memory block can be draining.
    for (ListHead = &MmStandbyPageListByPriority[0]; ListHead < &MmStandbyPageListByPriority[MI_PFN_PRIORITIES]; ListHead += 1) {
        ...
        for (Page = ListHead->Flink; Page != MM_EMPTY_LIST; Page = NextPage) {
            ...
            // Since the caller may do anything with these frames including mapping them uncached or write combined, 
            // don't give out frames that are being mapped by (cached) superpages.
            ...
            if (PagePlacementOk == TRUE) {
                ...
                // Add standby pages to the list of pages to be zeroed before returning.
                ...
                *MdlPage = Page;
                ...
                if (found == SizeInPages) {// All the pages requested are available.
                    ListHead = &MmStandbyPageListByPriority[MI_PFN_PRIORITIES];
                    break;
                }
            }
...
pass2_done:
    ...
    // Update the color to the last one allocated (excluding any repurposed transition pages) so subsequent allocations continue to round-robin fairly.
    // Since the local color has already been incremented, carefully decrement making sure not to cross nodes.
    ...
    // Any needed zeroing below will then use cached mappings as will our caller.
    if (CacheAttribute == MiCached) {
        // If all the allocated pages were either not mapped or were last mapped as cached, then skip the TB flush as there can be no conflicting attributes with the new mapping.
    ...
    if (ZeroCount != 0) {
        // Flush the entire TB since we overwrote the initial PFN attributes above with MiNotMapped.
        // This ensures that the zeroing below can use any attribute (it will use cached).
        if (CacheAttribute != MiCached) {
            MI_FLUSH_ENTIRE_TB_FOR_ATTRIBUTE_CHANGE(MiNonCached);
        }

        // Zero all the free & standby pages, fanning out the work.
        // This is done even on UP machines because the worker thread code maps large MDLs and is thus better performing than zeroing a single page at a time.
        MiZeroInParallel(ColoredPageInfoBase);

        // Denote that no pages are left to be zeroed because in addition to zeroing them, 
        // we have reset all their OriginalPte fields to demand zero so they cannot be walked by the zeroing loop below.
        ZeroCount = 0;
    }

    // Flush the entire TB since we overwrote the initial PFN attributes above with MiNotMapped.
    // Even if we didn't have to zero any of the
    // pages above, we still need to get rid of any stale 
    // (ie: like system PTE mappings to the pages in their previous lives) TB entries since our caller may choose to use any attribute.
    if (CacheAttribute != MiCached) {
        MI_FLUSH_ENTIRE_TB_FOR_ATTRIBUTE_CHANGE(MiNonCached);
    }
    ...
    if ((MdlPageSpan - found) > ((4 * PAGE_SIZE) / sizeof(PFN_NUMBER))) {
        ...
        if (MemoryDescriptorList2 != NULL) {
            RtlCopyMemory((PVOID)(MemoryDescriptorList2 + 1), (PVOID)(MemoryDescriptorList + 1), found * sizeof(PFN_NUMBER));
            ExFreePool(MemoryDescriptorList);
    ...
    // Mark the MDL's pages as locked so the the kernelmode caller can map the MDL using MmMapLockedPages* without asserting.
    MemoryDescriptorList->MdlFlags |= MDL_PAGES_LOCKED;
    return MemoryDescriptorList;
}


PMDL MmAllocatePagesForMdl(__in PHYSICAL_ADDRESS LowAddress,
    ...
    */
{
    return MiAllocatePagesForMdl(LowAddress, HighAddress, SkipBytes, TotalBytes, MiNotMapped, 0);
}


PMDL MmAllocatePagesForMdlEx(__in PHYSICAL_ADDRESS LowAddress,
                             __in PHYSICAL_ADDRESS HighAddress,
                             __in PHYSICAL_ADDRESS SkipBytes,
                             __in SIZE_T TotalBytes,
                             __in MEMORY_CACHING_TYPE CacheType,
    ...
    Arguments:
        ...
        Flags - Supplies flags passed by the caller, currently there are 2: MM_DONT_ZERO_ALLOCATION and MM_ALLOCATE_FROM_LOCAL_NODE_ONLY
    Return Value:
        MDL - An MDL mapping a range of pages in the specified range.
              This may map less memory than the caller requested if the full amount is not currently available.
        NULL - No pages in the specified range OR not enough virtually contiguous nonpaged pool for the MDL is available at this time.
    ...
    */
{
    ...
    if (Flags & ~(MM_DONT_ZERO_ALLOCATION | MM_ALLOCATE_FROM_LOCAL_NODE_ONLY)) {
        // Don't allow external callers to specify internal flags (like MI_ALLOCATION_IS_AWE).
        return NULL;
    }

    return MiAllocatePagesForMdl(LowAddress, HighAddress, SkipBytes, TotalBytes, CacheAttribute, Flags);
}


VOID MmFreePagesFromMdl(__in PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine walks the argument MDL freeing each physical page back to the PFN database.
    This is designed to free pages acquired via MmAllocatePagesForMdl only.
Arguments:
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
        ...
        if (Pfn1->u4.PteFrame != MI_MAGIC_AWE_PTEFRAME) {
            KeBugCheckEx(MEMORY_MANAGEMENT, 0x1236, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)Page, *Page);
        }
        ...
        // If the frame was never mapped, then default to calling it cached to reduce TB invalidates.
        ...
        if (Pfn1->u4.AweAllocation == 1) {
            do {
                ...
                OriginalCount = InterlockedCompareExchange(&Pfn1->AweReferenceCount, EntryCount - 1, EntryCount);
                if (OriginalCount == EntryCount) {
                    // This thread can be racing against other threads calling MmUnlockPages.
                    // All threads can safely do
                    ...
                    if (OriginalCount == 1) {
                        ...
                        // This path should be the frequent one since typically I/Os complete before these types of pages are freed by the app.
        ...
        *Page++ = MM_EMPTY_LIST;
        ...
        // Nothing magic about the divisor here - just releasing the PFN lock periodically to allow other processors and DPCs a chance to execute.
...
NTSTATUS MmMapUserAddressesToPage(__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes, __in PVOID PageAddress)
/*
Routine Description:
    This function maps a range of addresses in a physical memory VAD to the specified page address.
    This is typically used by a driver to nicely remove an application's access to things like video memory when the application is not responding to requests to relinquish it.

    Note the entire range must be currently mapped (ie, all the PTEs must be valid) by the caller.
Arguments:
    BaseAddress - Supplies the base virtual address where the physical address is mapped.
    NumberOfBytes - Supplies the number of bytes to remap to the new address.
    PageAddress - Supplies the virtual address of the page this is remapped to. This must be nonpaged memory.
Return Value:
...
*/
{
    ...
    if (Vad == NULL) {
        Status = STATUS_MEMORY_NOT_ALLOCATED;// No virtual address descriptor located.
        goto ErrorReturn;
    }

    if (NumberOfBytes == 0) {
        // If the region size is specified as 0, the base address must be the starting address for the region.
        // The entire VAD will then be re-pointed.
    ...
    if (Vad->EndingVpn < MI_VA_TO_VPN(EndingAddress)) {
        // The entire range to remap is not contained within a single virtual address descriptor.  Return an error.
...
#if DBG
    // All the PTEs must be valid or the filling will corrupt the UsedPageTableCounts.
    do {
        ASSERT(PointerPte->u.Hard.Valid == 1);
        PointerPte += 1;
    } while (PointerPte < LastPte);
    ...
    // Fill the PTEs and flush at the end - no race here because it doesn't matter whether the user app sees the old or 
    // the new data until we return (writes going to either page is acceptable prior to return from this function).
    // There is no race with I/O and ProbeAndLockPages because the PFN lock is acquired here.
    LOCK_PFN(OldIrql);
...
PHYSICAL_ADDRESS MmGetPhysicalAddress(__in PVOID BaseAddress)
/*
Routine Description:
...
*/
{
    ...
    if (MI_IS_PHYSICAL_ADDRESS(BaseAddress)) {
        ...
        if (MI_PDE_MAPS_LARGE_PAGE(PointerPte)) {
            PhysicalAddress.QuadPart = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte) + MiGetPteOffset(BaseAddress);
        } else {
            PointerPte = MiGetPteAddress(BaseAddress);
            if (PointerPte->u.Hard.Valid == 0) {
                KdPrint(("MM:MmGetPhysicalAddressFailed base address was %p", BaseAddress));
...
PVOID MmGetVirtualForPhysical(__in PHYSICAL_ADDRESS PhysicalAddress)
/*
Routine Description:
...
__out_bcount(NumberOfBytes) PVOID MmAllocateNonCachedMemory(__in SIZE_T NumberOfBytes)
/*
Routine Description:
    This function allocates a range of noncached memory in the non-paged portion of the system address space.
    This routine is designed to be used by a driver's initialization routine to allocate a noncached block of virtual memory for various device specific buffers.
Arguments:
    NumberOfBytes - Supplies the number of bytes to allocate.
Return Value:
    NON-NULL - Returns a pointer (virtual address in the nonpaged portion of the system) to the allocated physically contiguous memory.
...
*/
{
    ...
    // Even though an MDL is not needed per se, 
    // it is much more convenient to use the routine below because it checks for things like appropriate cachability of the pages, etc.
    // Note that the MDL returned may map fewer pages than requested - check for this and if so, return NULL.
    LowAddress.QuadPart = 0;
    HighAddress.QuadPart = (ULONGLONG)-1;
    SkipBytes.QuadPart = 0;
    CacheAttribute = MI_TRANSLATE_CACHETYPE(MmNonCached, FALSE);
    Mdl = MiAllocatePagesForMdl(LowAddress, HighAddress, SkipBytes, NumberOfBytes, CacheAttribute, 0);
    if (Mdl == NULL) {
        return NULL;
    }
    ...
    // Add an extra PTE so the MDL can be stashed now and retrieved on release.
...
VOID MmFreeNonCachedMemory(__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    This function deallocates a range of noncached memory in the non-paged portion of the system address space.
Arguments:
    BaseAddress - Supplies the base virtual address where the noncached memory resides.
    NumberOfBytes - Supplies the number of bytes allocated to the request.
                    This must be the same number that was obtained with the MmAllocateNonCachedMemory call.
Environment:
    Kernel mode, IRQL of APC_LEVEL or below.
*/
{
...
SIZE_T MmSizeOfMdl(__in_bcount_opt(Length) PVOID Base, __in SIZE_T Length)
/*
Routine Description:
...
PMDL MmCreateMdl(__in_opt PMDL MemoryDescriptorList, __in_bcount_opt(Length) PVOID Base, __in SIZE_T Length)
/*
Routine Description:
    This function optionally allocates and initializes an MDL.
Arguments:
    MemoryDescriptorList - Optionally supplies the address of the MDL to initialize.
                           If this address is supplied as NULL an MDL is allocated from non-paged pool and initialized.
    Base - Supplies the base virtual address for the buffer.
    Length - Supplies the size of the buffer in bytes.
Return Value:
...
#if defined (_WIN64)
    // Since the Length has to fit into the MDL's ByteCount field, ensure it doesn't wrap on 64-bit systems.
    if (Length >= _4gb) {
        return NULL;
    }
#endif
...
BOOLEAN MmSetAddressRangeModified(__in_bcount(Length) PVOID Address, __in SIZE_T Length)
/*
Routine Description:
...
#ifdef NT_UP
            // On uniprocessor systems no need to flush if this processor doesn't think the PTE is dirty.
...
PVOID MiCheckForContiguousMemory(IN PVOID BaseAddress,
    ...
    /*
    Routine Description:
        This routine checks to see if the physical memory mapped by the specified BaseAddress for the specified size is contiguous and that the first page is greater than or equal to
        the specified LowestPfn and that the last page of the physical memory is less than or equal to the specified HighestPfn.
    Arguments:
        BaseAddress - Supplies the base address to start checking at.
        BaseAddressPages - Supplies the number of pages to scan from the BaseAddress.
        ...
        BoundaryPfn - Supplies the PFN multiple the allocation must not cross.  0 indicates it can cross any boundary.
        CacheAttribute - Supplies the type of cache mapping that will be used for the memory.
    Return Value:
        Returns the usable virtual address within the argument range that the caller should return to his caller.
        NULL if there is no usable address.
    Environment:
    ...
    */
{
    ...
    if (MI_IS_PHYSICAL_ADDRESS(BaseAddress)) {
        // All physical addresses are by definition cached and therefore do not qualify for our caller.
        if (CacheAttribute != MiCached) {
            return NULL;
        }
        ...
        if (BoundaryPfn != 0) {
            do {
                if (((Page ^ (Page + SizeInPages - 1)) & BoundaryMask) == 0) {// This portion of the range meets the alignment requirements.
                    break;
                }
                Page |= (BoundaryPfn - 1);
                Page += 1;
            ...
            // Since specific alignment was requested, return the fit now without getting fancy.
        ...
        // The end chunk did not satisfy the requirements.
        // The next best option is to return a chunk from the beginning.
        // Since that's where the search began, just return the current chunk.
    ...
    // Initializing PreviousPage is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    PreviousPage = 0;
    while (PointerPte < LastPte) {
        PteContents = *PointerPte;
        ASSERT(PteContents.u.Hard.Valid == 1);
        ...
        // Before starting a new run, ensure that it can satisfy the location & boundary requirements (if any).
        if (PageCount == 0) {
            if ((Page >= LowestPfn) &&
                ...
                if (BoundaryPfn == 0) {
                    PageCount += 1;
                } else if (((Page ^ (Page + SizeInPages - 1)) & BoundaryMask) == 0) {// This run's physical address meets the alignment requirement.
                    PageCount += 1;
                }
            }

            if (PageCount == SizeInPages) {
                if (CacheAttribute != MiCached) {// Recheck the cachability while holding the PFN lock.
                    LOCK_PFN2(OldIrql);
                ...
                if (PageCount != 0) {// Success - found a single page satifying the requirements.
        ...
        if (Page != PreviousPage + 1) {// This page is not physically contiguous.  Start over.
            PageCount = 0;
            continue;
        }
...
VOID MmLockPageableSectionByHandle(__in PVOID ImageSectionHandle)
/*
Routine Description:
    This routine checks to see if the specified pages are resident in the process's working set and if so the reference count for the page is incremented.
    The allows the virtual address to be accessed without getting a hard page fault (have to go to the disk... except for extremely rare case when the page table page is removed from the working set and migrates to the disk.

    If the virtual address is that of the system wide global "cache" the virtual address of the "locked" pages is always guaranteed to be valid.
...
*/
{
    ...
    if (MI_IS_PHYSICAL_ADDRESS(ImageSectionHandle)) {
        return;// No need to lock physical addresses.
    }
    ...
    //  If the value is 1, this thread must block until the other thread's lock operation is complete.
    do {
        ...
        if (EntryCount != 1) {
            ...
            if (OriginalCount == EntryCount) {// Success - this is the first thread to update.
                ASSERT(OriginalCount != 1);
                break;
            }

            // Another thread updated the count before this thread's attempt so it's time to start over.
        } else {
            // A lock is in progress, wait for it to finish.
            // This should be generally rare, and even in this case, the pulse will usually wake us.
            // A timeout is used so that the wait and the pulse do not need to be interlocked.
            InterlockedIncrement(&MmCollidedLockWait);
            KeWaitForSingleObject(&MmCollidedLockEvent, WrVirtualMemory, KernelMode, FALSE, (PLARGE_INTEGER)&MmShortTime);
            InterlockedDecrement(&MmCollidedLockWait);
        }
    } while (TRUE);

    if (OriginalCount >= 2) {
        KeLeaveCriticalRegionThread(CurrentThread);// Already locked, just return.
        return;
    }
    ...
    // Value was 0 when the lock was obtained.
    // It is now 1 indicating a lock is in progress.
    MiLockCode(PointerPte, LastPte, MM_LOCK_BY_REFCOUNT);

    // Set lock count to 2 (it was 1 when this started) and check to see if any other threads tried to lock while this was happening.
    ASSERT(*SectionLockCountPointer == 1);
    ...
    // Enable user APCs now that the pulse has occurred.
    // They had to be blocked to prevent any suspensions of this thread as that would stop all waiters indefinitely.
...
Routine Description:
    This routine checks to see if the specified pages are resident in the process's working set and if so the reference count for the page is incremented.
    This allows the virtual address to be accessed without getting a hard page fault (have to go to the disk...) except
    for the extremely rare case when the page table page is removed from the working set and migrates to the disk.

    If the virtual address is that of the system wide global "cache", the virtual address of the "locked" pages is always guaranteed to be valid.

    NOTE: This routine is not to be used for general locking of user addresses - use MmProbeAndLockPages.
    This routine is intended for well behaved system code like the file system caches which allocates virtual addresses for mapping files AND guarantees that the mapping
...
Arguments:
    ...
    LockType - Supplies either MM_LOCK_BY_REFCOUNT or MM_LOCK_NONPAGE.
               LOCK_BY_REFCOUNT increments the reference count to keep the page in memory,
               LOCK_NONPAGE removes the page from the working set so it's locked just like nonpaged pool.
Environment:
    Kernel mode.
*/
{
    ...
    do {
        ...
        if (PteContents.u.Hard.Valid == 1) {
            ...
            if (LockType != MM_LOCK_BY_REFCOUNT) {
                ...
                // The system working set lock MUST be owned to check to see if this page is in the working set or not.
                // This is because the pager may have just released the PFN lock, acquired the system lock and is now trying to add the page to the system working set.

                // If the page is in the SESSION working set, it cannot be removed as all these pages are carefully accounted for.
                // Instead move it to the locked portion of the working set if it is not there already.
                if (Pfn1->u1.WsIndex != 0) {
                    UNLOCK_PFN(OldIrql);
                    if (SessionSpace == TRUE) {
                        WorkingSetIndex = MiLocateWsle(MiGetVirtualAddressMappedByPte(PointerPte), WorkingSetList, Pfn1->u1.WsIndex, FALSE);
                        if (WorkingSetIndex >= WorkingSetList->FirstDynamic) {
                            ...
                            if (WorkingSetIndex != WorkingSetList->FirstDynamic) {
                                ...
                                // Note that the working set index in the PTE is updated here as well.
                                MiSwapWslEntries(WorkingSetIndex, SwapEntry, Vm, FALSE);
                            }
                            ...
                            // Adjust available pages as this page is now not in any working set, just like a non-paged pool page.
        ...
        } else if (PteContents.u.Soft.Prototype == 1) {// Page is not in memory and it is a prototype.
            MiMakeSystemAddressValidPfnSystemWs(MiGetVirtualAddressMappedByPte(PointerPte), OldIrql);
            continue;
            ...
            // This means a paged pool PTE can be in transition while the page is still marked active.
            if (Pfn1->u3.e1.PageLocation == ActiveAndValid) {
                ASSERT(((Pfn1->PteAddress >= MiGetPteAddress(MmPagedPoolStart)) && (Pfn1->PteAddress <= MiGetPteAddress(MmPagedPoolEnd))) ||
                    ((Pfn1->PteAddress >= MiGetPteAddress(MmSpecialPoolStart)) && (Pfn1->PteAddress <= MiGetPteAddress(MmSpecialPoolEnd))));
                // Don't increment the valid PTE count for the paged pool page.
                ASSERT(Pfn1->u2.ShareCount != 0);
                ASSERT(Pfn1->u3.e2.ReferenceCount != 0);
                Pfn1->u2.ShareCount += 1;
            } else {
                if (MmAvailablePages == 0) {
                    // This can only happen if the system is utilizing a hardware compression cache.
                    // This ensures that only a safe amount of the compressed virtual cache is directly mapped so that if the hardware gets into trouble, we can bail it out.

                    // Just unlock everything here to give the compression reaper a chance to ravage pages and then retry.
                    UNLOCK_PFN(OldIrql);
                ...
                // Note the reference count may be 1 already if a modified page write is underway.
                // The systemwide locked page charges are correct in either case and nothing needs to be done just yet.
            ...
            // Increment the reference count one for putting it the working set list and one for locking it for I/O.
            if (LockType == MM_LOCK_BY_REFCOUNT) {
                // Lock the page in the working set by upping the reference count.
                MI_ADD_LOCKED_PAGE_CHARGE(Pfn1);
                Pfn1->u1.Event = NULL;
                UNLOCK_PFN(OldIrql);

                WorkingSetIndex = MiAllocateWsle(Vm, PointerPte, Pfn1, 0);
                if (WorkingSetIndex == 0) {
                    // No working set entry was available.
                    // Another (broken or malicious thread) may have already written to this page since the PTE was made valid.
                    // So trim the page instead of discarding it.

                    // Note the page cannot be a prototype because the PTE was transition above.
                    ASSERT(Pfn1->u3.e1.PrototypePte == 0);

                    LOCK_PFN(OldIrql);
                    MI_REMOVE_LOCKED_PAGE_CHARGE_AND_DECREF(Pfn1);// Undo the reference count & locked page charge (if any).
                    UNLOCK_PFN(OldIrql);

                    MiTrimPte(MiGetVirtualAddressMappedByPte(PointerPte), PointerPte, Pfn1, NULL, ZeroPte);

                    // Release all locks so that other threads (like the working set trimmer) can try to freely make memory.
                    UNLOCK_WORKING_SET(CurrentThread, Vm);
                    KeDelayExecutionThread(KernelMode, FALSE, (PLARGE_INTEGER)&Mm30Milliseconds);
                    LOCK_WORKING_SET(CurrentThread, Vm);
                    LOCK_PFN(OldIrql);
                    continue;// Retry the same page now.
                }

                LOCK_PFN(OldIrql);
            } else {
                // The wsindex field must be zero because the page is not in the system (or session) working set.
                ASSERT(Pfn1->u1.WsIndex == 0);

                // Adjust available pages as this page is now not in any working set, just like a non-paged pool page.
                // On entry this page was in transition so it was part of the available pages by definition.
...
NTSTATUS MmGetSectionRange(IN PVOID AddressWithinSection, OUT PVOID* StartingSectionAddress, OUT PULONG SizeofSection)
{
    ...
    // Search the loaded module list for the data table entry that describes the DLL that was just unloaded.
    // It is possible that an entry is not in the list if a failure occurred at a point in loading the DLL just before the data table entry was generated.
    ...
    if (DataTableEntry) {
        ...
        for (i = 0; i < NtHeaders->FileHeader.NumberOfSections; i += 1) {
            ...
            if ((Rva >= NtSection->VirtualAddress) && (Rva < NtSection->VirtualAddress + Span)) {// Found it.
...
PVOID MmLockPageableDataSection(__in PVOID AddressWithinSection)
/*
Routine Description:
    This functions locks the entire section that contains the specified section in memory.
    This allows pageable code to be brought into memory and to be used as if the code was not really pageable.
    This should not be done with a high degree of frequency.
Arguments:
    AddressWithinSection - Supplies the address of a function contained within a section that should be brought in and locked in memory.
Return Value:
    This function returns a value to be used in a subsequent call to MmUnlockPageableImageSection.
*/
{
    ULONG Span;
    PLONG SectionLockCountPointer;
    ...
    if (MI_IS_PHYSICAL_ADDRESS(AddressWithinSection)) {
        return AddressWithinSection;// Physical address, just return that as the handle.
    }
    ...
    Rva = (ULONG_PTR)((PUCHAR)AddressWithinSection - (ULONG_PTR)DataTableEntry->DllBase);
    NtHeaders = (PIMAGE_NT_HEADERS)RtlImageNtHeader(DataTableEntry->DllBase);
    if (NtHeaders == NULL) {// This is a firmware entry - no one should be trying to lock these.
        KeBugCheckEx(MEMORY_MANAGEMENT, 0x1234, (ULONG_PTR)AddressWithinSection, 1, 0);
    }

    NtSection = (PIMAGE_SECTION_HEADER)((ULONG_PTR)NtHeaders + sizeof(ULONG) + sizeof(IMAGE_FILE_HEADER) + NtHeaders->FileHeader.SizeOfOptionalHeader);
    for (i = 0; i < NtHeaders->FileHeader.NumberOfSections; i += 1) {
        // Generally, SizeOfRawData is larger than VirtualSize for each section because it includes the padding to get to the subsection alignment boundary.
        // However, if the image is linked with subsection alignment == native page alignment, 
        // the linker will have VirtualSize be much larger than SizeOfRawData because it will account for all the bss.
        ...
        if ((Rva >= NtSection->VirtualAddress) && (Rva < NtSection->VirtualAddress + Span)) {
            ...
            if (SECTION_BASE_ADDRESS(NtSection) != ((PUCHAR)DataTableEntry->DllBase + NtSection->VirtualAddress)) {
                // Overwrite the PointerToRelocations field (and on Win64, the PointerToLinenumbers field also) so that it contains the Va of this section.

                // NumberOfRelocations & NumberOfLinenumbers contains the Lock Count for the section.
...
Arguments:
    AddressWithinSection - Supplies the address of a function contained within the desired module.
    ResourceHeld - Supplies TRUE if the loaded module resource is already held, FALSE if not.
Return Value:
    The address of the loaded module list data table entry that maps the argument address.
*/
{
    PKTHREAD CurrentThread;
    PKLDR_DATA_TABLE_ENTRY DataTableEntry;
...
VOID MmUnlockPageableImageSection(__in PVOID ImageSectionHandle)
/*
Routine Description:
    This function unlocks from memory, the pages locked by a preceding call to MmLockPageableDataSection.
Arguments:
    ImageSectionHandle - Supplies the value returned by a previous call to MmLockPageableDataSection.
*/
{
    ...
    if (MI_IS_PHYSICAL_ADDRESS(ImageSectionHandle)) {
        return;// No need to unlock physical addresses.
    }
    ...
    // This puts this thread into the critical path that must finish as all other threads trying to lock the section will be waiting for this thread.
...
/*
Routine Description:
    This function examines the thread's page fault clustering information and determines if the current page fault is occurring during an I/O operation.
Return Value:
...
VOID MmMapMemoryDumpMdl(__inout PMDL MemoryDumpMdl)
/*
Routine Description:
    For use by crash dump routine ONLY.
    Maps an MDL into a fixed portion of the address space.  Only 1 MDL can be mapped at a time.
Arguments:
...
*/
{
    ...
    // If the pages don't span the entire dump virtual address range, build a barrier.
    // Otherwise use the default barrier provided at the end of the dump virtual address range.
    if (NumberOfPages < 16) {
...
VOID MmReleaseDumpAddresses(IN PFN_NUMBER NumberOfPages)
/*
Routine Description:
    For use by hibernate routine ONLY.  Puts zeros back into the used dump PTEs.
Arguments:
...
NTSTATUS MmSetBankedSection(__in HANDLE ProcessHandle,
...
*/
{
    ...
    // Block APCs so an APC which takes a page fault does not corrupt various structures.
    LOCK_ADDRESS_SPACE(Process);
    ...
    MI_MAKE_VALID_USER_PTE(TempPte, Bank->BasePhysicalPage, MM_READWRITE, PointerPte);
    ...
    // Set all PTEs within this range to zero.
    // Any faults within this range will call the banked routine before making the page valid.
    LOCK_WS_UNSAFE(Thread, Process);
...
VOID MmUnmapVideoDisplay(__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    This function unmaps a range of physical address which were previously mapped via an MmMapVideoDisplay function call.
Arguments:
    BaseAddress - Supplies the base virtual address where the physical address was previously mapped.
    NumberOfBytes - Supplies the number of bytes which were mapped.
Environment:
...
/*
Routine Description:
    Locks the specified address (which MUST reside in paged pool) into memory until MmUnlockPagedPool is called.
Arguments:
...
/*
Routine Description:
    Finds enough memory to fill in the pages of the MDL for power management hibernate function.
Arguments:
    Mdl - Supplies an MDL, the start VA field should be NULL.  The length field indicates how many pages to obtain.
...
*/
{
    ...
    do {
        ...
        // If we're being called at DISPATCH_LEVEL we cannot move pages to the standby list because mutexes must be acquired to do so.
...
/*
Routine Description:
    This routine enables the page attribute capability for individual PTE mappings.
Environment:
    Kernel mode.
*/
{
    ...
    // The attribute table is statically initialized with write combining being equivalent to noncached.
    // Once all the processors have booted as we don't know if they will all have a PAT (or equivalent functionality) until then,
    // we can override the PTE maps as below.

    // Note if the machine cannot support all the attribute types to main memory,
    // then the MmProtectToPteMask table will already have been modified in MiInitializeCacheOverrides.
    // If so, then nothing more needs to be done here.
    if (MiAllMainMemoryMustBeCached == TRUE) {
        return;// The table has already been updated.
    }
...
/*
Routine Description:
    This routine determines whether the specified system address is currently locked.
    This routine should only be called for debugging purposes, as it is not guaranteed upon return to the caller that the address is still locked.
...
*/
{
    ...
    // Note that the mapped page may not be in the PFN database.
...
LOGICAL MmAreMdlPagesLocked(IN PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine determines whether the pages described by the argument MDL are currently locked.
    This routine should only be called for debugging purposes, as it is not guaranteed upon return to the caller that the pages are still locked.
Arguments:
...
*/
{
    ...
    // We'd like to assert that MDL_PAGES_LOCKED is set but can't because some drivers have privately constructed MDLs and they never set the bit properly.
    if ((MemoryDescriptorList->MdlFlags & (MDL_IO_SPACE | MDL_SOURCE_IS_NONPAGED_POOL)) != 0) {
        return TRUE;
    }

    VirtualAddress = (PVOID)((PCHAR)MemoryDescriptorList->StartVa + MemoryDescriptorList->ByteOffset);
    NumberOfPages = ADDRESS_AND_SIZE_TO_SPAN_PAGES(VirtualAddress, MemoryDescriptorList->ByteCount);
    Page = (PPFN_NUMBER)(MemoryDescriptorList + 1);
    ...
    do {
        ...
        // Note that the mapped page may not be in the PFN database.  Treat this as locked.
        if (MI_IS_PFN(*Page)) {
            Pfn1 = MI_PFN_ELEMENT(*Page);
            ...
            if ((Pfn1->u3.e2.ReferenceCount <= Pfn1->u2.ShareCount) &&
                ...
                if ((Pfn1->u3.e1.PageLocation == ActiveAndValid) &&
                    ...
                    if ((VirtualAddress >= MmNonPagedPoolExpansionStart) && (VirtualAddress < MmNonPagedPoolEnd)) {
                        NOTHING;// This is expansion nonpaged pool.
                    } else if ((VirtualAddress >= MmSpecialPoolStart) &&
                        (VirtualAddress < MmSpecialPoolEnd) &&
                               (MmSpecialPoolStart != NULL) &&
                               ((MmQuerySpecialPoolBlockType(VirtualAddress) & BASE_POOL_TYPE_MASK) == NonPagedPool)) {
                        NOTHING;
                    } else {
                        UNLOCK_PFN2(OldIrql);
                        return FALSE;
