...
NTSTATUS MiGetWorkingSetInfo(IN PMEMORY_WORKING_SET_INFORMATION WorkingSetInfo, IN SIZE_T Length, IN PEPROCESS Process);
...
Routine Description:
    ...
    base address upward until either the entire range of pages has been scanned or until a page with a nonmatching set of attributes is encountered.
    The region attributes, the length of the region of pages with matching attributes, and an appropriate status value are returned.
        ...
        MemoryBasicInformation - Data type is PMEMORY_BASIC_INFORMATION.
            ...
            ULONG Protect - The protection of the pages within the region.
                ...
                PAGE_NOACCESS - No access to the region of pages is allowed.
                                An attempt to read, write, or execute within the region results in an access violation.
                ...
                PAGE_READONLY - Read-only and execute access to the region of pages is allowed.
                                An attempt to write within the region results in an access violation.
                ...
                PAGE_WRITECOMBINE - Disable the placement of committed pages into the data cache, combine the writes as well.
...
*/
{
    ...
    // If this is a wow64 process, 
    // then return the appropriate highest user address depending on whether the process has been started with a 2GB or a 4GB address space.
    ...
    if ((BaseAddress > HighestVadAddress) || (PAGE_ALIGN(BaseAddress) == (PVOID)MM_SHARED_USER_DATA_VA)) {
        Status = STATUS_INVALID_ADDRESS;// Indicate a reserved area from this point on.
        if (MemoryInformationClass == MemoryBasicInformation) {
            try {
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->AllocationBase = (PCHAR)HighestVadAddress + 1;
                ...
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->RegionSize = ((PCHAR)HighestUserAddress + 1) - (PCHAR)PAGE_ALIGN(BaseAddress);
    ...
    if (MemoryInformationClass == MemoryWorkingSetExInformation) {
        Status = MiGetWorkingSetInfoList((PMEMORY_WORKING_SET_EX_INFORMATION)MemoryInformation, MemoryInformationLength, TargetProcess);
        if (ProcessHandle != NtCurrentProcess()) {
            ObDereferenceObject(TargetProcess);
        }
        if (!NT_SUCCESS(Status)) {// If MiGetWorkingSetInfoList failed then inform the caller.
            return Status;
        }
    ...
    if (MemoryInformationClass == MemoryWorkingSetInformation) {
        Status = MiGetWorkingSetInfo((PMEMORY_WORKING_SET_INFORMATION)MemoryInformation, MemoryInformationLength, TargetProcess);
        if (ProcessHandle != NtCurrentProcess()) {
            ObDereferenceObject(TargetProcess);
        }
        if (!NT_SUCCESS(Status)) {// If MiGetWorkingSetInfo failed then inform the caller.
            return Status;
        }
    ...
    if (ProcessHandle != NtCurrentProcess()) {// If the specified process is not the current process, attach to the specified process.
    ...
    if (!Found) {
        ...
        if (Vad == NULL) {
            TheRegionSize = (((PCHAR)HighestVadAddress + 1) - (PCHAR)PAGE_ALIGN(BaseAddress));
        } else {
    ...
    while (MI_VA_TO_VPN(Va) <= Vad->EndingVpn) {
        ...
        if ((NewState != Info.State) || (NewProtect != Info.Protect)) {
            Leaped = FALSE;// The state for this address does not match, calculate size and return.
            break;
        }
        Va = NextVaToQuery;
    }
    ...
    // We may have aggressively leaped past the end of the VAD.
    // Shorten the Va here if we did.
    if (Leaped == TRUE) {
    ...
    Status = ObQueryNameString(FilePointer, (POBJECT_NAME_INFORMATION)MemoryInformation, MemoryInformationLengthUlong, &LocalReturnLength);
...
Environment:
    Kernel mode.  Address creation mutex held (exclusive) and working set pushlock held (shared).
    This routine may release and reacquire the working set pushlock, callers must be prepared to handle this.
*/
{
    KIRQL OldIrql;
    PMMPFN Pfn1;
    ...
    *NextVaToQuery = (PVOID)((PCHAR)Va + PAGE_SIZE);
    ...
    do {
        if (!MiDoesPxeExistAndMakeValid(PointerPxe, TargetProcess, MM_NOIRQL, &Waited)) {
        ...
        if (!MiDoesPpeExistAndMakeValid(PointerPpe, TargetProcess, MM_NOIRQL, &Waited)) {
        ...
        if (!MiDoesPdeExistAndMakeValid(PointerPde, TargetProcess, MM_NOIRQL, &Waited)) {
        ...
        if (PteContents.u.Long != 0) {
            ...
            if (MiIsPteDecommittedPage(&PteContents)) {
                ...
                if (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) {
                    // Physical mapping, there is no corresponding PFN element to get the page protection from.
                    Protect = MI_CONVERT_FROM_PTE_PROTECTION(Vad->u.VadFlags.Protection);
                } else if ((Vad->u.VadFlags.VadType == VadRotatePhysical) && (PteContents.u.Hard.Valid)) {
                    ProtoPte = MiGetProtoPteAddress(Vad, MI_VA_TO_VPN(Va));
                    PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(&PteContents);
                    Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
                    if ((!MI_IS_PFN(PageFrameIndex)) || (Pfn1->PteAddress != ProtoPte)) {
                        // This address in the view is currently pointing at the frame buffer (video RAM or AGP),
                        // protection is in the prototype (which may itself be paged out - so release the process working set pushlock before accessing it).
                        Thread = PsGetCurrentThread();
                        ...
                        // PTE is either demand zero, pagefile or transition, in all cases protection is in the prototype PTE.
                        Protect = MI_CONVERT_FROM_PTE_PROTECTION(CapturedProtoPte.u.Soft.Protection);
                        LOCK_WS_SHARED(Thread, TargetProcess);
                    } else {
                        ASSERT(Pfn1->u3.e1.PrototypePte == 1);
                        Protect = MI_CONVERT_FROM_PTE_PROTECTION(Pfn1->OriginalPte.u.Soft.Protection);
                    }
                } else if (Vad->u.VadFlags.VadType == VadAwe) {
                    // This is an AWE frame - the original PTE field in the PFN actually contains the AweReferenceCount.
                    // These pages are either noaccess, readonly or readwrite.
    ...
    if (PteIsZero) {
        // There is no PDE at this address, the template from the VAD supplies the information unless the VAD is for an image file.
        // For image files the individual protection is on the prototype PTE.
        ...
        if (Vad->u.VadFlags.VadType == VadAwe) {
            ...
            NOTHING;// Must be banked memory, just return reserved.
        } else if ((Vad->u.VadFlags.PrivateMemory == 0) && (Vad->ControlArea != NULL)) {
            // This VAD refers to a section.
            // Even though the PTE is zero, the actual page may be committed in the section.
            *NextVaToQuery = (PVOID)((PCHAR)Va + PAGE_SIZE);
            ProtoPte = MiGetProtoPteAddress(Vad, MI_VA_TO_VPN(Va));
            if (ProtoPte != NULL) {
                Thread = PsGetCurrentThread();
                ...
                if (CapturedProtoPte.u.Long != 0) {
                    ...
                    if (Vad->u.VadFlags.VadType != VadImageMap) {
                        ...
                        if (CapturedProtoPte.u.Hard.Valid == 0) {
                            ...
                            // The prototype PTE is valid but not in this process (at least not before we released the working set pushlock above).
                            // So the PFN's OriginalPte field contains the correct protection value, but it may get trimmed at any time.
                            // Acquire the PFN lock so we can examine it safely.
...
NTSTATUS MiGetWorkingSetInfoList(IN PMEMORY_WORKING_SET_EX_INFORMATION WorkingSetInfo, IN SIZE_T Length, IN PEPROCESS Process)
{
    PMDL Mdl;
    ...
    for (; WsSize != 0; Info->u1.Long = Entry.u1.Long, Info += 1, WsSize -= 1) {
        ...
        if (Process->PhysicalVadRoot != NULL) {
            ...
            if ((SearchResult == TableFoundNode) && (PhysicalView->VadType == VadDevicePhysicalMemory)) {
                ...
                Entry.u1.VirtualAttributes.Win32Protection = MI_CONVERT_FROM_PTE_PROTECTION(PhysicalView->Vad->u.VadFlags.Protection);
                continue;
            }
        }
        ...
        // We have a valid address and the corresponding PFN.
        // Extract the interesting information now.
        ...
        // AWE, large page and \Device\PhysicalMemory entries have no WSLE and so must be handled carefully.
        if (Entry.u1.VirtualAttributes.LargePage == 1) {
            ...
            if (Pfn1->u3.e1.PrototypePte) {
                ...
                if (Vad->u.VadFlags.VadType == VadLargePageSection) {
                    // Prior to Longhorn, the pagefile-backed large section code would increment the share count internally and
                    // this not something we want to return back to applications because it would confuse them so explicitly set it here for these types of sections.
                    // In Longhorn the share count is always 1 for these so no setting at this location in the code is needed.
                    ShareCount -= 1;
                }
        ...
        WorkingSetIndex = MiLocateWsle(VirtualAddress, MmWorkingSetList, Pfn1->u1.WsIndex, FALSE);
