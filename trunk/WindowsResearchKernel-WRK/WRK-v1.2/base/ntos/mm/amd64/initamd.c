...
Abstract:
    This module contains the machine dependent initialization for the memory management component.
    It is specifically tailored to the AMD64 architecture.
*/

#include "mi.h"

PFN_NUMBER MxGetNextPage(IN PFN_NUMBER PagesNeeded, IN LOGICAL UseSlush);
PFN_NUMBER MxPagesAvailable(VOID);
...
VOID MiComputeInitialLargePage(VOID);
LOGICAL MiIsRegularMemory(IN PLOADER_PARAMETER_BLOCK LoaderBlock, IN PFN_NUMBER PageFrameIndex);
...
/*
Routine Description:
    This function returns the next physical page number from the largest largest free descriptor.
    If there are not enough physical pages left to satisfy the request then a bugcheck is executed since the system cannot be initialized.
Arguments:
    PagesNeeded - Supplies the number of pages needed.
    UseSlush - Supplies TRUE if the slush descriptor can be used (ie, the allocation will never be freed and will always be cachable).
Return Value:
...
*/
{
    ...
    if (UseSlush == TRUE) {
        if ((MxSlushDescriptor1 != NULL) && (PagesNeeded <= MxSlushDescriptor1->PageCount)) {
        ...
        if ((MxSlushDescriptor2 != NULL) && (PagesNeeded <= MxSlushDescriptor2->PageCount)) {
...
/*
Routine Description:
    This function converts the backing for the supplied virtual address range to a large page mapping.
Arguments:
    VirtualAddress - Supplies the virtual address to convert to a large page.
    EndVirtualAddress - Supplies the end virtual address to convert to a large page.
Environment:
...
*/
{
    ...
    for (; PointerPde <= LastPde; PointerPde += 1) {
        ...
        // Here's a nasty little hack - the page table page mapping the kernel and HAL (built by the loader) does not necessarily fill all the page table entries (ie: any number of leading entries may be zero).

        // To deal with this, walk forward until a nonzero entry is found and re-index the large page based on this.
        ValidPteFound = FALSE;
        ...
        if (((PageFrameIndex >= MxOldSlushDescriptor1.BasePage) && (PageFrameIndex < MxOldSlushDescriptor1.BasePage + MxOldSlushDescriptor1.PageCount)) ||
            ((PageFrameIndex >= MxOldSlushDescriptor2.BasePage) && (PageFrameIndex < MxOldSlushDescriptor2.BasePage + MxOldSlushDescriptor2.PageCount))) {
            // Excess slush is given to expansion nonpaged pool here to ensure that it is always mapped fully cached since the rest of the large page is inserted as fully cached into the TB.
...
/*
Routine Description:
    This routine checks whether the argument page frame index represents regular memory in the loader descriptor block.
    It is only used very early during Phase0 init because the MmPhysicalMemoryBlock is not yet initialized.
Arguments:
...
*/
{
    ...
    while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
        ...
        if (PageFrameIndex >= MemoryDescriptor->BasePage) {
            ...
            // Since the loader memory list is sorted in ascending order, the requested page must not be in the loader list at all.
            break;
        }
...
/*
Routine Description:
    This routine is called during Phase 0 initialization once the MmPhysicalMemoryBlock has been constructed.
    It's job is to decide which large page ranges to enable later and also to construct a large page comparison list so any requests which are not fully cached can check this list in order to refuse conflicting requests.
Environment:
    Kernel mode.
    Phase 0 only.
...
*/
{
    ...
    // Examine the physical memory block to see whether large pages should be enabled.
    // The key point is that all the physical pages within a given large page range must have the same cache attributes (MmCached) in order to maintain TB coherency.
    // This can be done provided all the pages within the large page range represent real RAM (as described by the loader) so that memory management can control it.
    // If any portion of the large page range is not RAM, it is possible that it may get used as noncached or writecombined device memory and therefore large pages cannot be used.
    ...
    for (i = 0; i < MiLargeVaRangeIndex; i += 1) {
        ...
        // Round the start down to a page directory boundary and the end to the last page directory entry before the next boundary.
        ...
        if (EntryFound == FALSE) {
            ...
            // Zero it so this range will not be converted into large pages later.
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MM: Loader/HAL memory block indicates large pages cannot be used\n");
            MiLargeVaRanges[i].VirtualAddress = NULL;

            // Don't use large pages for anything if any individual range could not be used.
            // This is because 2 separate ranges may share a straddling large page.
            // If the first range was unable to use large pages, but the second one does ... then only part of the first range will get large pages if we enable large pages for the second range.
            // This would be very bad as we use the MI_IS_PHYSICAL macro everywhere and assume the entire range is in or out, so disable all large pages here instead.
            while (i != 0) {
                i -= 1;
    ...
    while (StartPde <= EndPde) {
        if (First == TRUE || MiIsPteOnPdeBoundary(StartPde)) {
            ...
            if (StartPxe->u.Hard.Valid == 0) {
                // Map in a page directory parent page, using the slush descriptor if one exists.
            ...
            if (StartPpe->u.Hard.Valid == 0) {
                // Map in a page directory, using the slush descriptor if one exists.
                TempPte.u.Hard.PageFrameNumber = MxGetNextPage(1, TRUE);
                *StartPpe = TempPte;
        ...
        if (StartPde->u.Hard.Valid == 0) {
            // Map in a page table page, using the slush descriptor if one exists.
            TempPte.u.Hard.PageFrameNumber = MxGetNextPage(1, TRUE);
            *StartPde = TempPte;
...
/*
Routine Description:
    This function computes the number of bytes needed to span the initial nonpaged pool and PFN database plus the color arrays.
    It rounds this up to a large page boundary and carves the memory from the free descriptor.
    If the physical memory is too sparse to use large pages for this, then fall back to using small pages.
Environment:
...
*/
{
    ...
    // Non-paged pool comprises 2 chunks.  The initial nonpaged pool grows up and the expansion nonpaged pool expands downward.

    // Initial non-paged pool is constructed so virtual addresses are also physically contiguous.
    if ((MmSizeOfNonPagedPoolInBytes >> PAGE_SHIFT) > (7 * (MxTotalFreePages >> 3))) {
    ...
    if (MmSizeOfNonPagedPoolInBytes < MmMinimumNonPagedPoolSize) {
        ...
        MmSizeOfNonPagedPoolInBytes += ((MxTotalFreePages - _x16mbnp) / _x1mbnp) * MmMinAdditionNonPagedPoolPerMb;
    }
    ...
    // If the registry specifies a total nonpaged pool percentage cap, enforce it here.
    if (MmMaximumNonPagedPoolPercent != 0) {
        ...
        MaximumNonPagedPoolInBytesLimit = ((MxTotalFreePages * MmMaximumNonPagedPoolPercent) / 100);
    ...
    MmSizeOfNonPagedPoolInBytes = MI_ROUND_TO_SIZE(MmSizeOfNonPagedPoolInBytes, PAGE_SIZE);

    // Don't let the initial nonpaged pool choice exceed what's actually available.
    ...
    // This is because the color arrays are going to be allocated at the end of the PFN database.
    ...
    // Adjust the number of secondary colors by the number of nodes in the machine.
    // The secondary color mask is NOT adjusted as it is used to control coloring within a node.
    // The node color is added to the color AFTER normal color calculations are performed.
    ...
    PfnAllocation = MI_ROUND_TO_SIZE(((MmHighestPossiblePhysicalPage + 1) * sizeof(MMPFN)) + (MmSecondaryColors * sizeof(MMCOLOR_TABLES) * 2), PAGE_SIZE);
    NumberOfBytes = MmSizeOfNonPagedPoolInBytes + PfnAllocation;

    // Align to large page size boundary, donating any extra to the nonpaged pool.
    ...
    if (MmMaximumNonPagedPoolInBytes == 0) {
        // Calculate the size of nonpaged pool, adding extra pages for every MB above 16mb.
    ...
    MmMaximumNonPagedPoolInBytes = MI_ROUND_TO_SIZE(MmMaximumNonPagedPoolInBytes, MM_MINIMUM_VA_FOR_LARGE_PAGE);
    ...
    if ((MmProtectFreedNonPagedPool == FALSE) && (MxMapLargePages != 0) && 
        ...
        if (BasePage == MxFreeDescriptor->BasePage) {
            // The descriptor starts on a large page aligned boundary so remove the large page span from the bottom of the free descriptor.
            ...
            if ((LastPage & ((MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT) - 1)) == 0) {
                // The descriptor ends on a large page aligned boundary so remove the large page span from the top of the free descriptor.
                MiInitialLargePage = LastPage - MiInitialLargePageSize;
                MxFreeDescriptor->PageCount -= (ULONG)MiInitialLargePageSize;
            } else {
                // The descriptor does not start or end on a large page aligned address so chop the descriptor.
                // The excess slush is added to the freelist by our caller.
        ...
        MiAddCachedRange(MiInitialLargePage, MiInitialLargePage + MiInitialLargePageSize - 1);
    } else {
        // Not enough contiguous physical memory in this machine to use large pages for the PFN database and color heads so fall back to small.
        // Continue to march on so the virtual sizes can still be computed properly.
        // Note this is not large page aligned so it can never be confused with a valid large page start.
    ...
    MmNonPagedPoolExpansionStart = (PVOID)((PCHAR)MmPfnDatabase + (MiInitialLargePageSize << PAGE_SHIFT));
...
*/
{
    ...
    // Note that the PAE mode of the processor does not support the global bit in PDEs which map 4K page table pages.
    TempPte = ValidKernelPte;
    TempPde = ValidKernelPde;
    ...
    // Get the lower bound of the free physical memory and the number of physical pages by walking the memory descriptor lists.
    ...
    while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
        ...
        if ((MemoryDescriptor->MemoryType != LoaderFirmwarePermanent) &&
            ...
            if ((MemoryDescriptor->MemoryType == LoaderFree) ||
                ...
                // Deliberately use >= instead of just > to force our allocation as high as physically possible.
                // This is to leave low pages for drivers which may require them.
    ...
    // Enabling special IRQL automatically disables mapping the kernel with large pages so we can catch kernel and HAL code.
    ...
    // MmDynamicPfn may have been initialized based on the registry to a value representing the highest physical address in gigabytes.
    ...
    *(PMEMORY_ALLOCATION_DESCRIPTOR)&MxOldFreeDescriptor = *MxFreeDescriptor;
    ...
    // This is because we will remove this amount from the free descriptor first and subsequently map it with large TB entries (so it requires natural alignment & size, thus take it before other allocations chip away at the descriptor).
    MiComputeInitialLargePage();

    // Calculate the starting address for nonpaged system space rounded down to a second level PDE mapping boundary.
    MmNonPagedSystemStart = (PVOID)(((ULONG_PTR)MmPfnDatabase - (((ULONG_PTR)MmNumberOfSystemPtes + 1) * PAGE_SIZE)) & (~PAGE_DIRECTORY2_MASK));
    if (MmNonPagedSystemStart < MM_LOWEST_NONPAGED_SYSTEM_START) {
        MmNonPagedSystemStart = MM_LOWEST_NONPAGED_SYSTEM_START;
        MmNumberOfSystemPtes = (ULONG)(((ULONG_PTR)MmPfnDatabase - (ULONG_PTR)MmNonPagedSystemStart) >> PAGE_SHIFT) - 1;
        ASSERT(MmNumberOfSystemPtes > 1000);
    }

    // Snap the system PTE start address as page directories and tables will be preallocated for this range.
    SystemPteStart = (PVOID)MmNonPagedSystemStart;

    // If special pool and/or the driver verifier is enabled, reserve extra virtual address space for special pooling now.
    // For now, arbitrarily don't let it be larger than paged pool (128gb).
    if ((MmVerifyDriverBufferLength != (ULONG)-1) || ((MmSpecialPoolTag != 0) && (MmSpecialPoolTag != (ULONG)-1))) {
        if (MmNonPagedSystemStart > MM_LOWEST_NONPAGED_SYSTEM_START) {
            ...
            // This is a pretty large machine.
            // Take some of the system PTEs and reuse them for special pool.
            MaxPool = (4 * _x4gb);
            ...
            MmNumberOfSystemPtes = (ULONG)(((ULONG_PTR)MmPfnDatabase - (ULONG_PTR)SystemPteStart) >> PAGE_SHIFT) - 1;
        }
    ...
    // Allocate page directory parents, directories and page table pages for system PTEs and expansion nonpaged pool.
    TempPte = ValidKernelPte;
    ...
    StartPde = MiGetPdeAddress((PVOID)((ULONG_PTR)MmPfnDatabase + (MiInitialLargePageSize << PAGE_SHIFT)));
    ...
    if (MxMapLargePages != 0) {
        ...
        // Ensure any slush in the encompassing large page(s) is always mapped as cached to prevent TB cache attribute conflicts.
        ...
        // or two distinct ranges which could potentially be both physically and virtually discontiguous) these two images reside in must contain only each other and/or FirmwareTemporary spans.
        // If there is anything else in the ranges then we cannot guarantee there will be no cache attribute conflicts so don't use large pages.
        ...
        if ((PageFrameIndex & (MM_PFN_MAPPED_BY_PDE - 1)) == 0) {
            ...
            if (MI_GET_PAGE_FRAME_FROM_PTE(PointerPte) + (DataTableEntry->SizeOfImage >> PAGE_SHIFT) == PageFrameIndexLow) {
                ...
                // Invert our locals so we can finish up with a single check below.
            ...
            if (PageFrameIndex == MI_GET_PAGE_FRAME_FROM_PTE(PointerPte)) {
                // The kernel and HAL are contiguous, note they may span more than one large page and usually (but not if the second image ends on a large page boundary) has slush on the end.
                PageFrameIndex += (DataTableEntry->SizeOfImage >> PAGE_SHIFT);
                if (PageFrameIndex & (MM_PFN_MAPPED_BY_PDE - 1)) {
                    // Locate the slush descriptor, if one is not found then do NOT use large pages.
                    ASSERT(MxSlushDescriptor1 == NULL);
                    TempSlushDescriptor = MiFindDescriptor(LoaderBlock, PageFrameIndex);
                    if (TempSlushDescriptor != NULL) {
                        MxSlushDescriptor1 = TempSlushDescriptor;
                        *(PMEMORY_ALLOCATION_DESCRIPTOR)&MxOldSlushDescriptor1 = *MxSlushDescriptor1;
                    }
                } else {
                    TempSlushDescriptor = (PMEMORY_ALLOCATION_DESCRIPTOR)1;
                    // The contiguous images end exactly on a large page boundary.
                }
                ...
                // The HAL does not immediately follow the kernel so check for a firmware temporary descriptor following each of them that must consume the rest of each large page.
                // Anything else is unexpected (ie, a downrev loader) and so don't use large pages in that case.
                if (PageFrameIndex & (MM_PFN_MAPPED_BY_PDE - 1)) {
                    // Locate the slush descriptor, if one is not found then do NOT use large pages.
                    ASSERT(MxSlushDescriptor1 == NULL);
                    TempSlushDescriptor = MiFindDescriptor(LoaderBlock, PageFrameIndex);
                ...
                if (TempSlushDescriptor != NULL) {
                    MiLargeVaRanges[MiLargeVaRangeIndex].VirtualAddress = DataTableEntryLow->DllBase;
                ...
                if (PageFrameIndex & (MM_PFN_MAPPED_BY_PDE - 1)) {
                    ...
                    if (TempSlushDescriptor != NULL) {
                        ...
                        *(PMEMORY_ALLOCATION_DESCRIPTOR)&MxOldSlushDescriptor2 = *MxSlushDescriptor2;
                    }
                } else {
                    TempSlushDescriptor = (PMEMORY_ALLOCATION_DESCRIPTOR)1;
                }
    ...
    // Initial nonpaged pool, the PFN database & the color arrays are placed here.
    TempPte = ValidKernelPte;
    TempPde = ValidKernelPde;
    ...
    if (MiInitialLargePage != (PFN_NUMBER)-1) {
        ...
        EndPde = MiGetPdeAddress((PVOID)((ULONG_PTR)MmPfnDatabase + (MiInitialLargePageSize << PAGE_SHIFT) - 1));
        ...
        EndPde = MiGetPdeAddress((PVOID)((ULONG_PTR)MmNonPagedPoolStart + (MmSizeOfNonPagedPoolInBytes - 1)));
    }
    ...
    while (StartPde <= EndPde) {
        if (First == TRUE || MiIsPteOnPdeBoundary(StartPde)) {
            ...
            StartPpe = MiGetPteAddress(StartPde);
            if (StartPpe->u.Hard.Valid == 0) {
                // Map in a page directory page, using the slush descriptor if one exists.
                NextPhysicalPage = MxGetNextPage(1, TRUE);
                TempPte.u.Hard.PageFrameNumber = NextPhysicalPage;
        ...
        if (MiInitialLargePage != (PFN_NUMBER)-1) {
            ...
            // Allocate a page table page here since we're not using large pages, using the slush descriptor if one exists.
            NextPhysicalPage = MxGetNextPage(1, TRUE);
    ...
    if (MiInitialLargePage != (PFN_NUMBER)-1) {
        ...
        // Large pages were not used because this machine's physical memory was not contiguous enough.

        // Go through the memory descriptors and for each physical page make sure the PFN database has a valid PTE to map it.
        // This allows machines with sparse physical memory to have a minimal PFN database.
        ...
        while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
            ...
            if ((MemoryDescriptor->MemoryType == LoaderFirmwarePermanent) ||
                (MemoryDescriptor->MemoryType == LoaderBBTMemory) ||
                (MemoryDescriptor->MemoryType == LoaderSpecialMemory)) {
                NextMd = MemoryDescriptor->ListEntry.Flink;// Skip these ranges.
                continue;
            }

            // Temporarily add back in the memory allocated since Phase 0 began so PFN entries for it will be created and mapped.

            // Note actual PFN entry allocations must be done carefully as memory from the descriptor itself could get used to map the PFNs for the descriptor !
    ...
    // Set subsection base to the address to zero as the PTE format allows the complete address space to be spanned.
    MmSubsectionBase = 0;

    // There must be at least one page of system PTEs before the expanded nonpaged pool.
    ...
    // This is due to the fact that the start and end of allocation bits for nonpaged pool are maintained in the PFN elements for the corresponding pages.
    ...
    // Ensure the hyperspace and session spaces are not mapped so they don't get made global by the loops below.
    ASSERT(MiGetPxeAddress(HYPER_SPACE)->u.Hard.Valid == 0);
    ASSERT(MiGetPxeAddress(MM_SESSION_SPACE_DEFAULT)->u.Hard.Valid == 0);

    // Go through the page table entries and for any page which is valid, update the corresponding PFN database element.
                        ...
                        if ((MmIsAddressValid(Pfn4)) && MmIsAddressValid((PUCHAR)(Pfn4 + 1) - 1)) {
                            if ((StartPte >= Kseg0StartPte) && (StartPte < Kseg0EndPte)) {
                                if (Pfn4->u3.e1.PageLocation == ActiveAndValid) {
                                    // Ignore stale KSEG0 mappings - the fact that the page is valid means we have already used it for a lower virtual address (like the system cache WSLEs).
                                    continue;
                                }
                            }
    ...
    // If the lowest physical page is zero and the page is still unused, mark it as in use.
    // This is because we want to find bugs where a physical page is specified as zero.
    Pfn1 = &MmPfnDatabase[MmLowestPhysicalPage];
    if ((MmLowestPhysicalPage == 0) && (Pfn1->u3.e2.ReferenceCount == 0)) {
        ASSERT(Pfn1->u3.e2.ReferenceCount == 0);

        // Make the reference count non-zero and point it into a page directory.
    ...
    // Since the LoaderBlock memory descriptors are ordered from low physical memory address to high, walk it backwards so the high physical pages go to the front of the freelists.
    // The thinking is that pages initially allocated by the system are less likely to be freed so don't waste memory below 16mb (or 4gb) that may be needed by ISA drivers later.
    NextMd = LoaderBlock->MemoryDescriptorListHead.Blink;
    while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
        MemoryDescriptor = CONTAINING_RECORD(NextMd, MEMORY_ALLOCATION_DESCRIPTOR, ListEntry);
        i = MemoryDescriptor->PageCount;
        PageFrameIndex = MemoryDescriptor->BasePage;
        if ((MemoryDescriptor == MxSlushDescriptor1) || (MemoryDescriptor == MxSlushDescriptor2)) {
            // Excess slush is given to expansion nonpaged pool here to ensure that it is always mapped fully cached since the rest of the large page is inserted as fully cached into the TB.
    ...
    if (MiInitialLargePage != (PFN_NUMBER)-1) {
        ...
        if (MmDynamicPfn == 0) {
            ...
            // This allows machines with sparse physical memory to have a minimal PFN database even when mapped with large pages.
            BottomPfn = MI_PFN_ELEMENT(MmHighestPhysicalPage);
            do {
                // Compute the address of the start of the page that is next lower in memory and scan backwards until that page address is reached or just crossed.
                ...
                // If the entire range over which the PFN entries span is completely zero and the PFN entry that maps the page is not in the range, then add the page to the free list.
    ...
    InitializePool(NonPagedPool, 0);// Initialize the nonpaged pool.

    // Adjust the memory descriptor to indicate that free pool has been used for nonpaged pool creation.

    // N.B.  This is required because the descriptors are walked upon return from this routine to create the MmPhysicalMemoryBlock.
    *MxFreeDescriptor = *(PMEMORY_ALLOCATION_DESCRIPTOR)&MxOldFreeDescriptor;
    ...
    // Map the hyper space page directory page into the top level parent directory & the hyper space page table page into the page directory and map an additional page that will eventually be used for the working set list.
    // Page tables after the first two are set up later on during individual process working set initialization.

    // The working set list page will eventually be a part of hyper space.
    // It is mapped into the second level page directory page so it can be zeroed and so it will be accounted for in the PFN database.
    // Later the page will be unmapped, and its page frame number captured in the system process object.
    TempPte = ValidKernelPte;
    ...
    *StartPde = TempPte;
    ...
    // Use the page frame number field of the first PTE as an offset into the available zeroing PTEs.
    ...
    // Initialize this process's memory management structures including the working set list.
    CurrentProcess = PsGetCurrentProcess();

    // The PFN element for the page directory has already been initialized, zero the reference count and the share count so they won't be wrong.
...
DECLSPEC_NOINLINE VOID MiGetStackPointer(OUT PULONG_PTR StackPointer)
/*
Routine Description:
...
*/
{
    // The AMD64 calling convention dictates that the home address of a callee's first parameter is the caller's stack pointer.
