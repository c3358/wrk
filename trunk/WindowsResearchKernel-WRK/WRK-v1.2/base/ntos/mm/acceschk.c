...
// It is indexed by a page protection.  The value of this array is added to the !WriteOperation value.
// If the value is 10 or less an access violation is issued (read-only - write_operation) = 9, (read_only - read_operation) = 10, etc.
CCHAR MmReadWrite[32] = {1, 10, 10, 10, 11, 11, 11, 11,
                         1, 10, 10, 10, 11, 11, 11, 11,
                         1, 10, 10, 10, 11, 11, 11, 11,
                         1, 10, 10, 10, 11, 11, 11, 11};

NTSTATUS MiClearStackGuardBits(IN PVOID CurrentSP, IN PVOID StackBase, IN PVOID DeallocationStack);
NTSTATUS MiClearBackingStoreGuardBits(IN PVOID CurrentBSP, IN PVOID StackBase, IN PVOID DeallocationBStore);
    ...
    */
{
    ...
    // Access is not allowed if the owner is kernel and the previous mode is user.
    // Access is also disallowed if the write operation is true and the write field in the PTE is false.

    // If both an access violation and a guard page violation could occur for the page, the access violation must be returned.
    ...
    if (MI_IS_GUARD(Protection)) {
        // If this thread is attached to a different process, return an access violation rather than a guard page exception.
        // This prevents problems with unwanted stack expansion and unexpected guard page behavior from debuggers.
        ...
        // Check to see if this is a transition PTE. If so, the PFN database original contents field needs to be updated.
        if ((PteContents.u.Soft.Transition == 1) && (PteContents.u.Soft.Prototype == 0)) {
            // Acquire the PFN lock and check to see if the PTE is still in the transition state.
            // If so, update the original PTE in the PFN database.
            ...
            if ((PteContents.u.Soft.Transition == 1) && (PteContents.u.Soft.Prototype == 0)) {
                ...
                // Note that forked processes using guard pages only take the guard page fault when the first thread in either process access the address.
                // This seems to be the best behavior we can provide users of this API as we must allow the first thread to make forward progress and the guard attribute is stored in the shared fork prototype PTE.
                if (PteContents.u.Soft.Protection == MM_NOACCESS) {
                    ASSERT((Pfn1->u3.e1.PrototypePte == 1) && (MiLocateCloneAddress(PsGetCurrentProcess(), Pfn1->PteAddress) != NULL));
                    if (CallerHoldsPfnLock == FALSE) {
                        UNLOCK_PFN(OldIrql);
                    }
                    return STATUS_ACCESS_VIOLATION;
                ...
                ASSERT((Pfn1->u3.e1.PrototypePte == 0) || (MiLocateCloneAddress(PsGetCurrentProcess(), Pfn1->PteAddress) != NULL));
...
NTSTATUS FASTCALL MiCheckForUserStackOverflow(IN PVOID FaultingAddress, IN PVOID OpaqueTrapInformation)
/*
Routine Description:
    This routine checks to see if the faulting address is within the stack limits and if so tries to create another guard page on the stack.
    A stack overflow is returned if the creation of a new guard page fails or if the stack is in the following form:
...
*/
{
    ...
    // Note we must bail when attached because we reference fields in our TEB below which have no relationship to the virtual address space of the process we are attached to.
    // Rather than introduce random behavior into applications, it's better to consistently return an AV for this scenario (one thread trying to grow another's stack).
    if (KeIsAttachedProcess()) {
        return STATUS_GUARD_PAGE_VIOLATION;
    }
    ...
    // Use an exception handler as the TEB is within the user's address space.
    try {
    ...
    {
        // An exception has occurred during the referencing of the TEB or TIB, just return a guard page violation and don't deal with the stack overflow.
        return STATUS_GUARD_PAGE_VIOLATION;
    }
    ...
    // If there is a stack guarantee (RegionSize nonzero), then increase our guard page size by 1 so that even a subsequent fault that occurs
    // midway (instead of at the beginning) through the first guard page will have the extra page to preserve the guarantee.

    // At the same time, reduce the MaximumAddress so that the highest possible guard page can still be used (otherwise our size increase would falsely disallow this).
    if (RegionSize != 0) {
    ...
    // The faulting address must be below the stack base and above the stack limit.
    if ((FaultingAddress >= StackBase) || (FaultingAddress < DeallocationStack)) {
        ...
        if (Process->Wow64Process != NULL) {
            ...
            if ((FaultingAddress >= StackBase) || (FaultingAddress < DeallocationStack)) {
                return STATUS_GUARD_PAGE_VIOLATION;// Not within the stack.
            }
            PageSize = PAGE_4K;
            MaximumAddress = DeallocationStack;

            // If there is a stack guarantee (RegionSize nonzero), then increase our guard page size by 1 so that even a subsequent
            // fault that occurs midway (instead of at the beginning) through the first guard page will have the extra page to preserve the guarantee.
            ...
            // Don't set the guard bit in the native PTE - just set it in the AltPte.
    ...
    if ((CurrentSP < StackBase) && (CurrentSP >= DeallocationStack)) {
        ...
        if (Vad != NULL) {
            ...
            while (CurrentSP >= DeallocationStack) {
                if (MiQueryAddressState(CurrentSP, Vad, Process, &OldProtection, &NextVaToQuery) == MEM_RESERVE) {
                    break;
                }

                // The state is committed (not reserved) so check for a guard bit.
        ...
        if (RegionSize != 0) {
            ...
            CurrentSP = (PVOID)((PCHAR)CurrentSP + PageSize);
            status = ZwAllocateVirtualMemory(NtCurrentProcess(), &CurrentSP, 0, &RegionSize, MEM_COMMIT, PAGE_READWRITE);
            if (NT_SUCCESS(status)) {
                try {
#if defined (_WIN64)
                    if (Teb32 != NULL) {
