...
Abstract:
    This module contains the routines which implement the NtFreeVirtualMemory service.
*/
...
Arguments:
    ...
    BaseAddress - The base address of the region of pages to be freed. This value is rounded down to the next host page address boundary.
    RegionSize - A pointer to a variable that will receive the actual size in bytes of the freed region of pages.
                 The initial value of this argument is rounded up to the next host page size boundary.
    FreeType - A set of flags that describe the type of free that is to be performed for the specified region of pages.
       FreeType Flags
...
*/
{
    ...
    if (((FreeType & (MEM_DECOMMIT | MEM_RELEASE)) == 0) || ((FreeType & (MEM_DECOMMIT | MEM_RELEASE)) == (MEM_DECOMMIT | MEM_RELEASE))) {
        return STATUS_INVALID_PARAMETER_4;
    }
    ...
    try {
        ...
        CapturedRegionSize = *RegionSize;// Capture the region size.
    ...
    if (CapturedBase > MM_HIGHEST_USER_ADDRESS) {
        return STATUS_INVALID_PARAMETER_2;// Invalid base address.
    }
    ...
    if (Vad->EndingVpn < MI_VA_TO_VPN(EndingAddress)) {
        // The entire range to delete is not contained within a single virtual address descriptor.
        // Return an error.
        Status = STATUS_UNABLE_TO_FREE_VM;
        goto ErrorReturn;
    }

    // Check to ensure this Vad is deletable.
    // Delete is required for both decommit and release.
    if (((Vad->u.VadFlags.PrivateMemory == 0) && (Vad->u.VadFlags.VadType != VadRotatePhysical)) || (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory)) {
        Status = STATUS_UNABLE_TO_DELETE_SECTION;
        goto ErrorReturn;
    }

    if (Vad->u.VadFlags.NoChange == 1) {
        // An attempt is being made to delete a secured VAD, check to see if this deletion is allowed.
        if (FreeType & MEM_RELEASE) {
            // Specify the whole range, this solves the problem with splitting the VAD and trying to decide where the various secure ranges need to go.
            Status = MiCheckSecuredVad((PMMVAD)Vad, MI_VPN_TO_VA(Vad->StartingVpn), ((Vad->EndingVpn - Vad->StartingVpn) << PAGE_SHIFT) + PAGE_SIZE, MM_SECURE_DELETE_CHECK);
        } else {
    ...
    if (FreeType & MEM_RELEASE) {
        ...
        // The descriptor for the address range is deletable.
        // Remove or split the descriptor.
        ...
        if (CapturedRegionSize == 0) {
            ...
            if (Vad->u.VadFlags.VadType == VadRotatePhysical) {
                Status = MiUnmapViewOfSection(Process, CapturedBase, UNMAP_ADDRESS_SPACE_HELD | UNMAP_ROTATE_PHYSICAL_OK);
            ...
            if (Vad->u.VadFlags.VadType == VadLargePages) {
                ...
                MiReleasePhysicalCharges(Vad->EndingVpn - Vad->StartingVpn + 1, Process);
                LOCK_WS_UNSAFE(CurrentThread, Process);
                MiFreeLargePages(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn), FALSE);
            ...
            // Free the VAD pool and release quota after releasing our mutexes to reduce contention.
        } else {
            // Region's size was not specified as zero, delete the whole VAD or split the VAD.
            if (MI_VA_TO_VPN(StartingAddress) == Vad->StartingVpn) {
                if (MI_VA_TO_VPN(EndingAddress) == Vad->EndingVpn) {
                    ...
                    // Free the VAD pool after releasing our mutexes to reduce contention.
                } else {
                    if ((Vad->u.VadFlags.VadType == VadAwe) ||
                        ...
                        // Splitting or chopping a physical VAD, large page VAD or a write-watch VAD is not allowed.
                        Status = STATUS_FREE_VM_NOT_AT_BASE;
                        goto ErrorReturn;
                    }
                    ...
                    // This Virtual Address Descriptor has a new starting address.
                ...
                if ((Vad->u.VadFlags.VadType == VadAwe) ||
                    ...
                    (Vad->u.VadFlags.VadType == VadRotatePhysical) ||
                    (Vad->u.VadFlags.VadType == VadWriteWatch)) {
                    Status = STATUS_FREE_VM_NOT_AT_BASE;// Splitting or chopping a physical VAD, large page VAD or a write-watch VAD is not allowed.
                    goto ErrorReturn;
                }
                ...
                if (MI_VA_TO_VPN(EndingAddress) == Vad->EndingVpn) {
                    ...
                    *NewVad = *Vad;
                    ...
                    NewVad->u.VadFlags.CommitCharge = 0;// Set the commit charge to zero so MiInsertVad will not charge commitment for splitting the VAD.

                    // Insert the VAD, this could fail due to quota charges.
                    Status = MiInsertVadCharges((PMMVAD)NewVad, Process);
                    if (!NT_SUCCESS(Status)) {
                        UNLOCK_ADDRESS_SPACE(Process);// The quota charging failed, free the new VAD and return an error.
                        ExFreePool(NewVad);
                        goto ErrorReturn2;
                    }
                    ...
                    // As we have split the original VAD into 2 separate VADs there is no way of knowing what the commit charge is for each VAD.
                    // Calculate the charge and reset each VAD.
                    // Note that we also use the previous value to make sure the books stay balanced.
                    QuotaCharge = MiCalculatePageCommitment(MI_VPN_TO_VA(Vad->StartingVpn), (PCHAR)StartingAddress - 1, (PMMVAD)Vad, Process);
    ...
    AllDone:
        ...
        {
            // An exception occurred, don't take any action (just handle the exception and return success.
        }
    ...
    if (Vad->u.VadFlags.VadType == VadAwe) {
        // Pages from a physical VAD must be released via NtFreeUserPhysicalPages, not this routine.
        Status = STATUS_MEMORY_NOT_ALLOCATED;
        goto ErrorReturn;
    }

    if ((Vad->u.VadFlags.VadType == VadLargePages) || (Vad->u.VadFlags.VadType == VadRotatePhysical)) {
        // Pages from a large page or rotate physical VAD must be released - they cannot be merely decommitted.
        Status = STATUS_MEMORY_NOT_ALLOCATED;
        goto ErrorReturn;
    }
    ...
    // Check to see if the entire range can be decommitted by just updating the virtual address descriptor.
...
*/
{
    ...
    while (PointerPte <= LastPte) {
        if (MiIsPteOnPdeBoundary(PointerPte) || (FirstTime)) {
            // This may be a PXE/PPE/PDE boundary, check to see if all the PXE/PPE/PDE pages exist.
            FirstTime = FALSE;
            ...
            do {
                ...
                retry:
                     ...
                     while (!MiDoesPxeExistAndMakeValid(PointerPxe, Process, MM_NOIRQL, &Waited)) {
                         // No PPE exists for the starting address, check the VAD to see if the pages are committed.
                         PointerPxe += 1;
                         ...
                         if (PointerPte > LastPte) {
                             if (Vad->u.VadFlags.MemCommit == 0) {// Make sure the entire range is committed.
                                 return FALSE;// The entire range to be decommitted is not committed, return an error.
                             }

                             return TRUE;
                         }

                         if (Vad->u.VadFlags.MemCommit == 0) {// Make sure the range thus far is committed.
                             return FALSE;// The entire range to be decommitted is not committed, return an error.
                         }
                     }

                     while (!MiDoesPpeExistAndMakeValid(PointerPpe, Process, MM_NOIRQL, &Waited)) {
                         // No PDE exists for the starting address, check the VAD to see if the pages are committed.
                         PointerPpe += 1;
                         PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                         PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
                         Va = MiGetVirtualAddressMappedByPte(PointerPte);
                     ...
                     while (!MiDoesPdeExistAndMakeValid(PointerPde, Process, MM_NOIRQL, &Waited)) {
                         ...
                         if (PointerPte > LastPte) {
                             ...
                             if (Vad->u.VadFlags.MemCommit == 0) {
                                 // The entire range to be decommitted is not committed, return an error.
                                 return FALSE;
                             }
                             return TRUE;
                         }
        ...
        if (PointerPte->u.Long == 0) {
            if (Vad->u.VadFlags.MemCommit == 0) {// This page has not been committed, check the VAD.
                return FALSE;// The entire range to be decommitted is not committed, return an error.
            }
        } else {
            if (MiIsPteDecommittedPage(PointerPte)) {// Has this page been explicitly decommitted?
                return FALSE;// This page has been explicitly decommitted, return an error.
            }
        }
...
*/
{
    ...
    // Loop through all the PDEs which map this region and ensure that they exist.
    // If they don't exist create them by touching a PTE mapped by the PDE.
    CurrentThread = PsGetCurrentThread();
    ...
    while (PointerPte <= EndingPte) {
        ...
        // The working set lock is held.
        // No PTEs can go from invalid to valid or valid to invalid.
        // Transition PTEs can go from transition to pagefile.
        ...
        if (PteContents.u.Long != 0) {
            if (PointerPte->u.Long == MmDecommittedPte.u.Long) {
                ...
                if (PteContents.u.Hard.Valid == 1) {
                    ...
                    if (Pfn1->u3.e1.PrototypePte) {
                        // MiDeletePte may release both the working set pushlock and the PFN lock so the valid PTE list must be processed now.
                        if (count != 0) {
                            MiProcessValidPteList(&ValidPteList[0], count);
                            count = 0;
                        }
                        ...
                        LOCK_PFN(OldIrql);
                        MiDeletePte(PointerPte, Va, FALSE, Process, NULL, NULL, OldIrql);
                        UNLOCK_PFN(OldIrql);
                        ...
                        // Add this entry to the list of free working set entries and adjust the working set count.
                        ...
                        if ((Locked.LockedInWs == 1) || (Locked.LockedInMemory == 1)) {
                            ...
                            if (WorkingSetIndex != MmWorkingSetList->FirstDynamic) {
                                ...
                                MiSwapWslEntries(Entry, WorkingSetIndex, &Process->Vm, FALSE);
                            }
                        }
                    ...
                    // Transition PTE, get the PFN database lock and reprocess this one.
                    LOCK_PFN(OldIrql);
                    PteContents = *PointerPte;
                    ...
                    if (PteContents.u.Soft.PageFileHigh != 0) {
                        ...
                        // Don't subtract out the private page count for a demand zero page.
                        Process->NumberOfPrivatePages += 1;
                    }
            ...
            if (PointerPte > CommitLimitPte) {// PTE is not committed.
                CommitReduction += 1;
            }
            MI_WRITE_INVALID_PTE(PointerPte, MmDecommittedPte);
        }
...
*/
{
    ...
    do {
        ...
        // Decrement the share and valid counts of the page table page which maps this PTE.
        PageTableFrameIndex = Pfn1->u4.PteFrame;
        Pfn2 = MI_PFN_ELEMENT(PageTableFrameIndex);
        MiDecrementShareCountInline(Pfn2, PageTableFrameIndex);
        MI_SET_PFN_DELETED(Pfn1);
        ...
        // As the page is private it will be put on the free list.
