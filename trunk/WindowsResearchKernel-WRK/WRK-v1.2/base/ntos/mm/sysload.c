...
Abstract:
    This module contains the code to load DLLs into the system portion of the address space and calls the DLL at its initialization entry point.
*/
...
// Enabled via the registry to identify drivers which unload without releasing resources or still have active timers, etc.
...
ULONG MmEnforceWriteProtection = 1;// This can be set by the registry.
...
VOID MiSetPagingOfDriver(IN PMMPTE PointerPte, IN PMMPTE LastPte);
PVOID MiLookupImageSectionByName(IN PVOID Base, IN LOGICAL MappedAsImage, IN PCHAR SectionName, OUT PULONG SectionSize);
...
LOGICAL MiCallDllUnloadAndUnloadDll(IN PKLDR_DATA_TABLE_ENTRY DataTableEntry);
...
/*
Routine Description:
    This function is a nonpaged wrapper which acquires the PsLoadedModuleList lock to insert a new entry.
Arguments:
    DataTableEntry - Supplies the loaded module list entry to insert/remove.
    Insert - Supplies TRUE if the entry should be inserted, FALSE if the entry should be removed.
Environment:
...
/*
Routine Description:
    Parse the registry settings and set up the list of driver names that we'll try to load in large pages.
Environment:
    Kernel mode, Phase 0 Initialization.
    Nonpaged pool exists but not paged pool.
    The PsLoadedModuleList has not been set up yet AND the boot drivers have NOT been relocated to their final resting places.
*/
{
    PWCHAR Start;
    PWCHAR End;
    ...
    while (Start < End) {
        ...
        Entry = ExAllocatePoolWithTag(NonPagedPool, sizeof(MI_LARGE_PAGE_DRIVER_ENTRY), 'pLmM');
        if (Entry == NULL) {
            break;
        }
...
Arguments:
    ...
    ImageBaseAddress - Supplies the current address the image header is at, and returns the (new) address for the image header.
...
*/
{
    ...
    // Capture cr4 to see if large page support has been enabled in the chip yet (late in Phase 1).
    // Large page PDEs cannot be used until then.
    ...
    // Check the number of free system PTEs left to prevent a runaway registry key from exhausting all the system PTEs.
    ...
    // Note we must allocate the entire large page here even though we will almost always only use a portion of it.
    // This is to ensure no other frames in it can be mapped with a different cache attribute.
    // After updating the cache attribute lists, we'll immediately free the excess.
    // Note that the driver's INIT section will be subsequently freed and clearly the cache attribute lists must be correct to support that as well.

    // Don't take memory below 16MB as we want to leave that available for ISA drivers supporting older hardware that may require it.
    NumberOfPages = (PFN_NUMBER)MI_ROUND_TO_SIZE(NumberOfPtes, MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT);
    PageFrameIndex = MiFindContiguousPages(
        (16 * 1024 * 1024) >> PAGE_SHIFT, MmHighestPossiblePhysicalPage,
        MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT,
        NumberOfPages,
        MmCached);

    // If a contiguous range is not available then large pages cannot be used for this driver at this time.
    if (PageFrameIndex == 0) {
        return FALSE;
    }

    // Add the contiguous range to the must-be-cached list so that the excess memory (and INIT section) can be safely freed to the page lists.
    ...
    LargeVa = MiMapWithLargePages(PageFrameIndex, NumberOfPages, MM_EXECUTE_READWRITE, MmCached);
    if (LargeVa == NULL) {
    ...
    // Inform our caller of the new (large page) address so the loader data table entry gets created with it & fixups done accordingly, etc.
    *ImageBaseAddress = LargeBaseVa;

    if (Pass != 0) {
        // The system is fully booted so get rid of the original mapping now.
        // Otherwise, we're in Phase 0, so the caller gets rid of the original mapping.
        SmallPte -= NumberOfPtes;

        // No explicit TB flush is done here, instead the system PTE management code will handle this in a lazy fashion.
        PagesRequired = MiDeleteSystemPageableVm(SmallPte, NumberOfPtes, 0, &ResidentPages);
        ...
        InterlockedExchangeAdd(&MmTotalSystemDriverPages, 0 - (ULONG)(PagesRequired - ResidentPages));
        MI_INCREMENT_RESIDENT_AVAILABLE(ResidentPages, MM_RESAVAIL_FREE_UNLOAD_SYSTEM_IMAGE1);
    ...
    // of the large page mapping are kept as they would have to be mapped cached to avoid TB attribute conflicts, 
    // and therefore cannot be placed on the general purpose freelists.
    return TRUE;
}
...
/*
Routine Description:
    This function stores the exception table information from the image in the loader data table entry.
Arguments:
...
*/
{
    ...
    if (IMAGE_DIRECTORY_ENTRY_EXCEPTION < NtHeader->OptionalHeader.NumberOfRvaAndSizes) {
        DataTableEntry->ExceptionTable = (PCHAR)CurrentBase + NtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].VirtualAddress;
        DataTableEntry->ExceptionTableSize = NtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size;
    }
#endif
}
...
*/
{
    ...
    // but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    ...
    if (LoadedBaseName) {// If there's a loaded base name, use it instead of the base name.
...
ReCheckLoaderList:
    ...
    if (NextEntry != &PsLoadedModuleList) {
        ...
        // If this thread already loaded the image below and upon rechecking finds some other thread did so also, 
        // then get rid of our object now and use the other thread's inserted entry instead.
        ...
        if ((LoadFlags & MM_LOAD_IMAGE_IN_SESSION) == 0) {
            if (MI_IS_SESSION_ADDRESS(DataTableEntry->DllBase) == TRUE) {
                // The caller is trying to load a driver in systemwide space that has already been loaded in session space.
                // This is not allowed.
        ...
        if (MI_IS_SESSION_ADDRESS(DataTableEntry->DllBase) == FALSE) {
            // The caller is trying to load a driver in session space that has already been loaded in system space.
            Status = STATUS_CONFLICTING_ADDRESSES;// This is not allowed.
            goto return2;
        }

        AlreadyOpen = TRUE;

        // This image has already been loaded systemwide.
        // If it's already been loaded in this session space as well, 
        // just bump the reference count using the already allocated address.
        // Otherwise, insert it into this session space.
        Status = MiSessionInsertImage(DataTableEntry->DllBase);
        if (!NT_SUCCESS(Status)) {
            if (Status == STATUS_ALREADY_COMMITTED) {// This driver's already been loaded in this session.
            ...
            // The LoadCount should generally not be 0 here, 
            // but it is possible in the case where an attempt has been made to unload a DLL on last dereference, 
            // but the DLL refused to unload.
            goto return2;
        }

        // This driver is already loaded in the system, but not in this particular session - share it now.
    ...
    } else if (SectionPointer == NULL) {// This image is not already loaded.
     // A NULL SectionPointer indicates this thread didn't already load this image below either, so go and get it.
     ...
     // Release the load lock first as getting the image is not cheap.
        ...
        // Check and see if a user wants to replace this binary via a transfer through the kernel debugger.
        // If this fails just continue on with the existing file.
        if ((KdDebuggerEnabled) && (KdDebuggerNotPresent == FALSE)) {
            Status = KdPullRemoteFile(ImageFileName, FILE_ATTRIBUTE_NORMAL, FILE_OVERWRITE_IF, FILE_SYNCHRONOUS_IO_NONALERT);
            if (NT_SUCCESS(Status)) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "MmLoadSystemImage: Pulled %wZ from kd\n", ImageFileName);
            }
        }
        ...
        Status = ZwOpenFile(&FileHandle, FILE_EXECUTE, &ObjectAttributes, &IoStatus, FILE_SHARE_READ | FILE_SHARE_DELETE, 0);
        if (!NT_SUCCESS(Status)) {
#if DBG
            if (NtGlobalFlag & FLG_SHOW_LDR_SNAPS) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MmLoadSystemImage: cannot open %wZ\n", ImageFileName);
            }
#endif
            goto return2;// File not found.
        }
        ...
        // Now attempt to create an image section for the file.
        // If this fails, then the driver file is not an image.
        // Session space drivers are shared text with copy on write data, so don't allow writes here.
        if (LoadFlags & MM_LOAD_IMAGE_IN_SESSION) {
        ...
        // Now reference the section handle.  If this fails something is very wrong because it is a kernel handle.
        ...
        if ((LoadFlags & MM_LOAD_IMAGE_IN_SESSION) &&
            ...
            // It is assumed that all session drivers live on the same physical drive, 
            // so when the very first session driver is loaded, this check can be made.

            // Note that this is important because these drivers are always paged directly in/out from the filesystem so the drive containing the filesystem better not get removed !

            // This is skipped for the WinPE removable media boot case because the user may be running WinPE in RAM and want to swap out the boot media.
            // In this case, the control area is marked as FloppyMedia (even if it was CD-based) as all the pages have already been converted to pagefile-backed.
            do {
                ...
                if (InterlockedCompareExchange(&MiFirstDriverLoadEver, OldValue | 0x2, OldValue) == OldValue) {
                    ...
                    if (!NT_SUCCESS(Status)) {
                        ...
                        // Failing the insertion of win32k.sys' device in the pagefile path is commented out until the storage drivers have been modified to correctly handle this request.
                        // If this is added later, add code here to release relevant resources for this error path.
                    }
        ...
        // So before using the section we just created, go back and see if any other threads have slipped through and did it already.
    ...
    // Normal drivers are dereferenced here and their images can then be overwritten.
    // This is ok because we've already read the whole thing into memory and from here until reboot (or unload), we back them with the pagefile.

    // Session space drivers are the exception - these images are inpaged from the filesystem and we need to keep our reference to the file so that it doesn't get overwritten.
    if ((LoadFlags & MM_LOAD_IMAGE_IN_SESSION) == 0) {
        if (NT_SUCCESS(Status)) {
            // Move the driver into a large page if requested via the registry.
            MiUseLargeDriverPage(SectionPointer->Segment->TotalNumberOfPtes, ImageBaseAddress, &BaseName, 1);
        }
    ...
    // The LoadCount will be >1 if it was attached to by a session (as opposed to just loaded).
    if (!NT_SUCCESS(Status)) {
        if (AlreadyOpen == TRUE) {
            // We're failing and we were just attaching to an already loaded driver.
            // We don't want to go through the forced unload path because we've already deleted the address space so decrement our reference and clear the DataTableEntry.
    ...
    // been loaded - and the DataTableEntry->LoadCount has been bumped - so the error recovery from here on out is to just call MmUnloadSystemImage with the DataTableEntry.

    // If this is the first load of a given driver into a session space, we have no DataTableEntry at this point.
    // The view has already been mapped and committed and the group/session addresses reserved for this DLL.
    // The error recovery path handles all this because MmUnloadSystemImage zeroes the relevant fields in the LDR_DATA_TABLE_ENTRY so that MmUnloadSystemImage works properly.
    IssueUnloadOnFailure = TRUE;
    if (AlreadyOpen == FALSE) {
        if (((LoadFlags & MM_LOAD_IMAGE_IN_SESSION) == 0) ||
            (*ImageBaseAddress != SectionPointer->Segment->BasedAddress)) {
            // Apply the fixups to the section.
            // Note session images need only be fixed up once per insertion in the loaded module list.
            try {
            ...
            if (!NT_SUCCESS(Status)) {
                goto return1;// Unload the system image and dereference the section.
            }
        }
        ...
        // Create a loader table entry for this driver before resolving the references so that any circular references can resolve properly.
        if (LoadFlags & MM_LOAD_IMAGE_IN_SESSION) {
                ...
                if (DataDirectory->VirtualAddress && DataDirectory->Size &&
                    ...
                    for (i = 0; i < DataDirectory->Size / sizeof(IMAGE_DEBUG_DIRECTORY); i += 1) {
                        if ((DebugDir + i)->AddressOfRawData &&
                            (DebugDir + i)->AddressOfRawData < NtHeaders->OptionalHeader.SizeOfImage &&
                            ((DebugDir + i)->AddressOfRawData + (DebugDir + i)->SizeOfData) < NtHeaders->OptionalHeader.SizeOfImage) {
            ...
            if (DebugDir) {
                ...
                for (i = 0; i < DataDirectory->Size / sizeof(IMAGE_DEBUG_DIRECTORY); i += 1) {
                    if ((DebugDir + i)->AddressOfRawData &&
                        ...
                        ((DebugDir + i)->AddressOfRawData + (DebugDir + i)->SizeOfData) < NtHeaders->OptionalHeader.SizeOfImage) {
                        RtlCopyMemory(
                            (PUCHAR)(ssHeader + 1) + DebugInfoSize,
                            (PUCHAR)(*ImageBaseAddress) + (DebugDir + i)->AddressOfRawData,
                            (DebugDir + i)->SizeOfData);

                        // Reset the offset in the debug directory to point to
                        (((PIMAGE_DEBUG_DIRECTORY)(ssHeader + 1)) + i)->AddressOfRawData = DebugInfoSize;
                        DebugInfoSize += (DebugDir + i)->SizeOfData;
                    } else {
                        (((PIMAGE_DEBUG_DIRECTORY)(ssHeader + 1)) + i)->AddressOfRawData = 0;
                    }
                }
            }
        }
        ...
        MiCaptureImageExceptionValues(DataTableEntry);// Capture the exception table data info

        // Acquire the loaded module list resource and insert this entry into the list.
        MiProcessLoaderEntry(DataTableEntry, TRUE);
        MissingProcedureName = NameBuffer;
        try {
            // Resolving the image references results in other DLLs being loaded if they are referenced by the module that was just loaded.
            // An example is when an OEM printer or FAX driver links with other general libraries.
            // This is not a problem for session space because the general libraries do not have the global data issues that win32k.sys and the video drivers do.
            // So we just call the standard kernel reference resolver and any referenced libraries get loaded into system global space.
            // Code in the routine restricts which libraries can be referenced by a driver.
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            if ((Status == STATUS_DRIVER_ORDINAL_NOT_FOUND) ||
                ...
                if ((ULONG_PTR)MissingProcedureName & ~((ULONG_PTR)(X64K - 1))) {
                    ...
                    DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MissingProcedureName 0x%p\n", MissingProcedureName);
                }
            }

            if (MissingDriverName != NULL) {
                PrintableMissingDriverName = (PWSTR)((ULONG_PTR)MissingDriverName & ~0x1);
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MissingDriverName %ws\n", PrintableMissingDriverName);
            }
#endif
        ...
        if (LoadFlags & MM_LOAD_IMAGE_IN_SESSION) {
            // Attempt to lookup the win32k system service table.
            // If the service table is found, then compact the table into an array of relative 32-bit pointers.
            ...
            Match = RtlEqualUnicodeString(&DataTableEntry->BaseDllName, &Name, TRUE);
            if (Match == TRUE) {
                Status1 = LookupEntryPoint(DataTableEntry->DllBase, "W32pServiceTable", &Table);
                Status2 = LookupEntryPoint(DataTableEntry->DllBase, "W32pServiceLimit", &Limit);
            ...
            // The session image was mapped entirely read-write on initial creation.
            // Now that the relocations (if any), image resolution and import table updates are complete, 
            // correct permissions can be applied.

            // Make the entire image copy on write.
            // The subsequent call to MiWriteProtectSystemImage will make various portions readonly.
            // Then apply flip various subsections into global shared mode if their attributes specify it.
            PointerPte = MiGetPteAddress(DataTableEntry->DllBase);
            MiSetSystemCodeProtection(PointerPte, PointerPte + NumberOfPtes - 1, MM_EXECUTE_WRITECOPY);
        }
        ...
        if (MiCacheImageSymbols(*ImageBaseAddress)) {//  TEMP TEMP TEMP rip out when debugger converted
    ...
    if (!NT_SUCCESS(Status)) {
        if (IssueUnloadOnFailure == TRUE) {
            if (DataTableEntry == NULL) {
                ...
                // If DataTableEntry is NULL, then we are unloading before one got created.
                // Once a LDR_DATA_TABLE_ENTRY is created, the load cannot fail, 
                // so if it exists here, at least one other session contains this image as well.
                ASSERT(DataTableEntry->LoadCount > 1);
            }
#endif
        ...
        if ((AlreadyOpen == FALSE) && (SectionPointer != NULL)) {
            // This is needed for failed win32k.sys loads or any session's load of the first instance of a driver.
            ObDereferenceObject(SectionPointer);
        }
    }
        ...
        *(&ErrorStrings[0]) = *ImageFileName;
        ...
        if ((Status == STATUS_DRIVER_ORDINAL_NOT_FOUND) ||
            ...
            *(&ErrorStrings[2]) = *(&DriverName);
            if ((ULONG_PTR)MissingProcedureName & ~((ULONG_PTR)(X64K - 1))) {// If not an ordinal, pass as a Unicode string
                ...
                UniqueErrorValue = PtrToUlong(MissingProcedureName);// Just pass ordinal values as is in the UniqueErrorValue.
            }
        } else {
        GenericError:
            UniqueErrorValue = (ULONG)Status;
            if (MmIsRetryIoStatus(Status)) {
                Status = STATUS_INSUFFICIENT_RESOURCES;// Coalesce the various low memory values into just one.
            } else {
                // Ideally, the real failing status should be returned. 
                // However, we need to do a full release worth of testing (ie Longhorn) before making that change.
        ...
        if (PacketSize < MAXUCHAR) {// Enforce I/O manager interface (ie: UCHAR) size restrictions.
            ErrLog = IoAllocateGenericErrorLogEntry((UCHAR)PacketSize);
            if (ErrLog != NULL) {// Fill it in and write it out as a single string.
        ...
        // The only way this pointer has the low bit set is if we are expected to free the pool containing the name.
        // Typically the name points at a loaded module list entry and so no one has to free it and in this case the low bit will NOT be set.
        // If the module could not be found and was therefore not loaded, 
        // then we left a piece of pool around containing the name since there is no loaded module entry already - this must be released now.
...
VOID MiReturnFailedSessionPages(IN PMMPTE PointerPte, IN PMMPTE LastPte)
/*
Routine Description:
    This routine is a nonpaged wrapper which undoes session image loads that failed midway through reading in the pages.
Arguments:
...
*/
{
    ...
    while (PointerPte <= LastPte) {
        if (PointerPte->u.Hard.Valid == 1) {
            PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte);// Delete the page.

            // Set the pointer to PTE as empty so the page is deleted when the reference count goes to zero.
...
*/
{
    ...
    if (LoadInSessionSpace != 0) {
        // Allocate a unique systemwide session space virtual address for the driver.
        if (FoundDataTableEntry == NULL) {
            Status = MiSessionWideReserveImageAddress(SectionPointer, &BaseAddress, &NewSectionPointer);
            if (!NT_SUCCESS(Status)) {
                return Status;
            }
        ...
        // All pristine pages of the image will be shared across all sessions, with each page treated as copy-on-write on first write.

        // NOTE: This makes the file image "busy", a different behavior as normal kernel drivers are backed by the paging file only.
        Status = MiShareSessionImage(BaseAddress, SectionPointer);
        if (!NT_SUCCESS(Status)) {
            MiRemoveImageSessionWide(FoundDataTableEntry, BaseAddress, NumberOfPtes << PAGE_SHIFT);
            return Status;
        }
    ...
    // Start out by charging for everything and subtract out any gap pages after the image loads successfully.
    ...
    if (FirstPte == NULL) {
        MI_INCREMENT_RESIDENT_AVAILABLE(PagesRequired, MM_RESAVAIL_FREE_LOAD_SYSTEM_IMAGE1);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    Process = PsGetCurrentProcess();// Map a view into the user portion of the address space.
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        MI_INCREMENT_RESIDENT_AVAILABLE(PagesRequired, MM_RESAVAIL_FREE_LOAD_SYSTEM_IMAGE2);
        MiReleaseSystemPtes(FirstPte, NumberOfPtes, SystemPteSpace);
        MiReturnCommitment(PagesRequired);
        return Status;
    }
    ...
    // Note for session drivers, the physical pages have already been allocated and just data copying is done here.
    ...
    *ImageBaseAddress = SystemVa;
    ...
    while (PointerPte < LastPte) {
        if (ProtoPte >= LastProto) {// Handle extended subsections.
        ...
        if ((PteContents.u.Hard.Valid == 1) || (PteContents.u.Soft.Protection != MM_NOACCESS)) {
            ...
            {
                // An exception occurred, unmap the view and return the error to the caller.
                ...
                MI_INCREMENT_RESIDENT_AVAILABLE(PagesRequired, MM_RESAVAIL_FREE_LOAD_SYSTEM_IMAGE3);
                MiReleaseSystemPtes(FirstPte, NumberOfPtes, SystemPteSpace);
                MiReturnCommitment(PagesRequired);
                Status = MiUnmapViewOfSection(TargetProcess, Base, 0);
                ASSERT(NT_SUCCESS(Status));

                // Purge the section as we want these pages on the freelist instead of at the tail of standby, as we're completely done with the section.
                // This is because other valuable standby pages end up getting reused (especially during bootup) when the section pages are the ones that really will never be referenced again.

                // Note this isn't done for session images as they're inpaged directly from the filesystem via the section.
                MmPurgeSection(ControlArea->FilePointer->SectionObjectPointer, NULL, 0, FALSE);
                KeUnstackDetachProcess(&ApcState);
                return ExceptionStatus;
            }
        } else {// The PTE is no access.
            MI_WRITE_ZERO_PTE(PointerPte);
        }
    ...
    if (PagesRequired != ActualPagesUsed) {
        ...
        MI_INCREMENT_RESIDENT_AVAILABLE(PagesRequired, MM_RESAVAIL_FREE_LOAD_SYSTEM_IMAGE_EXCESS);
        MiReturnCommitment(PagesRequired);
    }
...
/*
Routine Description:
    This routine removes the pages that relocate and debug information from the address space of the driver.
    NOTE:  This routine looks at the last sections defined in the image header and if that section is marked as DISCARDABLE in the characteristics,
           it is removed from the image.
           This means that all discardable sections at the end of the driver are deleted.
Arguments:
...
*/
{
    ...
    for (i = 0; i < NtHeaders->FileHeader.NumberOfSections; i += 1) {
        ...
        if ((NtSection->Characteristics & IMAGE_SCN_MEM_DISCARDABLE) != 0) {
            ...
            break;// There was a non discardable section between this section and the last non discardable section, don't discard this section and don't look any more.
        }
    }

    if (FoundSection != NULL) {
        StartVa = (PVOID)(ROUND_TO_PAGES((PCHAR)Base + FoundSection->VirtualAddress));
        PointerPte = MiGetPteAddress(StartVa);
        NumberOfPtes = (PFN_NUMBER)(LastPte - PointerPte);
        if (NumberOfPtes != 0) {
            if (MI_IS_PHYSICAL_ADDRESS(StartVa)) {
                // Don't free the INIT code for a driver mapped by large pages because if it unloads later, 
                // we'd have to deal with discontiguous ranges of pages to free.
                return;
            } else {
                PagesDeleted = MiDeleteSystemPageableVm(PointerPte, NumberOfPtes, MI_DELETE_FLUSH_TB, NULL);
            }

            MI_INCREMENT_RESIDENT_AVAILABLE(PagesDeleted, MM_RESAVAIL_FREE_DRIVER_INITIALIZATION);
...
*/
{
    ...
    // Put the PTEs in transition and decrement the number of valid PTEs within the containing page table page.
    // Note that for a private page, the page table page is still needed because the page is in transition.
    LOCK_PFN(OldIrql);
...
VOID MiEnablePagingOfDriver(IN PVOID ImageHandle)
{
    ...
    // Initializing LastPte is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    LastPte = NULL;
    while (i > 0) {
#if DBG
        if ((*(PULONG)FoundSection->Name == 'tini') || (*(PULONG)FoundSection->Name == 'egap')) {
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "driver %wZ has lower case sections (init or pagexxx)\n", &DataTableEntry->FullDllName);
        }
#endif

        // Mark as pageable any section which starts with the first 4 characters PAGE or .eda (for the .edata section).
        if ((*(PULONG)FoundSection->Name == 'EGAP') || (*(PULONG)FoundSection->Name == 'ade.')) {
            if (PointerPte == NULL) {// This section is pageable, save away the start and end.
                PointerPte = MiGetPteAddress((PVOID)(ROUND_TO_PAGES((PCHAR)Base + FoundSection->VirtualAddress)));// Previous section was NOT pageable, get the start address.
            }
            ...
            // This section is not pageable, if the previous section was pageable, enable it.
            if (PointerPte != NULL) {
...
Environment:
    ...
    This routine could be made PAGELK but it is a high frequency routine so it is actually better to keep it nonpaged to avoid bringing in the entire PAGELK section.
*/
{
    PVOID Base;
    ...
    while (PointerPte <= LastPte) {
        // Check to make sure this PTE has not already been made pageable (or deleted).
        // It is pageable if it is not valid, or if the PFN database wsindex element is non zero.
        if (PointerPte->u.Hard.Valid == 1) {
            PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte);
            Pfn = MI_PFN_ELEMENT(PageFrameIndex);
            ASSERT(Pfn->u2.ShareCount == 1);

            // If the wsindex is nonzero, then this page is already pageable and has a WSLE entry.
            // Ignore it here and let the trimmer take it if memory comes under pressure.
            if (Pfn->u1.WsIndex == 0) {
    ...
    if (PageCount != 0) {
        ...
        MI_INCREMENT_RESIDENT_AVAILABLE(PageCount, MM_RESAVAIL_FREE_SET_DRIVER_PAGING);
    }
}


PVOID MmPageEntireDriver(__in PVOID AddressWithinSection)
/*
Routine Description:
    This routine allows a driver to page out all of its code and data regardless of the attributes of the various image sections.
    Note, this routine can be called multiple times with no intervening calls to MmResetDriverPaging.
Arguments:
...
*/
{
    ...
    if (DataTableEntry->SectionPointer != NULL) {
        // Driver is mapped as an image (ie: session space), this is always pageable.
...
VOID MmResetDriverPaging(__in PVOID AddressWithinSection)
/*
Routine Description:
    This routines resets the driver paging to what the image specified.
    Hence image sections such as the IAT, .text, .data will be locked down in memory.

    Note, there is no requirement that MmPageEntireDriver was called.
Arguments:
    AddressWithinSection - Supplies an address within the driver, e.g. DriverEntry.
Environment:
    Kernel mode, APC_LEVEL or below.
*/
{
    ...
    DataTableEntry = MiLookupDataTableEntry(AddressWithinSection, FALSE);
    if (DataTableEntry->SectionPointer != NULL) {
        ASSERT(MI_IS_SESSION_IMAGE_ADDRESS(AddressWithinSection) == TRUE);// Driver is mapped by image hence already paged.
        return;
    }
    ...
    while (i > 0) {
        ...
        // Don't lock down code for sections marked as discardable or sections marked with the first 4 characters PAGE or .eda (for the .edata section) or INIT.
        if (((FoundSection->Characteristics & IMAGE_SCN_MEM_DISCARDABLE) != 0) ||
            ...
            NOTHING;
        } else {// This section is non-pageable.
            ...
            LastPte = MiGetPteAddress((PCHAR)Base + FoundSection->VirtualAddress + (Span - 1));
...
/*
Routine Description:
    Free up the import list and clear the pointer.  This stops the recursion performed in MiDereferenceImports().
Arguments:
...
*/
{
    ...
    if (DataTableEntry->LoadedImports == (PVOID)NO_IMPORTS_USED) {
        ...
        ExFreePool((PVOID)DataTableEntry->LoadedImports);// free the memory
    }

    DataTableEntry->LoadedImports = (PVOID)LOADED_AT_BOOT;// stop the recursion
}
...
/*
Routine Description:
    This routine saves information about unloaded drivers so that ones that forget to delete lookaside lists or queues can be caught.
Arguments:
...
*/
{
    ...
    if (DriverName->Length == 0) {
        return;// This is an aborted load and the driver name hasn't been filled in yet.  No need to save it.
    }

    // Serialization is provided by the caller, so just update the list now.
    // Note the allocations are nonpaged so they can be searched at bugcheck time.
...
/*
Routine Description:
    This routine attempts to find the specified virtual address in the unloaded driver list.
Arguments:
    VirtualAddress - Supplies a virtual address that might be within a driver that has already unloaded.
Return Value:
...
/*
Routine Description:
    This routine unloads a previously loaded system image and returns the allocated resources.
Arguments:
    ImageHandle - Supplies a pointer to the section object of the image to unload.
Return Value:
...
*/
{
    ...
    KeWaitForSingleObject(&MmSystemLoadLock, WrVirtualMemory, KernelMode, FALSE, (PLARGE_INTEGER)NULL);
    if (DataTableEntry->LoadedImports == (PVOID)LOADED_AT_BOOT) {
        // Any driver loaded at boot that did not have its import list and 
        // LoadCount reconstructed cannot be unloaded because we don't know how many other drivers may be linked to it.
        KeReleaseMutant(&MmSystemLoadLock, 1, FALSE, FALSE);
        KeLeaveCriticalRegionThread(CurrentThread);
    ...
    if (MI_IS_SESSION_IMAGE_ADDRESS(BasedAddress)) {
        // A printer driver may be referenced multiple times for the same session space.  Only unload the last reference.
        ...
        // The reference count for this image has dropped to zero in this session, so we can delete this session's view of the image.
        NumberOfBytes = DataTableEntry->SizeOfImage;

        // Free the session space taken up by the image, unmapping it from the current VA space - note this does not remove page table pages from the session PageTables[].
        // Each data page is only freed if there are no other references to it (ie: from any other sessions).
        PointerPte = MiGetPteAddress(BasedAddress);
        LastPte = MiGetPteAddress((PVOID)((ULONG_PTR)BasedAddress + NumberOfBytes));
        PagesRequired = MiDeleteSystemPageableVm(PointerPte, (PFN_NUMBER)(LastPte - PointerPte), MI_DELETE_FLUSH_TB, &ResidentPages);

        // Note resident available is returned here without waiting for load count to reach zero because it is charged each time a session space
        // driver locks down its code or data regardless of whether it is really the same copy-on-write backing page(s) that some other session has already locked down.
        MI_INCREMENT_RESIDENT_AVAILABLE(ResidentPages, MM_RESAVAIL_FREE_UNLOAD_SYSTEM_IMAGE);
        ...
        // Return the commitment we took out on the pagefile when the image was allocated.
        MiReturnCommitment(CommittedPages);
        MM_TRACK_COMMIT(MM_DBG_COMMIT_RETURN_DRIVER_UNLOAD, CommittedPages);

        // Tell the session space image handler that we are releasing our claim to the image.
        ASSERT(DataTableEntry->LoadCount != 0);
        MiRemoveImageSessionWide(DataTableEntry, BasedAddress, DataTableEntry->SizeOfImage);
    ...
    // Therefore, large pages are already in effect (if this platform supports it).
    if (ViewDeleted == FALSE) {
        ...
        if (DataTableEntry->Flags & LDRP_SYSTEM_MAPPED) {
            if (MI_PDE_MAPS_LARGE_PAGE(MiGetPdeAddress(BasedAddress))) {
                ...
                RoundedNumberOfPtes = MI_ROUND_TO_SIZE(NumberOfPtes, MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT);
                MiUnmapLargePages(BasedAddress, RoundedNumberOfPtes << PAGE_SHIFT);

                // MiFreeContiguousPages is going to return commitment and resident available so don't do it here.
                ...
                // No explicit TB flush is done here, instead the system PTE management code will handle this in a lazy fashion.
                PagesRequired = MiDeleteSystemPageableVm(PointerPte, NumberOfPtes, 0, &ResidentPages);

                // Note that drivers loaded at boot that have not been relocated have no system PTEs or commit charged.
            ...
            // This must be a boot driver that was not relocated into system PTEs.
            // If large or super pages are enabled, the image pages must be freed without referencing the non-existent page table pages.
            // If large/super pages are not enabled, note that system PTEs were not used to map the image and thus, cannot be freed.

            // This is further complicated by the fact that the INIT and/or discardable portions of these images may have already been freed.
            MI_INCREMENT_RESIDENT_AVAILABLE(NumberOfPtes, MM_RESAVAIL_FREE_UNLOAD_SYSTEM_IMAGE1);
    ...
    // It is possible an entry is not in the list if a failure occurred at a point in loading the DLL just before the data table entry was generated.
    ...
    // Handle unloading of any dependent DLLs that we loaded automatically for this image.
    ...
    if (MustFree == TRUE) {
        ...
        if (DataTableEntry->SectionPointer != NULL) {
            ObDereferenceObject(DataTableEntry->SectionPointer);// Dereference the section object (session images only).
        }
...
Routine Description:
    ...
    If a chain can be successfully built,
    then this driver's DLLs will be automatically unloaded if this driver goes away (provided no other driver is also using them).
    Otherwise, on driver unload, its dependent DLLs would have to be explicitly unloaded.

    Note that the incoming LoadCount values are not correct and thus, they are reinitialized here.
Return Value:
    Various NTSTATUS codes.
*/
{
...
#define HAL_NAME L"hal.dll"
    ...
    for (; NextEntry != &PsLoadedModuleList; NextEntry = NextEntry->Flink) {
        ...
        // Initializing these locals is not needed for correctness, 
        // but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
        j = 0;
        LastAddress = 0;
        for (i = 0; i < ImportSize; i += 1, ImportThunk += 1) {
            if (BaseAddress != 0) {// Check the hint first.
        ...
        // Initializing LastImageReference is not needed for correctness, 
        // but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
...
finished:
    ...
    if (UndoEverything == TRUE) {
        // An error occurred and this is an all or nothing operation so roll everything back.
        NextEntry = PsLoadedModuleList.Flink;
        while (NextEntry != &PsLoadedModuleList) {
            DataTableEntry = CONTAINING_RECORD(NextEntry, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
...
Routine Description:
    ...
    If the DLL has an unload routine that returns SUCCESS, then we clean it up and free up its memory now.

    Note this routine is NEVER called for drivers - only for DLLs that were loaded due to import references from various drivers.
Arguments:
...
*/
{
    ...
    if (Func) {
        Status = Func();// The unload function was found in the DLL so unload it now.
        if (NT_SUCCESS(Status)) {
            // Set up the reference count so the import DLL looks like a regular driver image is being unloaded.
            ASSERT(DataTableEntry->LoadCount == 0);
            DataTableEntry->LoadCount = 1;
...
*/
{
    ...
    if (ExportDirectory) {
        ...
        while (High >= Low) {
            // Compute the next probe index and compare the export name entry with the specified function name.
            Middle = (Low + High) >> 1;
        ...
        // Otherwise, get the ordinal number from the ordinal table and location the function address.
        if (High >= Low) {
            ...
            // then the function is forwarded, which is not allowed, so ignore it.
...
/*
Routine Description:
    Decrement the reference count on each DLL specified in the image import list.
    If any DLL's reference count reaches zero, then free the DLL.
...
*/
{
    ...
    for (i = 0; i < ImportList->Count && ImportList->Entry[i]; i += 1) {
        ...
        if (ImportTableEntry->LoadedImports == (PVOID)LOADED_AT_BOOT) {
            continue;// Skip this one - it was loaded by ntldr.
        }
...
#if DBG
        {
            ...
            // Assert that the first 2 entries are never dereferenced as unloading the kernel or HAL would be fatal.
            NextEntry = PsLoadedModuleList.Flink;
            ImageCount = 0;
        ...
        if (ImportTableEntry->LoadCount == 0) {
            // Unload this dependent DLL - we only do this to non-referenced non-boot-loaded drivers.
            // Stop the import list recursion prior to unloading - we know we're done at this point.

            // Note we can continue on afterwards without restarting regardless of which locks get released and reacquired because this chain is private.
            ...
            if (Unloaded == TRUE) {
                // This DLL was unloaded so recurse through its imports and attempt to unload all of those too.
...
*/
{
    ...
    *LoadedImports = NO_IMPORTS_USED;
    ...
    // Count the number of imports so we can allocate enough room to store them all chained off this module's LDR_DATA_TABLE_ENTRY.
    Count = 0;
    ...
    while (ImportDescriptor->Name && ImportDescriptor->OriginalFirstThunk) {
        ...
        // A driver can link with win32k.sys if and only if it is a GDI driver.
        // Also display drivers can only link to win32k.sys (and BBT ...).

        // So if we get a driver that links to win32k.sys and has more than one set of imports, we will fail to load it.
        LinkWin32k = LinkWin32k | (!_strnicmp(ImportName, "win32k", sizeof("win32k") - 1));

        // We don't want to count coverage, win32k and irt (BBT) since display drivers CAN link against these.
        ...
        if ((!_strnicmp(ImportName, "ntoskrnl", sizeof("ntoskrnl") - 1)) ||
            ...
            // These imports don't get refcounted because we don't ever want to unload them.
        ...
        if (NamePrefix && (_strnicmp(ImportName, "ntoskrnl", sizeof("ntoskrnl") - 1) && _strnicmp(ImportName, "hal", sizeof("hal") - 1))) {
            ImportDescriptorName_U.MaximumLength = (USHORT)(ImportName_U.Length + NamePrefix->Length);
            ImportDescriptorName_U.Buffer = ExAllocatePoolWithTag(NonPagedPool, ImportDescriptorName_U.MaximumLength, 'TDmM');
    ...
    ReCheck:
        ...
        // Initializing DataTableEntry is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
        DataTableEntry = NULL;
        while (NextEntry != &PsLoadedModuleList) {
            DataTableEntry = CONTAINING_RECORD(NextEntry, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
            if (RtlEqualUnicodeString(&ImportDescriptorName_U, &DataTableEntry->BaseDllName, TRUE)) {
                ...
                // Only bump the LoadCount if this thread did not initiate the load below.
                // If this thread initiated the load, then the LoadCount has already been bumped as part of the load - we only want to increment it here if we are "attaching" to a previously loaded DLL.
                if ((Loaded == FALSE) && (ReferenceImport == TRUE)) {
                    // Only increment the load count on the import if it is not circular (ie: the import is not from the original caller).
        ...
        if (ImportBase == NULL) {
            ...
            if (DllToLoad.Buffer) {
                ...
                // Add NULL termination in case the load fails so the name can be returned as the PWSTR MissingDriverName.
                ...
                if (NT_SUCCESS(st)) {
                    // No need to keep the temporary name buffer around now that there is a loaded module list entry for this DLL.
                    ExFreePool(DllToLoad.Buffer);
                } else {
                    if ((st == STATUS_OBJECT_NAME_NOT_FOUND) && (NamePrefix == NULL) && (MI_IS_SESSION_ADDRESS(ImageBase))) {
#define DRIVERS_SUBDIR_NAME L"drivers\\"
                        ...
                        // The DLL file was not located, attempt to load it from the drivers subdirectory.
                        // This makes it possible for drivers like win32k.sys to link to drivers that reside in the drivers subdirectory (like dxapi.sys).
                        DllToLoad2.MaximumLength = (USHORT)(ImportName_U.Length + DriverDirectory.Length + ImageFileDirectory->Length + sizeof(WCHAR));
                    ...
                    // Return the temporary name buffer to our caller so the name of the DLL that failed to load can be displayed.
                    // Set the low bit of the pointer so our caller knows to free this buffer when he's done displaying it 
                    // (as opposed to loaded module list entries which should not be freed).
                    *MissingDriverName = DllToLoad.Buffer;
                    *(PULONG)MissingDriverName |= 0x1;
                    *MissingProcedureName = NULL;// Set this to NULL so the hard error prints properly.
                }
            } else {
                // Initializing Section and BaseAddress is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
                Section = NULL;
                BaseAddress = NULL;
                st = STATUS_INSUFFICIENT_RESOURCES;
            }
        ...
        LoadFinished:
            ...
            if (NT_SUCCESS(st)) {
                ...
                // Call the dll's initialization routine if it has one.  
                // This routine will reapply verifier thunks to any modules that link to this one if necessary.
                st = MmCallDllInitialize(TableEntry, &PsLoadedModuleList);
                if (!NT_SUCCESS(st)) {// If the module could not be properly initialized, unload it.
        ...
        if ((ReferenceImport == TRUE) && (ImportList)) {
            // Only add the image providing satisfying our imports to the import list if the reference is not circular (ie: the import is not from the original caller).
            if ((DataTableEntry->Flags & LDRP_LOAD_IN_PROGRESS) == 0) {
    ...
    // All the imports are successfully loaded so establish and compact the import unload list.
    if (ImportList) {
        // Blank entries occur for things like the kernel, HAL & win32k.sys that we never want to unload.
        // Especially for things like win32k.sys where the reference count can really hit 0.

        // Initializing SingleEntry is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
        SingleEntry = NULL;
        ...
        if (Count == 0) {
            ...
            ImportListSize = Count * sizeof(PVOID) + sizeof(SIZE_T);
            CompactedImportList = (PLOAD_IMPORTS)ExAllocatePoolWithTag(PagedPool | POOL_COLD_ALLOCATION, ImportListSize, 'TDmM');
...
NTSTATUS MiSnapThunk(IN PVOID DllBase,
                     IN PVOID ImageBase,
                     IN PIMAGE_THUNK_DATA NameThunk,
                     OUT PIMAGE_THUNK_DATA AddrThunk,
                     IN PIMAGE_EXPORT_DIRECTORY ExportDirectory,
...
*/
{
    ...
    } else {// Change AddressOfData from an RVA to a VA.
        if (!SnapForwarder) {
            NameThunk->u1.AddressOfData = (ULONG_PTR)ImageBase + NameThunk->u1.AddressOfData;
        }

        strncpy(*MissingProcedureName, (const PCHAR)&((PIMAGE_IMPORT_BY_NAME)NameThunk->u1.AddressOfData)->Name[0], MAXIMUM_FILENAME_LENGTH - 1);

        // Lookup Name in NameTable
        NameTableBase = (PULONG)((PCHAR)DllBase + (ULONG)ExportDirectory->AddressOfNames);
        NameOrdinalTableBase = (PUSHORT)((PCHAR)DllBase + (ULONG)ExportDirectory->AddressOfNameOrdinals);

        // Before dropping into binary search, see if the hint index results in a successful match. If the hint index is zero, then drop into binary search.
        HintIndex = ((PIMAGE_IMPORT_BY_NAME)NameThunk->u1.AddressOfData)->Hint;
        if ((ULONG)HintIndex < ExportDirectory->NumberOfNames &&
            !strcmp((PSZ)((PIMAGE_IMPORT_BY_NAME)NameThunk->u1.AddressOfData)->Name, (PSZ)((PCHAR)DllBase + NameTableBase[HintIndex]))) {
            ...
            while (High >= Low) {
                ...
                Middle = (Low + High) >> 1;
                Result = strcmp((const PCHAR)&((PIMAGE_IMPORT_BY_NAME)NameThunk->u1.AddressOfData)->Name[0], (PCHAR)((PCHAR)DllBase + NameTableBase[Middle]));
                if (Result < 0) {
                    High = Middle - 1;
                } else if (Result > 0) {
                    Low = Middle + 1;
        ...
        if (((ULONG_PTR)AddrThunk->u1.Function > (ULONG_PTR)ExportDirectory) &&
            ...
            if (NT_SUCCESS(RtlAnsiStringToUnicodeString(&UnicodeString, &ForwardDllName, TRUE))) {
                ...
                while (NextEntry != &PsLoadedModuleList) {
                    ...
                    // We have to do a case INSENSITIVE comparison for forwarder because the linker just took what is in the def file, 
                    ...
                    if (RtlPrefixString((PSTRING)&UnicodeString, (PSTRING)&DataTableEntry->BaseDllName, TRUE)) {
                        ...
                        if (LocalExportDirectory != NULL) {
                            ...
                            addressOfData = (PIMAGE_IMPORT_BY_NAME)
                                ExAllocatePoolWithTag(PagedPool, length + sizeof(IMAGE_IMPORT_BY_NAME), '  mM');
                            if (addressOfData) {
                                RtlCopyMemory(&(addressOfData->Name[0]), ForwardDllName.Buffer + ForwardDllName.Length, length);
...
Arguments:
    ImageFileHandle - Supplies the file handle of the image.  This is a kernel handle (ie: cannot be tampered with by the user).
    PurgeSection - Supplies TRUE if the data section mapping the image should be purged prior to returning.
                   Note that the first page could be used to speed up subsequent image section creation,
                   but generally the cost of useless data pages sitting in transition is costly.
                   Better to put the pages immediately on the free list to preserve the transition cache for more useful pages.
Return Value:
    Status value.
*/
{
...
retry:
    ...
    if (NT_SUCCESS(Status)) {
        ...
        try {
            if (!LdrVerifyMappedImageMatchesChecksum(ViewBase, NumberOfBytes, StandardInfo.EndOfFile.LowPart)) {
            ...
            // Detect configurations inadvertently trying to load 32-bit drivers on NT64 or mismatched platform architectures, etc.
...
out:
    ...
    KeUnstackDetachProcess(&ApcState);
    if ((Status == STATUS_IMAGE_CHECKSUM_MISMATCH) && (SectionType == SEC_IMAGE)) {
        // We speculatively tried to verify the checksum using the image mapping but this did not match.
        // This can occur if the image has a self-signed certificate appended to it (this is a rare occurrence)
        // as the certificate data will be present in the data file but is not mapped when the file is used as an image.
        // So fall back to mapping the file as data and recompute.

        // Note there are other cases above that can get us to this check but it's always ok to retry as data ...
        ZwClose(Section);
...
*/
{
    ...
    // PAE PTEs are written in 2 separate writes so the working set pushlock must always be held even to examine PTEs in prototype format - because
    // a trim could be happening in parallel (writing the low half and then the upper half which would cause otherwise cause us to read the PTE contents split in the middle).
    WsHeld = TRUE;
    ...
    while (NumberOfPtes != 0) {
        ...
        if (PteContents.u.Long != 0) {
            if (PteContents.u.Hard.Valid == 1) {
                // Once the working set mutex is acquired, it is deliberately held until all the pages have been freed.
                // This is because when paged pool is running low on large servers, we need the segment dereference thread to be able to free large amounts quickly.
                // Typically this thread will free 64k chunks and we don't want to have to contend for the mutex 16 times to do this as there may be thousands of other threads also trying for it.
                if (WsHeld == FALSE) {
                    WsHeld = TRUE;
                    LOCK_WORKING_SET(Thread, Ws);
                }
                ...
                // Check to see if this is a pageable page in which case it needs to be removed from the working set list.
                ...
                if (WsIndex == 0) {
                    ...
                    if (Ws == &MmSystemCacheWs) {
                        ...
                        WsIndex = MiLocateWsle(VirtualAddress, MmSessionSpace->Vm.VmWorkingSetList, WsIndex, TRUE);
                        ASSERT(WsIndex != WSLE_NULL_INDEX);

                        // Check to see if this entry is locked in the working set or locked in memory.
                        ...
                        if (Locked.LockedInWs == 1 || Locked.LockedInMemory == 1) {// This entry is locked.
                            MM_BUMP_SESS_COUNTER(MM_DBG_SESSION_NP_DELVA, 1);
                            InterlockedDecrementSizeT(&MmSessionSpace->NonPageablePages);
                            ValidPages += 1;
                            ...
                            if (WsIndex != MmSessionSpace->Vm.VmWorkingSetList->FirstDynamic) {
                                ...
                                MiSwapWslEntries(Entry, WsIndex, &MmSessionSpace->Vm, FALSE);
                            }
                        } else {
                            ASSERT(WsIndex >= MmSessionSpace->Vm.VmWorkingSetList->FirstDynamic);
                        }
                ...
                if (Pfn1->u3.e1.PrototypePte == 1) {// Check if this is a prototype PTE.
                    PMMPTE PointerPde;
                    ASSERT(Ws != &MmSystemCacheWs);
                    PointerPde = MiGetPteAddress(PointerPte);// Capture the state of the modified bit for this PTE.
                ...
                MiDecrementShareCountInline(Pfn2, PageTableFrameIndex);// Decrement the share count for the containing page table page.
                MiDecrementShareCount(Pfn1, PageFrameIndex);// Decrement the share count for the physical page.
                UNLOCK_PFN(OldIrql);
                MI_WRITE_ZERO_PTE(PointerPte);
                if (Flags & MI_DELETE_FLUSH_TB) {
                    if (FlushCount != MM_MAXIMUM_FLUSH_COUNT) {// Flush the TB for this virtual address.
                        VaFlushList[FlushCount] = MiGetVirtualAddressMappedByPte(PointerPte);
                        FlushCount += 1;
                    }
                } else {
                    // Our caller will lazy flush this entry so stamp the time for him.
                    // Since zero is used by our caller as an indicator no flush is needed, 
                    // ensure that if the inserted field (which may be truncated) is zero, that a flush of this address is done before we return.
                ...
                PageFrameIndex = MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&PteContents);// Transition, release page.
                Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);// Set the pointer to PTE as empty so the page is deleted when the reference count goes to zero.
                MI_SET_PFN_DELETED(Pfn1);
                PageTableFrameIndex = Pfn1->u4.PteFrame;
                Pfn2 = MI_PFN_ELEMENT(PageTableFrameIndex);
                MiDecrementShareCountInline(Pfn2, PageTableFrameIndex);

                // Check the reference count for the page, if the reference count is zero, move the page to the free list, 
                // if the reference count is not zero, ignore this page.
    ...
    // Some callers flush the TB themselves, but for the rest we must flush here.
    if (FlushCount == 0) {
        NOTHING;
...
VOID MiMarkSectionWritable(IN PIMAGE_SECTION_HEADER SectionTableEntry)
/*
Routine Description:
    This function is a nonpaged helper routine that updates the characteristics field of the argument section table entry and marks the page dirty so that subsequent session loads share the same copy.
Arguments:
...
*/
{
    ...
    // Modify the PE header through hyperspace and mark the header page dirty so subsequent sections pick up the same copy.

    // Note this makes the entire .rdata (.sdata on IA64) writable instead of just the import tables.
...
*/
{
    ...
    // Note this is only called for image control areas that have at least PAGE_SIZE subsection alignment, 
    // and so the first subsection which maps the header can always be skipped.
    do {
        Subsection = Subsection->NextSubsection;
        if (Subsection == NULL) {
            break;
        ...
        // Don't mark global subsections as copy on write even when the image is relocated.
        // This is easily distinguishable because it is the only subsection that is marked readwrite.
...
*/
{
    ...
    // Note that the write bit may already be off (in the valid PTE) if the page has already been inpaged from the paging file and has not since been dirtied.
    LOCK_PFN(OldIrql);
    while (PointerPte <= LastPte) {
        PteContents = *PointerPte;
        if (PteContents.u.Long == 0) {
        ...
        if (PteContents.u.Hard.Valid == 1) {
            ...
            if (Pfn1->u3.e1.PrototypePte == 1) {
                // This must be a session address.
                // The prototype PTE contains the protection that is pushed out to the real PTE after it's been trimmed so update that too.
                PointerProtoPte = Pfn1->PteAddress;
                PointerPde = MiGetPteAddress(PointerProtoPte);
                if (PointerPde->u.Hard.Valid == 0) {
                    if (SessionAddress == TRUE) {
                        // Unlock the session working set and lock the system working set as we need to make the backing prototype PTE valid.
                        UNLOCK_PFN(OldIrql);
                    ...
                    if (SessionAddress == TRUE) {
                        // Unlock the system working set and lock the session working set as we have made the backing prototype PTE valid and can now handle the original session PTE.
                        UNLOCK_PFN(OldIrql);
                    ...
                    continue;// The world may have changed while we waited.
                }
            } else {
                if ((ProtectionMask & MM_COPY_ON_WRITE_MASK) == MM_COPY_ON_WRITE_MASK) {
                    // This page is already private so ignore the copy on write attribute (or anything else).
            ...
            MI_MAKE_VALID_PTE(TempPte, PteContents.u.Hard.PageFrameNumber, ProtectionMask, PointerPte);
            ...
            // Nothing needs to be done if the image was linked with greater than or equal to PAGE_SIZE subsection alignment
            // because image section creation assigned proper protections to each subsection.

            // However, if the image had less than PAGE_SIZE subsection alignment, then image creation uses a single copyonwrite
            // subsection to control the entire image, so individual protections need to be applied now.
            // Note well - this must only be done *ONCE* when the image is first loaded - subsequent
            // loads of this image in other sessions do not need to update the common prototype PTEs.
            ...
            // This page is already private so if copy on write is specified, skip this page.  This is very unusual.
            if ((ProtectionMask & MM_COPY_ON_WRITE_MASK) != MM_COPY_ON_WRITE_MASK) {
                Pfn1 = MI_PFN_ELEMENT(PteContents.u.Trans.PageFrameNumber);
                Pfn1->OriginalPte.u.Soft.Protection = ProtectionMask;
                PointerPte->u.Soft.Protection = ProtectionMask;
            }
        } else {// Must be page file space or demand zero.
         // This page is already private so if copy on write is specified, skip this page.  This is very unusual.
            if ((ProtectionMask & MM_COPY_ON_WRITE_MASK) != MM_COPY_ON_WRITE_MASK) {
                PointerPte->u.Soft.Protection = ProtectionMask;
            }
...
*/
{
    ...
    if (SectionProtection != 0) {
        ...
        // Always make images executable (unless they're completely no-access) for compatibility.
...
*/
{
    ...
    if (MI_IS_SESSION_ADDRESS(DllBase) == 0) {
        // Images prior to Win2000 were not protected from stepping all over their (and others) code and readonly data.
        // Here we somewhat preserve that behavior, but don't allow them to step on anyone else.
    ...
    // Don't include IMAGE_SCN_MEM_SHARED because it only applies to session drivers and that's handled by MiSessionProcessGlobalSubsections.
    RelevantSectionProtections = (IMAGE_SCN_MEM_WRITE | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_EXECUTE);

    // If the image has section alignment of at least PAGE_SIZE, 
    // then the image section was created with individual subsections and proper permissions already applied to the prototype PTEs.
    // However, our caller may have been changing the individual PTE protections in order to relocate the image, so march on regardless of section alignment.
    ...
    // Verify the image contains subsections ordered by increasing virtual address and that there are no overlaps.
    ...
    // Set writable PTE here so the image headers are excluded.
    // This is needed so that locking down of sections can continue to edit the image headers for counts.
    ...
    for (; NumberOfSubsections > 0; NumberOfSubsections -= 1, SectionTableEntry += 1) {
        ...
        if ((MergedPte != NULL) && (PointerPte > MergedPte)) {
            ...
            MergedProtection = MiComputeDriverProtection(SessionDriver, MergedSectionProtection);
            MiSetSystemCodeProtection(MergedPte, MergedPte, MergedProtection);
            if (MergedPte == FirstPte) {
                // The merged PTE overlapped with the first PTE of the prior subsection, so increment the first PTE now.
                FirstPte += 1;
            }
        ...
        if (PointerPte >= LastImagePte) {
            break;// Skip relocation subsections (which aren't given VA space).
        }

        SectionProtection = (SectionTableEntry->Characteristics & RelevantSectionProtections);
        if (SectionProtection == LastSectionProtection) {
            LastVirtualAddress = (PVOID)((PCHAR)VirtualAddress + SectionVirtualSize - 1);// Merge adjacent subsections that have the same protection.
            continue;
        }

        // This section has different permissions from the previous one so set the protections on the previous one now.

        // Update the PTE protections.  Make sure if it's sharing a PTE that the last PTE is set to the most relaxed permission combination.
        LastPte = (PVOID)MiGetPteAddress(LastVirtualAddress);
        if (LastPte == PointerPte) {
            // Don't include the last PTE from the previous subsection as it overlaps with this one (and may overlap with the next subsection if this one is small!).
            LastPte -= 1;

            // Compute the most relaxed permission for the straddling PTE
            // by merging subsection protections, but don't apply it till we have ascertained that there are no more subsections which can overlap into this PTE.
            if (MergedPte != NULL) {
        ...
        if (LastPte >= FirstPte) {
            ...
            LastProtection = MiComputeDriverProtection(SessionDriver, LastSectionProtection);
            MiSetSystemCodeProtection(FirstPte, LastPte, LastProtection);
        }

        // Initialize variables to describe the current subsection as it is the start of a new run.
        FirstPte = PointerPte;
        LastVirtualAddress = (PVOID)((PCHAR)VirtualAddress + SectionVirtualSize - 1);
...
/*
Routine Description:
    This function sets the protection of a session driver's subsections to globally shared if their PE header specifies them as such.
Arguments:
...
*/
{
    ...
    // Win XP and Win2000 did not support global shared subsections for session images.
    // To ensure backwards compatibility for existing drivers, ensure that only newer ones get this feature.
    ...
    if (ControlArea->u.Flags.Image == 1) {
        do {
            if (Subsection->u.SubsectionFlags.GlobalMemory == 1) {
                ...
                MiSetSystemCodeProtection(PointerPte, LastPte, Subsection->u.SubsectionFlags.Protection);
            }
        ...
        while (Image->GlobalSubs[Count].PteCount != 0) {
            ...
            MiSetSystemCodeProtection(PointerPte, LastPte, Image->GlobalSubs[Count].Protection);
            Count += 1;
        }
...
*/
{
    ...
    // This is because that module may itself be the one being relocated right now.
...
*/
{
...
#if !defined (_X86_)
    MmMakeLowMemory = FALSE;// Only try to preserve low memory on x86 machines.
#endif
    StopMoving = FALSE;
    ...
    NextEntry = LoaderBlock->LoadOrderListHead.Flink;
    for (; NextEntry != &LoaderBlock->LoadOrderListHead; NextEntry = NextEntry->Flink) {
        // Skip the kernel and the HAL.  Note their relocation sections will be automatically reclaimed.
        i += 1;
        if (i <= 2) {
            continue;
        }
        ...
        if (NtHeader == NULL) {// Ensure that the relocation section exists and that the loader hasn't freed it already.
            continue;
        }
        FileHeader = &NtHeader->FileHeader;
        ...
        if (DataDirectory->VirtualAddress == 0) {
            ...
            if (DataDirectory->VirtualAddress + DataDirectory->Size > DataTableEntry->SizeOfImage) {
                continue;// The relocation section has already been freed, the user must be using an old loader that didn't save the relocations.
            }
            HasRelocations = TRUE;
        }
        ...
        while (PointerPte < LastPte) {
            ...
            // Mark the page as modified so boot drivers that call MmPageEntireDriver don't lose their unmodified data !
        ...
        UsedLargePage = MiUseLargeDriverPage(NumberOfPtes, &NewImageAddress, &DataTableEntry->BaseDllName, 0);
        if (UsedLargePage == TRUE) {// This image has been loaded into a large page mapping.
        ...
        // Extra PTEs are allocated here to map the relocation section at the new address so the image can be relocated.
        ...
        // This assert is important because the assumption is made that PTEs (not superpages) are mapping these drivers.
        ASSERT(InitializationPhase == 0);

        // If the system is configured to make low memory available for ISA type drivers, then copy the boot loaded drivers now.
        // Otherwise only PTE adjustment is done.  Presumably some day when ISA goes away this code can be removed.
        RelocatedVa = NewImageAddress;
        NonRelocatedVa = (PCHAR)DataTableEntry->DllBase;
        while (PointerPte < LastPte) {
            ...
            if (MmMakeLowMemory == TRUE) {
                ...
                if (PageFrameIndex < (16 * 1024 * 1024) / PAGE_SIZE) {
                    // If the frames cannot be replaced with high pages then stop copying.
                ...
                MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, MM_EXECUTE_READWRITE, PointerPte);
                MI_SET_PTE_DIRTY(TempPte);
                MI_SET_ACCESSED_IN_PTE(&TempPte, 1);
                MiInitializePfnAndMakePteValid(PageFrameIndex, PointerPte, TempPte);
                Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
                MI_SET_MODIFIED(Pfn1, 1, 0x15);
                Pfn1->u1.WsIndex = 0;// Initialize the WsIndex just like the original page had it.
                UNLOCK_PFN(OldIrql);
    ...
    Fixup:
        ...
        if (HasRelocations == TRUE) {
            ...
            if (!NT_SUCCESS(Status)) {
                if (UsedLargePage == TRUE) {
                    ...
                    RoundedNumberOfPtes = MI_ROUND_TO_SIZE(NumberOfPtes, MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT);
                    MiUnmapLargePages(NewImageAddress, RoundedNumberOfPtes << PAGE_SHIFT);
                    MiRemoveCachedRange(PageFrameIndex, PageFrameIndex + RoundedNumberOfPtes - 1);
                    MiFreeContiguousPages(PageFrameIndex, RoundedNumberOfPtes);
                }
        ...
        MiCaptureImageExceptionValues(DataTableEntry);// Update the exception table data info

        // Update the PFNs of the image to support trimming.
        // Note that the loader addresses are freed now so no references to it are permitted after this point.
        ...
        while (PointerPte < LastPte) {
            ...
            if ((MmMakeLowMemory == TRUE) || (UsedLargePage == TRUE)) {
                ...
                // Decrement the share count on the original page table page so it can be freed.
                MiDecrementShareCount(Pfn2, Pfn1->u4.PteFrame);
                MI_SET_PFN_DELETED(Pfn1);
                MiDecrementShareCount(Pfn1, PageFrameIndex);
                LoaderPte += 1;
                ...
                // Increment the share count for the page table page that now contains the PTE that was copied.
        ...
        // The physical pages mapping the relocation section are freed later with the rest of the initialization code spanned by the DataTableEntry->SizeOfImage.
        if (StopMoving == TRUE) {
            MmMakeLowMemory = FALSE;
        }
    }

    KeSweepIcache(TRUE);// Flush the instruction cache on all systems in the configuration.
}
...
*/
{
    ...
    while (i > 0) {
        ...
        // Generally, SizeOfRawData is larger than VirtualSize for each section because it includes the padding to get to the subsection alignment boundary.
        // However, if the image is linked with
        // subsection alignment == native page alignment, 
        // the linker will have VirtualSize be much larger than SizeOfRawData because it will account for all the bss.
        ...
        if (*(PULONG)SectionTableEntry->Name == 'rsr.') {
            ...
            MiKernelResourceEndPte = MiGetPteAddress(ROUND_TO_PAGES((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + Span));
            break;
        }
#endif
        if (*(PULONG)SectionTableEntry->Name == 'LOOP') {
            if (*(PULONG)&SectionTableEntry->Name[4] == 'EDOC') {
                ...
                ExPoolCodeEnd = (PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + Span);
            } else if (*(PUSHORT)&SectionTableEntry->Name[4] == 'IM') {
                MmPoolCodeStart = (PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress);
                MmPoolCodeEnd = (PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + Span);
            }
            ...
            MmPteCodeEnd = (PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + Span);
        }
...
*/
{
    ...
    if (MI_IS_PHYSICAL_ADDRESS(MiGetVirtualAddressMappedByPte(PointerPte))) {
        return;// Mapped physically, doesn't need to be made readwrite.
    }
...
#endif
        {
            MI_MAKE_VALID_KERNEL_PTE(TempPte, PteContents.u.Hard.PageFrameNumber, MM_READWRITE, PointerPte);
#if !defined(NT_UP)
            TempPte.u.Hard.Writable = 1;
#endif
            MI_WRITE_VALID_PTE_NEW_PROTECTION(PointerPte, TempPte);
    ...
    MI_FLUSH_CURRENT_TB();// Only flush this processor as the state of the others is unknown.
#endif
}
...
*/
{
    ...
    // The data table entry is inserted in the loaded module list and the initialization order list in the order specified in the loader parameter block.
    // The data table entry is inserted in the memory order list in memory order.
    NextEntry = LoaderBlock->LoadOrderListHead.Flink;
    ...
    while (NextEntry != NextEntryEnd) {
        ...
        DataTableEntrySize = sizeof(KLDR_DATA_TABLE_ENTRY) + DataTableEntry2->BaseDllName.MaximumLength + sizeof(UNICODE_NULL);
        DataTableEntry1 = ExAllocatePoolWithTag(NonPagedPool, DataTableEntrySize, 'dLmM');
        if (DataTableEntry1 == NULL) {
            return FALSE;
        }

        *DataTableEntry1 = *DataTableEntry2;// Copy the data table entry.

        // Clear fields we may use later so they don't inherit irrelevant loader values.
        ...
        // Copy the strings.
        RtlCopyMemory(DataTableEntry1->FullDllName.Buffer, DataTableEntry2->FullDllName.Buffer, DataTableEntry1->FullDllName.MaximumLength);
        RtlCopyMemory(DataTableEntry1->BaseDllName.Buffer, DataTableEntry2->BaseDllName.Buffer, DataTableEntry1->BaseDllName.MaximumLength);
        DataTableEntry1->BaseDllName.Buffer[DataTableEntry1->BaseDllName.Length / sizeof(WCHAR)] = UNICODE_NULL;

        // Always charge commitment regardless of whether we were able to relocate the driver, use large pages, etc.
...
#if defined (_WIN64)
        RtlInsertInvertedFunctionTable(&PsInvertedFunctionTable, DataTableEntry1->DllBase, DataTableEntry1->SizeOfImage);
#endif
        NextEntry = NextEntry->Flink;
    }

    // Charge commitment for each boot loaded driver so that if unloads later, the return will balance.
    // Note that the actual number of free pages is not changing now so the commit limits need to be bumped by the same amount.

    // Resident available does not need to be charged here because it has been already (by virtue of being snapped from available pages).
...
NTSTATUS MmCallDllInitialize(IN PKLDR_DATA_TABLE_ENTRY DataTableEntry, IN PLIST_ENTRY ModuleListHead)
/*
Routine Description:
...
*/
{
    ...
    RegistryPath.MaximumLength = (USHORT)(CmRegistryMachineSystemCurrentControlSetServices.Length + ImportName.Length + 2 * sizeof(WCHAR));
    ...
    // Save the number of verifier thunks currently added so we know if this activation adds any.
    // To extend the thunk list, the module performs an NtSetSystemInformation call which calls back to the verifier's MmAddVerifierThunks, 
    // which increments MiVerifierThunksAdded.
    ...
    // If the module's initialization routine succeeded, 
    // and if it extended the verifier thunk list, and this is boot time, reapply the verifier to the loaded modules.

    // Note that boot time is the special case because after boot time, 
    // Mm loads all the DLLs itself and a DLL initialize is thus guaranteed to complete and add its thunks before the importing driver load finishes.
    // Since the importing driver is only thunked after its load finishes, 
    // ordering implicitly guarantees that all DLL-registered thunks are properly factored in to the importing driver.

    // Boot time is special because the loader (not the Mm) already loaded the DLLs *AND* the importing drivers so we have to look over our shoulder and make it all right after the fact.
...
NTKERNELAPI PVOID MmGetSystemRoutineAddress(__in PUNICODE_STRING SystemRoutineName)
/*
Routine Description:
