...
Abstract:
    This module contains the routines which implement the NtLockVirtualMemory service.
*/
...
*/
{
    ...
    if (CapturedBase > MM_HIGHEST_USER_ADDRESS) {
        return STATUS_INVALID_PARAMETER;// Invalid base address.
    }

    if ((ULONG_PTR)MM_HIGHEST_USER_ADDRESS - (ULONG_PTR)CapturedBase < CapturedRegionSize) {
        return STATUS_INVALID_PARAMETER;// Invalid region size;
    }
    ...
    if ((MapType & MAP_SYSTEM) != 0) {
        // In addition to PROCESS_VM_OPERATION access to the target process, 
        // the caller must have SE_LOCK_MEMORY_PRIVILEGE.
    ...
    // Get address creation mutex, this prevents the address range from being modified while it is examined.
    // Raise to APC level to prevent an APC routine from acquiring the address creation mutex.
    // Get the working set mutex so the number of already locked pages in the request can be determined.
    ...
    // Note the working set mutex must be held throughout the loop below to prevent other threads from locking or unlocking WSL entries.
    ...
    while (Va <= EndingAddress) {
        ...
        if (MiIsAddressValid(Va, TRUE)) {
            ...
            WorkingSetIndex = MiLocateWsle(Va, MmWorkingSetList, Pfn1->u1.WsIndex, FALSE);
            ASSERT(WorkingSetIndex != WSLE_NULL_INDEX);
            if (WorkingSetIndex < MmWorkingSetList->FirstDynamic) {
                NumberOfAlreadyLocked += 1;// This page is locked in the working set.
    ...
    // Check to ensure the working set list is still fluid after the requested number of pages are locked.
    ...
    // Set up an exception handler and touch each page in the specified range.
    ...
    // Note this would have the side effect of not allowing this thread to fault on guard pages in other data regions
    // while the accesses below are ongoing - but that could only happen in an APC and those are blocked right now anyway.
    ...
    // The complete address range is accessible, lock the pages into the working set.
    ...
    while (Va <= EndingAddress) {
        ...
        MiMakePdeExistAndMakeValid(PointerPde, TargetProcess, MM_NOIRQL);// Ensure the PDE (and any table above it) are still resident.

        // Make sure the page is in the working set.
        while (PointerPte->u.Hard.Valid == 0) {
            UNLOCK_WS_UNSAFE(Thread, TargetProcess);// Release the working set mutex and fault in the page.
            ...
            {
                // Since all the pages were accessed above with the address space mutex held and it is still held now,
                // the only way an exception could occur would be due to a device error, 
                // ie: hardware malfunction, net cable disconnection, CD being removed, etc.

                // Recompute EndingAddress so the actual number of pages locked is written back to the user now.
                // If this is the very first page then return a failure status.
            ...
            LOCK_WS_UNSAFE(Thread, TargetProcess);// Reacquire the working set mutex.

            // Ensure the PDE (and any table above it) are still resident.
            // They could have been trimmed from the working set before the working set lock was reacquired above.
            MiMakePdeExistAndMakeValid(PointerPde, TargetProcess, MM_NOIRQL);
        }

        // The page is now in the working set, lock the page into the working set.
        PointerPte1 = MiGetPteAddress(Va);
        Pfn1 = MI_PFN_ELEMENT(PointerPte1->u.Hard.PageFrameNumber);
...
*/
{
    ...
    if ((MapType & MAP_SYSTEM) != 0) {
        // In addition to PROCESS_VM_OPERATION access to the target process, the caller must have SE_LOCK_MEMORY_PRIVILEGE.
        if (!SeSinglePrivilegeCheck(SeLockMemoryPrivilege, PreviousMode)) {
            ObDereferenceObject(TargetProcess);
            return STATUS_PRIVILEGE_NOT_HELD;
        }
    ...
    // Get address creation mutex, this prevents the address range from being modified while it is examined.
    // Block APCs so an APC routine can't get a page fault and corrupt the working set list, etc.
    LOCK_ADDRESS_SPACE(TargetProcess);
    ...
    while (Va <= EndingAddress) {
        ...
        if ((Va > LastVa) || (FoundFirstVad == FALSE)) {
            ...
            if ((Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) ||
                ...
                (Vad->u.VadFlags.VadType == VadLargePageSection) ||
                (Vad->u.VadFlags.VadType == VadAwe)) {
                // It might have been better to return a failure for these types of VADs but since this wasn't done initially,
                // we continue to return success (though actually skipping them) to remain compatible.

                // Note that we never allowed addresses in these VADs to be locked so there is no problem of correctness.

                // Unfortunately, the original code not only broke out of the first loop here, it also did the same in the 2nd loop below.
                // But the size of the unlocked region being returned did not reflect this premature end.
                // Keep it this way (again for compatibility).
                break;
            }
        ...
        if (!MiIsAddressValid(Va, TRUE)) {
            Status = STATUS_NOT_LOCKED;// This page is not valid, therefore not in working set.
        } else {
    ...
    while (Va <= EndingAddress) {
        ...
        if ((MmWsle[Entry].u1.e1.LockedInMemory == 0) && MmWsle[Entry].u1.e1.LockedInWs == 0) {
            // The page is no longer should be locked, move it to the dynamic part of the working set.
            MmWorkingSetList->FirstDynamic -= 1;
            if (Entry != MmWorkingSetList->FirstDynamic) {
                // Swap this element with the last locked page, making this element the new first dynamic entry.
                MiSwapWslEntries(Entry, MmWorkingSetList->FirstDynamic, &TargetProcess->Vm, FALSE);
            }
        }
