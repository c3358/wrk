...
// note the assignments are not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
...
#if defined(_X86PAE_)
// PAE mode makes most kernel resources executable to improve compatibility with existing driver binaries.
...
#else
// NT64 drivers derived from 32-bit source have to be recompiled so there's no need to make everything executable - drivers can specify it explicitly.
#define MI_ADD_EXECUTE_TO_VALID_PTE_IF_PAE(TempPte)
#define MI_ADD_EXECUTE_TO_INVALID_PTE_IF_PAE(TempPte)
#endif
...
extern ULONG MmCritsectTimeoutSeconds;// A month's worth

extern PEPROCESS ExpDefaultErrorPortProcess;// this is the csrss process !
...
extern PFN_NUMBER MmHiberPages;// The total number of pages needed for the loader to successfully hibernate.
...
#define MI_COMPUTE_PAGES_SPANNED(Va, Size)     ((((ULONG_PTR)(Va) & (PAGE_SIZE -1)) + (Size) + (PAGE_SIZE - 1)) >> PAGE_SHIFT)
...
// ULONG MI_ROUND_WOULD_WRAP (IN ULONG LENGTH, IN ULONG ALIGNMENT)
// Routine Description:
// The ROUND_WOULD_WRAP macro takes a LENGTH in bytes and checks whether rounding it up to a multiple of the alignment would wrap.
...
#define MI_ROUND_WOULD_WRAP(LENGTH,ALIGNMENT)                         (((LENGTH) + ((ALIGNMENT) - 1)) <= (LENGTH))
...
// PVOID MI_64K_ALIGN (IN PVOID VA)
// Routine Description:
// The MI_64K_ALIGN macro takes a virtual address and returns a 64k-aligned virtual address for that page.
// Arguments:
//     VA - Virtual address.
// Return Value:
...
// PVOID MI_ALIGN_TO_SIZE (
//     IN PVOID VA
//     IN ULONG ALIGNMENT
//     )
// Routine Description:
// The MI_ALIGN_TO_SIZE macro takes a virtual address and returns a virtual address for that page with the specified alignment.
// Arguments:
//     VA - Virtual address.
//     ALIGNMENT - alignment to round to, must be a power of 2, e.g, 2**n.
// Return Value:
...
//     )
// Routine Description:
//    This macro takes a pointer to a PTE within a subsection and a pointer to that subsection and calculates the offset for that PTE within the file.
// Arguments:
...
// NTSTATUS MiFindEmptyAddressRangeDown (IN ULONG_PTR SizeOfRange, IN PVOID HighestAddressToEndAt, IN ULONG_PTR Alignment, OUT PVOID *Base)
// Routine Description:
//    The function examines the virtual address descriptors to locate an unused range of the specified size and returns the starting address of the range.
//    This routine looks from the top down.
// Arguments:
//    SizeOfRange - Supplies the size in bytes of the range to locate.
//    HighestAddressToEndAt - Supplies the virtual address to begin looking at.
//    Alignment - Supplies the alignment for the address.  Must be a power of 2 and greater than the page_size.
// Return Value:
...
#define MiFindEmptyAddressRangeDown(Root,SizeOfRange,HighestAddressToEndAt,Alignment,Base) \
               (MiFindEmptyAddressRangeDownTree((SizeOfRange), (HighestAddressToEndAt), (Alignment), Root, (Base)))


// PMMVAD MiGetPreviousVad (IN PMMVAD Vad)
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically precedes the specified virtual address descriptor.
// Arguments:
//     Vad - Supplies a pointer to a virtual address descriptor.
// Return Value:
//     Returns a pointer to the virtual address descriptor containing the next address range, NULL if none.
#define MiGetPreviousVad(VAD) ((PMMVAD)MiGetPreviousNode((PMMADDRESS_NODE)(VAD)))


// PMMVAD MiGetNextVad (IN PMMVAD Vad)
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically follows the specified address range.
// Arguments:
//     VAD - Supplies a pointer to a virtual address descriptor.
// Return Value:
//     Returns a pointer to the virtual address descriptor containing the next address range, NULL if none.
...
// PMMVAD MiGetFirstVad (Process)
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically is first within the address space.
// Arguments:
//     Process - Specifies the process in which to locate the VAD.
// Return Value:
//     Returns a pointer to the virtual address descriptor containing the first address range, NULL if none.
...
// Routine Description:
//     The function determines if any addresses between a given starting and ending address is contained within a virtual address descriptor.
// Arguments:
//     StartingAddress - Supplies the virtual address to locate a containing descriptor.
//     EndingAddress - Supplies the virtual address to locate a containing descriptor.
// Return Value:
//     Returns a pointer to the first conflicting virtual address descriptor if one is found, otherwise a NULL value is returned.
#define MiCheckForConflictingVad(CurrentProcess,StartingAddress,EndingAddress) \
    ((PMMVAD)MiCheckForConflictingNode(MI_VA_TO_VPN(StartingAddress), MI_VA_TO_VPN(EndingAddress), &CurrentProcess->VadRoot))


// PMMCLONE_DESCRIPTOR MiGetNextClone (IN PMMCLONE_DESCRIPTOR Clone)
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically follows the specified address range.
// Arguments:
...
// PMMCLONE_DESCRIPTOR MiGetPreviousClone (IN PMMCLONE_DESCRIPTOR Clone)
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically precedes the specified virtual address descriptor.
// Arguments:
//     Clone - Supplies a pointer to a virtual address descriptor.
...
// PMMCLONE_DESCRIPTOR MiGetFirstClone ()
// Routine Description:
//     This function locates the virtual address descriptor which contains the address range which logically is first within the address space.
// Arguments:
//     None.
...
// VOID MiInsertClone (IN PMMCLONE_DESCRIPTOR Clone)
// Routine Description:
//     This function inserts a virtual address descriptor into the tree and reorders the splay tree as appropriate.
// Arguments:
//     Clone - Supplies a pointer to a virtual address descriptor.
...
// VOID MiRemoveClone (IN PMMCLONE_DESCRIPTOR Clone)
// Routine Description:
//     This function removes a virtual address descriptor from the tree and reorders the splay tree as appropriate.
// Arguments:
//     Clone - Supplies a pointer to a virtual address descriptor.
...
// PMMCLONE_DESCRIPTOR MiLocateCloneAddress (IN PVOID VirtualAddress)

// /*++
// Routine Description:
//     The function locates the virtual address descriptor which describes a given address.
// Arguments:
//     VirtualAddress - Supplies the virtual address to locate a descriptor for.
// Return Value:
//     Returns a pointer to the virtual address descriptor which contains the supplied virtual address or NULL if none was located.
...
#define MiLockPfnDatabaseAtDpcLevel()         KeAcquireQueuedSpinLockAtDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueuePfnLock])
#define MiUnlockPfnDatabaseFromDpcLevel()     KeReleaseQueuedSpinLockFromDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueuePfnLock])
...
#define MiLockSystemSpaceAtDpcLevel()         KeAcquireQueuedSpinLockAtDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueueSystemSpaceLock])
#define MiUnlockSystemSpaceFromDpcLevel()     KeReleaseQueuedSpinLockFromDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueueSystemSpaceLock])
...
#if DBG
PKTHREAD MmPfnOwner;
#define MI_SET_PFN_OWNER() ASSERT (MmPfnOwner == NULL);                                               MmPfnOwner = KeGetCurrentThread ();
#define MI_CLEAR_PFN_OWNER() ASSERT (MmPfnOwner == KeGetCurrentThread ());                            MmPfnOwner = NULL;
#else
...
#define MM_SET_EXPANSION_OWNER()  ASSERT (MiExpansionLockOwner == NULL);                                   MiExpansionLockOwner = PsGetCurrentThread();
#define MM_CLEAR_EXPANSION_OWNER()  ASSERT (MiExpansionLockOwner == PsGetCurrentThread());                 MiExpansionLockOwner = NULL;
#else
#define MM_PFN_LOCK_ASSERT()
...
#define MM_ANY_WS_LOCK_HELD(THREAD)                                     (THREAD->SameThreadApcFlags & PS_SAME_THREAD_FLAGS_OWNS_A_WORKING_SET)
#define MM_SYSTEM_WS_LOCK_TIMESTAMP()                                   LOCK_WS_TIMESTAMP(((PEPROCESS)&MiSystemCacheDummyProcess));
#define MM_SYSTEM_WS_UNLOCK_TIMESTAMP()                                 UNLOCK_WS_TIMESTAMP(((PEPROCESS)&MiSystemCacheDummyProcess));
...
#define MM_SESSION_SPACE_WS_LOCK_ASSERT()                                           MM_WS_LOCK_ASSERT (&MmSessionSpace->Vm);
...
#define MI_FLUSH_MULTIPLE_TB(Number, Virtual, AllProcessors)                    KeFlushMultipleTb (Number, Virtual, AllProcessors);

#define MI_FLUSH_SINGLE_TB(Virtual, AllProcessors)                              KeFlushSingleTb (Virtual, AllProcessors);

#define MI_FLUSH_PROCESS_TB(AllProcessors)                              \
        ASSERT (AllProcessors == FALSE);                                \
        KeFlushProcessTb ();

#define MI_FLUSH_CURRENT_TB()                                                   KeFlushCurrentTb ();


// There is no flush single on just the current processor so specify FALSE and hope we don't IPI too many others.
#define MI_FLUSH_CURRENT_TB_SINGLE(Virtual)                                     KeFlushSingleTb (Virtual, FALSE);
...
//     )
// Routine Description:
//     The MI_CHECK_BIT macro checks to see if the specified bit is set within the specified array.
//     Note that for NT64, the ARRAY must be 8-byte aligned as the intrinsics rely on this.
// Arguments:
...
#define MI_CHECK_BIT(ARRAY,BIT)          BitTest64 ((LONG64 *)ARRAY, BIT)
#else
#define MI_CHECK_BIT(ARRAY,BIT)          (((ULONG)(((PULONG)ARRAY)[(BIT) / (sizeof(ULONG)*8)]) >> ((BIT) & 0x1F)) & 1)
#endif
...
//     )
// Routine Description:
//     The MI_SET_BIT macro sets the specified bit within the specified array.
//     Note that for NT64, the ARRAY must be 8-byte aligned as the intrinsics rely on this.
// Arguments:
//     ARRAY - First element of the array to set.
//     BIT - bit number.
#if _MI_USE_BIT_INTRINSICS
#define MI_SET_BIT(ARRAY,BIT)          BitTestAndSet64 ((LONG64 *)ARRAY, BIT)
#else
#define MI_SET_BIT(ARRAY,BIT)          ((PULONG)ARRAY)[(BIT) / (sizeof(ULONG)*8)] |= (1 << ((BIT) & 0x1F))
#endif
...
//     )
// Routine Description:
//     The MI_CLEAR_BIT macro sets the specified bit within the specified array.
//     Note that for NT64, the ARRAY must be 8-byte aligned as the intrinsics rely on this.
// Arguments:
//     ARRAY - First element of the array to clear.
//     BIT - bit number.
#if _MI_USE_BIT_INTRINSICS
#define MI_CLEAR_BIT(ARRAY,BIT)          BitTestAndReset64 ((LONG64 *)ARRAY, BIT)
#else
#define MI_CLEAR_BIT(ARRAY,BIT)          ((PULONG)ARRAY)[(BIT) / (sizeof(ULONG)*8)] &= ~(1 << ((BIT) & 0x1F))
#endif
...
// The cache type definitions are carefully chosen to line up with the MEMORY_CACHING_TYPE definitions to ease conversions.
// Any changes here must be reflected throughout the code.
...
// Note that the first range is for system RAM, the second range is for I/O space.
extern MI_PFN_CACHE_ATTRIBUTE MiPlatformCacheAttributes[2 * MmMaximumCacheType];


// MI_PFN_CACHE_ATTRIBUTE MI_TRANSLATE_CACHETYPE (IN MEMORY_CACHING_TYPE InputCacheType, IN ULONG IoSpace)
// Routine Description:
//     Returns the hardware supported cache type for the requested cachetype.
// Arguments:
//     InputCacheType - Supplies the desired cache type.
//     IoSpace - Supplies nonzero (not necessarily 1 though) if this is I/O space, zero if it is main memory.
// Return Value:
...
// VOID MI_SET_CACHETYPE_TRANSLATION (IN MEMORY_CACHING_TYPE InputCacheType, IN ULONG IoSpace, IN MI_PFN_CACHE_ATTRIBUTE NewAttribute)
// Routine Description:
//     This function sets the hardware supported cachetype for the specified cachetype.
// Arguments:
//     InputCacheType - Supplies the desired cache type.
//     IoSpace - Supplies nonzero (not necessarily 1 though) if this is I/O space, zero if it is main memory.
...
typedef struct _MMPFNENTRY {
    ...
    // This is not cleared when the frame is unmapped because in cases like system PTE mappings for MDLs, we lazy flush the TB (for performance) so we don't know when the final TB flush has really occurred.
    // To ensure that we never insert a TB that would cause an overlapping condition, whenever a PTE is filled with a freshly allocated frame, the filler
    // assumes responsibility for comparing the new cache attribute with the last known used attribute and if they differ, the filler must flush the entire TB.
    // KeFlushSingleTb cannot be used because we don't know what virtual address(es) this physical frame was last mapped at.

    // This may result in some TB overflushing in the case where no system PTE was used so it would have been safe to clear the attribute in the PFN,
    // but it reusing a page with a differing attribute should be a fairly rare case anyway.
...
#define MI_GET_PFN_PRIORITY(Pfn)                                ((ULONG)(Pfn)->u4.Priority)


// Reset the PFN priority to the default when a page is reused.


#define MI_RESET_PFN_PRIORITY(Pfn)                          MI_SET_PFN_PRIORITY(Pfn, MI_DEFAULT_PFN_PRIORITY);
...
typedef struct _MMPFN {
    union {
        ...
        // Note: NextStackPfn is actually used as SLIST_ENTRY, however because of its alignment characteristics, using that type would unnecessarily add padding to this structure.
    ...
    union {
        ...
        ULONG_PTR ShareCount;// ShareCount transitions are protected by the PFN lock.
    } u2;
    union {

        // ReferenceCount transitions are generally done with InterlockedXxxPfn sequences, and only the 0->1 and 1->0 transitions are protected by the PFN lock.
        // Note that a *VERY* intricate synchronization scheme is being used to maximize scalability.
        struct {
...
// VOID MI_SET_MODIFIED (IN PMMPFN Pfn, IN ULONG NewValue, IN ULONG CallerId)
// Routine Description:
...
// Note there are some places where these portable macros are not currently used because we are not in the correct address space required.
...
#define MI_INCREMENT_USED_PTES_BY_HANDLE(PDSHORT)     ((*(PUSHORT)(PDSHORT)) += 1); ASSERT (((*(PUSHORT)(PDSHORT)) <= PTE_PER_PAGE))
#define MI_INCREMENT_USED_PTES_BY_HANDLE_CLUSTER(PDSHORT,INCR)    (*(PUSHORT)(PDSHORT)) = (USHORT)((*(PUSHORT)(PDSHORT)) + (INCR)); ASSERT (((*(PUSHORT)(PDSHORT)) <= PTE_PER_PAGE))
#define MI_DECREMENT_USED_PTES_BY_HANDLE(PDSHORT)     ((*(PUSHORT)(PDSHORT)) -= 1); ASSERT (((*(PUSHORT)(PDSHORT)) < PTE_PER_PAGE))
#endif
...
#define MI_INCREMENT_LOCKED_COMMIT()                    MiLockedCommit += 1;
#define MI_DECREMENT_LOCKED_COMMIT()                    ASSERT (MiLockedCommit > 0);MiLockedCommit -= 1;
#else
...
#if defined(_WIN64)
#define MiChargeCommitmentRegardless()     MI_INCREMENT_LOCKED_COMMIT(); InterlockedIncrement64 ((PLONGLONG) &MmTotalCommittedPages);
#define MiReturnCommitmentRegardless()     MI_DECREMENT_LOCKED_COMMIT(); InterlockedDecrement64 ((PLONGLONG) &MmTotalCommittedPages);
#else
#define MiChargeCommitmentRegardless()     MI_INCREMENT_LOCKED_COMMIT(); InterlockedIncrement ((PLONG) &MmTotalCommittedPages);
#define MiReturnCommitmentRegardless()     MI_DECREMENT_LOCKED_COMMIT(); InterlockedDecrement ((PLONG) &MmTotalCommittedPages);
#endif
...
Arguments:
    Force - Supplies TRUE if the lock is short-term and should be forced through if necessary.
Return Value:
...
// Total number of available pages on the system.  This is the sum of the pages on the zeroed, free and standby lists.
extern PFN_NUMBER MmAvailablePages;


// Total number physical pages which would be usable if every process was at it's minimum working set size.
// This value is initialized at system initialization to MmAvailablePages - MM_FLUID_PHYSICAL_PAGES.
// Everytime a thread is created, the kernel stack is subtracted from this and every time a process is created, the minimum working set is subtracted from this.
...
// PFN_NUMBER MI_NONPAGEABLE_MEMORY_AVAILABLE(VOID);
// Routine Description:
//    This routine lets callers know how many pages can be charged against the resident available, factoring in earlier Mm promises that
//    may not have been redeemed at this point (ie: nonpaged pool expansion, etc, that must be honored at a later point if requested).
// Return Value:
...
*/
{
    ...
    // This is because the PFN reference count is updated lock free and so another thread can pass this one without charging against system, and if this thread
    // were to instead charge afterwards, it may fail below and it would be unable to stop the other thread(s) that have already proceeded.
...
Arguments:
    Pfn - Supplies the PFN entry to operate on.
    Force - Supplies TRUE if the lock is short-term and should be forced through if necessary.
    PointerPte - Supplies the PTE entry for the page being locked.
                 This should only be used for system space addresses where the share count is zero or the page is not active.
                 This is because this PTE argument is meaningless for large page addresses, but large page kernel addresses would never have a zero sharecount or non-active page state.
Return Value:
    TRUE if the charge succeeded, FALSE if not.
Environment:
...
*/
{
    ...
    if ((Pfn1->u2.ShareCount != 0) && (Pfn1->u3.e1.PageLocation == ActiveAndValid)) {
        ...
        // The only way this can happen is if a driver builds an MDL for a user address and then gets a system VA for it.
...
Routine Description:
    ...
    The PFN reference count is NOT decremented, instead the fault handler is going to up the share count and make this page valid.
Arguments:
    Pfn1 - Supplies the PFN entry to operate on.
Environment:
    Kernel mode.  PFN lock held.
...
Routine Description:
    ...
    The PFN reference checks are carefully ordered so the most common case is handled first, the next most common case next, etc.
    The case of a reference count of 2 occurs more than 1000x (yes, 3 orders of magnitude) more than a reference count of 1.
    And reference counts of >2 occur 3 orders of magnitude more frequently than reference counts of exactly 1.
...
*/
{
    ...
    if (RefCount == 2) {
        ...
        // There are still multiple referencers to this page (it may or may not be resident in any process address space).
        // Since the systemwide lock count can only be decremented by the last dereference (and this is not it), no action is taken here.
        NOTHING;
    }
}


// VOID MI_ZERO_WSINDEX (IN PMMPFN Pfn)
// Routine Description:
//     Zero the Working Set Index field of the argument PFN entry.
//     There is a subtlety here on systems where the WsIndex ULONG is overlaid with an Event pointer and sizeof(ULONG) != sizeof(PKEVENT).
//     Note this will need to be updated if we ever decide to allocate bodies of thread objects on 4GB boundaries.
// Arguments:
...
// Return Value:
//     The hash key.
#define MI_WSLE_HASH(Address, Wsl)     ((WSLE_NUMBER)(((ULONG_PTR)PAGE_ALIGN(Address) >> (PAGE_SHIFT - 2)) % ((Wsl)->HashTableSize - 1)))
...
typedef struct _MMWSLENTRY {
    ...
    // If Protection == MM_ZERO_ACCESS, it means the WSLE is using the same protection as the prototype PTE.
    // Otherwise the protection is real and potentially different from the prototype PTE.
...
// Mask off everything but the VPN and the valid bit so the WSLE compare can be done in one instruction.
...
// The claim estimate of unused pages in a working set is limited to grow by this amount per estimation period.
...
// If more than this "percentage" of the working set is estimated to be used then allow it to grow freely.
#define MI_REPLACEMENT_FREE_GROWTH_SHIFT 5


// If more than this "percentage" of the working set has been claimed then force replacement in low memory.
#define MI_REPLACEMENT_CLAIM_THRESHOLD_SHIFT 3


// If more than this "percentage" of the working set is estimated to be available then force replacement in low memory.
#define MI_REPLACEMENT_EAVAIL_THRESHOLD_SHIFT 3


// If while doing replacement a page is found of this age or older then replace it.  Otherwise the oldest is selected.
...
#define MI_TRIM_AGE_THRESHOLD 2// If not a forced trim, trim pages older than this age.
#define MI_FOREGROUND_CLAIM_AVAILABLE_SHIFT 3// This "percentage" of a claim is up for grabs in a foreground process.
#define MI_BACKGROUND_CLAIM_AVAILABLE_SHIFT 1// This "percentage" of a claim is up for grabs in a background process.


// DWORD MI_CALC_NEXT_VALID_ESTIMATION_SLOT (DWORD Previous, DWORD Minimum, DWORD Maximum, MI_NEXT_ESTIMATION_SLOT_CONST NextEstimationSlotConst, PMMWSLE Wsle)
// Routine Description:
//      We iterate through the working set array in a non-sequential manner so that the sample is independent of any aging or trimming.
//      This algorithm walks through the working set with a stride of 2^MiEstimationShift elements.
// Arguments:
...
// WSLE_NUMBER MI_NEXT_VALID_AGING_SLOT (DWORD Previous, DWORD Minimum, DWORD Maximum, PMMWSLE Wsle)
// Routine Description:
//      This finds the next slot to valid slot to age.  It walks through the slots sequentially.
// Arguments:
//      Previous - Last slot used
//      Minimum - Minimum acceptable slot (ie. the first dynamic one)
//      Maximum - Max slot number + 1
...
// ULONG MI_CALCULATE_USAGE_ESTIMATE (
//     IN PULONG SampledAgeCounts.
//     IN ULONG CounterShift
//     )
// Routine Description:
//      In Usage Estimation, we count the number of pages of each age in a sample.
//      The function turns the SampledAgeCounts into an estimate of the unused pages.
// Arguments:
...
// Return Value:
//      The number of pages to walk in the working set to get a good estimate of the number available.
...
// VOID MI_RESET_WSLE_AGE (IN PMMPTE PointerPte, IN PMMWSLE Wsle)
// Routine Description:
...
// ULONG MI_GET_WSLE_AGE (IN PMMPTE PointerPte, IN PMMWSLE Wsle)
// Routine Description:
//      Clear the age counter for the working set entry.
// Arguments:
//      PointerPte - pointer to the working set list entry's PTE
...
// VOID MI_INC_WSLE_AGE (IN PMMPTE PointerPte, IN PMMWSLE Wsle, )
// Routine Description:
...
// VOID MI_UPDATE_USE_ESTIMATE (IN PMMPTE PointerPte, IN PMMWSLE Wsle, IN ULONG *SampledAgeCounts)
// Routine Description:
...
// BOOLEAN MI_WS_GROWING_TOO_FAST (IN PMMSUPPORT VmSupport)
// Routine Description:
//      Limit the growth rate of processes as the available memory approaches zero.
//      Note the caller must ensure that MmAvailablePages is low enough so this calculation does not wrap.
// Arguments:
//      VmSupport - a working set.
// Return Value:
//      TRUE if the growth rate is too fast, FALSE otherwise.
#define MI_WS_GROWING_TOO_FAST(VmSupport)     ((VmSupport)->GrowthSinceLastEstimate > (((MI_CLAIM_INCR * (MmAvailablePages*MmAvailablePages)) / (64*64)) + 1))
...
typedef struct _SEGMENT {
    ...
    // Common fields are above and new common entries must be added to both the SEGMENT and MAPPED_FILE_SEGMENT declarations.
    union {
...
// Accesses to MMSUBSECTION_FLAGS2 are synchronized via the PFN lock (unlike MMSUBSECTION_FLAGS access which is not lock protected at all).
...
// Mapped data file subsection structure.  Not used for images or pagefile-backed shared memory.
...
// ULONG Mi4KStartForSubsection (IN PLARGE_INTEGER address, IN OUT PSUBSECTION subsection);
// Routine Description:
//    This macro sets into the specified subsection the supplied information indicating the start address (in 4K units) of this portion of the file.
// Arguments
//    address - Supplies the 64-bit address (in 4K units) of the start of this portion of the file.
...
// ULONG Mi4KStartFromSubsection (IN OUT PLARGE_INTEGER address, IN PSUBSECTION subsection);
// Routine Description:
//    This macro gets the start 4K offset from the specified subsection.
// Arguments
//    address - Supplies the 64-bit address (in 4K units) to place the start of this subsection into.
...
// This entry is used for calling the segment dereference thread to perform page file expansion.
// It has a similar structure to a control area to allow either a control area or a page file expansion entry to be placed on the list.
// Note that for a control area the segment pointer is valid whereas for page file expansion it is null.
...
// The MMINPAGE_FLAGS relies on the fact that a pool allocation is always QUADWORD aligned so the low 3 bits are always available.
...
typedef struct _AWEINFO {
    ...
    // The PushLock is used to allow most of the NtMapUserPhysicalPages{Scatter} to execute in parallel as this is acquired shared for these calls.
    // Exclusive acquisitions are used to protect maps against frees of the pages as well as to protect updates to the AweVadList.
    // Collisions should be rare because the exclusive acquisitions should be rare.
...
typedef struct _MMSESSION {
    // Never refer to the SystemSpaceViewLock directly - always use the pointer following it or you will break support for multiple concurrent sessions.
    KGUARDED_MUTEX SystemSpaceViewLock;

    // This points to the mutex above and is needed because the MMSESSION is mapped in session space and the mutex needs to be globally visible for proper KeWaitForSingleObject & KeSetEvent operation.
...
#define LOCK_SYSTEM_VIEW_SPACE(_Session)             KeAcquireGuardedMutex (_Session->SystemSpaceViewLockPointer)
#define UNLOCK_SYSTEM_VIEW_SPACE(_Session)           KeReleaseGuardedMutex (_Session->SystemSpaceViewLockPointer)
...
// If /NOLOWMEM is used, this is set to the boundary PFN (pages below this value are not used whenever possible).
extern PFN_NUMBER MiNoLowMemory;
...
//    IN PFN_NUMBER FRAME);
// Routine Description:
//    MiDecrementReferenceCountInline decrements the reference count inline, only calling MiDecrementReferenceCount if the count would go to zero which would cause the page to be released.
// Arguments:
...
// Routine Description:
//    MiDecrementShareCountInline decrements the share count inline, only calling MiDecrementShareCount if the count would go to zero which would cause the page to be released.
// Arguments:
//    PFN - Supplies the PFN to decrement.
//    FRAME - Supplies the frame matching the above PFN.
// Environment:
...
Routine Description:
    ...
    Any quota or commitment charged to the VAD has been (or will be) returned separately.
Arguments:
    Vad - Supplies a pointer to a virtual address descriptor.
...
// This variable is updated frequently, on MP systems we keep a separate system color per processor to avoid cache line thrashing.
...
#define MI_GET_COLOR_FROM_LIST_ENTRY(index,pfn)                             ((ULONG)(((pfn)->u3.e1.PageColor << MmSecondaryColorNodeShift) | MI_GET_SECONDARY_COLOR((index),(pfn))))
...
#define MiRemoveZeroPageIfAny(COLOR)                                        \
        KeNumberNodes > 1 ? (KeGetCurrentNode()->FreeCount[ZeroedPageList] != 0) ? MiRemoveZeroPage(COLOR) : 0 : (MmFreePagesByColor[ZeroedPageList][COLOR].Flink != MM_EMPTY_LIST) ? MiRemoveZeroPage(COLOR) : 0
...
#define MiRemoveZeroPageIfAny(COLOR)              (MmFreePagesByColor[ZeroedPageList][COLOR].Flink != MM_EMPTY_LIST) ? MiRemoveZeroPage(COLOR) : 0
...
Routine Description:
    ...
    It may release and reacquire the PFN lock to do so, as well as mapping the page in hyperspace to perform the actual zeroing if necessary.
Environment:
...
#if (_MI_PAGING_LEVELS <= 3)
// PMMPTE MiGetPxeAddress (IN PVOID va);
// Routine Description:
//    MiGetPxeAddress returns the address of the extended page directory parent entry which maps the given virtual address.
//    This is one level above the page parent directory.
// Arguments
...
// LOGICAL MiIsPteOnPxeBoundary (IN PVOID PTE);
// Routine Description:
//    MiIsPteOnPxeBoundary returns TRUE if the PTE is on an extended page directory parent entry boundary.
// Arguments
...
// PMMPTE MiGetPpeAddress (IN PVOID va);
// Routine Description:
//    MiGetPpeAddress returns the address of the page directory parent entry which maps the given virtual address.
//    This is one level above the page directory.
// Arguments
...
// LOGICAL MiIsPteOnPpeBoundary (IN PVOID VA);
// Routine Description:
//    MiIsPteOnPpeBoundary returns TRUE if the PTE is on a page directory parent entry boundary.
// Arguments
...
#if (_MI_PAGING_LEVELS >= 3)
#define MiDoesPpeExistAndMakeValid(PPE, PROCESS, PFNLOCKIRQL, WAITED)             MiDoesPdeExistAndMakeValid(PPE, PROCESS, PFNLOCKIRQL, WAITED)
#else
#define MiDoesPpeExistAndMakeValid(PPE, PROCESS, PFNLOCKIRQL, WAITED) 1
#endif

#if (_MI_PAGING_LEVELS >= 4)
#define MiDoesPxeExistAndMakeValid(PXE, PROCESS, PFNLOCKIRQL, WAITED)             MiDoesPdeExistAndMakeValid(PXE, PROCESS, PFNLOCKIRQL, WAITED)
#else
...
Routine Description:
    ...
    If the page table page exists and is not currently in memory, the working set pushlock and, if held, the PFN lock are released and the page table page is faulted into the working set.
    The pushlock and PFN lock are reacquired.
...
Arguments:
    ...
    Waited - Supplies a pointer to a ULONG to increment if the pushlock is released and reacquired.  Note this value may be incremented more than once.
Return Value:
...
*/
{
    ...
    // Check to make sure the protection field is really being interpreted correctly.
    if (PteContents.u.Hard.Valid == 1) {
        return FALSE;// The PTE is valid and therefore cannot be decommitted.
    }

    if ((PteContents.u.Soft.Prototype == 1) && (PteContents.u.Soft.PageFileHigh != MI_PTE_LOOKUP_NEEDED)) {
        return FALSE;// The PTE's protection is not known as it is in prototype PTE format.  Return FALSE.
    }

    return TRUE;// It is a decommitted PTE.
}
...
// Routine Description:
//    MiGetProtoPteAddress returns a pointer to the prototype PTE which is mapped by the given virtual address descriptor and address within the virtual address descriptor.
// Arguments
//    VAD - Supplies a pointer to the virtual address descriptor that contains the VA.
...
Routine Description:
    ...
    offset.  Note that images are stored in 512-byte units whereas data is stored in 4K units.
Arguments:
...
// Virtual alignment for PTEs (machine specific) minimum value is 4k maximum value is 64k.
// The maximum value can be raised by changing the MM_PROTO_PTE_ALIGNMENT constant and adding more reserved mapping PTEs in hyperspace.

extern PFN_COUNT MmNumberOfPhysicalPages;// Total number of physical pages on the system.
extern PFN_NUMBER MmHighestPossiblePhysicalPage;// Highest possible physical page number in the system.
...
extern ULONG MmSecondaryColorNodeMask;// Mask for isolating node color from combined node and secondary color.

// Width of MmSecondaryColorMask in bits.
// In multi node systems, the node number is combined with the secondary color to make up the page color.
...
extern KEVENT MmZeroingPageEvent;// Event for the zeroing page thread.

// Boolean to indicate if the zeroing page thread is currently active.
// This is set to true when the zeroing page event is set and set to false when the zeroing page thread is done zeroing all the pages on the free list.
extern BOOLEAN MmZeroingPageThreadActive;

extern PFN_NUMBER MmMinimumFreePagesToZero;// Minimum number of free pages before zeroing page thread starts.
extern KEVENT MmMappedFileIoComplete;// Global event to synchronize mapped writing with cleaning segments.

// Hyper space items.
extern PMMPTE MmFirstReservedMappingPte;
extern PMMPTE MmLastReservedMappingPte;

// System space sizes - MmNonPagedSystemStart to MM_NON_PAGED_SYSTEM_END defines the ranges of PDEs which must be copied into a new process's address space.
...
extern ULONG MmFlushCounter;// Counter for flushes of the entire TB.
...
Routine Description:
    ...
    Various optimizations may be made since this is only ever called with these types of addresses...

    Since paged pool is always mapped readwrite, just checking for validity is sufficient.
    Note also that the reference to the PTE may fault on x86 because the paged pool page directory entries are lazy filled, but the fault handler will take care of this for us.
Arguments:
    VirtualAddress - Supplies the virtual address to check.
Return Value:
    TRUE if no page fault would be generated writing the virtual address, FALSE otherwise.
Environment:
    Kernel mode, PFN lock held.
*/
{
...
// Virtual alignment for PTEs (machine specific) minimum value is 0 (no alignment) maximum value is 64k.
// The maximum value can be raised by changing the MM_PROTO_PTE_ALIGNMENT constant and adding more reserved mapping PTEs in hyperspace.
extern ULONG MmAliasAlignment;

// Mask to AND with virtual address to get an offset to go with the alignment.  This value is page aligned.
...
// Root of system space virtual address descriptors.  These define the pageable portion of the system.
extern PMMVAD MmVirtualAddressDescriptorRoot;
extern MM_AVL_TABLE MmSectionBasedRoot;
extern PVOID MmHighSectionBase;

extern KGUARDED_MUTEX MmSectionCommitMutex;// Section commit mutex.
extern KGUARDED_MUTEX MmSectionBasedMutex;// Section base address mutex.
...
extern KGUARDED_MUTEX MmPageFileCreationLock;// Pagefile creation mutex.
extern PKEVENT MmPagingFileCreated;// Event to set when first paging file is created.
extern ULONG_PTR MmPagingFileDebug[];// Paging file debug information.


// Fast mutex which guards the working set list for the system shared address space (paged pool, system cache, pageable drivers).
extern FAST_MUTEX MmSystemWsLock;

extern KSPIN_LOCK MmExpansionLock;// Spin lock for allowing working set expansion.
extern MMPTE GlobalPte;// To prevent optimizations.
...
// Worth a few TPS on a data base server.
...
// LOGICAL MI_UNUSED_SEGMENTS_SURPLUS (IN PVOID va);
// Routine Description:
//    This routine determines whether a surplus of unused segments exist.  If so, the caller can initiate a trim to free pool.
// Arguments
//    None.
// Return Value:
...
// Routine Description:
//    This routine inserts a control area into the unused segment list, also managing the associated pool charges.
// Arguments
...
// VOID MI_UNUSED_SEGMENTS_REMOVE_CHARGE (IN PCONTROL_AREA _ControlArea);
// Routine Description:
//    This routine manages pool charges during removals of segments from the unused segment list.
// Arguments
//    _ControlArea - Supplies the control area to obtain the pool charges from.
...
// Any value could have been used here that is not a valid system pointer or NULL - 1 was chosen because it simplifies checks for both NULL & HydraProcess by comparing for greater than HydraProcess.
...
#define MI_IS_SESSION_IMAGE_ADDRESS(VirtualAddress)     ((PVOID)(VirtualAddress) >= (PVOID)MiSessionImageStart && (PVOID)(VirtualAddress) < (PVOID)(MiSessionImageEnd))
#define MI_IS_SESSION_POOL_ADDRESS(VirtualAddress)      ((PVOID)(VirtualAddress) >= (PVOID)MiSessionPoolStart && (PVOID)(VirtualAddress) < (PVOID)MiSessionPoolEnd)
#define MI_IS_SESSION_ADDRESS(_VirtualAddress)          ((PVOID)(_VirtualAddress) >= (PVOID)MmSessionBase && (PVOID)(_VirtualAddress) < (PVOID)(MiSessionSpaceEnd))
#define MI_IS_SESSION_PTE(_Pte)                         ((PMMPTE)(_Pte) >= MiSessionBasePte && (PMMPTE)(_Pte) < MiSessionLastPte)
#define MI_IS_SESSION_IMAGE_PTE(_Pte)                   ((PMMPTE)(_Pte) >= MiSessionImagePteStart && (PMMPTE)(_Pte) < MiSessionImagePteEnd)
...
#define MM_BUMP_SESS_COUNTER(_index, bump) \
    if (_index >= MM_SESS_COUNTER_MAX) { \
        DbgPrintEx (DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "Mm: Invalid bump counter %d %d\n", _index, MM_SESS_COUNTER_MAX); \
        DbgBreakPoint(); \
    } \
...
// The value of SESSION_POOL_SMALL_LISTS is very carefully chosen for each architecture to avoid spilling over into an additional session data page.
...
// The session space data structure - allocated per session and only visible at MM_SESSION_SPACE_BASE when in the context of a process from the session.
// This virtual address space is rotated at context switch time when switching from a process in session A to a process in session B.
// This rotation is useful for things like providing paged pool per session so many sessions won't exhaust the VA space which backs the system global pool.

// A kernel PTE is also allocated to double map this page so that global pointers can be maintained to provide system access from any process context.
...
typedef struct _MM_SESSION_SPACE {
    ...
    LIST_ENTRY ProcessList;// This is the list of the processes in this group that have session space entries.

    LARGE_INTEGER LastProcessSwappedOutTime;

    // All the page tables for session space use this as their parent.
    // Note that it's not really a page directory - it's really a page table page itself (the one used to map this very structure).

    // This provides a reference to something that won't go away and is relevant regardless of which process within the session is current.
    PFN_NUMBER SessionPageDirectoryIndex;

    // This is the count of non paged allocations to support this session space.
    // This includes the session structure page table and data pages,
    // WSL page table and data pages, session pool page table pages and session image page table pages.
    // These are all charged against MmResidentAvailable.
    SIZE_T NonPageablePages;

    // This is the count of pages in this session that have been charged against the systemwide commit.
    // This includes all the NonPageablePages plus the data pages they typically map.
    SIZE_T CommittedPages;

    PVOID PagedPoolStart;// Start of session paged pool virtual space.

    PVOID PagedPoolEnd;// Current end of pool virtual space. Can be extended to the end of the session space.
    ...
    // This is the list of system images currently valid in this session space.
    // This information is in addition to the module global information in PsLoadedModuleList.
    ...
    // This is generally decremented in process delete (not clean) so that the session data page and mapping PTE can finally be freed when this reaches zero.
    // smss is the only process that decrements it in other places as smss never exits.
    LONG ProcessReferenceToSession;

    // This chain is in global system addresses (not session VAs) and can be walked from any system context, ie: for WSL trimming.
    LIST_ENTRY WsListEntry;

    // Session lookasides for fast pool allocation/freeing.
    GENERAL_LOOKASIDE Lookaside[SESSION_POOL_SMALL_LISTS];

    // Support for mapping system views into session space.
    // Each desktop allocates a 3MB heap and the global system view space is only 48M total.
    // This would limit us to only 20-30 users - rotating the system view space with each session removes this limitation.
    MMSESSION Session;
    ...
    POOL_DESCRIPTOR PagedPool;// Pool descriptor for less than 1 page allocations.

#if (_MI_PAGING_LEVELS >= 3)
    // The page directory that maps session space is saved here so trimmers can attach.
    MMPTE PageDirectory;
#else

    // The second level page tables that map session space are shared by all processes in the session.
    PMMPTE PageTables;

#endif

#if defined (_WIN64)
    // NT64 has enough virtual address space to support per-session special pool.
...
// Routine Description:
//    MiGetPdeSessionIndex returns the session structure index for the PDE will (or does) map the given virtual address.
// Arguments
...
// Session space contains the image loader and tracker, virtual address allocator, paged pool allocator, system view image mappings,
// and working set for kernel mode virtual addresses that are instanced for groups of processes in a Session process group.
// This process group is identified by a SessionId.

// Each Session process group's loaded kernel modules, paged pool allocations, working set,
// and mapped system views are separate from other Session process groups, even though they have the same virtual addresses.
// This is to support the Hydra multi-user Windows NT system by replicating WIN32K.SYS, and its complement of video and printer drivers, desktop heaps, memory allocations, etc.



// Structure linked into a session space structure to describe which system images in PsLoadedModuleTable and SESSION_DRIVER_GLOBAL_LOAD_ADDRESS's have been allocated for the current session space.
...
BOOLEAN FORCEINLINE MiCompareTbFlushTimeStamp(ULONG OldStamp, ULONG Mask)
{
    ...
    while (1) {
...
#else
        // If they are more than two apart it doesn't matter - a flush has occurred.
        if (Diff > 2) {
            return FALSE;
        }

        // If we captured the original stamp when unlocked then we only need the lock and unlock to have occurred.
        ...
        return TRUE;// Tell the caller to flush.
    }
}
...
#define MI_GET_HYPER_PAGE_TABLE_FRAME_FROM_PROCESS(_Process)         MI_GET_PAGE_FRAME_FROM_PTE((PMMPTE)(&((_Process)->Pcb.DirectoryTableBase[1])))
#else
#define MI_GET_DIRECTORY_FRAME_FROM_PROCESS(_Process)               (MI_GET_PAGE_FRAME_FROM_PTE(((PMMPTE)((_Process)->PaeTop)) + PD_PER_SYSTEM - 1))
...
// This field is zero if the module was loaded at boot time and the import information was never filled in.
// This field is -1 if no imports are defined by the module.
// This field contains a valid paged pool PLDR_DATA_TABLE_ENTRY pointer with a low-order (bit 0) tag of 1 if there is only 1 usable import needed by this driver.
// This field will contain a valid paged pool PLOAD_IMPORTS pointer in all other cases (ie: where at least 2 imports exist).
...
// ULONG MI_GET_PAGE_COLOR_FROM_PTE (IN PMMPTE PTEADDRESS);
// Routine Description:
//    This macro determines the pages color based on the PTE address that maps the page.
// Arguments:
...
// ULONG MI_GET_PAGE_COLOR_FROM_VA (IN PVOID ADDRESS);
// Routine Description:
//    This macro determines the pages color based on the PTE address that maps the page.
// Arguments:
...
// ULONG MI_GET_PAGE_COLOR_FROM_SESSION (IN PMM_SESSION_SPACE SessionSpace);
// Routine Description:
//    This macro determines the page's color based on the PTE address that maps the page.
// Arguments
//    SessionSpace - Supplies the session space the page will be mapped into.
// Return Value:
//    The page's color.

#define MI_GET_PAGE_COLOR_FROM_SESSION(_SessionSpace)           (((ULONG)((++_SessionSpace->Color) & MmSecondaryColorMask)) | MI_CURRENT_NODE_COLOR)


// ULONG MI_PAGE_COLOR_PTE_PROCESS (IN PMMPTE PTE, IN PUSHORT COLOR);
// Routine Description:
//    Select page color for this process.
// Arguments
//   PTE    Not used.
//   COLOR  Value from which color is determined.   This variable is incremented.
// Return Value:
//    The page's color.
#define MI_PAGE_COLOR_PTE_PROCESS(PTE,COLOR)           (((ULONG)(++(*(COLOR))) & MmSecondaryColorMask) | MI_CURRENT_NODE_COLOR)


// ULONG MI_PAGE_COLOR_VA_PROCESS (IN PVOID ADDRESS, IN PEPROCESS COLOR);
// Routine Description:
//    This macro determines the pages color based on the PTE address that maps the page.
// Arguments:
//    ADDRESS - Supplies the address the page is (or was) mapped at.
// Return Value:
//    The pages color.
#define MI_PAGE_COLOR_VA_PROCESS(ADDRESS,COLOR)          (((ULONG)(++(*(COLOR))) & MmSecondaryColorMask) | MI_CURRENT_NODE_COLOR)
