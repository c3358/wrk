...
#if defined(_M_AMD64)
VOID KeCompactServiceTable (IN PVOID Table, IN ULONG limit, IN BOOLEAN Win32k);
#define IMAGE_FILE_MACHINE_NATIVE   0x8664// Image header machine architecture
#endif
...
//  N.B. MODE_MASK and MODE_BIT assumes that all code runs at either ring-0 or ring-3 and is used to test the mode.
//       RPL_MASK is used for merging or extracting RPL values.
#define MODE_BIT 0
#define MODE_MASK 1                                                 // ntosp
#define RPL_MASK 3


// Startup count value for KeStallExecution.
// This value is used until KiInitializeStallExecution can compute the real one.
...
__forceinline VOID WRITE_REGISTER_UCHAR (volatile UCHAR *Register, UCHAR Value)
{
    *Register = Value;
    FastFence();
}
...
__forceinline VOID WRITE_REGISTER_USHORT (volatile USHORT *Register, USHORT Value)
{
    *Register = Value;
    FastFence();
}
...
__forceinline VOID WRITE_REGISTER_ULONG (volatile ULONG *Register, ULONG Value)
{
    *Register = Value;
    FastFence();
}
...
#define KeFillEntryTb(Virtual)                                      InvalidatePage(Virtual);
...
__forceinline VOID KiFlushProcessTb (VOID)
{
    ULONG64 Cr3;
...
// The acquire and release fast lock macros disable and enable interrupts on UP nondebug systems.
// On MP or debug systems, the spinlock routines are used.
...
#define KeQueryTickCount(CurrentCount)                                          *((PULONG64)(CurrentCount)) = *((volatile ULONG64 *)(SharedTickCount))
...
#define KiQueryInterruptTime(CurrentTime)                                       ((PLARGE_INTEGER)(CurrentTime))->QuadPart = *(PLONG64)(&SharedUserData->InterruptTime)
...
// Define two union definitions used for parsing addresses into the component fields required by a GDT.
...
// Task switches are not supported by the AMD64, but a task state segment must be present to define the kernel stack pointer and I/O map base.
...
// N.B. The kernel stack control area occupies the ending unused space in the NPX save area.
// N.B. The current kernel stack segment structure must immediately precede the previous kernel stack segment structure.
//      This adjacency is taken advantage of in the kernel stack segment enumeration routines.
...
// Start of the architecturally defined section of the PRCB.
// This section may be directly addressed by vendor/platform specific HAL code and will not change from version to version of NT.
    ULONG MxCsr;
    UCHAR Number;
    UCHAR NestingLevel;
...
// Numbered queued spin locks - 128-byte aligned.
    ...
    PP_LOOKASIDE_LIST PPLookasideList[16];// Nonpaged per processor lookaside lists - 128-byte aligned.
    PP_LOOKASIDE_LIST PPNPagedLookasideList[POOL_SMALL_LISTS];// Nonpaged per processor small pool lookaside lists - 128-byte aligned.
    PP_LOOKASIDE_LIST PPPagedLookasideList[POOL_SMALL_LISTS];// Paged per processor small pool lookaside lists.


// MP interprocessor request packet barrier - 128-byte aligned.

// This cache line shares per processor data with the packet barrier which is used to signal the completion of an IPI request.
...
// other data in the cache line increases the probability that the write will hit in the cache).
...
// the packet barrier cache line to transfer to the last finishing processor then back to respective processor.

// N.B. This results in minimal sharing of the cache line (no more than would
// have occurred if the packet barrier was in a cache line all by itself)) and increases the probability of a cache hit when packet barrier is initialized.
...
// Memory management counters.
    ...
    LONG LookasideIrpFloat;// I/O IRP float.
    ULONG KeSystemCalls;// Number of system calls.
...
// MP interprocessor request packet and summary - 128-byte aligned.
    ...
    REQUEST_MAILBOX RequestMailbox[MAXIMUM_PROCESSORS];// Interprocessor request summary - 128-byte aligned.
...
// N.B. Ready summary is in the first cache line as the queue for priority zero is never used.
...
// Processor information.
    ...
    PROCESSOR_POWER_STATE PowerState;// Processors power state
...
// Start of the architecturally defined section of the PCR.
// This section may be directly addressed by vendor/platform specific HAL code and will not change from version to version of NT.

// Certain fields in the TIB are not used in kernel mode.
// These include the exception list, stack base, stack limit, subsystem TIB, fiber data, and the arbitrary user pointer.
// Therefore, these fields are overlaid with other data to get better cache locality.
...
//      The preceding PCR layout cannot change for all time.
//      The initial architecturally defined part of the PRCB cannot change for all time either.
...
C_ASSERT(FIELD_OFFSET(KPCR, KdVersionBlock) == 0x108);// The offset of the DebuggerDataBlock must not change.
C_ASSERT(FIELD_OFFSET(KPCR, Prcb) == 0x180);// The offset to the PRCB must not change.
#endif

__forceinline ULONG KeGetContextSwitches (PKPRCB Prcb)
{
    return Prcb->KeContextSwitches;
}

VOID KeSaveLegacyFloatingPointState (PXMM_SAVE_AREA32 NpxFrame);
...
// N.B. The following values cannot be changed because the way in which compares are performed on these values.
...
/*
Routine Description:
    This function disables interrupts and returns whether interrupts were previously enabled.
Return Value:
    TRUE is returned if interrupts were previously enabled.
...
// If kernel mode, then caller can specify Carry, Parity, AuxCarry, Zero, Sign, Trap, Interrupt, Direction, Overflow, and identification.
// If user mode, then caller can specify Carry, Parity, AuxCarry, Zero, Sign, Trap, Interrupt, Direction, Overflow, identification, but Interrupt will always be forced on.
...
#define SANITIZE_MXCSR(_mxcsr_) ((_mxcsr_) & KiMxCsrMask)// Define macro to clear reserved bits from MXCSR.
#define SANITIZE_FCW(_fcw_) ((_fcw_) & 0x1f3f)// Define macro to clear reserved bits for legacy FP control word.
...
#define SIGN_EXTEND_BIT(_va_, _bit_)     (ULONG64)(((LONG64)(_va_) << (64 - (_bit_))) >> (64 - (_bit_)))


// Define routine to sanitize a virtual address based on previous mode and the specified segment selector.
FORCEINLINE ULONG64 SANITIZE_VA (IN ULONG64 VirtualAddress, IN USHORT Segment, IN KPROCESSOR_MODE PreviousMode)
/*
Routine Description:
    This routine canonicalizes a 64-bit virtual address according to the supplied segment selector.
Arguments:
    ...
    PreviousMode - Supplies the processor mode for which the exception and trap frames are being built.
Return Value:
...
*/
{
    ...
    if (PreviousMode == UserMode) {
        // Zero-extend 32-bit addresses, sign extend bit 48 of 64-bit addresses.
        if ((Segment == (KGDT64_R3_CMCODE | RPL_MASK)) || (Segment == (KGDT64_R3_DATA | RPL_MASK))) {
...
//  This frame is established when handling an exception.
//  It provides a place to save all nonvolatile registers. The volatile registers will already have been saved in a trap frame.

// N.B. The exception frame has a built in exception record capable of storing information for four parameter values.
//      This exception record is used exclusively within the trap handling code.
...
// Home address for the parameter registers.
    ...
    ULONG64 InitialStack;// Kernel callout initial stack value.
...
// Kernel callout frame variables.
    ...
    UCHAR ExceptionRecord[EXCEPTION_AREA_SIZE];// Exception record for exceptions.
    ULONG64 MxCsr;// Saved MXCSR when a thread is interrupted in kernel mode via a dispatch interrupt.
    ULONG64 Rbp;// Saved nonvolatile register - not always saved.
...
// Saved nonvolatile registers.
    ...
    ULONG64 Return;// EFLAGS and return address.
...
// This frame is established by code that trampolines to user mode (e.g. user APC, user callback, dispatch user exception, etc.).
// The purpose of this frame is to allow unwinding through these callbacks if an exception occurs.

// N.B. This frame is identical to the frame that is pushed for a trap without an error code and is identical to the hardware part of a trap frame.
...
// thread to the next and is used by the thread initialization code to construct a stack that will start the execution of a thread in the thread start up code.
...
// This frame is established by the AMD64 specific thread initialization code.
// It is used to store the initial context for starting a system thread.
...
// This frame is established when handling a trap. It provides a place to save all volatile registers.
// The nonvolatile registers are saved in an exception frame or through the normal C calling conventions for saved registers.
...
// Previous processor mode (system services only) and previous IRQL (interrupts only).
    KPROCESSOR_MODE PreviousMode;
    KIRQL PreviousIrql;

    UCHAR FaultIndicator;// Page fault load/store indicator.
...
//    2 - service frame.
    ...
    ULONG MxCsr;// Floating point state.

//  Volatile registers.

// N.B. These registers are only saved on exceptions and interrupts. They are not saved for system calls.
    ULONG64 Rax;
    ULONG64 Rcx;
    ULONG64 Rdx;
    ULONG64 R8;
...
//  Segment registers
    ...
    ULONG64 TrapFrame;// Previous trap frame address.

// Saved nonvolatile registers RBX, RDI and RSI. These registers are only saved in system service trap frames.
    ULONG64 Rbx;
    ULONG64 Rdi;
    ULONG64 Rsi;

// Saved nonvolatile register RBP. This register is used as a frame pointer during trap processing and is saved in all trap frames.
    ULONG64 Rbp;

// Information pushed by hardware.

// N.B. The error code is not always pushed by hardware. For those cases
//      where it is not pushed by hardware a dummy error code is allocated on the stack.
    union {
        ULONG64 ErrorCode;
...
// Copy of the global patch cycle at the time of the fault. Filled in by the invalid opcode and general protection fault routines.
    LONG CodePatchCycle;
...
NTKERNELAPI VOID KeProfileInterruptWithSource (IN PKTRAP_FRAME TrapFrame, IN KPROFILE_SOURCE ProfileSource);
NTKERNELAPI VOID KeUpdateRunTime (IN PKTRAP_FRAME TrapFrame, IN LONG Increment);
NTKERNELAPI VOID KeUpdateSystemTime (IN PKTRAP_FRAME TrapFrame, IN ULONG64 Increment);

// end_nthal

// The frame saved by the call out to user mode code is defined here to allow the kernel debugger to trace the entire kernel stack when user mode callouts are active.
...
#define DEFAULT_PROFILE_INTERVAL  39063// Define profile values.

// The minimum acceptable profiling interval is set to 1221 which is the fast RTC clock rate we can get.
// If this value is too small, the system will run very slowly.
...
#define PAGE_SIZE 0x1000// Define the page size for the AMD64 as 4096 (0x1000).
...
// Allow non-kernel components to capture the user probe address and use a local copy for efficiency.
...
#define MM_LOWEST_USER_ADDRESS (PVOID)0x10000// The lowest user address reserves the low 64k.
#define MM_LOWEST_SYSTEM_ADDRESS (PVOID)0xFFFF080000000000// The lowest address for system space.
...
#define KSEG0_BASE 0xFFFFF80000000000UI64// Define virtual base and alternate virtual base of kernel.
#define KSEG_ADDRESS(PAGE) ((PVOID)(KSEG0_BASE | ((ULONG_PTR)(PAGE) << PAGE_SHIFT)))// Generate kernel segment physical address.
...
// The following routines are provided for backward compatibility with old code.
// They are no longer the preferred way to accomplish these functions.
#if PRAGMA_DEPRECATED_DDK
...
__forceinline PKPCR KeGetPcr (VOID)// Get address of current processor block.
{
...
__forceinline ULONG KeGetCurrentNodeShiftedColor (VOID)// Get current node shifted color.
{
...
__forceinline PKPRCB KeGetCurrentPrcb (VOID)// Get address of current processor block.
{
...
__forceinline ULONG KeGetCurrentProcessorNumber (VOID)// Get the current processor number
{
...
NTKERNELAPI PKPRCB KeQueryPrcbAddress (__in ULONG Number);

// end_nthal end_ntosp


// N.B. The current processor number is stored in the architecturally defined region of the PRCB. The offset cannot change for all time.
...
// Is the current processor executing a DPC (either a threaded DPC or a legacy DPC).
...
__forceinline ULONG KeIsExecutingLegacyDpc (VOID)
{
...
__forceinline ULONG64 KeGetDpcStackBase (VOID)
{
...
#define KeIsIdleHaltSet(Prcb, Number) (((Prcb)->IdleHalt != 0) && ((Prcb)->Sleeping != 0))
...
NTHALAPI VOID FASTCALL HalRequestSoftwareInterrupt (KIRQL RequestIrql);

// Send an NMI interrupt to a set of processors.

NTHALAPI VOID FASTCALL HalSendNMI (KAFFINITY Affinity);
NTHALAPI VOID FASTCALL HalSendSoftwareInterrupt (KAFFINITY Affinity, KIRQL RequestIrql);


FORCEINLINE VOID KiRequestSoftwareInterrupt (KIRQL RequestIrql)
{
    PKPRCB Prcb;
...
// Define inline functions to get and set the handler address in and IDT entry.
...
__forceinline NTSTATUS KeSaveFloatingPointState (__out PVOID FloatingState)
{
...
__forceinline NTSTATUS KeRestoreFloatingPointState (__in PVOID FloatingState)
{
    UNREFERENCED_PARAMETER(FloatingState);
...
__forceinline VOID KeLowerIrql (__in KIRQL NewIrql)
/*
Routine Description:
...
NTKERNELAPI VOID KiCpuId (IN ULONG Function, IN ULONG Index, OUT PCPU_INFO CpuInfo);
...
ULONG64 __readmsr (IN ULONG Msr);
...
ULONG64 __readpmc (IN ULONG Counter);
...
__forceinline VOID KxAcquireSpinLock (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
...
__forceinline BOOLEAN KxTryToAcquireSpinLock (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
    This function attempts acquires a spin lock at the current IRQL.
    If the spinlock is already owned, then FALSE is returned.
    Otherwise, TRUE is returned.
Arguments:
    SpinLock - Supplies a pointer to a spin lock.
Return Value:
    If the spin lock is acquired a value TRUE is returned.
...
*/
{
    ...
    if (*(volatile LONG64 *)SpinLock == 0) {
        ...
        Thread = (LONG64)KeGetCurrentThread() + 1;
        return InterlockedCompareExchange64((PLONG64)SpinLock, Thread, 0) == 0 ? TRUE : FALSE;
#else
...
__forceinline PKSPIN_LOCK_QUEUE KiGetLockQueue (VOID)
{
    return (PKSPIN_LOCK_QUEUE)__readgsqword(FIELD_OFFSET(KPCR, LockArray));
}


__forceinline KIRQL KeAcquireSpinLockRaiseToDpc (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
    This function raises IRQL to DISPATCH_LEVEL and acquires the specified spin lock.
Arguments:
    SpinLock - Supplies a pointer to a spin lock.
Return Value:
...
__forceinline KIRQL KeAcquireSpinLockRaiseToSynch (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
    This function raises IRQL to SYNCH_LEVEL and acquires the specified spin lock.
Arguments:
    SpinLock - Supplies a pointer to a spin lock.
Return Value:
    The previous IRQL is returned as the function value.
...
__forceinline VOID KeAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
    This function acquires a spin lock at the current IRQL.
Arguments:
...
*/
{
    KxAcquireSpinLock(SpinLock);// Acquired the specified spin lock at the current IRQL.
}


__forceinline VOID KxReleaseSpinLock (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
...
__forceinline VOID KeReleaseSpinLock (__inout PKSPIN_LOCK SpinLock, __in KIRQL OldIrql)
/*
Routine Description:
    This function releases the specified spin lock and lowers IRQL to a previous value.
Arguments:
    SpinLock - Supplies a pointer to a spin lock.
...
__forceinline VOID KeReleaseSpinLockFromDpcLevel (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
...
__forceinline BOOLEAN KeTestSpinLock (__in PKSPIN_LOCK SpinLock)
/*
Routine Description:
...
__forceinline BOOLEAN KeTryToAcquireSpinLock (__inout PKSPIN_LOCK SpinLock, __out PKIRQL OldIrql)
/*
Routine Description:
    This function raises IRQL to DISPATCH level and attempts to acquire a spin lock.
    If the spin lock is already owned, then IRQL is restored to its previous value and FALSE is returned.
...
*/
{
    // Raise IRQL to DISPATCH level and attempt to acquire the specified spin lock.
    *OldIrql = KfRaiseIrql(DISPATCH_LEVEL);
...
__forceinline BOOLEAN KeTryToAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock)
/*
Routine Description:
    This function attempts acquires a spin lock at the current IRQL.
    If the spinlock is already owned, then FALSE is returned.
