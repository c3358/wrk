...
typedef enum _POOL_TYPE {
    ...
    // end_wdm
    ,
    // Note these per session types are carefully chosen so that the appropriate masking still applies as well as MaxPoolType above.
...
// The following two definitions control the raising of exceptions on quota and allocation failures.
...
NTKERNELAPI __bcount(NumberOfBytes) PVOID ExAllocatePool(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes);

DECLSPEC_DEPRECATED_DDK                     // Use ExAllocatePoolWithQuotaTag
NTKERNELAPI __bcount(NumberOfBytes) PVOID ExAllocatePoolWithQuota(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes);

NTKERNELAPI __bcount(NumberOfBytes) PVOID NTAPI ExAllocatePoolWithTag(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes, __in ULONG Tag);

// _EX_POOL_PRIORITY_ provides a method for the system to handle requests intelligently in low resource conditions.

// LowPoolPriority should be used when it is acceptable to the driver for the mapping request to fail if the system is low on resources.
// An example of this could be for a non-critical network connection where the driver can handle the failure case when system resources are close to being depleted.

// NormalPoolPriority should be used when it is acceptable to the driver for the mapping request to fail if the system is very low on resources.
// An example of this could be for a non-critical local filesystem request.

// HighPoolPriority should be used when it is unacceptable to the driver for the mapping request to fail unless the system is completely out of resources.
// An example of this would be the paging file path in a driver.

// SpecialPool can be specified to bound the allocation at a page end (or beginning).
// This should only be done on systems being debugged as the memory cost is expensive.

// N.B.  These values are very carefully chosen so that the pool allocation code can quickly crack the priority request.
...
NTKERNELAPI VOID NTAPI ExFreePool(__in PVOID P);
...
NTKERNELAPI VOID ExFreePoolWithTag(__in PVOID P, __in ULONG Tag);
...
NTKERNELAPI KIRQL ExLockPool(__in POOL_TYPE PoolType);
NTKERNELAPI VOID ExUnlockPool(__in POOL_TYPE PoolType, __in KIRQL LockHandle);
...
*/
{
    ...
    ASSERT(KeGetCurrentIrql() <= APC_LEVEL);// Raise IRQL to APC_LEVEL and attempt to acquire ownership of the fast mutex.
...
*/
{
    ...
    // N.B. On x86 this code may need to be compatible with the legacy fast mutex code in an OEM HAL.
    //      The HAL code stores the owning stack pointer and not the owning thread in the fast mutex owner field.
#if !defined (_X86_)
    ...
    OldIrql = (KIRQL)FastMutex->OldIrql;// Save the old IRQL and attempt to release the fast mutex.
    ...
    // If there are no waiters or a waiter has already been woken, then release the fast mutex. Otherwise, attempt to wake a waiter.
    if ((OldValue != 0) && ((OldValue & FM_LOCK_WAITER_WOKEN) == 0)) {
        // There must be at least one waiter that needs to be woken. Set the woken waiter bit and decrement the waiter count.
        // If the exchange fails, then another thread will do the wake.
        ...
        if (InterlockedCompareExchange(&FastMutex->Count, NewValue, OldValue) == OldValue) {
            KeSignalGateBoostPriority((PKGATE)&FastMutex->Gate);// There are one or more threads waiting for ownership of the mutex.
        }
    }
#endif
...
Arguments:
    FastMutex  - Supplies a pointer to a fast mutex.
Return Value:
    If the fast mutex was successfully acquired, then a value of TRUE is returned as the function value. Otherwise, a value of FALSE is returned.
*/
{
    KIRQL OldIrql;

    // Raise IRQL to APC_LEVEL and attempt to acquire ownership of the fast mutex.
    ASSERT(KeGetCurrentIrql() <= APC_LEVEL);
    OldIrql = KfRaiseIrql(APC_LEVEL);
    ...
    if (!InterlockedBitTestAndReset(&FastMutex->Count, FM_LOCK_BIT_V)) {
        ...
        // The fast mutex is owned - lower IRQL to its previous value and return FALSE.
        KeLowerIrql(OldIrql);
        KeYieldProcessor();
        return FALSE;
    } else {
        // Grant ownership of the fast mutex to the current thread and return TRUE.
        FastMutex->Owner = KeGetCurrentThread();
        FastMutex->OldIrql = OldIrql;
        return TRUE;
    }
...
*/
{
    ...
    FastMutex->Owner = NULL;// N.B. The first operation performed on the mutex is a write.
    OldIrql = (KIRQL)FastMutex->OldIrql;// Save the old IRQL and attempt to release the fast mutex.
    ...
    // If there are no waiters or a waiter has already been woken, then release the fast mutex. Otherwise, attempt to wake a waiter.
    if ((OldValue != 0) && ((OldValue & FM_LOCK_WAITER_WOKEN) == 0)) {
        // There must be at least one waiter that needs to be woken.
        // Set the woken waiter bit and decrement the waiter count.
        // If the exchange fails, then another thread will do the wake.
        OldValue = OldValue + FM_LOCK_BIT;
        NewValue = OldValue + FM_LOCK_WAITER_WOKEN - FM_LOCK_WAITER_INC;
        if (InterlockedCompareExchange(&FastMutex->Count, NewValue, OldValue) == OldValue) {
...
// The EX_SPIN_LOCK could be made a CHAR to save space - this would limit support to 127 processors since the high bit is used to denote exclusive.
// Thus we wouldn't want to export it this way, but could use this internally for structures that are tight on space
// (may get a lot of false cacheline pinging due to sharing though).
...
FORCEINLINE LOGICAL ExTryAcquireSpinLockExclusive(IN PEX_SPIN_LOCK SpinLock)
{
    ...
    do {
        ...
        if (LockContents & EXP_SPIN_LOCK_EXCLUSIVE) {// Otherwise it's safe to try to acquire exclusive ourselves.
            return FALSE;
        }
        ...
        if (InterlockedCompareExchangeAcquire(SpinLock, NewLockContents, LockContents) == LockContents) {
            ...
            return TRUE;// Now we finally own the lock exclusively.
        }
    } while (TRUE);
}
...
// A non-blocking queue is a singly link list of queue entries with a head pointer and a tail pointer.
// The head and tail pointers use sequenced pointers as do next links in the entries themselves.
// The queueing discipline is FIFO.
// New entries are inserted at the tail of the list and current entries are removed from the front of the list.

// Non-blocking queues require a descriptor for each entry in the queue.
// A descriptor consists of a sequenced next pointer and a PVOID data value.
// Descriptors for a queue must be preallocated and inserted in an SLIST before calling the function to initialize a non-blocking queue header.
// The SLIST should have as many entries as required for the respective queue.
...
// A sequenced interlocked list is a singly linked list with a header that contains the current depth and a sequence number.
// Each time an entry is inserted or removed from the list the depth is updated and the sequence number is incremented.
// This enables AMD64, IA64, and Pentium and later machines to insert and remove from the list without the use of spinlocks.
...
// N.B. Nonpaged lookaside list structures and pages lookaside list structures MUST be the same size for the system itself.
//      The per-processor lookaside lists for small pool and I/O are allocated with one allocation.
#if defined(_WIN64) || (!defined(_NTDDK_) && !defined(_NTIFS_) && !defined(_NDIS_))
...
#else
__inline PVOID ExAllocateFromPagedLookasideList(__inout PPAGED_LOOKASIDE_LIST Lookaside)
/*
Routine Description:
    This function removes (pops) the first entry from the specified paged lookaside list.
Arguments:
...
#if defined(_WIN2K_COMPAT_SLIST_USAGE) && defined(_X86_)
NTKERNELAPI VOID ExFreeToPagedLookasideList(__inout PPAGED_LOOKASIDE_LIST Lookaside, __in PVOID Entry);
#else
__inline VOID ExFreeToPagedLookasideList(__inout PPAGED_LOOKASIDE_LIST Lookaside, __in PVOID Entry)
/*
Routine Description:
    This function inserts (pushes) the specified entry into the specified paged lookaside list.
Arguments:
    Lookaside - Supplies a pointer to a nonpaged lookaside list structure.
    Entry - Supples a pointer to the entry that is inserted in the lookaside list.
*/
...
Routine Description:
    ...
    N.B. It is possible to context switch during the allocation from a per processor nonpaged lookaside list, but this should happen
         infrequently and should not aversely effect the benefits of per processor lookaside lists.
Arguments:
...
*/
{
    ...
    // If the per processor allocation attempt failed, then attempt to allocate from the system lookaside list.
    if (Entry == NULL) {
...
Routine Description:
    ...
    N.B. It is possible to context switch during the free of a per processor nonpaged lookaside list,
    but this should happen infrequently and should not aversely effect the benefits of per processor lookaside lists.
Arguments:
    Number - Supplies the per processor nonpaged lookaside list number.
    Entry - Supples a pointer to the entry that is inserted in the per processor lookaside list.
*/
{
    PGENERAL_LOOKASIDE Lookaside;
    PKPRCB Prcb;
    ...
    // If the current depth is less than of equal to the maximum depth, then free the specified entry to the per processor lookaside list.
    // Otherwise, free the entry to the system lookaside list;
    Prcb = KeGetCurrentPrcb();
    Lookaside = Prcb->PPLookasideList[Number].P;
...
// VOID ProbeAndReadUnicodeStringEx (OUT PUNICODE_STRING Destination, IN PUNICODE_STRING Source)
...
#else
#define ProbeAndReadStructureEx(Dst, Src, STRUCTURE)                             (Dst) = ProbeAndReadStructure(Src, STRUCTURE)
#endif
...
NTKERNELAPI VOID ExQueueWorkItem(__inout PWORK_QUEUE_ITEM WorkItem, __in WORK_QUEUE_TYPE QueueType);
...
NTKERNELAPI BOOLEAN ExIsProcessorFeaturePresent(__in ULONG ProcessorFeature);

// end_ntddk end_wdm end_nthal end_ntifs end_ntosp

// QueueDisabled indicates that the queue is being shut down, and new workers should not join the queue.
// WorkerCount indicates the total number of worker threads processing items in this queue.
// These two pieces of information need to do a RMW together, so it's simpler to smush them together than to use a lock.
typedef union {
    struct {
...
typedef struct _EX_WORK_QUEUE {
    KQUEUE WorkerQueue;// Queue objects that that are used to hold work queue entries and synchronize worker thread activity.
    ULONG DynamicThreadCount;// Number of dynamic worker threads that have been created "on the fly" as part of worker thread deadlock prevention
    ULONG WorkItemsProcessed;// Count of the number of work items processed.
...
// Return Value:
//     Pointer to previous block of storage that was at the head of the free list.
//     NULL implies the zone went from no available free blocks to at least one free block.
#if PRAGMA_DEPRECATED_DDK
...
// Routine Description:
//     This routine removes an entry from the zone and returns a pointer to it.
//     The removal is performed with the specified lock owned for the sequence to make it MP-safe.
// Arguments:
//     Zone - Pointer to the zone header controlling the storage from which the entry is to be allocated.
//     Lock - Pointer to the spin lock which should be obtained before removing the entry from the allocation list.
...
// Routine Description:
//     This routine places the specified block of storage back onto the free list in the specified zone.
//     The insertion is performed with the lock owned for the sequence to make it MP-safe.
// Arguments:
//     Zone - Pointer to the zone header controlling the storage to which the entry is to be inserted.
//     Block - Pointer to the block of storage to be freed back to the zone.
//     Lock - Pointer to the spin lock which should be obtained before inserting the entry onto the free list.
//        The lock is released before returning to the caller.
// Return Value:
//     Pointer to previous block of storage that was at the head of the free list.
//       NULL implies the zone went from no available free blocks to at least one free block.
...
// Waking bit designates that we are either traversing the list to wake threads or optimizing the list
...
typedef struct _EX_RUNDOWN_REF {// Rundown protection structure
...
typedef struct _EX_RUNDOWN_REF_CACHE_AWARE {
    PEX_RUNDOWN_REF RunRefs;//  Pointer to array of cache-line aligned rundown ref structures
...
//  The Ex/Ob handle table package uses a common handle definition.
//  The actual type definition for a handle is a pvoid and is declared in sdk/inc.
//  This package uses only the low 32 bits of the pvoid pointer.

//  For simplicity we declare a new typedef called an exhandle

//  The 2 bits of an EXHANDLE is available to the application and is ignored by the system.
//  The next 24 bits store the handle table entry index and is used to refer to a particular entry in a handle table.

//  Note that this format is immutable because there are outside programs with hardwired code that already assumes the format of a handle.
...
typedef struct _HANDLE_TABLE_ENTRY_INFO {
    //  The following field contains the audit mask for the handle if one exists.
    //  The purpose of the audit mask is to record all of the accesses that may have been audited when the handle was opened in order to support "per operation" based auditing.
    //  It is computed by walking the SACL of the object being opened and keeping a record of all of the audit ACEs that apply to the open operation going on.
...
//  The first field contains a pointer object and is overloaded with the three low order bits used by ob to denote inherited, protected, and audited objects.
//  The upper bit used as a handle table entry lock.
//  Note, this means that all valid object pointers must be at least longword aligned and have their sign bit set (i.e., be negative).

//  The next field contains the access mask (sometimes in the form of a granted access index,
//  and creator callback trace) if the entry is in use or a pointer in the free list if the entry is free.

//  Two things to note:
//  1. An entry is free if the object pointer is null, this means that the following field contains the FreeTableEntryList.
//  2. An entry is unlocked if the object pointer is positive and locked if its negative.
//     The handle package through callbacks and Map Handle to
//     Pointer will lock the entry (thus making the pointer valid) outside routines can then read and
//     reset the attributes field and the object provided they don't unlock the entry.
//     When the callbacks return the entry will be unlocked and the callers or MapHandleToPointer will need to call UnlockHandleTableEntry explicitly.
typedef struct _HANDLE_TABLE_ENTRY {
    ...
    //  This field either contains the granted access mask for the handle or an ob variation that also stores the same information.
    //  Or in the case of a free entry the field stores the index for the next free entry in the free list.
    //  This is like a FAT chain, and is used instead of pointers to make table duplication easier,
    //  because the entries can just be copied without needing to modify pointers.
    union {
        union {
...
typedef struct _HANDLE_TRACE_DEBUG_INFO {
    ...
    ULONG TableSize;// Size of the trace table in entries

    // this flag will clean the TraceDb.
    // once the TraceDb is cleaned, this flag will be reset.
    // it is needed for setting the HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE dynamically via KD
#define HANDLE_TRACE_DEBUG_INFO_CLEAN_DEBUG_INFO        0x1

    // this flag will do the following: for each close it will look for a matching open, remove the open entry and compact TraceDb
    // NOTE: this should not be used for HANDLE_TRACE_DB_BADREF because you will not have the close trace
#define HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE    0x2

    // setting this flag will break into debugger when the trace list wraps around.
    // This way you will have a chance to loot at old entries before they are deleted
...
//  This list is used by the snapshot handle tables call.
...
#define HANDLE_TABLE_LOCKS 4
    ...
    LIST_ENTRY HandleTableList;//  The list of global handle tables.  This field is protected by a global lock.
    EX_PUSH_LOCK HandleContentionEvent;// Define a field to block on if a handle is found locked.
    PHANDLE_TRACE_DEBUG_INFO DebugInfo;// Debug info. Only allocated if we are debugging handles

    //  The number of pages for additional info.
    //  This counter is used to improve the performance in ExGetHandleInfo
    LONG ExtraInfoPages;

    //  This is a singly linked list of free table entries.
    //  We don't actually use pointers, but have each store the index of the next free entry in the list.
    //  The list is managed as a lifo list.  We also keep track of the next index that we have to allocate pool to hold.
    ULONG FirstFree;

    // We free handles to this list when handle debugging is on or if we see that a thread has this handles bucket lock held.
    // The allows us to delay reuse of handles to get a better chance of catching offenders
    ULONG LastFree;

    ULONG NextHandleNeedingPool;// This is the next handle index needing a pool allocation. Its also used as a bound for good handles.
    LONG HandleCount;//  The number of handle table entries in use.

    // Define a flags field
    union {
        ULONG Flags;

        // For optimization we reuse handle values quickly.
        // This can be a problem for some usages of handles and makes debugging a little harder.
...
//  Macros for resetting the owner of the handle table, and current noop macro for setting fifo/lifo behavior of the table
...
//     It is unique only on the system it is generated on and not network wide.
//     N.B. A LUID is a 64-bit value and for all practical purposes will never carry in the lifetime of a single boot of the system.
...
NTKERNELAPI VOID ExGetCurrentProcessorCounts(__out PULONG IdleCount, __out PULONG KernelAndUser, __out PULONG Index);

// end_ntosp

extern BOOLEAN ExReadyForErrors;

// The following are global counters used by the EX component to indicate the amount of EventPair transactions being performed in the system.
...
extern LIST_ENTRY ExSystemLookasideListHead;// System lookaside list structure list.
...
extern BOOLEAN ExCmosClockIsSane;// Used for cmos clock sanity
extern PVOID ExPageLockHandle;// The lock handle for PAGELK section, initialized in init\init.c
...
NTKERNELAPI BOOLEAN ExVerifySuite(__in SUITE_TYPE SuiteType);
...
Return Value:
    LOGICAL - TRUE: There were cached references in the object, FALSE: No cached references are available.
*/
{
...
*/
{
    ...
    while (1) {
        ...
        // If the object pointer is null or if there are no cached references left then bail.
        // In the second case this reference will need to be taken while holding the lock.
        // Both cases are covered by the single test of the lower bits since a null pointer can never have cached refs.
        if (OldRef.RefCnt != 0) {
...
Routine Description:
    ...
    This routine could be called for a reference obtained directly from the object but presumably the chances of the pointer matching would be unlikely.
    The algorithm will work correctly in this case.
Arguments:
    FastRef - Fast reference block to be used
    Object - The original object that the reference was taken on.
Return Value:
    LOGICAL - TRUE: The fast dereference worked ok, FALSE: the dereference didn't.
*/
{
    EX_FAST_REF OldRef, NewRef;
...
Routine Description:
    ...
    Callers must have already biased the object by the RefsToAdd reference count.
    This operation can fail at which point the caller should removed the extra references added and continue.
Arguments:
    FastRef - Fast reference block to be used
    Object - The original object that has had its reference count biased.
    RefsToAdd - The number of references to add to the cache
Return Value:
    LOGICAL - TRUE: The references where cached ok, FALSE: The references could not be cached.
*/
{
    EX_FAST_REF OldRef, NewRef;
...
Routine Description:
    ...
    This routine must be called while holding the lock that protects the pointer field if concurrency with the slow ref path is possible.
    Its also possible to obtain and drop the lock after this operation has completed to force all the slow referencers from the slow reference path.
Arguments:
    FastRef - Fast reference block to be used
    Object - The new object that is to be placed in the structure.
             This object must have already had its reference count biased by the caller to account for the reference cache.
Return Value:
...
Arguments:
    ...
    Object - The new object that is to be placed in the structure.
             This object must have already had its reference count biased by the caller to account for the reference cache.
    OldObject - The object that must match the current object for the swap to occur.
Return Value:
    EX_FAST_REF - The old contents of the fast reference structure.
*/
{
