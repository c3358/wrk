...
#define HCELL_BIG_ROUND     (HBLOCK_SIZE*3)     // If someone tries to allocate a very large cell, round it up to HBLOCK_SIZE.
                                                // This is the rather arbitrary define for "very large"


// Never shrink the log files smaller than this, this prevents people
// from taking up all the disk space and then being unable to do critical registry operations (like logging on to delete some files)
...
#define HBLOCK_SIZE             0x1000                      // LOGICAL block size
                                                            // This is the size of one of the registry's logical/virtual pages.
                                                            // It has no particular relationship to page size of the machine.
...
#define HCELL_ALLOCATE_FILL 0xb2    // bz -> buzz buzz (yeah, it's a stretch)
                                    // must fill all newly allocated cells for security reasons
...
typedef struct  _HBIN {
    ...
    LARGE_INTEGER   TimeStamp;  // Old Link (from 1.0).  Usually trash, but
                                ...
                                // correspondence testing, only meaningful on disk.
    ULONG       Spare;          // this used to be MemAlloc.
                                // We don't use it anymore as we can't afford to touch the bin (it's not residing in paged-pool anymore, so touching it means modifying mnw pages).
...
// NOTE:    Hive storage is always allocated in units of 4K.
//          This size must be used on all systems, regardless of page size, since the file format needs to be transportable among systems.

// NOTE:    The integrity code depends on certain blocks (e.g., the BASE block) being at least as large as the size of a physical
//          sector.  (Otherwise data that should be left alone will
//          be written because the FS has to block/deblock.)  This means that the current code will not work with sectors > 4K.
...
//          This is not just for efficiency, but also to avoid risk of destruction of unlogged data.
//          Dirty bits keep track of what has been modified, they reside in a simple
//          bit map attached to the hive.  One bit for each logical sector of 512 bytes.

//          If the physical sector size of the machine is less than 512, no matter, we'll always write in clumps of 512.
//          If the physical sector size is greater than 512, we'll always clump
//          data together so that we log and write data in chunks of that size.
//          Physical sector sizes > 4K will not work correctly (logging will not work right, so system
...
// NOTE:    HBASE_BLOCK must be >= the size of physical sector, or integrity assumptions will be violated, and crash recovery may not work.
...
#define HSYS_MINOR_SUPPORTED    HSYS_WHISTLER   // Must be <= to write, always set up to writer's version.
...
// EXCEPT that the HBIN structures can be spread throughout memory rather than packed together.

// To find an HCELL in memory, a mechanism that takes an HCELL_INDEX and derives a memory address from it is used.
// That mechanism is very similar to a two level hardware paging table.
...
//  2. Discardable.  The HBIN containing this HBLOCK is completely free, but the bin is dirty and needs to be written to the hive file before it can be free.
//     This is the state we will be in if somebody frees a cell, causing the entire HBIN to become free.
//     HvpEnlistFreeCell will transition all the HBLOCKs in the free HBIN to this state, but will not free their memory.
//     After the dirty HBLOCKs are flushed to the file, the memory will be freed.

//     Note that if we need to allocate more storage from an HBIN in this
//     state, HvAllocateCell will simply change its state back to State 1 and it will be usable.

//     An HBLOCK in this state has a valid BlockAddress and BinAddress, but the HMAP_DISCARDABLE bit will be set.

//  3. Discarded.  The HBIN containing this HBLOCK is completely free, and is not dirty (i.e. it is marked as free in the hive file as well).
//     There is no memory allocated to contain this HBIN.
//     After HvSyncHive writes out an HBIN that is in State 2, it frees its pool and the HBIN moves into this state.

//     In order to use this HBIN, memory must be allocated to back it, and the HBIN and initial HCELL must be recreated.
//     (we could re-read it from the hive file, but there's not much point in that since we know
//     that it is entirely free, so we might as well just recreate it and save the disk i/o)

//     An HBLOCK in this state has a NULL BlockAddress in the map.
//     The BinAddress will contain the next HCELL in the free list, so we can reconstruct this when we need it.
...
#define HMAP_NEWALLOC       1               // the bin is the beginning of a new allocation. When bin is in view this doesn't really matter
#define HMAP_DISCARDABLE    2               // bin is discardable (i.e. is all free) first time when we get the chance we'll free it (if it is in paged pool)
...
typedef struct _HMAP_ENTRY {
    ULONG_PTR    BlockAddress;       // Low 2 bits always 0.  High bits are memory address of HBLOCK that HCELL starts in, add Offset to this.
                                    // (An HCELL can span several HBLOCKs)

    ULONG_PTR    BinAddress;         // Low bit set TRUE to mark beginning of a new allocation.
...
// NOTE:    Optimization - If the size of a hive is less than what can be mapped with a single HMAP_TABLE (HTABLE_SLOTS * HBLOCK_SIZE, or 2 megabytes) there is no real HMAP_DIRECTORY.
//          Instead, a single DWORD in the HHIVE acts as the 0th entry of the directory.

// NOTE:    Free Storage Management - When a hive is loaded, we build up a display (vector) of lists of free cells.
//          The first part of this vector contains lists that only hold one size cell.
...
//          The second part of this vector contains lists that hold more than one size cell.
//          Each size bucket is twice the previous size.
//          There are 8 of these lists, so all free cells between 136 and 32768 bytes are on these lists.

//          The last list in this vector contains all cells too large to fit in any previous list.

//          Example:    All free cells of size 1 HCELL_PAD (8 bytes) are on the list at offset 0 in FreeDisplay.
//                      All free cells of size 15 HCELL_PAD (120 bytes) are on the list at offset 0xe.
//                      All free cells of size 16-31 HCELL_PAD (128-248 bytes) are on the list at offset 0xf
//                      All free cells of size 32-63 HCELL_PAD (256-506 bytes) are on the list at offset 0x10.
//                      All free cells of size 2048 HCELL_PAD (16384 bytes) OR greater, are on the list at offset 0x17.
//          FreeSummary is a bit vector, with a bit set to true for each entry in FreeDisplay that is not empty.
...
typedef struct _HHIVE {
    ...
    ULONG                   Cluster;        // Usually 1 512 byte sector.
                                            // Set up force writes to be done in larger units on machines with larger sectors.
    ...
    struct _DUAL {
        ...
        FREE_DISPLAY        FreeDisplay[HHIVE_FREE_DISPLAY_SIZE];   // bitmap of freecells of the corresponding size
                                                                    // for every HBLOCK_SIZE - bin in the hive, a bit is set here if a free cell of the desired size lies in this block
