    ...
    for (i = Stable; i <= Volatile; i++) {
        for (j = 0; j < HHIVE_FREE_DISPLAY_SIZE; j++) {
            if (Hive->Storage[i].FreeDisplay[j].Display.Buffer != NULL) {
                ...
                (Hive->Free)(Hive->Storage[i].FreeDisplay[j].Display.Buffer, Hive->Storage[i].FreeDisplay[j].RealVectorSize);
...
*/
{
    ...
    if (NewBufferSize <= FreeDisplay->RealVectorSize) {
        // We don't shrink the vector; next time we grow, we'll perform the adjustments
    ...
    if (OldVector != NULL) {
        RtlCopyMemory(Vector, OldVector, OldBufferSize);// copy the already set bits
        (Hive->Free)(OldVector, OldBufferSize);// Free the old vector
    }
...
/*
Routine Description:
    Lookup for a free cell with the size NewSize in the CM_VIEW_SIZE window defined by Vicinity.
...
Return Value:
    A free cellindex with a size bigger than NewSize, or HCELL_NIL
Note:
    Vicinity is a physical file offset at this point. we need to convert it to a logical one prior to accessing the map
*/
{
    ULONG           FileOffsetEnd;
    HCELL_INDEX     Cell;
    ...
    while (FileOffsetStart < FileOffsetEnd) {
        ...
        if ((Me->BinAddress & (HMAP_INVIEW | HMAP_INPAGEDPOOL)) == 0) {
            ...
            // the hole hive; this will keep the view for this window mapped, as we hold the registry lock exclusive
...
/*
Routine Description:
    Lookup for a free cell with the size NewSize in the window defined by Vicinity.
    If it doesn't find a free cell for the specifed index, tries with the
Arguments:
    Hive - target hive.
    Index - index in FreeDisplay (based on the free cell size)
...
Return Value:
    A free cellindex with a size bigger than NewSize, or HCELL_NIL
Note:
    Vicinity is a logical file offset at this point.
    This function converts it to a physical one, and HvpScanForFindFreeCellInViewWindow converts it back to logical prior to getting the cell map.
*/
{
    HCELL_INDEX     Cell;
    ...
    // translated with HBLOCK_SIZE; HvpScanForFreeCellInViewWindow will do the reverse computation to adjust the offset)
    ...
    // We now have a summary of lists that are non-null and may contain entries large enough to satisfy the request.
    // Iterate through the list and pull the first cell that is big enough.
    // If no cells are big enough, advance to the next non-null list.
    ...
    while (Summary != 0) {
        ...
        // Clear the bit in the summary and try the next biggest size
...
/*
Routine Description:
    Lookup for a free cell. First try is in the CM_VIEW_SIZE window defined by Vicinity.
    If no free cell is found in this window (or vicinity if NIL), entire hive is searched (window by window).
Arguments:
    Hive - target hive.
    Index - index in FreeDisplay (based on the free cell size)
    NewSize - desired size
...
BOOLEAN HvpCheckViewBoundary(IN ULONG Start, IN ULONG End)
/*
Routine Description:
    Checks if addresses are in the same CM_VIEW_SIZE boundary.
    We may convert this function to a macro for performance reasons
Arguments:
