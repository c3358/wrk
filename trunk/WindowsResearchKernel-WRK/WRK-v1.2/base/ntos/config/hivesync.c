...
Abstract:
    This module implements procedures to write dirty parts of a hive's stable store to backing media.
*/
...
VOID CmpLogError(IN NTSTATUS NtStatusCode);
...
BOOLEAN HvMarkCellDirty(PHHIVE Hive, HCELL_INDEX Cell, BOOLEAN LockHeld)
/*
Routine Description:
...
*/
{
    ...
    // If it's an old format hive, mark the entire bin dirty, because the Last backpointers are such a pain to deal with in the partial alloc and free-coalescing cases.
...
BOOLEAN HvMarkDirty(PHHIVE Hive, HCELL_INDEX Start, ULONG Length, BOOLEAN DirtyAndPin)
/*
Routine Description:
    Marks the relevant parts of a hive dirty, so that they will be flushed to backing store.

    If Hive->Cluster is not 1, then adjacent all logical sectors in the given cluster will be forced dirty (and log space allocated for them.)
    This must be done here rather than in HvSyncHive so that we can know how much to grow the log.

    This is a noop for Volatile address range.
    NOTE:   Range will not be marked dirty if operation fails.
    ATTENTION:  This routine assumes that no more than a bin is marked dirty at the time.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
    Start - supplies a hive virtual address (i.e., an HCELL_INDEX or like form address) of the start of the area to mark dirty.
...
*/
{
    ...
    // make sure that between first and last all bins are valid (either pinned or allocated from paged pool).
...
/*
Routine Description:
    Adjust the log for growth in the number of sectors of dirty data that are desired.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
...
Routine Description:
    ...
    1. remove all discardable bins from FreeBins list. they'll be enlisted afterwards with the right (accurate) values.
    2. read the base block, and eventually free the tail of the hive
    3. unpin and purge all pinned views; also clear the free cell hint for mapped bins.
    4. remap views purged at 3 and reenlist the bins inside. this will fix free bins discarded at 1.
    5. iterate through the map; read and reenlist all bins that are in paged-pool (and dirty)
...
Comments:
    In the new implementation, bins are not discarded anymore. Step 1. above is not needed anymore.

    Discardable bins with FREE_HBIN_DISCARDABLE flag set fall into one of the categories:
    1. new bins (at the end of the hive) which didn't get a chance to be saved yet. HvFreeHivePartial will take care of them.
    ...
    Discardable bins with FREE_HBIN_DISCARDABLE flag NOT set are free bins which came from mapped views.
    Step 3 will remove them from the FreeBins list and step 4 will reenlist the ones that are still free after remapping
*/
{
    ...
    //    - remove the discardable flag from the ones that have not yet been discarded
    //    - for discarded ones, just remove the marker from the FreeBins list

    // Any bins that have been marked as discardable, but not yet flushed to disk, are going to be overwritten with old data.
    // Bring them back into memory and remove their FREE_HBIN marker from the list.
    // Other bins are  either discarded, or mapped into views
    ...
    // OverRead base block.
    Offset = 0;
    if ((Hive->FileRead)(Hive, HFILE_TYPE_PRIMARY, &Offset, CmpStashBuffer, HBLOCK_SIZE) != TRUE) {
        UNLOCK_STASH_BUFFER();
        return;
    }
    ...
    while (IsListEmpty(&(CmHive->PinViewListHead)) == FALSE) {
        ...
        // now, for every block in this range which is mapped in view clear the dirty bit, and the free cell hint
        ...
        // now we need to make sure the 256K window surrounding this offset is not mapped in any way
    ...
    // If we read the start of any HBINs into memory, it is likely their MemAlloc fields are invalid.
    // Walk through the HBINs and write valid MemAlloc values for any HBINs whose first sector was reread.

    // HvpFindNextDirtyBlock knows how to deal with free bins.
    // If we reread a free bin, we need to delist it from the list first and reenlist it again (it may not be free on the disk)
    p = 0;
    while (p < Hive->Storage[Stable].Length) {
        ...
        if (RtlCheckBit(&Hive->DirtyVector, p / HSECTOR_SIZE) == 1) {
            if ((t->BinAddress & HMAP_DISCARDABLE) != 0) {
                ...
                // get rid of the entry from FreeBins list
                // it'll be added again after sync is done if bin is still discardable
                RemoveEntryList(&FreeBin->ListEntry);
                (Hive->Free)(FreeBin, sizeof(FREE_HBIN));
            }
            ...
            // Reset the BinAddress to the Block address to cover the case where a few smaller bins have been coalesced into a larger bin.
            // We want the smaller bins back now.
            t->BinAddress = HBIN_FLAGS(t->BinAddress) | t->BlockAddress;

            // Check the map to see if this is the start of a memory allocation or not.
            if (t->BinAddress & HMAP_NEWALLOC) {
                // Walk through the map to determine the length of the allocation.
    ...
    // Finally we need to rewrite the parent field in the root hcell.
    // This is patched in at hive load time so the correct value could have just been overwritten with whatever happened to be on disk.
    RootNode = (PCM_KEY_NODE)HvGetCell(Hive, RootCell);
    if (RootNode == NULL) {
        // we couldn't map a view for this cell
    ...
    // means the file is too big. Do it here, where we are sure we have no pinned data whatsoever.
...
#if DBG
BOOLEAN HvIsCellDirty(IN PHHIVE Hive, IN HCELL_INDEX Cell)
/*
Routine Description:
    Given a hive and a cell, checks whether the corresponding sector is marked as dirty.
    NOTE: This function assumes the view containing the bin is mapped into system space.
Arguments:
...
/*
Routine Description:
    Adjust the log for growth in the size of the hive, in particular, account for the increased space needed for a bigger dirty vector.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
...
/*
Routine Description:
    Force backing store to match the memory image of the Stable part of the hive's space.

    Logs, primary, and alternate data can be written.
    Primary is always written.
    Normally either a log or an alternate, but not both, will also be written.
...
*/
{
    ...
    if (CmpMiniNTBoot) {
        ...
        for (Index = 0; Index < CM_NUMBER_OF_MACHINE_HIVES; Index++) {
            if ((CmpMachineHiveList[Index].Name != NULL) && ((CmpMachineHiveList[Index].CmHive == CurrentHive) || (CmpMachineHiveList[Index].CmHive2 == CurrentHive))) {
                SkipWrite = TRUE;
                break;
            }
        }
...
Routine Description:
    ...
    It will clean the bits in the DirtyVector for the blocks that are flushed.
...
*/
{
    ...
    while (IsListEmpty(&(CmHive->PinViewListHead)) == FALSE) {
        ...
        // now, for every block in this range which is mapped in view clear the dirty bit
        while (FileOffsetStart < FileOffsetEnd) {
            if (FileOffsetStart >= Hive->Storage[Stable].Length) {
                // This mean the hive has shrunk during the HvpTruncateBins call all we have to do is clear the dirty bits and bail out
            ...
            if (Me->BinAddress & HMAP_DISCARDABLE) {
                ...
                // bin is discardable, or discarded; still, if it was mapped, clear of the dirty bits
                if (Me->BinAddress & HMAP_INVIEW) {
                ...
                if (Me->BinAddress & HMAP_INVIEW) {
                    ...
                    // clear the dirty bits for this bin as dirty blocks will be saved while unpinning the view
        ...
        CmpUnPinCmView(CmHive, CmView, FALSE, TRUE);// UnPin the view; this will flush all dirty blocks to the backing storage
    } // while (IsListEmpty)
}
...
/*
Routine Description:
    Write dirty parts of the hive out to either its primary or alternate file.
    Write the header, flush, write all data, flush, update header, flush.
    Assume either logging or primary/alternate pairs used.

    NOTE:   TimeStamp is not set, assumption is that HvpWriteLog set that.
    It is only used for checking if Logs correspond anyway.
Arguments:
    Hive - pointer to Hive for which dirty data is to be written.
...
*/
{
    ...
    // flush first, so that the filesystem structures get written to disk if we have grown the file.
    ...
    if (BaseBlock->Sequence1 != BaseBlock->Sequence2) {
        // Some previous log attempt failed, or this hive needs to be recovered, so punt.
#if DBG
    ...
    if ((Hive->Log) && (Hive->LogSize > HLOG_MINSIZE(Hive))) {
        // Shrink log back down, reserve at least two clusters worth of space so that if all the disk space is
        // consumed, there will still be enough space prereserved to allow a minimum of registry operations so the user can log on.
...
/*
Routine Description:
    Write a header, the DirtyVector, and all the dirty data into the log file.
    Do flushes at the right places.
    Update the header.
Arguments:
...
*/
{
    ...
    // try to allocate a stash buffer. if we fail. we fail to save the hive only save what is relevant
...
/*
Routine Description:
    This routine finds and reports the largest run of dirty logical sectors in the hive, which are contiguous in memory and on disk.
Arguments:
    Hive - pointer to Hive of interest.
    BitMap - supplies a pointer to a bitmap structure, which describes what is dirty.
    Current - supplies a pointer to a variable that tracks position in the bitmap.  It is a bitnumber.  It is updated by this call.
    Address - supplies a pointer to a variable to receive a pointer to the area in memory to be written out.
    Length - supplies a pointer to a variable to receive the length of the region to read/write
    Offset - supplies a pointer to a variable to receive the offset in the backing file to which the data should be written.
    ...
    *Offset = FileBaseAddress + HBLOCK_SIZE;
    ...
    while (RunLength < FileLength) {
        ...
        if ((NextBlock - Block) != HBLOCK_SIZE) {
            // We've hit a discontinuity in memory.
            // RunLength is as long as it's going to get.
            break;
        }

        if ((FileEndAddress - FileBaseAddress) <= HBLOCK_SIZE) {
            // We've reached the tail block, all is contiguous, fill up to end and return.
            *Length = FileLength;
            *Current = End;
            return TRUE;
        }
...
BOOLEAN HvHiveWillShrink(IN PHHIVE Hive)
/*
Routine Description:
    Applies to stable storage only. Helps determining whether the hive will shrink on first flush
*/
{
    PHMAP_ENTRY Map;
    ULONG NewLength;
...
/*
Routine Description:
    Attempts to shrink the hive by truncating any bins that are discardable at the end of the hive.
    Applies to both stable and volatile storage.
Arguments:
...
*/
{
    ...
    for (i = 0; i < HTYPE_COUNT; i++) {
        ...
        if (NewLength < Hive->Storage[i].Length) {
            HvFreeHivePartial(Hive, NewLength, i);// There are some free bins to truncate.
        }
    }
}
...
/*
Routine Description:
    Frees all bins allocated from page pool, which are at the end of the hive, then update their map (clears the HMAP_INPAGEDPOOL flag).
    Next attempt to read from those bins will map a view for them. Checks for CM_VIEW_SIZE boundary, before freeing a bin.

    It also tags each start of the bin with HMAP_NEWALLOC;
    This will allow us to use MAP_NO_READ flag in CcMapData (now that we enabled MNW feature for registry streams, we know that Mm will fault only one page at the time for these kind of streams)

    Applies only to permanent storage.
Arguments:
    Hive - Supplies the hive to operate on..
    Check - debug only, beginning of the bin should already tagged as HMAP_NEWALLOC
*/
{
    ULONG_PTR   Address;
    LONG        Length;
    ...
    while (Length >= 0) {
        ...
        Length = Bin->FileOffset - HBLOCK_SIZE;// advance (backward) to the previous bin
...
#endif
                    Me->BinAddress |= HMAP_NEWALLOC;// tag it as a new alloc, so we can rely on this flag in CmpMapCmView
                } else {
                    Me->BinAddress &= ~HMAP_NEWALLOC;// remove the NEWALLOC flag (if any), so we can rely on this flag in CmpMapCmView
                }

                Offset += HBLOCK_SIZE;// advance to the next HBLOCK_SIZE of this bin
            }
    ...
    if (UnMap == TRUE) {
        ...
        while (Offset < Length) {
            ...
            Offset += HBLOCK_SIZE;// next, please ?
        }
    }
...
/*
Routine Description:
        Works as HvpDropPagedBins, only that it iterates through the entire hive. No views are mapped at this point.
Arguments:
    Hive - Supplies the hive to operate on..
*/
{
...
/*
Routine Description:
    Write the hive out.  Write only to the Primary file, neither logs nor alternates will be updated.
    The hive will be written to the HFILE_TYPE_EXTERNAL handle.

    Intended for use in applications like SaveKey.

    Only Stable storage will be written (as for any hive.)

    Presumption is that layer above has set HFILE_TYPE_EXTERNAL handle to point to correct place.

    Applying this call to an active hive will generally hose integrity measures.
...
*/
{
    ...
    while (BinOffset < Hive->Storage[Stable].Length) {
        ...
        BinOffset += BinSize;// advance to the next bin
    }

    CmpDereferenceHiveViewWithLock((PCMHIVE)Hive, CmView);// let go last view referenced (if any)
