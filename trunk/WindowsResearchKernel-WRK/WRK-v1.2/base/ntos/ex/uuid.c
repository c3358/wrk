...
Abstract:
    This module implements the core time and sequence number allocation for UUIDs (exposed to user mode),
    as well as complete UUID creation (exposed to kernel mode only).
...
*/
{
    ...
    if (ExpUuidSequenceNumberNotSaved == TRUE) {// Does the sequence number need to be saved?
        ...
        if (NT_SUCCESS(Status)) {// Indicate that it's now been saved.
...
*/
{
    ...
    KeQuerySystemTime(&ExpUuidLastTimeAllocated);// We can use the current time since we'll be changing the sequence number.
    return TRUE;
}
...
*/
{
    ...
    if (ExpUuidSequenceNumberValid == FALSE) {
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            // This should only happen when we're called for the first time on a given machine. (machine, not boot)
    ...
    // Get the current time, usually we will have plenty of available to give the caller.
    ...
    if (AvailableTime.QuadPart < 0) {
        // Time has been set time backwards. 
        // This means that we must make sure that somebody increments the sequence number and saves the new sequence number in the registry.
        ExpUuidSequenceNumberNotSaved = TRUE;
        ExpUuidSequenceNumber++;

        // The sequence number has been changed, so it's now okay to set time backwards.  
        // Since time is going backwards anyway, it's okay to set it back an extra millisecond or two.
    ...
    if (AvailableTime.QuadPart > 10 * 1000) {
        // We've got over a millisecond to give out.  
        // We'll save some time for another caller so that we can avoid returning STATUS_RETRY very often.
        *Range = 10 * 1000;
...
*/
{
    ...
    try {
        ...
        if ((Seed[0] & 0x80) == 0) {
            ExpUuidCacheValid = CACHE_VALID;// If the high bit is not set the NodeId is a valid IEEE 802 address and should be globally unique.
        } else {
...
*/
{
    ...
    // Get the sequence number and a range of times that can be used in UUID-generation.
    ...
    // If necessary, save the sequence number.
    // If there's an error, we'll just leave it marked as dirty, and retry on some future call.
...
*/
{
    ...
    // ExpAllocateUuids keeps time in SYSTEM_TIME format which is 100ns ticks since Jan 1, 1601.  UUIDs use time in 100ns ticks since Oct 15, 1582.
    ...
    for (;;) {
        ...
        *(PULONG)&UuidGen->NodeId[2] = *(PULONG)&ExpUuidCachedValues.NodeId[2];
        ...
        if (Time != ExpUuidCachedValues.Time) {
            // If our captured time doesn't match the cache then another thread already took the lock and updated the cache.
            // We'll just loop and try again.
            continue;
        }

        // If the cache hadn't already run dry, we can break out of this retry loop.
        if (Delta >= 0) {
            break;
        }
