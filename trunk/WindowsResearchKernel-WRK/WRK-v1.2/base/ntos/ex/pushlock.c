...
Abstract:
    ...
    Acquire and release is done lock free.
    On lock contention the waiters are chained through the lock and local stack space.
    ...
    The non-contented acquires and releases are trivial. Interlocked operations make the following transformations.
    ...
    There are two cases for shared acquire. Notice that we can only acquire in the absence of waiters. The first and later acquires:
    ...
    Contention causes the acquiring thread to produce a local stack based wait block and to enqueue it to the front of the list.

    Notice that after a contended acquire takes place the K bit is always set.
    It was either set by another thread or this thread. The thread that sets this bit has the right to walk the waiters chain and optimize it (place pointers to ending blocks etc).

    Owners of the K bit must either clear the K bit if the lock is still locked or clear the K bit and wake at least one thread (if the lock is not locked).
    ...
    After contention has caused one or more threads to queue up wait blocks releases are more complicated.
    The K bit arbitrates who is allowed to manipulate the waiters list but it does
    not guaranteed complete freedom. The following are the rights to manipulate the list:

    1) New waiters at any time may push themselves onto the list.
       If they are not the first waiter they will also attempt to set the K bit to perform list optimization.
    2) Shared releasers are allowed to walk the list (hopefully optimized by the waiters) to find the end of the chain where the share count lives.
       If they decrement the shared count to zero they behave like exclusive releasers and try to set the K bit.
    3) K bit owners, either waiters that pend and manage to set the bit or releasers who manage to set the bit have the right to walk the chain and break it and wake.
       They can't reorder without excluding shared walkers (we don't attempt to reorder just put in end pointers).

    The K bit is not a lock in the sense that we don't need to spin on it.
    Only one owner is needed at any time and failure to obtain K bit ownership means some other thread will wake etc.

    The M bit is needed for lock releasers who don't know if they own the lock shared.
    It also has the benefit that only when there are more than one shared owner do we need to traverse the wait list.
...
VOID ExpInitializePushLocks(VOID);

#ifdef ALLOC_PRAGMA
// Since pushlocks are used as the working set synchronization by memory management, these routines cannot be pageable.
...
*/
{
    ...
    // Exercise the following routines early, in order to patch any prefetchw instructions therein.
...
NTKERNELAPI VOID FASTCALL ExfWakePushLock(IN PEX_PUSH_LOCK PushLock, IN EX_PUSH_LOCK TopValue)
/*
Routine Description:
...
*/
{
    ...
    while (1) {
        ASSERT(!OldValue.MultipleShared);// Nobody should be walking the list while we manipulate it.
        ...
        while (OldValue.Locked) {
            ...
            if ((NewValue.Ptr = InterlockedCompareExchangePointer(&PushLock->Ptr, NewValue.Ptr, OldValue.Ptr)) == OldValue.Ptr) {
                return;
            }
            OldValue = NewValue;
        }
        ...
        if (WaitBlock->Flags&EX_PUSH_LOCK_FLAGS_EXCLUSIVE && (PreviousWaitBlock = WaitBlock->Previous) != NULL) {
    ...
    // If we are waking more than one thread then raise to DPC level to prevent us getting rescheduled part way through the operation
...
VOID FASTCALL ExfTryToWakePushLock(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
...
VOID FASTCALL ExpOptimizePushLockList(IN PEX_PUSH_LOCK PushLock, IN EX_PUSH_LOCK TopValue)
/*
Routine Description:
...
NTKERNELAPI VOID DECLSPEC_NOINLINE FASTCALL ExfAcquirePushLockExclusive(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
...
*/
{
    ...
    while (1) {
        // If the lock is already held exclusively/shared or there are waiters then we need to wait.
        if ((OldValue.Value&EX_PUSH_LOCK_LOCK) == 0) {
            ...
            if (OldValue.Waiting) {
                WaitBlock.Last = NULL;
                WaitBlock.Next = (PEX_PUSH_LOCK_WAIT_BLOCK)(OldValue.Value & ~EX_PUSH_LOCK_PTR_BITS);
                WaitBlock.ShareCount = 0;
                NewValue.Ptr = (PVOID)(((ULONG_PTR)&WaitBlock) | (OldValue.Value & EX_PUSH_LOCK_PTR_BITS) | EX_PUSH_LOCK_WAITING | EX_PUSH_LOCK_WAKING | EX_PUSH_LOCK_LOCK);
                if (!OldValue.Waking) {
                    Optimize = TRUE;
                }
            } else {
                ...
                if (WaitBlock.ShareCount > 1) {
                    NewValue.Ptr = (PVOID)(((ULONG_PTR)&WaitBlock) | EX_PUSH_LOCK_WAITING | EX_PUSH_LOCK_LOCK | EX_PUSH_LOCK_MULTIPLE_SHARED);
                } else {
                    WaitBlock.ShareCount = 0;
                    NewValue.Ptr = (PVOID)(((ULONG_PTR)&WaitBlock) | EX_PUSH_LOCK_WAITING | EX_PUSH_LOCK_LOCK);
                }
            }
...
NTKERNELAPI VOID DECLSPEC_NOINLINE FASTCALL ExfAcquirePushLockShared(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
    ...
    while (1) {
        ...
        if (!OldValue.Locked || (!OldValue.Waiting && OldValue.Shared > 0)) {
            ...
            if (OldValue.Waiting) {
                WaitBlock.Last = NULL;
                WaitBlock.Next = (PEX_PUSH_LOCK_WAIT_BLOCK)(OldValue.Value & ~EX_PUSH_LOCK_PTR_BITS);
                NewValue.Ptr = (PVOID)(((ULONG_PTR)&WaitBlock) | (OldValue.Value & (EX_PUSH_LOCK_LOCK | EX_PUSH_LOCK_MULTIPLE_SHARED)) | EX_PUSH_LOCK_WAITING | EX_PUSH_LOCK_WAKING);
                if (!OldValue.Waking) {
                    Optimize = TRUE;
                }
            } else {
                WaitBlock.Last = &WaitBlock;
                NewValue.Ptr = (PVOID)(((ULONG_PTR)&WaitBlock) | (OldValue.Value & EX_PUSH_LOCK_PTR_BITS) | EX_PUSH_LOCK_WAITING);
            }
...
NTKERNELAPI VOID DECLSPEC_NOINLINE FASTCALL ExfReleasePushLockShared(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
...
*/
{
    ...
    while (1) {
        if (OldValue.Waking) {
            ...
            NewValue.Value = (OldValue.Value & ~(EX_PUSH_LOCK_LOCK | EX_PUSH_LOCK_MULTIPLE_SHARED)) | EX_PUSH_LOCK_WAKING;
            ASSERT(NewValue.Waking && !NewValue.Locked && !NewValue.MultipleShared);
            TopValue = NewValue;
            if ((NewValue.Ptr = InterlockedCompareExchangePointer(&PushLock->Ptr, NewValue.Ptr, OldValue.Ptr)) == OldValue.Ptr) {
...
NTKERNELAPI VOID DECLSPEC_NOINLINE FASTCALL ExfReleasePushLockExclusive(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
...
NTKERNELAPI VOID DECLSPEC_NOINLINE FASTCALL ExfReleasePushLock(__inout PEX_PUSH_LOCK PushLock)
/*
Routine Description:
...
*/
{
    ...
    while (1) {
        ...
        ASSERT(OldValue.Waiting || OldValue.Shared == 0);
        if (!OldValue.Waiting) {
            NewValue.Ptr = InterlockedCompareExchangePointer(&PushLock->Ptr, (PVOID)(EX_PUSH_LOCK_LOCK | EX_PUSH_LOCK_SHARE_INC), OldValue.Ptr);
            if (NewValue.Ptr == OldValue.Ptr) {
                return;
            }
...
*/
{
    ...
    WaitBlock->Flags = EX_PUSH_LOCK_FLAGS_SPINNING;// Mark the wait block as not waiting yet
    OldValue = ReadForWriteAccess(PushLock);// Push the wait block on the list.
    while (1) {
...
NTKERNELAPI NTSTATUS FASTCALL ExTimedWaitForUnblockPushLock(__inout PEX_PUSH_LOCK PushLock, __inout PEX_PUSH_LOCK_WAIT_BLOCK WaitBlock, __in_opt PLARGE_INTEGER Timeout)
{
    ...
    if (InterlockedBitTestAndReset(&WaitBlock->Flags, EX_PUSH_LOCK_FLAGS_SPINNING_V)) {
        Status = KeWaitForSingleObject(&WaitBlock->WakeEvent, WrPushLock, KernelMode, FALSE, Timeout);
...
*/
{
    ...
    // If it looks like we will wake more than one thread then raise IRQL to prevent rescheduling during the operation.
    OldIrql = DISPATCH_LEVEL;
...
*/
{
    ...
    if (PushLockCacheAware != NULL) {
        ...
        if (KeNumberNodes == 1) {
            PaddedPushLock = ExAllocatePoolWithTag(PagedPool, sizeof(EX_PUSH_LOCK_CACHE_AWARE_PADDED)* EX_PUSH_LOCK_FANNED_COUNT, 'lclP');
            ...
            // Allocate a different block for each lock and set affinity so the allocation comes from that node's memory.
...
NTKERNELAPI VOID ExAcquireCacheAwarePushLockExclusive(__inout PEX_PUSH_LOCK_CACHE_AWARE PushLock)
/*
Routine Description:
...
*/
{
    ...
    // Take the first slot exclusive and then we can take the rest of the slots in any order we want.
...
NTKERNELAPI VOID ExReleaseCacheAwarePushLockExclusive(__inout PEX_PUSH_LOCK_CACHE_AWARE PushLock)
/*
Routine Description:
