...
Abstract:
   This module implements the executive event object.
   Functions are provided to create, open, set, reset, pulse, and query event objects.
*/

#include "exp.h"

ULONG ExpEventBoost = EVENT_INCREMENT;// Temporary so boost is patchable
POBJECT_TYPE ExEventObjectType;// Address of event object type descriptor.

// Structure that describes the mapping of generic access rights to object specific access rights for event objects.
...
*/
{
    ...
    RtlInitUnicodeString(&TypeName, L"Event");// Initialize string descriptor.
    ...
    Status = ObCreateObjectType(&TypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &ExEventObjectType);// Create event object type descriptor.
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(EventHandle, EVENT_MODIFY_STATE, ExEventObjectType, KeGetPreviousMode(), &Event, NULL);// Reference event object by handle.
    if (NT_SUCCESS(Status)) {// If the reference was successful, then set the state of the event object to Not-Signaled and dereference event object.
...
*/
{
    ...
    Status = ObCreateObject(PreviousMode, ExEventObjectType, ObjectAttributes, PreviousMode, NULL, sizeof(KEVENT), 0, 0, &Event);
    if (NT_SUCCESS(Status)) {// If the event object was successfully allocated, then initialize the event object and attempt to insert the event object in the current process' handle table.
        KeInitializeEvent((PKEVENT)Event, EventType, InitialState);
        Status = ObInsertObject(Event, NULL, DesiredAccess, 0, NULL, &Handle);

        // If the event object was successfully inserted in the current process' handle table, then attempt to write the event object handle value.
...
*/
{
    ...
    // If the open was successful, then attempt to write the event object handle value.
    // If the write attempt fails, then do not report an error.
    // When the caller attempts to access the handle value, an access violation will occur.
    if (NT_SUCCESS(Status)) {
        if (PreviousMode != KernelMode) {
...
*/
{
    ...
    // If the reference was successful, then pulse the event object, dereference event object, and write the previous state value if specified.
...
*/
{
    ...
    // Reference event object by handle.
    Status = ObReferenceObjectByHandle(EventHandle, EVENT_QUERY_STATE, ExEventObjectType, PreviousMode, &Event, NULL);

    // If the reference was successful, then read the current state of the event object, 
    // deference event object, fill in the information structure, and return the length of the information structure if specified.
    // If the write of the event information or the return length fails, then do not report an error.
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(EventHandle, EVENT_MODIFY_STATE, ExEventObjectType, PreviousMode, &Event, NULL);// Reference event object by handle.

    // If the reference was successful, then set the state of the event object to Not-Signaled, dereference event object, and write the previous state value if specified.
    // If the write of the previous state fails, then do not report an error.
    // When the caller attempts to access the previous state value, an access violation will occur.
    if (NT_SUCCESS(Status)) {
        PERFINFO_DECLARE_OBJECT(Event);
...
*/
{
    ...
    // If the reference was successful, then set the event object to the Signaled state, dereference event object, and write the previous state value if specified.
    // If the write of the previous state fails, then do not report an error.
    // When the caller attempts to access the previous state value, an access violation will occur.
    if (NT_SUCCESS(Status)) {
        PERFINFO_DECLARE_OBJECT(Event);
...
*/
{
    ...
    // If the event object is a notification event, then return an object type mismatch status.
    // Otherwise, set the specified event and boost the unwaited thread priority as appropriate.
    if (NT_SUCCESS(Status)) {
