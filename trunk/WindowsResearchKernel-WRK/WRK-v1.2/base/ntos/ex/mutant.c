...
Abstract:
   This module implements the executive mutant object. Functions are provided to create, open, release, and query mutant objects.
*/

#include "exp.h"

POBJECT_TYPE ExMutantObjectType;// Address of mutant object type descriptor.

// Structure that describes the mapping of generic access rights to object specific access rights for mutant objects.
...
/*
Routine Description:
    This function is called when an executive mutant object is about to be deleted.
    The mutant object is released with an abandoned status to ensure that it is removed from the owner thread's mutant list if the mutant object is currently owned by a thread.
Arguments:
    Mutant - Supplies a pointer to an executive mutant object.
*/
{
    // Release the mutant object with an abandoned status to ensure that it is removed from the owner thread's mutant list if the mutant is currently owned by a thread.
...
/*
Routine Description:
    This function creates the mutant object type descriptor at system initialization and stores the address of the object type descriptor in local static storage.
Return Value:
    A value of TRUE is returned if the mutant object type descriptor is successfully created. Otherwise a value of FALSE is returned.
*/
{
    ...
    RtlInitUnicodeString(&TypeName, L"Mutant");// Initialize string descriptor.
    ...
    // If the mutant object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
...
/*
Routine Description:
    This function creates a mutant object, sets its initial count to one (signaled), and opens a handle to the object with the specified desired access.
Arguments:
    MutantHandle - Supplies a pointer to a variable that will receive the mutant object handle.
    DesiredAccess - Supplies the desired types of access for the mutant object.
    ObjectAttributes - Supplies a pointer to an object attributes structure.
    InitialOwner - Supplies a boolean value that determines whether the creator of the object desires immediate ownership of the object.
*/
{
    HANDLE Handle;
    PVOID Mutant;
    ...
    // Establish an exception handler, probe the output handle address, and attempt to create a mutant object.
    // If the probe fails, then return the exception code as the service status. Otherwise return the status value returned by the object insertion routine.
    try {
        ...
        Status = ObCreateObject(PreviousMode, ExMutantObjectType, ObjectAttributes, PreviousMode, NULL, sizeof(KMUTANT), 0, 0, &Mutant);// Allocate mutant object.
        if (NT_SUCCESS(Status)) {// If the mutant object was successfully allocated, then initialize the mutant object and attempt to insert the mutant object in the current process' handle table.
...
/*
Routine Description:
    This function opens a handle to a mutant object with the specified desired access.
Arguments:
    MutantHandle - Supplies a pointer to a variable that will receive the mutant object handle.
    DesiredAccess - Supplies the desired types of access for the mutant object.
    ObjectAttributes - Supplies a pointer to an object attributes structure.
*/
{
    ...
    // Establish an exception handler, probe the output handle address, and attempt to open the mutant object.
...
/*
Routine Description:
    This function queries the state of a mutant object and returns the requested information in the specified record structure.
Arguments:
    MutantHandle - Supplies a handle to a mutant object.
    MutantInformationClass - Supplies the class of information being requested.
    MutantInformation - Supplies a pointer to a record that is to receive the requested information.
    MutantInformationLength - Supplies the length of the record that is to receive the requested information.
...
*/
{
    ...
    try {
        ...
        Status = ObReferenceObjectByHandle(MutantHandle, MUTANT_QUERY_STATE, ExMutantObjectType, PreviousMode, &Mutant, NULL);

        // If the reference was successful, then read the current state and abandoned status of the mutant object, dereference mutant object,
        // fill in the information structure, and return the length of the information structure if specified.
        // If the write of the mutant information or the return length fails, then do not report an error.
    ...
    {// If an exception occurs during the probe of the output arguments, then always handle the exception and return the exception code as the status value.
        return GetExceptionCode();
    }
...
Arguments:
    ...
    PreviousCount - Supplies an optional pointer to a variable that will receive the previous mutant count.
*/
{
    LONG Count;
    PVOID Mutant;
    ...
    // Establish an exception handler, probe the previous count address if specified, reference the mutant object, and release the mutant object.
    // If the probe fails, then return the exception code as the service status.
    // Otherwise return the status value returned by the reference object by handle routine.
    try {
        ...
        // Note that the desired access is specified as zero since only the owner can release a mutant object.
        Status = ObReferenceObjectByHandle(MutantHandle, 0, ExMutantObjectType, PreviousMode, &Mutant, NULL);

        // If the reference was successful, then release the mutant object.
        // If an exception occurs because the caller is not the owner of the mutant object, 
        // then dereference mutant object and return the exception code as the service status.
        // Otherise write the previous count value if specified.
        // If the write of the previous count fails, then do not report an error.
        // When the caller attempts to access the previous count value, an access violation will occur.
        if (NT_SUCCESS(Status)) {
            try {
    ...
    {// If an exception occurs during the probe of the previous count, then always handle the exception and return the exception code as the status value.
        return GetExceptionCode();
    }
