...
extern POBJECT_TYPE ExEventObjectType;// For referencing a user-supplied event handle
...
LIST_ENTRY  ExpFirmwareTableProviderListHead;// Firmware Table handler list
ERESOURCE  ExpFirmwareTableResource;// Firmware Table ERESOURCE
...
NTSTATUS ExpGetHandleInformationEx(OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT PULONG Length);
...
/*
Routine Description:
    This function reads the status of the 64-bit COM+ package from the registry and stick it inside the shared page.
*/
{
    NTSTATUS Status;
...
Routine Description:
    ...
    The package status indicates whether the 64-bit or the 32-bit runtime should be used when executing IL_ONLY COM+ images.
Arguments:
...
NTSTATUS NtSetDefaultUILanguage(__in LANGID DefaultUILanguageId)
{
    ...
    if (DefaultUILanguageId == 0) {//  if this is called during user logon, then we need to update the user's registry.
...
NTSTATUS ExpValidateLocale(IN LCID LocaleId)
{
    ...
    *Ptr = UNICODE_NULL;
    ...
    ZwClose(LangGroupKey);// Close opened keys
...
Failed1:
    if (!NT_SUCCESS(ReturnStatus)) {
        Status = ReturnStatus;// If an error happens, let's record it.
    }
...
NTSTATUS ExpQueryNumaProcessorMap(OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT PULONG ReturnedLength)
{
    ...
    // Return as many node masks as possible in the SystemInformation buffer.
...
NTSTATUS ExpQueryNumaAvailableMemory(OUT PVOID  SystemInformation, IN  ULONG  SystemInformationLength, OUT PULONG ReturnedLength)
{
    ...
    *ReturnedLength = FIELD_OFFSET(SYSTEM_NUMA_INFORMATION, AvailableMemory[ReturnCount]);
    ...
    // (It's approximate because no lock is taken and with respect to any user mode application its only a sample.
#if !defined(NT_UP)
    if (KeNumberNodes > 1) {
        ULONG i;

        for (i = 0; i < ReturnCount; i++) {
            Map->AvailableMemory[i] = ((ULONGLONG)KeNodeBlock[i]->FreeCount[ZeroedPageList] + (ULONGLONG)KeNodeBlock[i]->FreeCount[FreePageList]) << PAGE_SHIFT;
        }
    } else
#endif
...
NTSTATUS ExpGetSystemBasicInformation(OUT PSYSTEM_BASIC_INFORMATION BasicInfo)
{
...
NTSTATUS ExpGetSystemProcessorInformation(OUT PSYSTEM_PROCESSOR_INFORMATION ProcessorInformation)
{
    NTSTATUS NtStatus = STATUS_SUCCESS;
...
#if defined(_WIN64)
NTSTATUS ExpGetSystemEmulationBasicInformation(OUT PSYSTEM_BASIC_INFORMATION BasicInfo)
{
    NTSTATUS NtStatus = STATUS_SUCCESS;
    ...
    SystemInformation - A pointer to a buffer which receives the specified information.  The format and content of the buffer depend on the specified system information class.
        ...
        SystemDockInformation - Data type is SYSTEM_DOCK_INFORMATION
             ...
             SYSTEM_DOCK_INFORMATION Structure
                 ...
                 ULONG DockIdLength - Specifies the length in characters of the Dock ID string (not including terminating NULL).
...
*/
{
    ...
    Status = STATUS_SUCCESS; // Assume successful completion.
    try {
        // Get previous processor mode and probe output argument if necessary.
        PreviousMode = KeGetPreviousMode();
        if (PreviousMode != KernelMode) {
        ...
        case SystemPerformanceInformation:
            ...
            // some of these counters are kept on a per processor basis and must be totaled.
            ...
            // Idle process information.
            {
                ...
                TotalKernel = KeQueryRuntimeProcess(&PsIdleProcess->Pcb, &TotalUser);
                LocalPerformanceInfo.IdleProcessTime.QuadPart = UInt32x32To64(TotalKernel, KeMaximumIncrement);
            }
        ...
        case SystemProcessorPowerInformation:
            ...
            Status = ExpGetProcessorPowerInformation(SystemInformation, SystemInformationLength, &Length);
            if (ARGUMENT_PRESENT(ReturnLength)) {
                *ReturnLength = Length;
            }
            break;
        case SystemProcessorIdleInformation:
            ...
            Status = ExpGetProcessorIdleInformation(SystemInformation, SystemInformationLength, &Length);
            if (ARGUMENT_PRESENT(ReturnLength)) {
                *ReturnLength = Length;
            }
            break;
        case SystemTimeOfDayInformation:
            ...
            break;
        case SystemTimeAdjustmentInformation:// Query system time adjustment information.
        ...
        case SystemExtendedProcessInformation:
        {
            ...
            Status = ExpGetProcessInformation(SystemInformation, SystemInformationLength, ReturnLength, NULL, ExtendedInformation);
        }
        ...
        case SystemSessionProcessInformation:
            ...
            Status = ExpGetProcessInformation(ProcessInformation, ProcessInformationLength, ReturnLength, &SessionId, FALSE);
            break;
        ...
        case SystemFileCacheInformationEx:
        {
            ...
            if (SystemInformationLength >= sizeof(SYSTEM_FILECACHE_INFORMATION)) {
                ...
                FileCache->MinimumWorkingSet = CapturedFileCacheInformation.MinimumWorkingSet;
                FileCache->MaximumWorkingSet = CapturedFileCacheInformation.MaximumWorkingSet;
                FileCache->CurrentSizeIncludingTransitionInPages = CapturedFileCacheInformation.CurrentSizeIncludingTransitionInPages;
                FileCache->PeakSizeIncludingTransitionInPages = CapturedFileCacheInformation.PeakSizeIncludingTransitionInPages;
                FileCache->TransitionRePurposeCount = CapturedFileCacheInformation.TransitionRePurposeCount;
        ...
        case SystemPoolTagInformation:
            ...
            Status = ExGetPoolTagInfo(SystemInformation, SystemInformationLength, ReturnLength);
            break;
        case SystemBigPoolInformation:
            ...
            Status = ExGetBigPoolInfo(SystemInformation, SystemInformationLength, ReturnLength);
            break;
        case SystemSessionMappedViewInformation:
            SessionMappedViewInformation = (PSYSTEM_SESSION_MAPPED_VIEW_INFORMATION)SystemInformation;
            ...
            Status = MmGetSessionMappedViewInformation(SessionMappedViewInformation, SystemInformationLength, &Length, &SessionId);
            if (ARGUMENT_PRESENT(ReturnLength)) {
                *ReturnLength = Length;
            }
            break;
        case SystemVdmInstemulInformation:
            ...
            // Get system exception information which includes the number of exceptions that have dispatched, the number of alignment fixups,
            // and the number of floating emulations that have been performed.
        ...
        case SystemRecommendedSharedDataAlignment:
            ...
            // Alignment is guaranteed by the ProbeForWrite above so just store the value as a ULONG.
        ...
        case SystemComPlusPackage:
            ...
            if (SharedUserData->ComPlusPackage == COMPLUS_PACKAGE_INVALID) {
                SharedUserData->ComPlusPackage = 0;// The initialization happens one time.
        ...
        case SystemLogicalProcessorInformation:
            Status = KeQueryLogicalProcessorInformation(SystemInformation, SystemInformationLength, &Length);
            if (ARGUMENT_PRESENT(ReturnLength)) {
                *ReturnLength = Length;
            }
            break;
        case SystemFirmwareTableInformation:
            ...
            Status = ExpGetSystemFirmwareTableInformation(SystemInformation, PreviousMode, ReturnLength);
            break;
        default:
            return STATUS_INVALID_INFO_CLASS;// Invalid argument.
        }
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
        Status = GetExceptionCode();
    ...
    */
{
    ...
    try {
        ...
        switch (SystemInformationClass) // Dispatch on the system information class.
        {
        case SystemFlagsInformation:
            if (SystemInformationLength != sizeof(SYSTEM_FLAGS_INFORMATION)) {
                return STATUS_INFO_LENGTH_MISMATCH;
            ...
            if (!SeSinglePrivilegeCheck(SeDebugPrivilege, PreviousMode)) {
                ...
                Flags = ((PSYSTEM_FLAGS_INFORMATION)SystemInformation)->Flags & ~(FLG_KERNELMODE_VALID_BITS | FLG_BOOTONLY_VALID_BITS);
                Flags |= NtGlobalFlag & (FLG_KERNELMODE_VALID_BITS | FLG_BOOTONLY_VALID_BITS);
                NtGlobalFlag = Flags;
                ((PSYSTEM_FLAGS_INFORMATION)SystemInformation)->Flags = NtGlobalFlag;
            }
            break;
        case SystemTimeAdjustmentInformation:// Set system time adjustment information.// N.B. The caller must have the SeSystemTime privilege.
            if (SystemInformationLength != sizeof(SYSTEM_SET_TIME_ADJUST_INFORMATION)) {// If the system information buffer is not the correct length, then return an error.
                return STATUS_INFO_LENGTH_MISMATCH;
            }
        ...
        case SystemRegistryQuotaInformation:
            ...
            CmSetRegistryQuotaInformation((PSYSTEM_REGISTRY_QUOTA_INFORMATION)SystemInformation);// Set registry quota parameters.
            break;
        ...
        // N.B. If this function is called from user mode it can only be used to load the win32k subsystem.
        //      The calling process must be smss, the subsystem to be loaded must be win32k, and the caller must have the privilege to load drivers.
        //      If this function is called from kernel mode it can be used to load any driver.
        case SystemExtendServiceTableInformation:
        {
            ...
            // Check if the system information buffer is the correct length.
            if (SystemInformationLength != sizeof(UNICODE_STRING)) {
                return STATUS_INFO_LENGTH_MISMATCH;
            }

            // If the previous mode is not kernel, then verify that the caller can load the win32k subsystem.
            if (PreviousMode != KernelMode) {
                ...
                try {
                    ProbeAndReadUnicodeStringEx(&Image, (PUNICODE_STRING)SystemInformation);// Probe and read the unicode string descriptor.
                    ...
                    // Probe the unicode string buffer and check the path name.
                    ProbeForReadSmallStructure(Image.Buffer, WIN32K_PATH_SIZE, sizeof(UCHAR));
                    if (memcmp(Image.Buffer, &Win32kFullPath[0], WIN32K_PATH_SIZE) != 0) {
                        return STATUS_PRIVILEGE_NOT_HELD;
                    }

                    // Initialize string descriptor for win32k full path name.
            ...
            NtHeaders = RtlImageNtHeader(ImageBaseAddress);// Get the image base address.
            if (NtHeaders == NULL) {
                MmUnloadSystemImage(SectionPointer);
                return STATUS_INVALID_IMAGE_FORMAT;
            }

            // Extract the initialization routine address and call the driver initialization routine.
            ...
            // If the image initialization was unsuccessful, then unload the system image.
            // Otherwise, set the system image unload address so the session can be unloaded cleanly.
            if (NT_SUCCESS(Status) == FALSE) {
                MmUnloadSystemImage(SectionPointer);
            } else {
        ...
        case SystemFileCacheInformationEx:
        {
            ...
            return MmAdjustWorkingSetSizeEx(((PSYSTEM_FILECACHE_INFORMATION)SystemInformation)->MinimumWorkingSet,
                ((PSYSTEM_FILECACHE_INFORMATION)SystemInformation)->MaximumWorkingSet, TRUE, TRUE, Flags, &IncreasePerformed);
            break;
        }
        case SystemDpcBehaviorInformation:
        {
            ...
            DpcInfo = *(PSYSTEM_DPC_BEHAVIOR_INFORMATION)SystemInformation;// Exception handler for this routine will return the correct error if this access fails.
        ...
        case SystemSessionDetach:
        {
            ...
            Status = MmSessionDelete(SessionId);// Detach the current process from a session space if it has one.
            return Status;
        }
        break;
        case SystemCrashDumpStateInformation:
            // All this system information does when you set it is trigger a reconfigurating of the current crashdump state based on the registry.
        ...
        case SystemVerifierRemoveDriverInformation:
        {
            ...
            Buffer = NULL;
            if (PreviousMode != KernelMode) {
                // The caller's access mode is not kernel so check to ensure the caller has the privilege to add a verifier entry.
                if (!SeSinglePrivilegeCheck(SeDebugPrivilege, PreviousMode)) {
                    return STATUS_PRIVILEGE_NOT_HELD;
                }
        ...
        case SystemMirrorMemoryInformation:
            ...
            break;
        case SystemPrefetcherInformation:
            Status = CcPfSetPrefetcherInformation(SystemInformationClass, SystemInformation, SystemInformationLength, PreviousMode);
            break;
        case SystemComPlusPackage:
            if (SystemInformationLength != sizeof(ULONG)) {
                return STATUS_INFO_LENGTH_MISMATCH;
        ...
        case SystemHotpatchInformation:
            Status = ExApplyCodePatch(SystemInformation, SystemInformationLength);
            break;
        case SystemWow64SharedInformation:
            ...
            if (PreviousMode != KernelMode) {
                ...
                try {
                    ProbeForRead(SystemInformation, SystemInformationLength, sizeof(UCHAR));
                    if (SharedUserData->Wow64SharedInformation[0] == 0) {
                        RtlCopyMemory(&SharedUserData->Wow64SharedInformation[0], SystemInformation, SystemInformationLength);
                        KeUserPopEntrySListEndWow64 = UlongToPtr(SharedUserData->Wow64SharedInformation[SharedNtdll32ExpInterlockedPopEntrySListEnd]);
                        KeUserPopEntrySListFaultWow64 = UlongToPtr(SharedUserData->Wow64SharedInformation[SharedNtdll32ExpInterlockedPopEntrySListFault]);
                        KeUserPopEntrySListResumeWow64 = UlongToPtr(SharedUserData->Wow64SharedInformation[SharedNtdll32ExpInterlockedPopEntrySListResume]);
                    }
                } except(EXCEPTION_EXECUTE_HANDLER)
                {
                    return GetExceptionCode();
        ...
        case SystemRegisterFirmwareTableInformationHandler:
            Status = ExpRegisterFirmwareTableInformationHandler(SystemInformation, SystemInformationLength, PreviousMode);
            break;
        default:
            Status = STATUS_INVALID_INFO_CLASS;
            break;
...
/*
Routine Description:
    Wrapper for MmProbeAndLockPages.  Creates an MDL and locks the specified buffer with that MDL.
Arguments:
    ...
    LockedBuffer - returns a pointer to the locked buffer for use by the caller.
    LockVariable - returns a context pointer.  This must be passed into ExUnlockUserBuffer when complete so the MDL can be freed.
Return Value:
    Returns one of the following status codes:
        STATUS_SUCCESS - Normal, successful completion.
        STATUS_ACCESS_VIOLATION - The buffer is not accessible with the specified LockMode.
...
*/
{
    ...
    *LockVariable = NULL;
    ...
    Mdl = ExAllocatePoolWithQuotaTag(NonPagedPool | POOL_QUOTA_FAIL_INSTEAD_OF_RAISE, MdlSize, 'ofnI');
    if (Mdl == NULL) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    MmInitializeMdl(Mdl, Buffer, Length);// Initialize MDL for request.
...
VOID ExUnlockUserBuffer(__inout PVOID LockVariable)
{
...
*/
{
    ...
    try {
        ...
        for (Process = PsIdleProcess; Process != NULL; Process = PsGetNextProcess((Process == PsIdleProcess) ? NULL : Process)) {
            ...
            // Raise IRQL to SYNCH_LEVEL, acquire the kernel process lock, and get information for each thread.
            ...
            while (NextThread != &Process->Pcb.ThreadListHead) {
                ...
                if (TotalSize > SystemInformationLength) {
                    ...
                    // Lock dispatcher database to get atomic view of thread attributes.
            ...
            KeReleaseInStackQueuedSpinLock(&LockHandle);// Unlock kernel process lock and lower IRQL to its previous value.
            if (Process != PsIdleProcess) {// Get the image name.
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    ProcessInfo->ImageName.Buffer = (PWSTR)((PCHAR)SystemInformation + ((PCHAR)(ThreadInfo)-(PCHAR)MappedAddress));// Set the image name to point into the user's memory.
                }
            }

            if (NT_SUCCESS(status)) {
                ProcessInfo->NextEntryOffset = NextEntryOffset;// Point to next process.
            }
        }
...
NTSTATUS ExpGetProcessorPowerInformation(OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT PULONG Length)
{
    ...
    CallerPowerInfo = (PSYSTEM_PROCESSOR_POWER_INFORMATION)SystemInformation;// We will walk this pointer to store the user data...
    ...
    *Length = sizeof(SYSTEM_PROCESSOR_POWER_INFORMATION) * KeNumberProcessors;
    ...
    MmLockPageableSectionByHandle(ExPageLockHandle);// Lock everything down
    ...
    while (processors) {
        ...
        KeRaiseIrql(DISPATCH_LEVEL, &oldIrql);// Raise to DPC level to synchronize access to the data structures
        ...
        ProcessorPowerInfo.CurrentFrequencyTime = UInt32x32To64((Prcb->KernelTime + Prcb->UserTime - PState->PerfTickCount), KeMaximumIncrement);
        ...
        ProcessorPowerInfo.LastProcessorTime = UInt32x32To64(PState->LastKernelUserTime, KeMaximumIncrement);
        ...
        KeLowerIrql(oldIrql);// Return to the original level (should be IRQL 0)
        ...
        CallerPowerInfo++;// Point to the next structure element
    }

    Status = STATUS_SUCCESS;

exit_revert_and_unlock:
    KeRevertToUserAffinityThread();// Revert to the original affinity
    MmUnlockPageableImageSection(ExPageLockHandle);// Unlock everything
    return Status;
}
...
NTSTATUS ExpGetProcessorIdleInformation(OUT PVOID SystemInformation, IN  ULONG SystemInformationLength, OUT PULONG Length)
{
    ...
    *Length = sizeof(SYSTEM_PROCESSOR_IDLE_INFORMATION) * KeNumberProcessors;
    ...
    KeQueryPerformanceCounter(&PerfFrequency);// We need to know what frequency the perf counters are running at
    MmLockPageableSectionByHandle(ExPageLockHandle);// Lock everything down
    ...
    while (processors) {
        ...
        ProcessorIdleInfo.IdleTime = UInt32x32To64(Prcb->IdleThread->KernelTime, KeMaximumIncrement);// Grab the data that we care about
        ...
        CallerIdleInfo++;// Point to the next structure element
    }
...
*/
{
    ...
    if (Limit != 0) {
        Status = ExLockUserBuffer(Buffer, BufferLength, KeGetPreviousMode(), IoWriteAccess, &Lookaside, &BufferLock);
        if (NT_SUCCESS(Status)) {
            Status = STATUS_SUCCESS;
            ...
            // Copy nonpaged and paged system lookaside information to information buffer.
        ...
        Finish2:
            ExUnlockUserBuffer(BufferLock);// Unlock user buffer.
        }
    }
...
BOOLEAN ExIsProcessorFeaturePresent(__in ULONG ProcessorFeature)
{
    BOOLEAN rv;
...
VOID ExGetCurrentProcessorCpuUsage(__out PULONG CpuUsage)
/*
Routine Description:
...
*/
{
    ...
    Prcb = KeGetCurrentPrcb();
    *CpuUsage = 100 - (ULONG)(UInt32x32To64(Prcb->IdleThread->KernelTime, 100) / (ULONGLONG)(Prcb->KernelTime + Prcb->UserTime));
}


VOID ExGetCurrentProcessorCounts(__out PULONG IdleCount, __out PULONG KernelAndUser, __out PULONG Index)
/*
Routine Description:
    Returns information regarding idle time and kernel + user time for the current processor.
Arguments:
    IdleCount - Returns the kernel time of the idle thread on the current processor.
...
*/
{
    ...
    // pLangId is passed in as DWORD or WORD hex string LangId string in MUILanguages is set as WORD hex string
    while (pLangId[iLen]) {
        iLen++;
    }
...
Parameters:
    SystemFirmwareTableInformation  - points to the SYSTEM_FIRMWARE_TABLE_INFORMATION structure.
...
*/
{
    ...
    // Point the incoming SystemFirmwareTableInformation to TableInfo.
    // If this is a usermode call, we will copy all the usermode data into local buffers before calling the table provider.
    TableInfo = (PSYSTEM_FIRMWARE_TABLE_INFORMATION)SystemFirmwareTableInformation;
    if (PreviousMode != KernelMode) {
        try {
            ProbeForRead(&(TableInfo->TableBufferLength), sizeof(ULONG), sizeof(ULONG));

            // Save a copy of the TableBufferLength as this value may be changed by the table providers handler.
        ...
        // Allocate a local buffer. This way we always pass trusted memory to the firmware table providers.
        TableInfoLocal = ExAllocatePoolWithQuotaTag(PagedPool, TotalStructSize, 'TFRA');
        if (!TableInfoLocal) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        try {
            ...
            ProbeForRead(TableInfo, sizeof(SYSTEM_FIRMWARE_TABLE_INFORMATION), sizeof(ULONG));
            RtlCopyMemory(TableInfoLocal, TableInfo, sizeof(SYSTEM_FIRMWARE_TABLE_INFORMATION));
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            return GetExceptionCode();
        }

        // Now save a pointer to our copy of TableInfoLocal into Tableinfo.
        // This will improve code readability because all the shared code between user mode and kernel mode will use the same variable name.
    ...
    EX_FOR_EACH_IN_LIST(SYSTEM_FIRMWARE_TABLE_HANDLER_NODE, FirmwareTableProviderList, &ExpFirmwareTableProviderListHead, HandlerCurrent)
    {
        if (HandlerCurrent->SystemFWHandler.ProviderSignature == TableInfo->ProviderSignature) {
            ...
            if (PreviousMode != KernelMode) {
                ...
                try {
                    // Point Tableinfo back to the original usermode buffer. Our new data is safe in TableInfoLocal.
                    TableInfo = (PSYSTEM_FIRMWARE_TABLE_INFORMATION)SystemFirmwareTableInformation;
                    if (NT_SUCCESS(Status)) {
                        // The usermode buffer (stored in TableInfo) in untrusted.
                        // It's content could have changed. Use the cached value in BufferLength to do the probe and make sure the buffer is still valid.
                        ProbeForWrite(TableInfo->TableBuffer, BufferLength, sizeof(UCHAR));// Probe the Table buffer for writing.
                        RtlCopyMemory(TableInfo->TableBuffer, TableInfoLocal->TableBuffer, TableInfoLocal->TableBufferLength);// Copy over the returned buffer to the user mode buffer.
                    }

                    // Always update the returned buffer length.
                    ProbeForWrite(&(TableInfo->TableBufferLength), sizeof(ULONG), sizeof(ULONG));// Probe the structure for writing.
                    RtlCopyMemory(&(TableInfo->TableBufferLength), &(TableInfoLocal->TableBufferLength), sizeof(ULONG));// Copy over the number of bytes written or required.
                    TableInfo = TableInfoLocal;
                    if (ReturnLength) {// Fill in ReturnLength.
                        ProbeForWrite(ReturnLength, sizeof(ULONG), sizeof(ULONG));
                        *ReturnLength = TableInfo->TableBufferLength + sizeof(SYSTEM_FIRMWARE_TABLE_INFORMATION);
                    }
                } except(EXCEPTION_EXECUTE_HANDLER)
                {
                    Status = GetExceptionCode();
                ...
                if (ReturnLength) {
                    *ReturnLength = TableInfo->TableBufferLength + sizeof(SYSTEM_FIRMWARE_TABLE_INFORMATION);// Fill in ReturnLength.
                }
            }
...
*/
{
    ...
    if (PreviousMode != KernelMode) {
        ...
        if ((!SystemInformation) || (SystemInformationLength < sizeof(SYSTEM_FIRMWARE_TABLE_HANDLER))) {
            Status = STATUS_INFO_LENGTH_MISMATCH;
        } else {// Grab the resource to prevent state change via reentry.
            KeEnterCriticalRegion();
            ExAcquireResourceExclusiveLite(&ExpFirmwareTableResource, TRUE);
            SystemTableHandler = (PSYSTEM_FIRMWARE_TABLE_HANDLER)SystemInformation;
            EX_FOR_EACH_IN_LIST(SYSTEM_FIRMWARE_TABLE_HANDLER_NODE, FirmwareTableProviderList, &ExpFirmwareTableProviderListHead, HandlerListCurrent)
            {
            ...
            // Handler was not found and this is a register request, so allocate a new node and insert it into the list.
            if ((!HandlerFound) && (SystemTableHandler->Register)) {
                // This is a new Firmware table handler, allocate the space and add it to the list.
                HandlerListNew = ExAllocatePoolWithTag(PagedPool, sizeof(SYSTEM_FIRMWARE_TABLE_HANDLER_NODE), 'TFRA');
                if (HandlerListNew) {
                    ...
                    // Grab a reference to the providers driverobject so that the driver does not get unloaded without our knowledge.
                    // The handler must first be unregistered before unloading.
                    ObReferenceObject((PVOID)HandlerListNew->SystemFWHandler.DriverObject);

                    InsertTailList(&ExpFirmwareTableProviderListHead, &HandlerListNew->FirmwareTableProviderList);// Update the LinkList.
                } else {
                    Status = STATUS_INSUFFICIENT_RESOURCES;
                }
            } else if ((HandlerFound) && (!(SystemTableHandler->Register))) {
                if (HandlerListCurrent->SystemFWHandler.DriverObject == SystemTableHandler->DriverObject) {// Check to make sure that a matching driver object was sent in.
                    RemoveEntryList(&HandlerListCurrent->FirmwareTableProviderList); // Remove the entry from the list.
                    ObDereferenceObject((PVOID)HandlerListCurrent->SystemFWHandler.DriverObject);// Deref the device object.
                    ExFreePoolWithTag(HandlerListCurrent, 'TFRA');// Free the unregistered list element.
                } else {
                    Status = STATUS_INVALID_PARAMETER;
                }
            } else if ((HandlerFound) && (SystemTableHandler->Register)) {
                Status = STATUS_OBJECT_NAME_EXISTS;// A handler for this table has already been registered. return error.
            } else {
                Status = STATUS_INVALID_PARAMETER;
            }
