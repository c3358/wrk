...
FORCEINLINE ULONG POOLTAG_HASH(IN ULONG Key, IN SIZE_T Mask)
/*
Routine Description:
    This function builds a hash table index based on the supplied pool tag.
Arguments:
    Key - Supplies the key value to hash.
    Mask - Supplies the mask value to apply to the resultant key to generate a valid hash table index.
...
VOID ExpInsertPoolTracker(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType);
VOID ExpInsertPoolTrackerExpansion(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType);
VOID ExpRemovePoolTracker(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType);
VOID ExpRemovePoolTrackerExpansion(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType);
LOGICAL ExpAddTagForBigPages(IN PVOID Va, IN ULONG Key, IN ULONG NumberOfPages, IN POOL_TYPE PoolType);
ULONG ExpFindAndRemoveTagBigPages(IN PVOID Va, OUT PULONG BigPages, IN POOL_TYPE PoolType);
...
// Define the number of paged pools. This value may be overridden at boot time.
...
// The pool descriptors for paged pool are dynamically allocated since there can be more than one paged pool.
// There is always one more paged pool descriptor than there are paged pools.
// This descriptor is used when a page allocation is done for a paged pool and is the first descriptor in the paged pool descriptor array.
...
// The pool vector contains an array of pointers to pool descriptors.
// For nonpaged pool this is just a pointer to the nonpaged pool descriptor.
// For paged pool, this is a pointer to an array of pool descriptors.
// The pointer to the paged pool descriptor is duplicated so it can be found easily by the kernel debugger.
...
// We redefine the LIST_ENTRY macros to have each pointer biased by one so any rogue code using these pointers will access violate.
// See \nt\public\sdk\inc\ntrtl.h for the original definition of these macros.
...
#define CHECK_LIST(LIST)                                                    \
    if ((DecodeLink(DecodeLink((LIST)->Flink)->Blink) != (LIST)) || (DecodeLink(DecodeLink((LIST)->Blink)->Flink) != (LIST))) {         \
            KeBugCheckEx (BAD_POOL_HEADER, 3, (ULONG_PTR)LIST, (ULONG_PTR)DecodeLink(DecodeLink((LIST)->Flink)->Blink), (ULONG_PTR)DecodeLink(DecodeLink((LIST)->Blink)->Flink)); \
    }
...
VOID FORCEINLINE ExCheckPoolHeader(IN PPOOL_HEADER Entry)
{
    ...
    if (Entry->PreviousSize != 0) {
        ...
        if ((PreviousEntry->BlockSize != Entry->PreviousSize) || (DECODE_POOL_INDEX(PreviousEntry) != DECODE_POOL_INDEX(Entry))) {
    ...
    if (!PAGE_END(NextEntry)) {
        ...
        if ((NextEntry->PreviousSize != (Entry)->BlockSize) || (DECODE_POOL_INDEX(NextEntry) != DECODE_POOL_INDEX(Entry))) {
...
// and free build that will bugcheck the system if a list is ill-formed, and there is one for the final shipping version that has all the checked disabled.

// The check lookaside list macros also comes in two flavors and is used to verify that the look aside lists are well formed.

// The check pool header macro (two flavors) verifies that the specified pool header matches the preceding and succeeding pool headers.
...
*/
{
    ...
    // Initialize statistics fields, the pool type, the threshold value, and the lock address.
...
Routine Description:
    ...
    This is needed before deleting a pool lookaside list because the entries on the lookaside are already marked as free (by ExFreePoolWithTag)
    and so the normal lookaside deletion macros would hit false double free bugchecks if the list is not empty when the macros are called.
Arguments:
...
*/
{
    ...
    // Remove all pool entries from the specified lookaside structure, mark them as active, then free them.
    ...
    while ((Entry = ExAllocateFromPagedLookasideList(Lookaside)) != NULL) {
        ...
        ExpInsertPoolTracker(PoolHeader->PoolTag, PoolHeader->BlockSize << POOL_BLOCK_SHIFT, Lookaside->L.Type);

        // Set the depth to zero every time as a periodic scan may set it nonzero.
        // This isn't worth interlocking as the list will absolutely deplete regardless in this fashion anyway.
        Lookaside->L.Depth = 0;
        (Lookaside->L.Free)(Entry);
    }
}

// FREE_CHECK_ERESOURCE - If enabled causes each free pool to verify no active ERESOURCEs are in the pool block being freed.

// FREE_CHECK_KTIMER - If enabled causes each free pool to verify no active KTIMERs are in the pool block being freed.

// Checking for resources in pool being freed is expensive as there can
// easily be thousands of resources, so don't do it by default but do leave the capability for individual systems to enable it.
...
/*
Routine Description:
    This procedure initializes a pool descriptor for the specified pool type.  Once initialized, the pool may be used for allocation and deallocation.

    This function should be called once for each base pool type during system initialization.

    Each pool descriptor contains an array of list heads for free blocks.
    Each list head holds blocks which are a multiple of the POOL_BLOCK_SIZE.
    The first element on the list [0] links together free entries of size POOL_BLOCK_SIZE, the second element
     [1] links together entries of POOL_BLOCK_SIZE * 2, the third POOL_BLOCK_SIZE * 3, etc, up to the number of blocks which fit into a page.
Arguments:
...
*/
{
    ...
    if (PoolType == NonPagedPool) {
        ...
        do {
            ...
            PoolTrackTable = MiAllocatePoolPages(NonPagedPool, (PoolTrackTableSize + 1) * sizeof(POOL_TRACKER_TABLE));
            if (PoolTrackTable != NULL) {
                break;
            }

            if (PoolTrackTableSize == 1) {
                KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, NumberOfBytes, (ULONG_PTR)-1, (ULONG_PTR)-1, (ULONG_PTR)-1);
            }
        ...
        do {
            ...
            PoolBigPageTable = MiAllocatePoolPages(NonPagedPool, PoolBigPageTableSize * sizeof(POOL_TRACKER_BIG_PAGES));
        ...
        ExpInsertPoolTracker('looP', ROUND_TO_PAGES(PoolBigPageTableSize * sizeof(POOL_TRACKER_BIG_PAGES)), NonPagedPool);
        if (KeNumberNodes > 1) {
            ...
            for (Index = 0; Index < ExpNumberOfNonPagedPools; Index += 1) {
                ...
                // We'd like to use MmAllocateIndependentPages but can't because we'd need system PTEs to map the pages with and PTEs are not available until nonpaged pool exists.
                // So just use regular pool pages to hold the descriptors and spinlocks and hope they either
                // a) happen to fall onto the right node or b) that these lines live in the local processor cache all the time anyway due to frequent usage.
                Descriptor = (PPOOL_DESCRIPTOR)MiAllocatePoolPages(NonPagedPool, NumberOfBytes);
                if (Descriptor == NULL) {
                    KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, NumberOfBytes, (ULONG_PTR)-1, (ULONG_PTR)-1, (ULONG_PTR)-1);
                }
        ...
        KeInitializeSpinLock(&ExpTaggedPoolLock);// Initialize the spinlocks for nonpaged pool.
        PoolVector[NonPagedPool] = &NonPagedPoolDescriptor;
        ExInitializePoolDescriptor(&NonPagedPoolDescriptor, NonPagedPool, 0, Threshold, NULL);// Initialize the nonpaged pool descriptor.
    } else {
        ...
        // For NUMA systems, allocate both the pool descriptor and the associated lock from the local node for performance (even though it costs a little more memory).
        // For non-NUMA systems, allocate everything together in one chunk to reduce memory consumption as there is no performance cost for doing it this way.
        if (KeNumberNodes > 1) {
            ...
            for (Index = 0; Index < ExpNumberOfPagedPools + 1; Index += 1) {
                ...
                Descriptor = (PPOOL_DESCRIPTOR)MmAllocateIndependentPages(NumberOfBytes, Node);
                if (Descriptor == NULL) {
                    KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, NumberOfBytes, (ULONG_PTR)-1, (ULONG_PTR)-1, (ULONG_PTR)-1);
                }
                ...
                ExInitializePoolDescriptor(Descriptor, PagedPool, Index, Threshold, (PVOID)GuardedMutex);
            }
        } else {
            NumberOfBytes = (ExpNumberOfPagedPools + 1) * (sizeof(KGUARDED_MUTEX) + sizeof(POOL_DESCRIPTOR));
            Descriptor = (PPOOL_DESCRIPTOR)ExAllocatePoolWithTag(NonPagedPool, NumberOfBytes, 'looP');
            if (Descriptor == NULL) {
                KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, NumberOfBytes, (ULONG_PTR)-1, (ULONG_PTR)-1, (ULONG_PTR)-1);
            }
        ...
        ExpInsertPoolTracker('looP', ROUND_TO_PAGES(PoolTrackTableSize * sizeof(POOL_TRACKER_TABLE)), NonPagedPool);

#if defined (NT_UP)
        if (MmNumberOfPhysicalPages < 32 * 1024) {
            LARGE_INTEGER TwoMinutes;

            ExSetPoolFlags(EX_SEPARATE_HOT_PAGES_DURING_BOOT);// Set the flag to disable lookasides and use hot/cold page separation during bootup.

            // Start a timer so the above behavior is disabled once bootup has finished.
...
#endif
        if ((MmNumberOfPhysicalPages >= 127 * 1024) && ((ExpPoolFlags & EX_SPECIAL_POOL_ENABLED) == 0) && (!NT_SUCCESS(MmIsVerifierEnabled(&i)))) {
            ExSetPoolFlags(EX_DELAY_POOL_FREES);
        }

        if ((ExpPoolFlags & EX_SPECIAL_POOL_ENABLED) || (NT_SUCCESS(MmIsVerifierEnabled(&i)))) {
#if DBG
            ...
            ExFreePoolTraces = MiAllocatePoolPages(NonPagedPool, (ExFreePoolMask + 1) * sizeof(EX_FREE_POOL_TRACES));
            if (ExFreePoolTraces != NULL) {
                RtlZeroMemory(ExFreePoolTraces, (ExFreePoolMask + 1) * sizeof(EX_FREE_POOL_TRACES));
            }
        }
    }
}
...
FORCEINLINE VOID ExpInsertPoolTrackerInline(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType)
/*
Routine Description:
    This function inserts a pool tag in the tag table, increments the number of allocates and updates the total allocation size.
Arguments:
...
Environment:
    ...
    so pool may be freely allocated here as needed.
    In expansion table growth, the tagged spinlock is held on entry, but we are guaranteed to find an entry in the builtin table so a recursive acquire cannot occur.
*/
{
    ULONG Hash;
    ULONG Index;
    ...
    // Compute the hash index and search (lock-free) for the pool tag in the builtin table.
    ...
    do {
        ...
        if (TrackTableEntry->Key == 0) {
            if (PoolType & SESSION_POOL_MASK) {
                ...
                // Just fall through to common checks starting at this slot for both cases.
                continue;
            }
            ...
            if (Hash != PoolTrackTableSize - 1) {
                // New entries cannot be created with an interlocked compare exchange because any new entry must reside at the same index in each processor's private PoolTrackTable.
                // This is to make ExGetPoolTagInfo statistics gathering much simpler (faster).
    ...
    ExpInsertPoolTrackerExpansion(Key, NumberOfBytes, PoolType);// Use the expansion table instead.
}


FORCEINLINE VOID ExpRemovePoolTrackerInline(IN ULONG Key, IN SIZE_T NumberOfBytes, IN POOL_TYPE PoolType)
/*
Routine Description:
    This function increments the number of frees and updates the total allocation size.
Arguments:
    Key - Supplies the key value used to locate a matching entry in the tag table.
    NumberOfBytes - Supplies the allocation size.
    PoolType - Supplies the pool type.
*/
{
    ...
    do {
        ...
        // here where this processor's table still has no entry for the tag being freed because the allocation happened on a different processor.
        if (TrackTableEntry->Key == 0) {
#if !defined (NT_UP)
            if (((PoolType & SESSION_POOL_MASK) == 0) && (PoolTrackTable[Hash].Key != 0)) {
                TrackTableEntry->Key = PoolTrackTable[Hash].Key;
                continue;
            }
#endif
    ...
    // Linear search through the expansion table.
    // This is ok because the existence of an expansion table at all is extremely rare.
...
PVOID ExAllocatePoolWithTag(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes, __in ULONG Tag)
/*
Routine Description:
    ...
    The allocated block will be 64-bit aligned, but will not be page aligned.
    The pool allocator calculates the smallest number of POOL_BLOCK_SIZE that can be used to satisfy the request.
    If there are no blocks available of this size, then a block of the next larger block size is allocated and split.
    One piece is placed back into the pool, and the other piece is used to satisfy the request.
    If the allocator reaches the paged-sized block list, and nothing is there, the page-aligned pool allocator is called.
    The page is split and added to the pool.
Arguments:
...
#define CacheOverhead POOL_OVERHEAD
    ...
    if (ExpPoolFlags & (EX_KERNEL_VERIFIER_ENABLED | EX_SPECIAL_POOL_ENABLED)) {
        if (ExpPoolFlags & EX_KERNEL_VERIFIER_ENABLED) {
            if ((PoolType & POOL_DRIVER_MASK) == 0) {
                // Use the Driver Verifier pool framework.  Note this will result in a recursive callback to this routine.
#if defined (_X86_)
                RtlGetCallersAddress(&CallingAddress, &CallersCaller);
#else
                CallingAddress = (PVOID)_ReturnAddress();
#endif
                return VeAllocatePoolWithTagPriority(PoolType | POOL_DRIVER_MASK, NumberOfBytes, Tag, HighPoolPriority, CallingAddress);
            }
            PoolType &= ~POOL_DRIVER_MASK;
        }

        if ((ExpPoolFlags & EX_SPECIAL_POOL_ENABLED) && (MmUseSpecialPool(NumberOfBytes, Tag))) {// Use special pool if there is a tag or size match.
    ...
    // Isolate the base pool type and select a pool from which to allocate the specified block size.
    ...
    // Check to determine if the requested block can be allocated from one of the pool lists or must be directly allocated from virtual memory.
    if (NumberOfBytes > POOL_BUDDY_MAX) {
        // The requested size is greater than the largest block maintained by allocation lists.
        RequestType = (PoolType & (BASE_POOL_TYPE_MASK | SESSION_POOL_MASK | POOL_VERIFIER_MASK));
        Entry = (PPOOL_HEADER)MiAllocatePoolPages(RequestType, NumberOfBytes);
        if (Entry == NULL) {// If there are deferred free blocks, free them now and retry.
        ...
        if (Entry == NULL) {
            if (PoolType & MUST_SUCCEED_POOL_TYPE_MASK) {
                KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, NumberOfBytes, NonPagedPoolDescriptor.TotalPages, NonPagedPoolDescriptor.TotalBigPages, 0);
            }
        ...
        // Note nonpaged session allocations get turned into global session allocations internally, so they must be added to the global tag tables.
        // Paged session allocations go into their own tables.
        if (ExpAddTagForBigPages((PVOID)Entry, Tag, NumberOfPages, PoolType) == FALSE) {
            // Note that not being able to add the tag entry above implies 2 things: The allocation must now be tagged
            // as BIG because the subsequent free also won't find it in the big page tag table and so it must use BIG when removing it from the PoolTrackTable.
            // Also that the free must get the size from MiFreePoolPages since the big page tag table won't have the size in this case.
            Tag = ' GIB';
        }
    ...
    if (NumberOfBytes == 0) {
        // Besides fragmenting pool, zero byte requests would not be handled in cases where the minimum pool block size is the same as the pool header size
        // (no room for flink/blinks, etc).
#if DBG
    ...
    PERFINFO_POOLALLOC(PoolType, Tag, NumberOfBytes);// The requested size is less than or equal to the size of the maximum block maintained by the allocation lists.
    ...
    if (CheckType == PagedPool) {
        // If the requested pool block is a small block, then attempt to allocate the requested pool from the per processor lookaside list.
        // If the attempt fails, then attempt to allocate from the system lookaside list.
        // If the attempt fails, then select a pool to allocate from and allocate the block normally.

        // Also note that if hot/cold separation is enabled, allocations are not satisfied from lookaside lists as these are either :
        // 1. cold references
        // or
        // 2. we are still booting on a small machine, thus keeping pool locality dense (to reduce the working set footprint thereby
        //    reducing page stealing) is a bigger win in terms of overall speed than trying to satisfy individual requests more quickly.
        if ((PoolType & SESSION_POOL_MASK) == 0) {
            // Check for prototype pool - always allocate it from its own pages as the sharecounts applied on these allocations by 
            // memory management make it more difficult to trim these pages.
            // This is an optimization so that other pageable allocation pages (which are much easier to trim because their sharecount is
            // almost always only 1) don't end up being mostly resident because of a single prototype pool allocation within in.
            // Note this also makes it easier to remove specific pages for hot remove or callers that need contiguous physical memory.
            ...
            if ((NeededSize <= POOL_SMALL_LISTS) && (USING_HOT_COLD_METRICS == 0)) {
                ...
                if (Entry != NULL) {
                    ...
                    // Zero out any back pointer to our internal structures to stop someone from corrupting us via an uninitialized pointer.
            ...
            // If there is more than one paged pool, then attempt to find one that can be immediately locked.

            // N.B. The paged pool is selected in a round robin fashion using a simple counter.
            //      Note that the counter is incremented using a a noninterlocked sequence, but the pool index is never allowed to get out of range.
            if (USING_HOT_COLD_METRICS) {
                if ((PoolType & POOL_COLD_ALLOCATION) == 0) {
                    PoolIndex = 1;// Hot allocations come from the first paged pool.
                } else {
                    PoolIndex = ExpNumberOfPagedPools;// Force cold allocations to come from the last paged pool.
                }
            } else {
                if (KeNumberNodes > 1) {
                    // Use the pool descriptor which contains memory local to the current processor even if we have to wait for it.
                    // While it is possible that the paged pool addresses in the local descriptor have been paged out, on large memory NUMA machines this should be less common.
                    Prcb = KeGetCurrentPrcb();
        ...
        if (ExpNumberOfNonPagedPools <= 1) {
            PoolIndex = 0;
        } else {// Use the pool descriptor which contains memory local to the current processor even if we have to contend for its lock.
            Prcb = KeGetCurrentPrcb();
            PoolIndex = Prcb->ParentNode->Color;
...
restart2:
    ...
    do {
        // If the list is not empty, then allocate a block from the selected list.
        if (PrivateIsListEmpty(ListHead) == FALSE) {
            ...
            if (Entry->BlockSize != NeededSize) {
                // The selected block is larger than the allocation request.
                // Split the block and insert the remaining fragment in the appropriate list.

                // If the entry is at the start of a page, then take the allocation from the front of the block so as to minimize fragmentation.
                // Otherwise, take the allocation from the end of the block which may also reduce fragmentation if the block is at the end of a page.
                if (Entry->PreviousSize == 0) {// The entry is at the start of a page.
                    ...
                    // If the allocated block is not at the end of a page, then adjust the size of the next block.
                ...
                } else {// The entry is not at the start of a page.
                    SplitEntry = Entry;
                ...
                // Set the size of the allocated entry, clear the pool type of the split entry, set the index of the split entry,
                // and insert the split entry in the appropriate free list.
                ...
                // Only insert split pool blocks which contain more than just a header as only those have room for a flink/blink !
                // Note if the minimum pool block size is bigger than the header then there can be no blocks like this.
    ...
    // A block of the desired size does not exist and there are no large blocks that can be split to satisfy the allocation.
    // Attempt to expand the pool by allocating another page and adding it to the pool.
    ...
    if (Entry == NULL) {
        ...
        if ((PoolType & MUST_SUCCEED_POOL_TYPE_MASK) != 0) {// Must succeed pool was requested so bugcheck.
            KeBugCheckEx(MUST_SUCCEED_POOL_EMPTY, PAGE_SIZE, NonPagedPoolDescriptor.TotalPages, NonPagedPoolDescriptor.TotalBigPages, 0);
        }

        ExPoolFailures += 1;// No more pool of the specified type is available.
    ...
    // Split the allocated page and insert the remaining fragment in the appropriate listhead.

    // Set the size of the allocated entry, clear the pool type of the split entry, 
    // set the index of the split entry, and insert the split entry in the appropriate free list.

    // Note that if the request was for nonpaged session pool, we are not updating the session pool descriptor for this.
    // Instead we are deliberately updating the global nonpaged pool descriptor because the rest of the fragment goes into global nonpaged pool.
    // This is ok because the session pool descriptor TotalPages count is not relied upon.
...
__bcount(NumberOfBytes) PVOID ExAllocatePool(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    This function allocates a block of pool of the specified type and returns a pointer to the allocated block.
    This function is used to access both the page-aligned pools, and the list head entries (less than a page) pools.
...
__bcount(NumberOfBytes)
PVOID ExAllocatePoolWithTagPriority(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes, __in ULONG Tag, __in EX_POOL_PRIORITY Priority)
/*
Routine Description:
    This function allocates a block of pool of the specified type and returns a pointer to the allocated block.
    This function is used to access both the page-aligned pools, and the list head entries (less than a page) pools.
...
*/
{
    ...
    if ((Priority & POOL_SPECIAL_POOL_BIT) && (NumberOfBytes <= POOL_BUDDY_MAX)) {
        Entry = MmAllocateSpecialPool(NumberOfBytes, Tag, PoolType, (Priority & POOL_SPECIAL_POOL_UNDERRUN_BIT) ? 1 : 0);
        if (Entry != NULL) {
            return Entry;
        }
    ...
    // Pool and other resources can be allocated directly through the Mm without the pool code knowing - so always call the Mm for the up-to-date counters.
    if ((Priority != HighPoolPriority) && ((PoolType & MUST_SUCCEED_POOL_TYPE_MASK) == 0)) {
        if (MmResourcesAvailable(PoolType, NumberOfBytes, Priority) == FALSE) {
            // The Mm does not have very many full pages left.
            // Leave those for true high priority callers.
            // But first see if this request is small, and if so, if there is a lot of fragmentation, 
            // then it is likely the request can be satisfied from pre-existing fragments.
            if (NumberOfBytes > POOL_BUDDY_MAX) {
                return NULL;
            }
            ...
            CheckType = PoolType & BASE_POOL_TYPE_MASK;
            if ((CheckType == NonPagedPool) || ((PoolType & SESSION_POOL_MASK) == 0)) {
                PoolDesc = PoolVector[CheckType];
                TotalBytes = 0;
                TotalFullPages = 0;
                if (CheckType == PagedPool) {
            ...
            // If the pages are more than 80% populated then don't assume we're going to be able to satisfy ths request via a fragment.
            TotalFullPages |= 1;        // Ensure we never divide by zero.
            TotalBytes >>= PAGE_SHIFT;

            // The additions above were performed lock free so we must handle slicing which can cause nonexact sums.
    ...
    // There is a window between determining whether to proceed and actually doing the allocation.
    // In this window the pool may deplete.  This is not worth closing at this time.
    return ExAllocatePoolWithTag(PoolType, NumberOfBytes, Tag);
}


__bcount(NumberOfBytes) PVOID ExAllocatePoolWithQuota(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes)
/*
Routine Description:
    ...
    Otherwise, the appropriate binary buddy pool is used.
    The allocated block will be 64-bit aligned, but will not be page aligned.
    After the allocation completes, an attempt will be made to charge pool quota (of the appropriate type) to the current process object.
    If the quota charge succeeds, then the pool block's header is adjusted to point to the current process.
    The process object is not dereferenced until the pool is deallocated and the appropriate amount of quota is returned to the process.
...
FORCEINLINE PEPROCESS ExpGetBilledProcess(IN PPOOL_HEADER Entry)
{
    ...
    if (ProcessBilled != NULL) {
        if (((PKPROCESS)(ProcessBilled))->Header.Type != ProcessObject) {
            KeBugCheckEx(BAD_POOL_CALLER, 0xD, (ULONG_PTR)(Entry + 1), Entry->PoolTag, (ULONG_PTR)ProcessBilled);
        }
    }
...
__bcount(NumberOfBytes)  PVOID ExAllocatePoolWithQuotaTag(__in POOL_TYPE PoolType, __in SIZE_T NumberOfBytes, __in ULONG Tag)
/*
Routine Description:
    This function allocates a block of pool of the specified type, returns a pointer to the allocated block, and if the binary buddy
...
*/
{
    ...
    // Note for NT64, there is room in the pool header for both the tag and the quota pointer so no extra space is needed at the end.

    // Only add in the quota pointer if doing so won't cause us to spill the allocation into a full page.
    ASSERT(NumberOfBytes != 0);
    if (NumberOfBytes <= PAGE_SIZE - POOL_OVERHEAD - sizeof(PVOID)) {
        ...
        // Turn off the quota bit prior to allocating if we're not charging as there's no room to put (or subsequently query for) a quota pointer.
        PoolType = (POOL_TYPE)((UCHAR)PoolType - POOL_QUOTA_MASK);
    }
#endif

    p = ExAllocatePoolWithTag(PoolType, NumberOfBytes, Tag);
    if (!PAGE_ALIGNED(p)) {// Note - NULL is page aligned.
        ...
        if (Process != PsInitialSystemProcess) {
            Status = PsChargeProcessPoolQuota(Process, PoolType & BASE_POOL_TYPE_MASK, (ULONG)(Entry->BlockSize << POOL_BLOCK_SHIFT));
...
/*
Routine Description:
    This function inserts a pool tag in the tag table and increments the number of allocates and updates the total allocation size.
...
/*
Routine Description:
    This function seeds well-known hot tags into the pool tag tracking table when the table is first created.
    The goal is to increase the likelihood that the hash generated for these tags always gets a direct hit.
Environment:
...
*/
{
    ...
    ULONG KeyList[] = {
            ...
            // BIG is preseeded not because it is hot, but because allocations with this tag must be inserted successfully (ie: cannot be
            // retagged into the Ovfl bucket) because we need a tag to account for them in the PoolTrackTable counting when freeing the pool.
            ' GIB',
    };
    ...
    for (i = 0; i < sizeof(KeyList) / sizeof(ULONG); i += 1) {
        ...
        do {
            ...
            if ((TrackTable[Hash].Key == 0) && (Hash != PoolTrackTableSize - 1)) {
...
Environment:
    No pool locks held so pool may be freely allocated here as needed.
    This routine is only called if ExpInsertPoolTracker encounters a full builtin list.
*/
{
    ULONG Hash;
    KIRQL OldIrql;
    ...
    // Linear search through the expansion table.
    // This is ok because the case of no free entries in the built-in table is extremely rare.
    ExAcquireSpinLock(&ExpTaggedPoolLock, &OldIrql);
    ...
    if (PoolTrackTable[PoolTrackTableSize - 1].Key != 0) {
        ...
        // is not allowed because a subsequent free of a tag can go negative as the original allocation is in overflow and a newer allocation may be distinct.
    ...
    if (NewTable != NULL) {
        if (PoolTrackTableExpansion != NULL) {
            RtlCopyMemory(NewTable, PoolTrackTableExpansion, SizeInBytes);// Copy all the existing entries into the new table.
        }

        RtlZeroMemory((PVOID)(NewTable + PoolTrackTableExpansionSize), NewSizeInBytes - SizeInBytes);
        ...
        if (OldTable != NULL) {
            ...
            ExpRemovePoolTracker('looP', (SIZE_T)BigPages * PAGE_SIZE, NonPagedPool);
        }

        ExpInsertPoolTrackerExpansion(Key, NumberOfBytes, PoolType);// Finally insert the caller's original allocation.
    } else {
        // Use the very last entry as a bit bucket for overflows.
        Hash = (ULONG)PoolTrackTableSize - 1;
...
/*
Routine Description:
    This function increments the number of frees and updates the total allocation size in the expansion table.
Arguments:
    Key - Supplies the key value used to locate a matching entry in the tag table.
    NumberOfBytes - Supplies the allocation size.
    PoolType - Supplies the pool type.
    ...
    for (Hash = 0; Hash < PoolTrackTableExpansionSize; Hash += 1) {
        if (PoolTrackTableExpansion[Hash].Key == Key) {
            if ((PoolType & BASE_POOL_TYPE_MASK) == PagedPool) {
                ...
                ASSERT(PoolTrackTableExpansion[Hash].PagedAllocs >= PoolTrackTableExpansion[Hash].PagedFrees);
                ...
                ASSERT(PoolTrackTableExpansion[Hash].NonPagedAllocs >= PoolTrackTableExpansion[Hash].NonPagedFrees);
...
LOGICAL ExpExpandBigPageTable(IN KIRQL OldIrql)
/*
Routine Description:
    This function expands the big page tracking table and rehashes all the old entries into the new table at the same time.
Arguments:
...
*/
{
    ...
    // Rehash the valid tables in the old table into their new locations in the new table.
    ...
    ExpRemovePoolTracker('looP', (SIZE_T)BigPages * PAGE_SIZE, NonPagedPool);
    ExpInsertPoolTracker('looP', ROUND_TO_PAGES(NewSizeInBytes), NonPagedPool);
    return TRUE;
}
...
*/
{
    ...
    // The low bit of the address is set to indicate a free entry.
    // The high bit cannot be used because in some configurations the high bit is not set for all kernelmode addresses.
    ...
    do {
        ...
        do {
            ...
            if (((ULONG_PTR)OldVa & POOL_BIG_TABLE_ENTRY_FREE) &&
                ...
                if ((IterationCount >= 16) && ((ULONG)ExpPoolBigEntriesInUse > (ULONG)(TableSize / 4))) {
                    if (ExTryAcquireSpinLockExclusive(&ExpLargePoolTableLock) == TRUE) {
                        ASSERT(TableSize == PoolBigPageTableSize);
                        ExpExpandBigPageTable(OldIrql);// The expansion function always releases the lock.
                    } else {
        ...
        ASSERT(TableSize == PoolBigPageTableSize);// No thread could have grown this during the contention above, so ASSERT that this is the case.
...
Routine Description:
    ...
    Otherwise, the binary buddy pool deallocator is used.
    Deallocation looks at the allocated block's pool header to determine the pool type and block size being deallocated.
    If the pool was allocated using ExAllocatePoolWithQuota, then after the deallocation is complete,
    the appropriate process's pool quota is adjusted to reflect the deallocation, and the process object is dereferenced.
Arguments:
...
*/
{
    ...
    if (ExpPoolFlags & (EX_CHECK_POOL_FREES_FOR_ACTIVE_TIMERS |
        ...
        if (ExpPoolFlags & EX_SPECIAL_POOL_ENABLED) {
            ...
            if (ExFreePoolTraces != NULL) {
                ...
                if (SpecialPool == TRUE) {
                    ...
                    RtlZeroMemory(&Information->PoolHeader, sizeof(POOL_HEADER));
                    Information->PoolHeader.Ulong1 = MmGetSizeOfBigPoolAllocation(P);
                }

                RtlZeroMemory(&Information->StackTrace[0], EX_FREE_POOL_BACKTRACE_LENGTH * sizeof(PVOID));
                RtlCaptureStackBackTrace(1, EX_FREE_POOL_BACKTRACE_LENGTH, Information->StackTrace, &Hash);
            }
        ...
        if (!PAGE_ALIGNED(P)) {
            ...
            if ((PAGE_END(NextEntry) == FALSE) && (Entry->BlockSize != NextEntry->PreviousSize)) {
                KeBugCheckEx(BAD_POOL_HEADER, 0x20, (ULONG_PTR)Entry, (ULONG_PTR)NextEntry, Entry->Ulong1);
            }
            ...
            FREE_CHECK_ERESOURCE(Entry, NumberOfBytes);// Check if an ERESOURCE is currently active in this memory block.
            FREE_CHECK_KTIMER(Entry, NumberOfBytes);// Check if a KTIMER is currently active in this memory block.
            FREE_CHECK_WORKER(Entry, NumberOfBytes);// Look for work items still queued.
        }
    }
    ...
    if (PAGE_ALIGNED(P)) {
        ...
        if (ExpPoolFlags &
            ...
            FREE_CHECK_ERESOURCE(P, NumberOfBytes);// Check if an ERESOURCE is currently active in this memory block.
            FREE_CHECK_KTIMER(P, NumberOfBytes);// Check if a KTIMER is currently active in this memory block.
    ...
    // is not consistent, it's likely the previous allocation's owner is the corruptor and it's also likely he has not yet freed the block either.
    // However, this cannot be done safely with mutex or lock synchronization and this would be too high a performance penalty to pay.
    NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry + BlockSize);
    if ((PAGE_END(NextEntry) == FALSE) && (BlockSize != NextEntry->PreviousSize)) {
        KeBugCheckEx(BAD_POOL_HEADER, 0x20, (ULONG_PTR)Entry, (ULONG_PTR)NextEntry, Entry->Ulong1);
    }
    ...
    if (EntryPoolType & SESSION_POOL_MASK) {
        ...
        // All session space allocations have an index of 0 unless there are multiple nonpaged (session) pools.
    ...
    ExpRemovePoolTrackerInline(Tag, BlockSize << POOL_BLOCK_SHIFT, EntryPoolType - 1);// Update the pool tracking database.
    ...
    // If the pool block is a small block, then attempt to free the block to the single entry lookaside list.
    // If the free attempt fails, then free the block by merging it back into the pool data structures.
    if (((EntryPoolType & SESSION_POOL_MASK) == 0) || (CheckType == NonPagedPool)) {
        if ((BlockSize <= POOL_SMALL_LISTS) && (USING_HOT_COLD_METRICS == 0)) {
            // Try to free the small block to a per processor lookaside list.
            Prcb = KeGetCurrentPrcb();
            if (CheckType == PagedPool) {
                // Prototype pool is never put on general lookaside lists due to the sharecounts applied on these allocations when
                // they are in use (ie: the rest of this page is comprised of prototype allocations even though this allocation is being freed).
                // Pages containing prototype allocations are much more difficult for memory management to trim (unlike the rest of paged pool) due to the sharecounts generally applied.
                if (PoolDesc->PoolIndex == 0) {
                    goto NoLookaside;
                }

                // Only free the small block to the current processor's lookaside list if the block is local to this node.
                if (KeNumberNodes > 1) {
                ...
                if (ExQueryDepthSList(&LookasideList->ListHead) < LookasideList->Depth) {
                    ...
                    InterlockedPushEntrySList(&LookasideList->ListHead, (PSLIST_ENTRY)P);
                    return;
                }
...
NoLookaside:
    // If the pool block release can be queued so the pool mutex/spinlock acquisition/release can be amortized then do so.
    // Note "hot" blocks are generally in the lookasides above to provide fast reuse to take advantage of hardware caching.
    if (ExpPoolFlags & EX_DELAY_POOL_FREES) {
    ...
    if (PAGE_END(NextEntry) == FALSE) {
        if (NextEntry->PoolType == 0) {
            Combined = TRUE;// This block is free, combine with the released block.

            // If the split pool block contains only a header, then it was not inserted and therefore cannot be removed.
    ...
    if (PAGE_ALIGNED(Entry) && (PAGE_END((PPOOL_BLOCK)Entry + Entry->BlockSize) != FALSE)) {
        ...
        // If the freed block was combined with any other block, then adjust the size of the next block if necessary.
        if (Combined != FALSE) {
            // The size of this entry has changed, 
            // if this entry is not the last one in the page, update the pool block after this block to have a new previous allocation size.
            NextEntry = (PPOOL_HEADER)((PPOOL_BLOCK)Entry + BlockSize);
            if (PAGE_END(NextEntry) == FALSE) {
...
/*
Routine Description:
    This routine frees a number of pool allocations at once to amortize the synchronization overhead cost.
Arguments:
    PoolDesc - Supplies the relevant pool descriptor.
Environment:
    Kernel mode.  May be as high as APC_LEVEL for paged pool or DISPATCH_LEVEL for nonpaged pool.
*/
{
    LONG ListCount;
    KLOCK_QUEUE_HANDLE LockHandle;
    ...
    do {
        ...
        do {
            ...
            if (PAGE_ALIGNED(Entry) && (PAGE_END((PPOOL_BLOCK)Entry + Entry->BlockSize) != FALSE)) {
                ...
                // If the freed block was combined with any other block, then adjust the size of the next block if necessary.
                if (Combined != FALSE) {
                    // The size of this entry has changed, if this entry is not the last one in the page, 
                    // update the pool block after this block to have a new previous allocation size.
        ...
        if (InterlockedCompareExchangePointer(&PoolDesc->PendingFrees, NULL, FirstEntry) == FirstEntry) {
            break;
        }
...
SIZE_T ExQueryPoolBlockSize(__in PVOID PoolBlock, __out PBOOLEAN QuotaCharged)
/*
Routine Description:
    This function returns the size of the pool block.
Arguments:
    PoolBlock - Supplies the address of the block of pool.
    QuotaCharged - Supplies a BOOLEAN variable to receive whether or not the pool block had quota charged.
    NOTE: If the entry is bigger than a page, the value PAGE_SIZE is returned rather than the correct number of bytes.
Return Value:
    Size of pool block.
*/
{
...
/*
Routine Description:
    This function returns quota charged to a subject process when the specified pool block was allocated.
Arguments:
    P - Supplies the address of the block of pool being deallocated.
*/
{
    ...
    Entry = (PPOOL_HEADER)((PCHAR)P - POOL_OVERHEAD);// Align the entry address to a pool allocation boundary.

    // If quota was charged, then return the appropriate quota to the subject process.
...
/*
Routine Description:
    This function deletes the tag table for the specified processor number because the processor did not boot.
Arguments:
...
*/
{
    ...
    // Raise to DISPATCH to prevent a race when attempting to hot-add a processor while a pool-usage query is active.
...
*/
{
    ...
    // Make sure all DPCs are running (ie: spinning at DISPATCH_LEVEL) to prevent any pool allocations or frees from happening until all the counters are snapped.
    // Otherwise the counters could be misleading (ie: more frees than allocs, etc).
    if (KeSignalCallDpcSynchronize(SystemArgument2)) {
        // This processor (could be the caller or a target) is the final processor to enter the DPC spinloop.  Snap the data now.
#if defined (NT_UP)
        RtlCopyMemory((PVOID)Context->PoolTrackTable, (PVOID)PoolTrackTable, Context->PoolTrackTableSize * sizeof(POOL_TRACKER_TABLE));
#else
        RtlCopyMemory((PVOID)Context->PoolTrackTable, (PVOID)ExPoolTagTables[0], Context->PoolTrackTableSize * sizeof(POOL_TRACKER_TABLE));
    ...
    KeSignalCallDpcSynchronize(SystemArgument2);// Wait until everyone has got to this point before continuing.
    KeSignalCallDpcDone(SystemArgument1);// Signal that all processing has been done.
}
...
/*
Routine Description:
    This function copies the system pool tag information to the supplied USER space buffer.
    Note that the caller has already probed the USER address and wrapped this routine inside a try-except.
Arguments:
...
*/
{
    ...
    PoolTrackInfo = (PPOOL_TRACKER_TABLE)ExAllocatePoolWithTag(NonPagedPool, NumberOfBytes + NumberOfExpansionTableBytes, 'ofnI');
    ...
    // Wrap the user space accesses with an exception handler so we can free the pool track info allocation if the user address was bogus.
    try {
        while (TrackerEntry < LastTrackerEntry) {
...
Arguments:
    ...
    ActualEntries - Receives the total number of entries.
                    This can be more than ReturnedEntries if the caller's buffer is not large enough to hold all the data.
Return Value:
    Various NTSTATUS codes.
*/
{
    ...
    while (TrackerEntry < LastTrackerEntry) {
        if (TrackerEntry->Key != 0) {
            ...
            if (totalBytes > SystemInformationLength) {
                ...
                // Session pool tag entries are updated with interlocked sequences so it is possible here that we can read one that is in the middle of being updated.
                // Sanitize the data here so callers don't have to.
...
/*
Routine Description:
    This function copies the system big pool entry information to the supplied USER space buffer.
    Note that the caller has already probed the USER address and wrapped this routine inside a try-except.
    PAGELK was not used for this function so that calling it causes minimal disruption to actual memory usage.
Arguments:
    SystemInformation - Supplies a user space buffer to copy the data to.
    SystemInformationLength - Supplies the length of the user buffer.
...
*/
{
    ...
    do {
        ...
        SnappedBigTableSizeInBytes = SnappedBigTableSize * sizeof(POOL_TRACKER_BIG_PAGES);

        if (NewTable != NULL) {
            MiFreePoolPages(NewTable);
        }

        // Use MiAllocatePoolPages for the temporary buffer so we won't have to filter it out of the results before handing them back.
        NewTable = MiAllocatePoolPages(NonPagedPool, SnappedBigTableSizeInBytes);
        if (NewTable == NULL) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        if (SnappedBigTableSize >= PoolBigPageTableSize) {
            break;// Success - our table is big enough to hold everything.
        }

        ExReleaseSpinLockExclusive(&ExpLargePoolTableLock, OldIrql);
    } while (TRUE);

    RtlCopyMemory(NewTable, PoolBigPageTable, PoolBigPageTableSize * sizeof(POOL_TRACKER_BIG_PAGES));
    ...
    // Wrap the user space accesses with an exception handler so we can free the temp buffer if the user address was bogus.
    try {
...
*/
{
    ...
    if (PAGE_ALIGNED(P)) {
        ...
        KeCheckForTimer(P, PAGE_SIZE);// Just check the first page.
...
Routine Description:
    ...
    It's job is to disable various pool optimizations that are enabled to speed up booting and reduce the memory footprint on small machines.
Arguments:
    Dpc - Supplies a pointer to a control object of type DPC.
...
*/
{
    ...
    // Pretty much all pages are "hot" after bootup.
    // Since bootup has finished, use lookaside lists and stop trying to separate regular allocations as well.
