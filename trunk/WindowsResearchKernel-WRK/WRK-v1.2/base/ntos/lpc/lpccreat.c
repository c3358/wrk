...
*/
{
    ...
    //  For a waitable port, create the KEVENT that will be used to signal clients
    if (ConnectionPort->Flags & PORT_WAITABLE) {
        KeInitializeEvent(&ConnectionPort->WaitEvent, NotificationEvent, FALSE);
    }

    //  Set the maximum message length and connection info length based on the zone block size less the structs overhead.
    ...
    //  Sanity check that the max message length being asked for is not greater than the max message length possible in the system
    ...
    ConnectionPort->MaxMessageLength = (USHORT)MaxMessageLength;//  Save the MaxMessageLength to the connection port

    //  Sanity check that the max connection info length being asked for is not greater than the maximum possible in the system
    if (ConnectionPort->MaxConnectionInfoLength < MaxConnectionInfoLength) {
#if DBG
        LpcpPrint(("MaxConnectionInfoLength granted is %x but requested %x\n", ConnectionPort->MaxConnectionInfoLength, MaxConnectionInfoLength));
#endif
        ObDereferenceObject(ConnectionPort);
        return STATUS_INVALID_PARAMETER_3;
    }

    ConnectionPort->MaxConnectionInfoLength = (USHORT)MaxConnectionInfoLength;//  Save the max connection length to the port
