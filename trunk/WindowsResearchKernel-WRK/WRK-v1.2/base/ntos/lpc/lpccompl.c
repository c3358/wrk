...
Routine Description:
    ...
    If the connection request is accepted, then two communication port objects will be created and connected together.
    One will be inserted in the client process' handle table and returned to the client via the PortHandle parameter it specified on the NtConnectPort service.
    The other will be inserted in the server process' handle table and returned via the PortHandle parameter specified on the NtCompleteConnectPort service.
...
Arguments:
    ...
    ConnectionRequest - A pointer to a structure that describes the connection request being accepted or rejected:
        ...
        CLIENT_ID ClientId - Specifies a structure that contains the client identifier (CLIENT_ID) of the thread that sent the request.
    ...
    AcceptConnection - Specifies a boolean value which indicates where the connection request is being accepted or rejected.
        A value of TRUE means that the connection request is accepted and a server communication port handle will be created and connected to the client's communication port handle.
    ...
    ServerView - A pointer to a structure that specifies the section that the server process will use to send messages back to the client process connected to this port.
        ...
        ULONG SectionOffset - Specifies a field that will receive the actual offset, in bytes, from the start of the section.
        ...
        PVOID ViewBase - Specifies a field that will receive the base address of the port memory in the server's address space.

        PVOID ViewRemoteBase - Specifies a field that will receive the base address of the server port's memory in the client's address space.
            Used to generate pointers that are meaningful to the client.

    ClientView - An optional pointer to a structure that will receive information about the client process' view in the server's address space.
        The server process can use this information to validate pointers it receives from the client process.

        The ClientView Structure

        ULONG Length - Specifies the size of this data structure in bytes.
        PVOID ViewBase - Specifies a field that will receive the base address of the client port's memory in the server's address space.
        ULONG ViewSize - Specifies a field that will receive the size, in bytes, of the client's view in the server's address space.
...
*/
{
    ...
    //  Acquire the mutex that guards the LpcReplyMessage field of the thread and get the pointer to the message that the thread is waiting for a reply to.
    LpcpAcquireLpcpLock();

    //  See if the thread is waiting for a reply to the connection request specified on this call.
    ...
    if ((LpcpGetThreadMessage(ClientThread) == NULL) ||
        ...
        Msg = LpcpGetThreadMessage(ClientThread);//  Remember the LPCP message from the thread
        ConnectMsg = (PLPCP_CONNECTION_MESSAGE)(Msg + 1);//  Get connection message immediately following the LPCP message
        ClientPort = ConnectMsg->ClientPort;//  Remember the client port from the connection message
        ConnectionPort = ClientPort->ConnectionPort;//  Get a pointer to the connection port from the client port.
        if (ConnectionPort->ServerProcess != PsGetCurrentProcess()) {//  Check if the server process accept the connection
            LpcpReleaseLpcpLock();//  Release the LPC mutex
            ObDereferenceObject(ClientProcess);
            ObDereferenceObject(ClientThread);
            return (STATUS_REPLY_MESSAGE_MISMATCH);
        }

        ClientThread->LpcReplyMessage = NULL;//  Remove the LPC message from the thread
        ConnectMsg->ClientPort = NULL;//  Remove the client port from the connection message

        //  Clean up the rest of the client thread.
        //  This cleanup use to be done unconditionally right before releasing the mutex however this causes trouble if our caller supplied a bad reply message and we clobber an arbitrary threads state
        ClientThread->LpcReplyMessageId = 0;
    }

    LpcpReleaseLpcpLock();//  Release the mutex that guards the field.

    //  Now if we did not get an LPCP message from a client thread then this isn't a good call and we'll dereference what we thought was the client thread/process and tell our caller their mistake
    if (!Msg) {
        LpcpPrint(("%s Attempted AcceptConnectPort to Thread %lx (%s)\n", PsGetCurrentProcess()->ImageFileName, ClientThread, THREAD_TO_PROCESS(ClientThread)->ImageFileName));
    ...
    if (AcceptConnection) {
        ...
        //  Connect the newly created server communication port to the connection port with a referenced pointer.
        //  Prevents the connection port from going away until all of the communication ports have been closed.
        ObReferenceObject(ConnectionPort);

        ServerPort->ConnectionPort = ConnectionPort;
        ServerPort->MaxMessageLength = ConnectionPort->MaxMessageLength;

        //  Connect the client and server communication ports together with unreferenced pointers.
        //  They are unreferenced so that the PortObjectType delete procedure will get called when a communication port is closed.
        //  If this were not the case then we would need a special NtClosePort system service in order to tear down a pair of connected communication ports.
        ...
        //  If the client has allocated a port memory section that is mapped into the client's address space, 
        //  then map a view of the same section for the server process to see.
        LpcpAcquireLpcpLock();
        ...
        if (ClientSectionToMap) {
            ...
            if (NT_SUCCESS(Status)) {
                ...
                //  The client section was mapped. We'll add an extra reference to server process. This reference will be removed on port cleanup.
                ServerPort->MappingProcess = PsGetCurrentProcess();
                ObReferenceObject(ServerPort->MappingProcess);
            } else {
                //  At this point we're really going to drop all the way out to the label bailout: because everything else is protected with a test against Status.
                //  But first we have to release the server port that we've just created
                ObDereferenceObject(ServerPort);
            }
        }

        //  If the server process has allocated a port memory section for send data to the client on call back requests, 
        //  map two views of that section, the first for the server process and the second view for the client process.
        //  Return the location of the server's view to the caller of this function.
        //  Return the client's view to the client process via the reply to the connection request.
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        CapturedServerView.ViewRemoteBase = ClientPort->ServerSectionBase;//  Let the server know where the client's view of the section got mapped
                        ConnectMsg->ServerView.ViewBase = ClientPort->ServerSectionBase;//  Let the client know where the server's view of the section got mapped
        ...
        //  Insert the server communication port object in specified object table.  Set port handle value if successful.
        ...
        if (NT_SUCCESS(Status)) {
            ...
            ObDereferenceObject(ServerPort);//  Now we can remove the extra object reference
        }
    } else {//  Otherwise the server has not accepted the connection request
...
bailout:
    ...
    //  If the client is not null then this is an error condition and we need to cleanup and wake the client thread.
    //  In success cases the client thread is woken up with a call to Complete Connect Request
    if (ClientThread != NULL) {
        ...
        //  Wake up the thread that is waiting for an answer to its connection request inside of NtConnectPort.
        KeReleaseSemaphore(&ClientThread->LpcReplySemaphore, 0, 1L, FALSE);
        ObDereferenceObject(ClientThread);//  Dereference client thread and return the system service status.
    }
...
/*
Routine Description:
    This routine is called by the server after it calls NtAcceptConnectPort to wake up the client thread.
    Between calling NtAcceptConnectPort and NtCompleteConnectPort the server can do whatever work is necessary before waking up the client
Arguments:
...
*/
{
    ...
    PreviousMode = KeGetPreviousMode();//  Get previous processor mode
    ...
    //  Under the LPC lock we need to check for a client thread and if there is one we'll remember and remove the client thread,
    //  and then prepare to wake the client
    LpcpAcquireLpcpLock();
    ...
    if (LpcpGetThreadMessage(ClientThread) == NULL) {
        LpcpReleaseLpcpLock();
        ObDereferenceObject(PortObject);
        return STATUS_SUCCESS;// At this point the client has already been woken. We will get a client died message
    }

    //  The check needs to ensure that the client thread is really on the reply chain for the server's connection port.
    //  This is a quick and dirty fix for NT 5.0.
    //  We zoom down the connection port lpc reply chain looking for an entry that contains the client threads.
    //  If we find a match it's okay if we don't it's bad.
