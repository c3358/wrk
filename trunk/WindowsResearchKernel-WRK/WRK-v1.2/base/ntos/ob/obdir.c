...
POBJECT_DIRECTORY ObpGetShadowDirectory(POBJECT_DIRECTORY Dir);

// Defined in ntos\se\rmlogon.c
// private kernel function to obtain the LUID device map's directory object
// returns a kernel handle
NTSTATUS SeGetLogonIdDeviceMap(IN PLUID pLogonId, OUT PDEVICE_MAP* ppDevMap);
NTSTATUS ObpSetCurrentProcessDeviceMap();
POBJECT_HEADER_NAME_INFO ObpTryReferenceNameInfoExclusive(IN POBJECT_HEADER ObjectHeader);
VOID ObpReleaseExclusiveNameLock(IN POBJECT_HEADER ObjectHeader);
POBJECT_DIRECTORY_ENTRY ObpUnlinkDirectoryEntry(IN POBJECT_DIRECTORY Directory, IN ULONG HashIndex);
...
VOID ObpReleaseLookupContextObject(IN POBP_LOOKUP_CONTEXT LookupContext);
...
//  Global Object manager flags to control the case sensitivity lookup and the LUID devicemap lookup
...
BOOLEAN ObpIsUnsecureName(IN PUNICODE_STRING ObjectName, IN BOOLEAN CaseInsensitive)
{
    PWCHAR CrtName;
...
Arguments:
    DirectoryHandle - Receives the handle for the newly created directory object
    DesiredAccess - Supplies the access being requested for this new directory object
    ObjectAttributes - Supplies caller specified attributes for new directory object
Return Value:
...
*/
{
    ...
    //  Allocate and initialize a new Directory Object.
...
Arguments:
    DirectoryHandle - Receives the handle for the newly opened directory object
    DesiredAccess - Supplies the access being requested for this directory object
    ObjectAttributes - Supplies caller specified attributes for the directory object
Return Value:
    An appropriate status value.
*/
{
...
NTSTATUS NtQueryDirectoryObject(__in HANDLE DirectoryHandle,
...
Arguments:
    ...
    Buffer - Supplies the output buffer to receive the directory information.
        On return this contains one or more OBJECT DIRECTORY INFORMATION structures, the last one being null.
        And then this is followed by the string names for the directory entries.
    Length - Supplies the length, in bytes, of the user supplied output buffer
    ReturnSingleEntry - Indicates if this routine should just return one entry in the directory
    RestartScan - Indicates if we are to restart the scan or continue relative to the enumeration context passed in as the next parameter
    Context - Supplies an enumeration context that must be resupplied to this routine on subsequent calls to keep the enumeration in sync
    ReturnLength - Optionally receives the length, in bytes, that this routine has stuffed into the output buffer
Return Value:
    An appropriate status value.
*/
{
    ...
    //  Allocate space for a temporary work buffer, make sure we got it, and then zero it out.
    //  Make sure the buffer is large enough to hold at least one dir info record.
    //  This will make the logic work better when the a bad length is passed in.
    TempBuffer = ExAllocatePoolWithQuotaTag(PagedPool | POOL_QUOTA_FAIL_INSTEAD_OF_RAISE, Length + sizeof(OBJECT_DIRECTORY_INFORMATION), 'mNbO');
    if (TempBuffer == NULL) {
    ...
    //  Lock down the directory structures for the life of this procedure
    ObpLockDirectoryShared(Directory, &LookupContext);

    //  DirInfo is used to march through the output buffer filling in directory information.
    //  We'll start off by making sure there is room for a NULL entry at end.
    DirInfo = (POBJECT_DIRECTORY_INFORMATION)TempBuffer;
    TotalLengthNeeded = sizeof(*DirInfo);

    //  Keep track of the number of entries found and actual entry that we are processing
    EntryNumber = 0;
    EntriesFound = 0;

    Status = STATUS_NO_MORE_ENTRIES;//  By default we'll say there are no more entries until the following loop put in some data
    ...
    for (Bucket = 0; Bucket < NUMBER_HASH_BUCKETS; Bucket++) {
        ...
        while (DirectoryEntry) {
            //  The captured context is simply the entry count unless the user specified otherwise we start at zero, which means
            //  the first entry is always returned in the enumeration.
            //  If we have an match based on the entry index then we process this entry.
            //  We bump the captured context further done in the code.
            if (CapturedContext == EntryNumber++) {
                //  For this directory entry we'll get a pointer to the object body and see if it has an object name.
                //  If it doesn't have a name then we'll give it an empty name.
                ...
                //  Now compute the length needed for this entry.
                //  This would be the size of the object directory information record, plus the size of the object name and object type name both null terminated.
                LengthNeeded = sizeof(*DirInfo) + ObjectName.Length + sizeof(UNICODE_NULL) + ObjectHeader->Type->Name.Length + sizeof(UNICODE_NULL);

                //  If there isn't enough room then take the following error path.
                //  If the user wanted a single entry then tell the caller what length is really needed and say the buffer was too small.
                //  Otherwise the user wanted multiple entries, so we'll just say there are more entries in the directory.
                //  In both cases we drop down the entry number because we weren't able to fit it in on this call
                ...
                //  The information will fit in the buffer.
                //  So now fill in the output buffer.
                //  We temporarily put in pointers to the name buffer as stored in the object and object type.
                //  We copy the data buffer to the user buffer right before we return to the caller
                try {
                ...
                //  Push the dir info pointer to the next output location, and indicate how many entries we've processed
                TotalLengthNeeded += LengthNeeded;
                DirInfo++;
                EntriesFound++;

                //  If we are to return only one entry then move on to the post processing phase, otherwise indicate that we're
                //  processing the next entry and go back to the top of the inner loop
                if (ReturnSingleEntry) {
                    goto querydone;
                } else {
                    CapturedContext++;//  Bump the captured context by one entry.
                }
            }

            DirectoryEntry = DirectoryEntry->ChainLink;//  Get the next directory entry from the singly linked hash bucket chain
        }
    }

    //  At this point we've processed the directory entries and the first part of the output buffer now contains a bunch of object directory
    //  information records,  but the pointers in them refer to the wrong copies.  So now we have some fixup to do.

querydone:

    try {
        //  We'll only do this post processing if we've been successful so far.
        //  Note that this means we could be returning in the user's output buffer system address that are meaningless,
        //  but then getting back an error status should tell the caller to forget about everything in the output buffer.
        //  Given back a system address also isn't harmful because there is nothing that the user can really do with it.
        if (NT_SUCCESS(Status)) {
            //  Null terminate the string of object directory information records and point to where the actual names will go
            RtlZeroMemory(DirInfo, sizeof(*DirInfo));
            DirInfo++;
            NameBuffer = (PWCH)DirInfo;

            //  Now for every entry that we've put in the output buffer
            //  DirInfo will point to the entry and EntriesFound kept the count.
            //  Note that we are guaranteed space because of the math we did earlier in computing TotalLengthNeeded.
            DirInfo = (POBJECT_DIRECTORY_INFORMATION)TempBuffer;
            while (EntriesFound--) {
                //  Copy over the object name, set the dir info pointer into the user's buffer, then null terminate the string.
                //  Note that we are really copying the data into our temp buffer but the pointer fix up is for the user's buffer which we'll copy into right after this loop.
                ...
                *NameBuffer++ = UNICODE_NULL;
                ...
                RtlCopyMemory(NameBuffer, DirInfo->TypeName.Buffer, DirInfo->TypeName.Length);//  Do the same copy with the object type name
                ...
                *NameBuffer++ = UNICODE_NULL;
                ...
                DirInfo++;//  Move on to the next dir info record
            }

            *Context = EntryNumber;//  Set the enumeration context to the entry number of the next entry to return.
        }

        //  Copy over the results from our temp buffer to the users buffer.
        //  But adjust the amount copied just in case the total length needed exceeds the length we allocated.
        RtlCopyMemory(Buffer, TempBuffer, (TotalLengthNeeded <= Length ? TotalLengthNeeded : Length));

        //  In all cases we'll tell the caller how much space if really needed provided the user asked for this information
        if (ARGUMENT_PRESENT(ReturnLength)) {
    ...
    //  Unlock the directory structures, dereference the directory object, free up our temp buffer, and return to our caller
...
*/
{
    ...
    //  The caller needs to specify both a directory and a name otherwise we can't process the request
    if (!Directory || !Name) {
        goto UPDATECONTEXT;
    }
    ...
    //  Establish our local pointer to the input name buffer and get the number of unicode characters in the input name.
    //  Also make sure the caller gave us a non null name
    Buffer = Name->Buffer;
    ...
    // Hash four unicode chars at a time.  This is optimized for the ASCII case.
    if (WcharLength >= 4) {
        ULONG64 Chunk;
        ULONG64 ChunkHash;
        ULONG   Index;
        ...
        do {
            ...
            if ((Chunk & 0xFF80FF80FF80FF80) == 0) {
                ...
                // N.B. This will also transform some numerals and punctuation characters,
                // but will not significantly impact the hash distribution.
                Chunk &= ~0x0020002000200020;
            } else {
                // Chunk contains at least one non-ASCII character, upcase each character individually.
    ...
    while (1) {
        ...
        while ((DirectoryEntry = *HeadDirectoryEntry) != NULL) {
            if (DirectoryEntry->HashValue == HashValue) {
                ...
                //  This function assumes the name must exist, otherwise it wouldn't be in a directory
                ObjectHeader = OBJECT_TO_OBJECT_HEADER(DirectoryEntry->Object);
                NameInfo = OBJECT_HEADER_TO_NAME_INFO_EXISTS(ObjectHeader);

                //  Compare strings using appropriate function.
                if (RtlEqualUnicodeString(Name, &NameInfo->Name, CaseInSensitive)) {
                    //  If name matches, then exit loop with DirectoryEntry pointing to matching entry.
                    break;
                }
            }
        ...
        //   - we found an entry that matched and DirectoryEntry points to that entry.
        //     Update the bucket chain so that the entry found is at the head of the bucket chain.
        //     This is so the ObpDeleteDirectoryEntry and ObpInsertDirectoryEntry functions will work.
        //     Also repeated lookups of the same name will succeed quickly.
...
UPDATECONTEXT:
    ...
    if (Object) {
        //  Reference the name to keep it's directory alive and the object before returning from the lookup
        ObpReferenceNameInfo(OBJECT_TO_OBJECT_HEADER(Object));
        ObReferenceObject(Object);

        if (!LookupContext->DirectoryLocked) {//  We can safely drop the lock now
            ObpUnlockDirectory(Directory, LookupContext);
        }
    }
...
Routine Description:
    ...
    The directory must have already have been searched using ObpLookupDirectoryEntry because that routine sets the LookupContext.

    N.B. The ObpLookupDirectoryEntry before should be done with the LookupContext locked.
Arguments:
    Directory - Supplies the directory object being modified.
                This function assumes that we earlier did a lookup on the name that was successful or we just did an insertion
...
*/
{
...
#if DBG
    //  This function should be always called with a valid Directory and LookupContext. Test this on checked builds
    if ((LookupContext->Object != NULL) || !LookupContext->DirectoryLocked || (Directory != LookupContext->Directory)) {
        DbgPrint("OB: ObpInsertDirectoryEntry - invalid context %p %ld\n", LookupContext->Object, (ULONG)LookupContext->DirectoryLocked);
        DbgBreakPoint();
        return FALSE;
    }
#endif // DBG
    ...
    NewDirectoryEntry = (POBJECT_DIRECTORY_ENTRY)ExAllocatePoolWithTag(PagedPool, sizeof(OBJECT_DIRECTORY_ENTRY), 'iDbO');
    ...
    *HeadDirectoryEntry = NewDirectoryEntry;
    ...
    NameInfo->Directory = Directory;//  Point the object header back to the directory we just inserted it into.
...
/*
Routine Description:
    This routine deletes the most recently found directory entry from the specified directory object.
    It will only succeed after a successful ObpLookupDirectoryEntry call.
Arguments:
...
*/
{
    ...
    if (!Directory) {//  Make sure we have a directory and that it has a found entry
        return(FALSE);
    }
...
/*
Routine Description:
    This function sets the process' device map to the device map associated with the process token's LUID.
Return Values:
    STATUS_NO_TOKEN - process does not have a primary token
    STATUS_OBJECT_PATH_INVALID - could not obtain the device map associated with the process token's LUID
...
*/
{
    ...
    if (!RtlEqualLuid(&userLuid, &SystemAuthenticationId)) {
        ...
        Status = SeGetLogonIdDeviceMap(&userLuid, &pDevMap);// Get a handle to the Device Map for the user's LUID
        if (NT_SUCCESS(Status)) {
            DeviceMap = pDevMap;
        }
    } else {
        DeviceMap = ObSystemDeviceMap;// Process is Local_System, so use the System's device map
    }

    if (DeviceMap != NULL) {// set the process' device map
        ObpLockDeviceMap();
    ...
    if (DerefDeviceMap != NULL) {// If the process already had a device map, then deref it now
...
/*
Routine Description:
    This function obtains the correct device map associated with the caller.
    If LUID device maps are enabled, then we obtain the LUID's device map.

    We use the existing process device map field as a cache for the process token's LUID device map.

    If LUID device maps are disabled, then use the device map associated with the process
Return Values:
...
*/
{
    ...
    if (LUIDDeviceMapsEnabled == TRUE) {
        ...
        if (PS_IS_THREAD_IMPERSONATING(Thread)) {
            ...
            if (NT_SUCCESS(Status)) {
                ...
                // Verify the caller is not Local_System by comparing the LUID with Local_System's LUID
                if (!RtlEqualLuid(&userLuid, &SystemAuthenticationId)) {
                    PDEVICE_MAP pDevMap;

                    Status = SeGetLogonIdDeviceMap(&userLuid, &pDevMap);// Get a handle to the Device Map for the LUID
                    if (NT_SUCCESS(Status)) {
                    ...
                    LocalSystemRequest = TRUE;// Local_System will use the system's device map
                }
            }
        }
    }
    ...
    if (DeviceMap == NULL) {
        ...
        if (LocalSystemRequest == TRUE) {
            DeviceMap = ObSystemDeviceMap;// Use the system's device map
        } else {
            DeviceMap = PsGetCurrentProcess()->DeviceMap;// Use the device map from the process
        }
...
Routine Description:
    ...
    It will also create a new object specified by InsertObject.
Arguments:
    RootDirectoryHandle - Optionally supplies the directory being searched.  If not supplied then this routine searches the root directory
    ...
    ParseContext - Optionally supplies a parse context that is blindly passed to the parse callback routines
    SecurityQos - Optionally supplies a pointer to the passed Security Quality of Service parameter that is blindly passed to the parse callback routines
    InsertObject - Optionally supplies the object we think will be found.
        This is used if the caller did not give a root directory handle and the object name is "\" and the root object directory hasn't been created yet.
        In other cases where we wind up creating a new directory entry this is the object inserted.
    AccessState - Current access state, describing already granted access types, the privileges used to get them, and any access types yet to be granted.
        The access masks may not contain any generic access types.
    DirectoryLocked - Receives an indication if this routine has returned with the input directory locked
    FoundObject - Receives a pointer to the object body if found
Return Value:
    An appropriate status value.
    N.B. If the status returned is SUCCESS the caller has the responsibility to release the lookup context
*/
{
    ...
    //  Initialize our output variables to say we haven't lock or found anything but we were successful at it
    ObpInitializeLookupContext(LookupContext);
    *FoundObject = NULL;
    Status = STATUS_SUCCESS;
    Object = NULL;

    if (ObpCaseInsensitive) {//  If the global flag says that we need to perform a case-insensitive check
        if ((ObjectType == NULL) || (ObjectType->TypeInfo.CaseInsensitive)) {
            Attributes |= OBJ_CASE_INSENSITIVE;//  we'll force the OBJ_CASE_INSENSITIVE flag in attributes at lookup
        }
    }
    ...
    if (ARGUMENT_PRESENT(RootDirectoryHandle)) {
        ...
        if (ObjectHeader->Type != ObpDirectoryObjectType) {//  Now make sure that we do not have the directory of the object types
            //  We have an object directory that is not the object type directory.
            //  So now if it doesn't have a parse routine then there is nothing we can
            if (ObjectHeader->Type->TypeInfo.ParseProcedure == NULL) {
                ...
                //  The following loop cycles cycles through the various parse routine to we could encounter trying to resolve this name through symbolic links.
                while (TRUE) {
#if DBG
                    KIRQL SaveIrql;
#endif
                    ...
                    //  Invoke the callback routine to parse the remaining object name
                    ...
                    //  If the status was not to do a reparse and the lookup was not successful then we found nothing so we dereference the directory and return the status to our caller.
                    //  If the object we got back was null then we'll tell our caller that we couldn't find the name.
                    //  Lastly if we did not get a reparse and we were successful and the object is not null then everything gets nicely returned to our caller
                    if ((Status != STATUS_REPARSE) && (Status != STATUS_REPARSE_OBJECT)) {
                        ...
                        *FoundObject = Object;
                        ...
                        //  We got a reparse and we actually have a new name to go to we if we haven't exhausted our reparse attempts yet then just continue to the top of this loop.
                    } else if (--MaxReparse) {
                        continue;
                    } else {//  We got a reparse and we've exhausted our times through the loop so we'll return what we found.
                        ObDereferenceObject(RootDirectory);
                        *FoundObject = Object;

                        //  At this point we were failing in stress by returning to the caller with a success status but a null object pointer.
                        if (Object == NULL) {
                            Status = STATUS_OBJECT_NAME_NOT_FOUND;
                        }
            ...
            //  At this point the caller has given us the directory of object types.
            //  If the caller didn't specify a name then we'll return a pointer to the root object directory.
    ...
    } else {//  Otherwise the caller did not specify a directory to search so we'll default to the object root directory
        RootDirectory = ObpRootDirectoryObject;
        ...
        //  Which means that the caller really just wants to lookup the root directory.
        if (ObjectName->Length == sizeof(OBJ_NAME_PATH_SEPARATOR)) {
            //  If there is not a root directory yet.
            //  Then we really can't return it, however if the caller specified an insert object that is the one we'll reference and return to our caller
            if (!RootDirectory) {
                if (InsertObject) {
                ...
                //  At this point the caller did not specify a root directory, the name is "\"
                //  and the root object directory exists so we'll simply return the real root directory object
            } else {
                Status = ObReferenceObjectByPointer(RootDirectory, 0, ObjectType, AccessMode);
                if (NT_SUCCESS(Status)) {
            ...
            //  At this pointer the caller did not specify a root directory, and the name is more than just a "\"

            //  Now if the lookup is case insensitive,
            //  and the name buffer is a legitimate pointer (meaning that is it quadword aligned), and there is a dos device map for the process.
            //  Then we'll handle the situation here. First get the device map and make sure it doesn't go away while we're using it.
        } else {
        ParseFromRoot:
            if (DeviceMap != NULL) {
            ...
            if (!((ULONG_PTR)(ObjectName->Buffer) & (sizeof(ULONGLONG) - 1))) {
                //  Check if the object name is actually equal to the global dos devices short name prefix "\??\"
                if ((ObjectName->Length >= ObpDosDevicesShortName.Length) &&
                    ...
                    if ((DeviceMap = ObpReferenceDeviceMap()) != NULL) {
                        if (DeviceMap->DosDevicesDirectory != NULL) {
                            //  The user gave us the dos short name prefix so we'll look down the directory, and start the search at the dos device directory
                    ...
                    //  The name is not equal to "\??\" but check if it is equal to "\??"
                    ...
                    //  The user specified "\??" so we return to dos devices directory to our caller
                    if ((DeviceMap = ObpReferenceDeviceMap()) != NULL) {
                        if (DeviceMap->DosDevicesDirectory != NULL) {
                            Status = ObReferenceObjectByPointer(DeviceMap->DosDevicesDirectory, 0, ObjectType, AccessMode);
                            if (NT_SUCCESS(Status)) {
                                *FoundObject = DeviceMap->DosDevicesDirectory;
                            }

                            ObfDereferenceDeviceMap(DeviceMap);//  Dereference the Device Map
                            return(Status);
                        }
                    }
                }
    ...
    //  the user specified a directory that is not the object type directory and got reparsed back to the root directory

    //  the user specified the object type directory and gave us a name to actually look up

    //  the user did not specify a search directory (default to root object directory)
    //  and if the name did start off with the dos device prefix we've munged outselves back to it to the dos device directory for the process
    ...
    quickStart:
        ...
        while (TRUE) {
            ...
            //  If the remaining name for the object starts with a "\" then just gobble up the "\"
            ...
            //  The following piece of code will calculate the first component of the remaining name.
            //  If there is not a remaining component then the object name is malformed
            ...
            if (Directory == NULL) {//  Now we have the first component name to lookup so we'll look the directory is necessary
                Directory = RootDirectory;
            }

            //  Now if the caller does not have traverse privilege and there is a parent directory then we must check if the user has traverse access to the directory.
            //  Our local Reparse variable should be false at this point so we'll drop out of both loops
            if ((AccessCheckMode != KernelMode) && !(AccessState->Flags & TOKEN_HAS_TRAVERSE_PRIVILEGE)) {
                // If the privilege is to be checked, reference this directory which will be the ParentDirectory for the next iteration if any.
            ...
            //  If the object already exists in this directory, find it, else return NULL.
            if (RemainingName.Length == 0) {
                //  If we are searching the last name, take an additional reference to the directory.
                //  We need this to either insert a new object into this directory, or to check for traverse access if an insert object is not specified.
                //  If not referenced the directory could go away since ObpLookupDirectoryEntry releases the existing reference
                ...
                if (InsertObject != NULL) {
                    //  We lock the context exclusively before the lookup if we have an object to insert.
                    //  An insertion is likely to occur after this lookup if it fails,
                    //  so we need to protect this directory to be changed until the ObpInsertDirectoryEntry call
                    ObpLockLookupContext(LookupContext, Directory);
                }
            }

            Object = ObpLookupDirectoryEntry(Directory, &ComponentName, Attributes, InsertObject == NULL ? TRUE : FALSE, LookupContext);
            if (!Object) {
                //  We didn't find the object.  If there is some remaining
                //  name left (meaning the component name is a directory in path we trying to break) or
                //  the caller didn't specify an insert object then we then we'll break out here with an error status
                if (RemainingName.Length != 0) {
                ...
                //  Check that the caller has the access to the directory to either create a subdirectory (in the object type directory) or
                //  to create an object of the given component name.
                //  If the call fails then we'll break out of here with the status value set
                if (!ObCheckCreateObjectAccess(Directory,
                                               ObjectType == ObpDirectoryObjectType ? DIRECTORY_CREATE_SUBDIRECTORY : DIRECTORY_CREATE_OBJECT,
                                               AccessState,
                                               &ComponentName,
                                               FALSE,
                                               AccessCheckMode,
                ...
                if ((Directory->SessionId != OBJ_INVALID_SESSION_ID) &&
                    ((ObjectHeader->Type == MmSectionObjectType) || (ObjectHeader->Type == ObpSymbolicLinkObjectType))) {
                    //  This directory is restricted to session.
                    //  Check whether we are in the same session with the directory, and if we have privilege to access it otherwise
                    if ((Directory->SessionId != PsGetCurrentProcessSessionId()) &&
                        !SeSinglePrivilegeCheck(SeCreateGlobalPrivilege, AccessCheckMode) &&
                        !ObpIsUnsecureName(&ComponentName, ((Attributes & OBJ_CASE_INSENSITIVE) ? TRUE : FALSE))) {
                        Status = STATUS_ACCESS_DENIED;
                        break;
                    }
                }

                //  The object does not exist in the directory and we are allowed to create one.
                //  So allocate space for the name and insert the name into the directory
                ...
                //  We have an insert object so now get its name info, because we are going to change its name and insert it into the directory
            ...
            //  At this point we've found the component name within the directory.
            //  So we'll now grab the components object header, and get its parse routine

        ReparseObject:
            ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
            ParseProcedure = ObjectHeader->Type->TypeInfo.ParseProcedure;

            //  Now if there is a parse routine for the type and we are not inserting a new object
            //  or the parse routine is for symbolic links then we'll actually call the parse routine
            if (ParseProcedure && (!InsertObject || (ParseProcedure == ObpParseSymbolicLink))) {
                ...
                //  This will keep the object from going away with the directory unlocked
                ObpIncrPointerCount(ObjectHeader);
                Directory = NULL;
                ObpReleaseLookupContext(LookupContext);

                //  Release the extra references on the directory and parent directory if they were taken since we may go through a reparse,
                //  and we do not need these anymore.
                if (ReferencedDirectory != NULL) {
                ...
                if ((Status == STATUS_REPARSE) || (Status == STATUS_REPARSE_OBJECT)) {//  Check if we have some reparsing to do
                    if (--MaxReparse) {//  See if we've reparsed too many times already and if so we'll fail the request
                        Reparse = TRUE;//  Tell the outer loop to continue looping
                        if ((Status == STATUS_REPARSE_OBJECT) || (*(ObjectName->Buffer) == OBJ_NAME_PATH_SEPARATOR)) {//  Check if we have a reparse object or the name starts with a "\"
                            //  If the user specified a start directory then remove this information because we're taking a reparse point to someplace else
                            if (ARGUMENT_PRESENT(RootDirectoryHandle)) {
                                ObDereferenceObject(RootDirectory);
                                RootDirectoryHandle = NULL;
                            }
                            ...
                            //  Now if this is a reparse object
                            // (means we have encountered a symbolic link that has already been snapped so we have an object and remaining name that need to be examined)
                            //  and we didn't find an object from the parse routine object break out of both loops.
                            if (Status == STATUS_REPARSE_OBJECT) {
                                Reparse = FALSE;
                                if (Object == NULL) {
                                    Status = STATUS_OBJECT_NAME_NOT_FOUND;
                                } else {
                                    //  At this point we have a reparse object so we'll look the directory down and parse the new object
                                    goto ReparseObject;
                                }
                            } else {
                                // At this point Status must be equal to STATUS_REPARSE because [(Status equals(STATUS_REPARSE_OBJECT or STATUS_REPARSE)) && (Status != STATUS_REPARSE_OBJECT)]
                                ReparsedSymbolicLink = TRUE;
                                goto ParseFromRoot;
                            }

                            //  We did not have a reparse object and the name does not start with a "\".
                            //  Meaning we got back STATUS_REPASE, so now check if the directory is the root object directory
                            //  and if so then we didn't the name otherwise we'll drop out of the inner loop and reparse true to get back to outer loop
                        } else if (RootDirectory == ObpRootDirectoryObject) {
                            Object = NULL;
                            Status = STATUS_OBJECT_NAME_NOT_FOUND;
                            Reparse = FALSE;
                        }
                    } else {//  We return object not found if we've exhausted the MaxReparse time
                        Object = NULL;
                        Status = STATUS_OBJECT_NAME_NOT_FOUND;
                    }
                } else if (!NT_SUCCESS(Status)) {//We are not reparsing and if we did not get success then the object is null and we'll break out of our loops
                    Object = NULL;

                    //  We are not reparsing and we got back success but check
                    //  if the object is null because that means we really didn't find the object, and then break out of our loops

                } else if (Object == NULL) {//  If the object is not null then we've been successful and prosperous so break out with the object set.
                    Status = STATUS_OBJECT_NAME_NOT_FOUND;
                }

                break;
            } else {
                //  At this point we do not have a parse routine or if there is a parse routine it is not for symbolic links
                //  or there may not be a specified insert object
                if (RemainingName.Length == 0) {//  Check to see if we have exhausted the remaining name
                    //  Check if the caller specified an object to insert.
                    //  If specified then we'll break out of our loops with the object that we've found
                    if (!InsertObject) {
                        //  The user did not specify an insert object so we're opening an existing object.
                        //  Make sure we have traverse access to the container directory.
                        if ((AccessCheckMode != KernelMode) && !(AccessState->Flags & TOKEN_HAS_TRAVERSE_PRIVILEGE)) {
                            if (!ObpCheckTraverseAccess(Directory, DIRECTORY_TRAVERSE, AccessState, FALSE, AccessCheckMode, &Status)) {
                                Object = NULL;
                                break;
                            }
                        }
                    ...
                    //  There is some name remaining names to process if the directory we're looking at is the directory of object types
                    //  and set ourselves up to parse it all over again.
                    if (ObjectHeader->Type == ObpDirectoryObjectType) {
                        //  Setup for the next iteration, dereference the parent directory from the previous iteration
                        if (ReferencedParentDirectory != NULL) {
                            ObDereferenceObject(ReferencedParentDirectory);
                        }
                    ...
                    } else {//  Otherwise there has been a mismatch so we'll set our error status and break out of the loops
    ...
    //  We still need the directory to be locked if a new object is inserted into that directory,
    //  until we finish the initialization (i.e SD, handle, ...).
    //  Note the object is visible now and could be accessed via name. We need to hold the directory lock until we are done.
    if (!NT_SUCCESS(Status)) {
        ObpReleaseLookupContext(LookupContext);
    }

    if (DeviceMap != NULL) {//  If the device map has been referenced then dereference it
        ObfDereferenceDeviceMap(DeviceMap);
    }

    if (ReferencedDirectory != NULL) {//  Dereference the directory and parent directory that might have been referenced during the course of the lookup.
        ObDereferenceObject(ReferencedDirectory);
    }
    ...
    //  At this point we've parsed the object name as much as possible going through symbolic links as necessary.
    //  So now set the output object pointer, and if we really did not find an object then we might need to modify the error status.
    //  If the status was reparse or some success status then translate it to name not found.
    ...
    //  If the caller gave us a root directory to search (and we didn't zero out this value) then free up our reference
    if (ARGUMENT_PRESENT(RootDirectoryHandle)) {
        ObDereferenceObject(RootDirectory);
        RootDirectoryHandle = NULL;
    }
...
*/
{
    ...
    PreviousMode = KeGetPreviousMode();//  Get previous processor mode and probe output argument if necessary.

    //  The object is being changed to permanent, check if the caller has the appropriate privilege.
    ...
    //  Note that the object should still have a name and directory entry because its handle count is not zero
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
...
/*
    ...
    Assumptions:
        The parent directory is assumed to be exclusively locked
        (so that other threads waiting to reference the name will have first to grab the directory lock).
*/
{
    POBJECT_HEADER_NAME_INFO NameInfo;
    ...
    do {
        //  If this is not the only reference to the object then we cannot lock the name N.B. The caller needs also to have a reference to this name
        if (((References & (~OBP_NAME_KERNEL_PROTECTED)) != 2) || (NameInfo->Directory == NULL)) {
            return NULL;
        }

        NewReferences = InterlockedCompareExchange((PLONG)&NameInfo->QueryReferences, OBP_NAME_LOCKED | References, References);
        if (NewReferences == References) {// If the exchange compare completed ok then we did a reference so return true.
            return NameInfo;
        }

        References = NewReferences;// We failed because somebody else got in and changed the refence count on us. Use the new value to prime the exchange again.
    } while (TRUE);
...
/*
    Routine Description:
        This function removes the directory entry from a directory.
        Note that before calling this function a lookup is necessary.
    Arguments:
...
/*
    Routine Description:
        This function releases the object references (added during the lookup) but still keeps the directory locked.
        N.B. The caller must retain at least one reference to the object and to the name to make sure the deletion does not happen under the lock.
    Arguments:
...
NTSTATUS ObSwapObjectNames(IN HANDLE DirectoryHandle, IN HANDLE Handle1, IN HANDLE Handle2, IN ULONG Flags)
/*
    Routine Description:
...
#define INVALID_HASH_INDEX 0xFFFFFFFF
    ...
    Status = ObReferenceObjectByHandle(Handle1, DELETE, (POBJECT_TYPE)NULL, PreviousMode, &Object1, &HandleInformation);
    if (!NT_SUCCESS(Status)) {
        goto exit;
    }

    Status = ObReferenceObjectByHandle(Handle2, DELETE, (POBJECT_TYPE)NULL, PreviousMode, &Object2, &HandleInformation);
    if (!NT_SUCCESS(Status)) {
        goto exit;
    }
    ...
    if ((NameInfo1 == NULL) || (NameInfo2 == NULL)) {
        Status = STATUS_OBJECT_NAME_INVALID;
        goto exit;
    }

    if ((Directory != NameInfo1->Directory) || (Directory != NameInfo2->Directory)) {
        Status = STATUS_OBJECT_NAME_INVALID;
        goto exit;
    }
    ...
    if (Object1 != ObpLookupDirectoryEntry(Directory, &NameInfo1->Name, 0, FALSE, &LookupContext)) {
        Status = STATUS_OBJECT_NAME_NOT_FOUND;//  The object is no longer in directory
        goto exit;
    }
    ...
    if (Object2 != ObpLookupDirectoryEntry(Directory, &NameInfo2->Name, 0, FALSE, &LookupContext)) {
        Status = STATUS_OBJECT_NAME_NOT_FOUND;//  The object is no longer in directory
        goto exit;
    }
...
exit:
    ...
    if (NT_SUCCESS(Status) && (ObjectHeader1 != NULL) && (ObjectHeader2 != NULL)) {
        ...
        if ((ObjectHeader1->Flags ^ ObjectHeader2->Flags) & OB_FLAG_PERMANENT_OBJECT) {
            //  Both objects are required to have the same object type. We lock them all before swapping the flags
