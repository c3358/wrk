...
BOOLEAN WmipMceDelivery(IN PVOID Reserved, IN KERNEL_MCE_DELIVERY_OPERATION Operation, IN PVOID Argument2);
...
// Each type of MCE has a control structure that is used to determine whether to poll or wait for an interrupt to determine when to query for the logs.
// This is needed since we can get a callback from the HAL at high IRQL to inform us that a MCE log is available.
// Additionally Ke Timer used for polling will calls us at DPC level.
// So in the case of an interrupt we will queue a DPC.
// Within the DPC routine we will queue a work item so that we can get back to passive level and be able to call the hal to get the logs (Can only call hal at passive).
// The DPC and work item routines are common so a MCEQUERYINFO struct is passed around so that it can operate on the correct log type.
// Note that this implies that there may be multiple work items querying the hal for different log types at the same time.
// In addition this struct also contains useful log related information including the maximum log size (as reported by the HAL),
// the token that must be passed to the HAL when querying for the logs and the HAL InfoClass to use when querying for the logs.

// PollFrequency keeps track of the number of seconds before initiating a query.
// If it is 0 (HAL_CPE_DISABLED / HAL_CMC_DISABLED) then no polling occurs and if it is -1 (HAL_CPE_INTERRUPTS_BASED / HAL_CMC_INTERRUPTS_BASED) then no polling occurs either.
// There is only one work item active for each log type and this is enforced via ItemsOutstanding in that only whenever it transitions from 0 to 1 is the work item queued.
...
// MCA information obtained at boot and holds the MCA that caused the system to bugcheck on the previous boot
...
// Configurable parameters for managing thresholds for eventlog suppression and recovery action for corrected MCE

// Interval within which multiple identical errors will be reported as a single error to the system eventlog.
// Can be configured under HKLM\System\CurrentControlSet\Control\WMI\CoalesceCorrectedErrorInterval
// A value of 0 will cause no coalesce of identical errors
ULONG WmipCoalesceCorrectedErrorInterval = 5000;

// Number of single bit ecc errors that can occur in the same page before it is attempted to map out the page.
...
// more than this limit then the oldest ones are recycled. Can be configured under :
...
// Counter of maximum eventlog entries generated by any source. Can be configured under:
// HKLM\System\CurrentControlSet\Control\WMI\MaxCorrectedEventlogs
ULONG WmipCorrectedEventlogCounter = 20;

// Check if WBEM is already running and if not check if we've already kicked off the timer that will wait for wbem to start

#define WmipIsWbemRunning() ((WmipIsWbemRunningFlag == WBEM_IS_RUNNING) ? \
                                                       TRUE : \
                                                       FALSE)
void WmipInsertQueueMCEDpc(PMCEQUERYINFO QueryInfo);


NTSTATUS WmipWriteToEventlog(NTSTATUS ErrorCode, NTSTATUS FinalStatus)
{
...
NTSTATUS WmipFireOffWmiEvent(LPGUID Guid, ULONG DataSize, PVOID DataPtr)
{
    PVOID Ptr;
    PWNODE_SINGLE_INSTANCE Wnode;
    PWCHAR Wptr;
    ...
    Wnode = ExAllocatePoolWithTag(NonPagedPool,
                                  ...
                                  WmipMCAPoolTag);
    if (Wnode != NULL) {
        Wnode->WnodeHeader.BufferSize = sizeof(WNODE_SINGLE_INSTANCE) + sizeof(USHORT) + RoundedDataSize + sizeof(MCA_EVENT_INSTANCE_NAME);
        ...
        *Wptr++ = sizeof(MCA_EVENT_INSTANCE_NAME);
        RtlCopyMemory(Wptr, MCA_EVENT_INSTANCE_NAME, sizeof(MCA_EVENT_INSTANCE_NAME));
...
Arguments:
    ...
    McaCmcEvent is the MCA, CMC or CPE data payload to put into the event
...
*/
{
    ...
    Size = McaCmcSize + FIELD_OFFSET(MSMCAInfo_RawCMCEvent, Records) + FIELD_OFFSET(MSMCAInfo_Entry, Data);
...
/*
Routine Description:
    Utility routine that will query the hal for a log and then if one is returned successfully then will fire the appropriate WMI events
Arguments:
    QueryInfo is a pointer to the MCEQUERYINFO for the type of log that needs to be queried.
*/
{
    PWNODE_SINGLE_INSTANCE Wnode;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        Status2 = IoWMIWriteEvent(Wnode);// Fire the log off as a WMI event
        if (!NT_SUCCESS(Status2)) {
            ExFreePool(Wnode);// IoWMIWriteEvent will free the wnode back to pool, but not if it fails
        }
...
*/
{
    ...
    if (QueryInfo->WorkerInProgress == 0) {
        ...
        WmipLeaveSMCritSection();
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_MCA_LEVEL, "WMI: WmipMceWorkerRoutine %p in progress\n", QueryInfo));
        return;
    }

    // Check to see if access has already been disabled
    if (QueryInfo->PollFrequency != HAL_MCE_DISABLED) {
        // We get all of the records by calling into the hal and querying for the logs until the hal returns an error or we've retrieved 256 records.
        ...
        // Reset counter back to 0, but check if any errors had occured while we were processing.
        // If so we go back and make sure they are handled.
        // Note that this could cause a new worker thread to be created while we are still processing these, 
        // but that is ok since we only allow one worker thread to run at one time.
        WmipEnterSMCritSection();
...
)
{
    ...
    if (QueryInfo->PollFrequency == HAL_MCE_INTERRUPTS_BASED) {
        // HAL has instructed us to switch into polled mode and has informed us of the new polling interval.
        QueryInfo->PollFrequency = PollingInterval;

        li.QuadPart = -1 * (QueryInfo->PollFrequency * 1000000000);
        KeSetTimerEx(&QueryInfo->PollingTimer, li, QueryInfo->PollFrequency * 1000, &QueryInfo->PollingDpc);

        WmipWriteToEventlog(QueryInfo->SwitchToPollErrorCode, STATUS_SUCCESS);// Make a note in the eventlog that this has occured.
        WmipFireOffWmiEvent(&QueryInfo->SwitchToPollGuid, 0, NULL);// Inform any WMI consumers that the switch has occured
    } else {
        WmipMceDispatchRoutine(QueryInfo);// Our timer fired so we need to poll
    }
}
...
/*
Routine Description:
    This routine is called by the HAL when a CMC or CPE occurs. It is called at high irql
Arguments:
    ...
    Parameter for operation specified.
        For CmcSwitchToPolledMode and CpeSwitchToPolledMode, Parameter specifies the number of seconds to between polling.
Return Value:
...
*/
{
    ...
    case McaAvailable:
        ...
        // If we are ready to handle the logs and we are dealing with thse logs  on an interrupt basis,
        // then go ahead and queue a DPC to handle processing the log
        if ((WmipMCEState == MCE_STATE_RUNNING) && (QueryInfo->PollFrequency == HAL_MCE_INTERRUPTS_BASED)) {
            KeAcquireSpinLockAtDpcLevel(&QueryInfo->DpcLock);
            KeInsertQueueDpc(&QueryInfo->DeliveryDpc, QueryInfo, NULL);
...
/*
Routine Description:
    This routine is called by the HAL when a situation occurs between the HAL and SAL interface.
...
*/
{
    ...
    if ((MceOperation == KERNEL_MCE_OPERATION_GET_STATE_INFO) &&
        ...
        if (QueryInfo != NULL) {
            // If CMC or CPE are interrupt based then queue up a new DPC for performing the query. 
            // If polling based then there are no worries, we just wait for the next polling interval.
            if ((WmipMCEState == MCE_STATE_RUNNING) && (QueryInfo->PollFrequency == HAL_MCE_INTERRUPTS_BASED)) {
                KeAcquireSpinLockAtDpcLevel(&QueryInfo->DpcLock);
                KeInsertQueueDpc(&QueryInfo->DeliveryDpc, QueryInfo, NULL);
                KeReleaseSpinLockFromDpcLevel(&QueryInfo->DpcLock);
...
/*
Routine Description:
    This routine will flush out any of the previous MCA logs and then hang onto them for WMI to report.
*/
{
    NTSTATUS status;
    PERROR_LOGRECORD log;
    ...
    if (!IsListEmpty(&list)) {
        // We have collected a set of previous logs, so we need to build the buffer containing the aggregation of those logs.
        // The buffer will correspond to the entire MOF structure for the MSMCAInfo_RawMCAData class
        WmipRawMCA = (PMSMCAInfo_RawMCAData)ExAllocatePoolWithTag(PagedPool, sizeNeeded, WmipMCAPoolTag);
        ...
        while (!IsListEmpty(&list)) {
            ...
            if (WmipRawMCA != NULL) {
                ...
                // Note that we assume there will only be 1 record within the event
...
void WmipInsertQueueMCEDpc(PMCEQUERYINFO QueryInfo)
{
    ...
    KeAcquireSpinLock(&QueryInfo->DpcLock, &OldIrql);
    KeInsertQueueDpc(&QueryInfo->DeliveryDpc, QueryInfo, NULL);
    KeReleaseSpinLock(&QueryInfo->DpcLock, OldIrql);
}
...
/*
Routine Description:
    This routine will register a kernel MCA and CMC handler with the hal
*/
{
    ...
    if (Phase == 0) {
        // Phase 0 initialization is done before device drivers are loaded so that the kernel can register its kernel error handler before any driver gets a chance to do so.

        // Validate registry values
        if (WmipCorrectedEventlogCounter == 0) {
            // set corrected eventlog counter to -1 to indicate that no eventlog suppression should occur
        ...
        if ((NT_SUCCESS(Status)) && (ReturnSize >= sizeof(HAL_ERROR_INFO))) {
            ...
            KeInitializeDpc(&WmipMcaQueryInfo.DeliveryDpc, WmipMceDpcRoutine, NULL);
            KeInitializeDpc(&WmipMcaQueryInfo.PollingDpc, WmipPollingDpcRoutine, &WmipMcaQueryInfo);
            ...
            KeInitializeDpc(&WmipCmcQueryInfo.DeliveryDpc, WmipMceDpcRoutine, NULL);
            KeInitializeDpc(&WmipCmcQueryInfo.PollingDpc, WmipPollingDpcRoutine, &WmipCmcQueryInfo);
            ExInitializeWorkItem(&WmipCmcQueryInfo.WorkItem, WmipMceWorkerRoutine, &WmipCmcQueryInfo);
            KeInitializeTimerEx(&WmipCmcQueryInfo.PollingTimer, NotificationTimer);
            ...
            KeInitializeDpc(&WmipCpeQueryInfo.DeliveryDpc, WmipMceDpcRoutine, NULL);
            KeInitializeDpc(&WmipCpeQueryInfo.PollingDpc, WmipPollingDpcRoutine, &WmipCpeQueryInfo);
            ExInitializeWorkItem(&WmipCpeQueryInfo.WorkItem, WmipMceWorkerRoutine, &WmipCpeQueryInfo);
            KeInitializeTimerEx(&WmipCpeQueryInfo.PollingTimer, NotificationTimer);
        ...
        // Phase 1 initialization is done after all of the boot drivers have loaded and have had a chance to register for WMI event
        // notifications. At this point it is safe to go ahead and send wmi events for MCA, CMC, CPE, etc

        // If there were any MCA logs generated prior to boot then get them out of the HAL and process them.
        // Do this before starting any polling since the SAL likes to have the previous MCA records removed before being polled for CPE and CMC
        HalErrorInfo.Version = HAL_ERROR_INFO_VERSION;
        Status = HalQuerySystemInformation(HalErrorInformation, sizeof(HAL_ERROR_INFO), &HalErrorInfo, &ReturnSize);
        if ((NT_SUCCESS(Status)) && (ReturnSize >= sizeof(HAL_ERROR_INFO))) {
            if (HalErrorInfo.McaPreviousEventsCount != 0) {
                // We need to flush out any previous MCA logs and then make them available via WMI
        ...
        if ((WmipCmcQueryInfo.PollFrequency != HAL_CMC_DISABLED) &&
            ...
            // CMC is interrupt based so we need to kick off an attempt to read any CMC that had previously occured
        ...
        if ((WmipCpeQueryInfo.PollFrequency != HAL_CPE_DISABLED) &&
            ...
            // Cpe is interrupt based so we need to kick off an attempt to read any Cpe that had previously occured
            WmipInsertQueueMCEDpc(&WmipCpeQueryInfo);
        }

        WmipMCEState = MCE_STATE_RUNNING;// Flag that we are now able to start firing events
        Status = STATUS_SUCCESS;
    } else {
...
Arguments:
    InfoClass is the HalInformationClass that specifies the log information to retrieve
    Token is the HAL token for the log type
    *Wnode returns a pointer to a WNODE_EVENT_ITEM containing the log information if Wnode is not NULL
    *Mca returns a pointer to the log read from the hal. It may point into the memory pointed to by *Wnode
...
*/
{
    ...
    // Allocate a buffer to store the log reported from the hal.
    // Note that this must be in non paged pool as per the HAL.
    ...
    if (Ptr != NULL) {
        ...
        *(PVOID*)Log = Token;
        ...
        if (Status == STATUS_BUFFER_TOO_SMALL) {
            ...
            // This is ok as we'll handle this situation by reallocating and trying again
            ExFreePool(Log);
        ...
        if (NT_SUCCESS(Status)) {   // We successfully read the data from the hal so build up output buffers.
            if (Wnode != NULL) {
                // Caller requested buffer returned within a WNODE, so build up the wnode around the log data
        ...
        if ((!NT_SUCCESS(Status)) && (Ptr != NULL)) {
            ExFreePool(Ptr);// If the function failed, but we have an allocated buffer then clean it up
        }
    } else {
        Status = STATUS_INSUFFICIENT_RESOURCES;
    }
...
void WmipGenerateMCAEventlog(PUCHAR ErrorLog, ULONG ErrorLogSize, BOOLEAN IsFatal)
{
    ...
    // Right now the largest is MSMCAEvent_MemoryError.
    // If this changes then this code should be updated
    ...
    if (Buffer != NULL) {
        ...
        Wnode->DataBlockOffset = ((sizeof(WNODE_SINGLE_INSTANCE) + (sizeof(USHORT) + sizeof(MCA_EVENT_INSTANCE_NAME)) + 7) & ~7);
        ...
        *w++ = sizeof(MCA_EVENT_INSTANCE_NAME);
        ...
        // Construct the error event using the data in the error log we retrieved from the HAL.
#if defined(_AMD64_) || defined(i386)
        if (IsFatal) {
#endif
            Status = WmipConstructMCAErrorEvent((PVOID)ErrorLog, ErrorLogSize, Wnode, Header, &RawPtr, &IsFatal);
#if defined(_AMD64_) || defined(i386)
        }
#endif
        ...
        if ((!IsFatal) && (Header->LogToEventlog == 1)) {
            if (WmipCorrectedEventlogCounter != 0) {
                // Since this is a corrected error that is getting logged to the eventlog we need to account for it
                if ((WmipCorrectedEventlogCounter != 0xffffffff) && (--WmipCorrectedEventlogCounter == 0)) {
                    WmipWriteToEventlog(MCA_INFO_NO_MORE_CORRECTED_ERROR_LOGS, STATUS_SUCCESS);
                }
            } else {
                // We have exceeded the limit of corrected errors that
                // we are allowed to write into the eventlog, so we just suppress it
                Header->LogToEventlog = 0;
            }
        }

        // Now go and fire off the event
        if ((WmipDisableMCAPopups == 0) && (Header->LogToEventlog != 0)) {
            IoRaiseInformationalHardError(STATUS_MCA_OCCURED, NULL, NULL);
        }

        if ((Header->LogToEventlog == 1) || (WmipIsWbemRunning())) {
            // Only fire off a WMI event if we want to log to the eventlog or WBEM is up and running
...
NTSTATUS WmipWriteMCAEventLogEvent(PUCHAR Event)
{
    ...
    if (WmipIsWbemRunning() || WmipCheckIsWbemRunning()) {
        ...
        // WBEM is not currently running and so startup a timer that will keep polling it
        if (WmipIsWbemRunningFlag == WBEM_STATUS_UNKNOWN) {
            // No one has kicked off the waiting process for wbem so we do that here.
            // Note we need to maintain the critical section to guard against another thread that might be
            // trying to startup the waiting process as well.
            // Note that if the setup fails we want to stay in the unknown state
            // so that the next time an event is fired we can retry waiting for wbem
...
NTSTATUS WmipSetupWaitForWbem(void)
{
    ...
    // Initialize a kernel time to fire periodically so we can check if WBEM has started or not
...
BOOLEAN WmipCheckIsWbemRunning(void)
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (Status == STATUS_SUCCESS) {
            ...
            // We've determined that WBEM is running so now lets see if another thread has made that determination as well.
            // If not then we can flush the MCA event queue and set the flag that WBEM is running
            WmipEnterSMCritSection();
