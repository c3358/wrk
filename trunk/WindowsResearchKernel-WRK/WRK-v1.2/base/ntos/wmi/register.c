...
// WmipRegWorkItemCount starts at 1 so that all drivers who register before phase 1 of WMI initialization won't kick off the reg work
// item. In phase 1 we decrement the count and if it is not zero then we kick it off since it is now same to send the drivers reg info irps
...
void WmipInitializeRegistration(ULONG Phase)
{
    ...
    if (Phase == 0) {
        ...
        // Kick off work item that will send reg irps to all of the drivers that have registered. We are sure there is at least
        // one device that needs this since there is the internal wmi data device
...
/*
Routine Description:
    Allocate a REGENTRY structure. If successful the RegEntry returns with a ref count of 1.
    NOTE: This routine assumes that the registration critical section is held
Arguments:
    DeviceObject is the value to fill in the DeviceObject field of the RegEntry.
Return Value:
...
*/
{
    ...
    if (RegEntry != NULL) {
        // Initialize the RegEntry. Note that the regentry will start out with a ref count of 1
...
Return Value:
    On checked builds returns TRUE if last ref count was removed from REGENTRY and it was placed back on free list.
*/
{
    BOOLEAN Freed;
    ULONG ProviderId;
...
/*
Routine Description:
    Remember information about a new device being registered and then go and get the registration information.
Arguments:
    DeviceObject is a pointer to the device object being registered or the callback entry point

    RegistrationFlag is either WMIREG_FLAG_CALLBACK if DeviceObject is
        a callback pointer, or WMIREG_FLAG_TRACE_PROVIDER is DeviceObject can also generate event traces.
Return Value:
    NT status code
*/
{
    ...
    if (RegEntry == NULL) {
        if (!IsCallback) {
            // Data providers that register with a device object and not a callback have their device object referenced so that it will
            // stick around while WMI needs it. This reference is removed when the device unregisters with WMI and all WMI irps are completed.
            ...
            // No reference counting is done for callbacks. It is the data provider's responsibility to synchronize any unloading and
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (RegEntry != NULL) {
                // We need to take an extra ref count before releasing the critical section.  One class of drivers
                // (kmixer) will register and unregister multiple times in different threads and this can lead to a race where
                ...
                if (IsCallback) {
                    ...
                    // We need to send the registration irp from within a work item and not in the context of this
                    // routine. This is because some drivers will not process irps while in the StartDevice/AddDevice context, so we'd get deadlock
...
*/
{
    ...
    RegEntry = WmipFindRegEntryByDevice(DeviceObject, FALSE);
    if (RegEntry != NULL) {
        // Mark the regentry as invalid so that no more irps are sent to the device and the event will set when the last irp completes.
        Flags = InterlockedExchange(&RegEntry->Flags, (REGENTRY_FLAG_RUNDOWN | REGENTRY_FLAG_NOT_ACCEPTING_IRPS));

        // Once the regentry is marked as RUNDOWN then it will not be found later so it is safe to release the lock.
        WmipLeaveSMCritSection();
        WmipUnreferenceRegEntry(RegEntry);

        // Now if there are any outstanding irps for the device then we need to wait here until they complete.
...
Arguments:
    DeviceObject is a pointer to the device object that wants to update its information
Return Value:
    NT status code
*/
{
...
{
    // This routine assumes that any synchronization mechanism has been taken. This routine can be called at dispatch level
    PREGENTRY RegEntry;
...
/*
Routine Description:
    This routine will find a RegEntry that corresponds to the DeviceObject passed.
Arguments:
    DeviceObject is the device object that is the key for the RegEntry to find
    ReferenceIrp is TRUE then the Irp refcount will be incremented if a RegEntry is found for the device
Return Value:
...
/*
Routine Description:
    This routine will decrement one from the active irp count for the regentry.
    If the active irp count reaches 0 and the flag is set that the device is waiting to be unloaded then the unload event is signaled
...
*/
{
    ...
    if ((RegEntry->Flags & REGENTRY_FLAG_RUNDOWN) && (IrpCount == 0)) {
        // If this is the last outstanding irp for the device and the device is trying to unregister then set the event to
...
/*
Routine Description:
    This routine will return the device instance name that is associated with the PDO passed.
Arguments:
...
Routine Description:
    ...
    data provider and convert any PDO instance name references to a static instance name reference.
Arguments:
...
*/
{
    ...
    do {
        for (i = 0; i < WmiRegInfo->GuidCount; i++) {
            ...
            if (WmiRegGuid->Flags & WMIREG_FLAG_INSTANCE_PDO) {
                // This instance name must be translated from PDO to device instance name
                if (FreeSpacePtr == NULL) {
                    // Determine where any free space is in output buffer by figuring out where the last WmiRegInfo ends
                ...
                MaxInstanceNames = MaxInstanceNames < WmiRegGuid->InstanceCount ? WmiRegGuid->InstanceCount : MaxInstanceNames;
    ...
    // If we can do automatic support for device information guid so add registration for this guid to the registration information
    if (AllowPnPIdMap && (PnPIdPDO != NULL)) {
        Status = WmipPDOToDeviceInstanceName(PDO, &InstancePath);
        if (NT_SUCCESS(Status)) {
            // Pad so that new WmiRegInfo starts on 8 byte boundary and adjust free buffer size
            ...
            if ((WmiRegInfoTooSmall) || (ExtraRoom > FreeSpaceLeft)) {
                ...
                if (RegEntry->PDO == NULL) {
                    // If we haven't already established a PDO for this data provider then remember PDO and count of
                    // instance names for this device so we can get device properties
...
/*
Routine Description:
    This routine will loop through all WMIREGINFO passed and verify the sizes and offsets are not out of bounds of the buffer. It will register
    the guids for each one. Note that if at least one of the WMIREGINFOs does register successfully then STATUS_SUCCESS is returned, but all
...
Arguments:
    ...
    RequestObject is the request object associated with the UM provider. If this is NULL then the registration is for a driver
...
*/
{
    ...
    do {
        ...
        // buffer. Note that WmipAddDataSource verifies that the instance names within each guid is within bounds.
        ...
        // Now call the core to parse the registration info and build the data structures
        if (Update) {
            // CONSIDER: UM Code had held the critsect over all WMIREGINFOs linked together
...
NTSTATUS WmipRegisterOrUpdateDS(PREGENTRY RegEntry, BOOLEAN Update)
{
    ...
    do {
        ...
        if (Buffer != NULL) {
            // First send IRP_MN_REGINFO_EX to see if we've got a sophisticated client
            Status = WmipSendWmiIrp(IRP_MN_REGINFO_EX, RegEntry->ProviderId, UlongToPtr(Update ? WMIUPDATE : WMIREGISTER), SizeNeeded, Buffer, &IoStatus);
            if ((!NT_SUCCESS(Status)) && (Status != STATUS_BUFFER_TOO_SMALL)) {
                // If IRP_MN_REGINFO_EX doesn't work then try our old reliable IRP_MN_REGINFO
                Status = WmipSendWmiIrp(IRP_MN_REGINFO,
                                        RegEntry->ProviderId,
                                        UlongToPtr(Update ? WMIUPDATE : WMIREGISTER),
                                        SizeNeeded,
                                        Buffer,
                                        &IoStatus);
            }
    ...
    // If registration info irp was successful then go process registration information
    if (NT_SUCCESS(Status)) {
...
NTSTATUS WmipQueueRegWork(REGOPERATION RegOperation, PREGENTRY RegEntry)
{
    ...
    if (RegWork != NULL) {
        // Take an extra ref on the RegEntry which will be freed after the work item is processed
        WmipReferenceRegEntry(RegEntry);
...
#if defined(_WIN64)
ULONG IoWMIDeviceObjectToProviderId(__in PDEVICE_OBJECT DeviceObject)
/*
Routine Description:
    This routine will lookup the provider id that corresponds with the device object passed.
Return Value:
