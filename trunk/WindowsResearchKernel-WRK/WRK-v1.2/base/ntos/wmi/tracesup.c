...
Abstract:
    This is the source file that implements the private routines of the performance event tracing and logging facility.
    These routines work on manipulating the LoggerContext table and synchronization across event tracing sessions.
*/
...
NTSTATUS WmipLookupLoggerIdByName(IN PUNICODE_STRING Name, OUT PULONG LoggerId);
PWMI_LOGGER_CONTEXT WmipInitContext();
NTSTATUS WmipAllocateTraceBufferPool(IN PWMI_LOGGER_CONTEXT LoggerContext);
NTSTATUS WmipFreeTraceBufferPool(IN PWMI_LOGGER_CONTEXT LoggerContext);
...
NTSTATUS WmipStartLogger(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
    It is called by WmipIoControl in wmi.c, with IOCTL_WMI_START_LOGGER to start up an instance of the logger.
    It basically creates and initializes the logger instance context, and starts up a system thread for the logger (WmipLogger()).
    If the user has requested to turn on kernel tracing, it will also lock in the necessary routines after the logger has started.
    NOTE: A special instance (KERNEL_LOGGER) is reserved exclusively for logging kernel tracing.

    To turn on KERNEL_LOGGER, LoggerInfo->Wnode.Guid should be set to SystemTraceControlGuid, and sufficient space must be provided in LoggerInfo->LoggerName.
    To turn on other loggers, simply provide a name in LoggerName. The logger id will be returned.
Arguments:
    LoggerInfo     a pointer to the structure for the logger's control and status information
Return Value:
...
*/
{
    ...
    if (EnableFlags & EVENT_TRACE_FLAG_EXTENSION) {
        ...
        if ((FlagExt->Length == 0) || (FlagExt->Offset == 0) || (LoggerInfo->Wnode.BufferSize < FlagExt->Offset)) {
            return STATUS_INVALID_PARAMETER;
        }
    ...
    if (LogFileMode & EVENT_TRACE_USE_KBYTES_FOR_SIZE) {
        // Default Minimum Buffers and default Buffer Size are computed later in the Context initialization after Context allocation.
        // To avoid having to allocate memory for this error checking, we compute local parameters.
        ...
        LocalMaxBuffers = (ULONG)(WmiMaximumPoolInBytes / TRACE_MAXIMUM_NP_POOL_USAGE / LocalBufferSize);
        if (LoggerInfo->MaximumBuffers != 0 && LoggerInfo->MaximumBuffers < LocalMaxBuffers) {
            LocalMaxBuffers = LoggerInfo->MaximumBuffers;
        }
        if (LocalMinBuffers < LoggerInfo->MinimumBuffers && LoggerInfo->MinimumBuffers < LocalMaxBuffers) {
    ...
    if (EnableKernel) {
        ...
        if ((LogFileMode & EVENT_TRACE_REAL_TIME_MODE) && !(LogFileMode & EVENT_TRACE_DELAY_OPEN_FILE_MODE)) {
    ...
    Status = WmipCheckGuidAccess(&InstanceGuid, DesiredAccess, EtwpDefaultTraceSecurityDescriptor);
    if (!NT_SUCCESS(Status)) {
#if DBG
        RefCount =
#endif
    ...
    if (LoggerInfo->LogFileHandle != NULL) {
        ...
        Status = ObReferenceObjectByHandle(LoggerInfo->LogFileHandle, 0L, IoFileObjectType, RequestorMode, (PVOID *)&FileObject, &handleInformation);
        if (NT_SUCCESS(Status)) {
            TraceDebug((1, "WmipStartLogger: Referenced FDO %X %X %d\n", FileObject, LoggerInfo->LogFileHandle, ((POBJECT_HEADER)FileObject)->PointerCount));
            if (RequestorMode != KernelMode) {
                grantedAccess = handleInformation.GrantedAccess;
                if (!SeComputeGrantedAccesses(grantedAccess, FILE_WRITE_DATA)) {
                    TraceDebug((1, "WmipStartLogger: Deref FDO %x %d\n", FileObject, ((POBJECT_HEADER)FileObject)->PointerCount));
    ...
    if (LoggerInfo->AgeLimit >= 0) { // minimum is 15 minutes
        LoggerContext->BufferAgeLimit.QuadPart = max(DEFAULT_AGE_LIMIT, LoggerInfo->AgeLimit) * WmiOneSecond.QuadPart * 60;
    ...
    if (EnableKernel) {
        // Always reserve space for FileTable to allow file trace to be turn on/off dynamically
    ...
    try {
        ...
        if (LoggerContext->KernelTraceOn) {
            if (RequestorMode != KernelMode) {
                ProbeForWrite(LoggerInfo->LoggerName.Buffer, LoggerContext->LoggerName.Length + sizeof(WCHAR), sizeof(UCHAR));
            }
    ...
    if (LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_NEWFILE) {
        ...
        Status = WmipGenerateFileName(&LoggerContext->LogFilePattern, &LoggerContext->FileCounter, &LoggerContext->LogFileName);
        if (!NT_SUCCESS(Status)) {
#if DBG
            RefCount =
#endif
    ...
    if (NT_SUCCESS(Status)) {
        ...
        // later to impersonate the user, which we will do if we cannot access the file as SYSTEM.  This
        ...
        Status = SeCreateClientSecurity(CONTAINING_RECORD(KeGetCurrentThread(), ETHREAD, Tcb), &ServiceQos, FALSE, &LoggerContext->ClientSecurityContext);
    }
    if (!NT_SUCCESS(Status)) {
#if DBG
        RefCount =
    ...
    // Note that buffer allocation routine will also set NumberOfBuffers and MaximumBuffers.
    ...
    LoggerInfo->AgeLimit = (ULONG)(LoggerContext->BufferAgeLimit.QuadPart / WmiOneSecond.QuadPart / 60);
    LoggerInfo->BufferSize = LoggerContext->BufferSize / 1024;
    ...
    // Set the clock function and start up the logger as a system thread
    if (NT_SUCCESS(Status)) {
        ...
        // At this point, the clock type should be set and we take a reference timesamp, which should be the earliest timestamp
        // for the logger.  The order is this way since SystemTime is typically cheaper to obtain.
        ...
        if (NT_SUCCESS(Status)) {  // if SystemThread is started
            ...
            do {
                ...
                if (Status == STATUS_UNSUCCESSFUL) {
                    // If the logger thread hasn't responded yet, replace the LoggerStatus to STATUS_CANCELLED so that if it ever comes up, it will terminate.
                    // We clean up the logger cointext if that is the case.
                    Status = InterlockedCompareExchange(&LoggerContext->LoggerStatus, STATUS_CANCELLED, STATUS_UNSUCCESSFUL);
                } else {
                    Status = LoggerContext->LoggerStatus;
                }
            } while (Status == STATUS_PENDING);

            // If the logger is up and running properly, we can now turn on event tracing if kernel tracing is requested
            if (NT_SUCCESS(Status)) {
                LoggerContext->LoggerMode = LogFileMode;

                // After we release this mutex, any other thread can acquire the valid logger context and call the shutdown path for
                // this logger.  Until this, no other thread can call the enable or disable code for this logger.
                ...
                if (IsGlobalForKernel) {
                    ...
                    if (!NT_SUCCESS(Status)) {
                        ...
                        // Just do WmipStopLoggerInstance and let Logger Thread handle all the cleanup work.
                    ...
                    ASSERT(LoggerContext->KernelTraceOn);
                    ASSERT(EnableKernel);
                    PerfLogInTransition = InterlockedCompareExchange(&LoggerContext->PerfLogInTransition, PERF_LOG_START_TRANSITION, PERF_LOG_NO_TRANSITION);
                ...
                // The logger thread did not notify the starting thread, or the logger thread started OK, but something failed during
                // file creation. The logger thread will clean up the logger context. Just return.
                WmipDereferenceLogger(LoggerId);
                return Status;
            }
        }
...
NTSTATUS WmipQueryLogger(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo, IN PWMI_LOGGER_CONTEXT LoggerContext)
/*
Routine Description:
...
*/
{
    ...
    if (NoContext) {
        ...
        Status = WmipCheckGuidAccess(&LoggerContext->InstanceGuid, DesiredAccess, EtwpDefaultTraceSecurityDescriptor);
        if (!NT_SUCCESS(Status)) {
            InterlockedDecrement(&LoggerContext->MutexCount);
    ...
    try {
        if (LoggerContext->LogFileName.Length > 0 && LoggerInfo->LogFileName.MaximumLength > 0) {
            if (RequestorMode != KernelMode) {
                ProbeForWrite(LoggerInfo->LogFileName.Buffer, LoggerContext->LogFileName.Length + sizeof(WCHAR), sizeof(UCHAR));
            }
...
NTSTATUS WmipStopLoggerInstance(IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    LONG               LoggerOn;
    ...
    if (LoggerContext->KernelTraceOn) {
        ...
        // or stopping by other thread. PerfLogInTransition flag in the logger context should only be used here and UpdateTrace and NO WHERE ELSE.
        LONG PerfLogInTransition = InterlockedCompareExchange(&LoggerContext->PerfLogInTransition, PERF_LOG_STOP_TRANSITION, PERF_LOG_NO_TRANSITION);
    ...
    if (LoggerContext->KernelTraceOn) {
        ...
        // NOTE: If we start sharing callouts, the argument should be LoggerContext->EnableFlags
...
#endif
            WmipDereferenceLogger(LoggerId);
        TraceDebug((2, "WmipVerifyLoggerInfo(%s): Status=%X %d %d->%d\n", Caller, STATUS_WMI_INSTANCE_NOT_FOUND, LoggerId, RefCount + 1, RefCount));
...
PVOID WmipExtendBase(IN PWMI_LOGGER_CONTEXT Base, IN ULONG Size)
{
    ...
    // It rounds the size to a ULONGLONG alignment and expects EndPageMarker to already be aligned.
    PVOID Space = NULL;
    ULONG SpaceLeft;
...
VOID WmipFreeLoggerContext(IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    ...
    // One was set by WmipStartLogger() in the beginning, and the second must be done normally by WmiStopTrace() or anybody who
...
*/
{
    ...
    LoggerContext->EndPageMarker = (PUCHAR)LoggerContext + ALIGN_TO_POWER2(sizeof(WMI_LOGGER_CONTEXT), DEFAULT_TRACE_ALIGNMENT);
...
*/
{
    ...
    // Allocate the pointers the each buffer here by sharing the same page with LoggerContext context pointer
    ...
    SysMax_Buffers = (ULONG)(WmiMaximumPoolInBytes / TRACE_MAXIMUM_NP_POOL_USAGE / LoggerContext->BufferSize);
    ...
    LoggerContext->MaximumBuffers = max(LoggerContext->MaximumBuffers, NumberProcessors + DEFAULT_BUFFERS + DEFAULT_MAX_BUFFERS);

    LoggerContext->MinimumBuffers = max(LoggerContext->MinimumBuffers, SysMin_Buffers);

    // Ensure each parameter is in range of SysMin and SysMax
    LoggerContext->MaximumBuffers = max(LoggerContext->MaximumBuffers, SysMin_Buffers);
    LoggerContext->MaximumBuffers = min(LoggerContext->MaximumBuffers, SysMax_Buffers);
    LoggerContext->MinimumBuffers = max(LoggerContext->MinimumBuffers, SysMin_Buffers);
    LoggerContext->MinimumBuffers = min(LoggerContext->MinimumBuffers, SysMax_Buffers);

    // In case the MaximumBuffers and MinimumBuffers got reversed pick the larger value

    if (LoggerContext->MinimumBuffers > LoggerContext->MaximumBuffers) {
        LoggerContext->MaximumBuffers = LoggerContext->MinimumBuffers;
    }

    // NOTE: We do not return anything if we reset MaximumBuffers or MinimumBuffers provided by the caller.
    ...
    LoggerContext->ProcessorBuffers = (PWMI_BUFFER_HEADER *)WmipExtendBase(LoggerContext, sizeof(PWMI_BUFFER_HEADER)*NumberProcessors);
    if (LoggerContext->ProcessorBuffers == NULL) {
        WmipFreeTraceBufferPool(LoggerContext);
        return STATUS_NO_MEMORY;
    }

    // NOTE: We already know that we have allocated > number of processors buffers
...
NTSTATUS WmipShutdown(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
...
NTSTATUS WmipFlushLogger(IN OUT PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG Wait)
{
    LARGE_INTEGER TimeOut = {(ULONG)(-20 * 1000 * 1000 * 10), -1};
    NTSTATUS Status;
...
NTSTATUS FASTCALL WmipNotifyLogger(IN PWMI_LOGGER_CONTEXT LoggerContext)
...
NTSTATUS WmipNtDllLoggerInfo(IN OUT PWMINTDLLLOGGERINFO Buffer)
{
    ...
    if (Buffer->IsGet) {
        ...
        // Check to see if heap/critsec controller has access to proper Guids.
        Status = WmipCheckGuidAccess(&Guid, DesiredAccess, EtwpDefaultTraceSecurityDescriptor);
        if (!NT_SUCCESS(Status)) {
            if (GuidEntry) {
                WmipUnreferenceGE(GuidEntry);
            }
        ...
        if (SizeNeeded) {
            if (GuidEntry == NULL) {
                ...
                if (GuidEntry) {
                    // Initialize the guid entry and keep the ref count from creation.
                    // When tracelog enables we take a ref count and when it disables we release it
                    GuidEntry->Guid = Guid;
            ...
            if (GuidEntry) {
                ...
                //  The Extra Refcount taken at logger start is released by calling Disable trace.
                Status = WmipEnableDisableTrace(IOCTL_WMI_ENABLE_DISABLE_TRACELOG, &TraceEnableInfo);
                WmipUnreferenceGE(GuidEntry);
            }
        }
...
/*
Routine Description:
    This routine is called to validate the requested clock type in the LoggerInfo.
    If the requested type can not be handled, we will override to a type that this system will support.
